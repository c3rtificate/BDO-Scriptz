...
function PaGlobal_GlobalKeyBinder:Init()
  ...
  self:Register(UIMode.eUIMode_Arsha_Replay, self.Process_UIMode_Arsha_Replay)
  self:Register(UIMode.eUIMode_GroupCamera, self.Process_UIMode_GroupCamera)
  self:Register(UIMode.eUIMode_ProductionModify, self.Process_UIMode_ProductionModify)
  self:Register(UIMode.eUIMode_SiegeInstallMode, self.Process_UIMode_SiegeInstallMode)
  self:Register(UIMode.eUIMode_Tutorial, self.Process_UIMode_Tutorial)
  self:Register(UIMode.eUIMode_DrawEvent, self.Process_UIMode_DrawEvent)
  self:Register(UIMode.eUIMode_NpcSimpleDialog, self.Process_UIMode_NpcSimpleDialog)
  self:Register(UIMode.eUIMode_EnterAbyssOne, self.Process_UIMode_EnterAbyssOne)
  self:Register(UIMode.eUIMode_Sequence, self.Process_UIMode_SequenceMode)
  self:Register(UIMode.eUIMode_SequencePlayerControl, self.Process_UIMode_SequencePlayerControlMode)
  self:Register(UIMode.eUIMode_SequencePlayerControl, self.Process_UIMode_SkillkeyBinder, checkCustomCondition_CommonWindow)
  self:Register(UIMode.eUIMode_FieldWalkingCamera, self.Process_UIMode_FieldWalkingCameraMode)
  self:Register(UIMode.eUIMode_ElfWar, self.Process_UIMode_Elfwar)
  self:Register(UIMode.eUIMode_ElfWarMenu, self.Process_UIMode_ElfwarMenu)
  self:Register(UIMode.eUIMode_QuickMenuSetting, self.Process_UIMode_QuickMenu)
  self:Register(UIMode.eUIMode_EnterMorningLandPartyBoss, self.Process_UIMode_EnterMorningLandPartyBoss)
end
...
function PaGlobal_GlobalKeyBinder:Update(deltaTime)
  ...
  if currentUIMode ~= UIMode.eUIMode_KeyCustom_ButtonShortcuts and currentUIMode ~= UIMode.eUIMode_DeadMessage and (Panel_Window_Equip_CharacterTag_ItemCopy ~= nil and Panel_Window_Equip_CharacterTag_ItemCopy:GetShow() == false or _ContentsGroup_DuelCharacterCopyEquip == false) and ToClient_IsRemoteInstallMode() == false then
...
function GlobalKeyBinder_CheckCustomKeyPressed(uiInputType)
  ...
  return (PaGlobal_GlobalKeyBinder.curProcessedInputType == uiInputType or mouseKeyTable[uiInputType]) and not GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_MENU) and not isPhotoMode() and not isFirstFieldEnterMode()
end
...
function FromClient_AbyssHipassPostUpdate()
  SetUIMode(Defines.UIMode.eUIMode_Default)
end
registerEvent("FromClient_luaLoadComplete", "FromClient_GlobalKeyBinderLoad")
registerEvent("FromClient_AbyssHipassPostUpdate", "FromClient_AbyssHipassPostUpdate")
