...
function PaGlobal_Extraction_Costume_All:controlAll_Init()
  ...
  self._ui.btn_selectAll = UI.getChildControl(rightArea, "Button_SelectAll")
  ...
  if false == ToClient_IsContentsGroupOpen("74") then
    self._ui.btn_cron:SetShow(false)
    self._ui.btn_valks:SetSize(self._ui.btn_skipAni:GetSizeX(), self._ui.btn_skipAni:GetSizeY())
  end
end
...
function PaGlobal_Extraction_Costume_All:registEventHandler()
  ...
  self._ui.btn_selectAll:addInputEvent("Mouse_LUp", "HandleEventLUp_Extraction_Costume_All_Select()")
...
function PaGlobal_Extraction_Costume_All:updateExtractionList()
  ...
  local currentIndex = PaGlobal_Extraction_Costume_All._ui.list2_extractableItem:getCurrenttoIndex()
  ...
  PaGlobal_Extraction_Costume_All._extractionCnt = 0
  ToClient_UpdateExtractionCostumeDataList()
  local listCount = ToClient_GetExtractionCostumeDataListCount()
  for idx = 1, listCount do
    local costumeData = ToClient_GetExtractionCostumeDataByIndex(idx - 1)
    if costumeData ~= nil then
      local whereType = costumeData:getFromWhereType()
      local slotNo = costumeData:getSlotNo()
      local isEquipSlot = whereType == CppEnums.ItemWhereType.eEquip
      if isEquipSlot == true then
        ...
        if itemWrapper ~= nil then
          ...
          PaGlobal_Extraction_Costume_All._equipNo[PaGlobal_Extraction_Costume_All._costumeCnt] = slotNo
          ...
          if PaGlobal_Extraction_Costume_All._itemInfo.isExtractionEquip[PaGlobal_Extraction_Costume_All._costumeCnt] == true then
            HandleEventLUp_Extraction_Costume_All_SelectListItem(PaGlobal_Extraction_Costume_All._costumeCnt, true)
          end
        end
      else
  ...
  PaGlobal_Extraction_Costume_All._ui.list2_extractableItem:moveIndex(currentIndex)
...
function PaGlobal_Extraction_Costume_All:filerCostumeItem(itemWrapper)
  ...
  local isLockItem = ToClient_isLockItem(itemWrapper:getItemNoRaw())
  if isLockItem == true then
    return true
  end
  return false
end
