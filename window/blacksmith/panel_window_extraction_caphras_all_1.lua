...
function PaGlobal_Extraction_Caphras_All:controlAll_Init()
  ...
  self._ui.stc_MoneyArea = UI.getChildControl(leftMiddleArea, "Static_MyMoneyArea")
  ...
  local slot = {}
  SlotItem.new(slot, "Caphras_Extraction", 0, itemlistSlot, self._slotConfig)
  slot:createChild()
  slot.empty = true
  slot:clearItem()
  local itemText = UI.getChildControl(stc_itemSlotBg, "StaticText_ItemName")
  local text_equipment = UI.getChildControl(btn_list, "StaticText_EquipItem")
...
function PaGlobal_Extraction_Caphras_All:openUiSetting()
  ...
  PaGlobal_Extraction_Caphras_All:updateExtractionList()
  PaGlobal_Extraction_Caphras_All._moneyWhereType = CppEnums.ItemWhereType.eMoneyInventory
