...
function PaGlobal_Extraction_Crystal_All:controlAll_Init()
  ...
  local slot = {}
  SlotItem.new(slot, "Crystal_Extraction", 0, itemlistSlot, self._slotConfig)
  slot:createChild()
  slot.empty = true
  slot:clearItem()
...
function PaGlobal_Extraction_Crystal_All:filterInvenCrystalItem(slotNo, itemWrapper, whereType)
  ...
  if true == itemWrapper:getStaticStatus():get():isArtifact() then
    return true
  end
...
function PaGlobal_Extraction_Crystal_All:setCrystalSlotIcon(itemWrapper)
  ...
  for ii = 1, maxCount do
    ...
    if nil ~= itemStaticWrapper then
      ...
      PAGlobalFunc_SetItemTextColor(self._ui.stc_crystalInfo[ii].txt_itemName, itemStaticWrapper)
