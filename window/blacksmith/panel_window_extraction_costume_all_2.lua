...
function HandleEventLUp_Extraction_Costume_All_ListControlCreate(control, key)
  ...
  local cashIcon = UI.getChildControl(itemSlotBg, "Static_PmarkArea")
  local extractCount = UI.getChildControl(itemSlotBg, "StaticText_ExtractionCount")
  ...
  if nil == PaGlobal_Extraction_Costume_All._equipNo[key_32] then
    ...
    PAGlobalFunc_SetItemTextColor(itemText, itemKey:getStaticStatus())
    itemWrapper = itemKey
  ...
  if btn_Equip:GetShow() == true then
    itemText:SetTextMode(__eTextMode_LimitText)
  else
    itemText:SetTextMode(__eTextMode_None)
  end
  itemText:SetText(itemName)
  itemIcon:ChangeTextureInfoName("icon/" .. itemIconPath)
  if itemWrapper ~= nil then
    local isCheckValks = PaGlobal_Extraction_Costume_All._ui.btn_valks:IsCheck()
    local isCheckCron = PaGlobal_Extraction_Costume_All._ui.btn_cron:IsCheck()
    local spanSizeY = 0
    extractCount:SetTextMode(__eTextMode_LimitText)
    if isCheckValks == true then
      itemText:SetSpanSize(itemText:GetSpanSize().x, spanSizeY - 10)
      extractCount:SetShow(true)
      local count = Int64toInt32(itemWrapper:getStaticStatus():getExtractionCount_s64())
      extractCount:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_ITEMTOOLTIP_BALKS_EXTRACTION", "balksCount", count))
    elseif isCheckCron == true then
      itemText:SetSpanSize(itemText:GetSpanSize().x, spanSizeY - 10)
      extractCount:SetShow(true)
      local count = Int64toInt32(itemWrapper:getStaticStatus():getCronCount_s64())
      extractCount:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_ITEMTOOLTIP_CRONS_EXTRACTION", "cronsCount", count))
    else
      itemText:SetSpanSize(itemText:GetSpanSize().x, spanSizeY)
      extractCount:SetShow(false)
    end
    local isCash = itemWrapper:isCash()
    cashIcon:SetShow(isCash)
  end
  if itemText:IsLimitText() == true or extractCount:IsLimitText() == true then
    btn_item:addInputEvent("Mouse_On", "HandleEventOn_Extraction_Costume_All_ShowToolTipForLimitText(" .. key_32 .. ",true)")
    btn_item:addInputEvent("Mouse_Out", "HandleEventOn_Extraction_Costume_All_ShowToolTipForLimitText(" .. key_32 .. ",false)")
  end
...
function HandleEventLUp_Extraction_Costume_All_Select()
  for index = 1, PaGlobal_Extraction_Costume_All._costumeCnt do
    if false == PaGlobal_Extraction_Costume_All._itemInfo.isExtractionEquip[index] or nil == PaGlobal_Extraction_Costume_All._itemInfo.isExtractionEquip[index] then
      local whereType = PaGlobal_Extraction_Costume_All._itemInfo.whereType[index]
      if whereType ~= CppEnums.ItemWhereType.eEquip then
        HandleEventLUp_Extraction_Costume_All_CostumeItem(index, true)
      end
    end
  end
end
...
function HandleEventLUp_Extraction_Costume_All_SelectListItem(key, isForce)
  HandleEventLUp_Extraction_Costume_All_CostumeItem(key, isForce)
  PaGlobal_Extraction_Costume_All_ShowDetailKeyGuide(true, key)
end
function HandleEventLUp_Extraction_Costume_All_CostumeItem(key, isForce)
  ...
  if PaGlobal_Extraction_Costume_All._itemInfo.isExtractionEquip[key] == false or PaGlobal_Extraction_Costume_All._itemInfo.isExtractionEquip[key] == nil or isForce ~= nil and isForce == true then
...
function HandleEventOn_Extraction_Costume_All_ShowToolTipForLimitText(key_32, isShow)
  if nil == key_32 or false == isShow then
    TooltipSimple_Hide()
    return
  end
  local uiControl = PaGlobal_Extraction_Costume_All._ui.list2_extractableItem:GetContentByKey(toInt64(0, key_32))
  if nil == uiControl then
    return
  end
  local btn_item = UI.getChildControl(uiControl, "CheckButton_ExtractableItem")
  ...
  local itemText = UI.getChildControl(itemSlotBg, "StaticText_ItemName")
  local extractCount = UI.getChildControl(itemSlotBg, "StaticText_ExtractionCount")
  TooltipSimple_Show(btn_item, itemText:GetText(), extractCount:GetText())
end
