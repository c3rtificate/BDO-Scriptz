...
function PaGlobal_Extraction_System_All:controlAll_Init()
  ...
  leftBottomDescArea:SetShow(false)
  ...
  local slot = {}
  SlotItem.new(slot, "System_Extraction", 0, itemlistSlot, self._slotConfig)
  slot:createChild()
  slot.empty = true
  slot:clearItem()
...
function PaGlobal_Extraction_System_All:updateExtractionList()
  ...
  for slotNo = 0, invenMaxSize - 1 do
    sortSlotNo = selfPlayer:getRealInventorySlotNoNew(0, slotNo)
    local itemWrapper = getInventoryItemByType(whereType, sortSlotNo)
    if false == PaGlobal_Extraction_System_All:fillter_ForSacrificeTarget(sortSlotNo, itemWrapper, whereType) then
      ...
      PaGlobal_Extraction_System_All._itemInfo.slotNo[PaGlobal_Extraction_System_All._systemCnt] = sortSlotNo
...
function PaGlobal_Extraction_System_All:setExtractionIcon(itemWrapper, slotNo)
  ...
  if nil ~= self._ui._icon_resultItem.originalForDuelIcon then
    ...
    if true == isOriginalItem then
      self._ui._icon_resultItem.originalForDuelIcon:SetShow(true)
    else
      self._ui._icon_resultItem.originalForDuelIcon:SetShow(false)
    end
  end
  if false == self._isConsole then
...
function PaGlobal_Extraction_System_All:setNotice()
  ...
  local panelSizeY = titleBarSizeY + imageAreaSizeY + btnAreaSizeY + padding * 2
