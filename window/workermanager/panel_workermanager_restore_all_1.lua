...
function PaGlobal_WorkerManagerRestore_All:ControlAll_Init()
  ...
  self._ui.stc_auto = UI.getChildControl(Panel_Window_WorkerManagerRestore_All, "Static_Auto")
  self._ui.check_workerAutoRecovery = UI.getChildControl(self._ui.stc_auto, "CheckButton_Template_Import")
  self._ui.stc_autoTitle = UI.getChildControl(self._ui.stc_auto, "StaticText_AutoTitle")
  self._ui.stc_autoDesc = UI.getChildControl(self._ui.stc_auto, "StaticText_AutoDesc")
  ...
  self._ui.stc_autoDesc:SetTextMode(__eTextMode_AutoWrap)
  self._ui.stc_autoDesc:SetText(self._ui.stc_autoDesc:GetText())
  ...
  for resIdx = 0, self._restoreItemMaxCount - 1 do
    ...
    tempItemSlot.restorePoint = UI.createAndCopyBasePropertyControl(self._ui.stc_itemIconBg, "StaticText_Item_Restore_Value", tempItemSlot.slotBG, "workerManager_restorePoint_" .. resIdx)
  ...
  self._stcAutoOriginSizeY = self._ui.stc_auto:GetSizeY()
end
function PaGlobal_WorkerManagerRestore_All:ControlPc_Init()
  ...
  self._ui_pc.btn_confirm:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_WORLD_MAP_TOWN_WORKER_RESTORE"))
end
...
function PaGlobal_WorkerManagerRestore_All:ControlSetShow()
  ...
  if true == self._isPadSnap then
    ...
    self._ui_pc.btn_cancle:SetShow(_ContentsGroup_FamilyInventory)
...
function PaGlobal_WorkerManagerRestore_All:prepareOpen(listCount, workerArray)
  ...
  self._ui.stc_auto:SetShow(false)
  if _ContentsGroup_WorkerAutoRecovery == true then
    self._ui.stc_auto:SetShow(true)
    PaGlobal_WorkerManagerRestore_All._ui.check_workerAutoRecovery:SetCheck(getAutoRecoveryWorker())
  end
...
function PaGlobal_WorkerManagerRestore_All:registEventHandler()
  ...
  self._ui_pc.btn_cancle:addInputEvent("Mouse_LUp", "PaGlobalFunc_WorkerManagerRestore_All_FamilyInventoryOpen()")
  self._ui_pc.btn_confirm:addInputEvent("Mouse_LUp", "HandleEventLUp_WorkerManagerRestore_All_Confirm()")
  self._ui.check_workerAutoRecovery:addInputEvent("Mouse_LUp", "HandleEventLPress_WorkerManagerRestore_All_WorkerAutoRecovery()")
...
function PaGlobal_WorkerManagerRestore_All:resize()
  ...
  local afterListBgSizeY = self._ui.stc_itemlistBg:GetSizeY()
  if nil == self._workerArray then
    ...
    if _ContentsGroup_WorkerAutoRecovery == true then
      self._ui.stc_auto:SetShow(true)
      stcAutoSize = self._ui.stc_auto:GetSizeY()
    end
    afterPanelSizeY = afterPanelSizeY + stcAutoSize + self._ui.txt_workerCnt:GetTextSizeY() + self._ui.txt_restorePoint:GetTextSizeY() + self._ui.txt_possiblePoint:GetTextSizeY()
    if true == self._isPadSnap then
      self._ui.stc_itemlistBg:SetSize(self._ui.stc_itemlistBg:GetSizeX(), afterListBgSizeY)
      Panel_Window_WorkerManagerRestore_All:SetSize(Panel_Window_WorkerManagerRestore_All:GetSizeX(), afterPanelSizeY)
    else
...
function PaGlobal_WorkerManagerRestore_All:updateItemSlot()
  ...
  for itemIdx = 0, self._restoreItemMaxCount - 1 do
    ...
    self._restoreItemSlot[itemIdx].slot:clearItem()
  end
  ...
  local isHaveFamilyInventory = false
  local itemCnt = 0
  for itemIdx = self._sliderStartIdx, self._restoreItemHasCount - 1 do
    ...
    itemCnt = itemCnt + 1
    if recoveryItem._itemWhereType == CppEnums.ItemWhereType.eFamilyInventory then
      isHaveFamilyInventory = true
    end
  end
  if isHaveFamilyInventory == false then
    local orginalText = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_WORKERMANAGER_AUTO_RECOVERY_CHK")
    local warningText = PAGetString(Defines.StringSheet_GAME, "LUA_WORKER_AUTORESTORE_ALERT")
    self._ui.stc_autoDesc:SetText(orginalText .. "\n" .. warningText)
    self._ui.stc_auto:SetSize(self._ui.stc_auto:GetSizeX(), self._ui.stc_autoTitle:GetTextSizeY() + self._ui.stc_autoDesc:GetTextSizeY())
  else
    self._ui.stc_autoDesc:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_WORKERMANAGER_AUTO_RECOVERY_CHK"))
    self._ui.stc_auto:SetSize(self._ui.stc_auto:GetSizeX(), self._stcAutoOriginSizeY)
  end
  local gabY = 30
  local afterPanelSizeY = self._ui.stc_titleBg:GetSizeY() + self._ui.txt_itemlistGuide:GetTextSizeY() + self._restoreItemSlot[0].slotBG:GetSizeY() + self._ui.stc_slider:GetSizeY() + gabY
  local afterListBgSizeY = self._ui.stc_itemlistBg:GetSizeY()
...
function PaGlobal_WorkerManagerRestore_All:selectItem(itemIdx)
  ...
  if nil ~= self._workerArray then
    if true == self._isPadSnap then
      ...
      Panel_Window_WorkerManagerRestore_All:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandleEventLUp_WorkerManagerRestore_All_Confirm()")
    end
  ...
  for idx = 1, self._workerListCount do
    ...
    if nil ~= workerNoRaw then
      ...
      if itemNeedCount >= 1 then
        ...
        requestRecoveryWorker(WorkerNo(workerNoRaw), whereType, slotNo, itemNeedCount)
