...
function PaGlobal_WorkerManagerTooltip_All:ControlAll_Init()
  ...
  self._ui.txt_ability = UI.getChildControl(Panel_Widget_WorkerManagerTooltip_All, "StaticText_AbilityDesc")
  self._ui.txt_abilityTitle = UI.getChildControl(Panel_Widget_WorkerManagerTooltip_All, "StaticText_Ability")
  ...
  for idx = 0, self._maxSkillCount - 1 do
    ...
    tempSlot.skillBox:SetPosX(5 + (self._ui.stc_skillBoxBg:GetSizeX() + 5) * (idx % 2))
    tempSlot.skillBox:SetPosY(self._slotStartPosY + (tempSlot.skillBox:GetSizeY() + 3) * math.floor(idx / 2))
...
function PaGlobal_WorkerManagerTooltip_All:validate()
  ...
  self._ui.txt_ability:isValidate()
  self._ui.txt_abilityTitle:isValidate()
...
function PaGlobal_WorkerManagerTooltip_All:updateWorkerInfo()
  ...
  local workerType = workerWrapperLua:getWorkerType()
  self._ui.txt_ability:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_WORKERTYPE_SPEC_" .. workerType))
  local totalTextSize = self._ui.txt_abilityTitle:GetTextSizeX() + self._ui.txt_ability:GetTextSizeX()
  if totalTextSize > 210 then
    self._ui.txt_ability:SetSpanSize(30, 165)
  else
    self._ui.txt_ability:SetSpanSize(260, 165)
  end
...
function PaGlobal_WorkerManagerTooltip_All:updateSkillList()
  ...
  if nil ~= defaultSkill then
    ...
    Panel_Widget_WorkerManagerTooltip_All:SetSize(Panel_Widget_WorkerManagerTooltip_All:GetSizeX(), self._basePanelSizeY + addSizeY + 3)
  else
  ...
  for idx = 0, self._maxSkillCount - 1 do
    self._skill[idx].skillBox:SetPosX(5 + (self._ui.stc_skillBoxBg:GetSizeX() + 5) * ((idx + defaultSkillCnt) % 2))
    self._skill[idx].skillBox:SetPosY(self._slotStartPosY + (self._skill[idx].skillBox:GetSizeY() + 3) * math.floor((idx + defaultSkillCnt) / 2))
    local skillLev = 0
    if 0 == idx then
      skillLev = 1
    else
    ...
    if self._maxSkillCount - 1 == idx then
      local workerLevel = workerWrapperLua:getLevel()
      if _ContentsGroup_WorkerLv40 == false then
        self._skill[idx].skillBox:SetShow(false)
      elseif workerLevel >= __eWorkerPossibleLevelChangeStorage then
        self._skill[idx].name:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_40_WORKERSKILL_NAME"))
        self._skill[idx].desc:SetTextMode(__eTextMode_AutoWrap)
        self._skill[idx].desc:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_40_WORKERSKILL_TOOLTIP_ON"))
        self._skill[idx].icon:ChangeTextureInfoName("new_ui_common_forlua/skill/workerskill/2019.dds")
        self._skill[idx].icon:SetShow(true)
        self._skill[idx].icon:SetMonoTone(false)
        self._skill[idx].name:SetMonoTone(false)
        self._skill[idx].desc:SetMonoTone(false)
      else
        self._skill[idx].name:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_40_WORKERSKILL_NAME"))
        self._skill[idx].desc:SetTextMode(__eTextMode_AutoWrap)
        self._skill[idx].desc:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_40_WORKERSKILL_TOOLTIP_ON"))
        self._skill[idx].icon:ChangeTextureInfoName("new_ui_common_forlua/skill/workerskill/2019.dds")
        self._skill[idx].icon:SetShow(true)
        self._skill[idx].icon:SetMonoTone(true)
        self._skill[idx].name:SetMonoTone(true)
        self._skill[idx].desc:SetMonoTone(true)
      end
    else
  ...
  local addSizeY = 0
  if nil ~= defaultSkill then
    for index = 0, self._maxSkillCount - 1 do
      if 0 == index then
        if 50 < math.max(self._defaultSlot.desc:GetTextSizeY(), self._skill[index].desc:GetTextSizeY()) then
          self._defaultSlot.skillBox:SetSize(300, 100)
          self._defaultSlot.default:SetSize(300, 100)
          self._skill[index].skillBox:SetSize(300, 100)
          self._skill[index].default:SetSize(300, 100)
          addSizeY = addSizeY + 20
        else
          self._defaultSlot.skillBox:SetSize(300, 80)
          self._defaultSlot.default:SetSize(300, 80)
          self._skill[index].skillBox:SetSize(300, 80)
          self._skill[index].default:SetSize(300, 80)
        end
      else
        if math.floor(index / 2) ~= math.floor((index - 1) / 2) then
          if 50 < math.max(self._skill[index].desc:GetTextSizeY(), self._skill[index - 1].desc:GetTextSizeY()) then
            self._skill[index - 1].skillBox:SetSize(300, 100)
            self._skill[index - 1].default:SetSize(300, 100)
            self._skill[index].skillBox:SetSize(300, 100)
            self._skill[index].default:SetSize(300, 100)
            addSizeY = addSizeY + 20
          else
            self._skill[index - 1].skillBox:SetSize(300, 80)
            self._skill[index - 1].default:SetSize(300, 80)
            self._skill[index].skillBox:SetSize(300, 80)
            self._skill[index].default:SetSize(300, 80)
          end
        end
        if 1 == index then
          self._skill[index].skillBox:SetPosY(self._skill[index - 1].skillBox:GetPosY() + self._skill[index - 1].skillBox:GetSizeY() + 5)
        else
          self._skill[index].skillBox:SetPosY(self._skill[index - 2].skillBox:GetPosY() + self._skill[index - 2].skillBox:GetSizeY() + 5)
        end
      end
    end
  else
    for index = 1, self._maxSkillCount - 1 do
      if math.floor(index / 2) == math.floor((index - 1) / 2) then
        if 50 < math.max(self._skill[index].desc:GetTextSizeY(), self._skill[index - 1].desc:GetTextSizeY()) then
          self._skill[index - 1].skillBox:SetSize(300, 100)
          self._skill[index - 1].default:SetSize(300, 100)
  ...
  self._ui.stc_skillBg:SetSize(self._ui.stc_skillBg:GetSizeX(), self._ui.stc_skillBg:GetSizeY() + addSizeY + 10)
  Panel_Widget_WorkerManagerTooltip_All:SetSize(Panel_Widget_WorkerManagerTooltip_All:GetSizeX(), Panel_Widget_WorkerManagerTooltip_All:GetSizeY() + addSizeY + 10)
  self._ui.stc_skillBg:ComputePos()
end
