...
function PaGlobal_WorkerManagerChangeSkill_All:ControlAll_Init()
  ...
  self._ui.txt_notice:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORKERMANAGER_TOOLTIP_CHANGESKILL_DESC", "exp", expPercent))
  ...
  local lastSkillRadioBtnBG = self._skillList[self._maxSkillCount - 1].bg
  local beforeSkillBGSizeY = self._ui.stc_skillBg:GetSizeY()
  self._ui.stc_skillBg:SetSize(self._ui.stc_skillBg:GetSizeX(), lastSkillRadioBtnBG:GetPosY() + lastSkillRadioBtnBG:GetSizeY() + gabSizeY)
  local afterSkillBGSizeY = self._ui.stc_skillBg:GetSizeY()
  local skillBGSizeDiff = afterSkillBGSizeY - beforeSkillBGSizeY
  Panel_Window_WorkerManagerChangeSkill_All:SetSize(Panel_Window_WorkerManagerChangeSkill_All:GetSizeX(), Panel_Window_WorkerManagerChangeSkill_All:GetSizeY() + skillBGSizeDiff)
...
function PaGlobal_WorkerManagerChangeSkill_All:update()
  ...
  for skillIdx = 0, skillCount - 1 do
    ...
    if true == self._isPadSnap then
      ...
      self._skillList[skillIdx].name:addInputEvent("Mouse_LUp", "HandleEventLup_WorkerManagerChangeSkill_All_SelectSkill(" .. skillKeyRaw .. ")")
      self._skillList[skillIdx].desc:addInputEvent("Mouse_LUp", "HandleEventLup_WorkerManagerChangeSkill_All_SelectSkill(" .. skillKeyRaw .. ")")
    end
  end
end
