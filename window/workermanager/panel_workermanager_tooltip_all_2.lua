...
function FromClient_WorkerManagerTooltip_All_WorkerInfoChanged(workerNoRaw, worker, isLevelup)
  if true == isLevelup then
    ...
    local msg = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_WORKER_TOOLTIP_LVUP", "workName", name, "level", lev)
    if ToClient_isPlayingSequence() == true then
      chatting_sendMessage("", msg, CppEnums.ChatType.System)
    else
      chatting_sendMessage("", msg, CppEnums.ChatType.System, CppEnums.ChatSystemType.Worker)
    end
  end
end
