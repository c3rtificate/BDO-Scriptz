...
function PaGlobal_WorkerManager_All:ControlAll_Init()
  ...
  for resIdx = 0, self._restoreItemMaxCount - 1 do
    ...
    tempItemSlot.slot = SlotItem.new(tempItemSlot.slot, "workerManager_restoreSlot_" .. resIdx, resIdx, tempItemSlot.slotBG, slotConfig)
    ...
    tempItemSlot.slot.icon:SetSize(tempItemSlot.slotBG:GetSizeX(), tempItemSlot.slotBG:GetSizeY())
    tempItemSlot.slot:createChild()
    tempItemSlot.slot.icon:SetIgnore(true)
...
function PaGlobal_WorkerManager_All:ControlConsole_Init()
  ...
  self._ui_console.btn_changeName = UI.getChildControl(self._ui_console.stc_menuBg, "Button_ChangeName")
  ...
  self._detailBtnlist[6] = self._ui_console.btn_changeName
...
function PaGlobal_WorkerManager_All:ControlSetShow()
  ...
  if true == self._isPadSnap then
    ...
    local SpanSizeForPad = 15
    local comboSizeForPad = 250
    self._ui.combo_town:SetSize(comboSizeForPad, self._ui.combo_town:GetSizeY())
    self._ui.combo_grade:SetSize(comboSizeForPad, self._ui.combo_grade:GetSizeY())
    self._ui.combo_townlist:SetSize(comboSizeForPad, self._ui.combo_townlist:GetSizeY())
    self._ui.combo_gradelist:SetSize(comboSizeForPad, self._ui.combo_gradelist:GetSizeY())
    self._ui.combo_town:SetEnableArea(0, 0, self._ui.combo_town:GetSizeX(), self._ui.combo_town:GetSizeY())
    self._ui.combo_grade:SetEnableArea(0, 0, self._ui.combo_grade:GetSizeX(), self._ui.combo_grade:GetSizeY())
    local ScrollTown = UI.getChildControl(self._ui.combo_townlist, "Combobox_List_Scroll")
    if nil ~= ScrollTown then
      ScrollTown:ComputePos()
    end
    local ScrollGrade = UI.getChildControl(self._ui.combo_gradelist, "Combobox_List_Scroll")
    if nil ~= ScrollGrade then
      ScrollGrade:ComputePos()
    end
    local townComboBackStatic = UI.getChildControl(self._ui.combo_townlist, "Combobox_1_List_BackStatic")
    townComboBackStatic:SetSize(comboSizeForPad, townComboBackStatic:GetSizeY())
    local gradeComboBackStatic = UI.getChildControl(self._ui.combo_gradelist, "Combobox_1_List_BackStatic")
    gradeComboBackStatic:SetSize(comboSizeForPad, gradeComboBackStatic:GetSizeY())
    self._ui.combo_town:SetSpanSize(SpanSizeForPad, self._ui.combo_town:GetSpanSize().y)
    self._ui.combo_grade:SetSpanSize(SpanSizeForPad, self._ui.combo_town:GetSpanSize().y)
  else
...
function PaGlobal_WorkerManager_All:prepareOpen()
  ...
  for plantRawIdx = 1, #self._plantArray do
    ...
    if plantWorkerCount > 0 then
      ToClient_requestCheckWorkerCountQuestCondition(plantKey, plantWorkerCount)
    end
  end
end
...
function PaGlobal_WorkerManager_All:validate()
  ...
  self._ui_console.btn_changeName:isValidate()
...
function PaGlobal_WorkerManager_All:restoreItemUpdate()
  ...
  for itemIdx = self._sliderStartIdx, self._restoreItemHasCount - 1 do
    ...
    local itemStaticStatusWrapper = getItemEnchantStaticStatus(itemStatic._key)
    self._restoreItemSlot[itemCnt].slotBG:SetShow(true)
    self._restoreItemSlot[itemCnt].slot:setItemByStaticStatus(itemStaticStatusWrapper, recoveryItem._itemCount_s64, -1, false, false, false, 0, 0, false, false, false, CppEnums.ItemWhereType.eFamilyInventory == recoveryItem._itemWhereType)
    self._restoreItemSlot[itemCnt].restorePoint:SetText("+" .. tostring(recoveryItem._contentsEventParam1))
    self._restoreItemSlot[itemCnt].slotBG:addInputEvent("Mouse_RUp", "HandleEventRUp_WorkerManager_RestoreItemUse(" .. itemIdx .. ")")
    self._restoreItemSlot[itemCnt].slotBG:addInputEvent("Mouse_UpScroll", "HandleEventScroll_WorkerManager_All_RestoreSliderScroll(true)")
    self._restoreItemSlot[itemCnt].slotBG:addInputEvent("Mouse_DownScroll", "HandleEventScroll_WorkerManager_All_RestoreSliderScroll(false)")
    itemCnt = itemCnt + 1
  end
  self._ui.stc_restoreDesc:SetTextMode(__eTextMode_AutoWrap)
  self._ui.stc_restoreDesc:SetText(self._ui.stc_restoreDesc:GetText())
...
function PaGlobal_WorkerManager_All:restoreItemUse(itemIdx)
  ...
  local whereType = recoveryItem._itemWhereType
  ...
  if isKeyPressed(CppEnums.VirtualKeyCode.KeyCode_SHIFT) then
    ...
    if itemNeedCount >= 1 then
      requestRecoveryWorker(WorkerNo(workerNo), whereType, slotNo, itemNeedCount)
    else
  ...
  requestRecoveryWorker(WorkerNo(workerNo), whereType, slotNo, 1)
end
...
function PaGlobal_WorkerManager_All:stopWorking(workerIdx)
  ...
  if CppEnums.NpcWorkingState.eNpcWorkingState_HarvestWorking_Working == workingState then
    ...
    if _ContentsGroup_HarvestList_All then
      PaGlobal_HarvestList_All_Update()
    else
...
function PaGlobal_WorkerManager_All:upgradeWorker(workerIdx)
  ...
  local rateDesc = PAGetString(Defines.StringSheet_GAME, "LUA_WORKERMANAGER_UPGRADERATE")
...
function PaGlobal_WorkerManager_All:upgradeWorkerNow()
  ...
  local messageBoxMemo = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_GAMESHOP_NOTICE_11", "pearl", tostring(needPearl))
  if true == _ContentsGroup_ShowWorkerUpgradeNowRate then
    messageBoxMemo = messageBoxMemo .. "\n" .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORKER_UPGRADENOW_RATE", "successRate", tostring(workerWrapperLua:getWorkerUpgradeNowRate() / 10000))
  end
  ...
  if true == isJapan then
    ...
    messageBoxMemo = messageBoxMemo .. noticeString
...
local SetWorkingStateDesc = function(parentControl, workerWrapperLua)
  local txt_town = UI.getChildControl(parentControl, "StaticText_BelongTown")
  local txt_startTown = UI.getChildControl(parentControl, "StaticText_StartTown")
  local txt_arrow = UI.getChildControl(parentControl, "StaticText_TownArrow")
  local txt_storageTown = UI.getChildControl(parentControl, "StaticText_StorageTown")
  local storageRegionName = workerWrapperLua:getStorageRegionName()
  local isPlantZoneWorking = storageRegionName ~= ""
  local isContentsOptionOpen = _ContentsGroup_WorkerLv40
  if isContentsOptionOpen == false or isPlantZoneWorking == false then
    txt_town:SetShow(true)
    txt_startTown:SetShow(false)
    txt_arrow:SetShow(false)
    txt_storageTown:SetShow(false)
    return
  end
  txt_town:SetShow(false)
  txt_startTown:SetShow(true)
  txt_arrow:SetShow(true)
  txt_storageTown:SetShow(true)
  if _ContentsGroup_UsePadSnapping == true then
    txt_startTown:SetTextMode(__eTextMode_LimitText)
    txt_storageTown:SetTextMode(__eTextMode_LimitText)
  else
    UI.setLimitTextAndAddTooltip(txt_startTown)
    UI.setLimitTextAndAddTooltip(txt_storageTown)
  end
  txt_startTown:SetText(txt_town:GetText())
  txt_storageTown:SetText(storageRegionName)
end
...
function PaGlobal_WorkerManager_All:workerlistCreate(content, key)
  ...
  local stc_changeName = {
    btn = UI.getChildControl(stc_btnBg, "Button_ChangeName"),
    icon = UI.getChildControl(stc_btnBg, "Static_Icon_ChangeName")
  }
  local stc_repeat = {
  ...
  stc_changeName.btn:addInputEvent("Mouse_On", "HandleEventOnOut_WorkerManager_ButtonSimpleTooltip(true," .. tostring(key32) .. "," .. self._BUTTON.CHANGENAME .. " )")
  stc_changeName.btn:addInputEvent("Mouse_Out", "HandleEventOnOut_WorkerManager_ButtonSimpleTooltip(false," .. tostring(key32) .. "," .. self._BUTTON.CHANGENAME .. " )")
  ...
  stc_changeName.btn:addInputEvent("Mouse_LUp", "HandleEventLUp_WorkerManager_All_ChangeNameWindowOpen(" .. tostring(key32) .. ")")
  ...
  if false == isWorking then
    if true == workerWrapperLua:isWorkerRepeatable() and currentActionPoint > 0 then
  ...
  if true == self._isPadSnap then
    ...
    stc_changeName.btn:SetShow(false)
    ...
    stc_btnBg:addInputEvent("Mouse_On", "HandleEventOnOut_WorkerManager_All_ShowDetailMenuGuide(true, " .. tostring(key32) .. ")")
    stc_btnBg:addInputEvent("Mouse_Out", "HandleEventOnOut_WorkerManager_All_ShowDetailMenuGuide(false, " .. tostring(key32) .. ")")
  ...
  SetWorkingStateDesc(stc_btnBg, workerWrapperLua)
end
function PaGlobal_WorkerManager_All:upgradelistCreate(content, key)
  ...
  if -1 == self._upgradingWorkerNoRaw and workerUpgradeCount > 0 and true == workerWrapperLua:isUpgradable() then
    if true ~= isWorking then
      isUpgradable = true
    else
      isUpgradable = false
  ...
  if true == self._isPadSnap then
    ...
    if true == isUpgradable then
      ...
      stc_btnBg:addInputEvent("Mouse_On", "HandleEventOnOut_WorkerManager_ShowPromoteKeyGuide(true, true," .. tostring(key32) .. ")")
      stc_btnBg:addInputEvent("Mouse_Out", "HandleEventOnOut_WorkerManager_ShowPromoteKeyGuide(false, true," .. tostring(key32) .. ")")
    else
      btn_advence:SetMonoTone(true, true)
      stc_btnBg:registerPadEvent(__eConsoleUIPadEvent_Up_A, "")
      stc_btnBg:addInputEvent("Mouse_On", "HandleEventOnOut_WorkerManager_ShowPromoteKeyGuide(true, false," .. tostring(key32) .. ")")
      stc_btnBg:addInputEvent("Mouse_Out", "HandleEventOnOut_WorkerManager_ShowPromoteKeyGuide(false, false," .. tostring(key32) .. ")")
    end
  else
    UI.setLimitTextAndAddTooltip(txt_name)
    UI.setLimitTextAndAddTooltip(txt_town)
...
function PaGlobal_WorkerManager_All:detailMenuSet(workerIdx)
  ...
  self._ui_console.btn_changeName:SetShow(true)
  self._ui_console.btn_changeName:registerPadEvent(__eConsoleUIPadEvent_Up_A, "HandleEventLUp_WorkerManager_All_ChangeNameWindowOpen(" .. tostring(workerIdx) .. ")")
  local isAuction = workerWrapperLua:getIsAuctionInsert()
  if true == isAuction then
