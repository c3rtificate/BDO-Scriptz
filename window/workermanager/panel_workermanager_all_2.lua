...
function HandleEventLUp_WorkerManager_All_RestoreAll()
  ...
  if restoreItemCount <= 0 then
    chatting_sendMessage("", PAGetString(Defines.StringSheet_GAME, "LUA_WORLD_MAP_TOWN_WORKER_NOITEM"), CppEnums.ChatType.System, CppEnums.ChatSystemType.Worker)
    return
  end
...
function HandleEventLUp_WorkerManager_All_ChangeNameWindowOpen(workerIdx)
  if nil == PaGlobalFunc_WorkerManagerChangeName_All_Open then
    return
  end
  ...
  if true == workerWrapperLua:getIsAuctionInsert() then
    local message = PAGetString(Defines.StringSheet_GAME, "LUA_WORKER_NAME_MARKETWORKER_DONTNAME")
    Proc_ShowMessage_Ack(message)
    return
  end
  HandlePadEventDpad_WorkerManager_All_CloseDetailMenu()
  PaGlobalFunc_WorkerManagerChangeName_All_Open(workerIdx)
end
...
function HandleEventOnOut_WorkerManager_ButtonSimpleTooltip(isShow, key, tipType)
  ...
  elseif PaGlobal_WorkerManager_All._BUTTON.SKILL == tipType then
    local expPercent = ToClient_GetWorkerNeedExpPercentForSkillChange() / 10000
    name = PAGetString(Defines.StringSheet_GAME, "LUA_WORKERMANAGER_TOOLTIP_CHANGESKILL_NAME")
    desc = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORKERMANAGER_TOOLTIP_CHANGESKILL_DESC", "exp", expPercent)
  ...
  elseif PaGlobal_WorkerManager_All._BUTTON.CHANGENAME == tipType then
    name = PAGetString(Defines.StringSheet_GAME, "LUA_WORKER_NAME_MAKE_TOOLTIP_NAME")
    desc = PAGetString(Defines.StringSheet_GAME, "LUA_WORKER_NAME_MAKE_TOOLTIP_DESC")
    control = UI.getChildControl(stc_btnBg, "Button_ChangeName")
...
function HandlePadEventA_WorkerManager_All_ToggleDetailMenu(workerIdx)
  ...
  HandleEventOnOut_WorkerManager_WorkerInfoTooltip(false)
  ToClient_padSnapSetTargetGroupForce(PaGlobal_WorkerManager_All._ui_console.stc_detailMenu)
end
...
function HandleEventOnOut_WorkerManager_All_ShowDetailMenuGuide(isShow, key)
  if nil == Panel_Window_WorkerManager_All then
    return
  end
  if true == isShow then
  ...
  HandleEventOnOut_WorkerManager_WorkerInfoTooltip(isShow, key)
  PaGlobal_WorkerManager_All:consoleKeyAlign()
end
function HandleEventOnOut_WorkerManager_ShowPromoteKeyGuide(isShow, isUpgradable, key)
  if nil == Panel_Window_WorkerManager_All then
    return
  end
...
function FromClient_WorkerManager_All_StopWorkerWorking(workerNo, isUserRequest, working)
  ...
  if true == isUserRequest then
    chatting_sendMessage("", PAGetStringParam3(Defines.StringSheet_GAME, "LUA_WORLD_MAP_TOWN_WORKER_FINISHWORK_2", "workerName", workerName, "workingArea", workingArea, "workingName", workingName), CppEnums.ChatType.System, CppEnums.ChatSystemType.Worker)
  elseif false == isUserRequest then
    if working:isType(CppEnums.NpcWorkingType.eNpcWorkingType_PlantZone) then
      chatting_sendMessage("", PAGetStringParam2(Defines.StringSheet_GAME, "LUA_WORLD_MAP_TOWN_WORKER_FINISHWORK_3", "workerName", workerName, "workingArea", workingArea), CppEnums.ChatType.System, CppEnums.ChatSystemType.Worker)
    else
      chatting_sendMessage("", PAGetStringParam3(Defines.StringSheet_GAME, "LUA_WORLD_MAP_TOWN_WORKER_FINISHWORK_1", "workerName", workerName, "workingArea", workingArea, "workingName", workingName), CppEnums.ChatType.System, CppEnums.ChatSystemType.Worker)
    end
  end
end
...
function FromClient_WorkerManager_All_UpdateLastestWorkingResult(WorkerNoRaw)
  ...
  for idx = 1, result_Count do
    ...
    if itemWrapper:isSet() then
      ...
      local msg = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_WORLD_MAP_TOWN_WORKER_GOT_RESULT", "name", name, "count", count)
