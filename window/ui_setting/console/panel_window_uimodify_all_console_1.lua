...
function PaGlobal_UIModify:initializePanelPool()
  ...
  for idx = 1, self._panelCount do
    ...
    if nil ~= originalControl then
      ...
      if self._panel[idx].index == CppEnums.PAGameUIType.PAGameUIPanel_ClassResource then
        ...
        if PaGlobal_ClassResource_CanUseResourceIcon ~= nil and PaGlobal_ClassResource_CanUseResourceIcon(classType) == true then
          originalControl:SetShow(true)
          slot.close:SetShow(true)
          self._panel[idx].isShow = true
          self:updateClassResourceName()
        else
          self._panel[idx].isShow = false
          originalControl:SetShow(false)
        end
      end
...
function PaGlobal_UIModify:updateClassResourceName()
  if Panel_Window_UIModify_All_Console == nil then
    return
  end
  local name = ""
  local classType = getSelfPlayer():getClassType()
  if PaGlobal_ClassResource_CanUseResourceIcon ~= nil and PaGlobal_ClassResource_CanUseResourceIcon(classType) == true then
    ...
    elseif __eClassType_Corsair == classType then
      name = PAGetString(Defines.StringSheet_GAME, "LUA_CLASSRESOURCE_CORSAIR_NAME_4")
    ...
    elseif __eClassType_Drakania == classType then
      ...
      elseif PaGlobal_ClassResource_CanAwakenIconEnable(__eClassType_Drakania) == true then
        name = PAGetString(Defines.StringSheet_GAME, "LUA_CLASSRESOURCE_PQW_TAG")
      end
...
function PaGlobal_UIModify:prepareOpen()
  ...
  local levelLimit = 10
  if ToClient_CheckFamilyCharacterLevel(levelLimit) == false then
...
function PaGlobal_UIModify:updatePanelPool()
  ...
  for idx = 1, self._panelCount do
    ...
    if nil ~= slot and nil ~= slot.control then
      ...
      if idx == self._panelID.SkillCommand then
        if nil ~= FGlobal_IsChecked_SkillCommand then
      ...
      elseif idx == self._panelID.ClassResource then
        ...
        if PaGlobal_ClassResource_CanUseResourceIcon ~= nil and PaGlobal_ClassResource_CanUseResourceIcon(classType) == true then
          slot.control:SetShow(true)
          self:updateClassResourceName()
          if self._panel[idx].isShow == true then
            slot.control:SetText(self._panel[idx].name)
            self:changePanelBGTexture(idx, 2)
          else
            slot.control:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_UI_SETTING_SLOTCONTROL_OFF", "name", self._panel[idx].name))
  ...
  for idx = 1, self._panelCount do
    if idx ~= self._panelID.ClassResource or self._panelPool[idx].control ~= nil and self._panelPool[idx].control:GetShow() == true then
...
function PaGlobal_UIModify:getUiSettingPanelInfo(poolIndex, posX, posY, isShow, chatWindowIndex, relativePosX, relativePosY)
  ...
  if poolIndex == self._panelID.ClassResource then
    isShow = true
  end
