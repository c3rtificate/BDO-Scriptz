...
function HandleEventLUp_LocalWarInfo_All_ClickMainTree(key)
  ...
  for idx = 1, #PaGlobal_LocalWarInfo_All_New._treeMapTable do
    if nil ~= PaGlobal_LocalWarInfo_All_New._treeMapTable[idx] and true == PaGlobal_LocalWarInfo_All_New._treeMapTable[idx]._isMain then
      ...
      local isOpen = keyElement:getIsOpen()
...
function HandleEventOnOut_LocalWarInfo_AllClickMainTree(isShow, type)
  ...
  local textureName = ""
  if PaGlobal_LocalWarInfo_All_New._eMapKeyIndex._PAPUCRIO == type then
    return
  elseif PaGlobal_LocalWarInfo_All_New._eMapKeyIndex._NORMAL == type then
  ...
  elseif PaGlobal_LocalWarInfo_All_New._eMapKeyIndex._OCCUPATION == type then
    textureName = "new_ui_common_forlua/Window/LocalWarInfo/Map_Valencia.dds"
  end
...
function PaGlobal_LocalWarInfo_All_New_UpdateServerList(list_content, key)
  ...
  if true == treeData._isMain then
    ...
    local keyElement = PaGlobal_LocalWarInfo_All_New._ui._list2_serverList:getElementManager():getByKey(toInt64(0, treeData._keyIndex), false)
    rdo_btn:SetCheck(keyElement:getIsOpen())
  ...
  local isDisableCamouflage = ToClient_isDisableCamouflageLocalWar(getFieldKey)
  local warTime = ""
  ...
  elseif 2 == getCurrentState then
    ...
    local limitTime = 10
    if __eInstanceLocalWar_PapuCrio == getFieldKey then
      limitTime = 5
    end
    if warTimeMinute >= limitTime then
      btn_join:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_LOCALWARINFO_JOIN"))
      btn_join:SetIgnore(false)
    else
  ...
  getUniqueIndex = treeData._subIndex
  if "" == fieldNameString then
    txt_channel:SetText(treeData._string .. " - " .. tostring(getUniqueIndex + 1))
  else
    txt_channel:SetText(fieldNameString .. " - " .. tostring(getUniqueIndex + 1))
  end
  ...
  btn_join:addInputEvent("Mouse_On", "HandleEventOn_LocalWarInfo_All_New_JoinLocalWarDesc(" .. tostring(treeData._mapInfo._mapKey) .. ")")
  if true == isSeasonOnly then
    btn_seasonIcon:SetIgnore(false)
    btn_seasonIcon:addInputEvent("Mouse_On", "HandleEventOnOut_LocalWarInfo_All_SeasonIconTooltip(" .. tostring(key) .. ")")
    btn_seasonIcon:addInputEvent("Mouse_Out", "HandleEventOnOut_LocalWarInfo_All_SeasonIconTooltip()")
  end
  if true == isDisableCamouflage then
    btn_disableCamo:SetIgnore(false)
    btn_disableCamo:addInputEvent("Mouse_On", "HandleEventOnOut_LocalWarInfo_All_DisableCamoIconTooltip(" .. tostring(key) .. ")")
    btn_disableCamo:addInputEvent("Mouse_Out", "HandleEventOnOut_LocalWarInfo_All_DisableCamoIconTooltip()")
    if true == isSeasonOnly then
...
function HandleEventOnOut_LocalWarInfo_All_SeasonIconTooltip(uiKeyIndex)
  if nil == uiKeyIndex then
    TooltipSimple_Hide()
    return
  end
  ...
  if nil ~= uiContent then
    ...
    TooltipSimple_Show(uiContent, txt_title, txt_desc)
  end
end
function HandleEventOnOut_LocalWarInfo_All_DisableCamoIconTooltip(uiKeyIndex)
  if nil == uiKeyIndex then
    TooltipSimple_Hide()
    return
  end
...
function FromClient_LocalWarInfo_All_New_UpdateInstanceRoom()
  ...
  if nil ~= PaGlobal_LocalWarInfo_All_New._ui._list2_serverList and nil ~= PaGlobal_LocalWarInfo_All_New._ui._list2_serverList:getElementManager() then
    PaGlobal_LocalWarInfo_All_New._ui._list2_serverList:getElementManager():clearKey()
  end
  PaGlobal_LocalWarInfo_All_New:changeSizeY()
  PaGlobal_LocalWarInfo_All_New:controlMapList_Init()
  FromClient_LocalWarInfo_All_ResizePanel()
  PaGlobal_LocalWarInfo_All_New._ui._list2_serverList:getElementManager():refillKeyList()
