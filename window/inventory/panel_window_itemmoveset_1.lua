function PaGlobal_ItemMoveSet:initialize()
  ...
  self._ui._keyGuide_LTX = UI.getChildControl(self._ui._stc_keyGuide, "StaticText_LT_X_ConsoleUI")
  ...
  self._ui._keyGuide_LTX:SetShow(true == self._isConsole)
  local keyGuide = {
    self._ui._keyGuide_Y,
    self._ui._keyGuide_A,
    self._ui._keyGuide_LTX,
...
function PaGlobal_ItemMoveSet:createControl()
  ...
  for ii = 0, slotCount - 1 do
    ...
    if true == self._isConsole then
      ...
      slot.parent:registerPadEvent(__eConsoleUIPadEvent_LTPress_X, "PaGlobal_ItemMoveSet:clearItemSlot(" .. ii .. ")")
      ...
      if _ContentsGroup_RenewUI_Tooltip == true then
        slot.parent:registerPadEvent(__eConsoleUIPadEvent_Up_X, "PaGlobal_ItemMoveSet:showItemTooltipDetail(" .. ii .. ")")
      end
    else
...
function PaGlobal_ItemMoveSet:showItemTooltip(index)
  ...
  if _ContentsGroup_RenewUI_Tooltip == true then
    if true == PaGlobalFunc_TooltipInfo_GetShow() then
      ...
      PaGlobalFunc_FloatingTooltip_Open(Defines.TooltipDataType.ItemSSWrapper, itemSSW, Defines.TooltipTargetType.Item, self._itemSlots[index].icon, "WareHouse")
      PaGlobalFunc_TooltipInfo_Close()
    end
    return
  end
...
function PaGlobal_ItemMoveSet:showItemTooltipDetail(index)
  local slot = self._itemSlots[index]
  if nil == slot or 0 == slot.itemKey then
    return
  end
  ...
  if _ContentsGroup_RenewUI_Tooltip == true then
    if true == PaGlobalFunc_TooltipInfo_GetShow() then
      ...
      PaGlobalFunc_TooltipInfo_Open(Defines.TooltipDataType.ItemSSWrapper, itemSSW, Defines.TooltipTargetType.Item, 0, getScreenSizeX(), "WareHouse")
    end
  end
end
