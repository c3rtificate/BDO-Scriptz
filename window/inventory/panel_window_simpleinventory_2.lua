...
function HandleEventLUp_SimpleInventory_ToggleSortOption()
  if PaGlobal_SimpleInventory == nil then
    return
  end
  PaGlobalFunc_AutoSortFunctionPanel_ShowToggle(InvenSortLinkPanelIndex.AnotherCharacterInventory)
end
...
function SimpleInventory_updateSlotData()
  if PaGlobal_SimpleInventory == nil then
    return
  end
  PaGlobal_SimpleInventory:updateSlot()
  PaGlobal_SimpleInventory:updateSearchItem()
  if PaGlobal_SimpleInventory._isConsole == true then
    PaGlobal_SimpleInventory:updateSortKeyguide()
  end
end
function HandleEventLUp_SimpleInventory_ToggleSortCheckButton()
  if PaGlobal_SimpleInventory == nil then
    return
  end
  HandleLUp_AutoSortFunctionPanel_ToggleUseSort(InvenSortLinkPanelIndex.AnotherCharacterInventory, true)
end
...
function PaGlobalFunc_SimpleInventory_ShowSimpleToolTipSearch(isShow)
  ...
  TooltipSimple_Show(PaGlobal_SimpleInventory._ui._btn_search, name, desc, false)
end
function HandleEventLUp_SimpleInventory_TransportItemToMyBag(itemCount_s64, itemEnchantKey, itemWhereType, inventorySlotNo, isSealed)
  if PaGlobal_SimpleInventory == nil then
    return
  end
  if isSealed == nil then
    isSealed = false
  end
  ToClient_TransportItemToMyBag(PaGlobal_SimpleInventory._selectedCharacterNo, ItemEnchantKey(itemEnchantKey), itemWhereType, inventorySlotNo, itemCount_s64, isSealed)
end
function FromClient_ResponseTransportItem()
  if PaGlobal_SimpleInventory == nil then
    return
  end
  if PaGlobalFunc_TotalSearch_GetShow ~= nil and PaGlobalFunc_TotalSearch_GetShow() == true and PaGlobalFunc_TotalSearch_UpdateItemList ~= nil then
    PaGlobalFunc_TotalSearch_UpdateItemList()
  end
  if PaGlobalFunc_SimpleInventory_SearchResult_GetShow ~= nil and PaGlobalFunc_SimpleInventory_SearchResult_GetShow() == true and PaGlobalFunc_SimpleInventory_SearchResult_UpdateWhenTransportToMyBag ~= nil then
    PaGlobalFunc_SimpleInventory_SearchResult_UpdateWhenTransportToMyBag()
  end
  if Panel_Window_SimpleInventory ~= nil and Panel_Window_SimpleInventory:GetShow() == true then
    PaGlobalFunc_AutoSortFunctionPanel_LoadSortOptionAndDoSort(InvenSortLinkPanelIndex.AnotherCharacterInventory, false)
    PaGlobal_SimpleInventory:updateSlot()
  end
end
function FromClient_ItemEraseEvent()
  if PaGlobal_SimpleInventory == nil then
    return
  end
  if PaGlobalFunc_TotalSearch_GetShow ~= nil and PaGlobalFunc_TotalSearch_GetShow() == true and PaGlobalFunc_TotalSearch_UpdateItemList ~= nil then
    ...
    PaGlobalFunc_TotalSearch_UpdateItemList()
    if PaGlobal_TotalSearch._isConsole == false then
      PaGlobalFunc_TotalSearch_SetFocusEdit()
    end
  end
end
function PaGlobalFunc_SimpleInventory_TransportItemToBag(itemEnchantKey, itemWhereType, inventorySlotNo, stringItemCount_s64, isSealed)
  PaGlobal_SimpleInventory:itemTooltip_TransportItemToMyBag(itemEnchantKey, itemWhereType, inventorySlotNo, stringItemCount_s64, isSealed)
end
