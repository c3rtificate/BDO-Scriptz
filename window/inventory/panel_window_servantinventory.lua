...
function ServantInventoryOpenWithInventory(actorKeyRaw)
  ...
  if vehicleType == UI_VT.Type_CampingTent and true == _ContentsGroup_NewUI_Camp_All then
    Inventory_SetFunctor(PaGlobalFunc_ServantInventory_BasicItemFilter, FGlobal_PopupMoveItem_InitByInventory, PaGlobalFunc_CampWarehouse_All_Close, nil)
  elseif vehicleType == CppEnums.VehicleType.Type_ThrowStone then
    actorKeyForFilter = actorKeyRaw
    Inventory_SetFunctor(PaGlobalFunc_ServantInventory_BasicItemFilter_ThrowStone, FGlobal_PopupMoveItem_InitByInventory, ServantInventory_Close, nil)
  elseif vehicleType == CppEnums.VehicleType.Type_PracticeCannon or vehicleType == CppEnums.VehicleType.Type_Cannon or vehicleType == CppEnums.VehicleType.Type_Ballista then
    actorKeyForFilter = actorKeyRaw
    Inventory_SetFunctor(PaGlobalFunc_ServantInventory_BasicItemFilter_Cannon, FGlobal_PopupMoveItem_InitByInventory, ServantInventory_Close, nil)
  elseif vehicleType == CppEnums.VehicleType.Type_ThrowFire then
    actorKeyForFilter = actorKeyRaw
    Inventory_SetFunctor(PaGlobalFunc_ServantInventory_BasicItemFilter_ThrowFire, FGlobal_PopupMoveItem_InitByInventory, ServantInventory_Close, nil)
  else
    Inventory_SetFunctor(PaGlobalFunc_ServantInventory_BasicItemFilter, FGlobal_PopupMoveItem_InitByInventory, ServantInventory_Close, nil)
  end
...
function ServantInventory_OpenAll()
  ...
  if true == PaGlobalFunc_ServantInventory_GetShow() then
    Inventory_SetFunctor(PaGlobalFunc_ServantInventory_BasicItemFilter, FGlobal_PopupMoveItem_InitByInventory, nil, nil)
  end
...
function ServantInventory_Open()
  local isShow = Panel_Window_ServantInventory:GetShow()
  PaGlobalFunc_ServantInventory_CheckLoadUI()
...
function ServantInventory_Close(isAllPopupClose)
  if not PaGlobalFunc_ServantInventory_GetShow() then
    return
  end
  if nil ~= Panel_Popup_MoveItem and true == Panel_Popup_MoveItem:GetShow() and nil ~= PopupMoveItem_Close then
    PopupMoveItem_Close()
    if nil == isAllPopupClose or false == isAllPopupClose then
      return
    end
  end
  local self = servantInventory
  ...
  if _ContentsGroup_FamilyInventory == true and PaGlobal_WareHouse_All_GetShow() == false then
...
function PaGlobalFunc_ServantInventory_BasicItemFilter(slotNo, notUse_itemWrappers, whereType)
...
function PaGlobalFunc_ServantInventory_BasicItemFilter_ThrowStone(slotNo, notUse_itemWrappers, whereType)
  local itemWrapper = getInventoryItemByType(whereType, slotNo)
  if nil == itemWrapper then
    return true
  end
  if nil == actorKeyForFilter then
    return true
  end
  local vehicleActorWrapper = getVehicleActor(actorKeyForFilter)
  if nil == vehicleActorWrapper then
    return true
  end
  local enchantSS = itemWrapper:getStaticStatus()
  if nil ~= enchantSS then
    if true == vehicleActorWrapper:isHoldableItem(itemWrapper:get():getKey():getItemKey()) then
      return not enchantSS:isThrowStoneType()
    end
    return true
  end
  return true
end
function PaGlobalFunc_ServantInventory_BasicItemFilter_Cannon(slotNo, notUse_itemWrappers, whereType)
  local itemWrapper = getInventoryItemByType(whereType, slotNo)
  if nil == itemWrapper then
    return true
  end
  ...
  if nil ~= enchantSS then
    if true == vehicleActorWrapper:isHoldableItem(itemWrapper:get():getKey():getItemKey()) then
      return not enchantSS:isCannonBallType()
    end
    return true
  end
  return true
end
function PaGlobalFunc_ServantInventory_BasicItemFilter_ThrowFire(slotNo, notUse_itemWrappers, whereType)
  local itemWrapper = getInventoryItemByType(whereType, slotNo)
  if nil == itemWrapper then
    return true
  end
  ...
  if nil ~= enchantSS then
    if true == vehicleActorWrapper:isHoldableItem(itemWrapper:get():getKey():getItemKey()) then
      return not enchantSS:isThrowFireType()
    end
    return true
  end
  return true
