...
local FilterRadioTooltip = UI.createControl(__ePAUIControl_StaticText, Panel_Window_Inventory, "HelpWindow_For_InvenFilterRadio")
...
FilterRadioTooltip:SetTextMode(__eTextMode_AutoWrap)
...
function inven:init()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(self._buttonQuestion, "\"PanelWindowInventory\"")
  if true == _ContentsOption_CH_Help then
    self._buttonQuestion:SetShow(false)
    self.checkPopUp:SetSpanSize(self._buttonQuestion:GetSpanSize().x, self._buttonQuestion:GetSpanSize().y)
  end
  ...
  self.buttonMoney:addInputEvent("Mouse_On", "InventoryWindow_MoneyIconTooltip(true, 0)")
  self.buttonMoney:addInputEvent("Mouse_Out", "InventoryWindow_MoneyIconTooltip(false, 0)")
  self.buttonPearl:addInputEvent("Mouse_LUp", "HandleEventLUp_Inventory_OpenPearlShop()")
  self.buttonPearl:addInputEvent("Mouse_On", "InventoryWindow_MoneyIconTooltip(true, 1)")
  self.buttonPearl:addInputEvent("Mouse_Out", "InventoryWindow_MoneyIconTooltip(false, 1)")
  self.buttonMileage:addInputEvent("Mouse_On", "InventoryWindow_MoneyIconTooltip(true, 2)")
  self.buttonMileage:addInputEvent("Mouse_Out", "InventoryWindow_MoneyIconTooltip(false, 2)")
...
function HandleEventLUp_Inventory_OpenPearlShop()
...
function inven:createSlot()
  for ii = 0, self.config.slotCount - 1 do
    local slot = {}
    slot.empty = UI.createControl(__ePAUIControl_Static, Panel_Window_Inventory, "Inventory_Slot_Base_" .. ii)
    slot.lock = UI.createControl(__ePAUIControl_Static, Panel_Window_Inventory, "Inventory_Slot_Lock_" .. ii)
    slot.plus = UI.createControl(__ePAUIControl_Static, Panel_Window_Inventory, "Inventory_Slot_Plus_" .. ii)
    slot.onlyPlus = UI.createControl(__ePAUIControl_Static, Panel_Window_Inventory, "Inventory_Slot_OnlyPlus_" .. ii)
  ...
  for ii = 0, self.config.slotCount - 1 do
    ...
    slot.background = UI.createControl(__ePAUIControl_Static, Panel_Window_Inventory, "Inventory_Slot_BG_" .. ii)
...
function Inventory_GroundClick(whereType, slotNo)
  if false == Panel_Window_Inventory:GetShow() or MessageBoxGetShow() then
    return
  end
  local itemWrapper = getInventoryItemByType(whereType, slotNo)
  if nil == itemWrapper then
...
function Inventory_ItemDelete_Check(count, slotNo, whereType)
  ...
  itemCount = count
  DragManager:clearInfo()
  if Panel_Invertory_ExchangeButton:GetShow() then
    Panel_Invertory_ExchangeButton:SetShow(false)
  end
  ...
  if ToClient_isTempCashItem(itemWrapper:getStaticStatus():get()._key) == true and itemWrapper:checkTimeover() == true then
    luaDeleteItemMsg = luaDeleteItemMsg .. PAGetString(Defines.StringSheet_GAME, "LUA_BUY_PERMANENTUSE_ITEM")
  end
  local messageContent = luaDeleteItemMsg
  local messageboxData = {
    title = luaDelete,
  ...
  for ii = 0, self.config.slotCount - 1 do
    ...
    if nil ~= itemWrapper then
      ...
      if not isUsableServant and not Panel_Window_Exchange:GetShow() then
        if isEquip and nil ~= defencePoint and nil ~= offencePoint and currentEndurance > 0 and true == matchEquip and true == PaGlobalFunc_Util_CheckSeasonEquip(itemSSW) and false == isAccessory and defencePoint + offencePoint > equipDefencePoint + equipOffencePoint then
        ...
        if currentEndurance > 0 and true == matchEquip and true == PaGlobalFunc_Util_CheckSeasonEquip(itemSSW) and true == isAccessory and offencePoint + defencePoint > equipOffencePoint + equipDefencePoint then
          slot.icon:AddEffect("fUI_BetterItemAura01", true, 0, 0)
          local equipPos = itemWrapper:getStaticStatus():getEquipSlotNo()
          if false == _ContentsGroup_RemasterUI_Main_Alert then
            Panel_NewEquip_Update(equipPos)
      ...
      elseif false == PaGlobalFunc_Util_CheckSeasonEquip(itemSSW) then
...
function PaGlobal_Inventory_ChangeMileage()
  local self = inven
  local whereType = Inventory_GetCurrentInventoryType()
  self.buttonMileage:SetSize(30, 30)
  if CppEnums.ItemWhereType.eInventory == whereType or false == _ContentsGroup_MileageShop then
    inven:setOceanMoneyImage()
  elseif CppEnums.ItemWhereType.eCashInventory == whereType then
    inven:setMileageImage()
  end
  self.buttonMileage:setRenderTexture(self.buttonMileage:getBaseTexture())
  self:updateMileageValue()
  self.buttonMileage:ComputePos()
end
function inven:setMileageImage()
  ...
  self._eIconTypeValue = self._eIconTooltipType.mileage
end
function inven:setOceanMoneyImage()
  self:setMileageImage()
  if true == _ContentsGroup_OceanCurrent then
    self.buttonMileage:SetSize(19, 19)
    self.buttonMileage:ChangeTextureInfoName("Combine/Icon/Combine_Cashshop_Icon_00.dds")
    local x1, y1, x2, y2 = setTextureUV_Func(self.buttonMileage, 101, 24, 120, 43)
    self.buttonMileage:getBaseTexture():setUV(x1, y1, x2, y2)
    self._eIconTypeValue = self._eIconTooltipType.oceanMoney
  end
end
function inven:updateMileageValue()
  ...
  if self._eIconTooltipType.oceanMoney == self._eIconTypeValue and true == _ContentsGroup_OceanCurrent then
    mileage = Defines.s64_const.s64_0
    if nil ~= priceItemWrapper then
      mileage = priceItemWrapper:get():getCount_s64()
    end
  end
...
function Inventory_AddItem(itemKey, slotNo, itemCount, whereType)
  local self = inven
  for ii = 0, self.config.slotCount - 1 do
    if ii == slotNo then
...
function Inventory_UseItemTargetSelf(whereType, slotNo, equipSlotNo)
  ...
  if itemKey == 724051 and PaGlobalFunc_AbyssOne_SkillNakMessage_Open ~= nil then
    PaGlobalFunc_AbyssOne_SkillNakMessage_Open(2)
  end
end
...
function Inventory_DropHandler(index)
  ...
  if Panel_Window_Exchange:IsShow() or isManufactureOpen or isAlchemyOpen or MessageBoxGetShow() or false == _ContentsGroup_NewUI_XXX and true == FGlobal_Enchant_SetTargetItem() or Panel_Window_DetectUser:IsShow() or PaGlobal_PetRegister_All_GetShow() then
    DragManager:clearInfo()
...
function PuzzleButton_Over(slotIndex)
  ...
  Panel_Tooltip_Item_hideTooltip()
  _puzzleNoticeText:SetTextMode(__eTextMode_AutoWrap)
...
function InventoryWindow_MoneyIconTooltip(isShow, tipType)
  local self = inven
  local name, desc, control
  if 0 == tipType then
  ...
  elseif 1 == tipType then
    ...
    control = self.buttonPearl
  elseif 2 == tipType then
    if self._eIconTooltipType.oceanMoney == self._eIconTypeValue then
    ...
    elseif self._eIconTooltipType.mileage == self._eIconTypeValue then
...
function Inventory_ShowEquipUI()
  if _ContentsGroup_NewUI_Equipment_All == true then
    Panel_Window_Equipment_All:SetShow(true)
  else
...
function FGlobal_UpdateInventoryWeight()
  ...
  self.staticWeight:SetText(makeWeightString(s64_allWeight, 1) .. " / " .. makeWeightString(s64_maxWeight, 0) .. " " .. PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_WEIGHT"))
...
function FromClient_Inventory_Initialize()
  if isAlchemyStoneEnble then
    inven.btn_AlchemyStone:setButtonShortcuts("PANEL_SIMPLESHORTCUT_ALCHEMY_RECHARGE")
  end
end
...
registerEvent("FromClient_luaLoadComplete", "FromClient_Inventory_Initialize")
function PaGlobal_Inventory_UnCheckAutoSort()
  if true == isRealServiceMode() then
    return false
  end
  local self = inven
  self.checkButton_Sort:SetCheck(false)
  Inventory_SetSorted()
  return true
end
