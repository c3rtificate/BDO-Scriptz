function PaGlobal_Equip_CharacterTag_ItemCopy:init()
  ...
  local stc_mainCircle = UI.getChildControl(self._ui._stc_equipSlotGroup, "Static_MainCircle")
  local stc_mainCircleArtifact = UI.getChildControl(self._ui._stc_equipSlotGroup, "Static_MainCircle_Relics")
  if false == _ContentsGroup_ArtifactItem then
    self._ui._stc_mainCirce = stc_mainCircle
    stc_mainCircle:SetShow(true)
    stc_mainCircleArtifact:SetShow(false)
  else
    self._ui._stc_mainCirce = stc_mainCircleArtifact
    stc_mainCircle:SetShow(false)
    stc_mainCircleArtifact:SetShow(true)
    self._ui._stc_itemCopyDecoBG:SetSpanSize(self._ui._stc_itemCopyDecoBG:GetSpanSize().x, self._ui._stc_itemCopyDecoBG:GetSpanSize().y + 10)
    self._ui._stc_itemCopyDecoBG:ComputePos()
  end
...
function PaGlobal_Equip_CharacterTag_ItemCopy:setControlInit()
  for slotNo = self._EquipNoMin, self._EquipNoMax - 1 do
    if true == PaGlobal_Equip_CharacterTag_ItemCopy:checkUsableSlot(slotNo) then
      ...
      if nil ~= slotName then
        ...
        slot.disable = UI.getChildControl(slot.icon, "Static_Disable")
        slot.disable:SetShow(false)
  ...
  if true == _ContentsGroup_DuelCharacterCopyChangeEquip then
    ...
    self._changeCopySlot.disable = UI.getChildControl(self._changeCopySlot.icon, "Static_Disable")
    self._changeCopySlot.disable:SetShow(false)
    ...
    self._releaseCopySlot.disable = UI.getChildControl(self._releaseCopySlot.icon, "Static_Disable")
    self._releaseCopySlot.disable:SetShow(false)
  ...
  self._awakenWeaponContentsOpen = PaGlobalFunc_Util_GetIsAwakenWeaponContentsOpen(self._selfClassType)
  ...
  self._ui._btn_selectAwakenWeaponForShy = UI.getChildControl(self._equipSlotBG[__eEquipSlotNoAwakenWeapon], "Button_AddWeaponForShy")
  self._ui._btn_selectAwakenWeaponForShy:SetShow(false)
...
function PaGlobal_Equip_CharacterTag_ItemCopy:registerEventHandler()
  ...
  registerEvent("FromClient_PrePareCopyMyEquipItem", "FromClient_PaGlobal_Equip_CharacterTag_UpdateNeedMaterialItemCount")
  ...
  if true == _ContentsGroup_DuelCharacterCopyChangeEquip then
    self._ui._btn_copyAllTap:addInputEvent("Mouse_LUp", "PaGlobal_Equip_CharacterTag_ItemCopy:changeTap(" .. self.eTabIndex.ALL_COPY .. ")")
...
function PaGlobal_Equip_CharacterTag_ItemCopy:clear()
  ...
  if _ContentsGroup_ShyClassAwakenWeaponItemCopy == true then
...
function PaGlobal_Equip_CharacterTag_ItemCopy:prepareOpen()
  ...
  self._ui._btn_selectAwakenWeaponForShy:SetShow(false)
  if _ContentsGroup_ShyClassAwakenWeaponItemCopy == true and self._selfClassType == __eClassType_ShyWaman then
    local slot = self._equipSlot[__eEquipSlotNoAwakenWeapon]
    if slot ~= nil and slot.icon ~= nil and slot.icon:GetShow() == true then
      ...
      self._ui._btn_selectAwakenWeaponForShy:SetShow(true)
    end
  end
  self:open()
end
  ...
  if true == isShow then
    if false == _ContentsGroup_RenewUI_Inventory then
      if nil ~= PaGlobalFunc_Inventory_All_OpenWithItemCopy then
        PaGlobalFunc_Inventory_All_OpenWithItemCopy(true)
      end
...
function PaGlobal_Equip_CharacterTag_ItemCopy:duelCharacterNameSet()
  ...
  local duelCharIndex = PaGlobalFunc_CharacterTag_All_GetDuelCharcterIndex()
  self._toCharacterName = ""
  if self.LOCAL_DEFINE.NODUEL ~= duelCharIndex then
    local characterData = getCharacterDataByIndex(duelCharIndex, __ePlayerCreateNormal)
...
function PaGlobal_Equip_CharacterTag_ItemCopy:checkUsableSlot(index)
  ...
  return selfPlayerWrapper:get():checkUsableCopySlot(index)
end
function PaGlobal_Equip_CharacterTag_ItemCopy:setEquipment()
  ...
  for slotNo = self._EquipNoMin, self._EquipNoMax do
    if self:checkUsableSlot(slotNo) == true then
      local itemWrapper = ToClient_getEquipmentItem(slotNo)
      local slot = self._equipSlot[slotNo]
      local slotBG = self._equipSlotBG[slotNo]
      local doSetEquipSlot = true
      slot.disable:SetShow(false)
      if slot ~= nil and slotBG ~= nil then
        self:setEquipSlotBGIcon(slotNo)
        if _ContentsGroup_ShyClassAwakenWeaponItemCopy == true and slotNo == __eEquipSlotNoAwakenWeapon and self._selfClassType == __eClassType_ShyWaman then
          doSetEquipSlot = false
        end
        if doSetEquipSlot == true and itemWrapper ~= nil then
          ...
          slot.disable:SetShow(false)
          self:setItemInfoUseWrapper(slot, itemWrapper, false, false, slotNo, canCopyAble)
    ...
    for extendSlotNo, parentSlotNo in pairs(self._extendedSlotInfoArray) do
      ...
      if nil ~= slot and nil ~= slotBG then
        ...
        if nil ~= itemWrapper then
          ...
          self:setItemInfoUseWrapper(slot, itemWrapper, true, true, canCopyAble)
        end
      end
    end
  end
...
function PaGlobal_Equip_CharacterTag_ItemCopy:setAddCopyItemSecond(isClear)
  ...
  local needPearlItemCount = ToClient_GetMyDuelCharacterCount() - 1
  local needItemCount = toInt64(0, needPearlItemCount)
  if hasItemCount < needItemCount then
...
function PaGlobal_Equip_CharacterTag_ItemCopy:updateCopyEquipItemMaterialItemCount()
  if PaGlobal_Equip_CharacterTag_ItemCopy == nil then
    return
  end
  local needItemEnchantKey = ToClient_getNeedItemForDuelCopy()
  if needItemEnchantKey == nil then
    return
  end
  local itemEnchantSSW = getItemEnchantStaticStatus(needItemEnchantKey)
