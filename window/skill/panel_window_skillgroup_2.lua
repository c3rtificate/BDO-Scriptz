...
function SkillGroup.new(key, parent, subGroupNo, tabIndex)
  newObj = {}
  setmetatable(newObj, SkillGroup)
  newObj:init(key, parent, subGroupNo, tabIndex)
  return newObj
end
function SkillGroup:init(key, parent, subGroupNo, tabIndex)
  self._key = 0
  ...
  self._isActiveItemSkill = PaGlobalFunc_Skill_IsActiveItemSkill(skillKey)
  ...
  for ii = 1, maxSkillCCTypeCount - 1 do
    if true == skillSSW:isSetSkillCCType(ii - 1) and nil ~= skillCCTypeConfig.uv[ii] then
      if idxOfSkillCCType < 6 then
        ...
        self._groupBg:registerPadEvent(__eConsoleUIPadEvent_LTPress_X, "PaGlobal_SkillGroup_Controller_PlayBlackSpiritSkill(" .. self._key .. ")")
...
function SkillGroup:HandleEventLUp_ClickedLevelButton(isPlus)
  if isPlus == nil then
    return
  end
  ...
  PaGlobalFunc_Skill_ChangeSkillShowTypeKeyGuide(skillNo)
end
function SkillGroup:HandleEventLUp_BlackSpiritLock()
end
function SkillGroup:HandleEventLUp_SkillCommandLock()
  local skillKey = ToClient_getSkillKeyBySkillGroupNo(self._key, self._selectLevel)
  if false == skillKey:isDefined() then
    return
  end
  local skillNo = skillKey:getSkillNo()
  ...
  if nil ~= skillLevelInfo then
    ...
    if isBlockSkill then
      local groupMaxLevel = ToClient_getSkillGroupMaxLevel(self._key)
      for ii = 1, groupMaxLevel do
        local relatedSkillKey = ToClient_getSkillKeyBySkillGroupNo(self._key, ii)
  ...
  self:selectLevel(self._selectLevel)
end
...
function SkillGroup:showBlackSpiritLock(isShow)
  if nil == isShow then
    return
  end
  self._check_blackSpirit:SetShow(false)
  ...
  if nil ~= skillStatic then
    ...
    if nil ~= blackSkillTypeSS and true == blackSkillTypeSS:isValidLocalizing() then
      ...
      if nil ~= blackSkillStatic then
        self._check_blackSpirit:SetShow(true)
      end
    end
  end
end
...
function SkillGroup:selectLevel(level, isLearnSkill)
  ...
  if true == _ContentsGroup_SkillFilter then
    local filterSkillKey = ToClient_getSkillKeyBySkillGroupNo(self._key, 1)
    local skillNo = filterSkillKey:getSkillNo()
    isShowFilterEffect = PaGlobalFunc_SkillFilter_All_isFilterSkill(skillSSW, skillNo)
  end
  self._groupBg:EraseAllEffect()
  if true == isShowFilterEffect then
    self._groupBg:AddEffect("fUI_NewSkill_Guide_03A", true, 0, 0)
  end
  ...
  if true == isLearnedSkill and false == isOtherBranch then
    ...
    self._slot.icon:addInputEvent("Mouse_RUp", "HandleMRUp_SkillGroupWindow_ClearButtonClick(" .. skillNo .. ", false)")
    self._groupBg:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "HandleMRUp_SkillGroupWindow_ClearButtonClick(" .. skillNo .. ", false)")
    self._groupBg:registerPadEvent(__eConsoleUIPadEvent_LTPress_Y, "HandleMRUp_SkillGroupWindow_ClearButtonClick(" .. skillNo .. ", true)")
    ...
    if true == nextSkillKey:isDefined() then
      ...
      if nil ~= nextSkillLevelInfo and true == nextSkillLevelInfo._learnable and false == nextSkillLevelInfo._usable then
        ...
        self._groupBg:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobal_SkillGroup:HandleEventLUp_SkillGroup(" .. self._key .. "," .. nextSkillNo .. ")")
        self._groupBg:registerPadEvent(__eConsoleUIPadEvent_LTPress_A, "PaGlobal_SkillGroup:HandleEventLUp_SkillGroup(" .. self._key .. "," .. nextSkillNo .. ")")
      ...
      self._slot.icon:addInputEvent("Mouse_LUp", "PaGlobal_SkillGroup:HandleEventLUp_SkillGroup(" .. self._key .. ")")
      self._groupBg:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobal_SkillGroup:HandleEventLUp_SkillGroup(" .. self._key .. ")")
      self._groupBg:registerPadEvent(__eConsoleUIPadEvent_LTPress_A, "PaGlobal_SkillGroup:HandleEventLUp_SkillGroup(" .. self._key .. ")")
      self._groupBg:registerPadEvent(__eConsoleUIPadEvent_LTPress_X, "PaGlobal_SkillGroup_Controller_PlayBlackSpiritSkill(" .. self._key .. ")")
    end
  else
    if nil ~= skillSSW then
    ...
    self._groupBg:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "")
    self._slot.icon:SetEnableDragAndDrop(false)
  end
  if true == isOtherBranch then
    self._slot.icon:addInputEvent("Mouse_LUp", "SkillGroup:cannotLernSkill(" .. branchType .. ")")
    self._groupBg:registerPadEvent(__eConsoleUIPadEvent_Up_A, "SkillGroup:cannotLernSkill(" .. branchType .. ")")
    self._groupBg:registerPadEvent(__eConsoleUIPadEvent_LTPress_A, "SkillGroup:cannotLernSkill(" .. branchType .. ")")
  ...
  elseif true == skillLevelInfo._learnable then
    ...
    self._groupBg:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobal_SkillGroup:HandleEventLUpLearnSkill(" .. skillNo .. ")")
    self._groupBg:registerPadEvent(__eConsoleUIPadEvent_LTPress_A, "PaGlobal_SkillGroup:HandleEventLUpLearnSkill(" .. skillNo .. ")")
    self._groupBg:SetMonoTone(false)
    self._slot.icon:SetMonoTone(true, true)
    self._ableQuickSlotEffect:SetMonoTone(true, true)
  else
  ...
  for ii = 1, self._maxLevel do
    ...
    if self._isActiveItemSkill == true then
      circle._default:SetShow(false)
      circle._learn:SetShow(false)
    end
  end
  if skillSSW ~= nil and currentCircleLevel ~= nil and currentCircleLevel > 0 then
    self:updateSkillCCType(skillSSW)
  end
...
function SkillGroup:HandleMOver_SkillGroupWindow_ToolTipShow(skillNo, key)
  PaGlobal_SkillGroup._mouseOnSkillGroupNo = key
...
function SkillGroup:registerEvent(key)
  ...
  if true == PaGlobal_SkillGroup._isConsole then
    self._groupBg:addInputEvent("Mouse_Out", "PaGlobal_SkillGroup:HandleMOver_SkillGroupOut(" .. key .. ")")
    self._groupBg:registerPadEvent(__eConsoleUIPadEvent_LSClick, "PaGlobal_SkillGroup:HandleEventLUp_SkillCommandLock(" .. key .. ")")
    self._groupBg:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobal_SkillGroup:HandleEventLUp_SkillGroup(" .. key .. ")")
    self._groupBg:registerPadEvent(__eConsoleUIPadEvent_LTPress_A, "PaGlobal_SkillGroup:HandleEventLUp_SkillGroup(" .. key .. ")")
    self._groupBg:registerPadEvent(__eConsoleUIPadEvent_LTPress_X, "PaGlobal_SkillGroup_Controller_PlayBlackSpiritSkill(" .. key .. ")")
    self._groupBg:registerPadEvent(__eConsoleUIPadEvent_LTPress_DpadLeft, "PaGlobal_SkillGroup:HandleEventLUp_PrevLevelButton(" .. key .. ")")
    self._groupBg:registerPadEvent(__eConsoleUIPadEvent_LTPress_DpadRight, "PaGlobal_SkillGroup:HandleEventLUp_NextLevelButton(" .. key .. ")")
  end
end
