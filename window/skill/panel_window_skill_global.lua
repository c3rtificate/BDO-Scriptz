...
local beforSuccessionSkillNo = -1
function PaGlobal_Skill:initialize()
  if false == PaGlobal_SuccessionSkill_IsCheckOpenContents() then
    self.eTabIndex.awaken = 1
    self.eTabIndex.succession = 2
  end
  self.tabText[self.eTabIndex.combat] = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SKILL_COMBAT")
  self.tabText[self.eTabIndex.succession] = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SKILL_SUCCESSTIONSKILL_TAB_NAME")
  self.tabText[self.eTabIndex.awaken] = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SKILL_RELEASEWEAPONE")
  for i = self.eTabIndex.combat, self.eTabIndex.loopCount do
    self.radioButtons[i] = UI.getChildControl(Panel_Window_Skill, "RadioButton_Tab_" .. i)
    self.radioButtons[i]:SetText(self.tabText[i])
    self.frames[i] = UI.getChildControl(Panel_Window_Skill, "Frame_" .. i)
  end
  ...
  self._txt_MentalTip:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._txt_topGuide = UI.getChildControl(Panel_Window_Skill, "StaticText_TopGuide")
  self._stc_transcendenceIcon = UI.getChildControl(Panel_Window_Skill, "Static_TranscendenceIcon")
...
function PaGlobal_Skill:initControl()
  ...
  local _frame_Combat = self.frames[self.eTabIndex.combat]
  local _frame_scroll_Combat = UI.getChildControl(_frame_Combat, "Frame_1_Scroll")
  PaGlobal_Skill._txt_ResourceSaveDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  if _ContentsOption_CH_GameType == true then
    self._btn_MovieToolTip:SetShow(false)
    self._btn_MovieToolTipDesc:SetSize(self._btn_MovieToolTipBG:GetSizeX() - 10, self._btn_MovieToolTipDesc:GetSizeY())
    self._btn_MovieToolTipDesc:SetSpanSize(self._btn_MovieToolTipBG:GetSpanSize().x, self._btn_MovieToolTipBG:GetSpanSize().y)
    self._btn_MovieToolTipDesc:SetVerticalTop()
  end
    ...
    for col = 0, cols - 1 do
      ...
      if cell:isSkillType() then
        ...
        if nil == self._static_SlotBG[index] then
          slotBG._base = UI.createControl(__ePAUIControl_Static, parent, "combiSkillSlot_" .. index)
...
function PaGlobal_Skill:initTabControl_Combat()
  local targetFrame = self.frames[self.eTabIndex.combat]
  self.slots[self.eTabIndex.combat] = {}
  local classType = getSelfPlayer():getClassType()
  if classType >= 0 then
    local cellTable = getCombatSkillTree(classType)
    self:initTabControl(cellTable, targetFrame:GetFrameContent(), self.slots[self.eTabIndex.combat])
  end
  targetFrame:UpdateContentScroll()
end
function PaGlobal_Skill:initTabControl_AwakeningWeapon()
  local targetFrame = self.frames[self.eTabIndex.awaken]
  self.slots[self.eTabIndex.awaken] = {}
  local classType = getSelfPlayer():getClassType()
  if classType >= 0 then
    ...
    self:initTabControl(cellTable, targetFrame:GetFrameContent(), self.slots[self.eTabIndex.awaken])
  end
  targetFrame:UpdateContentScroll()
end
function PaGlobal_Skill:initTabControl_Succession()
  local targetFrame = self.frames[self.eTabIndex.succession]
  self.slots[self.eTabIndex.succession] = {}
  local classType = getSelfPlayer():getClassType()
  if classType >= 0 then
    ...
    self:initTabControl(cellTable, targetFrame:GetFrameContent(), self.slots[self.eTabIndex.succession])
  end
  targetFrame:UpdateContentScroll()
end
...
function PaGlobal_Skill:OnOffCombinationTab()
  ...
  if true == isLearnFusionSkill then
    ...
    Panel_Window_Skill:SetSize(Panel_Window_Skill:GetSizeX(), 800)
  ...
  self._stc_mouseL:ComputePos()
  self._stc_mouseR:ComputePos()
...
function PaGlobal_Skill:Skill_WindowPosSet(pos, skillType)
  local index = 0
  if __eSkillTypeParam_Normal == skillType then
    index = self.eTabIndex.combat
  elseif __eSkillTypeParam_Inherit == skillType then
    index = self.eTabIndex.succession
  elseif __eSkillTypeParam_Awaken == skillType then
    index = self.eTabIndex.awaken
  end
...
function PaGlobal_Skill:SkillWindowEffect(row, col, skillNo, isOn, skillType)
  local skillTypeStaticWrapper = getSkillTypeStaticStatus(skillNo)
  if skillTypeStaticWrapper:isValidLocalizing() then
    local posX, posY = self:SkillCalcPosYByColumn(col), self:SkillCalcPosYByRow(row)
    self:Skill_WindowPosSet(posY, skillType)
    self.savedAwaken = skillType
    if __eSkillTypeParam_Inherit == skillType then
      if -1 ~= beforSuccessionSkillNo and nil ~= self.slots[self.eTabIndex.succession][beforSuccessionSkillNo] then
        self.slots[self.eTabIndex.succession][beforSuccessionSkillNo].mouseOverButton:EraseAllEffect()
        self.slots[self.eTabIndex.succession][beforSuccessionSkillNo].mouseOverButton:SetShow(false)
        self.slots[self.eTabIndex.succession][beforSuccessionSkillNo].mouseOverButton:SetIgnore(true)
        beforSuccessionSkillNo = skillNo
      end
    elseif __eSkillTypeParam_Awaken == skillType then
      if -1 ~= beforAwakenSkillNo and nil ~= self.slots[self.eTabIndex.awaken][beforAwakenSkillNo] then
        self.slots[self.eTabIndex.awaken][beforAwakenSkillNo].mouseOverButton:EraseAllEffect()
        self.slots[self.eTabIndex.awaken][beforAwakenSkillNo].mouseOverButton:SetShow(false)
        self.slots[self.eTabIndex.awaken][beforAwakenSkillNo].mouseOverButton:SetIgnore(true)
        beforAwakenSkillNo = skillNo
      end
    elseif -1 ~= beforCombatSkillNo and nil ~= self.slots[self.eTabIndex.combat][beforCombatSkillNo] then
      self.slots[self.eTabIndex.combat][beforCombatSkillNo].mouseOverButton:EraseAllEffect()
      self.slots[self.eTabIndex.combat][beforCombatSkillNo].mouseOverButton:SetShow(false)
      beforCombatSkillNo = skillNo
    end
    if true == isOn then
      if __eSkillTypeParam_Inherit == skillType then
        if self.radioButtons[self.eTabIndex.succession]:IsCheck() == false then
          HandleMLUp_SkillWindow_UpdateData(self.eTabIndex.succession)
        end
        if nil ~= self.slots[self.eTabIndex.succession][skillNo] then
          self.slots[self.eTabIndex.succession][skillNo].mouseOverButton:EraseAllEffect()
          self.slots[self.eTabIndex.succession][skillNo].mouseOverButton:SetShow(true)
          self.slots[self.eTabIndex.succession][skillNo].mouseOverButton:SetIgnore(true)
          beforSuccessionSkillNo = skillNo
        end
      elseif __eSkillTypeParam_Awaken == skillType then
        if self.radioButtons[self.eTabIndex.awaken]:IsCheck() == false then
          HandleMLUp_SkillWindow_UpdateData(self.eTabIndex.awaken)
        end
        if nil ~= self.slots[self.eTabIndex.awaken][skillNo] then
          self.slots[self.eTabIndex.awaken][skillNo].mouseOverButton:EraseAllEffect()
          self.slots[self.eTabIndex.awaken][skillNo].mouseOverButton:SetShow(true)
          self.slots[self.eTabIndex.awaken][skillNo].mouseOverButton:SetIgnore(true)
          beforAwakenSkillNo = skillNo
        end
      else
        if self.radioButtons[self.eTabIndex.combat]:IsCheck() == false then
          HandleMLUp_SkillWindow_UpdateData(self.eTabIndex.combat)
        end
        if nil ~= self.slots[self.eTabIndex.combat][skillNo] then
          self.slots[self.eTabIndex.combat][skillNo].mouseOverButton:EraseAllEffect()
          self.slots[self.eTabIndex.combat][skillNo].mouseOverButton:SetShow(true)
          self.slots[self.eTabIndex.combat][skillNo].mouseOverButton:SetIgnore(true)
          beforCombatSkillNo = skillNo
        end
      end
    end
...
function PaGlobal_Skill:resetSkillWindowEffect()
  if -1 ~= beforSuccessionSkillNo and nil ~= self.slots[self.eTabIndex.succession][beforSuccessionSkillNo] then
    self.slots[self.eTabIndex.succession][beforSuccessionSkillNo].mouseOverButton:EraseAllEffect()
    self.slots[self.eTabIndex.succession][beforSuccessionSkillNo].mouseOverButton:SetShow(false)
    self.slots[self.eTabIndex.succession][beforSuccessionSkillNo].mouseOverButton:SetIgnore(true)
    beforSuccessionSkillNo = -1
  end
  if -1 ~= beforAwakenSkillNo and nil ~= self.slots[self.eTabIndex.awaken][beforAwakenSkillNo] then
    self.slots[self.eTabIndex.awaken][beforAwakenSkillNo].mouseOverButton:EraseAllEffect()
    self.slots[self.eTabIndex.awaken][beforAwakenSkillNo].mouseOverButton:SetShow(false)
    self.slots[self.eTabIndex.awaken][beforAwakenSkillNo].mouseOverButton:SetIgnore(true)
    beforAwakenSkillNo = -1
  end
  if -1 ~= beforCombatSkillNo and nil ~= self.slots[self.eTabIndex.combat][beforCombatSkillNo] then
    self.slots[self.eTabIndex.combat][beforCombatSkillNo].mouseOverButton:EraseAllEffect()
    self.slots[self.eTabIndex.combat][beforCombatSkillNo].mouseOverButton:SetShow(false)
    beforCombatSkillNo = -1
  end
end
function PaGlobal_Skill:SkillWindow_UpdateClearData()
  ...
  local slots
  for i = self.eTabIndex.combat, self.eTabIndex.loopCount do
    if true == self.radioButtons[i]:IsCheck() then
      slots = self.slots[i]
      break
    end
  end
  ...
  local skillPointInfo = ToClient_getSkillPointInfo(self.eTabIndex.combat)
...
function PaGlobal_Skill:SkillWindow_PlayerLearnableSkill()
  local slots = self.slots[self.eTabIndex.combat]
  for skillNo, slot in pairs(slots) do
    local skillLevelInfo = getSkillLevelInfo(skillNo)
    if nil ~= skillLevelInfo then
...
function SkillWindow_PlayerLearnableSkill()
  PaGlobal_Skill:SkillWindow_PlayerLearnableSkill()
end
...
function PaGlobal_Skill:setPreSkillList()
  self.preSkillList = {}
  local index = 1
  local slots = self.slots[self.eTabIndex.succession]
  for skillNo, slot in pairs(slots) do
    local skillLevelInfo = getSkillLevelInfo(skillNo)
    if nil == skillLevelInfo then
      return
    end
    local needSkill1 = getSkillStaticStatus(skillLevelInfo._needSkillNo1, 1)
    if nil ~= needSkill1 and __eSkillTypeParam_Normal == needSkill1:getSkillAwakenBranchType() then
      self.preSkillList[index] = skillLevelInfo._needSkillNo1
      index = index + 1
    end
    local needSkill2 = getSkillStaticStatus(skillLevelInfo._needSkillNo2, 1)
    if nil ~= needSkill2 and __eSkillTypeParam_Normal == needSkill2:getSkillAwakenBranchType() then
      self.preSkillList[index] = skillLevelInfo._needSkillNo2
      index = index + 1
    end
  end
end
function PaGlobal_Skill:SkillWindow_Show()
  ...
  if PaGlobal_AwakenSkill.isAwakenWeaponContentsOpen then
    self.radioButtons[self.eTabIndex.awaken]:SetMonoTone(false)
    self.radioButtons[self.eTabIndex.awaken]:SetEnable(true)
    if __eClassType_ShyWaman == classType then
      self.radioButtons[self.eTabIndex.awaken]:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SKILL_TALENT"))
    else
      self.radioButtons[self.eTabIndex.awaken]:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SKILL_RELEASEWEAPONE"))
    end
  else
    self.radioButtons[self.eTabIndex.awaken]:SetMonoTone(true)
    self.radioButtons[self.eTabIndex.awaken]:SetEnable(false)
  end
end
function PaGlobal_SkillWindow_GetConvertTabIndex(skillType)
  if __eSkillTypeParam_Inherit == skillType then
    return PaGlobal_Skill.eTabIndex.succession
  elseif __eSkillTypeParam_Awaken == skillType then
    return PaGlobal_Skill.eTabIndex.awaken
  else
    return PaGlobal_Skill.eTabIndex.combat
  end
end
