...
function HandleMLUp_SkillWindow_Close(isManualClick)
  PaGlobal_SkillGroup:setCloseFromDialog(true)
  PaGlobal_SkillGroup:close()
end
...
function FromClient_BeforeSelectSkillLevel()
  ...
  if self._currentTab == 3 or ToClient_isConsole() == true then
...
function PaGlobalFunc_SkillGroup_OnCreateUpgradeSkillListContent(content, key)
...
function PaGlobalFunc_SkillGroup_ShowUpgradeSkillEffectDescToolTip(isShow, skillIndex, effectIndex)
  local panel = Panel_Window_SkillGroup
  if nil == panel then
    return
  end
  ...
  if true == isShow then
    ...
    if nil == effectString then
      return
    end
    local content = self._upgradeSkillList:GetContentByKey(toInt64(0, skillIndex + 1))
    if nil == content then
      return
    end
...
function PaGlobalFunc_SkillGroup_OnClickedFindNpcButton()
  local panel = Panel_Window_SkillGroup
  if nil == panel then
    return
  end
  ...
  local selfPlayer = playerWrapper:get()
  if nil == selfPlayer then
    return
  end
  ToClient_DeleteNaviGuideByGroup(0)
  local spawnType = CppEnums.SpawnType.eSpawnType_SkillTrainer
  local position = playerWrapper:get3DPos()
  ...
  if TownfunctionNavi_Set ~= nil then
...
function PaGlobalFunc_SkillGroup_OnClickedGuideClose()
  local self = PaGlobal_SkillGroup
...
function FromClient_SkillGroupWindow_AllUpdate()
  ...
  PaGlobal_SkillGroup:refreshUpdateSkillList()
end
...
function FromClient_SkillGroupLearnedSkill(skillNo)
  ...
  PaGlobal_SkillGroup:updateSkillTooltip()
end
...
function SkillWindow_GetLearnSkillAlert(skillLevelInfo, branchType)
  if nil ~= branchType and __eSkillTypeParam_Inherit == branchType then
    ...
    if false == PaGlobalFunc_Util_IsSuccessionContentsOpen(selfPlayer:getClassType()) then
      return ""
    end
  end
...
function PaGlobal_SkillGroup_SelectTabForConsole(isNext)
  if PaGlobal_SkillGroup._ui._stc_skillShiftGuideMain:GetShow() == true then
    return
  end
...
function PaGlobalFunc_Skill_CloseShiftGuide()
  PaGlobal_SkillGroup._ui._stc_skillShiftGuideMain:SetShow(false)
end
function PaGlobalFunc_Skill_IsActiveItemSkill(skillKey)
...
function PaGlobalFunc_Skill_IsEquipActiveItemSkill(skillKey)
  if skillKey == nil then
    return false
  end
  if PaGlobalFunc_Skill_IsActiveItemSkill(skillKey) == false then
    return false
  end
  return ToClient_IsActiveItemSkillEquipBySkillKey(skillKey)
end
function PaGlobalFunc_Skill_IsEquipActiveItemSkillExcept()
  for index = 0, __eEquipSlotNoCount - 1 do
    local skillKey = ToClient_GetActiveItemSkillEquip(index)
    local isLearn = ToClient_IsActiveItemLearnSkillExceptAutoLearn(skillKey)
