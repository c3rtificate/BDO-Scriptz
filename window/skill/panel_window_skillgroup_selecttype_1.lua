...
function PaGlobal_SkillGroup_SelectType:show(selectTreeType)
  ...
  local ui = self._ui
  local moveAni = ui._stc_bigCharacter:addMoveAnimation(0, 0.3, CppEnums.PAUI_ANIM_ADVANCE_TYPE.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
  moveAni:SetStartPosition(ui._stc_centerMainArea:GetSizeX() / 2 - ui._stc_bigCharacter:GetSizeX() / 2 + ui._stc_bigCharacter:GetSizeX(), ui._stc_bigCharacter:GetPosY())
  local revision = 0
  if __eClassType_Corsair == classType and 2 == selectTreeType or __eClassType_WizardWoman == classType and 1 == selectTreeType then
    revision = 40
  elseif __eClassType_Combattant == classType and 1 == selectTreeType or __eClassType_Lhan == classType and 1 == selectTreeType then
    revision = 20
  end
  moveAni:SetEndPosition(ui._stc_centerMainArea:GetSizeX() / 2 - ui._stc_bigCharacter:GetSizeX() / 2 + revision, ui._stc_bigCharacter:GetPosY())
end
...
function PaGlobal_SkillGroup_SelectType:setClass(playerClassType, selectTreeType, skillNo)
  ...
  local currentData
  if selectTreeType == __eSkillTypeParam_Awaken then
    currentData = PaGlobalFunc_Util_GetClassTypeAwakenInfo(playerClassType)
  else
    currentData = PaGlobalFunc_Util_GetClassTypeSuccessionInfo(playerClassType)
  end
  local ui = self._ui
  ...
  ui._stc_bigCharacter:ChangeTextureInfoName(currentData._bigTexture)
  ui._txt_weaponTypeValue:SetText(currentData._weaponType)
  ui._txt_combatTypeValue:SetText(currentData._combatType)
  ...
  ui._txt_desc:SetText(currentData._desc)
  ...
  if __eClassType_Corsair == playerClassType then
