    ...
    for col = 0, cols - 1 do
      ...
      if true == cell:isSkillType() then
        ...
        for fusionIndex = 0, maxFusionSkill - 1 do
          ...
          if 1 == table[skillIndex]._learndLevelMainSkill and 1 == table[skillIndex]._learndLevelSubSkill and self._selfPlayerLevel >= table[skillIndex]._needCharacterLevel and self._remainSkillPoint >= table[skillIndex]._needSkillPoint and false == ToClient_isLearnedSkill(fusionSkillNo) and false == ToClient_isUsedSkillForFusionSkill(fusionMainNo) then
...
function PaGlobalFunc_Skill_ToggleBlackSkillLock()
  local self = Window_SkillInfo
  local skillInfo = self._currentSkillListInfo[self._currentSkillIndex]
  if nil == skillInfo then
    return
  ...
  local skillLevelInfo = getSkillLevelInfo(skillInfo._no)
  local skillTypeStaticWrapper = getSkillTypeStaticStatus(skillInfo._blackSkillNo)
...
function Window_SkillInfo:checkIsBlackSkill(id)
  local skillInfo = self._currentSkillListInfo[id]
  local blackSkillTypeSS = getSkillTypeStaticStatus(skillInfo._blackSkillNo)
  if 0 == skillInfo._blackSkillNo or nil == blackSkillTypeSS or true ~= blackSkillTypeSS:isValidLocalizing() then
    return false
  end
  for idx = 1, self._blackTable._skillCount do
    if nil ~= skillInfo._no and skillInfo._no == self._blackTable._skillTable[idx] then
      return true
    end
  end
  return false
end
function PaGlobalFunc_Skill_CheckIsBlackSkill(skillNo, blackSkillNo)
  local self = Window_SkillInfo
  local blackSkillTypeSS = getSkillTypeStaticStatus(blackSkillNo)
  if 0 == blackSkillNo or nil == blackSkillTypeSS or true ~= blackSkillTypeSS:isValidLocalizing() then
    return false
  end
  for idx = 1, self._blackTable._skillCount do
    if skillNo == self._blackTable._skillTable[idx] then
      return true
    end
  end
  return false
end
function Window_SkillInfo:checkIsShowBlackSkillLock(id)
  local skillInfo = self._currentSkillListInfo[id]
  if true == skillInfo._isLock then
    return false
  end
  local skillLevelInfo = getSkillLevelInfo(skillInfo._no)
  if nil ~= skillLevelInfo then
...
function Window_SkillInfo:setBlackSkill()
  ...
  for idx = 0, num - 1 do
    ...
    if nil ~= skillWrapper then
      ...
      if true == skillLevelInfo._usable and true == skillWrapper:isUseableActiveSkill() then
        count = count + 1
        self._blackTable._skillTable[count] = skillNo
      end
    end
  end
  self._blackTable._skillCount = count
end
...
function PaGlobalFunc_Skill_SelectTitle(titleType)
  ...
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(self._rightKeyGuideBtnGroup, self._ui._right._static_KeyGuide, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_AUTO_WRAP_LEFT, nil, 10)
end
...
function PaGlobalFunc_Skill_List2EventControlCreate(list_content, key)
  ...
  local uiInfo = {
    ...
    _static_blackSkillLockIcon = UI.getChildControl(list_content, "Static_BlackSkill_Lock_Icon")
  }
  ...
  local blackSkillTypeSS = getSkillTypeStaticStatus(skillInfo._blackSkillNo)
  if 0 ~= skillInfo._blackSkillNo and nil ~= blackSkillTypeSS and true == blackSkillTypeSS:isValidLocalizing() then
  ...
  local isBlackSkill = self:checkIsBlackSkill(id)
  if true == isBlackSkill then
    local isBlackSkillLock = self:checkIsShowBlackSkillLock(id)
    uiInfo._static_blackSkillLockIcon:SetShow(isBlackSkillLock)
    if _ContentsGroup_NewUI_BlackSpiritSkillLock_All then
      self._ui._right._radioButton_learnSkillKey:SetShow(true)
      self._ui._right._radioButton_learnSkillKey:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_SKILL_BLACKSPIRIT_LOCK"))
      Panel_Window_Skill:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobalFunc_Skill_ToggleBlackSkillLock()")
    end
  else
    uiInfo._static_blackSkillLockIcon:SetShow(false)
    if true == self._isAllSkillShow then
      self._ui._right._radioButton_learnSkillKey:SetShow(false)
    end
    self._ui._right._radioButton_learnSkillKey:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_COMBINATION_LEARNBUTTON"))
    Panel_Window_Skill:registerPadEvent(__eConsoleUIPadEvent_Up_A, "")
  end
...
function PaGlobalFunc_Skill_SelectSkill(id, isBlackSkillDesc)
  ...
  body._staticText_skillCommand:SetTextMode(__eTextMode_AutoWrap)
...
function Window_SkillInfo:InitControl()
  ...
  body._staticText_Desc:SetTextMode(__eTextMode_AutoWrap)
  ...
  body._staticText_EffectDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  body._radioButton_Close = UI.getChildControl(body._static_KeyGuide, "Radiobutton_SkillPoint_Close_Key")
  body._radioButton_Close:SetPosX(body._txt_toggleBlackSkillDesc:GetPosX() + body._txt_toggleBlackSkillDesc:GetTextSizeX() + 50)
  ...
  body._staticText_Command:SetTextMode(__eTextMode_AutoWrap)
  ...
  if classType == __eClassType_ShyWaman then
    right._radioButton_SkillAwaken:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_SKILLREINFORCE_AWAKEN_BYSHY"))
  end
  ...
  skillContentTitle:SetTextMode(__eTextMode_LimitText)
  ...
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(self._rightKeyGuideBtnGroup, right._static_KeyGuide, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_AUTO_WRAP_LEFT, nil, 10)
  if false == PaGlobalFunc_Util_GetIsAwakenWeaponContentsOpen(classType) then
    self._maxTabIndex = 0
...
function Window_SkillInfo:InitEvent()
  ...
  right._list2_Skill:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_Skill_List2EventControlCreate")
  right._list2_Skill:createChildContent(__ePAUIList2ElementManagerType_List)
  body._radioButton_Close:addInputEvent("Mouse_LUp", "PaGlobalFunc_Skill_Close()")
...
function PaGlobalFunc_Skill_UpdateForBlackSpiritLock()
  local self = Window_SkillInfo
  self:Update()
end
