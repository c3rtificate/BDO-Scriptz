...
PaGlobal_Util_RegistHelpTooltipEvent(skillReinforce.btnQuestion, "\"PanelSkillAwaken\"")
...
function skillReinforce:Init()
  ...
  if 0 == ToClient_getGameOptionControllerWrapper():getUIFontSizeType() then
    self.skillListDesc:SetTextMode(__eTextMode_Limit_AutoWrap)
    self.skillListDesc:setLineCountByLimitAutoWrap(2)
  else
    self.skillListDesc:SetTextMode(__eTextMode_LimitText)
    self.skillListDesc:setLineCountByLimitAutoWrap(1)
  end
  self.list2_SkillList:registEvent(__ePAUIList2EventType_LuaChangeContent, "SkillListControlCreate")
  self.list2_SkillList:createChildContent(__ePAUIList2ElementManagerType_List)
  self.list2_OptionList:registEvent(__ePAUIList2EventType_LuaChangeContent, "OptionListControlCreate")
  self.list2_OptionList:createChildContent(__ePAUIList2ElementManagerType_List)
  self.list2_OptionList2:registEvent(__ePAUIList2EventType_LuaChangeContent, "OptionListControlCreate2")
  self.list2_OptionList2:createChildContent(__ePAUIList2ElementManagerType_List)
...
function skillReinforce:update(skillType, skillNo, optionIndex, reinforceSkillIndex, skillIndex)
  ...
  self.optionListDesc:SetTextMode(__eTextMode_AutoWrap)
  self.optionListDesc2:SetTextMode(__eTextMode_AutoWrap)
...
function SkillReinforce_SkillList_Show()
  ...
  if __eSkillTypeParam_Inherit == _type then
    count = count + Reinforcable_SkillCount(__eSkillTypeParam_Normal)
  end
  if 0 == count then
...
function SkillListControlCreate(content, key)
  ...
  if 0 == ToClient_getGameOptionControllerWrapper():getUIFontSizeType() then
    skillListDesc:SetTextMode(__eTextMode_Limit_AutoWrap)
    skillListDesc:setLineCountByLimitAutoWrap(2)
  else
    skillListDesc:SetTextMode(__eTextMode_LimitText)
  ...
  for index = 0, reinforcableCount - 1 do
    ...
    if _type == awakeningType or __eSkillTypeParam_Inherit == _type and __eSkillTypeParam_Normal == awakeningType then
      if count == _key then
        local skillTypeSSW = skillSSW:getSkillTypeStaticStatusWrapper()
        skillNo = skillSSW:getSkillNo()
        skillListName:SetTextMode(__eTextMode_AutoWrap)
        skillListDesc:SetTextMode(__eTextMode_AutoWrap)
        skillListDesc:SetTextMode(__eTextMode_Limit_AutoWrap)
        skillListDesc:setLineCountByLimitAutoWrap(2)
...
function skillReinforce_SetSkill(skillNo, index)
  ...
  local skillTypeSSW = skillSSW:getSkillTypeStaticStatusWrapper()
  self.skillListName:SetTextMode(__eTextMode_AutoWrap)
  self.skillListDesc:SetTextMode(__eTextMode_AutoWrap)
  self.skillListName:SetText(tostring(skillSSW:getName()))
  self.skillListDesc:SetTextMode(__eTextMode_Limit_AutoWrap)
  self.skillListDesc:setLineCountByLimitAutoWrap(2)
  self.skillListDesc:SetText(tostring(skillTypeSSW:getDescription()))
      ...
      for index = 0, optionCount - 1 do
        ...
        if nil == self._currentOptionIndex2 then
          if index == _key then
            ...
            if optionListDesc:GetSizeX() < optionListDesc:GetTextSizeX() + 20 then
...
function SkillReinforce_Doit()
  ...
  if nil == skillReinforce._beforeSkillIndex then
    if _isAddOption then
      ToClient_RequestChangeAwakeningBitFlag(__eSkillAwakeningActorType_NonPlayer, skillReinforce._currentSkillIndex, skillReinforce._currentOptionIndex, skillReinforce._currentOptionIndex2)
    else
      ToClient_RequestSkillAwakening(__eSkillAwakeningActorType_NonPlayer, skillReinforce._currentSkillIndex, skillReinforce._currentOptionIndex, skillReinforce._currentOptionIndex2)
    end
  else
    ToClient_RequestChangeAwakeningSkill(__eSkillAwakeningActorType_NonPlayer, skillReinforce._beforeSkillIndex, skillReinforce._currentSkillIndex, skillReinforce._currentOptionIndex, skillReinforce._currentOptionIndex2)
  end
...
function Panel_SkillReinforce_Show(skillType, skillNo, skillIndex)
  ...
  local applyBookOfCombat = selfPlayer:get():isApplyChargeSkill(UI_BUFFTYPE.eUserChargeType_BookOfCombat)
  local inventory = selfPlayer:get():getInventory()
  local hasMemoryFlag = inventory:getItemCount_s64(ItemEnchantKey(44195, 0))
  local reinforcableCount = Reinforcable_SkillCount(skillType)
  if __eSkillTypeParam_Inherit == skillType then
    reinforcableCount = reinforcableCount + Reinforcable_SkillCount(__eSkillTypeParam_Normal)
  end
  ...
  if nil ~= skillNo then
    if toInt64(0, 0) == hasMemoryFlag and not applyAwakenSkillReset and not applyBookOfCombat then
...
function Panel_SkillReinforce_Close()
  Panel_SkillReinforce:ClearUpdateLuaFunc()
