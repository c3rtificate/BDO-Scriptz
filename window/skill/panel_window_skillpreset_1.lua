...
function PaGlobal_SkillPreset:registEventHandler()
  ...
  registerEvent("FromClient_FinishSkillPresetChangeProgressBar", "FromClient_FinishSkillPresetChangeProgressBar")
end
...
function PaGlobal_SkillPreset:applySkillPresetFromCooltimeBar()
  if self._selectSlot < 0 then
    return
  end
  ToClient_requestApplySkillPresetFromCoolTimeBar(self._selectSlot)
  self._selectSlot = -1
end
...
function PaGlobal_SkillPreset:applyConfirmMessageBox()
  local messageBoxData = {
    ...
    content = PAGetString(Defines.StringSheet_GAME, "LUA_SKILLPRESET_APPLY"),
