...
function PaGlobal_ItemFilter_All:updateSlot(isInit)
  ...
  self._familyInventoryItemCount = 0
  self:initRadioSlot()
  local selfPlayer = getSelfPlayer():get()
  if nil == selfPlayer then
    return
  end
  if true == _ContentsGroup_UseFamilyInventoryItem then
    ...
    for invenSlotNo = __eTInventorySlotNoUseStart, familyInventorySize - 1 do
      local itemWrapper = getInventoryItemByType(CppEnums.ItemWhereType.eFamilyInventory, invenSlotNo)
      if nil ~= itemWrapper then
        ...
        if nil ~= itemSSW and self._eType == itemSSW:getServantSupplyItemType() then
          ...
          if true == guildFliter then
            ...
            slotItem:setItem(itemWrapper, invenSlotNo, nil, nil, true)
            rdo_slot:SetShow(true)
            self._slotIdxList:push_back(invenSlotNo)
            self._familyInventoryItemCount = self._familyInventoryItemCount + 1
          end
        end
      end
    end
...
function PaGlobal_ItemFilter_All:getRadioSlot()
  ...
  if nil == self._rdo_slots[self._lastSlotIndex] then
    ...
    if true == _ContentsGroup_UsePadSnapping then
      slot.icon:addInputEvent("Mouse_On", "HandleEventMLUp_ItemFilter_ItemSelect(" .. self._lastSlotIndex .. ")")
...
function PaGlobal_ItemFilter_All:updateItemSpecText(selectIndex)
  ...
  if selectIndex <= PaGlobal_ItemFilter_All._familyInventoryItemCount then
    whereType = CppEnums.ItemWhereType.eFamilyInventory
  end
