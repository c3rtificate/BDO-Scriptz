...
function PaGlobalFunc_ShipInfoAll_ViewServantUpgrade()
...
function HandleEventOnOut_ShipInfo_All_EmptySlot_Tooltip(isShow, index, isCash)
  if nil == Panel_Window_ShipInfo_All then
    return
  end
  ...
  local self = PaGlobal_ShipInfo_All
...
function HandlerEventMouseOn_ShipInfoAll_EquipItem(index, isOn, isCash)
  ...
  if true == _ContentsGroup_RenewUI_Tooltip then
    ...
    if true == isOn then
      ...
      if slotNo == __eEquipSlotNoHelm and true == PaGlobal_ShipInfo_All._isBigShip then
    ...
    if true == _ContentsGroup_UsePadSnapping and slotNo == __eEquipSlotNoHelm and true == PaGlobal_ShipInfo_All._isBigShip then
      slotNo = __eEquipSlotNoChest
    end
...
function PaGlobalFunc_ShipInfoAll_CreateSkillList(control, key)
  ...
  if nil ~= chk_skillLockIcon then
    local skillNameSpanSizeX = PaGlobal_ShipInfo_All._skillNameBaseSpanSizeX
    local gabX = 4
