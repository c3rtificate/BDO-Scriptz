...
function servantIcon:init()
  ...
  for ii = 0, self.config.slotCount do
    local slot = {}
    slot.icon = UI.createControl(__ePAUIControl_Static, Panel_Window_Servant, "ServantIcon_Icon_" .. ii)
    slot.hpBackground = UI.createControl(__ePAUIControl_Static, slot.icon, "ServantIcon_HpBackground_" .. ii)
    slot.mpBackground = UI.createControl(__ePAUIControl_Static, slot.icon, "ServantIcon_MpBackground_" .. ii)
    slot.hp = UI.createControl(__ePAUIControl_Progress2, slot.icon, "ServantIcon_Hp_" .. ii)
    slot.mp = UI.createControl(__ePAUIControl_Progress2, slot.icon, "ServantIcon_Mp_" .. ii)
...
function servantIcon:registMessageHandler()
  ...
  registerEvent("FromClient_ApplyUISettingPanelInfo", "ServantIcon_Resize")
end
...
function ServantIcon_Resize()
  ...
  local relativePosX = Panel_Window_Servant:GetRelativePosX()
  local relativePosY = Panel_Window_Servant:GetRelativePosY()
  ...
  elseif true == _ContentsGroup_RenewUI then
    Panel_Window_Servant:SetRelativePosX(relativePosX)
    Panel_Window_Servant:SetRelativePosY(relativePosY)
    Panel_Window_Servant:SetPosX(getOriginScreenSizeX() * relativePosX - Panel_Window_Servant:GetSizeX() / 2)
    Panel_Window_Servant:SetPosY(getOriginScreenSizeY() * relativePosY - Panel_Window_Servant:GetSizeY() / 2)
  else
    Panel_Window_Servant:SetPosX(getScreenSizeX() * relativePosX - Panel_Window_Servant:GetSizeX() / 2)
    Panel_Window_Servant:SetPosY(getScreenSizeY() * relativePosY - Panel_Window_Servant:GetSizeY() / 2)
  end
  if false == _ContentsGroup_RenewUI then
    if 0 < ToClient_GetUiInfo(CppEnums.PAGameUIType.PAGameUIPanel_ServantWindow, 0, CppEnums.PanelSaveType.PanelSaveType_IsSaved) then
      local isShow = ToClient_GetUiInfo(CppEnums.PAGameUIType.PAGameUIPanel_ServantWindow, 0, CppEnums.PanelSaveType.PanelSaveType_IsShow)
      Panel_Window_Servant:SetShow(isShow)
    end
  else
    ...
    if 0 == isShow then
      ...
      PaGlobalFunc_TopIcon_ShowAll(false)
    else
      Panel_Window_Servant:SetShow(true)
      PaGlobalFunc_TopIcon_ShowAll(true)
    end
  end
...
function Servant_Call(index)
  ...
  if true == _ContentsGroup_PersonalField and true == ToClient_IsEnterMirrorField(__eMirrorFieldContentsKey_PersonalField) then
    local instanceSpawnWrapper = ToClient_getInstanceSpawnInfoWrapperByPosition(servantInfo:getPosition())
    ...
    if nil ~= selfPlayerWrapper and nil ~= selfPlayerWrapper:get() then
      local selfPlayerPosition = selfPlayerWrapper:get():getPosition()
      local playerInstanceSpawnWrapper = ToClient_getInstanceSpawnInfoWrapperByPosition(selfPlayerPosition)
      if nil ~= playerInstanceSpawnWrapper and instanceSpawnWrapper:getIndex() ~= playerInstanceSpawnWrapper:getIndex() then
        Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantCallServant"))
        return
      end
    end
...
function Servant_CallHelp(posX, posY, slotNo)
  if 0 == slotNo then
    ...
    servantIcon._statictext_Help:SetTextMode(__eTextMode_AutoWrap)
...
function ServantIcon_Close()
  ...
  if not isShow and false == _ContentsGroup_RenewUI then
    Panel_Window_Servant:SetShow(false, false)
  end
  servantIcon:update()
end
