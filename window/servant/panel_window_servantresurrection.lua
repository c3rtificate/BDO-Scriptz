...
selectTitle:SetTextMode(__eTextMode_AutoWrap)
...
function FromClient_ServantResurrectionAck(servantNo, servantWhereType)
  if servantWhereType ~= CppEnums.ServantWhereType.ServantWhereTypeGuild then
    ...
    if nil ~= servantInfo then
      ...
      PaGlobalFunc_ServantIcon_SetServantIconByState(false, servantInfo:getVehicleType())
    else
      for idx = 0, 2 do
        local crogWrapper = ToClient_getServantCrogdaloByIndex(idx)
        if crogWrapper ~= nil and crogWrapper:getServantNo() == servantNo then
          Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_SERVANTRESURRECTION_MSG"))
          PaGlobalFunc_ServantIcon_SetServantIconByState(false, crogWrapper:getVehicleType())
          if nil ~= PaGlobal_Window_CrogdaloStable then
