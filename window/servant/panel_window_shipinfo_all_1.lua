function PaGlobal_ShipInfo_All:initialize()
  ...
  self._ui.btn_buyItem = UI.getChildControl(self._ui.stc_sailor_rightBG, "Button_BuyItem")
  self:initLimitText()
...
function PaGlobal_ShipInfo_All:registerEventHandler()
  ...
  if false == self._isConsole then
    ...
    self._ui.btn_Upgrade:addInputEvent("Mouse_LUp", "PaGlobalFunc_ShipInfoAll_ViewServantUpgrade()")
    ...
    self._ui.btn_buyItem:addInputEvent("Mouse_LUp", "PaGlobal_EasyBuy:Open(114)")
  else
    Panel_Window_ShipInfo_All:registerPadEvent(__eConsoleUIPadEvent_LB, "HandlerEventLUp_ShipInfoAll_RadioButtonSelect()")
    Panel_Window_ShipInfo_All:registerPadEvent(__eConsoleUIPadEvent_RB, "HandlerEventLUp_ShipInfoAll_RadioButtonSelect()")
    self._ui.btn_foodEmergency:addInputEvent("Mouse_On", "HandleEventMOn_ShipInfoAll_ShipItemFilterTooltip(true)")
    self._ui.btn_foodEmergency:addInputEvent("Mouse_Out", "HandleEventMOn_ShipInfoAll_ShipItemFilterTooltip(false)")
    ...
    self._ui.btn_Upgrade:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobalFunc_ShipInfoAll_ViewServantUpgrade()")
    ...
    self._ui.stc_alert:addInputEvent("Mouse_On", "HandleEventMOn_ShipInfoAll_WeightTooltip(PaGlobal_ShipInfo_All._ui.stc_alert, true)")
    self._ui.stc_alert:addInputEvent("Mouse_Out", "HandleEventMOn_ShipInfoAll_WeightTooltip(PaGlobal_ShipInfo_All._ui.stc_alert, false)")
    self._ui.btn_buyItem:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobal_EasyBuy:Open(114)")
  end
  registerEvent("FromClient_updateShipStat", "FromClient_ShipInfo_ShipStatChangedByBuff")
  registerEvent("EventSelfPlayerRideOn", "FromClient_ShipInfo_ShipStatChangedByBuff")
  registerEvent("EventSelfPlayerRideOff", "FromClient_ShipInfo_ShipStatChangedByBuff")
  ...
  registerEvent("FromClient_notifyShipWeakenItem", "FromClient_ShipInfo_notifyShipWeakenItem")
...
function PaGlobal_ShipInfo_All:basic_createSlots()
  ...
  for ii = 0, self._config.CONST_COUNT - 1 do
    ...
    self._invenSlotBG[ii]:registerPadEvent(__eConsoleUIPadEvent_RStickUp, "HandlerEventScrollDown_ShipInfoAll_ShipInven(true)")
    self._invenSlotBG[ii]:registerPadEvent(__eConsoleUIPadEvent_RStickDown, "HandlerEventScrollDown_ShipInfoAll_ShipInven(false)")
...
function PaGlobal_ShipInfo_All:setControlShow()
  ...
  if true == _ContentsGroup_UsePadSnapping then
    ...
    self._ui.stc_LB_Console:SetShow(true)
    self._ui.stc_RB_Console:SetShow(true)
    self._ui._stc_KeyGuideBG:SetShow(true)
    self:setAlignKeyGuide()
  else
    self._ui.stc_LB_Console:SetShow(false)
    self._ui.stc_RB_Console:SetShow(false)
...
function PaGlobal_ShipInfo_All:basic_Update()
  ...
  PaGlobal_ShipInfo_All:updateShipSkill()
  PaGlobal_ShipInfo_All:updateShipInventory()
end
function PaGlobal_ShipInfo_All:updateShipInformation()
  if nil == Panel_Window_ShipInfo_All then
    return
  end
...
function PaGlobal_ShipInfo_All:updateShipStat()
  if nil == Panel_Window_ShipInfo_All then
    return
  end
  local servantWrapper = getServantInfoFromActorKey(self._actorKeyRaw)
...
function PaGlobal_ShipInfo_All:updateCannonStat()
  if nil == Panel_Window_ShipInfo_All then
    return
  end
  if nil == Panel_Window_ShipInfo_All then
...
function PaGlobal_ShipInfo_All:updateShipEquipment()
  if nil == Panel_Window_ShipInfo_All then
    return
  end
...
function PaGlobal_ShipInfo_All:updateShipSkill()
  if nil == Panel_Window_ShipInfo_All then
    return
  end
  local servantWrapper = getServantInfoFromActorKey(self._actorKeyRaw)
...
function PaGlobal_ShipInfo_All:updateShipInventory()
  if nil == Panel_Window_ShipInfo_All then
    return
  end
  local vehicleActorWrapper = getVehicleActor(self._actorKeyRaw)
...
function PaGlobal_ShipInfo_All:equipSlotUpdate(isCashSlot)
  ...
  for ii = 1, self._equipSlotMaxCount do
    ...
    if true == _ContentsGroup_UsePadSnapping then
      ...
      slotBG[ii]:addInputEvent("Mouse_On", "HandleEventOnOut_ShipInfo_All_EmptySlot_Tooltip(true, " .. ii .. ", " .. tostring(isCashSlot) .. ")")
      slotBG[ii]:addInputEvent("Mouse_Out", "HandleEventOnOut_ShipInfo_All_EmptySlot_Tooltip()")
    ...
    elseif true == _ContentsGroup_UsePadSnapping then
      slotBG[ii]:registerPadEvent(__eConsoleUIPadEvent_Up_A, "HandlerEventRUp_ShipInfoAll_EquipItem(" .. slotNo .. ")")
    else
      slotBG[ii]:addInputEvent("Mouse_LUp", "HandlerEventRUp_ShipInfoAll_EquipItem(" .. slotNo .. ")")
    end
  end
...
function PaGlobal_ShipInfo_All:prepareOpen(actorKeyRaw, isBigShip)
  ...
  local isShow = Panel_Window_ShipInfo_All:GetShow()
  self._actorKeyRaw = actorKeyRaw
  if true == self._isConsole then
    self._ui.radio_basicInfo:SetCheck(false)
    self._ui.radio_sailorInfo:SetCheck(true)
  else
  ...
  if true == _ContentsGroup_UsePadSnapping then
    if servantWrapper:getSortedSkillCountForDisplay() <= 0 then
      ...
      self._ui._txt_KeyGuideLS:SetShow(false and true == self._ui.radio_basicInfo:IsCheck())
    else
      self._ui._txt_KeyGuideLS:SetShow(true == self._ui.radio_basicInfo:IsCheck())
...
function PaGlobal_ShipInfo_All:inventory_SetFunctor(slotNo, itemWrapper, s64_count, inventoryType)
  ...
  function PaGlobal_ShipInfo_All_InventoryUseItem()
    inventoryUseItem(PaGlobal_ShipInfo_All._inventoryType, PaGlobal_ShipInfo_All._inventorySlotNo, __eEquipSlotNoCount, false)
  end
...
function PaGlobal_ShipInfo_All:ShipInfo_CheckSkillLock(actorKeyRaw, key32)
  if nil == Panel_Window_ShipInfo_All then
    return
  end
  if true == _ContentsGroup_UsePadSnapping then
...
function PaGlobal_ShipInfo_All:updateServantSkillLock(index, isClickedBg)
  local servantWrapper = getServantInfoFromActorKey(self._actorKeyRaw)
  if nil == servantWrapper then
    return
  end
