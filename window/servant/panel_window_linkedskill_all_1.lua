function PaGlobal_LinkedSkill_All:initialize()
  ...
  self._ui._txt_skillBgTitle = UI.getChildControl(Panel_Window_LinkedSkill_All, "StaticText_SkillTitle")
  ...
  self._ui._stc_skillStallionIcon = UI.getChildControl(Panel_Window_LinkedSkill_All, "Static_SkillStallionIcon")
  self:init_skill()
  self:init_equipSlot()
  self:setShowControl()
...
function PaGlobal_LinkedSkill_All:registEventHandler()
  ...
  registerEvent("FromClient_ServantPreviewEquipmentUpdate", "PaGlobal_LinkedSkill_All_UpdateEquip()")
end
...
function PaGlobal_LinkedSkill_All:init_skill()
  ...
  for idx = 0, self._loopCount do
    ...
    tempSkill._stc_skillStallionIcon = UI.createControl(__ePAUIControl_Static, tempSkill._stc_skillBg, "Static_SkillStallionIcon_" .. idx)
    CopyBaseProperty(self._ui._stc_skillStallionIcon, tempSkill._stc_skillStallionIcon)
    tempSkill._stc_skillStallionIcon:SetPosX(self._ui._stc_skillIcon:GetSizeX() + self._skillConfig.gapStallionIcon)
    tempSkill._stc_skillStallionIcon:SetPosY(self._skillConfig.startStallionIconY)
    tempSkill._stc_skillStallionIcon:SetShow(true)
...
function PaGlobal_LinkedSkill_All:updateSkill()
    ...
    if nil ~= skillWrapper then
      if startIndexCheck == self._skillStartIndex then
        if learnSkillCount < self._skillConfig.count then
          ...
          slot._stc_skillIcon:SetMonoTone(false)
          local isStallionSkill = servantInfo:isStallionSkill(skillWrapper:getKey())
          slot._stc_skillStallionIcon:SetShow(isStallionSkill)
          slot._stc_skillStallionIcon:SetMonoTone(false)
          local skillNamePosX = self._ui._stc_skillIcon:GetSizeX() + self._skillConfig.gapNameX
          if true == isStallionSkill then
            local gabX = 5
            skillNamePosX = skillNamePosX + slot._stc_skillStallionIcon:GetSizeX() - gabX
            if false == _ContentsGroup_UsePadSnapping then
              slot._stc_skillStallionIcon:addInputEvent("Mouse_On", "HandleEventOnOut_LinkedSkill_ShowStallionToolTip(true," .. learnSkillCount .. ")")
              slot._stc_skillStallionIcon:addInputEvent("Mouse_Out", "HandleEventOnOut_LinkedSkill_ShowStallionToolTip(false)")
            end
          end
          slot._txt_skillName:SetPosX(skillNamePosX)
          slot._txt_skillName:SetText(skillWrapper:getName())
          slot._txt_skillName:SetMonoTone(false)
          ...
          slot._progress_skillLearnGauge:SetShow(true)
          ...
          slot._txt_skillPercent:SetShow(true)
  ...
  for skillIndex = 0, skillCount - 1 do
    local skillWrapperXXX = servantInfo:getSkillXXX(skillIndex)
    if nil ~= skillWrapperXXX then
      if startIndexCheck == self._skillStartIndex then
        if learnSkillCount < self._skillConfig.count then
          local slot = self._skillSlot[learnSkillCount]
          slot._stc_skillBg:SetShow(true)
          slot._stc_skillIcon:ChangeTextureInfoName("Icon/" .. skillWrapperXXX:getIconPath())
          slot._stc_skillIcon:SetMonoTone(true)
          local isStallionSkill = servantInfo:isStallionSkill(skillWrapperXXX:getKey())
          slot._stc_skillStallionIcon:SetShow(isStallionSkill)
          slot._stc_skillStallionIcon:SetMonoTone(true)
          local skillNamePosX = self._ui._stc_skillIcon:GetSizeX() + self._skillConfig.gapNameX
          if true == isStallionSkill then
            local gabX = 5
            skillNamePosX = skillNamePosX + slot._stc_skillStallionIcon:GetSizeX() - gabX
          ...
          slot._txt_skillName:SetPosX(skillNamePosX)
          slot._txt_skillName:SetText(skillWrapperXXX:getName())
          slot._txt_skillName:SetMonoTone(true)
          slot._txt_skillCondition:SetText(skillWrapperXXX:getDescription())
          UI.setLimitTextAndAddTooltip(slot._txt_skillCondition)
          slot._progress_skillLearnGauge:SetShow(false)
          local percent = tonumber(string.format("%.0f", servantInfo:getSkillExp(skillIndex) / (skillWrapperXXX:get()._maxExp * 0.01)))
          if percent > 100 then
            percent = 100
          end
          slot._txt_skillPercent:SetShow(false)
          slot._stc_skillIcon:addInputEvent("Mouse_On", "HandleEventOn_LinkedSkill_SkillTooltip(" .. learnSkillCount .. ", " .. skillIndex .. ")")
          slot._stc_skillIcon:addInputEvent("Mouse_Out", "HandleEventOut_LinkedSkill_SkillTooltip()")
          slot.skillIndex = skillIndex
          learnSkillCount = learnSkillCount + 1
  ...
  local SkillCountForDisplay = servantInfo:getSortedSkillCountForDisplay()
  local skillCountString = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_SERVANTINFO_SKILLCOUNT", "servantSkillListCnt", SkillCountForDisplay .. " / " .. self._skillCount)
...
function PaGlobal_LinkedSkill_All:updateEquipData(equipList)
  ...
  for index, value in pairs(equipList) do
    local equipSlotNo = value._index
    local slot = self._equipSlot[equipSlotNo]
    local itemWrapper = servantInfo:getEquipItem(equipSlotNo)
    if nil ~= itemWrapper then
      slot:setItem(itemWrapper)
      slot.icon:SetIgnore(false)
      slot.icon:addInputEvent("Mouse_RUp", "HandleEventRUp_LinkedSkill_All_UnEquip(" .. equipSlotNo .. ")")
    else
      slot:clearItem()
...
function PaGlobal_LinkedSkill_All:repostion()
  ...
  local posX = Panel_Window_VehicleInfo_All:GetPosX() + 5
