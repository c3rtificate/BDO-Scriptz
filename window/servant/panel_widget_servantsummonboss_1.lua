function PaGlobal_ServantSummonBoss:initialize()
  ...
  self._ui._stc_actionTitle = UI.getChildControl(self._ui._stc_commandBG, "StaticText_ActionTitle")
  ...
  for ii = 1, self._MAX_ACTION_COUNT do
    self._ui._actionList[ii] = UI.createAndCopyBasePropertyControl(self._ui._stc_commandBG, "StaticText_ActionBasic", self._ui._stc_commandBG, "StaticText_ActionBasic_" .. ii)
    self._ui._actionList[ii]:SetShow(false)
  end
...
function PaGlobal_ServantSummonBoss:prepareOpen()
  ...
  for index = 1, #self._bossInfo do
    if self._bossInfo[index]._actorkey == actorKeyRaw then
      isHideHorseHP = self._bossInfo[index]._isHideServantHP
    end
  end
  self._isShowSkillCommand = false
  if nil ~= Panel_SkillCommand and true == Panel_SkillCommand:GetShow() or nil ~= isHideHorseHP and true == isHideHorseHP then
  ...
  if instanceMapKey == __eInstanceLocalWar_PapuCrio then
    local teamNo = ToClient_GetMyTeamNoLocalWar()
    if 0 == teamNo then
      FromAction_ServantSummonBoss_CrioStackActive()
    elseif 1 == teamNo then
      FromAction_ServantSummonBoss_PapuStackActive()
    end
  end
  self:open()
end
...
function PaGlobal_ServantSummonBoss:commandTextSet()
  ...
  for index = 1, #self._bossInfo do
    if self._bossInfo[index]._actorkey == getBossKey then
      ...
      actionCnt = #self._bossInfo[index]._actionText
    end
  end
  if 0 == bossIndex then
    return
  ...
  self._ui._stc_actionTitle:SetText(self._actionTitleStr)
  for index = 1, #self._flyTitleMonsterKey do
    if self._flyTitleMonsterKey[index] == getBossKey then
      self._ui._stc_actionTitle:SetText(self._flyTitleStr)
      break
    end
  end
  local lastSpanY = self._ui._stc_moveTitle:GetSpanSize().y
  local baseTextSizeY = self._ui._stc_moveTitle:GetTextSizeY()
  local maxSizeX = 0
  if moveCnt > 0 then
    ...
    for ii = 1, moveCnt do
      ...
      local addSizeY = baseTextSizeY
      local gap = 0
      if baseTextSizeY < self._ui._moveList[ii]:GetTextSizeY() then
        addSizeY = 35
        gap = 15
      end
      self._ui._moveList[ii]:SetSpanSize(self._ui._moveList[ii]:GetSpanSize().x, lastSpanY + addSizeY)
      lastSpanY = lastSpanY + addSizeY + gap
  ...
  if attackCnt > 0 then
    ...
    self._ui._stc_attackTitle:SetSpanSize(self._ui._stc_attackTitle:GetSpanSize().x, lastSpanY + baseTextSizeY)
    lastSpanY = lastSpanY + baseTextSizeY
    for ii = 1, attackCnt do
      ...
      local addSizeY = baseTextSizeY
      local gap = 0
      if baseTextSizeY < self._ui._attackList[ii]:GetTextSizeY() then
        addSizeY = 35
        gap = 15
      end
      self._ui._attackList[ii]:SetSpanSize(self._ui._attackList[ii]:GetSpanSize().x, lastSpanY + addSizeY)
      lastSpanY = lastSpanY + addSizeY + gap
  ...
  if actionCnt > 0 then
    self._ui._stc_actionTitle:SetShow(true)
    self._ui._stc_actionTitle:SetSpanSize(self._ui._stc_actionTitle:GetSpanSize().x, lastSpanY + baseTextSizeY)
    lastSpanY = lastSpanY + baseTextSizeY
    for ii = 1, actionCnt do
      self._ui._actionList[ii]:SetShow(true)
      self._ui._actionList[ii]:SetText(self._bossInfo[bossIndex]._actionText[ii])
      local addSizeY = baseTextSizeY
      local gap = 0
      if baseTextSizeY < self._ui._actionList[ii]:GetTextSizeY() then
        addSizeY = 35
        gap = 15
      end
      self._ui._actionList[ii]:SetSpanSize(self._ui._actionList[ii]:GetSpanSize().x, lastSpanY + addSizeY)
      lastSpanY = lastSpanY + addSizeY + gap
      if maxSizeX < self._ui._actionList[ii]:GetTextSizeX() then
        maxSizeX = self._ui._actionList[ii]:GetTextSizeX()
      end
    end
  else
    self._ui._stc_actionTitle:SetShow(false)
  end
  if moveCnt <= 0 and attackCnt <= 0 and actionCnt <= 0 then
    self._ui._stc_commandBG:SetShow(false)
    return
  end
  self._ui._stc_commandBG:ComputePos()
  self._ui._stc_commandBG:SetShow(true)
  self._ui._stc_commandBG:SetSize(maxSizeX + 30, lastSpanY + 25)
end
