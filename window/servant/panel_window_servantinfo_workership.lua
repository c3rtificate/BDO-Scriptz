...
local workerShipInfo = {
  ...
  _staticTextTitle_Mp = UI.getChildControl(Panel_WorkerShipInfo, "MP"),
...
workerShipInfo._vehicleAlert:SetTextMode(__eTextMode_AutoWrap)
...
function workerShipInfo:init()
  ...
  for ii = 0, self._config._skill.count - 1 do
    ...
    slot.condition:SetTextMode(__eTextMode_LimitText)
    slot.dec:SetTextMode(__eTextMode_LimitText)
...
function workerShipInfo:update()
  ...
  self._staticGaugeBar_Mp:SetSize(1.63 * (servantWrapper:getMp() / servantWrapper:getMaxMp() * 100), 6)
  self._staticTextValue_Mp:SetText(makeDotMoney(servantWrapper:getMp()) .. " / " .. makeDotMoney(servantWrapper:getMaxMp()))
  local vehicleType = vehicleInfo:get():getVehicleType()
  if true == _ContentsGroup_OceanCurrent then
    local characterKey = vehicleInfo:getCharacterKey()
    ...
    if nil ~= shipStaticStatus and false == shipStaticStatus:getIsSummonFull() then
      self._staticTextTitle_Mp:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_HORSEHP_TOOLTIP_FOOD_NAME"))
    else
      self._staticTextTitle_Mp:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_STABLEREGISTER_CARRIAGE_MP"))
    end
  elseif CppEnums.VehicleType.Type_PersonTradeShip == vehicleType or CppEnums.VehicleType.Type_SailingBoat == vehicleType or CppEnums.VehicleType.Type_PersonalBattleShip == vehicleType or CppEnums.VehicleType.Type_CashPersonalTradeShip == vehicleType or CppEnums.VehicleType.Type_CashPersonalBattleShip == vehicleType then
    self._staticTextTitle_Mp:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_HORSEHP_TOOLTIP_GUILDSHIP_NAME"))
  else
    self._staticTextTitle_Mp:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_STABLEREGISTER_CARRIAGE_MP"))
  end
  ...
  self._staticText_Value_Def:SetText(vehicleInfo:get():getDefense())
  ...
  for ii = 1, skillCount - 1 do
    ...
    if nil ~= skillWrapper then
      if self._skillStart <= self._skillCount and slotNo < self._config._skill.count then
        ...
        slot.icon:addInputEvent("Mouse_On", "HandleEventOn_WorkerShip_SkillTooltip(" .. servantWrapper:getActorKeyRaw() .. ", " .. ii .. ", " .. slotNo .. ")")
        if true == _ContentsGroup_NewUI_Tooltip_All then
          slot.icon:addInputEvent("Mouse_Out", "PaGlobal_Tooltip_Skill_Servant_All_Close()")
        else
...
function HandleEventOn_WorkerShip_SkillTooltip(actorKeyRaw, ii, slotNo)
  local servantWrapper = getServantInfoFromActorKey(actorKeyRaw)
  local skillWrapper = servantWrapper:getSkill(ii)
  if true == _ContentsGroup_NewUI_Tooltip_All then
    PaGlobal_Tooltip_Skill_Servant_All_Open(skillWrapper, workerShipInfo._skillSlots[slotNo], true)
  else
    PaGlobal_Tooltip_Servant_Open(skillWrapper, workerShipInfo._skillSlots[slotNo], true)
  end
end
...
function workerShipInfo:registEventHandler()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(self._buttonQuestion, "\"PanelServantinfo\"")
...
function WorkerShipInfo_LClick(slotNo)
  ...
  elseif DragManager.dragStartPanel == Panel_Window_Inventory_All then
    HandleEventRUp_Inventory_All_SlotRClick(DragManager.dragSlotInfo)
    DragManager.clearInfo()
  end
end
