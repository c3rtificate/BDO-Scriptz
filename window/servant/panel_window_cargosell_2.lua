...
function PaGlobal_CargoSellList_CreateControlList2(content, key)
  ...
  local totalPrice = itemWrapper:getSellPriceCalculate(CppDefine.e100Percent) * itemWrapper:getCount()
...
function HandleEventLUp_CargoSell_CheckSort()
  ...
  if true == PaGlobal_CargoSell._isConsole then
    if true == PaGlobal_CargoSell._isSorted then
      PaGlobal_CargoSell._ui._stc_keyGuide_LSClick:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_INVENTORY_KEYGUIDE_UNSORT"))
    else
      PaGlobal_CargoSell._ui._stc_keyGuide_LSClick:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_GENERIC_KEYGUIDE_XBOX_SORT"))
    end
  end
...
function HandleEventMOn_PaGlobal_CargoSell_IconOver(index)
  ...
  local itemWrapper = warehouseWrapper:getItem(slotNo)
  if nil ~= itemWrapper then
    Panel_Tooltip_Item_Show(itemWrapper, PaGlobal_CargoSell._slots[index].icon, false, true)
    Panel_Tooltip_Item:SetPosX(Panel_Window_CargoSell:GetPosX() - Panel_Tooltip_Item:GetSizeX())
    Panel_Tooltip_Item:SetPosY(Panel_Window_CargoSell:GetPosY())
  end
end
function HandleEventConsoleMouseOn_PaGlobal_CargoSell_IconOver(index, isShow)
  ...
  local itemWrapper = warehouseWrapper:getItem(slotNo)
  if nil ~= itemWrapper then
    PaGlobalFunc_FloatingTooltip_Open(Defines.TooltipDataType.ItemWrapper, itemWrapper, Defines.TooltipTargetType.Item, PaGlobal_CargoSell._slots[index].icon, "WareHouse")
  end
end
function HandleEventMOn_PaGlobal_CargoSellList_IconOver(index)
  ...
  Panel_Tooltip_Item_Show(itemWrapper, content, false, true)
  if true == PaGlobal_CargoSell._isConsole then
