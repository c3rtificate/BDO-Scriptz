...
function ShipInfo_LClick(slotNo)
  ...
  elseif DragManager.dragStartPanel == Panel_Window_Inventory then
...
function ShipInfo_SetShowTotem()
  ...
  if isCheck then
    ToClient_SetVehicleEquipSlotFlag(seaVehicleWrapper:getActorKeyRaw(), self._config._totemSlotNo)
  else
    ToClient_ResetVehicleEquipSlotFlag(seaVehicleWrapper:getActorKeyRaw(), self._config._totemSlotNo)
  end
end
...
function ShipInfo_Open()
  ...
  local servantWrapper = getServantInfoFromActorKey(self._actorKeyRaw)
  if nil ~= servantWrapper then
    local isCheck = ToClient_IsSetVehicleEquipSlotFlag(self._actorKeyRaw, self._config._totemSlotNo)
