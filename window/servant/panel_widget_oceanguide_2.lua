...
function FromClient_OceanGuide_UpdateWindDirection(_float3)
  ...
  if true == _ContentsGroup_OceanCurrent then
    ...
    PaGlobal_OceanGuide._oceanWind._ui.stc_Sequence:SetShow(power > 0)
...
function FromClient_OceanGuide_PlayerRideOff()
  ...
  PaGlobal_OceanGuide:prepareClose()
  if _ContentsGroup_ImprovedGalleyShip == true then
    PaGlobal_GalleyBoost_All_Close()
    PaGlobal_GalleyMember_All_Close()
  end
end
...
function PaGlobalFunc_OceanGuide_ChangeStateTexture(type, power)
  ...
  local powerLevel = 0
  if power >= 0 and power < 1.9 then
    powerLevel = 1
  elseif power >= 1.9 and power < 4.2 then
    powerLevel = 2
  elseif power >= 4.2 and power < 7 then
    powerLevel = 3
  elseif power >= 7 then
    powerLevel = 4
  else
...
function PaGlobalFunc_OceanGuide_FlowValueReturn()
  return PaGlobal_OceanGuide._oceanWind._ui.txt_flow:GetText()
end
