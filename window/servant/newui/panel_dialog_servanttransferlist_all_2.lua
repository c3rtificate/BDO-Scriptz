...
function HandleEventLUp_ServantTransferList_All_Transfer(regeionKey)
  ...
  local function TransferConfirm()
    ToClient_ChangeServantRegion(servantNo, regeionKey, CppEnums.ItemWhereType.eMoneyInventory)
    if true == PaGlobal_ServantTransferList_All._isWorldMapOpen then
  ...
  local _desc = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_SERVANT_CHANGE_REGION_NOTIFY_DESC", "cost", makeDotMoney(cost))
