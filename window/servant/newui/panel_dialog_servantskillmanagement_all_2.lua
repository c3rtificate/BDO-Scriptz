function PaGlobalFunc_ServantSkillManagement_All_Open(currentServantNo)
  ...
  if PaGlobal_ServantSkillManagement_All:getServantInfo() == nil then
    return
  end
...
function HandleEventLUp_ServantSkillManagement_All_TabChange(idx)
  ...
  if 0 == idx then
    ...
    if true == PaGlobal_ServantList_All._isConsole then
      ...
      PaGlobal_ServantSkillManagement_All._ui._stc_KeyGuide_LB_Y:SetShow(false)
      Panel_Dialog_ServantSkillManagement_All:registerPadEvent(__eConsoleUIPadEvent_LBPress_Y, "")
    end
  else
    ...
    local servantInfo = PaGlobal_ServantSkillManagement_All:getServantInfo()
    local skillChangeConfirmedCount = servantInfo:getServantSkillChangeConfirmedCount()
    if true == _ContentsGroup_UsePadSnapping then
      if nil == servantInfo or skillChangeConfirmedCount > 0 then
        Panel_Dialog_ServantSkillManagement_All:registerPadEvent(__eConsoleUIPadEvent_LBPress_Y, "")
        PaGlobal_ServantSkillManagement_All._ui._stc_KeyGuide_LB_Y:SetShow(false)
      else
        Panel_Dialog_ServantSkillManagement_All:registerPadEvent(__eConsoleUIPadEvent_LBPress_Y, "HandleEventLUp_ServantSkillManagement_All_SkillChangeConfirmed()")
        PaGlobal_ServantSkillManagement_All._ui._stc_KeyGuide_LB_Y:SetShow(true)
      end
    elseif nil == servantInfo or skillChangeConfirmedCount > 0 then
      PaGlobal_ServantSkillManagement_All._ui._btn_Change_PC:SetSpanSize(0, PaGlobal_ServantSkillManagement_All._ui._btn_Change_PC:GetSpanSize().y)
      PaGlobal_ServantSkillManagement_All._ui._btn_Change_Premium:SetShow(false)
    else
      local spanSizeX = -PaGlobal_ServantSkillManagement_All._ui._btn_Change_Premium:GetSpanSize().x
      PaGlobal_ServantSkillManagement_All._ui._btn_Change_PC:SetSpanSize(spanSizeX, PaGlobal_ServantSkillManagement_All._ui._btn_Change_PC:GetSpanSize().y)
      PaGlobal_ServantSkillManagement_All._ui._btn_Change_Premium:SetShow(true)
    end
  end
...
function HandleEventLUp_ServantSkillManagement_All_TrainingSkill(slotIdx)
  if nil == slotIdx or nil == PaGlobal_ServantSkillManagement_All._currentServantNo then
    return
  end
...
function HandleEventLUp_ServantSkillManagement_All_SkillChangeConfirmed()
  if nil == PaGlobal_ServantSkillManagement_All._currentChangeSkillIdx or nil == Panel_Dialog_ServantSkillManagement_All or false == Panel_Dialog_ServantSkillManagement_All:GetShow() then
    return
  end
  if false == _ContentsGroup_UsePadSnapping then
    local contentsEventType = __eContentsType_ChangeServantSkillConfirmed
    local itemCount = 0
    if true == _ContentsGroup_NewUI_Inventory_All then
      itemCount = PaGlobalFunc_Inventory_All_FindItemCountByEventType(contentsEventType)
    else
      itemCount = PaGlobal_Inventory:findItemCountByEventType(contentsEventType)
    end
    if 0 == itemCount then
      local EasyBuyOpen = function()
        local easyBuyUniqueNo = 90
        PaGlobal_EasyBuy:Open(easyBuyUniqueNo)
      end
      local messageboxData = {
        title = PAGetString(Defines.StringSheet_GAME, "LUA_STABLEINFO_MSG_TITLE_CHANGE"),
        content = PAGetString(Defines.StringSheet_GAME, "LUA_STABLEINFO_MSG_CONTENT_CHANGE_PREMIUM"),
  ...
  if servantInfo:getStateType() == CppEnums.ServantStateType.Type_StallionTraining then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_STABLELIST_CONDITION_STALLIONDESC"))
    return
  end
  local wishSKillIdx = PaGlobal_ServantSkillManagement_All._currentWishSkillIdx
  ...
  if nil == wishSKillIdx then
    local messageboxData = {
      ...
      content = PAGetString(Defines.StringSheet_GAME, "LUA_STABLEINFO_CHANGESKILL_BTN"),
  ...
  local changingSkillWrapper = servantInfo:getSkill(changingSkillIdx)
  if nil == changingSkillWrapper then
    return
  end
  local function skillChangeConfirm()
    audioPostEvent_SystemUi(3, 19)
    ToClient_changeServantSkillConfirmed(currentServantNo, changingSkillIdx, wishSKillIdx)
    PaGlobal_ServantSkillManagement_All:dataClear()
  end
  local function skillChangeCheck()
    local messageboxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_ALERT_DEFAULT_TITLE"),
      content = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_SERVANT_STABEL_SKILLCHANGE_PREMIUM_MSG", "skillname", changingSkillWrapper:getName(), "skillname2", wishSkillWrapper:getName()),
  ...
  local isStallionSkill = servantInfo:isStallionSkill(changingSkillWrapper:getKey())
  if true == isStallionSkill then
    local messageboxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_ALERT_DEFAULT_TITLE"),
      content = PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_SKILLCHANGE_STALLION_DESC"),
