...
function HandleEventLUp_ServantExchange_All_MixConfirm()
  ...
  if baseName == name or "" == name then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_STABLEINFO_EDITBOX_COMMENT"))
    return
  end
  local isUsedAnyAddStat = false
  local servantInfo = stable_getServantByServantNo(PaGlobal_ServantExchange_All._servantNo_Left)
  if servantInfo ~= nil and not isUsedAnyAddStat then
    isUsedAnyAddStat = servantInfo:isUsedAnyAddStatItem()
  end
  servantInfo = stable_getServantByServantNo(PaGlobal_ServantExchange_All._servantNo_Right)
  if servantInfo ~= nil and not isUsedAnyAddStat then
    isUsedAnyAddStat = servantInfo:isUsedAnyAddStatItem()
  end
  local function servantMix()
    stable_mix(PaGlobal_ServantExchange_All._servantNo_Left, PaGlobal_ServantExchange_All._servantNo_Right, CppEnums.ItemWhereType.eMoneyInventory, name)
    if nil ~= PaGlobalFunc_ServantInfo_All_ClearSealMyServantNo then
      PaGlobalFunc_ServantInfo_All_ClearSealMyServantNo()
    end
  end
...
function FromClient_ServantExchange_All_ServantExchangeFinish()
  ...
  if false == PaGlobal_ServantExchange_All._isConsole or false == _ContentsGroup_RenewUI then
    PaGlobal_ServantExchange_All._ui._edit_Naming:SetEditText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVANTREGIST_EDITDESC"), true)
  else
