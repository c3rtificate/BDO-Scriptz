...
function HandleEventLUp_ServantSwiftTraining_All_CronButton()
  if Panel_Dialog_ServantSwiftTraining_All == nil or Panel_Dialog_ServantSwiftTraining_All:GetShow() == false then
    return
  end
  local control = PaGlobal_ServantSwiftTraining_All._ui._btn_Check_Cron
  if control == nil then
    return
  end
  local isCheck = PaGlobal_ServantSwiftTraining_All._ui._btn_Check_Cron:IsCheck()
  ToClient_getGameUIManagerWrapper():setLuaCacheDataListBool(__eStallionSkillTrainingUseCron, isCheck, CppEnums.VariableStorageType.eVariableStorageType_User)
end
...
function HandleEventOut_ServantSwiftTraining_All_ShowRateGuide(isShow)
  ...
  if isShow == true then
    ...
    for i = 0, PaGlobal_ServantSwiftTraining_All._slotCount - 1 do
      expList[i] = PaGlobal_ServantSwiftTraining_All._trainingSlot[i]._skillExpCount
      totalExp = totalExp + expList[i]
    end
    for i = 0, PaGlobal_ServantSwiftTraining_All._slotCount - 1 do
      if totalExp == 0 then
        stringList[i] = "0%"
      else
        local temp = math.ceil(expList[i] / totalExp * 100 * 10000 - 0.5) / 10000
        if expList[i] == 0 then
          temp = 0
        end
        stringList[i] = tostring(temp) .. "%"
      end
    end
    control = PaGlobal_ServantSwiftTraining_All._ui._stc_RateGuide
    name = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_SWIFTING_RATING", "rate1", stringList[0], "rate2", stringList[1], "rate3", stringList[2])
    desc = nil
...
function HandleEventLUp_ServantSwiftTraining_All_ItemClear(slotType)
  ...
  if false == PaGlobal_ServantSwiftTraining_All._ui._btn_Train:IsIgnore() then
    local isFindMaterial = false
    for i = 0, PaGlobal_ServantSwiftTraining_All._slotCount - 1 do
      if nil ~= PaGlobal_ServantSwiftTraining_All._trainingSlot and nil ~= PaGlobal_ServantSwiftTraining_All._trainingSlot[i] then
        local materialSlot = PaGlobal_ServantSwiftTraining_All._trainingSlot[i]
...
function HandleEventLUp_ServantSwiftTraining_All_Training()
  ...
  if true == isTrainingAble and PaGlobal_ServantSwiftTraining_All._ui._chk_skipAni:IsCheck() == false then
...
function HandleEventLUp_ServantSwiftTraining_All_TrySwift()
  ...
  local _title = PAGetString(Defines.StringSheet_GAME, "LUA_ILLUSIONHORSETTRAINING_WARNING_TITLE")
  local _desc = ""
  local isUseCron = PaGlobal_ServantSwiftTraining_All._ui._btn_Check_Cron:IsCheck()
  if isUseCron == true then
    _desc = PAGetString(Defines.StringSheet_GAME, "LUA_ILLUSIONHORSETTRAINING_WARNING_CRONSTONE")
  else
    _desc = PAGetString(Defines.StringSheet_GAME, "LUA_ILLUSIONHORSETTRAINING_WARNING_NOCRONSTONE")
  end
  local servantInfo = stable_getServantByServantNo(PaGlobal_ServantSwiftTraining_All._currentServentNo)
...
]] .. PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_BUFF_DONTUSE_MSG")
  end
...
function FromClient_ServantSwiftTraining_All_IncreaseStallionSkillExpAck(servantNo, skillKey, skillExp)
  ...
  if PaGlobal_ServantSwiftTraining_All._ui._chk_skipAni:IsCheck() == true then
    PaGlobalFunc_ServantSwiftTraining_All_TrainingCompleteSkipAnim()
  end
end
...
function PaGlobalFunc_ServantSwiftTraining_All_TrainingCompleteSkipAnim()
  ...
  if PaGlobal_ServantSwiftTraining_All._enum_BtnType._TRAINING == PaGlobal_ServantSwiftTraining_All._isButtonClick then
    ...
    audioPostEvent_SystemUi(13, 27)
    _AudioPostEvent_SystemUiForXBOX(13, 27)
  ...
  elseif 1 == PaGlobal_ServantSwiftTraining_All._isButtonClick then
    ...
    if PaGlobal_ServantSwiftTraining_All._elapsedTime < 2 then
      if 0 == PaGlobal_ServantSwiftTraining_All._effectType then
        ...
        if PaGlobal_ServantSwiftTraining_All._ui._chk_skipAni:IsCheck() == true then
          PaGlobal_ServantSwiftTraining_All._elapsedTime = 3.8
          PaGlobal_ServantSwiftTraining_All._effectType = 2
        else
          for i = 0, PaGlobal_ServantSwiftTraining_All._slotCount - 1 do
