...
function PaGlobalFunc_ServantTransferBackwardList_All_Transfer(servantIndex, fromRegeionKey, isHorse)
  ...
  local servantInfo = ToClient_getReverseTransferableServantInfo(fromRegeionKey, servantIndex)
  if servantInfo == nil then
    return
  end
  local function TransferConfirm()
    ToClient_TransportBackward(servantInfo:getServantNo(), fromRegeionKey, CppEnums.ItemWhereType.eMoneyInventory)
