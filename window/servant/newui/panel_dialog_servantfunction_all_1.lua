function PaGlobal_ServantFunction_All:initialize()
  ...
  self._ui._rdo_HorseWorldMarket = UI.getChildControl(Panel_Dialog_ServantFunction_All, "RadioButton_HorseWorldMarket")
  self._ui._rdo_HorseSimulator = UI.getChildControl(Panel_Dialog_ServantFunction_All, "RadioButton_HorseSimulator")
  ...
  self._tabButtonForCategory[6] = self._ui._rdo_HorseWorldMarket
  if _ContentsGroup_HorseSimulator == true then
    self._tabButtonForCategory[7] = self._ui._rdo_HorseSimulator
    self._tabButtonForCategory[8] = self._ui._rdo_Exit
  else
    self._tabButtonForCategory[7] = self._ui._rdo_Exit
  end
  self:validate()
...
function PaGlobal_ServantFunction_All:validate()
  ...
  self._ui._rdo_HorseWorldMarket:isValidate()
  self._ui._rdo_HorseSimulator:isValidate()
...
function PaGlobal_ServantFunction_All:registerEventHandler()
  ...
  if false == self._isConsole then
    ...
    self._ui._rdo_HorseWorldMarket:addInputEvent("Mouse_LUp", "PaGlobalFunc_ServantFunction_All_ChangeTab(6)")
    self._ui._rdo_HorseSimulator:addInputEvent("Mouse_LUp", "PaGlobalFunc_ServantFunction_All_ChangeTab(7)")
  else
  end
end
...
function PaGlobal_ServantFunction_All:prepareOpen(isGuild, servantType)
  ...
  self._ui._rdo_HorseWorldMarket:SetShow(false)
  self._ui._rdo_HorseSimulator:SetShow(false)
  ...
  self:addButtonGroups(self._ui._rdo_FunctionList)
  if false == self._isGuild and CppEnums.ServantType.Type_Ship ~= servantType then
    if true == stable_isCarriage() and false == isSiegeStable() then
      self:addButtonGroups(self._ui._rdo_LinkCarridge)
    end
    if true == stable_isMix() and false == isSiegeStable() then
      self:addButtonGroups(self._ui._rdo_Exchange)
    end
    if true == stable_isMating() then
      self:addButtonGroups(self._ui._rdo_MatingMarket)
    end
    if true == stable_isMarket() then
      self:addButtonGroups(self._ui._rdo_HorseMarket)
      if true == _ContentsGroup_ServantRent then
        self:addButtonGroups(self._ui._rdo_RentalMarket)
      end
      if true == _ContentsGroup_ServantWorldMarket then
        self:addButtonGroups(self._ui._rdo_HorseWorldMarket)
      end
      if _ContentsGroup_HorseSimulator == true then
        self:addButtonGroups(self._ui._rdo_HorseSimulator)
      end
    end
  end
  if false == self._isConsole then
    self:addButtonGroups(self._ui._rdo_Exit)
  end
  self:open()
  FromClient_ServantFunction_All_OnScreenResize()
  self:repositionFunctionButton(self._functionButtonCount, self._functionButtons)
  ...
  Panel_Dialog_ServantFunction_All:EraseAllEffect()
  Panel_Dialog_ServantFunction_All:AddEffect("fUI_Servant_Dialog_01A", true, 0, 0)
end
