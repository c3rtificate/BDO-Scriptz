function PaGlobal_ServantInfo_All:initialize()
  ...
  self._ui._scroll_userInventory = UI.getChildControl(self._ui._stc_userInventoryBG_inStable, "Scroll_StableUserInven")
...
function PaGlobal_ServantInfo_All:registerEventHandler(isConsole)
  ...
  UIScroll.InputEvent(self._ui._scroll_userInventory, "HandleEventScroll_ServantInfo_All_UserInventory_UpdateScroll")
  UIScroll.InputEventByControl(self._ui._stc_userInventoryBG_inStable, "HandleEventScroll_ServantInfo_All_UserInventory_UpdateScroll")
          ...
          if nil ~= itemWrapper then
            ...
            if nil ~= itemSSW then
              ...
              if nil ~= itemKey then
                ...
                if servantInfo:getRegionName() == regionName and servantInfo:isChangingRegion() == false and _ContentsGroup_SealedServantEquip == true and (self._currentNpcType == self._ENUM_NPC_TYPE._SEA or self._currentNpcType == self._ENUM_NPC_TYPE._LAND) then
...
function PaGlobal_ServantInfo_All:createUserInvenSlot()
  local tmp_userInvenSlotBG = UI.getChildControl(self._ui._stc_userInventoryBG_inStable, "Static_SlotBG_Temp")
  local slotColCount = self._USERINVENTORY_CONFIG.col
  local slotColOffset = 25
  local slotRowOffset = 15
  local slotSizeX = 50
  local slotSizeY = 50
  for slotNo = 0, self._USERINVENTORY_CONFIG.minMaxSize - 1 do
    local slot = {}
    local slotBackground = UI.createControl(__ePAUIControl_Static, self._ui._stc_userInventoryBG_inStable, "Inventory_Slot_BG_" .. slotNo)
    local itemSlot = SlotItem.new(slot, "UserInvenOnlyServantEquip_" .. slotNo, slotNo, self._ui._stc_userInventoryBG_inStable, self._USERINVENTORY_SLOT_CONFIG)
    slot:createChild()
    local row = math.floor(slotNo / slotColCount)
    local col = slotNo % slotColCount
    slot.icon:SetPosX(slotSizeX * col + slotColOffset)
    slot.icon:SetPosY(slotSizeY * row + slotRowOffset)
    slot.icon:SetShow(false)
    UIScroll.InputEventByControl(slot.icon, "HandleEventScroll_ServantInfo_All_UserInventory_UpdateScroll")
    slot.isEmpty = true
    ...
    slot.classEquipBG:ChangeTextureInfoName("new_ui_common_forlua/window/inventory/Disable_Class.dds")
...
function PaGlobal_ServantInfo_All:updateUserInvenSlot()
  ...
  if self._ui._stc_servantEquipInvenBg:GetShow() == false then
    return
  end
  ...
  if self._ui._stc_userInventory_inStable:GetShow() == false then
    return
  end
  ...
  for slotNo = 0, #self._userInvenSlot do
  ...
  for ii = 0, inventoryUseSize do
    ...
    local slotNo = servantEquipCount - startRow * inventoryColSize
    if itemWrapper ~= nil then
      ...
      if itemisProper == true then
        if slotNo >= 0 and slotNo <= #self._userInvenSlot then
          local itemSSW = itemWrapper:getStaticStatus()
          self._userInvenSlot[slotNo]:setItem(itemWrapper, slotNo)
          self._userInvenSlot[slotNo].icon:SetShow(true)
          self._userInvenSlot[slotNo].icon:SetIgnore(false)
          ...
          if itemSSW ~= nil then
            ...
            if itemStatic:isServantTypeUsable(servantInfo:getServantKind(), servantInfo:getServantKindSub()) == false then
              ...
              if _ContentsGroup_ChinaFontColor == true then
                ...
                self._userInvenSlot[slotNo].icon:SetColor(Defines.Color.C_FFD20000)
              end
            end
            local itemKey = itemSSW:get()._key
            if itemKey ~= nil then
              self._userInvenSlot[slotNo].icon:addInputEvent("Mouse_On", "HandleEventOn_ServantInfo_All_EquipInvenToolTip(" .. itemKey:get() .. ",true)")
              self._userInvenSlot[slotNo].icon:addInputEvent("Mouse_Out", "HandleEventOn_ServantInfo_All_EquipInvenToolTip(" .. itemKey:get() .. ",false)")
              self._userInvenSlot[slotNo].icon:addInputEvent("Mouse_RUp", "HandleEventRUp_ServantInfo_All_EquipFromInventory(" .. realInvenSlotNo .. ")")
            end
          end
        end
        servantEquipCount = servantEquipCount + 1
      end
    end
  end
  if servantEquipCount > self._USERINVENTORY_CONFIG.minMaxSize then
    self._userInvenSize = math.ceil(servantEquipCount / self._USERINVENTORY_CONFIG.col) * self._USERINVENTORY_CONFIG.col
  else
    self._userInvenSize = self._USERINVENTORY_CONFIG.minMaxSize
  end
  UIScroll.SetButtonSize(PaGlobal_ServantInfo_All._ui._scroll_userInventory, self._USERINVENTORY_CONFIG.minMaxSize, self._userInvenSize)
end
