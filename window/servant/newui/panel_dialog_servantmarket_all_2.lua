function PaGlobalFunc_ServantMarket_All_Open()
  ...
  PaGlobal_ServantMarket_All:prepareOpen(false)
end
function PaGlobalFunc_ServantMarket_All_OpenByMenu()
  if nil == Panel_Dialog_ServantMarket_All or true == Panel_Dialog_ServantMarket_All:GetShow() then
    return
  end
  PaGlobal_ServantMarket_All:prepareOpen(true)
end
...
function HandleEventLUp_ServantMarket_All_OpenComboBox(idx)
  ...
  elseif 4 == idx then
    PaGlobal_ServantMarket_All._ui._stc_ComboBox_Level:ToggleListbox()
  end
end
function HandleEventLUp_ServantMarket_All_SelectComboBox(idx)
  ...
  if 1 == idx then
    local selectedIndex = PaGlobal_ServantMarket_All._ui._stc_ComboBox_Tier:GetSelectIndex()
    local tierIndex = PaGlobal_ServantMarket_All._showServantTier[selectedIndex]
  ...
  elseif 4 == idx then
    local levelIndex = PaGlobal_ServantMarket_All._ui._stc_ComboBox_Level:GetSelectIndex()
    if nil ~= levelIndex then
      if levelIndex <= 0 then
        levelIndex = 0
      end
      setAuctionServantLevelFilter(levelIndex, PaGlobal_ServantMarket_All._isFromNpc)
    end
  else
    return
  end
...
function HandleEventOn_MarketServant_Tooltip_Open(actorKeyRaw, key32, slotIdx)
  ...
  if true == PaGlobal_ServantMarket_All._isUsePadSnapping and slotIdx ~= PaGlobal_ServantMarket_All._curTargetSnapIndex then
...
function HandleEventLUp_ServantMarket_All_PageChange(isNext)
  ...
  if true == isNext then
    ...
    requestRenewalAuctionNextPage()
  else
    if 0 >= PaGlobal_ServantMarket_All._selectPage then
      return
    end
    requestRenewalAuctionPrevPage()
  end
...
function HandleEventLUp_ServantMarket_All_Buy(slotNo)
  ...
  local BuyConfirm = function()
    stable_requestBuyItNowServant(PaGlobal_ServantMarket_All._selectSlotNo, CppEnums.ItemWhereType.eMoneyInventory)
...
function HandleEventLUp_ServantMarket_All_Recieve(slotNo)
  ...
  local function receiveMoney()
    local rv = stable_popServantPrice(slotNo, CppEnums.AuctionType.AuctionGoods_ServantMarket, CppEnums.ItemWhereType.eMoneyInventory)
    if 0 == rv then
...
function FromClient_ServantMarket_All_UpdateSlot()
  ...
  PaGlobal_ServantMarket_All:update()
  if true == PaGlobal_ServantMarket_All._isUsePadSnapping then
...
function HandleEventLUp_ServantMarket_All_TabChange(isTabMyRegister)
  ...
  if true == isTabMyRegister then
    ...
    PaGlobal_ServantMarket_All._ui._stc_ComboBox_Level:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVANTMARKET_LEVELFILTER_ALL"))
    ...
    requestServantMarketList(PaGlobal_ServantMarket_All._isFromNpc)
  end
...
function PaGlobalFunc_ServantMarket_All_SetFilterShow(isShow)
  ...
  PaGlobal_ServantMarket_All._ui._stc_ComboBox_LevelTitle:SetShow(isShow)
  ...
  PaGlobal_ServantMarket_All._ui._stc_ComboBox_Level:SetShow(isShow)
end
...
function PaGlobalFunc_ServantMarket_All_SetKeyGuideText(type, slotIndex)
  ...
  if false == PaGlobal_ServantMarket_All._isUsePadSnapping then
    return
  end
  if nil == type then
