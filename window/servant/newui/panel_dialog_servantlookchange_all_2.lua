...
function HandleEventLUp_ServantLookChange_All_ChangeLook(isPremium)
  ...
  local cancel = function()
    ...
    if nil ~= Panel_Dialog_Servant_PremiumLookChange and true == Panel_Dialog_Servant_PremiumLookChange:GetShow() then
  ...
  local isPossibleLearnSkill = false
  local formIndex
  if 0 == PaGlobalFunc_ServantFunction_All_Get_NpcType() then
    isPossibleLearnSkill = stable_isPossibleLearnServantSkill(sortedSlotNo)
    local formManager = getServantFormManager()
    local tierIndex = PaGlobal_ServantLookChange_All._ui._comboBox_TierSelect_PC:GetSelectIndex()
    if tierIndex <= 0 then
      tierIndex = 0
    ...
    local function changeConfirm()
      stable_changeForm(sortedSlotNo, formIndex, 0, false, 0)
      PaGlobalFunc_ServantList_All_Update()
      PaGlobalFunc_ServantLookChange_All_Close()
    end
    local function premiumChangeConfirm()
      stable_changeForm(sortedSlotNo, formIndex, 1, isPossibleLearnSkill, 0)
      PaGlobalFunc_ServantList_All_Update()
      PaGlobalFunc_ServantLookChange_All_Close()
      if nil ~= Panel_Dialog_Servant_PremiumLookChange and true == Panel_Dialog_Servant_PremiumLookChange:GetShow() then
        PaGlobalFunc_Servant_PremiumSkillRate_Close()
  ...
  local isShowRate = false
  if true == _ContentsGroup_ServantChangeFormSkillRate and nil ~= Panel_Dialog_Servant_PremiumLookChange and true == isPremium and true == isPossibleLearnSkill then
    local skillLearnRate = ToClient_getServantChangeFormWithSkillRate() / CppDefine.e1Percent
    messageboxData.content = _desc .. [[


]] .. PAGetStringParam2(Defines.StringSheet_GAME, "LUA_PREMIUMLOOKCHANGE_RATE_DESC", "exp", 100 - skillLearnRate, "skill", skillLearnRate)
    isShowRate = true
  end
  MessageBox.showMessageBox(messageboxData)
  if true == isShowRate then
    PaGlobalFunc_Servant_PremiumSkillRate_Open(PaGlobal_ServantLookChange_All._currentServantNo, sortedSlotNo, formIndex, 1, true)
  end
end
