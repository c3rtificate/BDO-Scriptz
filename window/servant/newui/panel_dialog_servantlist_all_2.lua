...
function HandleEventOn_ServantList_All_ShowCountTooltip(iconType)
  ...
  if 2 ~= PaGlobalFunc_ServantFunction_All_Get_NpcType() and 3 ~= PaGlobalFunc_ServantFunction_All_Get_NpcType() then
    ...
    elseif 3 == iconType then
      uiControl = PaGlobal_ServantList_All._ui._txt_RaceHorseListValue
      name = PAGetString(Defines.StringSheet_GAME, "LUA_STABLELIST_HORSERACING_MAXCOUNT_TITLE")
      desc = PAGetString(Defines.StringSheet_GAME, "LUA_STABLELIST_HORSERACING_MAXCOUNT_DESC")
    end
...
function HandleEventOn_ServantList_All_ShowTransferBackwardTooltip()
  if nil == Panel_Dialog_ServantList_All or false == Panel_Dialog_ServantList_All:GetShow() then
    return
  end
  local uiControl, name, desc
  uiControl = PaGlobal_ServantList_All._ui._btn_IncreaseSlot
  name = PAGetString(Defines.StringSheet_GAME, "LUA_VEHICLEMOVE_TOOLTIP_MOVENAME2")
  if true == _ContentsGroup_Barter then
    desc = PAGetString(Defines.StringSheet_GAME, "LUA_VEHICLEMOVE_TOOLTIP_MOVEDESC2")
  else
    desc = PAGetString(Defines.StringSheet_GAME, "LUA_VEHICLEMOVE_TOOLTIP_MOVEDESC2_CS")
  end
  TooltipSimple_Show(uiControl, name, desc)
end
function HandleEventOut_ServantList_All_HideTransferBackwardTooltip()
  if nil == Panel_Dialog_ServantList_All then
    return
  end
  TooltipSimple_Hide()
end
function HandleEventOn_ServantList_All_ShowChangeServantRegionTooltip()
  if nil == Panel_Dialog_ServantList_All or false == Panel_Dialog_ServantList_All:GetShow() then
    return
  end
  local uiControl, name, desc
  uiControl = PaGlobal_ServantList_All._ui._btn_IncreaseSlot
  name = PAGetString(Defines.StringSheet_GAME, "LUA_VEHICLEMOVE_TOOLTIP_MOVENAME")
  desc = PAGetString(Defines.StringSheet_GAME, "LUA_VEHICLEMOVE_TOOLTIP_MOVEDESC")
  TooltipSimple_Show(uiControl, name, desc)
end
function HandleEventOut_ServantList_All_HideChangeServantRegionTooltip()
  if nil == Panel_Dialog_ServantList_All then
    return
  end
  TooltipSimple_Hide()
...
function PaGlobalFunc_ServantList_All_CharacterSceneResetServantNo(servantNo)
  ...
  PaGlobal_ServantList_All._selectSceneIndex = PaGlobalFunc_ServantFunction_All_ScenePushObject(servantInfo, PaGlobal_ServantList_All._selectSceneIndex, servantInfo:getActionIndex())
end
...
function HandleEventOn_ServantList_All_SelectServant(eType, slotNo)
  if nil == Panel_Dialog_ServantList_All or false == Panel_Dialog_ServantList_All:GetShow() then
    return
  end
  ...
  if _ContentsGroup_HorseSimulator == true and Panel_Dialog_ServantSimulator:GetShow() == true then
    return
  end
  if nil == slotNo or nil == eType then
    return
  end
  if nil ~= Panel_ImperialSupply_Horse_Confirm and true == Panel_ImperialSupply_Horse_Confirm:GetShow() then
    PaGlobal_ImperialSupplyMSG_Close()
  end
  ...
  if eType == PaGlobal_ServantList_All._ENUM_TYPESEALED then
    ...
    local servantInfo
    if PaGlobalFunc_ServantFunction_All_GetIsGuild() == false then
      if PaGlobal_ServantList_All:isCrogdaloServant(sortedSlotNo) == true then
        servantInfo = ToClient_getServantCrogdaloByIndex(math.abs(sortedSlotNo - tonumber(PaGlobal_ServantList_All._crogdalIndexStart)))
      else
    ...
    local getServantNo = servantInfo:getServantNo()
    PaGlobal_ServantList_All._selectSceneIndex = PaGlobalFunc_ServantFunction_All_ScenePushObject(servantInfo, PaGlobal_ServantList_All._selectSceneIndex, servantInfo:getActionIndex())
  ...
  PaGlobal_ServantList_All:hilightSelectedServant()
...
function HandleEventOut_ServantList_All_HideSortComponent()
  if PaGlobal_ServantList_All == nil then
    return
  end
  local panelPosX = PaGlobal_ServantList_All._ui._stc_SortComponent:GetPosX()
  local panelPosY = PaGlobal_ServantList_All._ui._stc_SortComponent:GetPosY()
  local panelSizeX = PaGlobal_ServantList_All._ui._stc_SortComponent:GetSizeX() - 0.5
  local panelSizeY = PaGlobal_ServantList_All._ui._stc_SortComponent:GetSizeY()
  ...
  if PaGlobal_ServantList_All._ui._stc_SortComponent:GetShow() == true then
    PaGlobal_ServantList_All._ui._stc_SortComponent:SetShow(false)
    PaGlobal_ServantList_All._ui._btn_SortButton:SetCheck(false)
  end
end
...
function HandleEventLUp_ServantList_All_ToggleSortOptionPanel()
  if PaGlobal_ServantList_All == nil then
    return
  end
  local isShow = PaGlobal_ServantList_All._ui._stc_SortComponent:GetShow()
  if isShow == true then
    PaGlobal_ServantList_All._ui._stc_SortComponent:SetShow(false)
    ToClient_padSnapResetControl()
  else
    PaGlobal_ServantList_All._ui._stc_SortComponent:SetShow(true)
    if PaGlobal_ServantList_All._sortBase == 0 then
      PaGlobal_ServantList_All._ui._rdo_Desc:SetCheck(false)
      PaGlobal_ServantList_All._ui._rdo_Asc:SetCheck(false)
    elseif PaGlobal_ServantList_All._isSortAsc == true then
      PaGlobal_ServantList_All._ui._rdo_Asc:SetCheck(true)
    else
      PaGlobal_ServantList_All._ui._rdo_Desc:SetCheck(true)
    end
    PaGlobal_ServantList_All:setSnapTarget(3)
  end
end
function HandleEventLUp_ServantList_All_SetSortOptionBase(idx)
  if PaGlobal_ServantList_All._sortBase ~= idx then
    PaGlobal_ServantList_All._isSortAsc = false
    PaGlobal_ServantList_All._ui._rdo_Desc:SetCheck(true)
    PaGlobal_ServantList_All._ui._rdo_Asc:SetCheck(false)
  end
  PaGlobal_ServantList_All._sortBase = idx
  if idx == 0 then
    PaGlobal_ServantList_All._ui._btn_SortButton:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_WINDOW_CARGOLOADING_SORT"))
  elseif idx == 1 then
    PaGlobal_ServantList_All._ui._btn_SortButton:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_WINDOW_SERVANTLIST_SORT_LEVEL"))
  elseif idx == 2 then
    PaGlobal_ServantList_All._ui._btn_SortButton:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_WINDOW_SERVANTLIST_SORT_TIER"))
  elseif idx == 3 then
    PaGlobal_ServantList_All._ui._btn_SortButton:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_WINDOW_SERVANTLIST_SORT_MIXCOUNT"))
  else
    PaGlobal_ServantList_All._ui._btn_SortButton:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_WINDOW_SERVANTLIST_SORT_GENDER"))
  end
  PaGlobal_ServantList_All._ui._stc_SortComponent:SetShow(false)
  PaGlobal_ServantList_All._ui._btn_SortButton:SetCheck(false)
  PaGlobal_ServantList_All:update()
...
function HandleEventLUp_ServantList_All_SetSortOptionAsc(isAsc)
  if PaGlobal_ServantList_All._sortBase == 0 then
    PaGlobal_ServantList_All._ui._rdo_Desc:SetCheck(false)
    PaGlobal_ServantList_All._ui._rdo_Asc:SetCheck(false)
    return
  ...
  if isAsc == true then
    PaGlobal_ServantList_All._isSortAsc = true
  else
    PaGlobal_ServantList_All._isSortAsc = false
  end
  PaGlobal_ServantList_All._ui._stc_SortComponent:SetShow(false)
...
function HandleEventLUp_ServantList_All_UnSealBy_CurrentServantNo()
  ...
  if _ContentsGroup_Crogdalo == true and PaGlobal_ServantList_All:isCrogdaloServant(sortedSlotNo) == true and isSiegeStable() == true and PaGlobalFunc_ServantFunction_All_GetIsGuild() == false then
    local crogServantWrapper = ToClient_getServantCrogdaloByIndex(math.abs(sortedSlotNo - tonumber(PaGlobal_ServantList_All._crogdalIndexStart)))
    if crogServantWrapper ~= nil and CppEnums.ServantStateType.Type_Coma == crogServantWrapper:getStateType() then
      local crogServantInfo = stable_getServantByServantNo(crogServantWrapper:getServantNo(), CppEnums.ServantWhereType.ServantWhereTypeCrogdaloStable)
      if crogServantInfo ~= nil then
        ToClient_requestUnSealServantCrogdalo(crogServantInfo:getServantNo())
        if nil ~= Panel_Widget_ServantIcon then
          reset_ServantHP(crogServantInfo:getHp())
        end
  ...
  if true == _ContentsGroup_InstanceHorseRacing and true == ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_HorseRacing) then
    ...
    ToClient_UnsealRaceHorseReq(getSevantNo)
    if nil ~= Panel_Widget_ServantIcon then
...
function HandleEventLUp_ServantList_All_SealMyServant(isCompulsion)
  ...
  local function seal_Go()
    if true == _ContentsGroup_InstanceHorseRacing and true == ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_HorseRacing) then
      ToClient_SealRaceHorseReq(isCompulsion)
      return
    end
    if false == PaGlobalFunc_ServantFunction_All_GetIsGuild() then
  ...
  local function crogSeal()
    if _ContentsGroup_Crogdalo == false then
      return
    end
    ToClient_requestSealServantCrogdalo(isCompulsion)
  end
  ...
  if _ContentsGroup_Crogdalo == true then
    local crogServantInfo = stable_getServantByServantNo(servantNo, CppEnums.ServantWhereType.ServantWhereTypeCrogdaloStable)
    if crogServantInfo ~= nil and crogServantInfo:getVehicleType() == __eVehicleType_Horse then
      _confirmFunction = crogSeal
    end
  end
  if isCompulsion then
    ...
    if nil ~= Panel_Dialog_ServantInfo_All then
      if false == PaGlobal_ServantList_All._isConsole then
        ...
        HandleEventOn_ServantList_All_SelectServant(0, 0)
      end
    end
  end
...
function HandleEventLUp_ServantList_All_Recovery_Sealed_Servant()
  ...
  local isCrogStable = false
  if false == PaGlobalFunc_ServantFunction_All_GetIsGuild() then
    if PaGlobal_ServantList_All:isCrogdaloServant(currentServantNo) == true then
      servantInfo = ToClient_getServantCrogdaloByIndex(math.abs(currentServantNo - tonumber(PaGlobal_ServantList_All._crogdalIndexStart)))
      isCrogStable = true
    else
  ...
  local function revive_servant()
    ...
    if false == PaGlobalFunc_ServantFunction_All_GetIsGuild() then
      if isCrogStable == true then
        ToClient_requestReviveServantCrogdalo(math.abs(currentServantNo - tonumber(PaGlobal_ServantList_All._crogdalIndexStart)))
      else
        stable_revive(currentServantNo, CppEnums.ItemWhereType.eMoneyInventory)
      end
    else
  ...
  local function recovery_servant()
    ...
    if false == PaGlobalFunc_ServantFunction_All_GetIsGuild() then
      if isCrogStable == true then
        ToClient_requestRecoverySealedServantCrogdalo(math.abs(currentServantNo - tonumber(PaGlobal_ServantList_All._crogdalIndexStart)))
      else
        stable_recovery(currentServantNo, CppEnums.ItemWhereType.eMoneyInventory)
      end
    else
      local guildWrapper = ToClient_GetMyGuildInfoWrapper()
      if nil == guildWrapper then
...
function HandleEventLUp_ServantList_All_Recovery_UnSealed_Servant()
  ...
  local recovery_Unseal = function()
    audioPostEvent_SystemUi(5, 7)
    stable_recoveryUnseal(CppEnums.ItemWhereType.eMoneyInventory)
  end
  local servantNo = servantWrapper:getServantNo()
  local crogRecovery = function()
    if _ContentsGroup_Crogdalo == false then
      return
    end
    ToClient_requestRecoveryUnsealServantCrogdalo()
  end
  local _confirmFunction = recovery_Unseal
  if _ContentsGroup_Crogdalo == true then
    local crogServantInfo = stable_getServantByServantNo(servantNo, CppEnums.ServantWhereType.ServantWhereTypeCrogdaloStable)
    if crogServantInfo ~= nil and crogServantInfo:getVehicleType() == __eVehicleType_Horse then
      _confirmFunction = crogRecovery
    end
  end
  local _contents = ""
...
function HandleEventLUp_ServantList_All_SellServant()
  ...
  if false == PaGlobalFunc_ServantFunction_All_GetIsGuild() then
    ...
    if CppEnums.ServantType.Type_Ship == servantType then
      _desc = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_SERVANT_SHIP_SELL_NOTIFY_MSG", "resultMoney", resultMoney) .. PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_SELL_WITHITEM_MSG")
    else
...
function PaGlobalFunc_ServantList_All_ServantInfo()
  if nil == Panel_Dialog_ServantList_All or false == Panel_Dialog_ServantList_All:GetShow() then
    return
  end
  PaGlobal_ServantList_All:subMenuClose()
...
function PaGlobalFunc_ServantList_All_CurrentSelectSlotNo()
  if nil == Panel_Dialog_ServantList_All or false == Panel_Dialog_ServantList_All:GetShow() then
    return
  end
  PaGlobal_ServantList_All:subMenuClose()
...
function HandleEventLUp_ServantList_All_SupplyServant()
  ...
  elseif true == _ContentsGroup_ServantFantasyMix then
    _desc = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_SERVANT_STABLE_SUPPLY_2", "resultMoney", resultMoney) .. PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_SELL_WITHITEM_MSG")
  elseif true == _ContentsGroup_HorseDeliveryEvent then
    ...
    _desc = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_SERVANT_STABLE_SUPPLY", "resultMoney", resultMoney) .. PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_SELL_WITHITEM_MSG")
  end
  messageboxData = {
    title = _title,
    content = _desc,
...
function HandleEventLUp_ServantList_All_Regist_To_ServantMarket()
  ...
  if false == servantInfo:isTrade() then
    messageboxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_STABEL_REGISTERMARKET_NOTIFY_TITLE"),
      content = PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_HORSEMARKET_CANTREGISTER"),
...
function HandleEventLUp_ServantList_All_Register_To_WorldMarket()
  if nil == Panel_Dialog_ServantList_All or false == Panel_Dialog_ServantList_All:GetShow() then
    return
  end
  PaGlobal_ServantList_All:subMenuClose()
  ...
  local function registerMarket()
    stable_pushServantToWorldMarket(currentServantNo)
  end
  local _title = PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_STABEL_REGISTERMARKET_NOTIFY_TITLE")
  local _desc = PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_STABEL_REGISTERMARKET_NOTIFY_MSG")
  local _confirmFunction = registerMarket
...
function HandleEventLUp_ServantList_All_TransferServant()
  ...
  PaGlobal_ServantList_All:subMenuClose()
  HandleEventOut_ServantList_All_HideChangeServantRegionTooltip()
  audioPostEvent_SystemUi(0, 0)
...
function HandleEventLUp_ServantList_All_TransferBackwardRegionList()
  if nil == Panel_Dialog_ServantList_All or false == Panel_Dialog_ServantList_All:GetShow() or nil == Panel_Dialog_ServantTransferList_All then
    return
  end
  PaGlobal_ServantList_All:subMenuClose()
  ...
  PaGlobalFunc_ServantTransferBackwardRegionList_All_Open()
end
function HandleEventLUp_ServantList_All_ResetServant()
  ...
  if __eVehicleType_Horse == servantInfo:getVehicleType() then
    ...
    if speedAddItemUseCount > 0 or acceAddItemUseCount > 0 or cornerAddItemUseCount > 0 or brakeAddItemUseCount > 0 then
      _desc = PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_SETBEGINNINGLEVEL_ADDSTAT_INFO")
    end
  end
...
function HandleEventLUp_ServantList_All_ResetDeadCount()
  ...
  local function guildClearDeadCount()
    ...
    local servantNo = servantInfo:getServantNo()
    guildStable_clearDeadCount(servantNo, false)
  end
  local function oceanShipClearDeadCount()
    stable_clearOceanShipDeadCount(currentServantNo, CppEnums.ItemWhereType.eMoneyInventory)
  end
  local _title = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ALERT_NOTIFICATIONS")
...
function HandleEventLUp_ServantList_All_StableSlotBuy()
  local _isNpcType = PaGlobalFunc_ServantFunction_All_Get_NpcType()
  if _isNpcType == nil then
    return
  end
  local function easyBuySlot()
    if nil ~= Panel_IngameCashShop_EasyPayment then
      if _isNpcType == PaGlobal_ServantList_All._ENUM_NPC_TYPE._LAND then
        PaGlobal_EasyBuy:Open(15, getCurrentWaypointKey())
      else
        PaGlobal_EasyBuy:Open(78, getCurrentWaypointKey())
      end
    end
  end
  ...
  if _isNpcType == PaGlobal_ServantList_All._ENUM_NPC_TYPE._LAND then
    _desc = PAGetString(Defines.StringSheet_GAME, "LUA_STABLELIST_EASYBUY")
  else
    _desc = PAGetString(Defines.StringSheet_GAME, "LUA_STABLELIST_SHIP_EASYBUY")
  end
...
function HandleEventLUp_ServantList_All_ServantLookChange()
  ...
  if servantInfo:isDreamAdooanat() == true and _ContentsGroup_FantasyHorse == true then
    PaGlobalFunc_ServantFantasySkin_Open()
  else
...
function HandleEventLUp_ServantList_All_Register_To_RentMarket()
  ...
  local currentServantNo = PaGlobal_ServantList_All:stableList_SelectSlotNo(PaGlobal_ServantList_All._selectSlotNo)
  if nil ~= currentServantNo then
    PaGlobalFunc_ServantNameRegist_All_OpenByEventType(CppEnums.ServantRegist.eEventType_RegisterRent)
  end
end
...
function HandleEventLUp_ServantList_All_FantasyMixOpen()
  if false == _ContentsGroup_ServantFantasyMix or false == Panel_Dialog_ServantList_All:GetShow() then
    return
  end
  ...
  PaGlobalFunc_ServantExchange_Fantasy_All_Open()
end
...
function HandleEventLUp_ServantList_All_RecoverInjury()
  ...
  local _desc = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_GUILDSTABLE_MSG_INJURYRECOVERY", "money", makeDotMoney(needMoney))
...
function HandleEventLUp_ServantList_All_Upgrade()
  ...
  if nil ~= servantInfo then
    PaGlobal_ServantUpgrade:open(servantInfo:getCharacterKeyRaw(), true)
  end
end
function HandlerEventLUp_ServantList_All_ShipFoodFeed()
  ...
  function shipFoodFeedConfirm()
    ...
    if false == isGuild then
      moneyWhereType = CppEnums.ItemWhereType.eMoneyInventory
    else
      local guildWrapper = ToClient_GetMyGuildInfoWrapper()
      if nil == guildWrapper then
        return
...
function PaGlobalFunc_ServantList_All_CheckMapaeExist()
  ...
  for invenType = 1, 2 do
    ...
    for slotIdx = inventorySlotNoUserStart(), self._maxInvenCount - 1 do
      ...
      if nil ~= itemWrapper and (GetUIMode() == Defines.UIMode.eUIMode_Stable or GetUIMode() == Defines.UIMode.eUIMode_Tutorial) and not PaGlobalFunc_ServantFunction_All_EffectFilter_Mapae(slotNo, itemWrapper) then
...
function PaGlobalFunc_ServantInfo_All_CheckHaveRegistrableVehicle()
  if nil == Panel_Dialog_ServantList_All then
    return false
  end
  return PaGlobal_ServantList_All:checkHaveRegistrableVehicle()
end
function PaGlobalFunc_ServantInfo_All_setRegistButton(isHaveRegistrableVehicle)
  if nil == Panel_Dialog_ServantList_All then
    return
  end
  PaGlobal_ServantList_All:setRegistButton(isHaveRegistrableVehicle)
end
function PaGlobalFunc_ServantInfo_All_ClearSealMyServantNo()
  PaGlobal_ServantList_All.sealMyServantNo = nil
end
function PaGlobalFunc_ServantInfo_All_RightCheck(eRightType)
  local selfPlayer = getSelfPlayer()
  if nil == selfPlayer then
    return false
  end
  local function OriginalCheck()
    if __eGuildRightType_ChangeServantToReward == eRightType then
      return true
    end
    return false
  end
...
function HandleEventOn_ServantList_All_ShowMutationHorseTooltip(sortedIndex)
  if nil == Panel_Dialog_ServantList_All then
    return
  end
  local uiControl, name, desc
  uiControl = PaGlobal_ServantList_All._tooltipShowList[sortedIndex]
  if uiControl == nil then
    return
  end
  name = PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_TWINS_TOOLTIP_TITLE")
  desc = PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_TWINS_TOOLTIP_DESC")
  TooltipSimple_Show(uiControl, name, desc)
end
function HandleEventOn_ServantList_All_HideMutationHorseTooltip()
  if nil == Panel_Dialog_ServantList_All then
    return
  end
  TooltipSimple_Hide()
end
function HandleEventOn_ServantList_All_ShowCurrentHorseTooltip()
  if nil == Panel_Dialog_ServantList_All then
    return
  end
  local uiControl, name, desc
  uiControl = PaGlobal_ServantList_All._ui._icon_CurrentServant_Swift
  if uiControl == nil then
    return
  end
  name = PAGetString(Defines.StringSheet_GAME, "LUA_SERVANT_TWINS_TOOLTIP_TITLE")
