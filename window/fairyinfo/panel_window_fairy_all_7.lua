...
function PaGlobal_FairyTierUpgrade_All:AlignKeyGuide()
  ...
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(self._keyguide, self._ui.stc_BottomKeyGuide, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP)
end
...
function PaGlobal_FairyTierUpgrade_All:setSubjectItemCount(count)
  ...
  local successPercentStr = ""
  if _ContentsGroup_ViewRate == true then
    successPercentStr = self._currentSuccessRate / 10000 * self._currentItemStackCount
  else
    successPercentStr = tostring(self:decimalRoundForPercentValue(self._currentSuccessRate / 10000 * self._currentItemStackCount))
  end
...
function PaGlobal_FairyTierUpgrade_All:TierUpgrade()
  ...
  if self._currentItemStackCount >= self._maxSubjectCount then
    ...
    local rateString = ""
    if _ContentsGroup_ViewRate == true then
      rateString = tostring(self._currentSuccessRate / 10000 * self._currentItemStackCount)
    else
      rateString = tostring(self:decimalRoundForPercentValue(self._currentSuccessRate / 10000 * self._currentItemStackCount))
    end
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_FAIRYTIERUPGRADE_ONTRY_TITLE"),
      content = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_FAIRYTIERUPGRADE_ONTRY_DESC", "percentage", rateString),
      functionYes = PaGlobal_FairyTierUpgrade_TierUpgradeConfirm_All,
      functionNo = MessageBox_Empty_function,
      priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
    }
