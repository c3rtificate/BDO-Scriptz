...
function HandleEventLUp_FairyInfo_OpenRateViewByLink()
  PaGlobalFunc_Util_OpenRateView(PaGlobal_FairyInfo_Close_All)
end
function HandleEventOnOut_FairyInfo_ChangeRateViewButtonText(isOn)
  if _ContentsGroup_UsePadSnapping == true then
    if isOn == true then
      Panel_FairyInfo_All:registerPadEvent(__eConsoleUIPadEvent_Up_A, "")
      PaGlobal_FairyInfo_All._ui_console.stc_KeyGuide_A:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_COMMON_SELECT"))
    else
      Panel_FairyInfo_All:registerPadEvent(__eConsoleUIPadEvent_Up_A, "InputMLClick_FairyInfo_SummonFairy_All()")
      if PaGlobal_FairyInfo_All._isUnseal == true then
        PaGlobal_FairyInfo_All._ui_console.stc_KeyGuide_A:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_FAIRY_INFO_UNSUMMON"))
      else
        PaGlobal_FairyInfo_All._ui_console.stc_KeyGuide_A:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_FAIRY_INFO_SUMMON"))
      end
    end
    PaGlobal_FairyInfo_All:AlignKeyGuide()
  end
  if isOn == true then
    PaGlobal_FairyInfo_All._ui.txt_RateView:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_RATEDESC_HERE_CLICK_ON"))
  else
    PaGlobal_FairyInfo_All._ui.txt_RateView:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_RATEDESC_HERE_CLICK"))
  end
end
...
function FromClient_FairyGrowth_PetInfoChanged_All(petNo)
  ...
  if petNo ~= PaGlobal_FairyInfo_GetFairyNo_All() then
    return
  end
  PaGlobal_FairyGrowth_All:petInfoChanged(petNo)
  FromClient_Fairy_Update_All()
end
...
function PaGlobal_FairySetting_Open_All(petNoStr)
  ...
  if _ContentsGroup_OneButtonBuff == true and PaGlobal_AutoUseBuffItem._isOpenedByFairy == true then
...
function PaGlobalFunc_FairySkill_OpenRate()
  if nil == Panel_Window_FairySkill_All then
    return
  end
  ...
  PaGlobal_FairySkill_All:prepareOpenRateView()
end
...
function InputMLUp_FairyChangeSkill_SelectSkill(key, isPremium)
  if _ContentsGroup_FiveTierFairy == false then
    isPremium = false
  end
  PaGlobal_FairyChangeSkill_All:selectSkill(key, isPremium)
end
function InputMLUp_FairyChangeSkill_SelectPremiumSkill(key)
  if _ContentsGroup_FiveTierFairy == false then
    return
  end
  PaGlobal_FairyChangeSkill_All:selectPremiumSkill(key)
  PaGlobal_FairySkill_All:SetCheck(key)
end
function InputMLUp_FairyChangeSkill_Confirm()
  PaGlobal_FairyChangeSkill_All:confirmMessageBox()
end
function InputMLUp_FairyPremiumChangeSkill_Confirm()
  if _ContentsGroup_FiveTierFairy == false then
    return
  end
  PaGlobal_FairyChangeSkill_All:confirmPremiumMessageBox()
end
function PaGlobal_FairyChangeSkill_Open_All(isPremium)
  ...
  if _ContentsGroup_FiveTierFairy == false then
    isPremium = _ContentsGroup_FiveTierFairy
  end
  PaGlobal_FairyChangeSkill_All:prepareOpen(isPremium)
end
...
function FromClient_FairyChangeSkill_UpdatePerFrame(deltaTime)
  ...
  if PaGlobal_FairyChangeSkill_All._ANI_STATE.PLAY == PaGlobal_FairyChangeSkill_All._changeAniState then
    ...
    if PaGlobal_FairyChangeSkill_All._aniEndTime <= PaGlobal_FairyChangeSkill_All._aniStartTime then
      ...
      if PaGlobal_FairyChangeSkill_All._isPremium == true then
        PaGlobal_FairyChangeSkill_All:premiumConfirm()
      else
...
function PaGlobal_FairyChangeSkill_Viewhelper_Open_All(isPremium)
  ...
  if true == Panel_FairySkillChange_Viewhelp:GetShow() then
    ...
    PaGlobal_FairyChangeSkill_Viewhelper_All:prepareOpen(isPremium)
  end
end
