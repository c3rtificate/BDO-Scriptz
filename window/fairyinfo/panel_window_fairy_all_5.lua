function PaGlobal_FairySkill_All:initialize()
  ...
  self._ui._txt_RateDescPremium = UI.getChildControl(self._ui._stc_BottomDescBG, "StaticText_Desc_Premium")
  self._ui_console._stc_KeyGuide_ConsoleUI = UI.getChildControl(Panel_Window_FairySkill_All, "Static_KeyGuide_ConsoleUI")
  self._ui_console._txt_A_ConsoleUI = UI.getChildControl(self._ui_console._stc_KeyGuide_ConsoleUI, "StaticText_A_ConsoleUI")
...
function PaGlobal_FairySkill_All:SetUiSetting()
  ...
  if true == _ContentsGroup_FairySkillChangeRateView and true == _ContentsGroup_UsePadSnapping then
    ...
    if PaGlobal_FairyChangeSkill_All._isPremium == true then
      keyGuides = {
        self._ui_console._txt_A_ConsoleUI,
  ...
  elseif false == _ContentsGroup_FairySkillChangeRateView and true == _ContentsGroup_UsePadSnapping then
    ...
    self._ui_console.stc_KeyGuide_B:SetShow(false)
    self._ui_console._txt_A_ConsoleUI:SetShow(false)
...
function PaGlobal_FairySkill_All:prepareOpen(isAllSkillShow)
  ...
  self._ui.list2_Skill:SetMonoTone(false)
  self._ui.list2_Skill:SetEnable(true)
...
function PaGlobal_FairySkill_All:prepareOpenRateView()
  ...
  if PaGlobal_FairyChangeSkill_All._isPremium == true then
    titleStr = PAGetString(Defines.StringSheet_GAME, "LUA_FAIRYINFO_SKILLCHANGE_BUTTON_PREMIUM")
  end
  ...
  if false == _ContentsGroup_UsePadSnapping then
    ...
    self._ui_console._txt_A_ConsoleUI:SetShow(PaGlobal_FairyChangeSkill_All._isPremium == true)
    self._ui_console._txt_Y_ConsoleUI:SetShow(PaGlobal_FairyChangeSkill_All._isPremium == false)
  end
  self._ui._stc_BottomDescBG:SetShow(true)
  self:setAlignKeyGuide()
  self._ui.list2_Skill:SetMonoTone(false)
  self._ui.list2_Skill:SetEnable(true)
  self._ui._txt_RateDesc:SetShow(PaGlobal_FairyChangeSkill_All._isPremium == false)
  self._ui._txt_RateDescPremium:SetShow(PaGlobal_FairyChangeSkill_All._isPremium == true)
  PaGlobal_FairySkill_All:open()
end
...
function PaGlobal_FairySkill_All:update_AllSkillShow(isScroll)
  ...
  if true == PaGlobal_FairyInfo_All._isShowSkillRate and true == _ContentsGroup_FairySkillChangeRateView then
    ...
    for ii = 0, skillCount - 1 do
      ...
      local isMaxLevel = Toclient_FairySkill_IsMaxLevel(PaGlobal_FairyInfo_GetFairyKey_All(), ii)
      local isSkip = _ContentsGroup_FiveTierFairy == true and PaGlobal_FairyChangeSkill_All._isPremium == true and isMaxLevel == false
      if isSkip == false and rate ~= 0 then
...
function PaGlobal_FairySkill_All:validate()
  ...
  self._ui_console._txt_A_ConsoleUI:isValidate()
...
function PaGlobal_FairySkill_All:SetCheck(key)
  if nil == Panel_Window_FairySkill_All then
    return
  end
  local control = self._ui.list2_Skill:GetContentByKey(toInt64(0, key))
  if PaGlobal_FairySkill_All._ui._stc_CheckBG ~= nil then
    PaGlobal_FairySkill_All._ui._stc_CheckBG:SetShow(false)
  end
  local checkBG = UI.getChildControl(control, "Static_SelectedBG")
  PaGlobal_FairySkill_All._ui._stc_CheckBG = checkBG
  if PaGlobal_FairySkill_All._ui._stc_CheckBG == nil then
    return
  end
  PaGlobal_FairySkill_All._ui._stc_CheckBG:SetShow(true)
end
function PaGlobal_FairySkill_CreateList_All(control, key)
  ...
  if true == PaGlobal_FairyInfo_All._isShowSkillRate and true == _ContentsGroup_FairySkillChangeRateView then
    ...
    if PaGlobal_FairyChangeSkill_All._isPremium == true then
      control:addInputEvent("Mouse_LUp", "InputMLUp_FairyChangeSkill_SelectPremiumSkill(" .. skillkey .. ")")
      _static_SkillIcon1:addInputEvent("Mouse_LUp", "InputMLUp_FairyChangeSkill_SelectPremiumSkill(" .. skillkey .. ")")
    else
      control:removeInputEvent("Mouse_LUp")
      rate = ToClient_GetLeanableFairySkillRate(skillkey)
    end
    if 0 ~= rate then
      _staticText_SkillRate:SetShow(true)
      _staticText_SkillRateText:SetShow(true)
      local rateString = PaGlobalFunc_Util_MakeRateDotString(rate / 100, 4, true)
      _staticText_SkillRateText:SetText(rateString)
    else
