...
function PaGlobal_FairySetting_All:initialize()
  ...
  self._ui.stc_HPItemSlot = {}
  SlotItem.new(self._ui.stc_HPItemSlot, "HpItem", 0, self._ui.stc_HPItemSlotBG, slotConfig)
  self._ui.stc_HPItemSlot:createChild()
  self._ui.stc_HPItemSlot.icon:SetSize(self._ui.stc_HPItemSlotBG:GetSizeX(), self._ui.stc_HPItemSlotBG:GetSizeY())
  self._ui.stc_HPItemSlot.icon:SetIgnore(true)
  ...
  self._ui.stc_MPItemSlot = {}
  SlotItem.new(self._ui.stc_MPItemSlot, "MpItem", 0, self._ui.stc_MPItemSlotBG, slotConfig)
  self._ui.stc_MPItemSlot:createChild()
  self._ui.stc_MPItemSlot.icon:SetSize(self._ui.stc_MPItemSlotBG:GetSizeX(), self._ui.stc_MPItemSlotBG:GetSizeY())
  self._ui.stc_MPItemSlot.icon:SetIgnore(true)
  ...
  local list2Content = UI.getChildControl(self._ui.list2_Potion, "List2_1_Content")
  local list2Button = UI.getChildControl(list2Content, "CheckButton_ItemList")
  local list2SlotBG = UI.getChildControl(list2Button, "Static_ItemIconBG")
  local slot = {}
  SlotItem.new(slot, "PotionList", 0, list2SlotBG, slotConfig)
  slot:createChild()
  ...
  self._ui_console.stc_KeyGuide_RS = UI.getChildControl(self._ui.stc_BottomKeyGuide, "StaticText_ControlScroll")
  self._keyguide = {
    self._ui_console.stc_KeyGuide_RS,
  ...
  self._currentTier = 0
  self._currentHpWhereType = CppEnums.ItemWhereType.eCount
  self._currentMpWhereType = CppEnums.ItemWhereType.eCount
...
function PaGlobal_FairySetting_All:AlignKeyGuide()
  ...
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(self._keyguide, self._ui.stc_BottomKeyGuide, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP, nil, nil)
end
function PaGlobal_FairySetting_All:registEventHandler()
  ...
  self._ui.stc_HPItemSlotBG:addInputEvent("Mouse_RUp", "InputMLUp_FairySetting_ClearHP_All()")
  self._ui.stc_MPItemSlotBG:addInputEvent("Mouse_RUp", "InputMLUp_FairySetting_ClearMP_All()")
...
function PaGlobal_FairySetting_All:updatePotionSetting(updateData)
  ...
  if nil ~= settingData and true == updateData then
    ...
    self._currentHpWhereType = settingData._hpWhereType
    self._currentMpWhereType = settingData._mpWhereType
  end
  ...
  if nil ~= ItemHpSSW then
    self._ui.stc_HPItemSlot:setItemByStaticStatus(ItemHpSSW, 0, -1, false, false, false, 0, 0, false, false, false, CppEnums.ItemWhereType.eFamilyInventory == self._currentHpWhereType)
    self._ui.stc_QuestionMarkHP:SetShow(false)
  else
    self._currentHpKey = nil
    self._ui.stc_HPItemSlot:clearItem()
  ...
  if nil ~= ItemMpSSW then
    self._ui.stc_MPItemSlot:setItemByStaticStatus(ItemMpSSW, 0, -1, false, false, false, 0, 0, false, false, false, CppEnums.ItemWhereType.eFamilyInventory == self._currentMpWhereType)
    self._ui.stc_QuestionMarkMP:SetShow(false)
  else
    self._currentMpKey = nil
    self._ui.stc_MPItemSlot:clearItem()
...
function PaGlobal_FairySetting_All:updatePotionList()
  ...
  if true == _ContentsGroup_FamilyInventory then
    local familyInventory = selfPlayer:getInventoryByType(CppEnums.ItemWhereType.eFamilyInventory)
    local familyInventorySize = familyInventory:size()
    for invenIdx = __eTInventorySlotNoUseStart, familyInventorySize - 1 do
      local itemWrapper = getInventoryItemByType(CppEnums.ItemWhereType.eFamilyInventory, invenIdx)
      if nil ~= itemWrapper and false == self:FileterForSetting(invenIdx, false, CppEnums.ItemWhereType.eFamilyInventory) then
        ...
        if nil ~= itemSSW and nil ~= itemKey then
          ...
          self._potionList[self._potionListCount]._whereType = CppEnums.ItemWhereType.eFamilyInventory
  ...
  for invenIdx = __eTInventorySlotNoUseStart, invenSize - 1 do
    ...
    if nil ~= itemWrapper and false == self:FileterForSetting(invenIdx, false, CppEnums.ItemWhereType.eInventory) then
      ...
      if nil ~= itemSSW and nil ~= itemKey then
        ...
        self._potionList[self._potionListCount]._slotNo = invenIdx
        self._potionList[self._potionListCount]._itemKey = itemKey
        self._potionList[self._potionListCount]._whereType = CppEnums.ItemWhereType.eInventory
        self._ui.list2_Potion:getElementManager():pushKey(toInt64(0, self._potionListCount))
      end
    end
  end
...
function PaGlobal_FairySetting_All:Request()
  ...
  if nil == self._currentHpKey then
    ...
    self._cacheSetting._hpWhereType = CppEnums.ItemWhereType.eCount
  end
  if nil == self._currentMpKey then
    self._currentMpKey = ItemEnchantKey(0, 0)
    self._cacheSetting._mpRate = 0
    self._cacheSetting._mpWhereType = CppEnums.ItemWhereType.eCount
  end
  if nil == self._currentFairyNoStr then
    return
  end
  ToClient_setFairySettingData(tonumber64(self._currentFairyNoStr), self._currentHpKey, self._currentHpRate, self._currentHpWhereType, self._currentMpKey, self._currentMpRate, self._currentMpWhereType)
  self._currentFairyNoStr = nil
  self:close()
...
function PaGlobal_FairySetting_All:SetPotion(key)
  ...
  elseif 0 == ToClient_getFairyPotionType(itemKey, classType) then
    ...
    self._currentHpWhereType = self._potionList[key]._whereType
    self._ui.stc_QuestionMarkHP:SetShow(false)
  else
    self._currentMpKey = itemKey
    self._currentMpWhereType = self._potionList[key]._whereType
    self._ui.stc_QuestionMarkMP:SetShow(false)
  end
  self:updatePotionSetting(false)
end
...
function PaGlobal_FairySetting_All:ShowToolTip(index, isShow)
  ...
  if nil ~= self._potionList[index] then
    whereType = self._potionList[index]._whereType
...
function PaGlobal_FairySetting_All:createPotionList(control, key64)
  ...
  local itemWrapper = getInventoryItemByType(self._potionList[key]._whereType, slotNo)
  if nil == itemWrapper then
    return
  end
  local itemSSW = itemWrapper:getStaticStatus()
  ...
  local stc_IconBg = UI.getChildControl(chk_Potion, "Static_ItemIconBG")
  local slot = {}
  SlotItem.reInclude(slot, "PotionList", 0, stc_IconBg, slotConfig)
  slot.icon:SetSize(stc_IconBg:GetSizeX(), stc_IconBg:GetSizeY())
  slot:setItem(itemWrapper, slotNo, false, false, CppEnums.ItemWhereType.eFamilyInventory == self._potionList[key]._whereType)
...
function PaGlobal_FairySetting_All:ShowPotionTooltip(key, isShow)
  ...
  if isShow then
    ...
    PaGlobalFunc_TooltipInfo_Open(Defines.TooltipDataType.ItemWrapper, getInventoryItemByType(self._potionList[key]._whereType, self._potionList[key]._slotNo), Defines.TooltipTargetType.Item, 0)
  else
    TooltipSimple_Hide()
    PaGlobalFunc_TooltipInfo_Close()
  end
