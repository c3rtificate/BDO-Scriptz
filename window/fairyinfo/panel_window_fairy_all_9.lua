function PaGlobal_FairyChangeSkill_All:initialize()
  ...
  self._ui_console.txt_KeyGuide_A = UI.getChildControl(self._ui_console.stc_KeyGuide, "StaticText_A_ConsoleUI")
  self._ui_console.txt_KeyGuide_B = UI.getChildControl(self._ui_console.stc_KeyGuide, "StaticText_B_Console")
  ...
  if _ContentsGroup_UsePadSnapping then
    ...
    PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyGuide, self._ui_console.stc_KeyGuide, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP)
  end
...
function PaGlobal_FairyChangeSkill_All:prepareOpen(isPremium)
  ...
  PaGlobal_FairyChangeSkill_All:preloadTexture()
  ...
  if self._isPremium == true then
    self._ui_pc.btn_Change_pc:SetMonoTone(true)
    self._ui_pc.btn_Change_pc:SetEnable(false)
    self._ui_pc.btn_Change_pc:addInputEvent("Mouse_LUp", "InputMLUp_FairyPremiumChangeSkill_Confirm()")
  else
    self._ui_pc.btn_Change_pc:addInputEvent("Mouse_LUp", "InputMLUp_FairyChangeSkill_Confirm()")
  end
  PaGlobal_FairyChangeSkill_FairySetting()
  PaGlobal_FairyChangeSkill_ChangeBtnSetting()
  PaGlobal_FairyChangeSkill_Viewhelper_Open_All(isPremium)
  PaGlobal_FairyChangeSkill_FairySkillUpdate(PaGlobal_FairyInfo_All._skillKey)
...
function PaGlobal_FairyChangeSkill_All:createSlot()
  ...
  if _ContentsGroup_FiveTierFairy == true then
    Panel_Window_FairyChangeSkill_All:SetSize(Panel_Window_FairyChangeSkill_All:GetSizeX(), Panel_Window_FairyChangeSkill_All:GetSizeY() + self._ui.btn_Template:GetSizeY() + 70)
    self._ui.btn_ViewSkill:ComputePos()
    self._ui.btn_VIewHelp:ComputePos()
    self._ui_pc.btn_Change_pc:ComputePos()
  end
end
function PaGlobal_FairyChangeSkill_All:update(skillKeyList)
  ...
  PaGlobal_FairyChangeSkill_FairySetting()
  local count = 0
  for ii = 0, #self._skillKeyList do
    local key = skillKeyList[ii]
    if nil ~= key then
      ...
      slot.SkillSlotBg:ChangeTextureInfoNameDefault("Icon/" .. skillTypeSSW:getIconPath())
      local x1, y1, x2, y2 = setTextureUV_Func(slot.SkillSlotBg, 0, 0, 44, 44)
      slot.SkillSlotBg:getBaseTexture():setUV(x1, y1, x2, y2)
      ...
      slot.Btn:addInputEvent("Mouse_LUp", "InputMLUp_FairyChangeSkill_SelectSkill(" .. key .. ", " .. tostring(self._isPremium) .. ")")
...
function PaGlobal_FairyChangeSkill_All:preloadTexture()
  for ii = 0, PaGlobal_FairyInfo_All.eFairyMaxEquipSkill do
    local skillSSW = ToClient_Fairy_EquipSkillWrraper(ii)
    if nil == skillSSW then
      return
    end
    local skillTypeSSW = skillSSW:getSkillTypeStaticStatusWrapper()
...
function PaGlobal_FairyChangeSkill_All:settingReset()
  ...
  self._selectChangeSkillKey = nil
  PaGlobal_FairyChangeSkill_ChangeBtnSetting()
  PaGlobal_FairyChangeSkill_CurrentItemCountSetting()
end
function PaGlobal_FairyChangeSkill_All:fairySetting()
  ...
  if nil ~= self._fairyLevel then
    ...
    if self._isPremium == true then
      self._ui_pc.btn_Change_pc:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_FAIRYINFO_SKILLCHANGE_BUTTON_PREMIUM"))
      self._ui.txt_Title_Name:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_FAIRYINFO_SKILLCHANGE_BUTTON_PREMIUM"))
    else
      ...
      self._ui.txt_Title_Name:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_WORKERCHANGESKILL_TITLE"))
    end
    self._ui.btn_VIewHelp:addInputEvent("Mouse_LUp", "PaGlobal_FairyChangeSkill_Viewhelper_Open_All(" .. tostring(self._isPremium) .. ")")
  end
end
function PaGlobal_FairyChangeSkill_All:selectSkill(key, isPremium)
  ...
  if true == _ContentsGroup_FairySkillChangeRateView then
    ToClient_UpdateLeanableFairySkillRate(key)
    PaGlobal_FairyInfo_All._isShowSkillRate = true
    PaGlobal_FairySkill_All._isAllSkillShow = true
    PaGlobalFunc_FairySkill_OpenRate()
  end
  PaGlobal_FairyChangeSkill_ChangeBtnSetting()
end
function PaGlobal_FairyChangeSkill_All:selectPremiumSkill(key)
  local skillSSW = ToClient_Fairy_EquipSkillWrraper(key)
  if nil == skillSSW then
    return
  end
  ...
  self._selectChangeSkillKey = key
  self._ui.stc_GetSkillSlot:SetShow(true)
  self._ui.stc_GetSkillSlot:getBaseTexture():setUV(0, 0, 1, 1)
  self._ui.stc_GetSkillSlot:ChangeTextureInfoName("Icon/" .. skillTypeSSW:getIconPath())
  self._ui.stc_GetSkillSlot:setRenderTexture(self._ui.stc_GetSkillSlot:getBaseTexture())
  self._ui.stc_GetSkillSlot:addInputEvent("Mouse_On", "PaGlobal_FairyChangeSkill_ShowTooltip(true, " .. key .. "," .. "200" .. ")")
...
function PaGlobal_FairyChangeSkill_All:premiumConfirm()
  ToClient_requestPremiumChangeFairySkill(self._fairyNo, self._selectSkillKey, self._selectChangeSkillKey)
  PaGlobal_FairyChangeSkill_FairySkillSettingReset()
end
...
function PaGlobal_FairyChangeSkill_All:confirmPremiumMessageBox()
  if self._selectSkillKey == nil or self._selectChangeSkillKey == nil then
    return
  end
  local skillSSW = ToClient_Fairy_EquipSkillWrraper(self._selectSkillKey)
  ...
  local changeSkillSSW = ToClient_Fairy_EquipSkillWrraper(self._selectChangeSkillKey)
  if changeSkillSSW == nil then
    return
  end
  local changeSkillTypeSSW = changeSkillSSW:getSkillTypeStaticStatusWrapper()
  if changeSkillTypeSSW == nil then
    return
  end
  local changeSkillName = changeSkillTypeSSW:getName()
  local selfPlayer = getSelfPlayer():get()
  ...
  for invenIdx = 0, invenSize - 1 do
    ...
    if itemWrapper ~= nil then
      local itemSSW = itemWrapper:getStaticStatus()
      if itemSSW:get():getContentsEventType() == __eContentsType_PremiumChangeFairySkill then
        itemCount = itemCount + Int64toInt32(itemWrapper:getCount())
      end
    end
  end
  if needCount > itemCount or 0 == itemCount then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_FAIRYINFO_SKILLCHANGE_NEEDMORETICKET"))
  else
    local FunctionYes = function()
      PaGlobal_FairyChangeSkill_All:confirmStartAni()
    end
    local _title = PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_NOTIFY")
    local _contenet = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_FAIRYINFO_CHANGE_PREMIUM_MSG", "skillname", skillName, "skillname2", changeSkillName)
    local messageBoxData = {
      title = _title,
      content = _contenet,
      functionYes = FunctionYes,
...
function PaGlobal_FairyChangeSkill_All:confirmStartAni()
  ...
  self._ui_pc.btn_Change_pc:SetMonoTone(true)
  self._ui_pc.btn_Change_pc:SetEnable(false)
  if self._isPremium == true and PaGlobal_FairySkill_All._ui.list2_Skill ~= nil then
    PaGlobal_FairySkill_All._ui.list2_Skill:SetMonoTone(true)
    PaGlobal_FairySkill_All._ui.list2_Skill:SetEnable(false)
  end
  for i = 0, self._learnSkillMaxCount - 1 do
    local slot = self._skillSlot[i]
...
function PaGlobal_FairyChangeSkill_All:confirmEndAni()
  ...
  self._ui_pc.btn_Change_pc:SetEnable(true)
  if self._isPremium == true and PaGlobal_FairySkill_All._ui.list2_Skill ~= nil then
    PaGlobal_FairySkill_All._ui.list2_Skill:SetMonoTone(false)
    PaGlobal_FairySkill_All._ui.list2_Skill:SetEnable(true)
  end
  for i = 0, self._learnSkillMaxCount - 1 do
    local slot = self._skillSlot[i]
    slot.Btn:SetMonoTone(false)
...
function PaGlobal_FairyChangeSkill_All:changeBtnSetting()
  if self._selectSkillKey ~= nil and self._isPremium == false then
    self._ui_pc.btn_Change_pc:SetEnable(true)
    self._ui_pc.btn_Change_pc:SetMonoTone(false)
  else
    self._ui_pc.btn_Change_pc:SetEnable(false)
...
function PaGlobal_FairyChangeSkill_All:currentItemCountSetting()
  ...
  local invenSize = selfPlayer:getInventorySlotCount(true)
  local itemCount = 0
  local checkContentsType = CppEnums.ContentsEventType.ContentsType_ChangeFairySkill
  if self._isPremium == true then
    checkContentsType = __eContentsType_PremiumChangeFairySkill
  end
  for invenIdx = 0, invenSize - 1 do
    local itemWrapper = getInventoryItemByType(CppEnums.ItemWhereType.eCashInventory, invenIdx)
    if nil ~= itemWrapper then
      local itemSSW = itemWrapper:getStaticStatus()
      if itemSSW:get():getContentsEventType() == checkContentsType then
        itemCount = itemCount + Int64toInt32(itemWrapper:getCount())
        if _ContentsGroup_FiveTierFairy == false or checkContentsType == CppEnums.ContentsEventType.ContentsType_ChangeFairySkill then
          break
        end
      end
    end
...
function InputMLUp_FairySkill_Window_FairyRateDesc_Open()
  if nil == Panel_Window_FairySkill_All or nil == Panel_Window_FairyRateDesc then
    return
  end
  if false == _ContentsGroup_FairySkillChangeRateView then
    return
  end
  if false == PaGlobal_FairyInfo_All._isShowSkillRate then
    return
  end
  PaGlobal_Window_FairyRateDesc_Open()
end
function InputMOnOut_FairySkill_Window_FairyRateDesc_ShowToolTip(isShow)
  ...
  if false == isShow then
    ...
    local str = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_FAIRYRATE_DESC_TITLE")
    TooltipSimple_Show(PaGlobal_FairySkill_All._ui._btn_Question_pc, str, "")
  end
end
