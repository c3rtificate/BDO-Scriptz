...
function FromClient_CheckChinaProhibitedWord_Memo(filteredText)
  ...
  PaGlobal_Memo_All:SaveFilteredText(self._currentSaveMode, self._currentFocusId, filteredText)
end
function FromClient_CheckChinaProhibitedWord_Memo_Fail()
  if _ContentsOption_CH_CheckProhibitedWord == false then
    return
  end
  local self = PaGlobal_Memo_All
  ...
  if memoInfo:isOn() == true and self._isClose == true then
