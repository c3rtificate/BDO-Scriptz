...
function PaGlobal_Memo_All:registEventHandler()
  ...
  registerEvent("FromClient_CheckChinaProhibitedWord_Memo", "FromClient_CheckChinaProhibitedWord_Memo")
  registerEvent("FromClient_CheckChinaProhibitedWord_Memo_Fail", "FromClient_CheckChinaProhibitedWord_Memo_Fail")
  if _ContentsGroup_MemoRenewal == true then
    registerEvent("FromClient_SubAppUIResize", "HandleEventLUp_Memo_Main_All_StickyEnd")
  end
end
...
function PaGlobal_Memo_All:Save(saveMode, inputId)
  if _ContentsOption_CH_CheckProhibitedWord == true and saveMode ~= self._SaveMode.SETTING then
    PaGlobal_Memo_All:CheckProhibitedWord(saveMode, inputId)
    return
  end
  if inputId ~= nil then
...
function PaGlobal_Memo_All:StickyClearFocus()
  ...
  PaGlobal_Memo_All._currentSaveMode = nil
end
...
function PaGlobal_Memo_All:getInfoByCurData(saveMode, id)
  if nil == Panel_Window_Memo_Main_All then
    return
  end
  local stickyMemo = PaGlobal_Memo_All._stickyMemoList[id]
...
function PaGlobal_Memo_All:SaveFilteredText(saveMode, inputId, filteredText)
  local id = inputId
  local recheckSaveMode = PaGlobal_Memo_All:ReCheckSaveMode(saveMode)
  if recheckSaveMode == nil then
    return
  end
  local info = PaGlobal_Memo_All:getInfoByCurData(saveMode, id)
  local result = ToClient_updateMemoFilteredText(info, recheckSaveMode, filteredText)
  if result == false or recheckSaveMode == PaGlobal_Memo_All._SaveMode.SETTING then
    return
  end
  PaGlobal_Memo_All:StickyTextUpdate(id)
  PaGlobal_Memo_All:ListUpdate()
