...
function PaGlobal_ReconnectUI_UpdateReconnectUI(deltaTime, panel)
  ...
  if true == isKeyDown_Once(CppEnums.VirtualKeyCode.KeyCode_ESCAPE) then
    if nil == PaGlobal_ChangeNickName_All then
      if false == self._isExit then
        exitGameClient(true)
      end
      self._isExit = true
    elseif false == PaGlobal_ChangeNickName_All._isChangeNickName then
      if false == self._isExit then
        exitGameClient(true)
      end
      self._isExit = true
  ...
  if false == self._isExit then
    if nil ~= PaGlobal_ChangeNickName_All and true == PaGlobal_ChangeNickName_All._isChangeNickName then
      stc_EscDesc:SetShow(false)
      stc_LoadingEffect:SetText(PAGetString(Defines.StringSheet_GAME, "GAME_WINDOW_RECONNECT_CHANGE_NICKNAME_MSG"))
    else
      stc_EscDesc:SetShow(true)
