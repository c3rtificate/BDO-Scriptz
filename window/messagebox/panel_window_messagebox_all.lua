...
local stc_MyMoney = UI.getChildControl(Panel_Window_MessageBox_All, "Static_MyMoney")
local txt_MyMoney = UI.getChildControl(stc_MyMoney, "StaticText_2")
local progress = UI.getChildControl(textContent, "Progress2_1")
local selectGradeBg = UI.getChildControl(Panel_Window_MessageBox_All, "Static_SelectGradeBg")
local textGrade = UI.getChildControl(selectGradeBg, "StaticText_Grade")
local buttonDown = UI.getChildControl(selectGradeBg, "Button_Down")
local buttonUp = UI.getChildControl(selectGradeBg, "Button_Up")
selectGradeBg:SetShow(false)
...
local checkCustom3 = UI.getChildControl(Panel_Window_MessageBox_All, "Button_CustomString3")
checkCustom1:SetShow(false)
checkCustom2:SetShow(false)
checkCustom3:SetShow(false)
...
local static_consoleUI_Y = UI.getChildControl(static_consloeUI, "StaticText_Y_ConsoleUI")
local static_consoleUI_LB = UI.getChildControl(selectGradeBg, "Static_Down_ConsoleUI")
local static_consoleUI_RB = UI.getChildControl(selectGradeBg, "Static_Up_ConsoleUI")
...
local selectResult = false
local solareMessageOpen = false
MessageBoxCheck = {}
...
local function updateRefuseType(refuseType)
  ...
  if _refuseType.PARTY == refuseType and false == isConsole then
    ...
    isTooltip = true
  elseif _refuseType.PVP == refuseType and false == isConsole then
    ...
    refuseInvite:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_ALWAYSREFUSE"))
    isTooltip = true
  elseif _refuseType.EXCHANGE == refuseType and false == isConsole then
    ...
    refuseInvite:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_ALWAYSREFUSE"))
    isTooltip = true
  elseif _refuseType.UISCALE == refuseType and false == isConsole then
  ...
  elseif _refuseType.GUILDGIFT == refuseType then
    if true == PaGlobal_IngameCashshop_GuildGiveaway._isNotifyBuyer then
      refuseInvite:SetCheck(false)
    else
      refuseInvite:SetCheck(true)
    end
    refuseInvite:SetShow(true)
    if true == isConsole then
      Panel_Window_MessageBox_All:registerPadEvent(__eConsoleUIPadEvent_Y, "PaGlobalFunc_Clicked_RefuseButton(4)")
      static_consoleUI_Y:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GUILDGIFT_BUYER_NAME_DISPLAY_CHECK"))
      static_consoleUI_Y:SetShow(true)
      PaGlobalFunc_MessageBox_RefreshConsoleKeyGuide()
    else
      refuseInvite:addInputEvent("Mouse_LUp", "PaGlobal_MessageBox_RefuseOption(4)")
    end
    refuseInvite:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GUILDGIFT_BUYER_NAME_DISPLAY_CHECK"))
    refuseInvite:addInputEvent("Mouse_On", "")
    refuseInvite:addInputEvent("Mouse_Out", "")
  elseif _refuseType.HOPEGAUGE == refuseType then
    if true == isConsole then
      Panel_Window_MessageBox_All:registerPadEvent(__eConsoleUIPadEvent_Y, "PaGlobalFunc_Clicked_RefuseButton(5)")
      static_consoleUI_Y:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_COMMON_SELECT"))
      static_consoleUI_Y:SetShow(true)
      PaGlobalFunc_MessageBox_RefreshConsoleKeyGuide()
    end
    refuseInvite:SetShow(true)
    refuseInvite:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_HOPE_POPUP_CHECKBOX_CHARGE_DESC"))
    refuseInvite:addInputEvent("Mouse_LUp", "")
  elseif _refuseType.PETGUIDE == refuseType then
    refuseInvite:SetCheck(false)
    refuseInvite:SetShow(true)
    refuseInvite:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_UPDATE_POPUP_BTN_CLOSE"))
    if true == isConsole then
      Panel_Window_MessageBox_All:registerPadEvent(__eConsoleUIPadEvent_Y, "PaGlobalFunc_Clicked_RefuseButton(6)")
      static_consoleUI_Y:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_UPDATE_POPUP_BTN_CLOSE"))
      static_consoleUI_Y:SetShow(true)
      PaGlobalFunc_MessageBox_RefreshConsoleKeyGuide()
    else
      refuseInvite:addInputEvent("Mouse_LUp", "PaGlobal_MessageBox_RefuseOption(6)")
    end
  elseif _refuseType.SOLARECUSTOMGAME == refuseType then
    refuseInvite:SetCheck(false)
    refuseInvite:SetShow(true)
    refuseInvite:addInputEvent("Mouse_LUp", "PaGlobal_MessageBox_RefuseOption(7)")
    refuseInvite:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_ALWAYSREFUSE"))
    isTooltip = true
  elseif _refuseType.HOPEGAUGESHOW == refuseType then
    if true == isConsole then
      Panel_Window_MessageBox_All:registerPadEvent(__eConsoleUIPadEvent_Y, "PaGlobalFunc_Clicked_RefuseButton(8)")
      static_consoleUI_Y:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_COMMON_SELECT"))
      static_consoleUI_Y:SetShow(true)
      PaGlobalFunc_MessageBox_RefreshConsoleKeyGuide()
    end
    refuseInvite:SetShow(true)
    refuseInvite:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_HOPE_POPUP_AUTOOPEN_CHECK"))
    refuseInvite:addInputEvent("Mouse_LUp", "")
  else
    refuseInvite:SetShow(false)
  end
  if true == isTooltip and false == isConsole then
...
local function messageCheckBox_Resize()
  ...
  local checkButtonGapY = 0
  if checkCustom2:GetShow() == true then
    checkButtonGapY = checkButtonGapY + checkCustom2:GetSizeY()
  end
  if checkCustom3:GetShow() == true then
    checkButtonGapY = checkButtonGapY + checkCustom3:GetSizeY()
  end
  Panel_Window_MessageBox_All:SetSize(Panel_Window_MessageBox_All:GetSizeX(), textBG:GetSizeY() + 97 + checkButtonGapY + padding * 2)
  orginPanelSizeY = Panel_Window_MessageBox_All:GetSizeY()
  Panel_Window_MessageBox_All:ComputePosAllChild()
end
function setCurrentMessageData(currentData, position, refuseType)
  if currentData ~= nil then
    ...
    static_consoleUI_Y:SetShow(false)
    selectGradeBg:SetShow(false)
    static_consoleUI_LB:SetShow(false)
    static_consoleUI_RB:SetShow(false)
    buttonDown:SetShow(false)
    buttonUp:SetShow(false)
    static_iconArea_BG:SetShow(false)
    stc_MyMoney:SetShow(false)
    ...
    if currentData.functionYes ~= nil then
      ...
      if currentData.btnYesText ~= nil then
        buttonYes:SetText(currentData.btnYesText)
        static_consoleUI_A:SetText(currentData.btnYesText)
      else
    ...
    elseif currentData.functionApply ~= nil then
      ...
      buttonShowCount = buttonShowCount + 1
      static_consoleUI_A:SetShow(isConsole)
      if currentData.btnApplyText ~= nil then
        buttonApply:SetText(currentData.btnApplyText)
        static_consoleUI_A:SetText(currentData.btnApplyText)
      else
    ...
    if currentData.functionNo ~= nil then
      ...
      if currentData.btnNoText ~= nil then
        buttonNo:SetText(currentData.btnNoText)
        static_consoleUI_B:SetText(currentData.btnNoText)
      else
    ...
    if currentData.functionCancel ~= nil then
      ...
      buttonShowCount = buttonShowCount + 1
      static_consoleUI_B:SetShow(isConsole)
      if currentData.btnCancelText ~= nil then
        buttonCancel:SetText(currentData.btnCancelText)
        static_consoleUI_B:SetText(currentData.btnCancelText)
      else
        static_consoleUI_B:SetText(PAGetString(Defines.StringSheet_RESOURCE, "MESSAGEBOX_BTN_CANCEL_WITHOUT_KEY"))
      end
      if isConsole == true and currentData.isLoading == true then
        static_consoleUI_B:SetShow(false)
      end
    end
    ...
    if false == ToClient_isGamePlayProcessor() and true == _ContentsGroup_UsePadSnapping and false == _ContentsGroup_RenewUI and false == ToClient_IsPadContollerForPC() then
    ...
    if currentData.isSelectGrade ~= nil and currentData.isSelectGrade == true then
      selectGradeBg:SetShow(true)
      selectGrade = currentData.selectGrade
      minGrade = currentData.minGrade
      maxGrade = currentData.maxGrade
      static_consoleUI_LB:SetShow(_ContentsGroup_UsePadSnapping)
      static_consoleUI_RB:SetShow(_ContentsGroup_UsePadSnapping)
      buttonDown:SetShow(not _ContentsGroup_UsePadSnapping)
      buttonUp:SetShow(not _ContentsGroup_UsePadSnapping)
      textGrade:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_MSG_POPUP_GRADE_DESC", "grade", tostring(currentData.selectGrade)))
      Panel_Window_MessageBox_All:SetSize(Panel_Window_MessageBox_All:GetSizeX(), orginPanelSizeY + selectGradeBg:GetSizeY())
      selectGradeBg:ComputePos()
      Panel_Window_MessageBox_All:registerPadEvent(__eConsoleUIPadEvent_LB, "messageBox_GradeButtonDown()")
      Panel_Window_MessageBox_All:registerPadEvent(__eConsoleUIPadEvent_RB, "messageBox_GradeButtonUp()")
    else
      selectGradeBg:SetShow(false)
      Panel_Window_MessageBox_All:registerPadEvent(__eConsoleUIPadEvent_LB, "")
      Panel_Window_MessageBox_All:registerPadEvent(__eConsoleUIPadEvent_RB, "")
    end
    if isConsole then
      local keyGuideTable = {
        static_consoleUI_Y,
      ...
      PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyGuideTable, static_consloeUI, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_CENTER)
    end
...
function MessageBoxCheck.showMessageBox(MessageCheckData, position, keyUse)
  ...
  local myInvenMoney = selfPlayer:get():getMoneyInventory():getMoney_s64()
  functionKeyUse = keyUse
  setKeyUseStringForBtn(functionKeyUse)
  while true do
  ...
  if nil ~= MessageCheckData.buttonStrings then
    buttonsCount = table.getn(MessageCheckData.buttonStrings)
  end
  local useCustomButtonFlag = MessageCheckData.buttonStrings and buttonsCount >= 2
  if useCustomButtonFlag then
    ...
    if buttonsCount >= 3 then
      checkCustom3:SetShow(true)
      checkCustom3:SetText(MessageCheckData.buttonStrings[3])
      Panel_Window_MessageBox_All:registerPadEvent(__eConsoleUIPadEvent_DpadUp, "PaGlobalFunc_MessageCheck_MoveAndCheck(true)")
      Panel_Window_MessageBox_All:registerPadEvent(__eConsoleUIPadEvent_DpadDown, "PaGlobalFunc_MessageCheck_MoveAndCheck(false)")
    end
    stc_MyMoney:SetShow(false)
  else
    checkCustom1:SetShow(false)
    checkCustom2:SetShow(false)
    checkCustom3:SetShow(false)
    stc_MyMoney:SetShow(true)
  end
  local padding = 10
  if false == checkCustom1:GetShow() then
    Panel_Window_MessageBox_All:SetSize(Panel_Window_MessageBox_All:GetSizeX(), orginPanelSizeY - padding)
  elseif true == checkCustom3:GetShow() then
    Panel_Window_MessageBox_All:SetSize(Panel_Window_MessageBox_All:GetSizeX(), orginPanelSizeY + checkCustom3:GetSizeY() + padding)
  else
    Panel_Window_MessageBox_All:SetSize(Panel_Window_MessageBox_All:GetSizeX(), orginPanelSizeY)
  end
  txt_MyMoney:SetText(makeDotMoney(myInvenMoney))
  ...
  if true == _ContentsGroup_UsePadSnapping then
    if useCustomButtonFlag then
      if true == checkCustom1:IsCheck() then
        ...
        checkCustom3:SetCheck(false)
      elseif true == checkCustom3:GetShow() and true == checkCustom3:IsCheck() then
        ToClient_padSnapChangeToTarget(checkCustom3)
        checkCustom1:SetCheck(false)
        checkCustom2:SetCheck(false)
        checkCustom3:SetCheck(true)
      else
...
function PaGlobal_MessageBox_RefuseOption(refuseType)
  if _refuseType.PARTY == refuseType then
    setRefuseRequests(refuseInvite:IsCheck())
  elseif _refuseType.PVP == refuseType then
    setIsPvpRefuse(refuseInvite:IsCheck())
  elseif _refuseType.EXCHANGE == refuseType then
    setIsExchangeRefuse(refuseInvite:IsCheck())
  elseif _refuseType.UISCALE == refuseType then
  ...
  elseif _refuseType.GUILDGIFT == refuseType then
    if true == refuseInvite:IsCheck() then
      PaGlobal_IngameCashshop_GuildGiveaway._isNotifyBuyer = false
    else
      PaGlobal_IngameCashshop_GuildGiveaway._isNotifyBuyer = true
    end
  elseif _refuseType.HOPEGAUGE == refuseType then
    if refuseInvite:IsCheck() then
      local _year = ToClient_GetThisYear()
      local _month = ToClient_GetThisMonth()
      local _day = ToClient_GetToday()
      ToClient_getGameUIManagerWrapper():setLuaCacheDataListTime(__eItemCollectionScrollBuffShowToday, _year, _month, _day, 0, 0, 0, CppEnums.VariableStorageType.eVariableStorageType_User)
    end
  elseif _refuseType.PETGUIDE == refuseType then
    local isCheck = refuseInvite:IsCheck()
    ToClient_getGameUIManagerWrapper():setLuaCacheDataListBool(__eUiFiveTierPetGuideNeverShowFlag, isCheck, CppEnums.VariableStorageType.eVariableStorageType_User)
  elseif _refuseType.SOLARECUSTOMGAME == refuseType then
    setSolareCustomGameRefuse(refuseInvite:IsCheck())
  else
    if _refuseType.HOPEGAUGESHOW == refuseType then
      if refuseInvite:IsCheck() then
        local _year = ToClient_GetThisYear()
        local _month = ToClient_GetThisMonth()
        local _day = ToClient_GetToday()
        ToClient_getGameUIManagerWrapper():setLuaCacheDataListTime(__eHopeGuageDontShowToday, _year, _month, _day, 0, 0, 0, CppEnums.VariableStorageType.eVariableStorageType_User)
      else
        ToClient_getGameUIManagerWrapper():setLuaCacheDataListTime(__eHopeGuageDontShowToday, 0, 0, 0, 0, 0, 0, CppEnums.VariableStorageType.eVariableStorageType_User)
      end
    else
    end
  end
...
function messageBoxCheckComputePos()
  ...
  checkCustom3:SetPosY(checkCustom2:GetPosY() + checkCustom1:GetSizeY() + padding)
  if 1 == globalButtonShowCount then
    buttonApply:SetPosX(Panel_Window_MessageBox_All:GetSizeX() / 2 - buttonApply:GetSizeX() / 2)
    buttonCancel:SetPosX(Panel_Window_MessageBox_All:GetSizeX() / 2 - buttonCancel:GetSizeX() / 2)
  elseif 2 == globalButtonShowCount then
...
function clearFreeButtonMessageData()
  if list == nil then
    return
  end
  local node = list
  while nil ~= node and nil ~= node.data do
    if false == node.data.exitButton then
      node.data = nil
...
function MessageBoxCheck.isCheck()
  return CppEnums.ItemWhereType.eMoneyInventory
end
function MessageBoxCheck.setCheck(checkIndex)
...
function messageBox_GradeButtonUp()
  local functionCancel
  if list ~= nil and list.data.isSelectGrade ~= nil and list.data.isSelectGrade == true then
    list.data.selectGrade = list.data.selectGrade + 1
    if list.data.maxGrade < list.data.selectGrade then
      list.data.selectGrade = list.data.maxGrade
      return
    end
    textGrade:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_MSG_POPUP_GRADE_DESC", "grade", tostring(list.data.selectGrade)))
    local text = ""
    text = getHouseGradeMessage(list.data.selectGrade)
    textContent:SetTextMode(__eTextMode_AutoWrap)
...
function messageBox_GradeButtonDown()
  local functionCancel
  if list ~= nil and list.data.isSelectGrade ~= nil and list.data.isSelectGrade == true then
    list.data.selectGrade = list.data.selectGrade - 1
    if list.data.selectGrade < list.data.minGrade then
      list.data.selectGrade = list.data.minGrade
      return
    end
    textGrade:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_MSG_POPUP_GRADE_DESC", "grade", tostring(list.data.selectGrade)))
    local text = ""
...
function PaGlobalFunc_MessageBox_PcPadDisableWithExitToCharacterSelect()
  ...
  if ToClient_cutsceneIsPlay() == true then
    ...
    ToClient_ForceMoveOutToCharacterSelect()
  end
end
function PaGlobalFunc_MessageBox_ShowCharacterEnterButtonForPcPadDisable()
  if false == _ContentsGroup_useUISnappingOption then
    return
  end
  if nil ~= FGlobal_TermsofGameUse_CheckAndSwitchPadUI then
    FGlobal_TermsofGameUse_CheckAndSwitchPadUI()
  end
  if nil ~= PaGlobal_LoginPassword_All_CheckAndSwitchPadUI then
    PaGlobal_LoginPassword_All_CheckAndSwitchPadUI()
  end
  if nil ~= PaGlobal_ServerSelect_All_CheckAndSwitchPadUI then
    PaGlobal_ServerSelect_All_CheckAndSwitchPadUI()
  end
  if nil ~= PaGlobalFunc_CharacterSelect_All_CheckAndSetPadUI then
    PaGlobalFunc_CharacterSelect_All_CheckAndSetPadUI()
  end
  messageBox_CloseButtonUp()
end
function Event_MessageBox_NotifyMessage_EnablePriority(message, isField)
  local curUIMode
  ...
  local titleText = PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_NOTIFY")
  local disconnectPadMsg = message
  local time = 15
  local messageboxData = {
    title = titleText,
    content = disconnectPadMsg,
    functionApply = MessageBox_Empty_function,
    priority = UI_PP.PAUIMB_PRIORITY_2,
    exitButton = false,
    enablePriority = true
  }
  if true == _ContentsGroup_useUISnappingOption and true == _ContentsGroup_UsePadSnapping and false == _ContentsGroup_RenewUI then
    messageboxData.exitButton = true
    messageboxData.isTimeCount = true
    messageboxData.countTime = time
    messageboxData.isStartTimer = true
    messageboxData.timeString = PAGetString(Defines.StringSheet_GAME, "LUA_PADDISCONNECT_DESC_1")
    if true == isField then
      messageboxData.functionApply = disConnectToGame
      messageboxData.content = PAGetString(Defines.StringSheet_GAME, "LUA_PADDISCONNECT_DESC_2")
      messageboxData.afterFunction = PaGlobalFunc_MessageBox_PcPadDisableWithExitToCharacterSelect
    elseif false == isField then
      messageboxData.afterFunction = PaGlobalFunc_MessageBox_ShowCharacterEnterButtonForPcPadDisable
    end
  end
  MessageBox.showMessageBox(messageboxData)
end
...
function MessageBox_CheckNationNeedWordOrderChange()
  local languageType = ToClient_getResourceType()
  local needChange = languageType == CppEnums.ServiceResourceType.eServiceResourceType_DE or languageType == CppEnums.ServiceResourceType.eServiceResourceType_FR or languageType == CppEnums.ServiceResourceType.eServiceResourceType_SP or languageType == CppEnums.ServiceResourceType.eServiceResourceType_TH or languageType == CppEnums.ServiceResourceType.eServiceResourceType_RU or languageType == CppEnums.ServiceResourceType.eServiceResourceType_ID or languageType == CppEnums.ServiceResourceType.eServiceResourceType_EN
  return needChange
end
function MessageBoxCheck.getSelectedButtonIndex()
  ...
  elseif checkCustom2:IsCheck() then
    return 2
  elseif checkCustom3:IsCheck() then
    return 3
  end
  return 0
end
function messageBox_UpdatePerFrame(deltaTime)
  ...
  if elapsedTime < list.data.countTime then
    if nil ~= list.data.timeString or true == list.data.isProgress then
      ...
      elseif true == MessageBox_CheckNationNeedWordOrderChange() then
        textTitle:SetText(list.data.timeString .. remainTime .. secondString)
      else
        textTitle:SetText(remainTime .. secondString .. list.data.timeString)
      end
    end
  ...
  if true == _ContentsGroup_useUISnappingOption and true == isPadDisConnect and true == _ContentsGroup_UsePadSnapping and false == _ContentsGroup_RenewUI and true == MessageBoxGetShow() and true == MessageBox_GetPadState() and GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_ESCAPE) then
...
buttonDown:addInputEvent("Mouse_LUp", "messageBox_GradeButtonDown()")
buttonUp:addInputEvent("Mouse_LUp", "messageBox_GradeButtonUp()")
...
registerEvent("EventNotifyClearFreeButtonMessageData", "clearFreeButtonMessageData")
...
registerEvent("FromClient_SolareDuelEnterTry", "FromClient_SolareReEnterAck")
registerEvent("FromClient_FailedSeasonGraduateAck", "FromClient_FailedSeasonGraduateAck")
Panel_Window_MessageBox_All:RegisterUpdateFunc("messageBox_UpdatePerFrame")
...
function PaGlobalFunc_MessageCheck_MoveAndCheck(isUp)
  if false == checkCustom1:GetShow() or false == checkCustom2:GetShow() or false == checkCustom3:GetShow() then
    return
  end
  local checkList = {
    [0] = checkCustom1,
  ...
  if true == checkCustom1:IsCheck() then
    pivotCheck = 0
  elseif true == checkCustom2:IsCheck() then
    pivotCheck = 1
  elseif true == checkCustom3:IsCheck() then
    pivotCheck = 2
  end
  if true == isUp then
    pivotCheck = pivotCheck + 1
  else
    pivotCheck = pivotCheck - 1
  end
  if pivotCheck < 0 then
    pivotCheck = 0
  elseif pivotMax < pivotCheck then
    pivotCheck = pivotMax
  end
  checkCustom1:SetCheck(0 == pivotCheck)
  checkCustom2:SetCheck(1 == pivotCheck)
  checkCustom3:SetCheck(2 == pivotCheck)
  ToClient_padSnapChangeToTarget(checkList[pivotCheck])
  _AudioPostEvent_SystemUiForXBOX(50, 0)
end
...
function PaGlobalFunc_Clicked_RefuseButton(refuseType)
  if false == _ContentsGroup_UsePadSnapping then
    return
  end
  if false == refuseInvite:GetShow() then
    return
  end
  if true == refuseInvite:IsCheck() then
    refuseInvite:SetCheck(false)
  ...
  if _refuseType.HOPEGAUGE ~= refuseType then
    PaGlobal_MessageBox_RefuseOption(refuseType)
  end
  if _refuseType.HOPEGAUGESHOW ~= refuseType then
    PaGlobal_MessageBox_RefuseOption(refuseType)
  end
end
function PaGlobalFunc_MessageBox_HopeGaugeDayCheck()
  PaGlobal_MessageBox_RefuseOption(_refuseType.HOPEGAUGE)
end
function PaGlobalFunc_MessageBox_HopeGaugeDayShowCheck()
  PaGlobal_MessageBox_RefuseOption(_refuseType.HOPEGAUGESHOW)
end
function PaGlobalFunc_MessageBox_GetSelectResult()
  return selectResult
end
function FromClient_SolareReEnterAck()
  if nil == Panel_Window_MessageBox_All then
    return
  end
  solareMessageOpen = true
  local ReEnter = function()
    ToClient_ReEntrySolare()
  end
  local messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_SOLARE_RENTER_DESC")
  local messageBoxData = {
    title = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_NEWGAMEOPTION_FUNCTION_ALERT_VIEW"),
...
function PaGlobalFunc_SolareReEnterAck()
  if nil == Panel_Window_MessageBox_All then
    return
  end
  if solareMessageOpen == false then
    return
  end
  local ReEnter = function()
    ToClient_ReEntrySolare()
  ...
  MessageBox.showMessageBox(messageBoxData)
  solareMessageOpen = false
end
function PaGlobalFunc_MessageBox_RefreshConsoleKeyGuide()
  local keyGuideTable = {
    static_consoleUI_Y,
    static_consoleUI_A,
    static_consoleUI_B
...
function FromClient_MessageBox_luaLoadComplete()
  local rv = ToClient_getJoinInstanceFailRv()
  if rv == nil or rv == 0 then
    return
  end
  messageBox_CloseButtonUp()
  messageBoxCheck_CloseButtonUp()
  ...
  if rv == 3102905437 then
    local openInventory = function()
      InventoryWindow_Show()
      Inventory_SetFunctor(PaGlobalFunc_MessageBox_BarterOrTradeItemFilter, nil, nil, nil)
    end
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_NOTIFY"),
      content = msg,
      functionYes = openInventory,
      priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
    }
    MessageBox.showMessageBox(messageBoxData)
  else
...
function PaGlobalFunc_MessageBox_BarterOrTradeItemFilter(slotNo, itemWrapper)
  local returnValue = true
  if itemWrapper:getStaticStatus() == nil then
    return returnValue
  end
  if itemWrapper:getStaticStatus():isForJustTrade() == true or itemWrapper:getStaticStatus():isBarterItem() == true then
    returnValue = false
  end
  if itemWrapper:getStaticStatus():get():getContentsEventType() == CppEnums.ContentsEventType.ContentsType_InventoryBag then
    local bagType = itemWrapper:getStaticStatus():getContentsEventParam1()
    if bagType == CppEnums.InventoryBagType.eInventoryBagType_Fish then
      local bagSize = itemWrapper:getStaticStatus():getContentsEventParam2()
      local bagFreeCount = getInventoryBagFreeCount(CppEnums.ItemWhereType.eInventory, slotNo)
      if bagSize > bagFreeCount then
        returnValue = false
      end
    end
  end
...
function FromClient_FailedSeasonGraduateAck(reason, challengeKeyRaw, titleMsg)
  local titleText = PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_NOTIFY")
  local fixtitleMsg = "[ " .. titleMsg .. " ]"
  local message = PAGetStringParam1(Defines.StringSheet_RESOURCE, "PANEL_SEASON_RENEWAL_FINISH_FAIL_DESC", "ChallengeTitle", tostring(fixtitleMsg))
  local messageboxData = {
    title = titleText,
    content = message,
    functionApply = MessageBox_Empty_function,
...
function FromClient_AlertForDebug(title, content)
...
function Event_MessageBox_NotifyCsAuthConnectFailMessage(message)
  local csCenterDesc = ""
  local titleText = PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_NOTIFY")
  local messageboxData = {
    title = titleText,
    content = message .. csCenterDesc,
    functionApply = PaGlobal_URL_OpenChCsCenterLoginFail,
    priority = UI_PP.PAUIMB_PRIORITY_LOW,
    exitButton = false
  }
  MessageBox.showMessageBox(messageboxData)
end
function Event_MessageBox_ChinaCreditPointFail(type)
  local titleText = PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_NOTIFY")
  local message = ""
  if type == __eChinaCreditPoint_Chatting then
    message = PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantPassCreditChatting")
  elseif type == __eChinaCreditPoint_Friend then
    message = PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantPassCreditFriend")
  elseif type == __eChinaCreditPoint_CreateGuild or type == __eChinaCreditPoint_EnterGuild then
    message = PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantPassCreditGuild")
  elseif type == __eChinaCreditPoint_ItemMarket then
    message = PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantPassCreditItemMarket")
  else
    _PA_ASSERT_NAME(false, "\236\131\136\235\161\156\236\154\180 ChinaCreditPointType \235\163\168\236\149\132 \236\158\145\236\151\133 \236\139\156 \237\153\149\236\157\184\237\149\180\236\163\188\236\132\184\236\154\148.", "\236\152\164\236\131\129\235\175\188")
  end
  local messageboxData = {
    title = titleText,
    content = message,
    functionYes = PaGlobal_URL_OpenCreditPointCsCenter,
    functionNo = MessageBox_Empty_function,
    priority = UI_PP.PAUIMB_PRIORITY_LOW,
    exitButton = true
  }
...
registerEvent("FromClient_AlertForDebug", "FromClient_AlertForDebug")
registerEvent("EventNotifyMessageCashAlert", "NotifyMessage_CashAlert")
registerEvent("FromClient_ApplyUISettingPanelInfo", "FromClient_MessageBox_luaLoadComplete")
registerEvent("FromClient_MagnusEasyTeleportEnterFailed", "FromClient_MessageBox_luaLoadComplete")
registerEvent("EventMessageBoxNotifyCsAuthConnectFailMessage", "Event_MessageBox_NotifyCsAuthConnectFailMessage")
registerEvent("EventMessageBoxChinaCreditPointFail", "Event_MessageBox_ChinaCreditPointFail")
