...
function PaGlobal_UseItem_All:prepareOpen()
  ...
  self._isPopupDescLoading = false
  self._popupDescLoadingItemKey = 0
...
function PaGlobal_UseItem_All:deferredPopupItemShowRequest(itemEnchatWrapper, whereType, slotNo, equipSlotNo, specialYesFunction)
  if ToClient_isTotalGameClient() == false then
    return
  end
  if itemEnchatWrapper == nil then
    return
  end
  self._popupItemWrapper = itemEnchatWrapper
  self._popupItem.selectWhereType = whereType
  self._popupItem.selectSlotNo = slotNo
  self._popupItem.selectEquipNo = equipSlotNo
  self._popupItem.specialYesFunction = specialYesFunction
  if "" == itemEnchatWrapper:getPopupItemDesc() then
    self._isPopupDescLoading = true
    self._popupDescLoadingItemKey = itemEnchatWrapper:getItemKey()
    return
  end
  self:prepareOpen()
  self:resetUIcontrol()
  self:fillPopupUseItemData(itemEnchatWrapper)
end
function PaGlobal_UseItem_All:deferredPopupItemOpen(itemKey)
  if ToClient_isTotalGameClient() == false then
    return
  end
  if false == self._isPopupDescLoading then
    return
  end
  if self._popupDescLoadingItemKey ~= itemKey then
    return
  end
  ...
  self:prepareOpen()
  self:resetUIcontrol()
  self:fillPopupUseItemData(itemStaticStatus)
  self._isPopupDescLoading = false
end
function PaGlobal_UseItem_All:inventorySlotChanged(itemKey)
  if ToClient_isTotalGameClient() == false then
    return
  end
  self._popupDescLoadingItemKey = itemKey
end
