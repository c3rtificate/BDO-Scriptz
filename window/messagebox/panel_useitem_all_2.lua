...
function PaGlobal_UseItem_All_PopupItem(itemEnchatWrapper, whereType, slotNo, equipSlotNo, specialYesFunction)
  if Panel_UseItem_All == nil then
    return
  end
  if ToClient_isTotalGameClient() == true then
    PaGlobal_UseItem_All:deferredPopupItemShowRequest(itemEnchatWrapper, whereType, slotNo, equipSlotNo, specialYesFunction)
    return
  end
  ...
  PaGlobal_UseItem_All._popupItem.specialYesFunction = specialYesFunction
  PaGlobal_UseItem_All:fillPopupUseItemData(itemEnchatWrapper)
end
function PaGlobal_UseItem_All_NotifyInventorySlotChanged(itemKey)
  if Panel_UseItem_All == nil then
    return
  end
  ...
  PaGlobal_UseItem_All:inventorySlotChanged(itemKey)
end
...
function HandleEventLUp_UseItem_All_UseItem()
  ...
  PaGlobal_UseItem_All_Close()
  if PaGlobal_UseItem_All._popupItem.specialYesFunction ~= nil then
    PaGlobal_UseItem_All._popupItem.specialYesFunction()
    return
  end
