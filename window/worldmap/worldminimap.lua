...
local swapRadar = function(radarType)
  ...
  self._panelSizeScale = not ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(__eRadar3DExpand)
...
function PaGlobal_WorldMiniMap:changePanelSize()
  ...
  ToClient_getGameUIManagerWrapper():setLuaCacheDataListBool(__eRadar3DExpand, self._panelSizeScale, CppEnums.VariableStorageType.eVariableStorageType_User)
  if false == _ContentsGroup_RemasterUI_Main_RightTop then
    FGlobal_PersonalIcon_ButtonPosUpdate()
  else
    FromClient_Widget_FunctionButton_Resize()
...
function PaGlobal_WorldMiniMap:alphaBtnOverEvent(isOver)
  ...
  if isOver then
    TooltipSimple_Show(self._ui.btn_changeAlphaPlus, PAGetString(Defines.StringSheet_GAME, "LUA_WORLDMINIMAP_ALPHABUTTON_TOOLTIP"), nil, false)
  else
    TooltipSimple_Hide()
  end
end
...
function worldMiniMap_updateRegion(regionData)
  ...
  self._ui.regionSubName:SetFontColor(Defines.Color.C_FFEFEFEF)
  self._ui.regionSubName:useGlowFont(false)
  ...
  if isSafetyZone then
    ...
    self._ui.regionName:useGlowFont(true, "BaseFont_12", 4281961144)
  ...
  elseif isArenaZone then
    if true == _ContentsGroup_ChinaFontColor then
      self._ui.regionName:SetFontColor(Defines.Color.C_FFFF9C00)
      self._ui.regionName:useGlowFont(true, "BaseFont_12", 4286580487)
      self._ui.regionNodeName:SetFontColor(Defines.Color.C_FFFF9C00)
      self._ui.regionNodeName:useGlowFont(true, "BaseFont_8_Glow", 4286580487)
      self._ui.regionWarName:SetFontColor(Defines.Color.C_FFFF9C00)
  ...
  local regionNameSizeX = Panel_WorldMiniMap:GetSizeX() - self._ui.btn_changeRadar:GetSizeX() * 2 - self._ui.btn_changeAlphaPlus:GetSizeX()
  self._ui.regionName:SetSize(regionNameSizeX, self._ui.regionName:GetSizeY())
  ...
  if nil ~= linkSiegeWrapper then
    ...
    self._ui.regionName:SetTextMode(__eTextMode_AutoWrap)
    self._ui.regionName:SetText(regionData:getAreaName())
  end
...
function worldMiniMap_NodeLevelRegionNameShow(wayPointKey)
  ...
  if "" == self._nodeWarRegionName or nil == self._nodeWarRegionName then
    ...
    self._ui.regionWarName:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_RADER_VILLAGEWARABLE_NAME", "regionName", self._nodeWarRegionName))
...
function PaGlobal_WorldMiniMap:setPositionRegionNameList()
  ...
  local regionNameSizeX = Panel_WorldMiniMap:GetSizeX() - self._ui.btn_changeRadar:GetSizeX() * 2 - self._ui.btn_changeAlphaPlus:GetSizeX()
  self._ui.regionName:SetSize(regionNameSizeX, self._ui.regionName:GetSizeY())
  self._ui.regionName:SetText(self._ui.regionName:GetText())
  self._ui.regionName:ComputePos()
  self._ui.regionSubName:ComputePos()
  self._ui.regionNodeName:SetPosX(Panel_WorldMiniMap:GetSizeX() - self._ui.regionNodeName:GetSizeX())
  self._ui.regionNodeName:SetPosY(Panel_WorldMiniMap:GetSizeY() - self._ui.regionNodeName:GetSizeY())
  self._ui.regionNodeName:ComputePos()
  self._ui.regionWarName:SetPosX(Panel_WorldMiniMap:GetSizeX() - self._ui.regionWarName:GetSizeX())
...
function PaGlobal_WorldMiniMap:InitWorldMiniMap()
  ...
  self._ui.regionSubName = UI.getChildControl(Panel_WorldMiniMap, "StaticText_RegionSubName")
  self._ui.regionSubName:SetShow(true)
  self._ui.regionSubName:SetAutoResize(true)
  self._ui.regionSubName:SetNotAbleMasking(true)
  ...
  if ToClient_CheckFamilyCharacterLevel(10) == true then
    swapRadar(GLOBAL_CHECK_WORLDMINIMAP)
  ...
  FromClient_reSizeWorldMiniMap()
  PaGlobal_WorldMiniMap:hidePanelConditionCheck()
end
function PaGlobal_WorldMiniMap:hidePanelConditionCheck()
  if nil == Panel_WorldMiniMap then
    _PA_ASSERT_NAME(false, "Panel_WorldMiniMap\236\157\180 nil \236\158\133\235\139\136\235\139\164.", "\236\160\149\236\167\128\237\152\156")
    return
  end
  ...
  if true == bHide then
    Panel_WorldMiniMap:SetRenderAndEventHide(true)
    Panel_TimeBar:SetRenderAndEventHide(true)
  else
    Panel_WorldMiniMap:SetRenderAndEventHide(false)
...
function PaGlobalFunc_WorldMinimap_SetRegionSubNameText(text)
  local self = PaGlobal_WorldMiniMap
  self._ui.regionSubName:SetText(text)
  self._ui.regionSubName:ComputePos()
  self._ui.regionSubName:SetShow(true)
end
function PaGlobalFunc_WorldMinimap_ClearRegionSubNameText()
  local self = PaGlobal_WorldMiniMap
  self._ui.regionSubName:SetText("")
  self._ui.regionSubName:SetShow(false)
end
