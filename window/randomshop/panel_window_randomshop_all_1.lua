...
function PaGlobal_RandowShop_All:controlAll_Init()
  ...
  local stcMyMoneyArea = UI.getChildControl(self._ui.stc_selectMoneyBg, "Static_MyMoneyArea")
  self._ui.stc_MoneyArea = UI.getChildControl(stcMyMoneyArea, "StaticText_MyMoney")
  self._ui.txt_MoneyValue = UI.getChildControl(stcMyMoneyArea, "StaticText_MoneyValue")
end
  ...
  for ii = 0, sellCount - 1 do
    ...
    if slotNo == shopItem.shopSlotNo then
      ...
      if nil ~= itemRandomSS then
        ...
        PAGlobalFunc_SetItemTextColor(self._ui.txt_itemName, itemRandomSS)
      end
    end
  end
  local useWp = 0
...
function PaGlobal_RandowShop_All:moneyUpdate()
  ...
  self._ui.txt_MoneyValue:SetText(makeDotMoney(silverInInventory))
end
