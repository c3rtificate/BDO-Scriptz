...
function FromClient_RandomShop_All_WpChanged()
  ...
  PaGlobal_RandowShop_All._ui.txt_myWp:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_RANDOMSHOP_ALL_MYWP", "myWp", tostring(myWp)))
end
function FromClient_RandomShop_All_RegionUpdate()
  if nil == Panel_Window_RandomShop_All then
    return
  end
  if false == PaGlobal_RandowShop_All._initialize then
  ...
  PaGlobal_RandowShop_All:randomMoneyCheckUpdate()
end
...
function PaGlobalFunc_RandomShop_All_BuyItem()
  ...
  if getSelfPlayer():get():getMoneyInventory():getMoney_s64() < PaGlobal_RandowShop_All._randomShopItemPrice then
