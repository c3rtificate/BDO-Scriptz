function HandleEventLUp_MansionContract_ClickContract()
  ...
  local itemWhereType = CppEnums.ItemWhereType.eMoneyInventory
  local moneyValue = getSelfPlayer():get():getMoneyInventory():getMoney_s64()
  ...
  local messagebox = {
    ...
    content = PaGlobal_MansionContract._confirmMsg,
    functionYes = doYes,
...
function FromClient_ContractMansion(householdNo, characterKey)
  if nil == Panel_Window_Mansion_Contract_All then
    return
  end
  ...
  PaGlobal_MansionContract:prepareOpen(householdNo, characterKey)
end
