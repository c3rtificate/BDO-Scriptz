...
function PaGlobal_DeleteMaxEnchanterName:updateList()
  ...
  local slotIndex = 0
  for slotNo = useStartSlot, invenSize - 1 do
    local NameStr = getItemNaming(getTItemNoBySlotNo(slotNo), __eItemWhereTypeEquip)
    if nil ~= NameStr then
...
function PaGlobal_DeleteMaxEnchanterName:selectItem(index)
  ...
  local playerName = getItemNaming(getTItemNoBySlotNo(self._toInventorySlotNo), __eItemWhereTypeEquip)
