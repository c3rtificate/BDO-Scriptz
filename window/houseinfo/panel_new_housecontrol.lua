...
local template = {
  ...
  _buttonText = UI.getChildControl(Panel_HouseControl, "ButtonText_Smithy"),
...
function Set_HouseUseType_Texture_BG(_control)
  ...
  local path = CppEnums.HouseIconType_BGTexture.path
  local iconTypeBG = CppEnums.HouseIconType_BGTexture[HouseControlManager._currentIconType]
  if iconTypeBG ~= nil then
    HouseProgressSection_SetBaseTextureUV(_Target, path, iconTypeBG.x1, iconTypeBG.y1, iconTypeBG.x2, iconTypeBG.y2)
  else
    local emptyHouse = CppEnums.HouseIconType_BGTexture[CppEnums.eHouseIconType.Count]
    HouseProgressSection_SetBaseTextureUV(_Target, path, emptyHouse.x1, emptyHouse.y1, emptyHouse.x2, emptyHouse.y2)
  end
end
function HouseInfoSection:init()
  self._Name:SetText(HouseControlManager._feature1)
  self._Desc:SetAutoResize(true)
  self._Desc:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._UseType_Desc:SetTextMode(__eTextMode_AutoWrap)
...
function HouseControlManager:initialize()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(self._buttonQuestion, "\"PanelHouseControl\"")
  ...
  for index = 1, contentsViewcount do
    do
      ...
      local button = UI.createControl(__ePAUIControl_RadioButton, self._controlBG, "Static_Button_" .. index)
      ...
      local buttonText_PosX = template._buttonText:GetSpanSize().x - template._button:GetSpanSize().x
      local buttonText_PosY = template._buttonText:GetSpanSize().y - template._button:GetSpanSize().y
      local buttonText = UI.createControl(__ePAUIControl_StaticText, button, "StaticText_Button_" .. index)
      CopyBaseProperty(template._buttonText, buttonText)
      buttonText:SetPosX(buttonText_PosX)
      buttonText:SetPosY(buttonText_PosY)
      buttonText:SetShow(true)
      buttonText:addInputEvent("Mouse_LUp", "handleClickedHouseControlSetUseType(" .. index .. ")")
      buttonText:addInputEvent("Mouse_On", "HandleOnOut_HouseControlManager_ShowFullText(" .. index .. ", true)")
      buttonText:addInputEvent("Mouse_Out", "HandleOnOut_HouseControlManager_ShowFullText(" .. index .. ", false)")
      uigroup._buttonText = buttonText
      ...
      local onUpgrade = UI.createControl(__ePAUIControl_Static, button, "Static_onUpgrade_" .. index)
      ...
      local UseTypeIcon = UI.createControl(__ePAUIControl_Static, button, "Static_UseTypeIcon_" .. index)
      ...
      for level = 1, 5 do
        local grade = UI.createControl(__ePAUIControl_Button, button, "Static_Grade_" .. index .. "_" .. level)
        local grade_Mask = UI.createControl(__ePAUIControl_Static, button, "Static_Grade_Mask_" .. index .. "_" .. level)
      ...
      function uigroup:update(houseInfoStaticStatusWrapper)
        ...
        self._maxLevel = houseInfoCraftWrapper:getLevel()
        ...
        local iconType = houseInfoCraftWrapper:getIconType()
        HouseControlManager:UseTypeIconTexture(self._index, iconType)
      end
      ...
      function uigroup:uicontrol(maxLevel, name)
        ...
        self.ui._button:SetText("")
        local sizeX = uigroup.ui._grade[1]:GetPosX() - uigroup.ui._button:GetPosX() - 10
        uigroup._buttonText:SetSize(sizeX, uigroup._buttonText:GetSizeY())
        uigroup._buttonText:SetTextMode(__eTextMode_LimitText)
        uigroup._buttonText:SetText(name)
      end
...
function HouseControlManager:UseTypeIconTexture(index, iconType)
  local control = HouseControlManager_buttonList[index]._UseTypeIcon
  if control == nil then
    return
  end
  local path = CppEnums.HouseIconType_UseTypeIconTexture.path
  local x1, y1, x2, y2 = 0, 0, 0, 0
  local textureInfo = CppEnums.HouseIconType_UseTypeIconTexture[iconType]
  if textureInfo ~= nil then
    x1, y1, x2, y2 = textureInfo.x1, textureInfo.y1, textureInfo.x2, textureInfo.y2
  end
  HouseProgressSection_SetBaseTextureUV(control, path, x1, y1, x2, y2)
end
function HouseControlManager:WorkinUseTypeIconTexture(index, iconType)
  local control = HouseControlManager_buttonList[index]._onUpgrade
  if control == nil then
    return
  end
  local fileName = CppEnums.HouseIconType_WorkingIconFileName[iconType].fileName
  if fileName == nil then
    fileName = CppEnums.HouseIconType_WorkingIconFileName[CppEnums.eHouseIconType.House].fileName
  end
  control:ChangeTextureInfoName("/New_UI_Common_forLua/Window/HouseInfo/useType/" .. fileName)
...
local handleClickedHouseControlBuyHouseContinue = function()
  ToClient_BuyHouse(HouseControlManager._houseKey, HouseControlManager._clickedUseType, HouseControlManager._selectGradeLevel)
...
function handleClickedHouseControlBuyHouse()
  ...
  local maxlevel = houseInfoStaticStatusWrapper:getLevelByKey(HouseControlManager._clickedUseType)
  local itemExplain = ""
  ...
  local messageboxData = {
    ...
    isSelectGrade = true,
    selectGrade = nextRentHouseLevel,
    minGrade = nextRentHouseLevel,
    maxGrade = maxlevel,
    priority = UI_PP.PAUIMB_PRIORITY_LOW
  }
  MessageBox.showMessageBox(messageboxData, "top")
  HouseControlManager._selectGradeLevel = nextRentHouseLevel
end
function getHouseGradeMessage(selectLevel)
  local houseInfoStaticStatusWrapper = ToClient_GetHouseInfoStaticStatusWrapper(HouseControlManager._houseKey)
  if false == houseInfoStaticStatusWrapper:isSet() then
    return
  end
  ...
  if ToClient_IsMyHouse(HouseControlManager._houseKey) == false then
    nextRentHouseLevel = 1
  else
    ...
    elseif HouseControlManager._clickedUseType ~= useType and eHouseUseGroupType.Count ~= HouseControlManager._currentGroupType then
      nextRentHouseLevel = 1
    end
    isMyHouse = true
  end
  local realIndex = houseInfoStaticStatusWrapper:getIndexByReceipeKey(HouseControlManager._clickedUseType)
  local houseInfoCraftWrapper = houseInfoStaticStatusWrapper:getHouseCraftWrapperByIndex(realIndex)
  local listCount = houseInfoStaticStatusWrapper:getNeedAllItemListCount(HouseControlManager._clickedUseType, nextRentHouseLevel, selectLevel)
  local needTime_sec = houseInfoStaticStatusWrapper:getTransperTime(HouseControlManager._clickedUseType, nextRentHouseLevel, selectLevel)
  local needTime = Util.Time.timeFormatting(needTime_sec)
  local houseName = HouseControlManager._houseName
  local useTypeName = houseInfoCraftWrapper:getReciepeName()
  local maxlevel = houseInfoStaticStatusWrapper:getLevelByKey(HouseControlManager._clickedUseType)
  local itemExplain = ""
  if isMyHouse == false then
    itemExplain = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_HOUSECONTROL_BUYHOUSE_CONTENT", "houseName", houseName, "useTypeName", useTypeName) .. [[


]] .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_HOUSECONTROL_NEEDPOINT", "needPoint", HouseControlManager._needExplorePoint) .. "\n"
  else
    local workingcnt = ToClient_getHouseWorkingWorkerList(houseInfoStaticStatusWrapper:get())
    realIndex = houseInfoStaticStatusWrapper:getIndexByReceipeKey(HouseControlManager._clickedUseType)
    ...
    elseif targetUseTypeName == currentUseTypeName then
      itemExplain = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_HOUSECONTROL_CHANGESTATE_CONTENT_4", "currentTypeName", currentUseTypeName, "rentHouseLevel", rentHouseLevel, "nextLevel", selectLevel) .. [[
  ...
  for index = 0, listCount - 1 do
    local itemKey = houseInfoStaticStatusWrapper:getNeedAllItemListItemKey(HouseControlManager._clickedUseType, nextRentHouseLevel, selectLevel, index)
    local itemName = getItemEnchantStaticStatus(itemKey):getName()
    local itemCount = houseInfoStaticStatusWrapper:getNeedAllItemListItemCount(HouseControlManager._clickedUseType, nextRentHouseLevel, selectLevel, index)
    HouseControlManager._itemCheck[index] = false
    for invenIndex = 0, inventory_getSize() - 1 do
      if nil ~= getInventoryItem(invenIndex) and itemKey:getItemKey() == getInventoryItem(invenIndex):get():getKey():getItemKey() and itemCount <= getInventoryItem(invenIndex):get():getCount_s64() then
        HouseControlManager._itemCheck[index] = true
  ...
  itemExplain = itemExplain .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_HOUSECONTROL_NEEDTIME", "needTime", needTime)
  HouseControlManager._selectGradeLevel = selectLevel
  return itemExplain
end
...
function HouseControlManager:UpdateMyHouse()
  ...
  self._currentIconType = rentHouseWrapper:getIconType()
  ...
  if nil ~= HouseControlManager_buttonList[index + 1] then
    ...
    elseif workingcnt > 0 then
      ...
      HouseControlManager:WorkinUseTypeIconTexture(index + 1, self._currentIconType)
...
function FGlobal_UpdateHouseControl(houseInfoSSWrapper)
  ...
  if nil ~= rentHouse and true == rentHouse:isSet() then
    ...
    HouseControlManager._currentIconType = rentHouse:getIconType()
    ...
    HouseControlManager._currentIconType = CppEnums.eHouseIconType.Count
...
function HouseWorkListSection:initialize()
  ...
  for index = 0, self.contentCount - 1 do
    self.bgList[index] = UI.createControl(__ePAUIControl_Static, self.Frame, "bgList_" .. tostring(index))
    CopyBaseProperty(self.BG, self.bgList[index])
    self.bgList[index]:SetPosY(bgPosY)
    self.bgList[index]:SetPosX(bgPosX)
    self.borderList[index] = UI.createControl(__ePAUIControl_Static, self.Frame, "borderList_" .. tostring(index))
    ...
    self.iconList[index] = UI.createControl(__ePAUIControl_Static, self.Frame, "iconList_" .. tostring(index))
    CopyBaseProperty(self.icon, self.iconList[index])
    self.iconList[index]:SetPosX(iconPosX)
    self.iconList[index]:SetPosY(iconPosY)
    self.overList[index] = UI.createControl(__ePAUIControl_Static, self.Frame, "overList_" .. tostring(index))
    ...
    if 1 == coloumCount then
      self.levelList[rowCount] = UI.createControl(__ePAUIControl_StaticText, self.Frame, "levelList_" .. tostring(rowCount))
  ...
  for index = 0, self.viewCount_nonCraft - 1 do
    self.levelList_nonCraft[index] = UI.createControl(__ePAUIControl_StaticText, self.Frame, "levelList_nonCraft_" .. tostring(index))
    CopyBaseProperty(self.level_nonCraft, self.levelList_nonCraft[index])
    self.levelList_nonCraft[index]:SetPosY(nonCraft_PosY)
    self.guideList[index] = UI.createControl(__ePAUIControl_StaticText, self.Frame, "guideList_" .. tostring(index))
  ...
  self.guideList.MyHouse = UI.createControl(__ePAUIControl_StaticText, self.Frame, "guideList_MyHouse")
  CopyBaseProperty(self.guide_MyHouse, self.guideList.MyHouse)
  self.guideList.MyHouse:SetAutoResize(true)
  self.guideList.MyHouse:SetTextMode(__eTextMode_AutoWrap)
...
function HouseManageSection:update_ChangeCost()
  ...
  local inventory = selfPlayer:getMoneyInventory()
...
function HouseProgressSection:Init()
  ...
  self._WorkName:SetTextMode(__eTextMode_LimitText)
      ...
      if currentKey == ToClient_getWorkableExchangeKeyByIndex(index - 1) then
        ...
        if esSSW:isSet() then
          ...
          for idx = 1, workingCout do
            local workerNo = ToClient_getHouseWorkingWorkerByIndex(houseInfoSS, idx - 1).workerNo:get()
...
function Set_HouseUseType_Texture_Icon(_Target)
  ...
  local textureTarget = CppEnums.HouseIconType_SelectTexture[HouseControlManager._currentIconType]
  if textureTarget ~= nil then
    HouseProgressSection_SetBaseTextureUV(_Target, path, textureTarget.x1, textureTarget.y1, textureTarget.x2, textureTarget.y2)
  else
...
function HouseProgressSection_Immediately_CraftItem()
  ...
  if esSSW:isSet() then
    ...
    local messageBoxMemo = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_NEW_HOUSECONTROL_USESILVER", "workName", workName, "needSilver", makeDotMoney(HouseProgressSection._needPearl))
    local messageboxData = {
...
function HouseProgressSection_Immediately_CraftItemYes()
  ...
  local workerNo = HouseProgressSection._worker:get()
...
function HandleOnOut_HouseControlManager_ShowFullText(index, isOn)
  local button = HouseControlManager_buttonList[index]
  button._isOn = isOn
  if true == isOn then
    if false == button._buttonText:IsLimitText() then
      return
    end
    button._buttonText:SetSize(button.ui._button:GetSizeX(), button.ui._button:GetSizeY())
    button._buttonText:SetText(button._buttonText:GetText())
    for idx = 1, #button.ui._grade do
      button.ui._grade[idx]:SetShow(false)
    end
  else
    local sizeX = button.ui._grade[1]:GetPosX() - button.ui._button:GetPosX() - 10
    local curSizeX = button._buttonText:GetSizeX()
    if sizeX == curSizeX then
      return
    end
    button._buttonText:SetSize(sizeX, button.ui._button:GetSizeY())
    button._buttonText:SetText(button._buttonText:GetText())
    local maxLevel = button._maxLevel
    for key, value in pairs(button.ui._grade) do
      local isShow = key <= maxLevel
      value:SetShow(isShow)
    end
  end
