...
function PaGlobal_WorldMap_StableList:prepareOpen()
  ...
  if false == self:SetOpenFirstData() then
    ...
    if false == self._isConsole then
      self:prepareClose()
    elseif ToClient_WorldMapIsShow() == true then
...
function PaGlobal_WorldMap_StableList:updateSlot()
  ...
  for servantIdx = startIndex, endIndex do
    ...
    if nil ~= servantInfo then
      ...
      if false == servantInfo:isGuildServant() and true == servantInfo:doHaveVehicleSkill() and (servantType == CppEnums.VehicleType.Type_Horse or servantType == CppEnums.VehicleType.Type_Camel or servantType == CppEnums.VehicleType.Type_Donkey or servantType == CppEnums.VehicleType.Type_Elephant or servantType == CppEnums.VehicleType.Type_RidableBabyElephant) then
        slot._txt_skillChangeTitle:SetShow(true)
        slot._txt_skillChangeValue:SetShow(true)
        if 0 < servantInfo:getServantSkillChangeConfirmedCount() then
        ...
        slot._txt_skillChangeTitle:SetShow(false)
        slot._txt_skillChangeValue:SetShow(false)
      end
      local isNineTier = servantInfo:getTier() < 9
      local servantType = servantInfo:getVehicleType()
      ...
      if __eVehicleType_Horse == servantType then
        ...
        if acceAddItemUseCount > 0 then
          ...
          slot._stc_OnlyAcce:addInputEvent("Mouse_On", "HandleEventOnOut_WorldMap_StableList_ShowUseAddStatItemTooltip(true," .. slotIndex .. "," .. __eServantStatTypeAcceleration .. ")")
          slot._stc_OnlyAcce:addInputEvent("Mouse_Out", "HandleEventOnOut_WorldMap_StableList_ShowUseAddStatItemTooltip(false)")
        end
        ...
        if cornerAddItemUseCount > 0 then
          ...
          slot._stc_OnlyCorner:addInputEvent("Mouse_On", "HandleEventOnOut_WorldMap_StableList_ShowUseAddStatItemTooltip(true," .. slotIndex .. "," .. __eServantStatTypeCornering .. ")")
          slot._stc_OnlyCorner:addInputEvent("Mouse_Out", "HandleEventOnOut_WorldMap_StableList_ShowUseAddStatItemTooltip(false)")
        end
        ...
        if brakeAddItemUseCount > 0 then
          ...
          slot._stc_OnlyBrake:addInputEvent("Mouse_On", "HandleEventOnOut_WorldMap_StableList_ShowUseAddStatItemTooltip(true," .. slotIndex .. "," .. __eServantStatTypeBrake .. ")")
          slot._stc_OnlyBrake:addInputEvent("Mouse_Out", "HandleEventOnOut_WorldMap_StableList_ShowUseAddStatItemTooltip(false)")
        end
      end
  ...
  for territoryKey, territoryInfo in pairs(self._territoryInfoList) do
    ...
    if territoryKey == self._currentSelectTerritoryKey then
      for ii = 1, #self._territoryInfoList[territoryKey]._regionKeyList do
        local key = self._territoryInfoList[territoryKey]._regionKeyList[ii] + self._SEPARATORNUMBER
        if key < self._SEPARATORNUMBER then
          self._ui._list2_region:getElementManager():pushKey(toInt64(0, key))
        elseif ToClient_IsElfWarServantNpcRegion(self._territoryInfoList[territoryKey]._regionKeyList[ii]) == false then
          self._ui._list2_region:getElementManager():pushKey(toInt64(0, key))
        end
      end
    end
