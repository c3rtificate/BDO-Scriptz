...
function defalut_Control:Init_Control()
  ...
  self._work_Info._Work_Volume_Text:SetTextMode(__eTextMode_AutoWrap)
...
function HandleOn_Harvest_Worker_List(index)
  ...
  if nil ~= workerWrapper then
    ...
    PaGlobalFunc_WorkerManagerTooltip_All_OpenByWorkerNoRaw(Worker_List._data_Table[workerIndex]._workerNo, uiBase, true)
  end
end
...
function FGlobal_Harvest_WorkManager_Open(index)
  ...
  if true == _ContentsGroup_HarvestList_All then
    harvestPanel = Panel_HarvestList_All
  else
    harvestPanel = Panel_HarvestList
  end
  if harvestPanel:GetShow() then
...
function HandleClick_Harvest_doWork()
  ...
  local isAbleAutoRecovery = getAutoRecoveryWorker() and 0 < ToClient_getNpcRecoveryItemSizeByFamilyInventory()
  if Worker_List._data_Table[Worker_List._selected_Index]._currentPoint < 2 and isAbleAutoRecovery == false then
