...
function FGlobal_WorldmapHouseHold_GetControlNameList()
  return CppEnums.HouseIconType_ControlNameList
end
function FGlobal_HouseHoldButtonSetBaseTexture(houseBtn)
  ...
  local stringName = CppEnums.HouseIconType_ControlNameList[-1]
  ...
  if true == ToClient_IsMyHouse(houseKey) then
    if ToClient_GetProgressRateChangeHouseUseType(houseKey) < 100 then
      stringName = CppEnums.HouseIconType_ControlNameList._changeUseType
      isShowConstructionAni = true
    elseif workingcnt >= 1 then
      stringName = CppEnums.HouseIconType_ControlNameList._crafting
      isShowConstructionAni = true
    else
      if nil == rentHouseWrapper then
        return
      end
      local iconType = rentHouseWrapper:getIconType()
      if ToClient_IsMyLiveHouse(houseKey) == false and iconType == 0 then
        iconType = -1
      end
      stringName = CppEnums.HouseIconType_ControlNameList[iconType]
      if nil == stringName then
        stringName = CppEnums.HouseIconType_ControlNameList[0]
      end
    end
  elseif false == houseInfo:isPurchasable(CppEnums.eHouseUseType.Depot) then
    stringName = CppEnums.HouseIconType_ControlNameList[-1]
  else
    stringName = CppEnums.HouseIconType_ControlNameList._noPurchasable
  end
  if true == houseInfo:isMansion() then
    stringName = CppEnums.HouseIconType_ControlNameList._BigHouse
    if houseInfo ~= nil then
      local houseKey = houseInfo:getHouseKey()
      if houseKey == 3841 then
        stringName = CppEnums.HouseIconType_ControlNameList._BigHouse1
      elseif houseKey == 3843 then
        stringName = CppEnums.HouseIconType_ControlNameList._BigHouse2
      elseif houseKey == 3847 then
        stringName = CppEnums.HouseIconType_ControlNameList._BigHouse3
      end
    end
  end
  ...
  local uniqueTextureID = houseInfo:getUnqiueWorldMapTextureType()
  if uniqueTextureID == 1 and isShowConstructionAni == false then
    if ToClient_IsMyHouse(houseKey) then
      stringName = "Static_DokevJar_Active"
    else
      stringName = "Static_DokevJar_Inactive"
    end
  end
...
function FromClient_LClickedWorldMapHouse(houseBtn)
  ...
  local isMansion = houseInfo:isMansion()
  if true == isMansion then
    return
  end
