...
function FromClient_WorldMap_AbyssOnePortalIcon_MouseOn(abyssOnePortalIcon)
  if abyssOnePortalIcon == nil then
    return
  end
  local name = PAGetString(Defines.StringSheet_GAME, "LUA_TOOLTIP_WORLDMAP_ABYSSONE")
  local desc
  TooltipSimple_Show(abyssOnePortalIcon, name, desc)
end
function FromClient_WorldMap_AbyssOnePortalIcon_MouseOut(abyssOnePortalIcon)
  if abyssOnePortalIcon == nil then
    return
  end
  TooltipSimple_Hide()
end
function FromClient_WorldMap_AbyssOnePortalIcon_RClick(pos3D)
  if pos3D == nil then
    return
  end
  FromClient_RClickWorldmapPanel(pos3D, true, false)
end
function FromClient_WorldMap_MorningLandBossIcon_RClick(pos3D)
  if pos3D == nil then
    return
  end
  FromClient_RClickWorldmapPanel(pos3D, true, false)
end
local notepadPresetSample = 1
function FromClient_MClickWorldmapPanel(pos3D)
  local sampleTitle = "Title_" .. notepadPresetSample .. ""
  local sampleDesc = "Desc_" .. notepadPresetSample .. ""
  ToClient_setWorldMapPinData(notepadPresetSample, pos3D, sampleTitle, sampleDesc)
end
function FromClient_RClickWorldmapPanel(pos3D, immediately, isTopPicking)
  ...
  local selfPlayer = getSelfPlayer()
  local regionInfoWrapper = selfPlayer:getRegionInfoWrapper()
  isCullingNaviBtn = true
  local isResistDesert = selfPlayer:isResistDesert()
  local isToSea = ToClient_WorldMapNaviPickingIsSea(pos3D)
  local isToOcean = ToClient_WorldMapNaviPickingIsOcean(pos3D)
  local isToDesert = ToClient_WorldMapNaviPickingIsDesert(pos3D)
  local isToGround = (isToSea or isToOcean) == false
  local isFromSea = regionInfoWrapper:isSea()
  local isFromOcean = regionInfoWrapper:isOcean()
  local isFromDesert = regionInfoWrapper:isDesert()
  local isPlayerInSea = ToClient_IsPlayerInSea()
  local isFromGround = (isFromSea or isFromOcean) == false and isPlayerInSea == false
  local isLoopAvailable = true
  if isFromOcean or isToOcean or isPlayerInSea then
    if isResistDesert == false or isFromGround or isToGround then
  ...
  if ToClient_WorldMapNaviIsLoopPath() == false and isToDesert == false and isFromDesert == false and isLoopAvailable == true and ToClient_WorldMapNaviEmpty() == false and HideAutoCompletedNaviBtn == false then
...
function FGlobal_OpenOtherPanelWithNodeMenu(node, isShow)
  ...
  local nodeKey = plantKey:getWaypointKey()
  local wayPlant = ToClient_getPlant(plantKey)
  local isPurchased = node:isPurchased()
  if isPurchased == true and wayPlant ~= nil and wayPlant:getType() == CppEnums.PlantType.ePlantType_Zone then
    ...
    if 0 == workingcnt then
      ...
      local isFinanceNode = explorationSSW:get():isFinance()
      local isMonopolyNode = explorationSSW:get():isMonopoly()
      local isNormalNode = isFinanceNode == false and isMonopolyNode == false
      if explorationSSW:get():isFinance() then
        ...
        FGlobal_Plant_WorkManager_Open(node, isNormalNode)
      end
    elseif 1 == workingcnt then
      FGlobal_ShowWorkingProgress(node, 1, true)
    end
  end
end
...
function FGlobal_PopCloseWorldMapActual()
  PaGlobalFunc_FullScreenFade_FadeOut()
  ToClient_worldmapManagerClearFocusUiButtonList()
  ...
  if PaGlobalFunc_WorldMapReightBottomUIListOnOff ~= nil then
    PaGlobalFunc_WorldMapReightBottomUIListOnOff(false)
  end
  if nil ~= PaGlobalFunc_WorldMap_Grand_NodeLevel_GetShow and true == PaGlobalFunc_WorldMap_Grand_NodeLevel_GetShow() then
    PaGlobalFunc_WorldMap_Grand_NodeLevel_Close()
  end
  if nil ~= Panel_Window_NumberPad_All and true == Panel_Window_NumberPad_All:GetShow() then
    PanelCloseFunc_ExchangeNumber()
  end
end
function FromClient_WorldMapOpen()
  ...
  if Defines.UIMode.eUIMode_Sequence ~= GetUIMode() and Defines.UIMode.eUIMode_SequencePlayerControl ~= GetUIMode() then
  ...
  if _ContentsGroup_RenewlDropUI == true and PaGlobalFunc_RenewDropItem_GetShow() == true then
    PaGlobalFunc_RenewDropItem_Close()
  end
  isCullingNaviBtn = true
...
function FGlobal_WorldMapCloseBeforeCheck()
  ...
  if false == _ContentsGroup_RenewUI and Panel_Window_Quest_All:GetShow() and Panel_Window_Quest_All:IsUISubApp() == false then
    PaGlobalFunc_Quest_All_Close()
    return false
  end
  ...
  if nil ~= Panel_Dialog_ServantMarket_All and true == Panel_Dialog_ServantMarket_All:GetShow() then
  ...
  elseif nil ~= Panel_Window_WorkerAuction_All then
    HandleEventLUp_WorkerAuction_All_Close()
  elseif nil ~= Panel_Dialog_WorkerTrade_Renew then
    FGlobal_WorkerTrade_Close()
  end
  if nil ~= PaGlobalFunc_WorkerManagerChangeName_All_GetShow and true == PaGlobalFunc_WorkerManagerChangeName_All_GetShow() then
    PaGlobalFunc_WorkerManagerChangeName_All_Close()
    return false
  end
  if nil ~= Panel_Dialog_Trade_Tooltip_All and true == Panel_Dialog_Trade_Tooltip_All:GetShow() then
  ...
  if nil ~= Panel_Window_WareHouseHistory and true == Panel_Window_WareHouseHistory:GetShow() then
    PaGlobal_WareHouseHistory:close()
    return false
  end
  if PaGlobalFunc_Working_Progress_GetShow() == true then
    PaGlobalFunc_WorkingProgress_HideWorkingProgress()
    return true
  end
  return true
end
function FGlobal_WorldMapWindowEscape()
  ...
  local _panel_TradeMarket = Panel_Worldmap_Trade_MarketItemList_All or Panel_Trade_Market_Graph_Window
  local _panel_WorkerManager = Panel_Window_WorkerManager_All or Panel_WorkerManager
  local _worldmapUI = {
    ...
    [14] = PaGlobalFunc_ItemMarket_GetShow() == false or true == PaGlobalFunc_ItemMarket_IsUISubApp(),
    [15] = Panel_WorldMap_MovieGuide:GetShow() == false,
    [16] = Panel_Worldmap_NodeLevel:GetShow() == false,
    [17] = Panel_Window_ExpirienceWiki_All:GetShow() == false,
    [18] = Panel_ProductNote:GetShow() == false or Panel_ProductNote:IsUISubApp() == true
  }
...
function FGlobal_WorldMapWindowEscapeForMouseClick(panel)
  TooltipSimple_Hide()
  local _panel_TradeMarket_EventInfo = true == _ContentsGroup_isUsedNewTradeEventNotice and Panel_Window_TradeEventNotice_Renewal_All or Panel_TradeEventNotice_Renewal or Panel_TradeMarket_EventInfo
  local _panel_HouseControl = true == _ContentsGroup_ForXBoxFinalCert and Panel_Worldmap_HouseCraft or Panel_HouseControl
  local _panel_TradeMarket = Panel_Worldmap_Trade_MarketItemList_All or Panel_Trade_Market_Graph_Window
  ...
  if true == ToClient_WorldMapIsShow() then
    ...
    if not WorldMapPopupManager:popPanelForMouseClick(panel) then
      FGlobal_PopCloseWorldMap()
    end
  end
  if 0 > WorldMapPopupManager._currentMode then
...
function FGlobal_WorldMapClose()
  ...
  isFirstCall = false
  if false == _ContentsGroup_RenewUI_Worker and nil ~= Panel_Window_WorkerManager_All and true == Panel_Window_WorkerManager_All:GetShow() then
    PaGlobalFunc_WorkerManager_All_Close()
  end
  ...
  if PaGlobal_MiniGame_GalleyPaddle_All_responseOtherWindowClose ~= nil then
    PaGlobal_MiniGame_GalleyPaddle_All_responseOtherWindowClose()
  end
...
function FGlobal_WorldMapCloseSubPanel()
  ...
  if Panel_ProductNote ~= nil and Panel_ProductNote:IsShow() == true and Panel_ProductNote:IsUISubApp() == false then
    Panel_ProductNote_ShowToggle()
  end
...
function FromClient_OpenRemoteInstall_Ready(regionKey, isMinorSiege)
  ...
  if true == isMinorSiege then
    siegeWrapper = regionWrapper:getMinorSiegeWrapper()
  elseif true == _ContentsGroup_ShowMajorSiegeArea then
    siegeWrapper = regionWrapper:getSiegeWrapper()
  end
  if nil == siegeWrapper then
    return
  end
  local regionPosition = siegeWrapper:getPosition()
  local function startRemoteInstall()
    ToClient_RemoteInstallModeOnOff(true, regionKey, isMinorSiege)
  end
  local messageboxData = {
    title = "",
    content = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_REMOTE_CONSTRUCTION_POPUP_DESC", "territory", siegeWrapper:getTerritoryName(), "region", regionWrapper:getAreaName()),
    functionYes = startRemoteInstall,
    functionCancel = MessageBox_Empty_function
  }
  MessageBox.showMessageBox(messageboxData, "top")
end
function FromWeb_SelfPlayerWorldMapNaviStart_PC(x, y, z)
  local picturePosition = float3(x, y, z)
  if ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_HorseRacing) == true then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_HORSERACING_CANT_WAYPOINT"))
  end
  ...
  ToClient_DeleteNaviGuideByGroup(0)
  ToClient_WorldMapNaviStart(picturePosition, NavigationGuideParam(), false, true)
  audioPostEvent_SystemUi(0, 14)
end
...
registerEvent("FromClient_MClickWorldmapPanel", "FromClient_MClickWorldmapPanel")
...
registerEvent("FromClient_WorldMap_AbyssOnePortalIcon_MouseOn", "FromClient_WorldMap_AbyssOnePortalIcon_MouseOn")
registerEvent("FromClient_WorldMap_AbyssOnePortalIcon_MouseOut", "FromClient_WorldMap_AbyssOnePortalIcon_MouseOut")
registerEvent("FromClient_WorldMap_AbyssOnePortalIcon_RClick", "FromClient_WorldMap_AbyssOnePortalIcon_RClick")
registerEvent("FromClient_WorldMap_MorningLandBossIcon_RClick", "FromClient_WorldMap_MorningLandBossIcon_RClick")
...
registerEvent("FromClient_OpenRemoteInstall", "FromClient_OpenRemoteInstall_Ready")
...
registerEvent("FromWeb_SelfPlayerWorldMapNaviStart", "FromWeb_SelfPlayerWorldMapNaviStart_PC")
