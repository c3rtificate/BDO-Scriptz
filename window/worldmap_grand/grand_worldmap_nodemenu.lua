...
local _currentNodeLv, lastFindNodeKey
...
if ToClient_isConsole() == false and _ContentsGroup_MinorSiege2022 == false then
  Btn_SetCapital = UI.getChildControl(Panel_Worldmap_MainCitadelSelect, "Button_Apply")
end
local capitalRegionKey
...
if true == _ContentsGroup_ChinaFontColor then
  NodeTextColor[NodeTextType.NODE_UNUPGRADE_TEXT] = UI_color.C_FFFFAD38
end
...
local villageSiegeType = CppEnums.VillageSiegeType
...
local function FillContryInfo(nodeStaticStatus)
  ...
  elseif tostring(getNodeNationalName(nodeStaticStatus)) == PAGetString(Defines.StringSheet_GAME, "LUA_CONTRYNAME_7") then
    countryIcon:ChangeTextureInfoName("Renewal/ETC/WordMap/territorymark_Odyllita_small.dds")
  elseif tostring(getNodeNationalName(nodeStaticStatus)) == PAGetString(Defines.StringSheet_GAME, "LUA_WORLDMAP_NODE_CONTRY_SNOWYMOUNTAIN") then
    countryIcon:ChangeTextureInfoName("Renewal/ETC/WordMap/territorymark_SnowMountain_small.dds")
  elseif tostring(getNodeNationalName(nodeStaticStatus)) == PAGetString(Defines.StringSheet_GAME, "LUA_QUESTLIST_HISTORY_TXT_REGION_13") then
    countryIcon:ChangeTextureInfoName("Renewal/ETC/WordMap/territorymark_MorningLand_small.dds")
  elseif tostring(getNodeNationalName(nodeStaticStatus)) == PAGetString(Defines.StringSheet_GAME, "LUA_QUESTLIST_HISTORY_TXT_REGION_14") then
    countryIcon:ChangeTextureInfoName("Renewal/ETC/WordMap/territorymark_Ulukita_small.dds")
  end
...
local function FillTerritoryInfo(nodeStaticStatus)
  ...
  if tostring(getNodeTerritoryName(nodeStaticStatus)) == PAGetString(Defines.StringSheet_GAME, "LUA_WORLDMAP_NODE_BALENOS") then
    territoryIcon:ChangeTextureInfoName("Renewal/ETC/WordMap/territorymark_valenos_Small.dds")
  elseif tostring(getNodeTerritoryName(nodeStaticStatus)) == PAGetString(Defines.StringSheet_GAME, "LUA_WORLDMAP_NODE_SERENDIA") then
    territoryIcon:ChangeTextureInfoName("Renewal/ETC/WordMap/territorymark_selendia_Small.dds")
  elseif tostring(getNodeTerritoryName(nodeStaticStatus)) == PAGetString(Defines.StringSheet_GAME, "LUA_WORLDMAP_NODE_KALPEON") then
    territoryIcon:ChangeTextureInfoName("Renewal/ETC/WordMap/territorymark_Calpheon_Small.dds")
  elseif tostring(getNodeTerritoryName(nodeStaticStatus)) == PAGetString(Defines.StringSheet_GAME, "LUA_WORLDMAP_NODE_MEDIA") then
    territoryIcon:ChangeTextureInfoName("Renewal/ETC/WordMap/territorymark_mediah_Small.dds")
  elseif tostring(getNodeTerritoryName(nodeStaticStatus)) == PAGetString(Defines.StringSheet_GAME, "LUA_WORLDMAP_NODE_BALENCIA") then
    territoryIcon:ChangeTextureInfoName("Renewal/ETC/WordMap/territorymark_valencia_Small.dds")
  elseif tostring(getNodeTerritoryName(nodeStaticStatus)) == PAGetString(Defines.StringSheet_GAME, "LUA_TERRITORYNAME_6") then
  ...
  elseif tostring(getNodeNationalName(nodeStaticStatus)) == PAGetString(Defines.StringSheet_GAME, "LUA_TERRITORYNAME_8") then
    territoryIcon:ChangeTextureInfoName("Renewal/ETC/WordMap/territorymark_Odyllita_small.dds")
  elseif tostring(getNodeNationalName(nodeStaticStatus)) == PAGetString(Defines.StringSheet_GAME, "LUA_QUESTLIST_HISTORY_TXT_REGION_14") then
    territoryIcon:ChangeTextureInfoName("Renewal/ETC/WordMap/territorymark_Ulukita_small.dds")
  else
...
local function FillExploreUpgradeAble(nodeStaticStatus, nodeKey)
  ...
  if ToClient_isAbleInvestnWithdraw(nodeKey) then
    ...
    Txt_Node_Desc:SetTextMode(__eTextMode_AutoWrap)
...
local function GenerateNodeInfo(nodeStaticStatus, nodeKey, isAffiliated)
  ...
  if "" ~= nodeManagerName then
    ...
    if Txt_NodeManager_Name:GetSizeX() <= Txt_NodeManager_Name:GetTextSizeX() then
      Txt_NodeManager_Name:SetTextMode(__eTextMode_LimitText)
...
local function update_ExplorePoint()
  ...
  Stc_ExplorePoint_Icon:SetTextMode(__eTextMode_AutoWrap)
  ...
  if isGameTypeEnglish() or isGameTypeSA() or isGameTypeTH() or isGameTypeASIA() or isGameTypeID() or isGameTypeTR() then
...
local function FillNodeInfo(nodeStaticStatus, nodeKey, isAffiliated, isPurchased, isMaxLevel)
  GenerateNodeInfo(nodeStaticStatus, nodeKey, isAffiliated)
  ...
  if isExploreUpgradable(nodeKey) then
    if isPurchased == false then
      ...
      NodeLevelGroup.Btn_NodeLevHelp:SetShow(false)
      ...
      if isWithdrawablePlant(nodeKey) then
        ...
        TooltipSimple_Hide()
      elseif needPoint > 0 or needMoney > 0 or recipeItems > 0 then
...
function NodeLevelGroup:SetNodeLevel(nodeKey)
  ...
  local nodeLevelMax = ToClient_GetNodeMaxLevel()
  local nodeExpPercent = nodeExp / nodeExpMax * 100
  if nodeLv >= nodeLevelMax then
    self.Txt_NodeLevel:SetText("Lv. Max")
    nodeExpPercent = 100
    NodeLevelGroup.Btn_NodeLev:SetShow(false)
    NodeLevelGroup.Btn_NodeLevHelp:SetShow(false)
    if PaGlobalFunc_WorldMap_Grand_NodeLevel_GetShow ~= nil and PaGlobalFunc_WorldMap_Grand_NodeLevel_GetShow() == true and PaGlobalFunc_WorldMap_Grand_NodeLevel_Close ~= nil then
      PaGlobalFunc_WorldMap_Grand_NodeLevel_Close()
    end
  else
    self.Txt_NodeLevel:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_LV") .. ". " .. tostring(nodeLv) .. "/" .. nodeLevelMax)
  end
...
function FGlobal_ShowInfoNodeMenuPanel(node)
  ...
  if (plantKey:get() == CppEnums.MiniGameParam.eMiniGameParamLoggiaCorn or plantKey:get() == CppEnums.MiniGameParam.eMiniGameParamLoggiaFarm or plantKey:get() == CppEnums.MiniGameParam.eMiniGameParamAlehandroHoney or plantKey:get() == CppEnums.MiniGameParam.eMiniGameParamImpCave) and true == node:isPurchased() then
  ...
  ClearNodeInfo()
  FillNodeInfo(node:getStaticStatus(), _wayPointKey, _wayPointKey == affiliatedTownKey, node:isPurchased(), node:isMaxLevel())
...
function InvestNodeLevelNumpad(wayPointKey)
  ...
  PaGlobalFunc_WorldMap_NodeLevel_Show(EnterUseWp, wayPointKey)
end
function EnterUseWp(wayPointKey, characterIdx, wp)
  ...
  Panel_NumberPad_Show(true, s64_maxNumber, wayPointKey, InvestNodeLevelExecute, false, characterIdx)
end
function InvestNodeLevelExecute(inputNumber, param1, param2)
...
function FGlobal_ShowBuildingInfo(nodeBtn)
  ...
  PaGlobal_Worlmap_CapitalButtonOpen(nodeBtn)
end
...
function FromClient_BuildingNodeRClick(nodeBtn, isSiege2024)
  if isSiege2024 == true then
    FromClient_RClickWorldmapPanel(nodeBtn:getTentPosition(), true, false)
  else
...
function FromClient_SetCapitalSiegeTentIcon(nodeBtn)
  if nil == nodeBtn then
    return
  end
  nodeBtn:ChangeTextureInfoNameDefault("New_UI_Common_forLua/Widget/worldmap/worldmap_icon_04.dds")
  local x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 0, 203, 167, 398)
  nodeBtn:getBaseTexture():setUV(x1, y1, x2, y2)
  nodeBtn:setRenderTexture(nodeBtn:getBaseTexture())
end
function FromClient_ResetCapitalSiegeTentIcon(nodeBtn)
  if nil == nodeBtn then
    return
  end
  nodeBtn:ChangeTextureInfoNameDefault("New_UI_Common_forLua/Widget/worldmap/worldmap_icon_04.dds")
  local x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 171, 4, 340, 200)
  nodeBtn:getBaseTexture():setUV(x1, y1, x2, y2)
  nodeBtn:setRenderTexture(nodeBtn:getBaseTexture())
end
...
function worldmapNodeIcon_GuildWarSet(nodeBtn)
  ...
  local guildModeGuildMarkBg = nodeBtn:FromClient_getNodeGuildIconBg()
  if nil ~= regionInfo then
      ...
      if minorSiegeWrapper:isSiegeBeing() then
        ...
        if true == siegeWrapper:doOccupantExist() then
          ...
          if false == _ContentsOption_CH_NoCustomGuildMark then
            ...
            if nil ~= PaGlobalFunc_ConsoleGuildMarkSet then
              PaGlobalFunc_ConsoleGuildMarkSet(siegeWrapper:getGuildNo(), guildMark, guildMarkBG)
            end
            warStateIcon:SetShow(false)
            guildMark:SetShow(true)
            guildMarkBG:SetShow(true)
    ...
    guildModeGuildMarkBg:SetShow(false)
  end
end
function FromClient_CreateNodeIcon(nodeBtn)
  ...
  if true == _ContentsOption_CH_NoCustomGuildMark then
    ...
    local guildIconBg = nodeBtn:FromClient_getNodeGuildIconBg()
    guildIcon:SetHorizonCenter()
    guildIcon:SetVerticalMiddle()
    guildIcon:SetPosXY(0, 0)
    guildIcon:SetSpanSize(0, 0)
    guildIcon:ComputePos()
  end
  if nil ~= explorationInfo then
    ...
    if nil ~= minorSiegeRegion then
      ...
      if isWeekNode then
        ...
        if true == _ContentsOption_CH_NoCustomGuildMark then
          local siegeWrapper = ToClient_GetSiegeWrapperByRegionKey(regionKey)
          if nil ~= siegeWrapper and nil ~= PaGlobalFunc_ConsoleGuildMarkSet then
            PaGlobalFunc_ConsoleGuildMarkSet(siegeWrapper:getGuildNo(), guildMark, guildMarkBG)
            PaGlobalFunc_ConsoleGuildMarkSet(siegeWrapper:getGuildNo(), guildIcon, guildIconBg)
          end
        end
      end
    end
...
function FromClient_SetGuildModeeWorldMapNodeIcon(nodeBtn)
  if FGlobal_isGuildWarMode() then
    ...
    local taxGrade = 0
    local taxGradeOneMaxLevel = 2
    if nil ~= siegeNode then
      taxGrade = siegeNode:getVillageTaxLevel()
      if taxGrade > 0 then
        villageSiegeType = taxGrade + taxGradeOneMaxLevel
      else
        villageSiegeType = siegeNode:getSiegeOptionTypeByRegion()
        if taxGradeOneMaxLevel < villageSiegeType then
          villageSiegeType = villageSiegeType.eVillageSiegeType_Count
        end
      end
    end
    if nil ~= siegeNode and villageSiegeType < 7 then
      ...
      local siegeWrapper = ToClient_GetSiegeWrapperByRegionKey(siegeNode._regionKey:get())
      local textureUrl = "Combine/Etc/Combine_Etc_StrongholdWar_01.dds"
      if 1 == siegeNode:getConquestSiegeType() then
        if nil ~= siegeWrapper and true == siegeWrapper:doOccupantExist() then
          nodeBtn:ChangeTextureInfoName(textureUrl)
          local x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 218, 1, 328, 141)
          nodeBtn:getBaseTexture():setUV(x1, y1, x2, y2)
          nodeBtn:setRenderTexture(nodeBtn:getBaseTexture())
          nodeBtn:ChangeOnTextureInfoName(textureUrl)
          x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 329, 1, 439, 141)
          nodeBtn:getOnTexture():setUV(x1, y1, x2, y2)
          nodeBtn:ChangeClickTextureInfoName(textureUrl)
          x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 440, 1, 550, 141)
          nodeBtn:getClickTexture():setUV(x1, y1, x2, y2)
        else
          nodeBtn:ChangeTextureInfoName(textureUrl)
          local x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 218, 283, 328, 423)
          nodeBtn:getBaseTexture():setUV(x1, y1, x2, y2)
          nodeBtn:setRenderTexture(nodeBtn:getBaseTexture())
          nodeBtn:ChangeOnTextureInfoName(textureUrl)
          x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 329, 283, 439, 423)
          nodeBtn:getOnTexture():setUV(x1, y1, x2, y2)
          nodeBtn:ChangeClickTextureInfoName(textureUrl)
          x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 440, 283, 550, 423)
          nodeBtn:getClickTexture():setUV(x1, y1, x2, y2)
        end
      elseif nil ~= siegeWrapper and true == siegeWrapper:doOccupantExist() then
        nodeBtn:ChangeTextureInfoName(textureUrl)
        local x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 218, 142, 328, 282)
        nodeBtn:getBaseTexture():setUV(x1, y1, x2, y2)
        nodeBtn:setRenderTexture(nodeBtn:getBaseTexture())
        nodeBtn:ChangeOnTextureInfoName(textureUrl)
        x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 329, 142, 439, 282)
        nodeBtn:getOnTexture():setUV(x1, y1, x2, y2)
        nodeBtn:ChangeClickTextureInfoName(textureUrl)
        x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 440, 142, 550, 282)
        nodeBtn:getClickTexture():setUV(x1, y1, x2, y2)
      else
        nodeBtn:ChangeTextureInfoName(textureUrl)
        local x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 218, 424, 328, 564)
        nodeBtn:getBaseTexture():setUV(x1, y1, x2, y2)
        nodeBtn:setRenderTexture(nodeBtn:getBaseTexture())
        nodeBtn:ChangeOnTextureInfoName(textureUrl)
        x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 329, 424, 439, 564)
        nodeBtn:getOnTexture():setUV(x1, y1, x2, y2)
        nodeBtn:ChangeClickTextureInfoName(textureUrl)
        x1, y1, x2, y2 = setTextureUV_Func(nodeBtn, 440, 424, 550, 564)
        nodeBtn:getClickTexture():setUV(x1, y1, x2, y2)
      end
    end
    if nil ~= explorationInfo then
...
function FGlobal_LoadWorldMap_WarehouseOpen()
  if nil ~= _wayPointKey then
    ...
    if nil ~= regionInfoWrapper and regionInfoWrapper:get():isMainOrMinorTown() and regionInfoWrapper:get():hasWareHouseNpc() then
      PaGlobal_Warehouse_All_OpenPanelFromWorldmap(_wayPointKey, CppEnums.WarehoouseFromType.eWarehoouseFromType_Worldmap)
    end
  end
end
    ...
    if 0 == buttonType then
      ...
      if _ContentsGroup_NewNodeDropBuff == true then
        ...
        if selfplayerActorProxyWrapper ~= nil then
          ...
          local minorSiegeRegionKey = ToClient_GetSiegeRegionKeyRawByWaypointKey(_wayPointKey)
          local regionKey = ToClient_GetRegionKeyRawByWaypointKey(_wayPointKey)
        ...
        local percent
        desc = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORLDMAP_DROPRATETOOLTIP_DESC_1", "percent", currentNodeBuffPercent)
        local huntingGroundNameList = ToClient_getHuntingGroundNameByWaypointKey(_wayPointKey)
      ...
      elseif currentNodeBuffPercent > 0 then
...
function FromClient_Worldmap_GuildVehicleIcon_NewClicked(actorKeyRaw, isShip)
  guildVehicleActorKeyRawForRecall = actorKeyRaw
  local ApplyDistroy = function()
    ToClient_Worldmap_GuildVehicleRecall(guildVehicleActorKeyRawForRecall)
  end
  local desc = ""
  if 1 == isShip then
    local needMoney = tostring(getServantCompulsionSealPrice())
    desc = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_GUILDWHAFT_COMPULSIONSEALDESC", "needMoney", needMoney)
  else
    desc = PAGetString(Defines.StringSheet_GAME, "LUA_STABLELIST_ISCOMPULSION_MESSAGEBOX")
  end
  local messageBoxData = {
...
function PaGlobal_Worlmap_CapitalButtonOpen(nodeButton)
...
registerEvent("FromClient_SetCapitalSiegeTentIcon", "FromClient_SetCapitalSiegeTentIcon")
registerEvent("FromClient_ResetCapitalSiegeTentIcon", "FromClient_ResetCapitalSiegeTentIcon")
if false == _ContentsGroup_RenewUI then
