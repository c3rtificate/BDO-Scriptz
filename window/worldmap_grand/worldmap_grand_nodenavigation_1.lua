...
function PaGlobal_Worldmap_Grand_NodeNavigation:onSelectedToNode(nodeName)
  ...
  if currentHaveCP < needCP then
    if true == _ContentsOption_CH_GameType then
      showCPText = "<PAColor0xFFE4A84A>" .. tostring(needCP) .. "<PAOldColor>"
    else
  ...
  if true == applyPremium then
    ...
    if true == _ContentsOption_CH_GameType then
      isPossibleText = "<PAColor0xFFE4A84A>" .. PAGetString(Defines.StringSheet_GAME, "LUA_MAIDLIST_IMPOSSIBLE") .. "<PAOldColor>"
    else
  ...
  if needWP > currentHaveWP then
    if true == _ContentsOption_CH_GameType then
      showWPText = "<PAColor0xFFE4A84A>" .. tostring(needWP) .. "<PAOldColor>"
    else
