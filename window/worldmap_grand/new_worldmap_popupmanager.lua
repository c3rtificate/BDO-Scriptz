...
function WorldMapPopupManager:pop()
  ...
  local check_Panel
  if popArray ~= nil and 0 ~= popArray:length() then
    for key, value in pairs(popArray) do
  ...
  if 0 <= self._currentMode then
    ...
    for key, value in pairs(popArray) do
      ...
      elseif value._panel == Panel_NodeMenu then
...
function WorldMapPopupManager:popWantPanel(panel)
  if 0 > self._currentMode then
    return false
  end
  local mode = self._currentMode
  ...
  local popArray = self._scenePopupArray[mode]
  local cacheArray = Array.new()
  if 1 < popArray:length() then
    for key, value in pairs(popArray) do
      if value._panel == panel then
        value._panel:SetShow(false)
        if nil ~= value._cloSefunc then
          value._cloSefunc()
        end
      else
        cacheArray:push_back(value)
      end
    end
    self._scenePopupArray[mode] = cacheArray
    return true
  else
    self:pop()
  end
