...
function LargCraft_List:_updateSlot()
  ...
  if self._onGoingIndex == self._currentIndex then
    ...
    defalut_Control._largCraft_List._Guide:SetTextMode(__eTextMode_LimitText)
    defalut_Control._largCraft_List._Guide:SetText(guideText)
    if true == defalut_Control._largCraft_List._Guide:IsLimitText() then
      defalut_Control._largCraft_List._Guide:SetIgnore(false)
      defalut_Control._largCraft_List._Guide:addInputEvent("Mouse_On", "HandleEventOnOut_LargeCraft_GuideTooltip(true)")
      defalut_Control._largCraft_List._Guide:addInputEvent("Mouse_Out", "HandleEventOnOut_LargeCraft_GuideTooltip(false)")
    else
      defalut_Control._largCraft_List._Guide:SetIgnore(true)
      defalut_Control._largCraft_List._Guide:addInputEvent("Mouse_On", "")
      defalut_Control._largCraft_List._Guide:addInputEvent("Mouse_Out", "")
    end
...
function SubWork_List:_updateSlot(isRefresh)
  ...
  for idx = 1, self._rowMax do
    ...
    if nil == self._data_Table[index]._resource[_idx] then
      ...
      UI.setLimitTextAndAddTooltip(defalut_Control._subWork_List._Button[idx])
...
function SubWork_List_Immediately_CraftItem()
  ...
  if esSSW:isSet() then
    ...
    local messageBoxMemo = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_NEW_WORLDMAP_WORKMANAGER_HOUSE_IMMEDIATELY_SILVER", "workName", workName, "needSilver", makeDotMoney(defalut_Control._subWork_info._needPearl))
    local messageboxData = {
...
local function Worker_SortByRegionInfo()
  ...
  local waitingWorkerCount = ToClient_getHouseWaitWorkerList(houseInfoSS, productCategory, workableKey, receipeKey)
  local workVolume = Int64toInt32(esSS._productTime / toInt64(0, 1000))
  ...
  if esSSW:isSet() then
    ...
    for Index = 1, waitingWorkerCount do
      ...
      if true == ToClient_isWaitWorker(npcWaitingWorker) and false == workerWrapperLua:getIsAuctionInsert() then
        ...
        local name
        if isWorkable then
          name = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_LV") .. "." .. npcWaitingWorker:getLevel() .. " " .. workerWrapperLua:getName() .. "(<PAColor0xff868686>" .. workerRegionWrapper:getAreaName() .. "<PAOldColor>)"
        else
          name = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_LV") .. "." .. npcWaitingWorker:getLevel() .. " " .. PAGetStringParam2(Defines.StringSheet_GAME, "LUA_NEW_WORLDMAP_WORKMANAGER_HOUSE_NOTFINDNODE", "npcWaitingWorkerSS", workerWrapperLua:getName(), "getAreaName", workerRegionWrapper:getAreaName())
        end
...
function HandleOn_LargeCraft_Worker_List(index)
  Worker_List._over_Index = Worker_List._offsetIndex + index
  defalut_Control._worker_List._ActionPoint[index]:SetShow(true)
  SubWork_Estimated_Update(false)
  local self = Worker_List
  local workerIndex = Worker_List._offsetIndex + index
  ...
  if nil ~= npcWaitingWorker then
    ...
    PaGlobalFunc_WorkerManagerTooltip_All_OpenByWorkerNoRaw(Worker_List._data_Table[workerIndex]._workerNo_s64, uiBase, true)
  end
end
...
function SubWork_Estimated_Update(isWorkOver)
  ...
  defalut_Control._subWork_Estimated._Time_Value:SetText("--")
...
function HandleEventOnOut_LargeCraft_GuideTooltip(isShow)
  if false == isShow then
    TooltipSimple_Hide()
    return
  end
  TooltipSimple_Show(defalut_Control._largCraft_List._Guide, "", defalut_Control._largCraft_List._Guide:GetText())
end
...
function HandleClick_doWork_LargeCraft()
  ...
  if currentKey ~= selected_Work then
    ...
    if 0 < LimitWorkableCount_LargeCraft() then
      ...
      PaGlobalFunc_WorkerManager_All_RedoWork(1, houseInfoSS, selected_Worker, nil, selected_Work, selected_SubWork, workCount, nil, nil, affiliatedTownKey)
    else
...
function HandleClick_OnGoing_Cancel_LargeCraft(idx)
  ...
  local _workerNo = worker_StopWork:get()
