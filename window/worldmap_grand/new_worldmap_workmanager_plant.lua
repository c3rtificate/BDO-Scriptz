...
local defalut_Control = {
  ...
  _work_Info = {
    ...
    _WorkDestinationText = UI.getChildControl(Panel_Plant_WorkManager, "StaticText_WarehouseRegion_Text"),
    _WorkDestination = UI.getChildControl(Panel_Plant_WorkManager, "Combobox_WarehouseRegion")
  }
}
defalut_Control._btn_Question:addInputEvent("Mouse_LUp", "Panel_WebHelper_ShowToggle( \"HouseManageWork\" )")
PaGlobal_Util_RegistHelpTooltipEvent(defalut_Control._btn_Question, "\"HouseManageWork\"")
if true == _ContentsOption_CH_Help then
  defalut_Control._btn_Question:SetShow(false)
end
function defalut_Control:Init_Control()
  ...
  self._work_Info._WorkDestination:setListTextHorizonCenter()
end
...
function defalut_Control:Init_Function()
  ...
  self._work_Info._WorkDestination:addInputEvent("Mouse_LUp", "HandleEventLUp_WorkDestination_ShowToWaypointKey()")
end
...
local function Worker_SortByRegionInfo()
  ...
  for index = 1, waitingWorkerCount do
    ...
    if true == ToClient_isWaitWorker(npcWaitingWorker) and false == workerWrapperLua:getIsAuctionInsert() then
      ...
      sortElement[possibleWorkerIndex] = {}
      sortElement[possibleWorkerIndex]._isConnected = workerWrapperLua:isWorkingable(Work_Info._plantKey:getWaypointKey())
      sortElement[possibleWorkerIndex]._workTime = Int64toInt32(totalWorkTime)
    end
  end
  ...
  local function newWorkerSortingFunction(sortInfoList, sortInfoListCount)
    local function workerMemorySwapper(idx, jdx)
      local temp = Worker_List._data_Table[idx]
      Worker_List._data_Table[idx] = Worker_List._data_Table[jdx]
      Worker_List._data_Table[jdx] = temp
    ...
    selectionSort(sortInfoList, 1, sortInfoListCount, isConnectedSorter)
    local middleIdx = split(sortInfoList, sortInfoListCount, isConnectedChecker)
    selectionSort(sortInfoList, 1, middleIdx - 1, isFasterSorter)
    selectionSort(sortInfoList, middleIdx, sortInfoListCount, isFasterSorter)
  end
...
function Worker_List:_updateSlot()
  ...
  for index = 1, self._rowMax do
    ...
    defalut_Control._worker_List._Button[index]:SetTextMode(__eTextMode_LimitText)
...
function Plant_Worker_List_Select(index)
  ...
  if nil ~= Worker_List._data_Table[selectedIndex] then
    ...
    UpdateStorageComboboxList(Worker_List._data_Table[selectedIndex]._level)
  else
    defalut_Control._worker_List._No_Worker:SetShow(true)
    UpdateStorageComboboxList(-1)
  end
  if nil == Worker_List._data_Table[selectedIndex] then
...
function HandleOn_Plant_Worker_List(index)
  ...
  if nil ~= npcWaitingWorker then
    ...
    PaGlobalFunc_WorkerManagerTooltip_All_OpenByWorkerNoRaw(Worker_List._data_Table[workerIndex]._workerNo_s64, uiBase, false, true)
  end
end
...
function Plant_Work_Info_Update(isWorkerOver)
  ...
  if nil ~= workerData then
    ...
    defalut_Control._work_Info._WorkDestination:SetText(workerData._regionNameRaw)
  else
    ...
    defalut_Control._work_Info._WorkDestination:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_DELIVERY_SELECT_TO_REGION"))
  end
end
local Scroll_UpDown = function(isUp, _target)
  if nil == _target._offsetIndex then
    return
  end
  if false == isUp then
...
function FGlobal_Plant_WorkManager_Open(node, isNormalNode)
  ...
  PrepareOpenStorageComboboxList(isNormalNode)
end
...
function HandleClick_Plant_doWork()
  ...
  local selectedIndex = defalut_Control._work_Info._WorkDestination:GetSelectIndex()
  ...
  local doWorkNow = ToClient_requestStartPlantWorkingToNpcWorker(Worker_List._selected_Worker, Work_Info._plantKey, Work_Info._workKey, workingCount, targetWaypoint)
  PaGlobalFunc_WorkerManager_All_RedoWork(2, nil, Worker_List._selected_Worker, Work_Info._plantKey, Work_Info._workKey, nil, workingCount, nil, nil, affiliatedTownKey)
  if doWorkNow then
    FGlobal_ShowWorkingProgress(FGlobal_SelectedNode(), 1, false)
  end
...
function HandleEventLUp_WorkDestination_ShowToWaypointKey()
  defalut_Control._work_Info._WorkDestination:ToggleListbox()
end
function PrepareOpenStorageComboboxList(isNormalNode)
  local destinationBox = defalut_Control._work_Info._WorkDestination
  local destinationBoxText = defalut_Control._work_Info._WorkDestinationText
  ...
  destinationBox:SetShow(isNormalNode)
  destinationBoxText:SetShow(isNormalNode)
...
function UpdateStorageComboboxList(workerLevel)
  local destinationBox = defalut_Control._work_Info._WorkDestination
  local cantUseComboBox = workerLevel < __eWorkerPossibleLevelChangeStorage
