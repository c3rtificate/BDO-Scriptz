...
function Work_Info_Update(isWorkOver)
  ...
  for idx = 1, resourceCount do
    ...
    if enchantLevel > 0 then
      ...
      defalut_Control._work_Info._Resource_Level[idx]:SetText(printenchantLevel)
      defalut_Control._work_Info._Resource_Level[idx]:SetShow(true)
    else
      defalut_Control._work_Info._Resource_Level[idx]:SetShow(false)
    end
...
function Work_Estimated_Update(isWorkOver)
  ...
  if isWorkOver ~= nil and isWorkOver == true then
    if Work_List._dataIndex[Work_List._over_Index] == nil then
      return
    end
    rowIndex = Work_List._dataIndex[Work_List._over_Index]._rowIndex
    collumIndex = Work_List._dataIndex[Work_List._over_Index]._collumIndex
...
function Item_Tooltip_Show_Test(isResult, idx)
  ...
  if nil == staticStatusKey and true == isResult then
    ...
    defalut_Control._tooltip_WorkName:SetTextMode(__eTextMode_AutoWrap)
...
function FGlobal_RentHouse_WorkManager_Open(houseInfoSSWrapper, _param)
  ...
  defalut_Control._houseInfo._UseType_Desc:SetTextMode(__eTextMode_AutoWrap)
...
function HandleClick_doWork_Confirm()
  ...
  if nil ~= itemNoRaw then
    ...
    PaGlobalFunc_WorkerManager_All_RedoWork(0, houseInfoSS, Worker_List._selected_Worker, nil, Work_List._selected_Work, nil, workingCount, itemNoRaw, nil, _affiliatedTownKey)
