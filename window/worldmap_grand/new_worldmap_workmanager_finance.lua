...
if true == _ContentsGroup_ChinaFontColor then
  defalut_Control._Button_NoResource:SetFontColor(Defines.Color.C_FFFFAD38)
end
...
function defalut_Control:Init_Control()
  ...
  defalut_Control._workInfo_Default._Desc:SetTextMode(__eTextMode_AutoWrap)
end
...
function Worker_List:_setData()
  ...
  for Index = 1, waitingWorkerCount do
    ...
    if true == ToClient_isWaitWorker(npcWaitingWorker) and false == workerWrapperLua:getIsAuctionInsert() then
      ...
      local name = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_LV") .. "." .. npcWaitingWorker:getLevel() .. " " .. workerWrapperLua:getName() .. "(<PAColor0xff868686>" .. workerRegionWrapper:getAreaName() .. "<PAOldColor>)"
...
function HandleOn_Finance_Worker_List(index)
  ...
  if nil ~= npcWaitingWorker then
    ...
    PaGlobalFunc_WorkerManagerTooltip_All_OpenByWorkerNoRaw(Worker_List._data_Table[workerIndex]._workerNo_s64, uiBase, false)
  end
end
...
function HandleClick_Finance_doWork()
  ...
  local doWorkNow = ToClient_requestStartPlantWorkingToNpcWorker(Worker_List._selected_Worker, Work_Info._plantKey, Work_Info._workKey, workingCount, -1)
  PaGlobalFunc_WorkerManager_All_RedoWork(4, nil, Worker_List._selected_Worker, Work_Info._plantKey, Work_Info._workKey, nil, workingCount, nil, nil, _affiliatedTownKey)
  if doWorkNow then
