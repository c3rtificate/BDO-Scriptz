...
toolTip:SetTextMode(__eTextMode_AutoWrap)
...
PaGlobal_Util_RegistHelpTooltipEvent(_buttonQuestion, "\"WarInfo\"")
...
local function territoryInfo_Init()
  ...
  for territory_idx = 0, territoryCount - 1 do
    ...
    if toInt64(0, 5) ~= territoryKey and true == ToClient_isStartMajorSiege(territoryKey:get()) then
      local territoryName = getTerritoryNameByKey(territoryKey)
      if nil ~= territoryKey then
        local territoryDATA = {}
        territoryDATA.Name = UI.createControl(__ePAUIControl_StaticText, Panel_Win_Worldmap_WarInfo, "StaticText_Territory_Name_" .. territory_idx)
        ...
        territoryDATA.FindCount = UI.createControl(__ePAUIControl_StaticText, Panel_Win_Worldmap_WarInfo, "StaticText_Territory_FindCount_" .. territory_idx)
        ...
        territoryDATA.AllCount = UI.createControl(__ePAUIControl_StaticText, Panel_Win_Worldmap_WarInfo, "StaticText_Territory_AllCount_" .. territory_idx)
