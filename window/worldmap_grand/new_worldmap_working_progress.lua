...
WorkingProgress._txt_WorkName:SetTextMode(__eTextMode_LimitText)
...
function WorkingProgressUpdate()
  ...
  for idx = 0, workingcnt - 1 do
    ...
    local replaceText = WorkingProgress_ReplaceRemainText(WorkingProgress._remainTime)
    WorkingProgress._remainTimeText:SetText(replaceText)
...
function WorkingProgress_ReplaceRemainText(text)
  local tempStr = ""
  if nil == text then
    return tempStr
  end
...
function WorkingProgress_WorkerTooltip(isShow)
  if true == isShow then
    ...
    local workerNo = worker:get()
    local workerDATA = ToClient_getNpcWorkerByWorkerNo(workerNo)
    PaGlobalFunc_WorkerManagerTooltip_All_OpenByWorkerNoRaw(workerNo, WorkingProgress._workerName)
  else
...
function WorkingProgress_CancelWork()
  if nil == WorkingProgress._node then
    return
  end
  local node = WorkingProgress._node
  local plantKey = node:getPlantKey()
...
function WorkingProgress_Immediately()
  ...
  if esSSW:isSet() then
    ...
    local messageBoxMemo = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_WORLDMAP_WORKDING_PROGRESS_IMMEDIATELY_SILVER", "workName", workName, "needSilver", makeDotMoney(_needPearl))
    local messageboxData = {
...
function WorkingProgress_After_CancelWork(workerNo, isUserRequest)
  ...
  if workType == eWorkType.plant then
    ...
    if exploreLevel > 0 and plant ~= nil and plant:getType() == PlantType.ePlantType_Zone then
      ...
      if isFinanceNode == true then
        FGlobal_Finance_WorkManager_Open(FGlobal_SelectedNode())
      else
        FGlobal_Plant_WorkManager_Open(FGlobal_SelectedNode(), isNormalNode)
      end
    end
  end
end
function FGlobal_ShowWorkingProgress(nodeInfo, workType, immediatePanelOpen)
  ...
  if immediatePanelOpen == true then
    Panel_Working_Progress:SetShow(true, false)
  else
    PanelOpenReady = true
  end
end
function PaGlobalFunc_WorkingProgress_HideWorkingProgress()
...
function FromClient_PanelWorkingProgress_SetShow(flag)
  if PanelOpenReady ~= true then
    return
  end
  PanelOpenReady = false
  Panel_Working_Progress:SetShow(flag, false)
  if Panel_Plant_WorkManager ~= nil then
    Panel_Plant_WorkManager:SetShow(false)
  end
  if Panel_Finance_WorkManager ~= nil then
    Panel_Finance_WorkManager:SetShow(false)
  end
end
function PaGlobalFunc_Working_Progress_GetShow()
  if Panel_Working_Progress == nil then
    return false
  end
  return Panel_Working_Progress:GetShow()
end
Panel_Working_Progress_ImmediatelyShowUpdate(false)
WorkingProgress._btnClose:addInputEvent("Mouse_LUp", "PaGlobalFunc_WorkingProgress_HideWorkingProgress()")
...
registerEvent("FromClient_PanelWorkingProgress_SetShow", "FromClient_PanelWorkingProgress_SetShow")
