...
local territoryName = {
  ...
  [8] = tostring(PAGetString(Defines.StringSheet_GAME, "LUA_TERRITORYNAME_8")),
  [9] = tostring(PAGetString(Defines.StringSheet_GAME, "LUA_TERRITORYNAME_9"))
}
local npcName = {
  ...
  [8] = tostring(PAGetString(Defines.StringSheet_GAME, "LUA_TERRITORYSUPPLY_NPC_8")),
  [9] = tostring(PAGetString(Defines.StringSheet_GAME, "LUA_TERRITORYSUPPLY_NPC_9"))
}
function initialize_TradeNpcInfoUI()
  ...
  for count = 1, maxCount do
    local miniPanel = UI.createControl(__ePAUIControl_StaticText, Panel_TradeNpcItemInfo, "static_ItemPanel_" .. count)
    CopyBaseProperty(_basePanel, miniPanel)
    _itemStatic[count] = miniPanel
    local tempItemSlotBG = UI.createControl(__ePAUIControl_Static, Panel_TradeNpcItemInfo, "Static_Slot_" .. count)
