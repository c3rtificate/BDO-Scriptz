...
function Control_WorkerList:Init()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(defaul_Control._buttonQuestion, "\"Worker\"")
  if true == _ContentsOption_CH_Help then
    defaul_Control._buttonQuestion:SetShow(false)
  end
  ...
  for idx = 0, defaul_Control._Slot_MaxCount - 1 do
    self._worker_BG[idx] = UI.createControl(__ePAUIControl_Static, defaul_Control._List_BG, "Worker_BG_" .. tostring(idx))
    self._worker_RestorePoint[idx] = UI.createControl(__ePAUIControl_Progress2, defaul_Control._List_BG, "Worker_RestorePoint_" .. tostring(idx))
    self._worker_CurrentPoint[idx] = UI.createControl(__ePAUIControl_Progress2, defaul_Control._List_BG, "Worker_CurrentPoint_" .. tostring(idx))
    self._worker_CheckBox[idx] = UI.createControl(__ePAUIControl_CheckButton, defaul_Control._List_BG, "Worker_CheckBox_" .. tostring(idx))
    self._worker_Name[idx] = UI.createControl(__ePAUIControl_StaticText, defaul_Control._List_BG, "Worker_Name_" .. tostring(idx))
    self._worker_State[idx] = UI.createControl(__ePAUIControl_Button, defaul_Control._List_BG, "Worker_State_" .. tostring(idx))
    self._worker_Move[idx] = UI.createControl(__ePAUIControl_Button, defaul_Control._List_BG, "Worker_Move" .. tostring(idx))
    self._worker_Redo[idx] = UI.createControl(__ePAUIControl_Button, defaul_Control._List_BG, "Worker_ReDo_" .. tostring(idx))
    self._List_Empty[idx] = UI.createControl(__ePAUIControl_StaticText, defaul_Control._List_BG, "Worker_Empty_" .. tostring(idx))
  ...
  defaul_Control._Guide_Restore:SetTextMode(__eTextMode_AutoWrap)
...
function Control_RestoreItem:Init()
  self._Item_BG = UI.createControl(__ePAUIControl_Static, defaul_Control._List_BG, "Item_SliderBG")
  ...
  self._Btn_CloseItem = UI.createControl(__ePAUIControl_Button, self._Item_BG, "Btn_CloseItem")
  self._Item_SliderBG = UI.createControl(__ePAUIControl_Static, self._Item_BG, "Item_SliderBG")
  ...
  for idx = 0, defaul_Control._Item_MaxCount - 1 do
    self._Item_IconBG[idx] = UI.createControl(__ePAUIControl_Static, self._Item_BG, "Item_Icon_BG_" .. tostring(idx))
    self._Item_Icone[idx] = UI.createControl(__ePAUIControl_Static, self._Item_BG, "Item_Icon_" .. tostring(idx))
    self._Item_Count[idx] = UI.createControl(__ePAUIControl_StaticText, self._Item_BG, "Item_Count_" .. tostring(idx))
    self._Item_RestoreValue[idx] = UI.createControl(__ePAUIControl_StaticText, self._Item_BG, "Item_RestoreValue_" .. tostring(idx))
...
function Worker_Table_Set()
  ...
  for workerIndex = 0, maxWorkerCount - 1 do
    ...
    if workerIndex < waitWorkerCount then
      ...
      local workerNoRaw = workerData:getWorkerNo():get()
      ...
      Worker_Table[workerIndex]._Name = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_LV") .. "." .. workerData:getLevel() .. " " .. ConvertFromGradeToColor(workerGrade, true) .. workerWrapperLua:getName() .. "<PAOldColor>"
...
function RestoreItem_Table_Set()
  ...
  for idx = 0, Cnt_RestoreItem - 1 do
    ...
    RestoreItem_Table[idx]._itemWhereType = NpcRecoveryItem._itemWhereType
  ...
  for idx = 0, scroll_Param._slotMax - 1 do
    if nil ~= Worker_Table[workerIndex] and (true ~= isAdjust or idx < _Index or idx >= _Index + adjust) then
      if false == Worker_Table[workerIndex]._isEmpty then
        ...
        local workStringNo = tostring(Worker_Table[workerIndex]._WorkerNo:get())
        ...
        if Worker_Table[workerIndex]._State == eWorkerStateType.waiting then
          ...
          if nil ~= workerData then
            local workerNo = workerData:getWorkerNo():get()
        ...
        if nil ~= _workerData then
          local _workerNo = _workerData:getWorkerNo():get()
...
function WorkerManage_CancelWork(workerIndex)
  ...
  local _workerNo = Worker_Table[workerIndex]._WorkerNo:get()
...
function Control_Scroll(isUp, isSlider)
  ...
  if save_offset ~= self._offsetIndex then
    ...
    elseif true == isSlider then
      ...
      if ItemTooltip_Index ~= nil and tooltipPanel:GetShow() then
...
function HandleOn_WorkerTooltip(idx)
  ...
  if true == Worker_Table[workerIndex]._isEmpty then
    ...
    HandleEventOn_WorkerManagerTooltip_All_Show(ToClient_getPlantWaitWorkerNoRawByIndex(plantKey, workerIndex), uiBase)
...
function HandleOut_WorkerTooltip(idx, isReset)
  ...
  if WorkerTooltip_Index == idx then
    ...
    HandleEventOut_WorkerManagerTooltip_All_Hide(uiBase)
...
function HandleClick_RestoreItem(idx)
  ...
  local whereType = RestoreItem_Table[index]._itemWhereType
  ...
  requestRecoveryWorker(workerNo, whereType, slotNo, 1)
end
