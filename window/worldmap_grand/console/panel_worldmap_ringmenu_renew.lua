...
function PaGlobalFunc_WorldMap_RingMenu_IsViewMenuButton()
  if nil == Window_WorldMap_RingMenuInfo._currentWayPointKey or Window_WorldMap_RingMenuInfo._currentWayPointKey < 0 then
    return false
  end
  local regionInfoWrapper = ToClient_getRegionInfoWrapperByWaypoint(Window_WorldMap_RingMenuInfo._currentWayPointKey)
  ...
  return receipeCount > 0 or isStableOpen or isWareHouseOpen
end
...
function PaGlobalFunc_FromClient_WorldMap_RingMenu_LClickedWorldMapNode(explorationNode)
  ...
  local isInvest = nodeInfo:isPurchased()
  ...
  self._currentWayPointKey = nodeInfo:getPlantKey():getWaypointKey()
end
...
function PaGlobalFunc_WorldMap_RingMenu_OnOutWorldMapMonsterInfo(isOn)
  local self = Window_WorldMap_RingMenuInfo
  if self == nil then
    return
  end
