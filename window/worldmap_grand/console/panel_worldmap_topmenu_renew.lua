...
function Window_WorldMap_TopMenuInfo:UpdateWarInfoSize()
  ...
  if _ContentsGroup_MinorSiege2022 == true then
    self._ui.txt_warInfoTitle:SetPosY(self._ui.stc_warInfoBG:GetSizeY() * 0.5 - self._ui.txt_warInfoTitle:GetSizeY() * 0.5)
  else
...
function Window_WorldMap_TopMenuInfo:SetWarInfoString(territoryKeyIndex)
  ...
  if _ContentsGroup_MinorSiege2022 == true then
    self._ui.txt_warInfoTitle:SetText(title)
    self._ui.txt_warInfoDesc:SetShow(false)
    return
  end
...
function Window_WorldMap_TopMenuInfo:UpdateContributeText()
  ...
  local descText = PAGetString(Defines.StringSheet_GAME, "LUA_CONTRIBUTE_POINT_DESC") .. "\n"
