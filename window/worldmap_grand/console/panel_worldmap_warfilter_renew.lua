...
local _buttonStrings = {
  [1] = {
    ...
    PAGetString(Defines.StringSheet_GAME, "LUA_NODEGRADE_4"),
    PAGetString(Defines.StringSheet_GAME, "LUA_NODEGRADE_5")
  },
  [2] = {
    PAGetString(Defines.StringSheet_GAME, "LUA_VILLAGETENT_SELECTDEFALUT"),
...
function WarFilter:init()
  ...
  self._ui.txt_categoryRight:SetShow(_ContentsGroup_Siege2024 == false)
  ...
  local territoryCountIndex = 0
  for ii = 1, #_buttonStrings[3] do
    if 7 ~= ii and (ii < 8 or ToClient_isConsole() == false) then
      local indexY = ii
      if ii > 7 then
        indexY = indexY - 1
      end
      ...
      self._ui.btn_territoryFilters[indexY]:SetPosY(self._ui.btn_template:GetPosY() + (territoryCountIndex - 1) * 49)
      self._ui.btn_territoryFilters[indexY]:addInputEvent("Mouse_On", "InputMOn_WarFilter_TerritoryFilter(" .. indexY .. ")")
      if ii == 3 or ii == 5 or ii == 7 then
        self._ui.btn_territoryFilters[indexY]:SetShow(false)
      else
        territoryCountIndex = territoryCountIndex + 1
      end
  ...
  if _ContentsGroup_Siege2024 == true then
    local bottomButton = self._ui.btn_gradeFilters[#self._ui.btn_gradeFilters]
...
function WarFilter:open()
  ...
  self:setTerritoryFilterTo(self._currentTerritoryFilter, nil)
  self:snapToColumn(self._currentColumn)
  if nil ~= Panel_Worldmap_TopMenu then
    _panel:ComputePos()
...
function InputMOn_WarFilter_TerritoryFilter(index)
  WarFilter:setTerritoryFilterTo(index, nil)
end
function WarFilter:setTerritoryFilterTo(index, dPadtype)
  if ToClient_isConsole() == true then
    ...
    elseif dPadtype == __eJoyPadInputType_DPad_Down then
      if index == 3 then
        self._currentTerritoryFilter = 4
      elseif index == 5 then
        self._currentTerritoryFilter = 6
      elseif index == 7 then
        self._currentTerritoryFilter = 1
      else
    ...
    elseif dPadtype == __eJoyPadInputType_DPad_Up then
      if index == 3 then
        self._currentTerritoryFilter = 2
      elseif index == 5 then
        self._currentTerritoryFilter = 4
      elseif index == 7 then
        self._currentTerritoryFilter = 6
...
function WarFilter:snapToColumn(index)
  ...
  self._ui.stc_selectedTerritory[self._currentTerritoryFilter]:SetShow(2 == index)
  if _ContentsGroup_Siege2024 == false then
    self._ui.stc_selectedDay[self._currentDayFilter]:SetShow(3 == index)
  end
...
function Input_WarFilter_DPad(dPadtype)
  ...
  if dPadtype == __eJoyPadInputType_DPad_Down then
    ...
    elseif 2 == self._currentColumn then
      self:setTerritoryFilterTo(self._currentTerritoryFilter + 1, dPadtype)
...
function WarFilter:initFilter()
  local filterColumn = self._COLUMN_COUNT
  self:setGradeFilterTo(1)
  self:setDayFilterTo(1)
  self:setTerritoryFilterTo(1, nil)
