  ...
  for territory_idx = 0, territoryCount - 1 do
    if OCEAN_TERRITORY_KEY ~= territory_idx then
      ...
      if nil ~= territoryKey then
        ...
        territoryDATA.Name = UI.createControl(__ePAUIControl_StaticText, statusTemplate, "StaticText_NodeWar_Name_" .. territory_idx)
        ...
        territoryDATA.IngCount = UI.createControl(__ePAUIControl_StaticText, statusTemplate, "StaticText_NodeWar_IngCount_" .. territory_idx)
        CopyBaseProperty(self._ui.txt_ingCount, territoryDATA.IngCount)
        territoryDATA.IngCount:SetShow(true)
        territoryDATA.EndCount = UI.createControl(__ePAUIControl_StaticText, statusTemplate, "StaticText_NodeWar_EndCount_" .. territory_idx)
        CopyBaseProperty(self._ui.txt_endCount, territoryDATA.EndCount)
        territoryDATA.EndCount:SetShow(true)
        territoryDATA.IngIcon = UI.createControl(__ePAUIControl_Static, statusTemplate, "Static_NodeWar_Ing_Icon_" .. territory_idx)
        ...
        templatePosY_Value = templatePosY_Value + (self._ui.stc_statusTemplate:GetSizeY() + self.line_gap)
      end
    end
  end
  self:registEventHandler()
  _panel:SetSize(_panel:GetSizeX(), self._ui.stc_contentBg:GetPosY() + templatePosY_Value + 10)
...
function PaGlobal_Worldmap_NodeWarInfo_RePosition()
  _panel:ComputePos()
  _panel:SetPosY(Panel_Worldmap_BottomMenu:GetPosY() - _panel:GetSizeY() - 10)
end
