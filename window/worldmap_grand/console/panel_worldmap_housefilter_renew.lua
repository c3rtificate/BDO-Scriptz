...
function Window_WorldMap_HouseFilterInfo:SetPurposeFilter()
  ...
  if nil == allFilter then
    allFilter = {}
    local allButtonControl = UI.createControl(__ePAUIControl_RadioButton, self._ui._static_PurposeBg, "Radiobutton_Purpose_" .. 0)
    ...
    local allIconControl = UI.createControl(__ePAUIControl_StaticText, allButtonControl, "button_Purpose_" .. 0)
    ...
    allIconControl:ChangeTextureInfoName(CppEnums.HouseIconType_ConsoleFilterTexture.path)
    local x1, y1, x2, y2 = setTextureUV_Func(allIconControl, 1, 319, 28, 346)
  ...
  for index = 1, count do
    ...
    if nil == filter then
      filter = {}
      local buttonControl = UI.createControl(__ePAUIControl_RadioButton, self._ui._static_PurposeBg, "Radiobutton_Purpose_" .. index)
      ...
      local iconControl = UI.createControl(__ePAUIControl_StaticText, buttonControl, "button_Purpose_" .. index)
    ...
    local houseIconType = ToClient_getHouseIconType(ToClient_getReceipeTypeByIndex(index - 1))
    local filterTextureInfo = CppEnums.HouseIconType_ConsoleFilterTexture[houseIconType]
    filter._icon:ChangeTextureInfoName(CppEnums.HouseIconType_ConsoleFilterTexture.path)
    local x1, y1, x2, y2 = 0, 0, 0, 0
    if filterTextureInfo ~= nil then
      x1, y1, x2, y2 = setTextureUV_Func(filter._icon, filterTextureInfo.x1, filterTextureInfo.y1, filterTextureInfo.x2, filterTextureInfo.y2)
    end
...
function Window_WorldMap_HouseFilterInfo:UpdateHouseList(isNotifyUI)
  if isNotifyUI == nil then
    isNotifyUI = true
  end
  ...
  self:SetFilterTitle()
  ToCleint_findHouseByFilter(self._currentHouseFilterIndex, self._currentPurposeFilterType, self._currentGradeFilterIndex + 1, isNotifyUI)
end
function Window_WorldMap_HouseFilterInfo:InitControl()
  ...
  self._ui._staticText_PurposeFilter:SetTextMode(__eTextMode_AutoWrap)
...
function Window_WorldMap_HouseFilterInfo:InitEvent()
  ...
  self._ui._list2_HouseList:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_WorldMap_HouseFilter_List2EventControlCreate")
  self._ui._list2_HouseList:createChildContent(__ePAUIList2ElementManagerType_List)
end
...
function PaGlobalFunc_WorldMap_HouseFilter_SelectHouseFilter(index, isNotifyUI)
  local self = Window_WorldMap_HouseFilterInfo
  self._currentHouseFilterIndex = index
  self:UpdateHouseList(isNotifyUI)
...
function PaGlobalFunc_FromClient_WorldMap_HouseFilter_RefreshHouseListFinish()
  ...
  if 0 == self._currnetHouseListIndex and true == self._isOpenFromCraftManage then
    ...
    PaGlobalFunc_WorldMap_HouseFilter_SelectHouseFilter(0, false)
  end
end
function PaGlobalFunc_FromClient_WorldMap_HouseFilter_HouseFilterOn(houseBtn)
  ...
  if nil == houseInClientWrapper or true == houseInClientWrapper:isMansion() then
    return
  end
...
function PaGlobalFunc_WorldMap_HouseFilter_List2EventControlCreate(list_content, key)
  ...
  staticText_Name:SetTextMode(__eTextMode_AutoWrap)
  ...
  if nil ~= rentHouse and true == rentHouse:isSet() then
    ...
    local houseIconType = rentHouse:getIconType()
    local filterTextureInfo = CppEnums.HouseIconType_ConsoleFilterTexture[houseIconType]
    icon:ChangeTextureInfoName(CppEnums.HouseIconType_ConsoleFilterTexture.path)
    local x1, y1, x2, y2 = 0, 0, 0, 0
    if filterTextureInfo ~= nil then
