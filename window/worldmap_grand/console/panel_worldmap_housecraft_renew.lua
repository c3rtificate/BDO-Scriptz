...
function Window_WorldMap_HouseCraftInfo:SetWorkListData()
  ...
  for index = 0, workCount - 1 do
    local esSSW = ToClient_getHouseDataWorkableItemExchangeByIndex(index)
...
function Window_WorldMap_HouseCraftInfo:SetCraftItem()
  ...
  for index = 0, self._config._slotMax - 1 do
    ...
    if nil ~= craftItemInfo then
      esSSW = ToClient_getHouseDataWorkableItemExchangeByIndex(itemIndex)
    end
    if nil ~= craftItemInfo and nil ~= esSSW then
      local slot = self._craftSlotList[index]
      if nil == slot then
        local control = UI.createControl(__ePAUIControl_RadioButton, self._ui._static_CraftItemListBg, "radioButton_craftItem_" .. index)
      ...
      slot.icon:addInputEvent("Mouse_On", "PaGlobalFunc_WorldMap_HouseCraft_ShowCraftTooltip(" .. craftItemInfo._index .. "," .. tostring(itemStaticWrapper:isMoney()) .. ")")
...
function PaGlobalFunc_WorldMap_HouseCraft_ShowCraftTooltip(craftIndex, isMoney)
  ...
  if esSSW:isSet() then
    ...
    if nil ~= isMoney and true == isMoney then
      staticStatusWrapper = esSSW:getResultItemStaticStatusWrapper()
    end
...
function Window_WorldMap_HouseCraftInfo:SetResourceItem()
  ...
  for index = 0, craftItemInfo._resourceCount - 1 do
    ...
    if nil ~= resourceInfo then
      ...
      if nil == slot then
        local control = UI.createControl(__ePAUIControl_Static, self._ui._static_InfoBg, "static_NeedItem_" .. index)
      ...
      local enchantLevel = resourceInfo._itemKey:getEnchantLevel()
      if enchantLevel > 0 then
        ...
        slot.enchantText:SetText(printenchantLevel)
      ...
      if resourceInfo._haveCount >= 1000000 then
        resourceHaveCountValue = tostring(math.floor(resourceInfo._haveCount / 1000000)) .. "." .. tostring(math.floor(resourceInfo._haveCount / 100000 - math.floor(resourceInfo._haveCount / 1000000) * 10)) .. "M"
      elseif resourceInfo._haveCount >= 1000 then
        resourceHaveCountValue = tostring(math.floor(resourceInfo._haveCount / 1000)) .. "." .. tostring(math.floor(resourceInfo._haveCount / 100 - math.floor(resourceInfo._haveCount / 1000) * 10)) .. "K"
      else
        resourceHaveCountValue = tostring(resourceInfo._haveCount)
      end
      if 1000000 <= resourceInfo._needCount then
        resourceNeedCountValue = tostring(math.floor(resourceInfo._needCount / 1000000)) .. "." .. tostring(math.floor(resourceInfo._needCount / 100000 - math.floor(resourceInfo._needCount / 1000000) * 10)) .. "M"
      elseif 1000 <= resourceInfo._needCount then
        resourceNeedCountValue = tostring(math.floor(resourceInfo._needCount / 1000)) .. "." .. tostring(math.floor(resourceInfo._needCount / 100 - math.floor(resourceInfo._needCount / 1000) * 10)) .. "K"
      else
        resourceNeedCountValue = tostring(resourceInfo._needCount)
      end
...
function Window_WorldMap_HouseCraftInfo:SetWorkerList()
  ...
  if true == esSSW:isSet() then
    ...
    local receipeKey = self._param._useType
    local waitingWorkerCount = ToClient_getHouseWaitWorkerList(self._houseInfoSS, productCategory, workableKey, receipeKey)
...
function Window_WorldMap_HouseCraftInfo:InitData(houseInfoSSWrapper, _param)
  ...
  if rentHouseWrapper ~= nil and self._ui._staticText_CraftTitle ~= nil then
    self._ui._staticText_CraftIcon:ChangeTextureInfoName(CppEnums.HouseIconType_ConsoleFilterTexture.path)
    local x1, y1, x2, y2 = 0, 0, 0, 0
    local filterTextureInfo = CppEnums.HouseIconType_ConsoleFilterTexture[rentHouseWrapper:getIconType()]
    if filterTextureInfo ~= nil then
      x1, y1, x2, y2 = setTextureUV_Func(self._ui._staticText_CraftIcon, filterTextureInfo.x1, filterTextureInfo.y1, filterTextureInfo.x2, filterTextureInfo.y2)
    end
    self._ui._staticText_CraftIcon:getBaseTexture():setUV(x1, y1, x2, y2)
    self._ui._staticText_CraftIcon:setRenderTexture(self._ui._staticText_CraftIcon:getBaseTexture())
  end
  self:SetWorkerList()
...
function Window_WorldMap_HouseCraftInfo:InitControl()
  ...
  self._ui._staticText_CraftDesc:SetTextMode(__eTextMode_AutoWrap)
  self._ui._staticText_CraftIcon = UI.getChildControl(self._ui._static_TopDescBg, "Static_Icon")
  ...
  self._ui._staticText_ItemName:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._config._workerTooltipStartPosX = Panel_Worldmap_HouseCraft:GetPosX() + Window_WorldMap_HouseCraftInfo._ui._list2_WorkerList:GetPosX()
  self._config._workerTooltipStartPosY = Panel_Worldmap_HouseCraft:GetPosY() + Window_WorldMap_HouseCraftInfo._ui._static_CenterBg:GetPosY()
end
function Window_WorldMap_HouseCraftInfo:InitEvent()
  self._ui._list2_WorkerList:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_WorldMap_HouseCraft_List2EventControlCreate")
  self._ui._list2_WorkerList:createChildContent(__ePAUIList2ElementManagerType_List)
...
function PaGlobalFunc_WorldMap_HouseCraft_List2EventControlCreate(list_content, key)
  ...
  staticText_name:SetTextMode(__eTextMode_LimitText)
  staticText_name:SetText(workerInfo._name)
  local isLimitText = staticText_name:IsLimitText()
  static_button:addInputEvent("Mouse_LUp", "PaGlobalFunc_WorldMap_HouseCraft_SelectWorker(" .. id .. ")")
  static_button:addInputEvent("Mouse_On", "PaGlobalFunc_WorldMap_HouseCraft_SetSelectButton(" .. id .. ",true," .. tostring(isLimitText) .. ")")
  static_button:addInputEvent("Mouse_Out", "PaGlobalFunc_WorldMap_HouseCraft_SetSelectButton(-1,true," .. tostring(isLimitText) .. ")")
  static_image:ChangeTextureInfoName(workerIcon)
  staticText_town:SetText(workerInfo._regionName)
  staticText_workingNameCount:SetTextMode(__eTextMode_LimitText)
...
function HandleEventOnOut_WorldMap_HouseCraft_WorkerNameTooltip(id)
  if -1 == id then
    TooltipSimple_Hide()
    return
  end
  local listControl = Window_WorldMap_HouseCraftInfo._ui._list2_WorkerList:GetContentByKey(id)
  if nil == listControl then
    return
  end
  local control = UI.getChildControl(listControl, "StaticText_WorkerTitle")
  if nil == control then
    return
  end
  local workerInfo = Window_WorldMap_HouseCraftInfo._workerList[id]
  if nil == workerInfo then
    return
  end
  local name = workerInfo._name
  local pos = {}
  pos.x = Window_WorldMap_HouseCraftInfo._config._workerTooltipStartPosX + control:GetPosX() + control:GetSizeX()
  pos.y = Window_WorldMap_HouseCraftInfo._config._workerTooltipStartPosY + listControl:GetPosY() + control:GetPosY()
  TooltipSimple_ShowSetSetPos_Console(pos, name, "")
end
function PaGlobalFunc_WorldMap_HouseCraft_SetSelectButton(id, isWorker, isLimitText)
  local self = Window_WorldMap_HouseCraftInfo
  if true == isLimitText then
    HandleEventOnOut_WorldMap_HouseCraft_WorkerNameTooltip(id)
  end
