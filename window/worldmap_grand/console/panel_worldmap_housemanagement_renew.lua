...
function WorldMapHouseManager:ResetAlign()
  ...
  self._ui.txt_tipIcon:SetTextMode(__eTextMode_None)
end
...
function WorldMapHouseManager:updateTypeList(houseInfoSSWrapper)
  ...
  for ii = 1, self._houseTypeMax do
    ...
    if ii <= self._receipeCount then
      ...
      self._useTypeData[ii].iconType = houseInfoCraftWrapper:getIconType()
      ...
      txt_name:ChangeTextureInfoName(CppEnums.HouseIconType_ConsoleFilterTexture.path)
      local filterTextureInfo = CppEnums.HouseIconType_ConsoleFilterTexture[self._useTypeData[ii].iconType]
      local x1, y1, x2, y2 = 0, 0, 0, 0
      if filterTextureInfo ~= nil then
        x1, y1, x2, y2 = setTextureUV_Func(txt_name, filterTextureInfo.x1, filterTextureInfo.y1, filterTextureInfo.x2, filterTextureInfo.y2)
      end
...
function WorldMapHouseManager:updateRentedHouse(rentHouse, houseInfoStaticStatusWrapper)
  ...
  for ii = 1, #self._useTypeData do
    if self._rentedUseType == self._useTypeData[ii].receipeKey then
      ...
      if self._ui.txt_tipIcon:GetSizeX() < self._ui.txt_tipIcon:GetTextSizeX() then
        self._ui.txt_tipIcon:SetTextMode(__eTextMode_AutoWrap)
        self._ui.txt_tipIcon:SetTextSpan(0, self._originalTextSpanY)
      else
  ...
  icon:SetShow(true)
  icon:ChangeTextureInfoName(CppEnums.HouseIconType_ConsoleFilterTexture.path)
  local filterTextureInfo = CppEnums.HouseIconType_ConsoleFilterTexture[self._useTypeData[rentedIndex].iconType]
  local x1, y1, x2, y2 = 0, 0, 0, 0
  if filterTextureInfo ~= nil then
    x1, y1, x2, y2 = setTextureUV_Func(icon, filterTextureInfo.x1, filterTextureInfo.y1, filterTextureInfo.x2, filterTextureInfo.y2)
  end
          ...
          if nil ~= data[index].itemIndexList[jj] then
            ...
            if esSSW:isSet() then
              ...
              if nil ~= staticStatusWrapper then
                ...
                self._ui.itemSlots[slotIndex].icon:addInputEvent("Mouse_On", "PaGlobalFunc_WorldMapHouseManager_ShowTooltip(" .. slotIndex .. "," .. index .. "," .. jj .. "," .. tostring(staticStatusWrapper:isMoney()) .. ")")
...
function PaGlobalFunc_WorldMapHouseManager_ShowTooltip(slotIndex, index, jj, isMoney)
  local self = WorldMapHouseManager
  ...
  if esSSW:isSet() then
    ...
    if nil ~= isMoney and true == isMoney then
      local itemStatic = esSS:getFirstDropGroup():getItemStaticStatus()
      local itemKey = esSS:getFirstDropGroup()._itemKey
      staticStatusWrapper = getItemEnchantStaticStatus(itemKey)
    end
...
function WorldMapHouseManager:updateBottomAsNormalState()
  ...
  for index = 0, listCount - 1 do
    ...
    itemCount[index] = houseInfoSSW:getNeedItemListItemCount(currentUseType, nextRentHouseLevel, index)
  end
...
function PaGlobalFunc_WorldMapHouseManager_BuyMansionLandByInteraction()
  local actorWrapper = interaction_getInteractable()
  if nil == actorWrapper then
    return
  end
  if false == actorWrapper:isMansionLand() then
    return
  end
  ...
  local itemExplain = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_HOUSECONTROL_BUYHOUSE_CONTENT", "houseName", houseName, "useTypeName", useTypeName) .. [[
...
function Input_WorldMapHouseManager_Buy()
  ...
  local nextRentHouseLevel = 1
  local receipeKey = self._useTypeData[self._selected].receipeKey
  local realIndex = SSW:getIndexByReceipeKey(receipeKey)
  ...
  local maxlevel = SSW:getLevelByKey(receipeKey)
  for index = 0, listCount - 1 do
  ...
  local function buyConfirmed()
    ToClient_BuyHouse(self._houseKey, receipeKey, self._selectGradeLevel)
  ...
  MessageBox.showMessageBox(messageboxData, "top")
  self._selectGradeLevel = nextRentHouseLevel
end
...
function Input_WorldMapHouseManager_SetUseType(index)
  ...
  self._ui.txt_manageTitle:ChangeTextureInfoName(CppEnums.HouseIconType_ConsoleFilterTexture.path)
  local filterTextureInfo = CppEnums.HouseIconType_ConsoleFilterTexture[self._useTypeData[index].iconType]
  local x1, y1, x2, y2 = 0, 0, 0, 0
  if filterTextureInfo ~= nil then
    x1, y1, x2, y2 = setTextureUV_Func(self._ui.txt_manageTitle, filterTextureInfo.x1, filterTextureInfo.y1, filterTextureInfo.x2, filterTextureInfo.y2)
  end
...
function PaGlobalFunc_WorldMapHouseManager_ChangeStateHouseContinue()
  ...
  ToClient_BuyHouse(self._houseKey, self._useTypeData[self._selected].receipeKey, self._selectGradeLevel)
end
...
function Input_WorldMapHouseManager_UpgradeOrChange()
  ...
  local nextRentHouseLevel = 1
  local selectedUseType = self._useTypeData[self._selected].receipeKey
  local maxlevel = houseInfoStaticStatusWrapper:getLevelByKey(selectedUseType)
  if selectedUseType == rentedReceipeKey and eHouseUseGroupType.Count ~= self._currentGroupType then
    nextRentHouseLevel = rentHouseWrapper:getLevel() + 1
  elseif selectedUseType ~= rentedReceipeKey and eHouseUseGroupType.Count ~= self._currentGroupType then
...
function getHouseGradeMessage(selectLevel)
  ...
  if ToClient_IsMyHouse(self._houseKey) == false then
    nextRentHouseLevel = 1
  else
    local rentHouseWrapper = ToClient_GetRentHouseWrapper(self._houseKey)
    local useType = rentHouseWrapper:getType()
    if self._useTypeData[self._selected].receipeKey == useType then
      nextRentHouseLevel = rentHouseWrapper:getLevel() + 1
    elseif self._useTypeData[self._selected].receipeKey ~= useType then
      nextRentHouseLevel = 1
    end
    isMyHouse = true
  end
  local receipeKey = self._useTypeData[self._selected].receipeKey
  local realIndex = houseInfoStaticStatusWrapper:getIndexByReceipeKey(receipeKey)
  local houseInfoCraftWrapper = houseInfoStaticStatusWrapper:getHouseCraftWrapperByIndex(realIndex)
  local listCount = houseInfoStaticStatusWrapper:getNeedAllItemListCount(receipeKey, nextRentHouseLevel, selectLevel)
  local needTime_sec = houseInfoStaticStatusWrapper:getTransperTime(receipeKey, nextRentHouseLevel, selectLevel)
  local needTime = Util.Time.timeFormatting(needTime_sec)
  local houseName = self._houseName
  local useTypeName = houseInfoCraftWrapper:getReciepeName()
  local maxlevel = houseInfoStaticStatusWrapper:getLevelByKey(receipeKey)
  local itemExplain = ""
  if isMyHouse == false then
    itemExplain = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_HOUSECONTROL_BUYHOUSE_CONTENT", "houseName", houseName, "useTypeName", useTypeName) .. [[


]] .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_HOUSECONTROL_NEEDPOINT", "needPoint", self._needExplorePoint) .. "\n"
  else
    local rentHouseWrapper = ToClient_GetRentHouseWrapper(self._houseKey)
    local rentHouseLevel = rentHouseWrapper:getLevel()
    local workingcnt = ToClient_getHouseWorkingWorkerList(houseInfoStaticStatusWrapper:get())
    realIndex = houseInfoStaticStatusWrapper:getIndexByReceipeKey(receipeKey)
    houseInfoCraftWrapper = houseInfoStaticStatusWrapper:getHouseCraftWrapperByIndex(realIndex)
    local targetUseTypeName = houseInfoCraftWrapper:getReciepeName()
    realIndex = houseInfoStaticStatusWrapper:getIndexByReceipeKey(rentHouseWrapper:getType())
    houseInfoCraftWrapper = houseInfoStaticStatusWrapper:getHouseCraftWrapperByIndex(realIndex)
    local currentUseTypeName = houseInfoCraftWrapper:getReciepeName()
    if false == ToClient_IsUsable(self._houseKey) then
      itemExplain = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_HOUSECONTROL_CHANGESTATE_CONTENT_1", "currentTypeName", currentUseTypeName, "targetTypeName", targetUseTypeName) .. [[
  ...
  for index = 0, listCount - 1 do
    local itemKey = houseInfoStaticStatusWrapper:getNeedAllItemListItemKey(receipeKey, nextRentHouseLevel, selectLevel, index)
    local itemName = getItemEnchantStaticStatus(itemKey):getName()
    local itemCount = houseInfoStaticStatusWrapper:getNeedAllItemListItemCount(receipeKey, nextRentHouseLevel, selectLevel, index)
    local needCost = makeDotMoney(Int64toInt32(itemCount)) .. " " .. itemName
    itemExplain = itemExplain .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_HOUSECONTROL_NEEDCOST", "needCost", needCost) .. "\n"
  end
  itemExplain = itemExplain .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_HOUSECONTROL_NEEDTIME", "needTime", needTime)
  self._selectGradeLevel = selectLevel
  return itemExplain
end
