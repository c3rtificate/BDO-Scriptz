...
function TerritoryTooltipInfo:init()
  ...
  self._ui.stc_guildOccupationEffect = UI.getChildControl(TerritoryTooltipInfo._ui.stc_OccupiersInfoGroup, "StaticText_OccupationEffect")
  self._ui.txt_occupyDesc = UI.getChildControl(TerritoryTooltipInfo._ui.stc_OccupiersBenefitGroup, "StaticText_OccupyDesc")
  self._ui.txt_occupyDesc:SetTextMode(__eTextMode_AutoWrap)
...
function TerritoryTooltipInfo:update_OccupiersInfoGroup(territoryInfo, territoryKeyRaw)
  ...
  self._ui.stc_guildOccupationEffect:SetShow(false)
end
function TerritoryTooltipInfo:update_OccupiersBenefitGroup(territoryKeyRaw)
  if ToClient_isConsole() == true then
    if territoryKeyRaw ~= BENEFIT_TERRITORY.MAP_BALENOS and territoryKeyRaw ~= BENEFIT_TERRITORY.MAP_CALPHEON and territoryKeyRaw ~= BENEFIT_TERRITORY.MAP_VALENCIA then
      self._ui.stc_OccupiersBenefitGroup:SetShow(false)
      self.needToUpdate = true
      return
    end
  elseif territoryKeyRaw < BENEFIT_TERRITORY.MAP_CALPHEON or territoryKeyRaw > BENEFIT_TERRITORY.MAP_VALENCIA then
    self._ui.stc_OccupiersBenefitGroup:SetShow(false)
    self.needToUpdate = true
    return
  end
  if ToClient_isConsole() == true then
    if BENEFIT_TERRITORY.MAP_BALENOS == territoryKeyRaw then
      self._ui.txt_occupyDesc:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_WORLDMAP_TERRITORYINFO_BENEFIT_CS_1"))
    elseif BENEFIT_TERRITORY.MAP_CALPHEON == territoryKeyRaw then
      self._ui.txt_occupyDesc:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_WORLDMAP_TERRITORYINFO_BENEFIT_CS_2"))
    elseif BENEFIT_TERRITORY.MAP_VALENCIA == territoryKeyRaw then
      self._ui.txt_occupyDesc:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_WORLDMAP_TERRITORYINFO_BENEFIT_CS_3"))
    end
  elseif BENEFIT_TERRITORY.MAP_CALPHEON == territoryKeyRaw then
