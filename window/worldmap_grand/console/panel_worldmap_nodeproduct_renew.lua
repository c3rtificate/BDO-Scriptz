...
function HandleEventLUp_WorldMapConsole_WorkDestination_ShowWaypointList()
  Window_WorldMap_NodeProductInfo._ui._staticText_WorkDestination:ToggleListbox()
  ToClient_padSnapRefreshTarget(Window_WorldMap_NodeProductInfo._ui._staticText_WorkDestination)
end
local function updateStorageComboboxList(workerInfo, nodeInfo)
  if Panel_Worldmap_NodeProduct:GetShow() == false then
    return
  end
  ...
  local destinationBox = self._ui._staticText_WorkDestination
  local destinationBoxText = self._ui._staticText_WorkDestinationText
  destinationBox:DeleteAllItem()
  destinationBox:SetShow(false)
  destinationBoxText:SetShow(false)
  ToClient_padSnapIgnoreGroupMoveReal()
  local isContentsOptionOpen = _ContentsGroup_WorkerLv40
  local isNormalNode = nodeInfo._isFinance == false and nodeInfo._isMonopoly == false
  if isContentsOptionOpen == false or isNormalNode == false then
    return
  end
  local waypointInfoList = ToClient_WorkerStorageGetListWaypointInfo(true)
  if waypointInfoList == nil then
    return
  end
  local size = waypointInfoList:size()
  if size <= 0 then
    return
  end
  for ii = 0, size - 1 do
    destinationBox:AddItem(waypointInfoList:atPointer(ii):getName())
  end
  destinationBox:SetShow(true)
  destinationBoxText:SetShow(true)
  if workerInfo == nil then
    destinationBox:SetSelectItemIndex(-1)
    destinationBox:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_DELIVERY_SELECT_TO_REGION"))
    ToClient_padSnapIgnoreGroupMoveReal()
    return
  end
  local workerWrapperLua = getWorkerWrapper(workerInfo._workerNo:get(), false)
  local workingRegionName = ""
  if workerWrapperLua ~= nil then
    workingRegionName = workerWrapperLua:getStorageRegionName()
  end
  local cantUseComboBox = workerInfo._level < __eWorkerPossibleLevelChangeStorage or workingRegionName ~= ""
  if workingRegionName == "" then
    workingRegionName = workerInfo._regionNameRaw
  end
  destinationBox:SetIgnore(cantUseComboBox)
  destinationBox:SetMonoTone(cantUseComboBox)
  destinationBox:SetText(workingRegionName)
  if cantUseComboBox == true then
    ToClient_padSnapIgnoreGroupMoveReal()
  else
    ToClient_padSnapAllowGroupMoveReal()
  end
end
...
function Window_WorldMap_NodeProductInfo:SetWorkerData()
  ...
  for index = 0, waitingWorkerCount - 1 do
    ...
    local workerNoRaw = npcWaitingWorker:getWorkerNo():get()
    local workerWrapperLua = getWorkerWrapper(workerNoRaw, false)
    if workerWrapperLua:getIsAuctionInsert() == false then
      ...
      local workerNoChar = firstWorkerNo:get()
      ...
      local name = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_LV") .. "." .. npcWaitingWorker:getLevel() .. " " .. workerWrapperLua:getName()
      ...
      local level = npcWaitingWorker:getLevel()
      local regionNameRaw = workerRegionWrapper:getAreaName()
      workingWorkerList[workingWorkerIndex] = {
        ...
        _level = level,
        _regionNameRaw = regionNameRaw
      }
      local nodeInfo = self._currentNodeInfo
      local workerInfo = workingWorkerList[workingWorkerIndex]
      local distance = ToClient_getCalculateMoveDistance(workerNo, nodeInfo._position) / 100
      ...
      sortElement[workingWorkerIndex] = {}
      sortElement[workingWorkerIndex]._isConnected = workerWrapperLua:isWorkingable(nodeInfo._plantKey:getWaypointKey())
      sortElement[workingWorkerIndex]._workTime = totalWorkTime
      workingWorkerIndex = workingWorkerIndex + 1
    end
  end
  local possibleWorkerCount = workingWorkerIndex
  local function newWorkerSortingFunction(sortInfoList, sortInfoListCount)
    local function workerMemorySwapper(idx, jdx)
      local temp = workingWorkerList[idx]
      workingWorkerList[idx] = workingWorkerList[jdx]
      workingWorkerList[jdx] = temp
      local temp = sortElement[idx]
      sortElement[idx] = sortElement[jdx]
      sortElement[jdx] = temp
    end
    local function selectionSort(list, beginIdx, endIdx, condition)
      for idx = beginIdx, endIdx - 1 do
        for jdx = idx + 1, endIdx do
          if condition(idx, jdx) == true then
            workerMemorySwapper(idx, jdx)
          end
        end
      end
    end
    local function isConnectedSorter(idx, jdx)
      if sortElement[idx]._isConnected == false and sortElement[jdx]._isConnected == true then
        return true
      end
      return false
    end
    local function isFasterSorter(idx, jdx)
      if sortElement[idx]._workTime > sortElement[jdx]._workTime then
        return true
      end
      return false
    end
    local function isConnectedChecker(idx)
      return sortElement[idx]._isConnected == false
    end
    local split = function(list, listCount, condition)
      local middleIdx = 0
    ...
    selectionSort(sortInfoList, 0, sortInfoListCount - 1, isConnectedSorter)
    local middleIdx = split(sortInfoList, sortInfoListCount, isConnectedChecker)
    selectionSort(sortInfoList, 0, middleIdx - 1, isFasterSorter)
    selectionSort(sortInfoList, middleIdx, sortInfoListCount - 1, isFasterSorter)
  end
  newWorkerSortingFunction(sortElement, possibleWorkerCount)
...
function Window_WorldMap_NodeProductInfo:InitControl()
  ...
  self._ui._staticText_ProductDesc:SetTextMode(__eTextMode_AutoWrap)
  self._ui._staticText_FinanceDesc = UI.getChildControl(self._ui._static_FinanceBg, "StaticText_Desc")
  self._ui._staticText_FinanceDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui._staticText_WorkDestinationText = UI.getChildControl(self._ui._static_InfoBg, "StaticText_WarehouseRegion_Text")
  self._ui._staticText_WorkDestination = UI.getChildControl(self._ui._static_InfoBg, "Combobox_WarehouseRegion")
...
function Window_WorldMap_NodeProductInfo:InitEvent()
  self._ui._list2_Worker:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_WorldMap_NodeProduct_List2EventControlCreate")
  self._ui._list2_Worker:createChildContent(__ePAUIList2ElementManagerType_List)
  ...
  self._ui._staticText_WorkDestination:addInputEvent("Mouse_LUp", "HandleEventLUp_WorldMapConsole_WorkDestination_ShowWaypointList()")
end
...
function PaGlobalFunc_FromCLient_WorldMap_NodeProduct_FindSubNode(explorationNodeInClient)
  ...
  if true ~= explorationNodeInClient:isPurchased() then
    return
  end
  ...
  for index = 0, workableCnt - 1 do
    ...
    local _isMonopoly = eSSW:get():isMonopoly()
    ...
    self._subNodeInfoList[self._subNodeCount]._isMonopoly = _isMonopoly
...
function PaGlobalFunc_WorldMap_NodeProduct_List2EventControlCreate(list_content, key)
  ...
  workerName:SetTextMode(__eTextMode_LimitText)
  ...
  workingCount:SetTextMode(__eTextMode_LimitText)
...
function Window_WorldMap_NodeProductInfo:Clear()
  ...
  self._ui._staticText_WorkDestinationText:SetShow(false)
  self._ui._staticText_WorkDestination:SetShow(false)
  ToClient_padSnapAllowGroupMoveReal()
end
function Window_WorldMap_NodeProductInfo:UpdateNodeUI(id)
  ...
  updateStorageComboboxList(workerInfo, nodeInfo)
end
...
function PaGlobalFunc_WorldMap_NodeProduct_DoWork(id)
  ...
  ToClient_padSnapSetTargetGroupForce(self._ui._list2_Worker)
  local selectedIndex = self._ui._staticText_WorkDestination:GetSelectIndex()
  local wareHouseList = ToClient_WorkerStorageGetListWaypointInfo(false)
  local targetWaypoint = -1
  if wareHouseList ~= nil and selectedIndex ~= -1 then
    targetWaypoint = wareHouseList:atPointer(selectedIndex):getWaypointKey()
  end
  local workingCount = self._currentNodeInfo._workingCount
  ToClient_requestStartPlantWorkingToNpcWorker(workerInfo._workerNo, self._currentNodeInfo._plantKey, self._currentNodeInfo._workKey, workingCount, targetWaypoint)
