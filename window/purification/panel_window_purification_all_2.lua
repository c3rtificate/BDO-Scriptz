...
function HandleEventLUp_Purification_All_SelectItem(slotNo)
  ...
  PaGlobal_Purification_All._itemSlotBg_Icon:setItem(itemWrapper)
  ...
  if nil ~= prevStatic then
    ...
    if true == isOriginalForDuel then
      if nil ~= PaGlobal_Purification_All._resultSlotBg_Icon.originalForDuelIcon then
        PaGlobal_Purification_All._resultSlotBg_Icon.originalForDuelIcon:SetShow(true)
      else
        PaGlobal_Purification_All._resultSlotBg_Icon.originalForDuelIcon:SetShow(false)
      end
    end
  end
end
function PaGlobalFunc_Purification_All_RequestPurificationAni(delta)
  ...
  if PaGlobal_Purification_All._CONST_ANI_TIME < PaGlobal_Purification_All._delta_ani_time or true == skipCheck then
    ...
    ToClient_WeakenEncantByNpc(PaGlobal_Purification_All._fromWhereType, PaGlobal_Purification_All._fromSlotIdx, CppEnums.ItemWhereType.eMoneyInventory)
...
function FromClient_Purification_All_EnchantSuccess()
  ...
  local staticStatus = itemWrapper:getStaticStatus()
  PaGlobal_Purification_All._resultSlotBg_Icon:setItem(itemWrapper)
  if true == PaGlobal_Purification_All._isConsole then
