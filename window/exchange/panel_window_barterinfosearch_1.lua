function PaGlobal_BarterInfoSearch:init()
  ...
  self._ui.txt_checkDesc = UI.getChildControl(self._ui._chk_hideComplete, "StaticText_1")
  UI.setLimitTextAndAddTooltip(self._ui.txt_checkDesc)
  if true == self._isConsole then
  ...
  self._ui._chk_hideComplete:SetEnableArea(0, 0, self._ui._chk_hideComplete:GetSizeX() + self._ui.txt_checkDesc:GetTextSizeX() + 10, self._ui._chk_hideComplete:GetSizeY())
  ...
  self._ui._chk_hasServantInventory:SetShow(false)
  self._isInit = true
end
  ...
  if nil ~= regionList then
    for ii = 0, #regionList do
      ...
      if nil ~= barterWrapper then
        if self._ui._chk_hasServantInventory:IsCheck() == true then
          if ToClient_hasBarterItemInServantInventory(barterWrapper:getFromItemEnchantKey()) == true then
            self._ui._list2_barterInfoList:getElementManager():pushKey(regionList[ii]:get())
          end
        elseif true == self._ui._chk_hideComplete:IsCheck() then
...
function PaGlobal_BarterInfoSearch:refreshSpecialBarter()
  ...
  if nil ~= specialBarterWrapper then
    ...
    self._ui._txt_specialInfo:EraseAllEffect()
    self._ui._txt_specialInfo:AddEffect("fUI_Swap_01A", true, 11, 0)
  else
    self._ui._txt_specialInfo:EraseAllEffect()
...
function PaGlobal_BarterInfoSearch:setFilter()
  ...
  if self._selectFilterIndex < __eItemGradeNormal or __eItemGradeEpic < self._selectFilterIndex then
    self._selectFilterIndex = __eItemGradeTypeCount
  end
  self:refresh()
end
function PaGlobal_BarterInfoSearch:setNextValueLimit()
  ...
  self:setValueLimit()
  ToClient_padSnapChangeToTarget(self._ui._btn_position)
end
...
function PaGlobal_BarterInfoSearch:setNextCategory()
  ...
  if __eBarterCategory_Count - 1 == oldIndex then
    oldIndex = 1
  else
    oldIndex = oldIndex + 1
  end
...
function PaGlobal_BarterInfoSearch:setCategory()
  ...
  if self._selectCategoryIndex < __eBarterCategory_NormalToLevel1 or __eBarterCategory_Count < self._selectCategoryIndex then
