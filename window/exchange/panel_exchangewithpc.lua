...
function ExchangeHideAni()
  ...
  if true == _ContentsGroup_NewUI_Inventory_All then
    PaGlobalFunc_Inventory_All_ResetExchangeData()
  else
...
function exchangePC:Init()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(exchangePC._buttonQuestion, "\"PanelExchangeWithPC\"")
  if true == _ContentsOption_CH_Help then
    exchangePC._buttonQuestion:SetShow(false)
  end
  ...
  exchangePC._staticText_LimitPrice:SetTextMode(__eTextMode_AutoWrap)
...
function Panel_ExchangePC_Update_Slot()
  ...
  exchangePC._textOtherWeight:SetText(makeWeightString(otherWeight, 1) .. " " .. PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_WEIGHT"))
  ...
  exchangePC._textMyWeight:SetText(makeWeightString(s64_myWeight, 1) .. " " .. PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_WEIGHT"))
...
function ExChangePC_UpdateFilter()
  for ii = 1, exchangePC.MAX_SLOT_COUNT do
    if true == _ContentsGroup_NewUI_Inventory_All then
      PaGlobalFunc_Inventory_All_EraseEffect_WhileExchange(ii - 1)
    else
  ...
  for ii = 1, exchangePC.MAX_SLOT_COUNT do
    ...
    if nil ~= itemWrapper then
      ...
      if true == _ContentsGroup_NewUI_Inventory_All then
        PaGlobalFunc_Inventory_All_AddEffect_WhileExchange(slotNo)
      else
...
function ExchangePC_MessageBox_ResponseConfirm()
  ...
  if not _ContentsGroup_RenewUI then
    if true == _ContentsGroup_NewUI_Equipment_All then
      if true == Panel_Window_Equipment_All:GetShow() then
        PaGlobalFunc_Equipment_All_SetShow(false)
      end
    elseif true == Panel_Equipment:GetShow() then
...
function EventTradePC_ReceiveCancel(reason)
  ...
  MessageBox.showMessageBox(messageboxData)
  if nil ~= FromClient_EnablePCExChangeButton then
    FromClient_EnablePCExChangeButton()
  end
end
...
function Panel_ExchangePC_MySlotRClick(slotIndex)
  ...
  if true == _ContentsGroup_NewUI_Inventory_All then
    PaGlobalFunc_Inventory_All_EraseEffect_WhileExchange(slotIndex)
  else
...
function Panel_ExchangePC_InteractionFromNumberWindow(inputNumber, invenSlotNo, whereType)
  if true == _ContentsGroup_NewUI_Inventory_All then
    PaGlobalFunc_Inventory_All_AddEffect_WhileExchange(invenSlotNo)
  else
