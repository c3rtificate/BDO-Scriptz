function PaGlobal_EventNotify_All:initialize()
  ...
  self._ui._stc_deco = UI.getChildControl(self._ui._txt_titleBar, "Static_TitleDeco")
...
function PaGlobal_EventNotify_All:getURL()
  ...
  elseif CppEnums.CountryType.NA_ALPHA == getGameServiceType() then
    ...
    if 1 == nationType then
      url = PAGetString(Defines.StringSheet_GAME, "LUA_EVENTNOTIFY_URL_EU_ALPHA")
    else
      url = PAGetString(Defines.StringSheet_GAME, "LUA_EVENTNOTIFY_URL_NA_ALPHA")
    end
  elseif CppEnums.CountryType.NA_REAL == getGameServiceType() then
    local nationType = getServiceNationType()
    if 1 == nationType then
      url = PAGetString(Defines.StringSheet_GAME, "LUA_EVENTNOTIFY_URL_EU")
    else
      url = PAGetString(Defines.StringSheet_GAME, "LUA_EVENTNOTIFY_URL_NA")
    end
  elseif CppEnums.CountryType.ASIA_ALPHA == getGameServiceType() then
    url = "https://asia-game-portal.blackdesert.pearlabyss.com/Event"
  elseif CppEnums.CountryType.ASIA_REAL == getGameServiceType() then
    url = "https://asia-game-portal.blackdesert.pearlabyss.com/Event"
  elseif _ContentsOption_CH_GameType == true then
    url = "https://bd.qq.com/cp/a20240715gwyqyy/latestcontent.html"
  else
    url = PAGetString(Defines.StringSheet_GAME, "LUA_EVENTNOTIFY_URL")
  end
