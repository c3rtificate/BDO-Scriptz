...
function PaGlobal_EventNotifyContent_All:getURL(eventIndex)
  ...
  elseif CppEnums.CountryType.NA_ALPHA == getGameServiceType() then
    ...
    if 1 == nationType then
      url = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_EVENTCONTENT_URL_EU_ALPHA", "index", eventIndex)
    else
      url = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_EVENTCONTENT_URL_NA_ALPHA", "index", eventIndex)
    end
  elseif CppEnums.CountryType.NA_REAL == getGameServiceType() then
    local nationType = getServiceNationType()
    if 1 == nationType then
      url = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_EVENTCONTENT_URL_EU", "index", eventIndex)
    else
      url = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_EVENTCONTENT_URL_NA", "index", eventIndex)
    end
