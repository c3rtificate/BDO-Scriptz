...
function Panel_EventNotifyContent_Initialize()
  _Web = UI.createControl(__ePAUIControl_WebControl, Panel_EventNotifyContent, "WebControl_EventNotifyContent_WebLink")
  _Web:SetShow(true)
  _Web:SetPosX(43)
  _Web:SetPosY(63)
...
function FGlobal_EventNotifyContent_Open(eventIndex)
  ...
  elseif CppEnums.CountryType.JPN_ALPHA == getGameServiceType() then
    url = PAGetString(Defines.StringSheet_GAME, "LUA_EVENTCONTENT_URL_JP_ALPHA")
  elseif CppEnums.CountryType.JPN_REAL == getGameServiceType() then
    url = PAGetString(Defines.StringSheet_GAME, "LUA_EVENTCONTENT_URL_JP")
  elseif CppEnums.CountryType.ASIA_ALPHA == getGameServiceType() then
    url = "https://asia-game-portal.blackdesert.pearlabyss.com/Viewr?id=" .. tostring(eventIndex)
  elseif CppEnums.CountryType.ASIA_REAL == getGameServiceType() then
    url = "https://asia-game-portal.blackdesert.pearlabyss.com/Viewr?id=" .. tostring(eventIndex)
  else
    url = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_EVENTCONTENT_URL", "index", eventIndex)
  end
  local isType = false
  if isGameTypeTaiwan() or isGameTypeTR() or isGameTypeTH() or isGameTypeID() or isGameTypeRussia() or isGameTypeASIA() then
    isType = true
  else
    isType = false
  end
...
function FGlobal_EventNotifyContent_OpenByUrl(urlValue)
  ...
  _AudioPostEvent_SystemUiForXBOX(13, 6)
  local isType = false
  if isGameTypeTaiwan() or isGameTypeTR() or isGameTypeTH() or isGameTypeASIA() or isGameTypeID() or isGameTypeRussia() or isGameTypeEnglish() then
    isType = true
  else
    isType = false
  end
