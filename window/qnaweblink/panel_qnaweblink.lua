...
function Panel_QnAWebLink_Initialize()
  _qnaWeb = UI.createControl(__ePAUIControl_WebControl, Panel_QnAWebLink, "WebControl_QnAWebLink")
...
function FGlobal_QnAWebLink_Open()
  if isGameTypeGT() then
    ToClient_OpenChargeWebPage(PAGetString(Defines.StringSheet_GAME, "LUA_CS_WEBLINK_URL"), false)
  elseif true == isKorea and true == isNaver then
  ...
  elseif isGameTypeJapan() then
    ToClient_OpenChargeWebPage(PAGetString(Defines.StringSheet_GAME, "LUA_QNAWEBLINK_URL_JP_REAL"), false)
  elseif isGameTypeRussia() then
    ToClient_OpenChargeWebPage(PAGetString(Defines.StringSheet_GAME, "LUA_QNAWEBLINK_URL_RU_REAL"), false)
  elseif isGameTypeEnglish() then
    ToClient_OpenChargeWebPage(PAGetString(Defines.StringSheet_GAME, "LUA_ONEONONE_QNA_NA"), false)
  elseif isGameTypeASIA() then
    local tag = ToClient_getURLAlphaTag()
    if tag == "alpha-" then
      tag = "alphaqa-"
    end
    local url = "https://" .. tag .. "support.pearlabyss.com/Support/Start/Index/asia"
    ToClient_OpenChargeWebPage(url, false)
  elseif _ContentsOption_CH_GameType == true then
    PaGlobal_URL_OpenChCsCenter(Defines.ChCsCenterOpenType.CsCenterOpen_Esc)
  else
    ToClient_OpenChargeWebPage(PAGetString(Defines.StringSheet_GAME, "LUA_QNAWEBLINK_URL"), false)
  end
end
