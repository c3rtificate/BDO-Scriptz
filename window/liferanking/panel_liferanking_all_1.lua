function PaGlobal_LifeRanking_All:initialize()
  ...
  if true == _ContentsGroup_ChinaFontColor then
    txtGradeA:SetFontColor(Defines.Color.C_FFFF9A38)
  end
  ...
  if _ContentsGroup_MusicUpgrade2023 == true then
    defaultPosY = rankTitle:GetPosY() - 10
  end
  ...
  PaGlobal_LifeRanking_All._ui.stc_emptyList = UI.getChildControl(PaGlobal_LifeRanking_All._ui.stc_ListTitleBG, "StaticText_NoStatic")
  ...
  PaGlobal_LifeRanking_All._ui.txt_KeyGuide_LT_X = UI.getChildControl(PaGlobal_LifeRanking_All._ui.stc_BottomBG, "StaticText_LT_X_ConsoleUI")
  ...
  PaGlobal_LifeRanking_All._ui.txt_KeyGuide_LT_X:SetShow(_ContentsGroup_RenewUI)
  PaGlobal_LifeRanking_All._keyGuideGroup = {
    PaGlobal_LifeRanking_All._ui.txt_KeyGuide_LT_X,
  ...
  if true == _ContentsGroup_ChinaFontColor then
    self._ui.txt_Comment:SetFontColor(Defines.Color.C_FFFF9A38)
    self._gradeFontColor = {
      A = Defines.Color.C_FFFF9A38,
...
function PaGlobal_LifeRanking_All:registEventHandler()
  ...
  registerEvent("FromClient_UpdateGuildCacheData", "FromClient_LifeRanking_Update_GuildCacheData")
...
function PaGlobal_LifeRanking_All:update()
  ...
  local currentTabListCount = Int64toInt32(PaGlobal_LifeRanking_All._ui.list2_Ranking:getElementManager():getSize())
  if currentTabListCount == 0 then
    PaGlobal_LifeRanking_All._ui.stc_emptyList:SetShow(true)
  else
    PaGlobal_LifeRanking_All._ui.stc_emptyList:SetShow(false)
  end
end
...
function PaGlobal_LifeRanking_All:updateCurrentTabGuildData(targetGuildNo)
  if nil == Panel_LifeRanking_All then
    return
  end
  if false == Panel_LifeRanking_All:GetShow() then
  ...
  local currentTabListCount = Int64toInt32(PaGlobal_LifeRanking_All._ui.list2_Ranking:getElementManager():getSize())
  for index = 0, currentTabListCount - 1 do
    if nil ~= PaGlobal_LifeRanking_All._rankerData[index] then
      local rankerGuildNo = PaGlobal_LifeRanking_All._rankerData[index]._guildNo
      if nil ~= rankerGuildNo and true == rankerGuildNo:isDefined() and rankerGuildNo:get() == targetGuildNo:get() then
        local contents = PaGlobal_LifeRanking_All._ui.list2_Ranking:GetContentByKey(toInt64(0, index))
        if nil ~= contents then
          local guildNameText = UI.getChildControl(contents, "StaticText_GuildName")
          local lifeRanker = PaGlobal_LifeRanking_All:GetLifeRankerAt(index)
          if nil ~= lifeRanker then
            guildNameText:SetText(lifeRanker:getGuildName())
          end
        end
      end
    end
...
function PaGlobal_LifeRanking_All:updateCurrentTab()
  ...
  elseif self._eType._MUSIC == PaGlobal_LifeRanking_All._selectedTabIdx then
    PaGlobal_LifeRanking_All._listCount = ToClient_GetContentsRankCount(4)
    PaGlobal_LifeRanking_All._ui.txt_RankTitle:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_LIFERANKING_TAB_MUSIC"))
  end
...
function PaGlobal_LifeRanking_All:RequestLifeRank(tab)
  ...
  elseif self._eType._MUSIC == tab then
    ToClient_RequestContentsRank(4)
  end
end
function PaGlobal_LifeRanking_All:GetMyLifeRank(tab)
  ...
  elseif self._eType._MUSIC == tab then
    myRank = ToClient_GetContentsMyRank(4)
  end
  return myRank
end
function PaGlobal_LifeRanking_All:GetUserCount(tab)
  ...
  elseif self._eType._MUSIC == tab then
    userCount = ToClient_GetContentsRankUserCount(4)
  end
  return userCount
end
function PaGlobal_LifeRanking_All:GetLifeRankerAt(listIdx)
  ...
  elseif self._eType._MUSIC == PaGlobal_LifeRanking_All._selectedTabIdx then
    lifeRanker = ToClient_GetMusicRankAt(listIdx)
  end
  return lifeRanker
end
function PaGlobal_LifeRanking_All:GetLifeRankerLv(lifeRanker, tab)
  ...
  elseif self._eType._MUSIC == tab then
    lifeRankerLv = lifeRanker:getMusicLevel()
  else
...
function PaGlobal_LifeRanking_All:showFullNameTooltip(index)
  ...
  if self._isFullNameTooltipShow == true then
    self._isFullNameTooltipShow = false
    TooltipSimple_Hide()
    return
  end
  self._isFullNameTooltipShow = true
  local uiControl = PaGlobal_LifeRanking_All._ui.list2_Ranking:GetContentByKey(toInt64(0, index))
  local name = PaGlobal_LifeRanking_All._rankerData[index]._name
  local charName = PaGlobal_LifeRanking_All._rankerData[index]._charName
  local gamerTag = PaGlobal_LifeRanking_All._rankerData[index]._gamerTag
  local familyStr = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_PS5_ONLINEID_TOOLTIP_FAMILYNAME", "familyName", name)
  local charStr = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_PS5_ONLINEID_TOOLTIP_CHARACTERNAME", "characterName", charName)
...
function PaGlobal_LifeRanking_All:hideFullNameTooltip()
  if self == nil or index == nil then
    return
  end
  if self._isConsole == false or self._isFullNameToolTipShow == false then
    return
  end
  self._isFullNameTooltipShow = false
  TooltipSimple_Hide()
...
function PaGlobal_LifeRanking_All_CreateRankingList(control, listIdx64)
  ...
  PaGlobal_LifeRanking_All._rankerData[listIdx]._guildNo = lifeRanker:getGuildNo()
  ...
  elseif PaGlobal_LifeRanking_All._eType._PVP == PaGlobal_LifeRanking_All._selectedTabIdx then
    gradeText:SetText(lifeRankerLv)
  elseif PaGlobal_LifeRanking_All._eType._MUSIC == PaGlobal_LifeRanking_All._selectedTabIdx then
    local musicRatio = lifeRanker:getMusicExperienceRatio()
    local ratioString = string.format("%.2f", musicRatio)
    local musicString = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_LIFERANKING_MUSIC_EXP", "MusicGrade", lifeRanker:getMusicRank(), "MusicGradeExp", ratioString)
    gradeText:SetText(musicString)
  end
  ...
  if false == PaGlobal_LifeRanking_All._isPadSnap then
    ...
    control:registerPadEvent(__eConsoleUIPadEvent_LTPress_X, "PaGlobal_LifeRanking_All:showFullNameTooltip( " .. listIdx .. ")")
...
function FGlobal_LifeRanking_CheckEnAble(rankType)
  ...
  elseif self.tab_Music == rankType then
    returnValue = _ContentsGroup_MusicUpgrade2023
  end
  return returnValue
end
