...
join._ruleContent:SetTextMode(__eTextMode_AutoWrap)
join._rewardContent:SetTextMode(__eTextMode_AutoWrap)
local localWar = {
...
function FromClient_JoinLocalWar(teamNo)
  ...
  if __eInstanceLocalWar_PapuCrio == ToClient_getJoinLocalWarMapKey() then
    if 1 == teamNo then
      teamName = PAGetString(Defines.StringSheet_GAME, "LUA_LOCALWAR_BELONG_5")
    else
      teamName = PAGetString(Defines.StringSheet_GAME, "LUA_LOCALWAR_BELONG_4")
    end
  ...
  if false == dialogShow then
    FGlobal_NewLocalWar_Show()
  else
    Panel_Join_Close()
  end
  if nil ~= NewLocalWar_Update then
    NewLocalWar_Update()
  end
  if nil ~= LocalwarReloadUI then
    LocalwarReloadUI()
  end
end
