...
function PaGlobal_HorseRacing_Waiting:prepareOpen()
  ...
  if __eHorseRacing_WaitTime == state then
    ...
    self._ui.stc_infoESCDesc:SetShow(true)
  elseif __eHorseRacing_PrepareTime == state then
    ...
    self._waitForStartDesc = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_HORSERACING_WAITING_INFO_02")
    self._ui.stc_infoESCDesc:SetShow(false)
  else
