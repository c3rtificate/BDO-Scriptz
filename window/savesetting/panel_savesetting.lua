...
function PaGlobal_SaveSetting:initialize()
  ...
  local panelSizeY = 415
  self._webSizeY = 345
  if true == _ContentsGroup_SettingSaveLoad then
    panelSizeY = 650
    self._webSizeY = 580
  end
  Panel_SaveSetting:SetSize(Panel_SaveSetting:GetSizeX(), panelSizeY)
...
function PaGlobal_Panel_SaveSetting_Show(ignoreRollBack)
  ...
  if nil ~= url then
    ...
    PaGlobal_SaveSetting._ui._Web:SetUrl(PaGlobal_SaveSetting._webSizeX, PaGlobal_SaveSetting._webSizeY, realUrl, false, true)
  end
end
...
function PaGlobal_Panel_SaveSetting_GetShow()
  if Panel_SaveSetting == nil then
    return false
  end
  return Panel_SaveSetting:GetShow()
end
