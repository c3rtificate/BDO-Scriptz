...
function PaGlobal_ExitTrayConfirm_All:prepareOpen(openType)
  ...
  if nil ~= ToClient_getGameUIManagerWrapper() then
    local isTrayCheck = ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(__eMinimizeTray)
    self._ui.chk_tray:SetCheck(isTrayCheck)
  end
  if openType == self._openType.EXIT then
    self._ui.chk_tray:SetShow(false)
    self._ui.txt_trayHelp:SetShow(false)
    self._ui.btn_confirm:addInputEvent("Mouse_LUp", "HandleEventLUp_ExitTrayConfirm_All_ConfirmExit()")
    ...
    PaGlobal_ExitTrayConfirm_All._ui.txt_gameExit:SetText(PAGetString(Defines.StringSheet_GAME, "GAMEEXIT_TEXT_COMMENT_FINAL_CHECK"))
    PaGlobal_ExitTrayConfirm_All._ui.txt_gameExit:SetPosY(self._exitOriginPosY + Panel_Window_TrayConfirm_All:GetSizeY() / 4)
  else
    self._ui.chk_tray:SetShow(true)
    self._ui.txt_trayHelp:SetShow(true)
    ...
    PaGlobal_ExitTrayConfirm_All._ui.txt_gameExit:SetPosY(self._exitOriginPosY)
  end
...
function PaGlobal_ExitTrayConfirm_All:prepareClose()
  ...
  if nil ~= ToClient_getGameUIManagerWrapper() then
    local isTrayCheck = self._ui.chk_tray:IsCheck()
    ToClient_getGameUIManagerWrapper():setLuaCacheDataListBool(__eMinimizeTray, isTrayCheck, CppEnums.VariableStorageType.eVariableStorageType_User)
  end
...
function PaGlobal_ExitTrayConfirm_All:resize()
  ...
  self._exitOriginPosY = self._ui.txt_gameExit:GetPosY()
end
