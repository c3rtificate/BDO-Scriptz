function PaGlobal_ExitConfirm_All:initialize()
  ...
  self._ui._btn_buyItem = UI.getChildControl(Panel_ExitConfirm, "Button_BuyItem")
  self._ui._stc_console_keyGuide = UI.getChildControl(Panel_ExitConfirm, "Static_Console_KeyGuide")
  self._ui._stc_consoleB = UI.getChildControl(self._ui._stc_console_keyGuide, "StaticText_Console_A")
  self._ui._stc_consoleA = UI.getChildControl(self._ui._stc_console_keyGuide, "StaticText_Console_B")
  ...
  if ToClient_isUsePadSnapping() == true then
    self._ui._stc_console_keyGuide:SetShow(true)
    PaGlobal_ExitConfirm_All:alignKeyGuide()
  else
    self._ui._stc_console_keyGuide:SetShow(false)
  end
...
function PaGlobal_ExitConfirm_All:registEventHandler()
  ...
  self._ui._btn_buyItem:addInputEvent("Mouse_LUp", "HandleEventLUp_ExitConfirm_All_BuyItem()")
end
function PaGlobal_ExitConfirm_All:prepareOpen()
  ...
  Panel_ExitConfirm:ignorePadSnapMoveToOtherPanel()
...
function PaGlobal_ExitConfirm_All:open()
  ...
  if ToClient_isUsePadSnapping() == true then
    ToClient_padSnapChangeToTarget(self._ui._btn_confirm)
  end
end
    ...
    if nil ~= dailyStampKeys[daliystampIndex] then
      ...
      if totalDayCount > attantCount then
        ...
        if nil ~= itemWrapper then
          ...
          self._dayControl[slotIndex]._itemName:SetTextMode(__eTextMode_AutoWrap)
...
function PaGlobal_ExitConfirm_All:alignKeyGuide()
  if ToClient_isUsePadSnapping() == false then
    return
  end
  local keyGuideList = Array.new()
  keyGuideList:push_back(self._ui._stc_consoleB)
  keyGuideList:push_back(self._ui._stc_consoleA)
  for key, control in pairs(keyGuideList) do
    if control ~= nil then
      control:SetShow(true)
    end
  end
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyGuideList, self._ui._stc_console_keyGuide, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHT)
end
