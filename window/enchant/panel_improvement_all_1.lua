...
function PaGlobal_Improvement_All:controlAll_Init()
  ...
  local stc_tabBg = UI.getChildControl(Panel_Window_Improvement_All, "Static_MainTabTypeBg")
  ...
  self._ui.rdo_tabGoonWang:SetShow(_ContentsGroup_ItemSkillOption)
  if _ContentsGroup_ItemSkillOption == false then
    stc_tabBg:SetShow(false)
  end
  local tablSelectBar = UI.getChildControl(stc_tabBg, "Static_SelectBar")
  self._ui.rdo_tabImprove:SetIgnore(true)
  tablSelectBar:SetSpanSize(self._ui.rdo_tabImprove:GetSpanSize().x, self._ui.rdo_tabImprove:GetSpanSize().y)
  ...
  local displayCount = ToClient_GetImprovementSourceItemCount()
  for index = 0, displayCount - 1 do
    self._materialItemCount = self._materialItemCount + 1
    self._useMaterialItemKey[self._materialItemCount] = ToClient_GetDefaultDisplayImprovementSourceItemByIndex(index)
  end
end
...
function PaGlobal_Improvement_All:prepareOpen()
  ...
  self._ui.rdo_tabImprove:SetCheck(true)
  self._ui.rdo_tabGoonWang:SetCheck(false)
...
function PaGlobal_Improvement_All:registEventHandler()
  ...
  if true == self._isConsole then
    ...
    if _ContentsGroup_ItemSkillOption == true then
      Panel_Window_Improvement_All:registerPadEvent(__eConsoleUIPadEvent_Up_RB, "PaGlobalFunc_Improvement_All_tabClick(1)")
    end
  else
    if _ContentsGroup_ItemSkillOption == true then
      self._ui.rdo_tabGoonWang:addInputEvent("Mouse_LUp", "PaGlobalFunc_Improvement_All_tabClick(1)")
    end
...
function PaGlobal_Improvement_All:updateEquipList()
  ...
  for invenIdx = 0, invenSize - 1 do
    slotNo = selfPlayer:getRealInventorySlotNoNew(0, invenIdx)
    local itemWrapper = getInventoryItemByType(self._whereType, slotNo)
    if itemWrapper ~= nil and true == self:filterImproveEquipItem(slotNo, itemWrapper, self._whereType) then
      local isPush = true
      local useType = itemWrapper:getStaticStatus():getSeasonItemType()
      if useType == __eSeasonItemTypeUseSeasonCharacter then
        isPush = ToClient_Inventory_MaxEnchantLevelCheck(self._whereType, slotNo, false)
      end
      if isPush == true then
  ...
  for equipIdx = __eEquipSlotNoRightHand, __eEquipSlotNoCount do
    ...
    if nil ~= itemWrapper and true == self:filterImproveEquipItem(equipIdx, itemWrapper, self._whereType, true) then
      ...
      local useType = itemWrapper:getStaticStatus():getSeasonItemType()
      if useType == __eSeasonItemTypeUseSeasonCharacter then
        isPush = ToClient_Inventory_MaxEnchantLevelCheck(self._whereType, equipIdx, true)
      end
      if isPush == true then
        self._equipListCount = self._equipListCount + 1
