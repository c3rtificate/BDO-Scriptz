...
function PaGlobal_StackStorage_All_UpdatePerFrame(deltaTime)
  ...
  if 1 > PaGlobal_StackStorage_All._aniTime then
    ...
    if true == isRight then
      ...
      PaGlobal_StackStorage_All._startSlotNo = PaGlobal_StackStorage_All._startSlotNo - 4
      PaGlobal_StackStorage_All._currentPageNo = PaGlobal_StackStorage_All._currentPageNo - 1
    else
      for index = 0, endIndex do
        local slot = PaGlobal_StackStorage_All._ui._slot[index]
        local temp
...
function HandleEventScroll_StackStorage_All_ChangeStartSlot(isUp)
  ...
  elseif nil ~= slotSize then
    local endSlotNo = PaGlobal_StackStorage_All._startSlotNo + PaGlobal_StackStorage_All._defaultCount
  ...
  if true == isChanged then
    ...
    for index = 1, 2 do
      ...
      if nil ~= slot then
        ...
        slot.bg:ComputePosAllChild()
        slot.bg:SetAlphaExtraChild(0)
        if slotNo >= 0 and slotSize >= slotNo then
          local failCount = ToClient_GetSlotFailCount(slotNo)
          local valksCount = ToClient_GetSlotValksCount(slotNo)
          local bonusStackCount = getEnchantInformation():ToClient_getBonusStackCount()
          local totalCount = failCount + valksCount + bonusStackCount
...
function HandleEventLUp_StackStorage_All_PageChange(pageIdx)
  if nil == Panel_Widget_StackStorage_All then
    return
  end
  if false == Panel_Widget_StackStorage_All:GetShow() then
    return
  end
  if pageIdx < 0 or pageIdx > PaGlobal_StackStorage_All._maxPageCount then
    return
  end
  PaGlobal_StackStorage_All._currentPageNo = pageIdx
  PaGlobal_StackStorage_All._startSlotNo = PaGlobal_StackStorage_All._defaultCount * (pageIdx - 1)
  PaGlobalFunc_StackStorage_All_Update()
end
