...
function HandleEventLUp_AddStack_All_ClickedItem(itemType)
  ...
  PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType = itemType
  PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemKey = PaGlobal_AddStack_All._STACK_ITEM_KEY[PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType]
  PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemWhereType = PaGlobal_AddStack_All._ITEM_WHERETYPE[PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType]
  if PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType == PaGlobal_AddStack_All._STACK_ITEM_TYPE._ITEM_TYPE_VALKS or PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType == PaGlobal_AddStack_All._STACK_ITEM_TYPE._ITEM_TYPE_VALKS_EVENT then
    PaGlobal_AddStack_All_ShowNumPadForValks()
  end
  PaGloabl_AddStack_All_ChangeInvenTab(PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemWhereType)
...
function PaGlobal_AddStack_All_UpdateSelectList(contents, key)
  ...
  if PaGlobal_AddStack_All._STACK_ITEM_TYPE._ITEM_TYPE_BLACKSTONE_WEAPON == PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType or PaGlobal_AddStack_All._STACK_ITEM_TYPE._ITEM_TYPE_BLACKSTONE_ARMO == PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType or PaGlobal_AddStack_All._STACK_ITEM_TYPE._ITEM_TYPE_DESPAIRCRYSTAL == PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType then
    PaGlobal_AddStack_All_UpdateItemToFailCountList_BlackStone(contents, key)
  elseif PaGlobal_AddStack_All._STACK_ITEM_TYPE._ITEM_TYPE_VALKS_SCROLL == PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType then
    PaGlobal_AddStack_All_UpdateItemToFailCountList_ValksScroll(contents, key)
  elseif PaGlobal_AddStack_All._STACK_ITEM_TYPE._ITEM_TYPE_CONSUME == PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType then
...
function PaGlobal_AddStack_All_UpdateItemToFailCountList_BlackStone(contents, key)
  ...
  local blackStoneItemKey = ItemEnchantKey(PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemKey)
  ...
  local currentItemCount = PaGlobal_AddStack_All:getItemCount(CppEnums.ItemWhereType.eInventory, PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemKey)
  ...
  if itemCount > currentItemCount then
    ...
    itemButtonControl:addInputEvent("Mouse_LUp", "HandleEventLUp_AddStack_All_SelectListBlackStone(" .. tostring(idx) .. "," .. tostring(PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemKey) .. ")")
...
function PaGlobal_AddStack_All_UpdateItemToFailCountList_ValksScroll(contents, key)
  ...
  SlotItem.reInclude(slot, "SelectListSlotIcon_" .. 0, 0, itemSlotControl, PaGlobal_AddStack_All._INVENTORY_SLOT_CONFIG)
  slot:clearItem()
  local itemCount = PaGlobal_AddStack_All:getItemCount(PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemWhereType, itemEnchantKey)
  slot:setItemByStaticStatus(itemEnchantSSW, itemCount)
  slot.icon:addInputEvent("Mouse_On", "")
  slot.icon:addInputEvent("Mouse_Out", "")
  itemButtonControl:SetCheck(false)
  ...
  if totalCount > 0 then
    ...
    if triedStack * 5 <= tonumber(stackCount + valksCount + bonusCount) and tonumber(stackCount + valksCount + bonusCount) <= triedStack * 5 + 4 then
      mostStack:SetShow(true)
    end
  end
  if true == _ContentsGroup_UsePadSnapping then
    itemButtonControl:registerPadEvent(__eConsoleUIPadEvent_Up_X, "")
...
function HandleEventLUp_AddStack_All_SelectListBlackStone(idx, itemKey)
  ...
  if PaGlobal_AddStack_All._STACK_ITEM_TYPE._ITEM_TYPE_BLACKSTONE_WEAPON == PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType or PaGlobal_AddStack_All._STACK_ITEM_TYPE._ITEM_TYPE_BLACKSTONE_ARMO == PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType or PaGlobal_AddStack_All._STACK_ITEM_TYPE._ITEM_TYPE_DESPAIRCRYSTAL == PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType then
    if nil ~= itemKey and 0 ~= itemKey then
      PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemKey = itemKey
    end
    local itemToFailCountSSW = getItemToFailCountStaticStatus(ItemEnchantKey(PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemKey, 0))
    if nil == itemToFailCountSSW then
      return
    end
    local itemCount = itemToFailCountSSW:getItemCount(idx)
    local stackCount = itemToFailCountSSW:getStackCount(idx)
    PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentIndex = idx
    PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemWhereType = PaGlobal_AddStack_All._ITEM_WHERETYPE[PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType]
    PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemCount = itemCount
    PaGloabl_AddStack_All_ChangeInvenTab(PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemWhereType)
    PaGlobal_AddStack_All:setStackRate(stackCount, 0)
  end
  PaGlobal_AddStack_All._ui._list2_selectList:moveTopIndex()
  PaGlobal_AddStack_All:setApplyButton()
  PaGlobal_AddStack_All:setCheckItemButton(PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType)
  PaGloabl_AddStack_All_CloseSelectList()
end
function HandleEventLUp_AddStack_All_SelectListValksScroll(itemKey)
  ...
  PaGlobal_AddStack_All:clearControlText()
  if PaGlobal_AddStack_All._STACK_ITEM_TYPE._ITEM_TYPE_VALKS_SCROLL == PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType then
    if nil ~= itemKey and 0 ~= itemKey then
      PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemKey = itemKey
    end
    ...
    PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemWhereType = PaGlobal_AddStack_All._ITEM_WHERETYPE[PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType]
    PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemCount = 1
    PaGloabl_AddStack_All_ChangeInvenTab(PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemWhereType)
    PaGlobal_AddStack_All:setStackRate(stackCount, 0)
  end
  PaGlobal_AddStack_All._ui._list2_selectList:moveTopIndex()
...
function HandleEventLUp_AddStack_All_SelectListConsume(slotNo)
  ...
  PaGlobal_AddStack_All:clearControlText()
  if PaGlobal_AddStack_All._STACK_ITEM_TYPE._ITEM_TYPE_CONSUME == PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType then
    PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemSlotNo = slotNo
    PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemWhereType = PaGlobal_AddStack_All._ITEM_WHERETYPE[PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemType]
    PaGloabl_AddStack_All_ChangeInvenTab(PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemWhereType)
...
function PaGlobal_AddStack_All_SetNumpadValksItemCount(inputNum)
  ...
  PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemCount = Int64toInt32(inputNum)
  PaGlobal_AddStack_All:setStackRate(0, PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemCount)
  PaGlobal_AddStack_All._ui._stc_stackValue:SetShow(true)
  PaGlobal_AddStack_All._ui._stc_stackValue:SetText("+" .. tostring(PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemCount))
  PaGlobal_AddStack_All:setApplyButton()
end
...
function PaGlobal_AddStack_All_ShowNumPadForValks()
  ...
  local currentItemCount = PaGlobal_AddStack_All:getItemCount(PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemWhereType, PaGlobal_AddStack_All._currentUiInfo[PaGlobal_AddStack_All._currentBgType]._currentItemKey)
