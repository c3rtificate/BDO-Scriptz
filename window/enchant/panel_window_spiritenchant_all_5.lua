...
function PaGlobal_SpiritEnchant_All:SetFailCountToItem()
  local itemkey = ToClient_GetFailCountToItem()
  local invenitemKey = ToClient_GetFailCountToItemInInven()
  local cashitemkey = ToClient_GetFailCountToItemInCashInven()
  local failCount = self._enchantInfo:ToClient_getFailCount()
  if failCount == 0 then
    return
  end
  if 0 == invenitemKey and 0 == cashitemkey then
    local itemSSW = getItemEnchantStaticStatus(ItemEnchantKey(itemkey))
    if nil ~= itemSSW then
      self._slotBookItem:setItemByStaticStatus(itemSSW)
      self._slotBookItem.icon:addInputEvent("Mouse_On", "HandleEventOnOut_SpiritEnchant_All_FailCountToItemToolTip(true)")
      self._slotBookItem.icon:addInputEvent("Mouse_Out", "HandleEventOnOut_SpiritEnchant_All_FailCountToItemToolTip(false)")
      self._slotBookItem.icon:addInputEvent("Mouse_RUp", "")
      self._slotBookItem.icon:SetMonoTone(true)
      ...
      if true == self._failCountExtractEnable then
        ...
        if nil ~= balsksItemSSW then
          ...
          self._slotValksItem.icon:SetMonoTone(true)
          self._slotValksItem:setItemByStaticStatus(balsksItemSSW, 1)
        end
      end
    end
    self._failCountToItemKeyEx = itemkey
  elseif 0 == cashitemkey then
    local slotNo = ToClient_InventoryGetSlotNoByType(CppEnums.ItemWhereType.eInventory, ItemEnchantKey(invenitemKey))
    local itemWrapper = getInventoryItemByType(CppEnums.ItemWhereType.eInventory, slotNo)
    if nil ~= itemWrapper then
      PaGlobalFunc_SpiritEnchant_All_RClickForEnchantTargetItem(slotNo, itemWrapper, itemWrapper:getCount(), CppEnums.ItemWhereType.eInventory)
    end
    self._failCountToItemKeyEx = invenitemKey
  elseif 0 == invenitemKey then
    local slotNo = ToClient_InventoryGetSlotNoByType(CppEnums.ItemWhereType.eCashInventory, ItemEnchantKey(cashitemkey))
    local itemWrapper = getInventoryItemByType(CppEnums.ItemWhereType.eCashInventory, slotNo)
    if nil ~= itemWrapper then
      PaGlobalFunc_SpiritEnchant_All_RClickForEnchantTargetItem(slotNo, itemWrapper, itemWrapper:getCount(), CppEnums.ItemWhereType.eCashInventory)
    end
    self._failCountToItemKeyEx = cashitemkey
  else
    local invenslotNo = ToClient_InventoryGetSlotNoByType(CppEnums.ItemWhereType.eInventory, ItemEnchantKey(invenitemKey))
    local itemWrapper = getInventoryItemByType(CppEnums.ItemWhereType.eInventory, invenslotNo)
    local invenContentsParam = 0
    local cashContentsParam = 0
    if nil == itemWrapper then
      return
    end
    if nil ~= itemWrapper:getStaticStatus() then
      invenContentsParam = itemWrapper:getStaticStatus():getContentsEventParam1()
    end
    local cashslotNo = ToClient_InventoryGetSlotNoByType(CppEnums.ItemWhereType.eCashInventory, ItemEnchantKey(cashitemkey))
    itemWrapper = getInventoryItemByType(CppEnums.ItemWhereType.eCashInventory, cashslotNo)
    if nil == itemWrapper then
      return
    end
    if nil ~= itemWrapper:getStaticStatus() then
      cashContentsParam = itemWrapper:getStaticStatus():getContentsEventParam1()
    end
    if invenContentsParam <= cashContentsParam then
      itemWrapper = getInventoryItemByType(CppEnums.ItemWhereType.eInventory, invenslotNo)
      PaGlobalFunc_SpiritEnchant_All_RClickForEnchantTargetItem(invenslotNo, itemWrapper, itemWrapper:getCount(), CppEnums.ItemWhereType.eInventory)
      self._failCountToItemKeyEx = invenitemKey
    else
      itemWrapper = getInventoryItemByType(CppEnums.ItemWhereType.eCashInventory, cashslotNo)
      PaGlobalFunc_SpiritEnchant_All_RClickForEnchantTargetItem(cashslotNo, itemWrapper, itemWrapper:getCount(), CppEnums.ItemWhereType.eCashInventory)
      self._failCountToItemKeyEx = cashitemkey
    end
  end
end
function PaGlobal_SpiritEnchant_All:clearBookSlot()
  ...
  self._failCountToItemKeyEx = nil
  if true == self._isConsole then
    Panel_Window_StackExtraction_All:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "HandlePadEventY_SpiritEnchant_All_BookExtractionApplyButton()")
  end
end
...
function PaGlobal_SpiritEnchant_All:bookExtractionApplyButton()
  ...
  local failCount = getSelfPlayer():get():getEnchantFailCount()
  local itemName = getItemName(itemWrapper:getStaticStatus():get())
  local messageBoxMemo = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_ENCHANTCOUNTEXTRACTION_MSGDESC", "itemName", itemName, "failCount", failCount, "failCount2", failCount)
