...
function PaGlobalFunc_Improvement_All_tabClick(index)
...
function HandleEventOnOut_Improvement_All_ShowToolTip(slotIdx, isShow, isEquip)
  ...
  if false == _ContentsGroup_RenewUI_Tooltip then
    Panel_Tooltip_Item_Show(itemWrapper, Panel_Window_Improvement_All, false, true)
  else
...
function HandleEventOnOut_Improvement_All_ShowToolTipItemKey(itemKey, isShow)
  ...
  if false == _ContentsGroup_RenewUI_Tooltip then
    Panel_Tooltip_Item_Show(itemSSW, Panel_Window_Improvement_All, true, false)
  else
    if true == PaGlobalFunc_TooltipInfo_GetShow() then
      PaGlobalFunc_TooltipInfo_Close()
      return
...
function PaGlobalFunc_Improvement_All_UpdateEquipList(control, key)
  ...
  if _key <= PaGlobal_Improvement_All._indexInvenEnd then
    ...
    if false == _ContentsGroup_UsePadSnapping then
      ...
      if false == ToClient_isConsole() then
        btn_list:registerPadEvent("Mouse_Out", "HandleEventOnOut_Improvement_All_ShowToolTip(" .. slotIdx .. ",false)")
      end
    end
  ...
  if itemSSW:getItemType() == 1 then
    ...
    if equipslotNo >= 7 and equipslotNo <= 12 then
      isAccessory = true
    end
  end
  if isAccessory == true then
    if enchantLevel > 0 then
      enchantText = HighAccessoryEnchantLevel_ReplaceString(enchantLevel) .. " "
    end
