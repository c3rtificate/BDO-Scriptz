PaGlobal_EnchantStackConfirm_All = {
  ...
  _enchantInfo = nil,
  _totalStack = nil,
  _isGroupEnchantStackConfirm = nil,
  _MAXINPUT = 5,
  _NUMBER_KEYCODE = {
...
function PaGlobalFunc_EnchantStackConfirm_All_Open(isGroupEnchant)
  if nil == Panel_Window_EnchantStackConfirm_All or true == Panel_Window_EnchantStackConfirm_All:GetShow() then
    return
  end
  PaGlobal_EnchantStackConfirm_All._isGroupEnchantStackConfirm = isGroupEnchant
  ClearFocusEdit()
...
function HandleEventLUp_EnchantStackConfirm_All_isValidateText()
  ...
  local resumeEnchant = function()
    if PaGlobal_EnchantStackConfirm_All._isGroupEnchantStackConfirm == true then
      HandleEventLUp_SpiritEnchant_All_GroupEnchantReadyWithMyStackCount()
    else
  ...
  if userInput == myStack then
    local itemWrap = getInventoryItemByType(PaGlobal_SpiritEnchant_All._grantItemWhereType, PaGlobal_SpiritEnchant_All._grantItemSlotNo)
    ...
    if true == itemWrap:isSetLimitOption(__eItemLimitOptionType_LimitItemUseOriginalDuelItem) then
      ...
      local messageBoxData = {
        ...
        functionNo = PaGlobalFunc_EnchantStackConfirm_All_Close,
