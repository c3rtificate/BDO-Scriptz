...
function CompetitionGame_Init()
  ...
  self._list2:registEvent(__ePAUIList2EventType_LuaChangeContent, "CompetitionGame_ListUpdate")
  self._list2:createChildContent(__ePAUIList2ElementManagerType_List)
  self._list2_Observer:changeAnimationSpeed(10)
  self._list2_Observer:registEvent(__ePAUIList2EventType_LuaChangeContent, "CompetitionGame_ListUpdate_Observer")
  self._list2_Observer:createChildContent(__ePAUIList2ElementManagerType_List)
...
function FromClient_InviteCompetiton(hostCharacterName, isObserver, isOfflineMember)
  local self = competitionGame
  local isEnterHadumField = ToClient_IsEnterHadumField()
  local function InviteCompetition()
    if isEnterHadumField == true then
  ...
  local function RefuseCompetition()
    if isEnterHadumField == true then
      Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantDoingInThisField"))
      return
    end
    ToClient_ResponseInviteCompetition(false, isObserver)
  end
  if isOfflineMember == true then
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ALERT_NOTIFICATIONS"),
      content = PAGetString(Defines.StringSheet_GAME, "LUA_COMPETITION_RES_FAIL"),
      priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW,
