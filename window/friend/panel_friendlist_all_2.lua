...
function PaGlobal_FriendList_All:prepareClose()
  ...
  elseif true == PaGlobal_FriendList_All._ui.stc_PopupFunctionBG:GetShow() then
    ...
    return
  elseif true == PaGlobal_FriendList_All._ui.stc_Popup2FunctionBG:GetShow() then
    PaGlobal_FriendList_All:hidePopupMenu()
    return
...
function PaGlobal_FriendList_All:close()
  ...
  Panel_FriendList_All:SetShow(false, false)
...
function PaGlobal_FriendList_All:updateKeyGuides()
  ...
  local isLTShow = Type._Popup ~= PaGlobal_FriendList_All._currentKeyGuideType and PaGlobal_FriendList_All._tab._pcFriendTab == PaGlobal_FriendList_All._currentTab and Type._Popup2 ~= PaGlobal_FriendList_All._currentKeyGuideType
...
function PaGlobal_FriendList_All:guildInvite()
  ...
  if _ContentsGroup_GuildAuthoritySeparation == true then
    ...
    isRightInviteGuild = PaGlobalFunc_FriendList_All_RightCheck(__eGuildRightType_Join)
  end
  ...
  if nil ~= guildInfo then
    ...
    if nil ~= guildInfo:getMemberByUserNo(userNo) then
  ...
  if 0 == guildGrade then
    ...
    ToClient_RequestSimpleInviteGuildByChatSubMenu(userName, userCharacterName)
  end
end
...
function PaGlobal_FriendList_All:deleteFriendGroup()
  if nil == Panel_FriendList_All then
    return
  end
  local _groupListData = PaGlobal_FriendList_All._groupListData
...
function PaGlobal_FriendList_All:showPopupMenu(isShow)
  ...
  if isShow then
    ...
    PaGlobal_FriendList_All._currentKeyGuideType = PaGlobal_FriendList_All._keyGuideType._Popup
    PaGlobal_FriendList_All:updateKeyGuides()
    PaGlobal_FriendList_All._ui.stc_Popup2FunctionBG:SetShow(false)
    PaGlobal_FriendList_All._ui.stc_SubFunctionBG:SetShow(false)
  end
end
function PaGlobal_FriendList_All:showPopupMenu2(isShow)
  if nil == Panel_FriendList_All then
    return
  end
  local friendListData = PaGlobal_FriendList_All._friendListData
  local groupListData = PaGlobal_FriendList_All._groupListData
  local popupBG = PaGlobal_FriendList_All._ui.stc_Popup2FunctionBG
  popupBG:SetShow(isShow)
  local control
  local index = PaGlobal_FriendList_All._groupListData._selectedGroupIndex
  if -1 ~= index then
    control = PaGlobal_FriendList_All._ui.list2_Friend:GetContentByKey(toInt64(0, index))
  end
  if nil ~= control then
  ...
  PaGlobal_FriendList_All._ui.stc_Popup2FunctionList[PaGlobal_FriendList_All._popupFunction2.GROUP_RENAME]:SetShow(true)
  PaGlobal_FriendList_All._ui.stc_Popup2FunctionList[PaGlobal_FriendList_All._popupFunction2.GROUP_DELETE]:SetShow(true)
  local gapY = PaGlobal_FriendList_All._ui.stc_Popup2FunctionList[0]:GetSizeY()
  local activeCount = 0
  for i = 0, 1 do
    stc_function = PaGlobal_FriendList_All._ui.stc_Popup2FunctionList[i]
    if true == stc_function:GetShow() then
      stc_function:SetPosY(gapY * activeCount)
      activeCount = activeCount + 1
    end
  ...
  popupBG:SetSize(popupBG:GetSizeX(), gapY * activeCount)
  PaGlobal_FriendList_All._currentKeyGuideType = PaGlobal_FriendList_All._keyGuideType._Popup2
  PaGlobal_FriendList_All:updateKeyGuides()
  PaGlobal_FriendList_All._ui.stc_SubFunctionBG:SetShow(false)
...
function HandleEventRUp_FriendList_All_ShowPopup2(index)
  PaGlobal_FriendList_All._groupListData._selectedGroupIndex = index
  PaGlobal_FriendList_All._friendListData._selectedFriendIndex = nil
  PaGlobal_FriendList_All._listMoveIndex = nil
  PaGlobal_FriendList_All:showPopupMenu2(true)
end
    ...
    elseif nil ~= _groupListData._groupInfo[index] then
      ...
      elseif friendGroup:getGroupNo() == friendGroupNoPartyFriend then
        ...
        if true == _ContentsGroup_Messenger then
          if true == _ContentsGroup_UsePadSnapping then
            control:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandleEventRUp_FriendList_All_ShowPopup2(" .. index .. ")")
          else
            parentBG:addInputEvent("Mouse_RUp", "HandleEventRUp_FriendList_All_ShowPopup2(" .. index .. ")")
          end
...
function PaGlobal_FriendList_All_ToggleGroup(id)
  ...
  local groupListData = PaGlobal_FriendList_All._groupListData._groupInfo[PaGlobal_FriendList_All._groupListData._selectedGroupIndex]
...
function PaGlobalFunc_FriendList_All_RightCheck(eRightType)
