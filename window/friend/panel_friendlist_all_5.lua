...
function PaGlobal_Friend_GroupRename_All_RenameGroup()
  ...
  if true == _ContentsGroup_Messenger then
    _groupListData = PaGlobal_FriendInfoList_All._groupListData
  else
    _groupListData = PaGlobal_FriendList_All._groupListData
  end
  ...
  if _ContentsOption_CH_CheckProhibitedWord == true then
    ToClient_CheckChinaProhibitedWord(editText, __eSceneIdRenameFriendGroup)
    return
  end
...
function PaGlobal_Friend_GroupRename_All_AddGroup()
  local editText = PaGlobal_Friend_GroupRename_All._ui.edit_GroupName:GetEditText()
  if _ContentsOption_CH_CheckProhibitedWord == true then
    ToClient_CheckChinaProhibitedWord(editText, __eSceneIdAddFriendGroup)
    return
  end
...
function FromClient_CheckChinaProhibitedWord_AddFriendGroup(filteredText)
  if _ContentsOption_CH_CheckProhibitedWord == false then
    return
  end
  local self = PaGlobal_Friend_GroupRename_All
  if self == nil then
    return
  end
  local friendGroupCount = RequestFriends_getFriendGroupCount()
  for groupIndex = 0, friendGroupCount - 1 do
    local friendGroup = RequestFriends_getFriendGroupAt(groupIndex)
    if friendGroup:getName() == filteredText then
      Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_DUPLICATION_FRIEND_GROUP_NAME"))
      return
    end
  end
  requestFriendList_addFriendGroup(filteredText)
  self:prepareClose()
end
function FromClient_CheckChinaProhibitedWord_RenameFriendGroup(filteredText)
  if _ContentsOption_CH_CheckProhibitedWord == false then
    return
  end
  local self = PaGlobal_Friend_GroupRename_All
  ...
  if 0 <= _groupListData._selectedGroupIndex then
    local friendGroup = _groupListData._groupInfo[_groupListData._selectedGroupIndex]
    if filteredText == friendGroup:getName() then
      return
    end
    if "" == filteredText then
      return
    end
    requestFriendList_renameGroup(_groupListData._groupInfo[_groupListData._selectedGroupIndex]:getGroupNo(), filteredText)
  end
  self:prepareClose()
end
