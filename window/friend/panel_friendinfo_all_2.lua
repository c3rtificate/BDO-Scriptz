...
function PaGlobal_FriendInfoList_All:updateKeyGuides()
  ...
  if Type._DefaultGroup == PaGlobal_FriendInfoList_All._currentKeyGuideType then
    ...
    PaGlobal_FriendInfoList_All._ui.txt_KeyGuide_RT_X:SetShow(false)
  ...
  elseif Type._Friend == PaGlobal_FriendInfoList_All._currentKeyGuideType then
    ...
    if nil == PaGlobal_FriendInfoList_All._currentOpenMessangerId then
      ...
      PaGlobal_FriendInfoList_All._ui.txt_KeyGuide_RT_X:SetShow(true)
    end
...
function PaGlobal_FriendInfoList_All:guildInvite()
  ...
  if _ContentsGroup_GuildAuthoritySeparation == true then
    ...
    isRightInviteGuild = PaGlobalFunc_FriendInfoList_All_RightCheck(__eGuildRightType_Join)
  end
  if false == isRightInviteGuild then
    if _ContentsGroup_GuildAuthoritySeparation == true then
      PaGlobalFunc_Guild_Authorization_ShowErrMessage(__eGuildRightNew_Join)
    end
    return
  end
  ...
  if 0 == guildGrade then
    ToClient_RequestInviteGuild(userCharacterName, 0, 0, 0)
  else
...
function PaGlobal_FriendInfoList_All:messangerOpen(messangerId, friendUserNo)
  ...
  if true == _ContentsGroup_UsePadSnapping then
    ...
    PaGlobal_FriendInfoList_All._ui.stc_Emo:SetShow(false)
    ...
    Panel_FriendInfoList_All:registerPadEvent(__eConsoleUIPadEvent_RTPress_X, "PaGlobalFunc_FriendInfoList_All_OpenChatMacro()")
    ...
    PaGlobal_FriendInfoList_All._ui.stc_Emo:SetShow(true)
    ...
    PaGlobal_FriendInfoList_All._ui.stc_Emo:addInputEvent("Mouse_LUp", "PaGlobalFunc_FriendInfoList_All_OpenChatMacro()")
    PaGlobal_FriendInfoList_All._ui.stc_Edit:RegistReturnKeyEvent("PaGlobalFunc_FriendInfoList_All_sendMessageByKey()")
  end
  ...
  PaGlobal_FriendInfoList_All:updateKeyGuides()
  PaGlobal_FriendInfoList_All:EmoticonAni()
end
function PaGlobal_FriendInfoList_All:clearAllMessage()
  for index = 0, PaGlobal_FriendInfoList_All._messageCount - 1 do
    ...
    PaGlobal_FriendInfoList_All._StaticEmoAni[index]:SetShow(false)
    ...
    UI.deleteControl(PaGlobal_FriendInfoList_All._StaticEmoAni[index])
    for _, info in pairs(PaGlobal_FriendInfoList_All._StaticText[index]) do
  ...
  PaGlobal_FriendInfoList_All._messageIndex = nil
  PaGlobal_FriendInfoList_All._contentIndex = nil
end
function PaGlobal_FriendInfoList_All:updateMessage(chattingMessage, friendClass, isOnline)
  PaGlobal_FriendInfoList_All:showMessage(chattingMessage, friendClass, isOnline)
  PaGlobal_FriendInfoList_All._messageCount = PaGlobal_FriendInfoList_All._messageCount + 1
end
function PaGlobal_FriendInfoList_All:showMessage(chattingMessage, friendClass, isOnline)
  local msg = chattingMessage:getContent()
  local msgTime = chattingMessage:getTime()
  local isMe = chattingMessage.isMe
  PaGlobal_FriendInfoList_All:createMessageUI(chattingMessage)
...
function PaGlobal_FriendInfoList_All:createMessageUI(chattingMessage)
  local msg = chattingMessage:getContent()
  local isMe = chattingMessage.isMe
  ...
  PaGlobal_FriendInfoList_All._StaticText[messageCount] = {}
  ...
  local textSizeX = Panel_FriendInfoList_All:GetSizeX() - 100
  ...
  if 0 ~= emoticonCount then
    ...
    while emoticonCount >= emoNum do
      ...
      if msgstartindex == emoticonindex then
        local emoticonControl = UI.createControl(__ePAUIControl_Static, frameContent, "Static_Emo_" .. messageCount .. "_" .. contentindex)
        ...
        PaGlobal_FriendInfoList_All._StaticText[messageCount][contentindex] = contentInfo
        if nil == PaGlobal_FriendInfoList_All._messageIndex or nil == PaGlobal_FriendInfoList_All._contentIndex then
          PaGlobal_FriendInfoList_All._messageIndex = messageCount
          PaGlobal_FriendInfoList_All._contentIndex = contentindex
        end
        emoNum = emoNum + 1
        ...
        while msgDataLen > 0 do
          ...
          contentInfo.isEmoticon = false
          contentInfo.emoticonKey = 0
          local textControl = UI.createControl(__ePAUIControl_StaticText, frameContent, "Static_Text_" .. messageCount .. "_" .. contentindex)
          ...
          checkmsg, msgData, msgDataLen = PaGlobal_FriendInfoList_All:getTextLimitWidth(contentInfo.content, msgData)
          contentInfo.content:SetText(checkmsg)
          PaGlobal_FriendInfoList_All._StaticText[messageCount][contentindex] = contentInfo
          contentindex = contentindex + 1
          curTextSizeX = curTextSizeX + contentInfo.content:GetTextSizeX()
    ...
    while msgDataLen > 0 do
      ...
      checkmsg, msgData, msgDataLen = PaGlobal_FriendInfoList_All:getTextLimitWidth(contentInfo.content, msgData)
      contentInfo.content:SetText(checkmsg)
      contentInfo.content:SetSize(contentInfo.content:GetTextSizeX(), textControl:GetSizeY())
      PaGlobal_FriendInfoList_All._StaticText[messageCount][contentindex] = contentInfo
      contentindex = contentindex + 1
      curTextSizeX = curTextSizeX + contentInfo.content:GetTextSizeX()
      if "" == checkmsg or nil == checkmsg then
  ...
  PaGlobal_FriendInfoList_All._StaticEmoAni[messageCount] = UI.createControl(__ePAUIControl_Static, frameContent, "Static_EmoAni_" .. messageCount)
  CopyBaseProperty(styleEmoAni, PaGlobal_FriendInfoList_All._StaticEmoAni[messageCount])
  ...
  PaGlobal_FriendInfoList_All._StaticEmoAni[messageCount]:SetShow(false)
  if false == isMe then
  ...
  PaGlobal_FriendInfoList_All._emoticonSize = styleEmoticon:GetSizeY()
end
function PaGlobal_FriendInfoList_All:resizeMessageUI(isMe, msg, msgTime, friendClass, isOnline)
  ...
  local maxTextSizeX = (panelSizeX - 100) * scaleX
  ...
  for _, info in pairs(staticText) do
    if nil ~= info.content then
      ...
      if false == info.isEmoticon then
        info.content:SetSize(info.content:GetTextSizeX() * scaleX, PaGlobal_FriendInfoList_All._emoticonSize * scaleY)
      else
        info.content:SetSize(info.content:GetSizeX() * scaleX, PaGlobal_FriendInfoList_All._emoticonSize * scaleY)
      end
      if false == info.isEmoticon then
  ...
  staticBg:SetSize(contentXSize + 30 * scaleX, contentYSize + 15)
end
function PaGlobal_FriendInfoList_All:setPosMessageUI(isMe)
  ...
  if false == isMe then
    ...
    contentStartPosX = PaGlobal_FriendInfoList_All._StaticBg[messageCount]:GetPosX() + gapX * 1.2
  else
    ...
    contentStartPosX = PaGlobal_FriendInfoList_All._StaticBg[messageCount]:GetPosX() + gapX
  end
  ...
  contentStartPosY = PaGlobal_FriendInfoList_All._StaticBg[messageCount]:GetPosY() + 10
  local panelSizeX = Panel_FriendInfoList_All:GetSizeX()
  local maxTextSizeX = PaGlobal_FriendInfoList_All._StaticBg[messageCount]:GetSizeX()
  local curTextSizeX = 0
  local index = 0
  local addPosY = 5 * scaleY
  local staticText = PaGlobal_FriendInfoList_All._StaticText[messageCount]
  while true do
...
function PaGlobal_FriendInfoList_All:getTextLimitWidth(chatting_contents, msgOrigin)
...
function PaGlobal_FriendInfoList_All:resizeMessageUIDrag(messageCount, isMe, msg, msgTime)
  ...
  if false == isMe then
    ...
    PaGlobal_FriendInfoList_All._StaticBg[messageCount]:SetPosX(PaGlobal_FriendInfoList_All._ui.FrameScroll:GetPosX() - PaGlobal_FriendInfoList_All._StaticBg[messageCount]:GetSizeX() - gapX)
    PaGlobal_FriendInfoList_All._StaticTime[messageCount]:SetPosX(Panel_FriendInfoList_All:GetSizeX() - PaGlobal_FriendInfoList_All._StaticTime[messageCount]:GetSizeX() - gapX * 2)
    contentStartPosX = Panel_FriendInfoList_All:GetSizeX() - PaGlobal_FriendInfoList_All._StaticBg[messageCount]:GetSizeX() - gapX * 2
  end
  PaGlobal_FriendInfoList_All._StaticBg[messageCount]:SetPosY(prevBgPosY + prevBgSizeY)
  PaGlobal_FriendInfoList_All._StaticTime[messageCount]:SetPosY(PaGlobal_FriendInfoList_All._StaticBg[messageCount]:GetPosY() + PaGlobal_FriendInfoList_All._StaticBg[messageCount]:GetSizeY() + 5)
  PaGlobal_FriendInfoList_All._StaticClass[messageCount]:SetPosY(PaGlobal_FriendInfoList_All._StaticBg[messageCount]:GetPosY())
...
function PaGlobal_FriendInfoList_All:EmoticonAni()
  if nil == PaGlobal_FriendInfoList_All._currentOpenMessangerId then
    return
  end
  if nil == PaGlobal_FriendInfoList_All._messageIndex or nil == PaGlobal_FriendInfoList_All._contentIndex then
  ...
  local emoticonControl = PaGlobal_FriendInfoList_All._StaticText[PaGlobal_FriendInfoList_All._messageIndex][PaGlobal_FriendInfoList_All._contentIndex].content
  if nil == emoticonControl then
    return
  end
  local emoticonKey = PaGlobal_FriendInfoList_All._StaticText[PaGlobal_FriendInfoList_All._messageIndex][PaGlobal_FriendInfoList_All._contentIndex].emoticonKey
  if nil == emoticonKey then
    return
  end
  local emoticonAniControl = PaGlobal_FriendInfoList_All._StaticEmoAni[PaGlobal_FriendInfoList_All._messageIndex]
...
function PaGlobalFunc_FriendInfoList_All_MessangerCloseInFriendList()
  ...
  if nil ~= PaGlobal_FriendInfoList_All._currentOpenMessangerId then
    ...
    messangerId = tostring(PaGlobal_FriendInfoList_All._currentOpenMessangerId)
  end
  ...
  Panel_FriendInfoList_All:registerPadEvent(__eConsoleUIPadEvent_RTPress_X, "")
  PaGlobal_FriendInfoList_All:clearAllMessage()
  PaGlobal_FriendInfoList_All._currentKeyGuideType = PaGlobal_FriendInfoList_All._keyGuideType._Friend
  PaGlobal_FriendInfoList_All:updateKeyGuides()
  PaGlobalFunc_ChatEmoticon_CloseByMessanger(messangerId)
  ClearFocusEdit()
end
...
function PaGlobalFunc_FriendInfoList_All_SetFocusEdit()
  ...
  if PAGetString(Defines.StringSheet_RESOURCE, "FRIEND_STCTXT_EDIT_DESC") == PaGlobal_FriendInfoList_All._ui.stc_Edit:GetEditText() then
...
function PaGlobalFunc_FriendInfoList_All_IsFocusEdit()
  if true == PaGlobal_FriendInfoList_All._ui.stc_Edit:GetFocusEdit() then
    return true
  end
  return false
end
...
function PaGlobalFunc_FriendInfoList_All_sendMessage(messangerIdstr)
  ...
  PaGlobalFunc_ChatEmoticon_CloseByMessanger(tostring(messangerIdstr))
end
function PaGlobalFunc_FriendInfoList_All_OpenChatMacro()
  if ToClient_getEmoticonInfoList() == 0 then
    local text = PAGetString(Defines.StringSheet_GAME, "LUA_EMOTICON_BUY_MSG")
    Proc_ShowMessage_Ack(text)
    return
  end
  SetFocusEdit(PaGlobal_FriendInfoList_All._ui.stc_Edit)
  local posX = Panel_FriendInfoList_All:GetPosX() - Panel_Chat_Emoticon:GetSizeX()
  local posY = Panel_FriendInfoList_All:GetPosY()
  PaGlobalFunc_ChatEmoticon_OpenByMessanger(posX, posY, tostring(PaGlobal_FriendInfoList_All._currentOpenMessangerId), false)
  if PAGetString(Defines.StringSheet_RESOURCE, "FRIEND_STCTXT_EDIT_DESC") == PaGlobal_FriendInfoList_All._ui.stc_Edit:GetEditText() then
    PaGlobal_FriendInfoList_All._ui.stc_Edit:SetEditText("")
  end
end
...
function PaGlobalFunc_FriendInfoList_All_FriendMessangerInputEmoticon(inputStr)
  ReleaseImeComposition()
  SetFocusEdit(PaGlobal_FriendInfoList_All._ui.stc_Edit)
  local message = PaGlobal_FriendInfoList_All._ui.stc_Edit:GetEditText()
  if message == PAGetString(Defines.StringSheet_RESOURCE, "FRIEND_STCTXT_EDIT_DESC") then
    PaGlobal_FriendInfoList_All._ui.stc_Edit:SetEditText("")
  end
  local originTextSize = PaGlobal_FriendInfoList_All._ui.stc_Edit:GetEditTextSize()
  if originTextSize > 0 then
    PaGlobal_FriendInfoList_All._ui.stc_Edit:AddEditText(inputStr)
  else
    PaGlobal_FriendInfoList_All._ui.stc_Edit:SetEditText(message .. inputStr, true)
  end
end
...
function FromClient_ChangeUserNicknameDirectly_Friend()
  local self = PaGlobal_FriendInfoList_All
  if self == nil then
    return
  end
  if self._tab._consoleFriendTab == self._currentTab then
    self:updateFriendListForConsole()
  else
