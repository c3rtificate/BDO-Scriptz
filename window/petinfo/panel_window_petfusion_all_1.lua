function PaGlobal_PetFusion_All:initialize()
  ...
  self._ui.txt_RateView = UI.getChildControl(self._ui.stc_FusionBG, "StaticText_RateView_Link")
  ...
  self._ui_pc.btn_skillPercent = UI.getChildControl(self._ui.stc_SelectBG, "Button_Skill_Percent")
  self._ui_console.txt_skillPercent = UI.getChildControl(self._ui_console.stc_Bottom_KeyGuideBG, "StaticText_Skill_Percent")
  ...
  self._ui.txt_RateView:SetSpanSize(self._ui.txt_RateView:GetSpanSize().x, self._ui.stc_SelectDesc_BG:GetPosY() + self._ui.stc_SelectDesc_BG:GetSizeY() + 10)
  ...
  self._originKeyBGSizeY = self._ui_console.stc_Bottom_KeyGuideBG:GetSizeY()
  self._originPanelSizeY = Panel_Window_PetFusion_All:GetSizeY()
...
function PaGlobal_PetFusion_All:resizeOnConsole()
  ...
  if self._isFusionStart then
    ...
    Panel_Window_PetFusion_All:SetSize(Panel_Window_PetFusion_All:GetSizeX(), self._originPanelSizeY)
    self._ui.stc_CompleteEffectBG:SetSize(self._ui.stc_CompleteEffectBG:GetSizeX(), self._originPanelSizeY)
...
function PaGlobal_PetFusion_All:platformSetting()
  ...
  if self._isConsole then
    ...
    self._ui_console.txt_skillPercent:SetShow(false)
    if true == self._showSkillPercentButton then
      keyGuides = {
        self._ui_console.txt_KeyGuideY,
        self._ui_console.txt_KeyGuideA,
        self._ui_console.txt_KeyGuideB,
        self._ui_console.txt_skillPercent
      }
      self._ui_console.txt_skillPercent:SetShow(true)
    end
    PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyGuides, self._ui_console.stc_Bottom_KeyGuideBG, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_AUTO_WRAP_LEFT)
    if true == self._ui_console.txt_skillPercent:GetShow() then
      if self._ui_console.txt_KeyGuideY:GetPosY() ~= self._ui_console.txt_skillPercent:GetPosY() then
        self._ui_console.stc_Bottom_KeyGuideBG:SetSpanSize(0, 25)
      end
    else
      self._ui_console.stc_Bottom_KeyGuideBG:SetSpanSize(0, 0)
    end
    self._ui_console.stc_Bottom_KeyGuideBG:ComputePos()
    keyGuides = {
...
function PaGlobal_PetFusion_All:registEventHandler()
  ...
  self._ui.txt_RateView:addInputEvent("Mouse_LUp", "HandleEventLUp_PetFusion_OpenRateViewByLink()")
  self._ui.txt_RateView:addInputEvent("Mouse_On", "HandleEventOnOut_PetFusion_ChangeRateViewButtonText(true)")
  self._ui.txt_RateView:addInputEvent("Mouse_Out", "HandleEventOnOut_PetFusion_ChangeRateViewButtonText(false)")
  if self._isConsole then
    ...
    Panel_Window_PetFusion_All:registerPadEvent(__eConsoleUIPadEvent_LTPress_X, "HandleEventLUp_PetSkill_Percent_All_Open()")
  else
    ...
    self._ui_pc.btn_skillPercent:addInputEvent("Mouse_LUp", "HandleEventLUp_PetSkill_Percent_All_Open()")
  end
end
function PaGlobal_PetFusion_All:prepareOpen()
  ...
  if PaGlobalFunc_PetList_All_Show_AcquireBG ~= nil then
    PaGlobalFunc_PetList_All_Show_AcquireBG(false)
  end
  HandleEventOnOut_PetFusion_ChangeRateViewButtonText(false)
...
function PaGlobal_PetFusion_All:prepareClose(forceClose)
  ...
  if PaGlobalFunc_PetList_All_Show_AcquireBG ~= nil then
    PaGlobalFunc_PetList_All_Show_AcquireBG(true)
  end
...
function PaGlobal_PetFusion_All:validate()
  ...
  self._ui_pc.btn_skillPercent:isValidate()
  self._ui_console.txt_skillPercent:isValidate()
end
function PaGlobal_PetFusion_All:clear()
  if nil == Panel_Window_PetFusion_All then
    return
  end
  self._skillLearnCount = 0
  self._ui_pc.btn_skillPercent:SetShow(false)
  self._ui_console.txt_skillPercent:SetShow(false)
...
function PaGlobal_PetFusion_All:rateClear()
  ...
  if true == _ContentsGroup_PetSkillPercent then
    PaGlobal_PetSkill_Percent_All:setTierButton(-1, -1)
    self:setShowSkillPercentButton(false)
  end
end
...
function PaGlobal_PetFusion_All:PetFusionInitControlSetting(isOn)
  ...
  self._ui.txt_RateView:SetShow(isOn)
...
function PaGlobal_PetFusion_All:openSelectLookBG()
  ...
  if nil ~= Panel_Window_PetSkill_Percent_All and true == Panel_Window_PetSkill_Percent_All:GetShow() then
...
function PaGlobal_PetFusion_All:updateSkillSlot()
  ...
  self._skillLearnCount = skillLearnCount
  if true == _ContentsGroup_PetSkillPercent then
    self:setShowSkillPercentButton(true)
  end
end
...
function PaGlobal_PetFusion_All:checkSetAble(petNo, petRace, sealPetIndex, petTier)
  ...
  if petTier == 5 then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_PETFUSION_LEADERPET"))
    return false
  end
...
function PaGlobal_PetFusion_All:setRate()
  ...
  if true == _ContentsGroup_PetSkillPercent then
    PaGlobal_PetSkill_Percent_All:setTierButton(targetTier, targetTier - 1)
    self:setShowSkillPercentButton(true)
  end
end
function PaGlobal_PetFusion_All:fusionPet()
  ...
  self:closeSelectLookBG()
  self:closeSelectSkillBG()
  if self._isFusionAble == false then
  ...
  if self._petName == "" or self._petName == self._config._defaultName then
  ...
  local messageBoxData = {
    title = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SERVANTMIX_TITLE"),
    content = PAGetString(Defines.StringSheet_GAME, "PANEL_PETLIST_PETCOMPOSE_MSGCONTENT"),
...
function PaGlobal_PetFusion_All:showFusionResult()
  ...
  if not _ContentsGroup_PetLevelDelete then
    ...
    self._ui.txt_PetNameLevel:SetText(self._completeDataTable._name)
  end
...
function PaGlobal_PetFusion_All:setShowSkillPercentButton(isShow)
  if false == _ContentsGroup_PetSkillPercent then
    self._ui_pc.btn_skillPercent:SetShow(false)
    self._ui_console.txt_skillPercent:SetShow(false)
    return
  end
  if false == self._isConsole then
    self._ui_pc.btn_skillPercent:SetShow(isShow)
    self._ui_console.txt_skillPercent:SetShow(false)
  else
    self._ui_pc.btn_skillPercent:SetShow(false)
    self._ui_console.txt_skillPercent:SetShow(isShow)
    self._showSkillPercentButton = isShow
    self:platformSetting()
  end
end
