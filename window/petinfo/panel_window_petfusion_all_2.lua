...
function HandleEventLUp_PetFusion_OpenRateViewByLink()
  PaGlobalFunc_Util_OpenRateView(nil)
end
function HandleEventOnOut_PetFusion_ChangeRateViewButtonText(isOn)
  if isOn == true then
    PaGlobal_PetFusion_All._ui.txt_RateView:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_RATEDESC_HERE_CLICK_ON"))
  else
    PaGlobal_PetFusion_All._ui.txt_RateView:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_RATEDESC_HERE_CLICK"))
  end
end
function FromClient_PetFusionResult_All(rv)
  if PaGlobal_PetFusion_All == nil then
    return
  end
