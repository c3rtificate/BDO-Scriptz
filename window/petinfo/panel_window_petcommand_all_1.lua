...
function PaGlobal_PetCommand_All:update()
  ...
  local petNo64 = pcPetData:getPcPetNo()
  ...
  if nil == pcPetOrderOption then
    if nil ~= pcPetData:getSkillParam(1) then
      ...
      isFind = isCheckTalent
    else
      isFind = false
    end
    isFollow = true
    isGetItem = true
    ToClient_savePetOrderOptionByPetNo(petNo64, __ePetOrderOptionType_DoFollowAndWait, isFollow)
    ToClient_savePetOrderOptionByPetNo(petNo64, __ePetOrderOptionType_DoFind, isFind)
    ToClient_savePetOrderOptionByPetNo(petNo64, __ePetOrderOptionType_DoGetItem, isGetItem)
  else
    isFind = pcPetOrderOption:getDoFind()
    isFollow = pcPetOrderOption:getDoFollowAndWait()
    isGetItem = pcPetOrderOption:getDoGetItem()
  end
  for index = 0, #self._playBtns do
...
function PaGlobal_PetCommand_All:orderPet(petNo, isLeader, index, orderType, value)
  if 0 == orderType then
    if value then
  ...
  ToClient_savePetOrderOptionByPetNo(petNo, orderType, value)
end
