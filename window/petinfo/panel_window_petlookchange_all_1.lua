...
function PaGlobal_PetLookChange_All:registEventHandler()
  ...
  if true == _ContentsOption_CH_Help then
    self._ui_pc.btn_Title_Question:SetShow(false)
  end
end
...
function PaGlobal_PetLookChange_All:createListContents(contents, key)
  ...
  local tierText = "[" .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_SERVANT_TIER", "tier", petTier) .. "]"
  if not _ContentsGroup_PetLevelDelete then
    local levelText = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_LV") .. "." .. petLevel
    txt_PetLevel:SetText(tierText .. " " .. levelText)
  else
    txt_PetLevel:SetText(tierText)
  end
