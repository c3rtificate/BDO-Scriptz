...
function click_DeliveryForPerson_GetOn()
  ...
  local characterData = getCharacterDataByIndex(deliveryForPerson._selectCharacterIndex, __ePlayerCreateNormal)
  ...
  local removeTime = getCharacterDataRemoveTime(deliveryForPerson._selectCharacterIndex, __ePlayerCreateNormal)
  ...
  local messageContent = PAGetStringParam2(Defines.StringSheet_RESOURCE, "DELIVERY_PERSON_READY_CHK", "now_character", getSelfPlayer():getName(), "change_character", getCharacterName(getCharacterDataByIndex(deliveryForPerson._selectCharacterIndex, __ePlayerCreateNormal)))
...
local function initialize()
  ...
  if true == _ContentsOption_CH_Help then
    deliveryForPerson._buttonQuestion:SetShow(false)
  end
  PaGlobal_Util_RegistHelpTooltipEvent(deliveryForPerson._buttonQuestion, "\"DeliveryPerson\"")
