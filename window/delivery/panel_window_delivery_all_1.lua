function PaGlobal_Delivery_All:initialize()
  ...
  if true == self._isConsole then
    ...
    local x1, y1, x2, y2 = setTextureUV_FuncByTextureType(self._ui.stc_itemSlotTemplete, 50, 195, 94, 239, __eUITextureTypeOn)
...
function PaGlobal_Delivery_All:deliveryInformation_UpdateSlot()
  ...
  if true == PaGlobal_Delivery_All._isTransList then
    ...
    if dialog_isEventWarehouseNpc() == true then
      regionKey = ToClient_NewDeliveryGetRegionKey(warehouse_getEventWarehouseWaypointKey())
    end
...
function PaGlobal_Delivery_All:prepareOpen()
  ...
  if ToClient_WorldMapIsShow() then
    ...
    if nil == Panel_Window_Barter_Search or true ~= Panel_Window_Barter_Search:GetShow() or Panel_Window_Barter_Search:IsUISubApp() == false then
    end
