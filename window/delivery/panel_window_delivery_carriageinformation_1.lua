function PaGlobal_CarriageInformation:initialize()
  ...
  for ii = 0, PaGlobal_CarriageInformation.config.slotCount - 1 do
    ...
    slot.base = UI.createControl(__ePAUIControl_Static, Panel_Window_Delivery_CarriageInformation, "Delivery_Slot_" .. slot.slotNo)
    CopyBaseProperty(PaGlobal_CarriageInformation._ui.stc_Slot, slot.base)
    slot.departure = UI.createControl(__ePAUIControl_StaticText, slot.base, "Delivery_Slot_Departure_" .. slot.slotNo)
    CopyBaseProperty(PaGlobal_CarriageInformation._ui.txt_Departure, slot.departure)
    slot.destination = UI.createControl(__ePAUIControl_StaticText, slot.base, "Delivery_Slot_Destination_" .. slot.slotNo)
    CopyBaseProperty(PaGlobal_CarriageInformation._ui.txt_Destination, slot.destination)
    slot.static_Arrow = UI.createControl(__ePAUIControl_Button, slot.base, "Delivery_Slot_Arrow_" .. slot.slotNo)
...
function PaGlobal_CarriageInformation:registEventHandler()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(PaGlobal_CarriageInformation._ui.buttonQuestion, "\"DeliveryCarriageinformation\"")
  ...
  if true == _ContentsOption_CH_Help then
    PaGlobal_CarriageInformation._ui.buttonQuestion:SetShow(false)
  end
end
