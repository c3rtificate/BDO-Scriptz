...
function deliveryForGameExit.fillData()
  ...
  local count = getCharacterDataCount(__ePlayerCreateNormal)
...
function click_DeliveryForGameExit_GetOn()
  ...
  local characterData = getCharacterDataByIndex(deliveryForGameExit._selectCharacterIndex, __ePlayerCreateNormal)
  if nil == characterData then
    return
  end
  ...
  local removeTime = getCharacterDataRemoveTime(deliveryForGameExit._selectCharacterIndex, __ePlayerCreateNormal)
  ...
  local duelCharacterIndex = PaGlobalFunc_CharacterTag_All_GetDuelCharcterIndex()
  ...
  local messageContent = preText .. PAGetStringParam2(Defines.StringSheet_RESOURCE, "DELIVERY_PERSON_READY_CHK", "now_character", getSelfPlayer():getName(), "change_character", getCharacterName(getCharacterDataByIndex(deliveryForGameExit._selectCharacterIndex, __ePlayerCreateNormal)))
  local messageboxData = {
...
function click_DeliveryForGameExit_Dest()
  ...
  ToClient_padSnapRefreshTarget(deliveryForGameExit._comboBoxDest)
end
...
function click_DeliveryForGameExit_SwapCharacter()
  ...
  ToClient_padSnapRefreshTarget(deliveryForGameExit._comboBoxSwapCharacter)
end
