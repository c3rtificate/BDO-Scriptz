...
function PaGlobal_Delivery_Request:registEventHandler()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(PaGlobal_Delivery_Request._ui._buttonQuestion, "\"DeliveryRequest\"")
  if true == _ContentsOption_CH_Help then
    PaGlobal_Delivery_Request._ui._buttonQuestion:SetShow(false)
  end
...
function PaGlobal_Delivery_Request:prepareOpen()
  ...
  PaGlobal_Delivery_Request._ui.deliveryHelpDesc:SetTextMode(__eTextMode_AutoWrap)
...
function PaGlobal_Delivery_Request:update()
  ...
  local totalCount = 0
  local totalPrice = 0
  totalPrice = Int64toInt32(Toclient_NewDeliveryGetPackItemTotalPrice(DeliveryInformation_WaypointKey(), PaGlobal_Delivery_Request.selectWaypointKey, PaGlobal_Delivery_Request.selectDeliverer))
  ...
  local maxCarriageWeight = ToClient_NewDeliveryGetMaxWeightPerCarriage(selectedCarriageKey)
  if -1 ~= selectedCarriageIndex then
    local remainWeight = ToClient_NewDeliveryGetRemainWeight(selectedCarriageKey)
    PaGlobal_Delivery_Request._ui.staticText_WeightCount:SetText(tostring(math.floor(remainWeight / 10000)) .. "/" .. tostring(maxCarriageWeight / 10000) .. " " .. PAGetString(Defines.StringSheet_RESOURCE, "TRADEMARKET_STD_TXT_LT"))
  else
    PaGlobal_Delivery_Request._ui.staticText_WeightCount:SetText("0/" .. tostring(maxCarriageWeight / 10000) .. " " .. PAGetString(Defines.StringSheet_RESOURCE, "TRADEMARKET_STD_TXT_LT"))
  end
  PaGlobal_Delivery_Request:clearSlot()
  for ii = 0, PaGlobal_Delivery_Request.config.slotCount - 1 do
...
function PaGlobal_Delivery_Request:updateCarriage(destination)
  ...
  PaGlobal_Delivery_Request.usableCount = 0
  ...
  for ii = 0, size - 1 do
    ...
    if true == isView then
      ...
      PaGlobal_Delivery_Request.usableCount = PaGlobal_Delivery_Request.usableCount + 1
    end
  end
  local controlSize = math.max(PaGlobal_Delivery_Request.usableCount, 1)
  PaGlobal_Delivery_Request._ui.comboBox_Carriage:GetListControl():SetSize(PaGlobal_Delivery_Request._ui.comboBox_Carriage:GetSizeX(), 10 + controlSize * 20)
  PaGlobal_Delivery_Request._ui.staticText_TotalDeliverer:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_DELIVERYNEW_REMAIN_SENDCOUNT", "count", tostring(PaGlobal_Delivery_Request.usableCount)))
  if 0 == PaGlobal_Delivery_Request.usableCount then
...
function DeliveryRequest_PushPackingItem(warehouseSlotNo, s64_count)
  ...
  local selectedCarriageIndex = PaGlobal_Delivery_Request._ui.comboBox_Carriage:GetSelectIndex()
  local selectedCarriageKey = PaGlobal_Delivery_Request._ui.comboBox_Carriage:GetSelectKey()
  if -1 == selectedCarriageIndex or 0 == PaGlobal_Delivery_Request.usableCount then
