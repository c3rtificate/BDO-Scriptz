...
function PaGlobal_Cash_Revival_BuyItem_All:setConsole()
  if true == PaGlobal_Cash_Revival_BuyItem_All._isConsole then
    ...
    local keyGuideA = UI.getChildControl(self._ui.stc_keyGuide_ConsoleUI, "StaticText_Select_Console")
    local keyGuideB = UI.getChildControl(self._ui.stc_keyGuide_ConsoleUI, "StaticText_Cancel_Console")
    local keyGuideList = {keyGuideA, keyGuideB}
    PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyGuideList, self._ui.stc_keyGuide_ConsoleUI, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP)
  else
...
function PaGlobal_Cash_Revival_BuyItem_All:registEventHandler()
  ...
  registerEvent("EventSelfPlayerRevive", "PaGlobal_Cash_Revival_BuyItem_All_Close")
end
...
function PaGlobal_Cash_Revival_BuyItem_All:update()
  ...
  if true == _ContentsGroup_FamilyInventory then
    for familyInvenIdx = 0, ToClient_InventoryGetSize(CppEnums.ItemWhereType.eFamilyInventory) - 1 do
      local familyInvenItem = ToClient_GetInventoryItemByProductCategory(CppEnums.ItemWhereType.eFamilyInventory, __eContentsType_Respawn, familyInvenIdx)
  ...
  for cashInvenIdx = 0, ToClient_InventoryGetSize(CppEnums.ItemWhereType.eCashInventory) - 1 do
    cashInvenItem = ToClient_GetInventoryItemByProductCategory(CppEnums.ItemWhereType.eCashInventory, __eContentsType_Respawn, cashInvenIdx)
  ...
  for itemIdx = 0, PaGlobal_Cash_Revival_BuyItem_All._cashRevivalCount - 1 do
    if nil == PaGlobal_Cash_Revival_BuyItem_All._itemUIPOOL[itemIdx] then
      ...
      tempSlot.slot = SlotItem.new(tempSlot.slot, "Static_ItemIcon_" .. itemIdx, itemIdx, tempSlot.itemIconBG, slotConfig)
      tempSlot.slot:createChild()
      tempSlot.slot.icon:SetIgnore(true)
    ...
    local itemStaticStatusWrapper = getItemEnchantStaticStatus(PaGlobal_Cash_Revival_BuyItem_All._cashRevivalData[itemIdx].itemKey)
    if nil == itemStaticStatusWrapper then
      return
    end
    UiBase.slot:setItemByStaticStatus(itemStaticStatusWrapper, 0, -1, false, false, false, 0, 0, false, false, false, CppEnums.ItemWhereType.eFamilyInventory == PaGlobal_Cash_Revival_BuyItem_All._cashRevivalData[itemIdx].itemWhereType)
    UiBase.itemNameText:SetText(itemStaticStatusWrapper:getName() .. "(" .. tostring(PaGlobal_Cash_Revival_BuyItem_All._cashRevivalData[itemIdx].count) .. ")")
    UiBase.itemIconBG:addInputEvent("Mouse_LUp", "HandleEventLUp_Cash_Revival_BuyItem_All_SelectItem( " .. itemIdx .. " )")
  ...
  if PaGlobal_Cash_Revival_BuyItem_All._cashRevivalCount < 4 then
    ...
    Panel_Cash_Revival_BuyItem_All:ComputePos()
  end
  if true == _ContentsGroup_UsePadSnapping then
    self._ui.stc_keyGuide_ConsoleUI:ComputePos()
  end
end
