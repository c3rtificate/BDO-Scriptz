  ...
  for slotIndex = 0, self._config._maxShowChallengeCount - 1 do
    ...
    for index = 0, self._config._maxSelectItemSlotCount - 1 do
      if nil == selectItemSlotBG[index] then
        ...
        slot.classEquipBG:SetPosY(info.slotBG:GetSizeY() - UI.itemSlotConfig.disableClass)
...
function dailyChallenge:prepareOpen()
...
function dailyChallenge:setButtonState(control, challnegeType, dataIndex, existRewardCount, UiIndex)
  local eChallengeType = self._config._challengeType
  ...
  elseif eChallengeType.benefit == challnegeType then
    ...
    control:addInputEvent("Mouse_LUp", "PaGlobalFunc_DailyChallenge_ReceiveReward(" .. dataIndex .. "," .. UiIndex .. ")")
  elseif eChallengeType.reward == challnegeType then
    control:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_CHALLENGE_BTNGETREWARD", "existRewardCount", existRewardCount))
    control:addInputEvent("Mouse_LUp", "PaGlobalFunc_DailyChallenge_ReceiveReward(" .. dataIndex .. "," .. UiIndex .. ")")
    local isDone = false
...
function dailyChallenge:createList(UiIndex, dataIndex)
  ...
  self:setButtonState(_button_Receive, challengeInfo.challengeType, dataIndex, rewardCount, UiIndex)
  local baseCount = rewardInfo:getBaseRewardCount()
  local selectCount = rewardInfo:getSelectRewardCount()
  _staticText_RewardTitle:SetTextMode(__eTextMode_Limit_AutoWrap)
  ...
  _staticText_RewardDesc:SetTextMode(__eTextMode_Limit_AutoWrap)
...
function dailyChallenge:eraseRewardIconEffect()
  for UiIndex = 0, self._config._maxShowChallengeCount - 1 do
    local selectItemSlotBG = self._static_SelectRewardBg[UiIndex]
    for index = 0, self._config._maxSelectItemSlotCount - 1 do
      selectItemSlotBG[index].slot.icon:EraseAllEffect()
      selectItemSlotBG[index].slot.icon:EraseAllEffect()
    end
  end
end
function dailyChallenge:remindRewardIconEffect(UiIndex, selectCount)
  local selectItemSlotBG = self._static_SelectRewardBg[UiIndex]
  for index = 0, self._config._maxSelectItemSlotCount - 1 do
    if index < selectCount then
      selectItemSlotBG[index].slot.icon:AddEffect("fUI_Tuto_ItemHp_01A", false, 0, 0)
    end
  end
end
...
function dailyChallenge:showRewardItemTooltip(isShow, dataIndex, uiIndex, isBase)
  ...
  local uiSlot
  local rewardType = "Dialog_ChallengeReward_Base"
  if true == isBase then
    uiSlot = self._static_ItemSlotBg[dataIndex - 1]
  else
    rewardType = "Dialog_ChallengeReward_Select"
...
function PaGlobalFunc_DailyChallenge_ReceiveReward(dataIndex, UiIndex)
  dailyChallenge:receiveReward(dataIndex, UiIndex)
end
function dailyChallenge:receiveReward(dataIndex, UiIndex)
  local eChallengeType = self._config._challengeType
  local challengeInfo = self._dataList[dataIndex]
  ...
  dailyChallenge:eraseRewardIconEffect()
  if 0 ~= selectRewardCount and (nil == self._selectItemData._dataIndex or nil == self._selectItemData._slotIndex) then
    ...
    dailyChallenge:remindRewardIconEffect(UiIndex, selectRewardCount)
    return
  end
  ...
  if true == _ContentsGroup_ChallengeReward then
    ToClient_requestChallengeReward(challengeKey, self._selectItemData._slotIndex, 1)
  else
...
function FromClient_DailyChallenge_Update()
  ...
  if nil ~= PaGlobalFunc_DailyStamp_All_IsDailyChallengeShow and false == PaGlobalFunc_DailyStamp_All_IsDailyChallengeShow() then
    return
  end
  ...
  if false == isSafeZone then
    ToClient_getGameUIManagerWrapper():setLuaCacheDataListBool(__eDailyAttendanceUpdateCheckSafeZone, true, CppEnums.VariableStorageType.eVariableStorageType_User)
    return
  end
  dailyChallenge:prepareOpen()
end
function FromClient_DailyChallengeAll_Update(isNextDay)
  if false == dailyChallenge._isInitialized then
    return
  end
  local selfPlayer = getSelfPlayer()
...
function FromClient_DailyChallenge_regionCheck(regionData)
  if false == dailyChallenge._isInitialized then
    return
  end
  if nil == regionData then
    UI.ASSERT_NAME(nil ~= regionData, "FromClient_DailyChallenge_regionCheck regionData nil", "\236\160\149\236\167\128\237\152\156")
    return
  end
  if true == _ContentsGroup_RenewUI and GetUIMode() ~= Defines.UIMode.eUIMode_Default then
    return
  end
  if true == ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(__eDailyAttendanceUpdateCheckSafeZone) then
    local isSafeZone = regionData:get():isSafeZone()
    if true == isSafeZone and false == Panel_Window_DailyChallenge:GetShow() then
      dailyChallenge:prepareOpen()
    end
  end
end
function dailyChallenge:registEventHandler()
  ...
  registerEvent("selfPlayer_regionChanged", "FromClient_DailyChallenge_regionCheck")
end
