...
function PaGlobal_DailyStamp_All:controlAll_Init()
  ...
  self._ui.stc_upperScreen = UI.getChildControl(self._ui.stc_centerBg, "Static_UpperScreen")
  self._ui.stc_guildDesc = UI.getChildControl(self._ui.stc_upperScreen, "StaticText_GuildDesc")
...
function PaGlobal_DailyStamp_All:controlSetShow()
  ...
  if false == _ContentsGroup_UsePadSnapping then
    ...
    self._ui_pc.btn_tabTitle:SetShow(false)
    self._ui.stc_topBg:SetShow(false)
...
function PaGlobal_DailyStamp_All:tabInit()
  ...
  local tabCount = self._dailyStampCount
  local gabX = 10
  local topBgSizeX = self._ui_console.stc_topBg:GetSizeX()
  if true == _ContentsGroup_UsePadSnapping then
    local consoleGap = self._ui_console.stc_iconLB:GetSizeX() + self._ui_console.stc_iconLB:GetSpanSize().x
    if tabCount <= 2 then
      self._ui_console.stc_tabTitle:SetSize(self._originTabSizeX, self._ui_console.stc_tabTitle:GetSizeY())
    else
      local btnSizeX = (topBgSizeX - consoleGap * 2 - gabX * (tabCount - 1)) / tabCount
      self._ui_console.stc_tabTitle:SetSize(btnSizeX, self._ui_console.stc_tabTitle:GetSizeY())
    end
    local controlSizeX = self._ui_console.stc_tabTitle:GetSizeX()
    local startPosX = (topBgSizeX - consoleGap * 2 - controlSizeX * tabCount - gabX * (tabCount - 1)) / 2 + consoleGap
    for idx = 1, tabCount do
      self._tabTitlePos[idx] = startPosX + (controlSizeX + gabX) * (idx - 1)
    end
    PaGlobal_DailyStamp_All:consoleTabUpdate()
  else
    if tabCount <= 3 then
      self._ui_pc.btn_tabTitle:SetSize(self._originTabSizeX, self._ui_pc.btn_tabTitle:GetSizeY())
    else
      local btnSizeX = (topBgSizeX - gabX * (tabCount - 1) - 20) / tabCount
      self._ui_pc.btn_tabTitle:SetSize(btnSizeX, self._ui_pc.btn_tabTitle:GetSizeY())
    end
    local controlSizeX = self._ui_pc.btn_tabTitle:GetSizeX()
    local startPosX = (topBgSizeX - controlSizeX * tabCount - gabX * (tabCount - 1)) / 2
    for idx = 1, tabCount do
      self._tabTitlePos[idx] = startPosX + (controlSizeX + gabX) * (idx - 1)
    end
...
function PaGlobal_DailyStamp_All:pcTabUpdate()
  ...
  local function tabControlSet(index)
    ...
    self._tabControl[index]:SetTextMode(__eTextMode_AutoWrap)
    self._tabControl[index]:SetText(dailyStampTabName)
    self._tabControl[index]:SetPosX(self._tabTitlePos[index])
...
function PaGlobal_DailyStamp_All:dailyInit()
  ...
  for index = 0, self._rewardCount - 1 do
    local temp = {}
    temp._dayControl = UI.createControl(__ePAUIControl_CheckButton, self._ui.stc_buttonGroup, "Button_RewardSlot_" .. index)
    ...
    temp._dayText = UI.createControl(__ePAUIControl_StaticText, temp._dayControl, "StaticText_Day_" .. index)
    ...
    temp._slotItemBg = UI.createControl(__ePAUIControl_Static, temp._dayControl, "Static_RewardIconBG_" .. index)
    ...
    temp._stamp = UI.createControl(__ePAUIControl_Static, temp._dayControl, "Static_Stamp_" .. index)
...
function PaGlobal_DailyStamp_All:descInit()
  ...
  self._ui.txt_eventPeriod:SetTextMode(__eTextMode_AutoWrap)
  self._ui.txt_acceptPeriod:SetTextMode(__eTextMode_AutoWrap)
  self._ui.txt_eventAlert:SetTextMode(__eTextMode_AutoWrap)
  self._ui.txt_weekEndDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  local eventAlertStr = ""
  eventAlertStr = self._ui.txt_eventAlert:GetText()
  self._ui.txt_eventAlert:SetText(eventAlertStr)
...
function PaGlobal_DailyStamp_All:prepareOpen()
  ...
  if self._dailyStampCount ~= currentCount then
    ...
    for index = 1, self._dailyStampCount do
      ...
      if nil ~= self._dailyStampKeys[index] and nil ~= self._dailyStampKeys[index][1] and false == self._dailyStampKeys[index][1]:isPossibleOverlapAttendance() and false == self._dailyStampKeys[index][1]:isPossibleAttendance() then
        self._dailyStampKeys[index][3] = true
      end
    end
    self:tabInit()
  ...
  ToClient_getGameUIManagerWrapper():setLuaCacheDataListBool(__eDailyAttendanceUpdateCheckSafeZone, false, CppEnums.VariableStorageType.eVariableStorageType_User)
  ToClient_getGameUIManagerWrapper():setLuaCacheDataListTime(__eDailyAttendanceShowToday, ToClient_GetThisYear(), ToClient_GetThisMonth(), ToClient_GetToday(), 0, 0, 0, CppEnums.VariableStorageType.eVariableStorageType_User)
...
function PaGlobal_DailyStamp_All:resize()
  ...
  self._ui.txt_acceptPeriod:SetPosY(self._ui.txt_eventPeriod:GetPosY() + self._ui.txt_eventPeriod:GetTextSizeY())
  self._ui.txt_eventAlert:SetPosY(self._ui.txt_acceptPeriod:GetPosY() + self._ui.txt_acceptPeriod:GetTextSizeY())
  self._ui.txt_weekEndDesc:SetPosY(self._ui.txt_eventAlert:GetPosY() + self._ui.txt_eventAlert:GetTextSizeY())
  local descTotalSizeY = self._ui.txt_eventPeriod:GetTextSizeY() + self._ui.txt_acceptPeriod:GetTextSizeY() + self._ui.txt_eventAlert:GetTextSizeY() + self._ui.txt_weekEndDesc:GetTextSizeY()
  self._ui.stc_bottomBg:SetSize(self._ui.stc_bottomBg:GetSizeX(), descTotalSizeY + 20)
  self._ui.stc_bottomBg:SetPosY(self._ui.stc_centerBg:GetPosY() + self._ui.stc_centerBg:GetSizeY())
  local panelSizeY = self._ui.stc_mainTitleBg:GetSizeY() + self._ui.stc_topBg:GetSizeY() + self._ui.stc_centerBg:GetSizeY() + self._ui.stc_bottomBg:GetSizeY()
  Panel_Window_DailyStamp_All:SetSize(Panel_Window_DailyStamp_All:GetSizeX(), panelSizeY + 20)
  Panel_Window_DailyStamp_All:SetPosX(getOriginScreenSizeX() / 2 - Panel_Window_DailyStamp_All:GetSizeX() / 2)
  Panel_Window_DailyStamp_All:SetPosY(getOriginScreenSizeY() / 2 - Panel_Window_DailyStamp_All:GetSizeY() / 2)
  if false == _ContentsGroup_UsePadSnapping then
    ...
    PaGlobalFunc_ConsoleKeyGuide_SetAlign(self._keyGuideAlign, self._ui_console.stc_keyGuideBg, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP)
...
function PaGlobal_DailyStamp_All:registEventHandler()
  ...
  registerEvent("FromClient_AttendanceNoticeView", "FromClient_AttendanceNoticeView_DailyStamp")
  ...
  registerEvent("FromClient_OpenDailyStamp", "FromClient_OpenDailyStamp")
...
function PaGlobal_DailyStamp_All:setData(tabIndex)
  ...
  local isGuildAttendance = ToClient_IsGuildAttendance(attendanceKey)
  local checkGuildAttendanceCondition = ToClient_CheckGuildAttendanceCondition(attendanceKey)
  ...
  for index = 0, totalDayCount - 1 do
    ...
    elseif false == _ContentsGroup_RenewUI_Tooltip then
      self._dayControl[index]._dayControl:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandlePadEvent_DailyStamp_All_ItemTooltipToggle(" .. index .. "," .. tabIndex .. ")")
      self._dayControl[index]._dayControl:addInputEvent("Mouse_Out", "HandleEventOnOut_DailyStamp_All_ItemTooltip()")
    end
  ...
  if isGuildAttendance == true and checkGuildAttendanceCondition == false then
    self._ui.stc_upperScreen:SetShow(true)
    self._ui.stc_guildDesc:SetShow(true)
    self._ui.stc_upperScreen:SetSize(self._ui.stc_centerBg:GetSizeX(), self._ui.stc_centerBg:GetSizeY())
    self._ui.stc_guildDesc:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_DAILYSTAMP_GUILDDESC"))
  else
    self._ui.stc_upperScreen:SetShow(false)
    self._ui.stc_guildDesc:SetShow(false)
  end
...
function PaGlobal_DailyStamp_All:dailyTabSort()
  ...
  local isGuildAttendance = false
  local checkGuildAttendanceCondition = false
  ...
  for index = 1, self._dailyStampCount do
    if nil ~= self._dailyStampKeys[index][1] then
      ...
      isGuildAttendance = ToClient_IsGuildAttendance(attendanceKey)
      checkGuildAttendanceCondition = ToClient_CheckGuildAttendanceCondition(attendanceKey)
    end
    if rewardCount == totalDayCount or isGuildAttendance == true and checkGuildAttendanceCondition == false then
      tabSort[back] = index
      back = back - 1
    else
      tabCheck = true
  ...
  for index = 1, self._dailyStampCount do
    self._tabControl[tabSort[index]]:SetPosX(self._tabTitlePos[index])
  end
...
function PaGlobalFunc_DailyStamp_All_StampAnimation(deltaTime)
  ...
  if false == PaGlobal_DailyStamp_All._rewardRemindShow then
    PaGlobal_DailyStamp_All._rewardRemindTime = PaGlobal_DailyStamp_All._rewardRemindTime + deltaTime
    if PaGlobal_DailyStamp_All._rewardRemindTime > 1 then
      PaGlobal_DailyStamp_All._rewardRemindShow = true
      if false == PaGlobal_DailyStamp_All._nextDayShow then
        PaGlobalFunc_Daily_Reward_Notice_All_Open()
      end
    end
  end
  if PaGlobal_DailyStamp_All._tabIndex == PaGlobal_DailyStamp_All._animationTabIndex and PaGlobal_DailyStamp_All._animationTime > 0.3 and PaGlobal_DailyStamp_All._animationTime < 1 then
    for index = 0, #PaGlobal_DailyStamp_All._animationdayIndex do
      if nil ~= PaGlobal_DailyStamp_All._animationdayIndex[index] and PaGlobal_DailyStamp_All._animationdayIndex[index] <= PaGlobal_DailyStamp_All._rewardCount - 1 then
