function PaGlobalFunc_DailyStamp_All_Open(isManualOpen)
  if isManualOpen == false and ToClient_UserHasOverLevelCharacter(60) == false then
    return
  end
...
function PadEventLBRB_DailyStamp_All_SelectTab(addIndex)
  ...
  elseif tabIndex > PaGlobal_DailyStamp_All._dailyStampCount then
...
function HandlePadEvent_DailyStamp_All_ItemTooltipToggle(index, rewardIndex)
  if nil == Panel_Window_DailyStamp_All then
    return
  end
  if false == _ContentsGroup_NewUI_Tooltip_All and nil ~= Panel_Tooltip_Item and true == Panel_Tooltip_Item:GetShow() then
    Panel_Tooltip_Item_hideTooltip()
    return
  end
  if nil == index or nil == rewardIndex then
...
function FromClient_DailyStamp_All_AttendanceUpdate(attendanceKey)
  ...
  PaGlobal_DailyStamp_All._isFirstAttendanceChecked = PaGlobal_DailyStamp_All._secondAttendanceCheck
  for index = 1, PaGlobal_DailyStamp_All._dailyStampCount do
    ...
    if dailyKey:get() == attendanceKey:get() then
      ...
      PaGlobal_DailyStamp_All:selectTab(index)
  ...
  if true == PaGlobal_DailyStamp_All._nextDayShow or true == PaGlobal_DailyStamp_All._isFirstAttendanceChecked then
...
function FromClient_DailyStamp_All_AttendanceUpdateAll(isNextDay)
  ...
  PaGlobalFunc_DailyStamp_All_CheckAttandance()
end
...
function FromClient_DailyStamp_All_regionCheck(regionData)
  ...
  if true == PaGlobal_DailyStamp_All._nextDayShow and false == Panel_Window_DailyStamp_All:GetShow() then
    PaGlobal_DailyStamp_All._nextDayShow = false
    return
  end
  if true == PaGlobal_DailyStamp_All._isFirstAttendanceChecked and false == Panel_Window_DailyStamp_All:GetShow() then
    PaGlobal_DailyStamp_All._isFirstAttendanceChecked = false
    return
  end
end
...
function FromClient_OpenDailyStamp()
  if PaGlobal_DailyStamp_All._rewardRemindShow == false then
    PaGlobalFunc_DailyStamp_All_Open(true)
    luaTimer_AddEvent(HandleEventLUp_PaGlobal_Daily_Reward_Notice_All_CloseAnimation, 5000, false, 0)
  end
end
function PaGlobalFunc_DailyStamp_All_CheckAttandance()
  if nil == Panel_Window_DailyStamp_All then
    return
  end
  if nil == getSelfPlayer() then
