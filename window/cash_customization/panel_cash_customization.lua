...
function CashCustomization:Initialize()
  ...
  TemplateUI.ItemName:SetTextMode(__eTextMode_LimitText)
  TemplateBuyPanelUI.ItemName:SetTextMode(__eTextMode_AutoWrap)
  local pearlAmount = Defines.s64_const.s64_0
  ...
  if pearlItemWrapper ~= nil then
    pearlAmount = pearlItemWrapper:get():getCount_s64()
  end
  self.PearlValue:SetText(makeDotMoney(pearlAmount))
end
function CashCustomization:Update()
  ...
  for invenIdx = 0, ToClient_InventoryGetSize(CppEnums.ItemWhereType.eInventory) - 1 do
    invenItem = ToClient_GetInventoryItemByProductCategory(CppEnums.ItemWhereType.eInventory, __eContentsType_Customize, invenIdx)
  ...
  for cashInvenIdx = 0, ToClient_InventoryGetSize(CppEnums.ItemWhereType.eCashInventory) - 1 do
    cashInvenItem = ToClient_GetInventoryItemByProductCategory(CppEnums.ItemWhereType.eCashInventory, __eContentsType_Customize, cashInvenIdx)
  ...
  for cumaIdx = 0, CashCustomizationDataCount - 1 do
    if nil == self.ItemUiPOOL[cumaIdx] then
      local tempSlot = {}
      local CreateItemBG = UI.createControl(__ePAUIControl_Static, self.myProduct_panelBG, "CashCustomization_ItemBG_" .. cumaIdx)
      ...
      local CreateItemNameBG = UI.createControl(__ePAUIControl_Static, CreateItemBG, "CashCustomization_ItemNameBG_" .. cumaIdx)
      ...
      local CreateItemName = UI.createControl(__ePAUIControl_StaticText, CreateItemBG, "CashCustomization_ItemName_" .. cumaIdx)
      ...
      local CreateItemPeriod = UI.createControl(__ePAUIControl_StaticText, CreateItemBG, "CashCustomization_ItemPeriod_" .. cumaIdx)
...
function CashCumaBuy:Update(applyType)
  ...
  elseif applyType == "pearl" then
    ...
    for productIdx = 0, productSize - 1 do
      ...
      if product ~= nil then
        local cashPrice = 0
        if __ePriceType_Cash == product:getProductPriceType() then
          cashPrice = product:getProductPrice()
        end
        CashCustomizationData[idx] = {}
        CashCustomizationData[idx].name = product:getName()
        CashCustomizationData[idx].productNo = product:getNoRaw()
        CashCustomizationData[idx].cashPrice = cashPrice
  ...
  for cumaIdx = 0, CashCustomizationDataCount - 1 do
    if nil == self.ItemUiPOOL[cumaIdx] then
      local tempSlot = {}
      local CreateItemBG = UI.createControl(__ePAUIControl_Static, Panel_Cash_Customization_BuyItem, "CashCustomizationBuy_ItemBG_" .. cumaIdx)
      CopyBaseProperty(TemplateBuyPanelUI.ItemBG, CreateItemBG)
      CreateItemBG:SetPosX(25)
      CreateItemBG:SetPosY(80)
      CreateItemBG:SetShow(false)
      tempSlot.ItemBG = CreateItemBG
      local CreateItemNameBG = UI.createControl(__ePAUIControl_Static, CreateItemBG, "CashCustomizationBuy_ItemNameBG_" .. cumaIdx)
      CopyBaseProperty(TemplateBuyPanelUI.ItemNameBG, CreateItemNameBG)
      CreateItemNameBG:SetPosX(5)
      CreateItemNameBG:SetPosY(5)
      CreateItemNameBG:SetShow(true)
      tempSlot.ItemNameBG = CreateItemNameBG
      local CreateItemSelectBTN = UI.createControl(__ePAUIControl_RadioButton, CreateItemBG, "CashCustomizationBuy_ItemSelectBTN_" .. cumaIdx)
      ...
      local CreateItemName = UI.createControl(__ePAUIControl_StaticText, CreateItemBG, "CashCustomizationBuy_ItemName_" .. cumaIdx)
      CopyBaseProperty(TemplateBuyPanelUI.ItemName, CreateItemName)
      CreateItemName:SetPosX(20)
      CreateItemName:SetPosY(3)
      CreateItemName:SetShow(true)
      tempSlot.ItemName = CreateItemName
      local CreateItemPeriod = UI.createControl(__ePAUIControl_StaticText, CreateItemBG, "CashCustomizationBuy_ItemPeriod_" .. cumaIdx)
      ...
      tempSlot.ItemPeriod = CreateItemPeriod
      local CreatePearlIcon = UI.createControl(__ePAUIControl_Static, CreateItemBG, "CashCustomizationBuy_PearlIcon_" .. cumaIdx)
      ...
      local CreateItemPrice = UI.createControl(__ePAUIControl_StaticText, CreateItemBG, "CashCustomizationBuy_ItemPrice_" .. cumaIdx)
...
function HandleClicked_CashCustomization_Apply()
  ...
  if 0 < ToClient_InventorySizeByProductCategory(CppEnums.ItemWhereType.eInventory, __eContentsType_Customize) then
    if true == _ContentsGroup_RenewUI_Customization then
      CashCumaBuy_OpenForConsole()
    else
      CashCumaBuy_Open("inven")
    end
  elseif 0 < ToClient_InventorySizeByProductCategory(CppEnums.ItemWhereType.eCashInventory, __eContentsType_Customize) then
    if true == _ContentsGroup_RenewUI_Customization then
      CashCumaBuy_OpenForConsole()
    else
      CashCumaBuy_Open("inven")
...
function HandleClicked_CashCumaBuy_Confirm()
  ...
  local isJapan = CppEnums.CountryType.JPN_ALPHA == gameServiceType or CppEnums.CountryType.JPN_REAL == gameServiceType
  ...
  if true == isJapan or true == isJapanConsole then
    local pearlPrice = CashCustomizationData[self.SelectedItemNo].pearlPrice
    if nil ~= pearlPrice then
      local buyMethod = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_PEARL")
      local priceMsg = "\n" .. PAGetStringParam2(Defines.StringSheet_GAME, "LUA_GAMESHOP_NOTICE_14", "pearl", tostring(CashCustomizationData[self.SelectedItemNo].pearlPrice), "buyMethod", buyMethod)
      msgContent = msgContent .. priceMsg .. "\n" .. PAGetString(Defines.StringSheet_GAME, "LUA_GAMESHOP_NOTICE_13")
    end
  end
  local messageboxData = {
    title = msgTitle,
...
function ToClient_CashCumaBuy_Confirm_Do()
  ...
  elseif CashCumaBuy.ApplyType == "goods" then
    ToClient_requestBuyItem(CashCustomizationData[self.SelectedItemNo].productNo, 1, CashCustomizationData[self.SelectedItemNo].pearlPrice, CppEnums.BuyItemReqTrType.eBuyItemReqTrType_UiUpdate, toInt64(0, -1), 0, CppEnums.ItemWhereType.eInventory, false, CppEnums.ItemWhereType.eCount, __eTInventorySlotNoUndefined)
  elseif CashCumaBuy.ApplyType == "pearl" then
    ToClient_requestBuyItem(CashCustomizationData[self.SelectedItemNo].productNo, 1, CashCustomizationData[self.SelectedItemNo].cashPrice, CppEnums.BuyItemReqTrType.eBuyItemReqTrType_UiUpdate, toInt64(0, -1), 0, CppEnums.ItemWhereType.eInventory, false, CppEnums.ItemWhereType.eCount, __eTInventorySlotNoUndefined)
  else
    ToClient_requestBuyItem(CashCustomizationData[self.SelectedItemNo].productNo, 1, CashCustomizationData[self.SelectedItemNo].pearlPrice, CppEnums.BuyItemReqTrType.eBuyItemReqTrType_ImmediatelyUse, toInt64(0, -1), 0, CppEnums.ItemWhereType.eInventory, false, CppEnums.ItemWhereType.eCount, __eTInventorySlotNoUndefined)
  end
end
...
function PaGlobal_Panel_Cash_Customization_GetApplyButtonPosY()
  local self = CashCustomization
  return self.BTN_Apply:GetPosY()
end
