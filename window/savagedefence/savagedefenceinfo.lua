...
function FGlobal_SavegeDefenceInfo_Initionalize()
  ...
  self._stcTitleBG = UI.getChildControl(Panel_SavageDefenceInfo, "Static_TitleBG")
  self._txtTitle = UI.getChildControl(self._stcTitleBG, "StaticText_Title")
  self._btnClose = UI.getChildControl(self._stcTitleBG, "Button_Win_Close")
  ...
  self._list2:registEvent(__ePAUIList2EventType_LuaChangeContent, "FGlobal_SavegeDefenceInfo_ListUpdate")
...
function FGlobal_SavegeDefenceInfo_ListUpdate(contents, key)
  ...
  if nil ~= curChannelData then
    ...
    local joinWave = ToClient_getSavageDefenceJoinWave()
    ...
    if 0 == getCurrentState then
      ...
      getWave = 0
    elseif 1 == getCurrentState or 2 == getCurrentState and joinWave >= getWave then
      ...
      if 2 == getCurrentState and joinWave >= getWave then
        savageDefenceListJoinBtn:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_LOCALWARINFO_JOIN"))
      else
...
function FGlobal_SavageDefenceInfo_Close()
  Panel_SavageDefenceInfo:ClearUpdateLuaFunc()
...
function FGlobal_SavegeDefenceInfo_InformationOpen(deltaTime)
  local self = PaGlobal_SavegeDefenceInfo
  local gapY = 10
  local contentGapY = self._desc_Rule_Title:GetSizeY() + self._desc_Reward_Title:GetSizeY() + self._desc_Explanation_Title:GetSizeY() + gapY * 2
  if self._desc_Rule_Title:IsCheck() then
    ...
    self._frame_VScroll:SetShow(self._frame:GetSizeY() < value + contentGapY)
    self._frame_Content:SetSize(self._frame_Content:GetSizeX(), value + contentGapY)
    ...
    self._desc_rule:SetSize(self._desc_rule:GetSizeX(), value)
    if 10 >= self._desc_rule:GetSizeY() then
      self._desc_rule:SetShow(false)
    end
  end
...
function FGlobal_SavageDefenceInfo_ESCMenuFunc()
