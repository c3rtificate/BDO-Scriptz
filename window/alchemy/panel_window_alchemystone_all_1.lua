...
function PaGlobal_AlchemyStone_All:ControlAll_Init()
  ...
  local skipAniBg = UI.getChildControl(Panel_Window_AlchemyStone_All, "CheckButton_SkipBG")
  self._ui.chk_skipAni = UI.getChildControl(skipAniBg, "CheckButton_Skip")
...
function PaGlobal_AlchemyStone_All:selectTab(tabType)
  ...
  elseif self._TAB_TYPE.UPGRADE == tabType then
    ...
    if false == self._isConsole then
      self._ui.txt_descDetail:SetPosY(self._descDetail_OriginPosY + tempSizeY / 2 + 3)
      Panel_Window_AlchemyStone_All:SetSize(Panel_Window_AlchemyStone_All:GetSizeX(), self._panel_OriginSizeY + tempSizeY)
    end
...
function PaGlobal_AlchemyStone_All:invenStoneRClick(slotNo, itemWrapper, count, inventoryType)
  ...
  elseif self._TAB_TYPE.UPGRADE == self._selectedTabIdx then
    ...
    if true == self._isConsole then
      ...
      local needItemKey = ToClient_GetAlchemyEvolveNeedItemKey(itemSSW:get()._key:getItemKey())
      self._blackStonSlotNo = ToClient_InventoryGetSlotNo(needItemKey)
      if nil ~= self._blackStonSlotNo and 255 ~= self._blackStonSlotNo then
        local blackStoneWrapper = getInventoryItemByType(inventoryType, self._blackStonSlotNo)
        self:invenMatRClick(self._blackStonSlotNo, blackStoneWrapper, toInt64(0, 1), inventoryType)
      end
...
function PaGlobal_AlchemyStone_All:expTab_UnSetSlot(slotType)
  ...
  if PaGlobal_AlchemyStone_All._SLOT_TYPE.STONE == slotType then
...
function test()
  ToClient_requestCookNAlchemySimulateDEVReq(17565, 1, 1000000)
end
function test2(...)
  ToClient_requestCookNAlchemySimulateDEVReq(17001, 0, 100000000)
end
function test3(...)
  ToClient_requestCookNAlchemySimulateDEVReq(18149, 2, 10000000)
end
