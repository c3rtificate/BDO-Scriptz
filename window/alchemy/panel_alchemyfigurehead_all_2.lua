...
function PaGlobal_AlchemyFigureHead_All_IsShow()
  if Panel_AlchemyFigureHead_All == nil then
    return nil
  end
  return Panel_AlchemyFigureHead_All:GetShow()
end
...
function PaGlobal_AlchemyFigureHead_All_FigureHeadRfunction(slotNo, itemWrapper, count, inventoryType)
  ...
  if PaGlobal_AlchemyFigureHead_All._isConsole then
    if ToClient_isConsole() then
      ...
      PaGlobal_AlchemyFigureHead_All._ui.stc_totemSlotBG:registerPadEvent(__eConsoleUIPadEvent_X, "HandleEventOnOut_AlchemyFigureHead_All_ItemToolTipToggle( true )")
  ...
  local function setStuffFunc(itemCount)
    ...
    if PaGlobal_AlchemyFigureHead_All._isConsole then
      if ToClient_isConsole() then
        ...
        PaGlobal_AlchemyFigureHead_All._ui.stc_blackstoneSlotBG:registerPadEvent(__eConsoleUIPadEvent_X, "HandleEventOnOut_AlchemyFigureHead_All_ItemToolTipToggle( false )")
...
function HandleEventOnOut_AlchemyFigureHead_All_ItemToolTipToggle(isTotem)
  if _ContentsGroup_NewUI_Tooltip_All == true or Panel_Tooltip_Item ~= nil and Panel_Tooltip_Item:GetShow() == true then
    Panel_Tooltip_Item_hideTooltip()
    return
  end
  local slotNo = PaGlobal_AlchemyFigureHead_All._totemData.slotNo
...
function FromClient_AlchemyFigureHead_All_ReSizePanel()
  ...
  Panel_AlchemyFigureHead_All:SetPosX(invenPosX - Panel_AlchemyFigureHead_All:GetSizeX() - 5)
  Panel_AlchemyFigureHead_All:SetPosY(invenPosY)
end
