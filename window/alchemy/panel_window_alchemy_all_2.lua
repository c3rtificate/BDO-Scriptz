...
function PaGlobal_Alchemy_All_CreateKnowledgeListContent(content, key)
  ...
  if nil ~= mentalCardStaticWrapper then
    ...
    if false == PaGlobal_Alchemy_All._isConsole then
      ...
      button:registerPadEvent(__eConsoleUIPadEvent_LTPress_Y, "PaGlobal_Alchemy_All_SetShowRecentCook(" .. knowledgeIndex .. ", true)")
      PaGlobal_Alchemy_All._ui.stc_iconX:SetShow(true)
      PaGlobal_Alchemy_All._ui.stc_iconRS:SetShow(true)
    end
    button:SetShow(true)
  else
...
function PaGlobal_Alchemy_All_SetShowRecentCook(knowledgeIndex, isShow)
  local mentalCardStaticWrapper = ToClient_AlchemyGetKnowledge(knowledgeIndex)
  if nil ~= PaGlobal_RecentCook then
    PaGlobal_RecentCook:closePanel()
  end
  local isLearn = ToClient_AlchemyIsLearntMentalCard(mentalCardStaticWrapper:getKey())
  PaGlobal_Alchemy_All._ui.stc_iconLTY:SetShow(isLearn)
  PaGlobal_Alchemy_All:setAlignKeyGuide()
  if PaGlobal_Alchemy_All._isWantShowRecentCook == isShow then
    return
  end
  PaGlobal_Alchemy_All._isWantShowRecentCook = isShow
  if true == PaGlobal_Alchemy_All._isWantShowRecentCook then
    if true == isLearn then
      ToClient_AlchemyRequestRecord(knowledgeIndex)
    end
  elseif nil ~= PaGlobal_RecentCook then
    PaGlobal_RecentCook:closePanel()
  end
end
function PaGlobal_Alchemy_All_SelectKnowledge(knowledgeIndex)
  local mentalCardStaticWrapper = ToClient_AlchemyGetKnowledge(knowledgeIndex)
  if nil ~= mentalCardStaticWrapper then
    if false == PaGlobal_Alchemy_All._isConsole and nil ~= PaGlobal_RecentCook then
      PaGlobal_RecentCook:closePanel()
      local isLearn = ToClient_AlchemyIsLearntMentalCard(mentalCardStaticWrapper:getKey())
      if true == isLearn then
        PaGlobal_RecentCook:showPanel(knowledgeIndex, PaGlobal_Alchemy_All._isCook, Panel_Window_Alchemy_All:GetPosX() - Panel_RecentCook:GetSizeX() - 5, Panel_Window_Alchemy_All:GetPosY())
      end
    end
...
function PaGlobal_Alchemy_All_ScrollMaterialDescription(isUp)
  if isUp == nil then
    return
  end
  local self = PaGlobal_Alchemy_All
  if self == nil then
    return
  end
  local currentPos = self._ui.scroll_vertical:GetControlPos()
  if isUp == true then
  ...
  self._ui.scroll_vertical:SetControlPos(currentPos)
  self._ui.frame_descBG:UpdateContentScroll()
  self._ui.frame_descBG:UpdateContentPos()
end
...
function PaGlobal_Alchemy_All_InventoryOpen()
  ...
  if nil ~= currentSnapControl and currentSnapControl:GetID() == PaGlobal_Alchemy_All._ui.btn_rateView:GetID() then
    return
  end
...
function FromClient_UpdateRecenltlyAlchemyList()
  local self = PaGlobal_Alchemy_All
  if self == nil then
    return
  end
  local panel = Panel_Window_Alchemy_All
  if panel == nil then
    return
  end
  if panel:GetShow() == true and self._ui.chk_knownRecipe:IsCheck() == true then
...
function HandleEventOnOut_Alchemy_All_ShowMaterialTooltip(isShow, slotIndex)
  if true == PaGlobal_Alchemy_All._isConsole then
    PaGlobal_Alchemy_All._ui.stc_iconLTY:SetShow(false)
  end
...
function HandleEventRUp_Alchemy_All_PopMaterial(slotIndex)
  ...
  if _ContentsGroup_LifeSimulator == true then
    if PaGlobal_Alchemy_All._isCook == true then
...
function HandleEventLUp_Alchemy_All_CheckKnownRecipeOnly(isRecent)
  PaGlobal_Alchemy_All:updateCheckButton(isRecent)
  PaGlobal_Alchemy_All:updateKnowledgeList()
end
function HandleEventLUp_Alchemy_All_CheckKnownRecipeOnlyForPad(isRecent)
  if isRecent == true then
    local isChecked = PaGlobal_Alchemy_All._ui.chk_knownRecipe:IsCheck()
    PaGlobal_Alchemy_All._ui.chk_knownRecipe:SetCheck(not isChecked)
  else
    local isChecked = PaGlobal_Alchemy_All._ui.chk_onlyKnownRecipe:IsCheck()
    PaGlobal_Alchemy_All._ui.chk_onlyKnownRecipe:SetCheck(not isChecked)
  end
  PaGlobal_Alchemy_All:updateCheckButton(isRecent)
  PaGlobal_Alchemy_All:updateKnowledgeList()
end
function HandleEventLUp_Alchemy_All_SearchButton()
  ...
  if PaGlobal_Alchemy_All._ui.chk_knownRecipe:IsCheck() == true then
    ToClient_setRecentlyProcessAlchemyFilter(PaGlobal_Alchemy_All._isCook, PaGlobal_Alchemy_All._filterText)
  else
...
function HandleEventLUp_Alchemy_All_SelectKnowledgeAndTooltip(isShow, index)
  ...
  if true == PaGlobal_Alchemy_All._isConsole then
    ...
    PaGlobal_Alchemy_All._ui.stc_iconRS:SetShow(true)
    PaGlobal_Alchemy_All:setAlignKeyGuide()
    PaGlobal_Alchemy_All_SetShowRecentCook(index, false)
  end
end
function PaGlobalFunc_Alchemy_All_IsCook()
  if nil == Panel_Window_Alchemy_All or false == Panel_Window_Alchemy_All:GetShow() then
    return false
  end
  return PaGlobal_Alchemy_All._isCook
end
function PaGlobalFunc_Alchemy_All_ShowRateViewButton(isShow)
  if Panel_Window_Alchemy_All == nil or Panel_Window_Alchemy_All:GetShow() == false then
    return
  end
  if isShow == true then
    PaGlobal_Alchemy_All._ui.btn_rateView:SetShow(true)
  else
    PaGlobal_Alchemy_All._ui.btn_rateView:SetShow(false)
  end
end
function HandleEventLUp_Alchemy_All_OpenRateViewList()
  ...
  if Panel_RecentCook ~= nil and Panel_RecentCook:GetShow() == true then
    PaGlobal_Alchemy_All._isPanelRecentCookOpened = true
  else
