  ...
  if CppEnums.ItemWhereType.eInventory == materialWhereType or CppEnums.ItemWhereType.eCashInventory == materialWhereType then
    if false == PaGlobal_Manufacture_All._isConsole then
      ...
      local optionalPadEvent = {
        func = PaGlobal_Manufacture_All_InventoryYFunc,
        button = __eConsoleUIPadEvent_Up_Y,
        keyGuideString = PAGetString(Defines.StringSheet_GAME, "ALCHEMY_MANUFACTURE_BTN_MANUFACTURE")
      }
      Inventory_SetFunctor(PaGlobal_Manufacture_All_InventoryFilterFunc, PaGlobal_Manufacture_All_InventoryRClickFunc, PaGlobalFunc_Manufacture_All_Close, nil)
    end
  else
...
function PaGlobalFunc_Manufacture_All_IsShow()
  if Panel_Window_Manufacture_All == nil then
    return nil
  end
  return Panel_Window_Manufacture_All:GetShow()
end
...
function PaGlobal_Manufacture_All_SelectManufactureAction(index, isClear, ignoreAction, isRepeat)
  ...
  if CppEnums.ItemWhereType.eInventory == PaGlobal_Manufacture_All._materialItemWhereType or CppEnums.ItemWhereType.eCashInventory == PaGlobal_Manufacture_All._materialItemWhereType then
    if false == PaGlobal_Manufacture_All._isConsole then
      ...
      Inventory_SetFunctor(PaGlobal_Manufacture_All_InvenFiler, PaGlobal_Manufacture_All_PushItemFromInventory, nil, nil, optionalPadEvent)
    end
...
function PaGlobal_Manufacture_All_PushItemFromInventory(slotNo, itemWrapper, count, inventoryType, preSelectedIndex)
  local self = PaGlobal_Manufacture_All
  if isManufactureAction() and (0 == #self._noneStackItemList[0] or nil == #self._noneStackItemList[0]) then
  ...
  if self._materialItemWhereType ~= Inventory_GetCurrentInventoryType() then
    self._materialSlotNoList = {
      [0] = 255,
      255,
      255,
      255,
    ...
    self._materialSlotNoListItemIn = {
      [0] = false,
      false,
      false,
      false,
  ...
  for ii = 0, self._usingSlotCount - 1 do
    if slotNo == self._materialSlotNoList[ii] then
      if true == self._isConsole then
        HandleEventRUp_Manufacture_All_ClearMaterial(ii)
      end
      break
    end
    if self._defaultSlotNo == self._materialSlotNoList[ii] then
      self._materialSlotNoList[ii] = slotNo
  ...
  if preSelectedIndex == nil then
    for index = 0, 4 do
      if (0 == #self._noneStackItemList[index] or nil == #self._noneStackItemList[index]) and true ~= self._materialSlotNoListItemIn[index] then
        selectedIndex = index
        break
      end
    end
  ...
  PaGlobal_Manufacture_All:addSelectedItemAndFoundList(selectedIndex, slotNo, inventoryType)
  PaGlobal_Manufacture_All:updateSlot()
  if _ContentsGroup_LifeSimulator == true then
    local currentActionName = self._listAction[self._actionIndex]._actionName
    if PaGloabl_Alchemy_RateViewList_CalculateManufacture ~= nil then
      PaGloabl_Alchemy_RateViewList_CalculateManufacture(currentActionName, self._materialSlotNoList[0], self._materialSlotNoList[1], self._materialSlotNoList[2], self._materialSlotNoList[3], self._materialSlotNoList[4])
    end
  end
end
function PaGlobal_Manufacture_All:addSelectedItemAndFoundList(index, slotNo, inventoryType)
  local self = PaGlobal_Manufacture_All
  if index < 0 or index > 4 then
    return
  end
  if true ~= self._materialSlotNoListItemIn[index] and (0 == #self._noneStackItemList[index] or nil == #self._noneStackItemList[index]) then
    self._noneStackItemList[index] = Array.new()
    ...
    local selectedItemEnchantLevel = selectedItemWrapper:get():getKey():getEnchantLevel()
    local selectedItemEnchanted = selectedItemWrapper:isEnchanted()
    ...
    for ii = 2, invenMaxSize - 1 do
      ...
      if nil ~= itemWrapper then
        ...
        if selectedItemKey == itemKey and not selectedItemWrapper:getStaticStatus():isStackable() and (selectedItemEnchanted == false and itemWrapper:isEnchanted() == false or selectedItemEnchantLevel == itemWrapper:get():getKey():getEnchantLevel()) then
          local canUseItem = true
          local itemExpiration = itemWrapper:getExpirationDate()
          if nil ~= itemExpiration and false == itemExpiration:isIndefinite() then
            local s64_remainTime = getLeftSecond_s64(itemExpiration)
            if s64_remainTime <= Defines.s64_const.s64_0 then
              canUseItem = false
            end
          end
          if slotNo ~= ii and false == ToClient_Inventory_CheckItemLock(ii, inventoryType) and false == self:checkCrystal(itemWrapper) and canUseItem == true then
            self._noneStackItemList[index]:push_back(ii)
          end
        end
      end
    end
    if 0 < #self._noneStackItemList[index] then
      if false == self._isConsole then
        self._ui.chk_identicalItem:SetCheck(false)
        self._ui.chk_identicalItem:SetShow(true)
      else
        self._canRepeat = true
      end
    end
  end
end
function PaGlobal_Manufacture_All_PushItemFromWarehouse(slotNo, itemWrapper, count)
  ...
  if isManufactureAction() and (0 == #PaGlobal_Manufacture_All._noneStackItemList[0] or nil == #PaGlobal_Manufacture_All._noneStackItemList[0]) then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "ALCHEMY_MANUFACTURE_DONT_CHANGE_ACTION"))
    return
  end
...
function PaGlobal_Manufacture_All_ManufactureReOpen()
  ...
  if nil ~= #PaGlobal_Manufacture_All._noneStackItemList[0] and 0 < #PaGlobal_Manufacture_All._noneStackItemList[0] and true == PaGlobal_Manufacture_All._noneStackItemCheck then
    return
  end
...
function PaGlobal_Manufacture_All_GetNextNoneStackItemSlotNo_ByWarehouse()
  ...
  for idx = 0, self._slotMaxCount - 1 do
    if self._selectedWarehouseItemKey[idx] ~= nil and self._selectedWarehouseItemKey[idx] ~= -1 then
      for ii = 1, useMaxCount - 1 do
        local itemWrapper = warehouseWrapper:getItem(ii)
        if nil ~= itemWrapper then
          local itemKey = itemWrapper:get():getKey():getItemKey()
          if self._selectedWarehouseItemKey[idx] == itemKey then
            self._targetWarehouseSlotNo[idx] = ii
            self._hasNoneStackItem = true
            self._selectedWarehouseItemSlotNo[idx] = self._defaultSlotNo
            break
          end
        end
      end
...
function PaGlobal_Manufacture_All_ContinueGrindJewel()
  ...
  if CppEnums.ItemWhereType.eInventory == self._materialItemWhereType or CppEnums.ItemWhereType.eCashInventory == self._materialItemWhereType then
    if self._noneStackItemCheck == true and nil ~= #self._noneStackItemList[0] and 0 < #self._noneStackItemList[0] then
      PaGlobal_Manufacture_All_RepeatManufacture()
      for idx = 0, self._slotMaxCount - 1 do
        if nil ~= self._noneStackItemList[idx] and #self._noneStackItemList[idx] > 0 then
          local nextSlotNo = self._noneStackItemList[idx][1]
          ...
          if nil ~= itemWrapper then
            if idx == 0 then
              ...
              if nil ~= self._currentRepeatItemKey[idx] and self._currentRepeatItemKey[idx] ~= selectedItemKey then
            ...
            local preSelectedIndex = idx
            PaGlobal_Manufacture_All_PushItemFromInventory(nextSlotNo, itemWrapper, itemWrapper:get():getCount_s64(), PaGlobal_Manufacture_All._materialItemWhereType, preSelectedIndex)
            PaGlobal_Manufacture_All._noneStackItemList[idx]:pop_front()
          end
        end
      end
    ...
    if true == hasNext and true == self._hasNoneStackItem and true == self._noneStackItemCheck then
      local warehouseWrapper = warehouse_get(self._waypointKey_ByWareHouse)
      ...
      PaGlobal_Manufacture_All_RepeatManufacture()
      for idx = 0, self._slotMaxCount - 1 do
        local itemWrapper = warehouseWrapper:getItem(self._targetWarehouseSlotNo[idx])
        if nil ~= itemWrapper then
          local selectedItemKey = itemWrapper:get():getKey():getItemKey()
          if nil ~= self._currentRepeatItemKey[idx] and self._currentRepeatItemKey[idx] ~= selectedItemKey then
            Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_MANUFACTURE_NOMATERIAL"))
          ...
          PaGlobal_Manufacture_All_PushItemFromWarehouse(self._targetWarehouseSlotNo[idx], itemWrapper, 1)
        end
      end
      HandleEventLUp_Manufacture_All_StartManufacture()
    end
...
function PaGlobal_Manufacture_All_ResultGrindJewel()
  if CppEnums.ItemWhereType.eInventory == PaGlobal_Manufacture_All._materialItemWhereType or CppEnums.ItemWhereType.eCashInventory == PaGlobal_Manufacture_All._materialItemWhereType then
    if true == PaGlobal_Manufacture_All._noneStackItemCheck and nil ~= #PaGlobal_Manufacture_All._noneStackItemList[0] and 0 < #PaGlobal_Manufacture_All._noneStackItemList[0] then
      for idx = 1, 4 do
        if true == PaGlobal_Manufacture_All._materialSlotNoListItemIn[idx] and (nil == #PaGlobal_Manufacture_All._noneStackItemList[idx] or #PaGlobal_Manufacture_All._noneStackItemList[idx] <= 0) then
...
function PaGlobal_Manufacture_All_InventoryYFunc()
  ...
  elseif PaGlobalFunc_InventoryInfo_GetShow() then
    Inventory_SetFunctor(nil, nil, nil, nil, nil)
    InventoryWindow_Close()
    HandleEventLUp_Manufacture_All_RepeatAction(false)
  end
end
function PaGlobal_Manufacture_All_InventoryLTYFunc()
  local itemWrapper = ToClient_getEquipmentItem(__eEquipSlotNoSubTool)
  if itemWrapper == nil then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_MANUFACTURE_MASSMANUFACTURE_NOTICE"))
    return
  end
  local itemSSW = itemWrapper:getStaticStatus()
  if __ePlayerLifeStatType_Manufacture ~= itemSSW:getLifeStatMainType() then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_MANUFACTURE_MASSMANUFACTURE_NOTICE"))
    return
  end
  if true == PaGlobal_Manufacture_All._isFromWarehouse then
...
function PaGlobal_Manufacture_All_InventoryOpen()
  ...
  if nil ~= currentSnapControl and currentSnapControl:GetID() == PaGlobal_Manufacture_All._ui.btn_rateView:GetID() then
    return
  end
  if true == PaGlobal_Manufacture_All._isFromWarehouse then
