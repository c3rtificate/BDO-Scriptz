function PaGlobal_Manufacture_All:resetGrindJewelSetting()
  self._noneStackItemList[0] = Array.new()
  self._noneStackItemList[1] = Array.new()
  self._noneStackItemList[2] = Array.new()
  self._noneStackItemList[3] = Array.new()
  self._noneStackItemList[4] = Array.new()
  self._noneStackItemCheck = false
  self._hasNoneStackItem = false
  self._selectedWarehouseItemKey = {
    [0] = -1,
    -1,
    -1,
    -1,
  ...
  self._selectedWarehouseItemSlotNo = {
    [0] = 255,
    255,
    255,
    255,
  ...
  self._ui.chk_identicalItem:SetShow(false)
  self._canRepeat = false
end
...
function PaGlobal_Manufacture_All:hasNoneStackItem(slotNo)
  ...
  self._hasNoneStackItem = false
  if self._selectedWarehouseItemSlotNo == nil then
    return
  end
  local slotListIndex = -1
  for idx = 0, self._slotMaxCount - 1 do
    if self._selectedWarehouseItemSlotNo[idx] == self._defaultSlotNo then
      slotListIndex = idx
      break
    end
  end
  if self._selectedWarehouseItemSlotNo[slotListIndex] == nil then
    return
  end
  self._selectedWarehouseItemSlotNo[slotListIndex] = slotNo
  ...
  self._selectedWarehouseItemKey[slotListIndex] = selectedItemWrapper:get():getKey():getItemKey()
  local hasNoneStackItemCount = 0
  for ii = 1, useMaxCount - 1 do
    local itemWrapper = warehouseWrapper:getItem(ii)
    if nil ~= itemWrapper then
      local itemKey = itemWrapper:get():getKey():getItemKey()
      if self._selectedWarehouseItemKey[slotListIndex] == itemKey and not selectedItemWrapper:getStaticStatus():isStackable() and slotNo ~= ii then
...
function PaGlobal_Manufacture_All:refreshIsMassCheckButton(actionIndex)
  if true == _ContentsGroup_LifeStatManufacturing then
    ...
    if itemWrapper ~= nil then
      ...
      if __ePlayerLifeStatType_Manufacture == itemSSW:getLifeStatMainType() and (actionIndex + 1 == itemSSW:getLifeStatSubType() or 0 == itemSSW:getLifeStatSubType()) then
    ...
    if true == isManufactureMassItemEquip then
      PaGlobal_Manufacture_All._ui.stc_iconM:SetShow(true)
      ...
      PaGlobal_Manufacture_All._ui.stc_iconM:SetShow(false)
...
function PaGlobal_Manufacture_All:updateLifePoint()
  ...
  if false == isSubLifeStat or true == PaGlobal_Manufacture_All._isConsole then
    ...
    subLifeStat = math.max(0, subLifeStat)
...
function PaGlobal_Manufacture_All:setAlignKeyGuide()
  local keyGuides = {
    PaGlobal_Manufacture_All._ui.stc_iconM,
    PaGlobal_Manufacture_All._ui.stc_iconY,
