...
local manufacture_Init = function()
  ...
  if true == _ContentsGroup_NewUI_Inventory_All then
    invenSizeY = Panel_Window_Inventory_All:GetSizeY()
  else
    invenSizeY = Panel_Window_Inventory:GetSizeY()
  end
  Panel_Manufacture:SetPosX((screenSizeX - Panel_Manufacture:GetSizeX()) / 2)
  Panel_Manufacture:SetPosY((screenSizeY - invenSizeY) / 2)
end
manufacture_Init()
function manufacture_Repos()
  ...
  local knowledgePanelSize = 0
  if knowledgeBG:GetShow() then
    knowledgePanelSize = knowledgeBG:GetSizeX() + 30
  end
  Panel_Manufacture:SetPosX((screenSizeX - Panel_Manufacture:GetSizeX()) / 2 - knowledgePanelSize)
...
PaGlobal_Util_RegistHelpTooltipEvent(_buttonQuestion, "\"PanelManufacture\"")
...
_textDesc:SetTextMode(__eTextMode_AutoWrap)
_textDesc:SetText("")
...
_defaultMSG1:SetTextMode(__eTextMode_AutoWrap)
...
function Manufacture_Show(installationType, materialWhereType, isClear, showType, waypointKey)
  ...
  if nil ~= installationType then
    ...
    Manufacture_Button_LUp_Shake(true, true)
...
function Manufacture_Close()
  ...
  if true == _ContentsGroup_NewUI_Inventory_All then
    isInvenOpen = not Panel_Window_Inventory_All:GetShow()
  else
    isInvenOpen = FGlobal_InventoryIsClosing()
  end
  if true == invenShow and false == isInvenOpen then
...
function Manufacture_PushItemFromInventory(slotNo, itemWrapper, count, inventoryType)
  ...
  if materialItemWhereType ~= Inventory_GetCurrentInventoryType() then
    _materialSlotNoList = {
      [0] = _defaultSlotNo,
      _defaultSlotNo,
      _defaultSlotNo,
  ...
  for ii = 0, _slotCount - 1 do
    ...
    if nil ~= _materialSlotNoList[ii] and invenSize >= _materialSlotNoList[ii] then
      ...
      if CppEnums.ItemWhereType.eWarehouse == materialItemWhereType then
        itemWrapper = PaGlobal_Warehouse_All_GetWarehouseWarpper():getItem(_materialSlotNoList[ii])
      else
        itemWrapper = getInventoryItemByType(materialItemWhereType, _materialSlotNoList[ii])
      end
      if nil ~= itemWrapper then
        _slotList[ii]:setItemByStaticStatus(itemWrapper:getStaticStatus(), itemWrapper:get():getCount_s64())
        _slotList[ii].icon:SetShow(true)
...
function Manufacture_ContinueGrindJewel()
  if CppEnums.ItemWhereType.eInventory == materialItemWhereType or CppEnums.ItemWhereType.eCashInventory == materialItemWhereType then
    if nil ~= #noneStackItemList and #noneStackItemList > 0 and true == noneStackItemCheck then
      ...
      local itemWrapper = getInventoryItemByType(materialItemWhereType, nextSlotNo)
      if nil == itemWrapper then
        return
      end
      local itemCount = itemWrapper:get():getCount_s64()
      Manufacture_PushItemFromInventory(nextSlotNo, itemWrapper, itemCount, materialItemWhereType)
...
function Manufacture_ResponseResultItem(itemDynamicListWrapper, failReason)
  ...
  if size > 0 then
    ...
    for index = 0, size - 1 do
      ...
      local idx
      local count = #Manufacture_Notify._data_Result
      if count > 0 then
        for index = 1, count do
          if Manufacture_Notify._data_Result[index]._key == itemKey then
            idx = index
          end
        end
      end
    ...
    elseif 8 == failReason then
      message = PAGetString(Defines.StringSheet_RESOURCE, "UI_ENDURANCE_STCTXT_NOTICEINVENSLOT")
      Manufacture_ClearValues()
      Manufacture_Response()
...
function Manufacture_Mouse_LUp()
  ...
  for index = 0, _usingItemSlotCount - 1 do
    if true == _materialSlotNoListItemIn[index] and size >= _materialSlotNoList[index] then
      ...
      local itemWrapper
      if CppEnums.ItemWhereType.eInventory == materialItemWhereType or CppEnums.ItemWhereType.eCashInventory == materialItemWhereType then
        itemWrapper = getInventoryItemByType(materialItemWhereType, _materialSlotNoList[index])
      else
        local warehouseWrapper = warehouse_get(waypointKey_ByWareHouse)
      ...
      Manufacture_Notify._data_Resource[idx]._key = itemStaticWrapper:get()._key:getItemKey()
...
function Material_Mouse_On(index)
  ...
  if materialItemWhereType == CppEnums.ItemWhereType.eInventory or CppEnums.ItemWhereType.eCashInventory == materialItemWhereType then
    ...
    itemWrapper = PaGlobal_Warehouse_All_GetWarehouseWarpper():getItem(_materialSlotNoList[index])
  end
...
function Manufacture_Button_LUp_Shake(isClear, ignoreAction)
  if MessageBoxGetShow() then
    return
  end
  ...
  if CppEnums.ItemWhereType.eInventory == materialItemWhereType or CppEnums.ItemWhereType.eCashInventory == materialItemWhereType then
    ...
    PaGlobal_Warehouse_All_SetFunctor(ManufactureAction_WarehouseFilter, Manufacture_PushItemFromWarehouse)
    PaGlobal_Warehouse_All_UpdateSlotData()
  end
...
function Manufacture_KnowledgeList_SelectKnowledge(index)
  ...
  if nil ~= mentalCardStaticWrapper then
    ...
    if true == isLearn then
      _uiKnowledgeDesc:SetTextMode(__eTextMode_AutoWrap)
...
function ManufactureKnowledge_UpdateList()
  ...
  if true == ToClient_IsContentsGroupOpen("597") then
    if _actionIndex == 10 then
      _startKnowledgeIndex = 22
    elseif _actionIndex == 11 then
      _startKnowledgeIndex = 19
    else
      _startKnowledgeIndex = 0
    end
  else
...
function Material_Update(slotCount)
  ...
  for ii = 0, slotCount - 1 do
    if nil ~= _materialSlotNoList[ii] and invenSize >= _materialSlotNoList[ii] then
      local itemWrapper = getInventoryItemByType(materialItemWhereType, _materialSlotNoList[ii])
      if nil ~= itemWrapper then
        _slotList[ii]:setItemByStaticStatus(itemWrapper:getStaticStatus(), itemWrapper:get():getCount_s64())
      end
    end
...
function Manufacture_Notify:createResultControl(index)
  if nil == self._item_Result[index] or nil == self._icon_ResultBG[index] or nil == self._icon_Result[index] then
    self._item_Result[index] = UI.createControl(__ePAUIControl_StaticText, Panel_Manufacture_Notify, "item_Result_" .. index)
    self._icon_ResultBG[index] = UI.createControl(__ePAUIControl_Static, Panel_Manufacture_Notify, "icon_ResultBG_" .. index)
    self._icon_Result[index] = UI.createControl(__ePAUIControl_Static, Panel_Manufacture_Notify, "icon_Result_" .. index)
...
function Manufacture_Notify:createResourceControl(index)
  if nil == self._item_Resource[index] or nil == self._icon_ResourceBG[index] or nil == self._icon_Resource[index] then
    self._item_Resource[index] = UI.createControl(__ePAUIControl_StaticText, Panel_Manufacture_Notify, "item_Resource_" .. index)
    self._icon_ResourceBG[index] = UI.createControl(__ePAUIControl_Static, Panel_Manufacture_Notify, "icon_ResourceBG_" .. index)
    self._icon_Resource[index] = UI.createControl(__ePAUIControl_Static, Panel_Manufacture_Notify, "icon_Resource_" .. index)
...
function Manufacture_ListControlCreate(content, key)
  ...
  if nil ~= mentalCardStaticWrapper then
    local isLearn = isLearnMentalCardForAlchemy(mentalCardStaticWrapper:getKey())
    recipe:SetTextMode(__eTextMode_LimitText)
    if true == isLearn then
...
function Manufacture_ShowKnowledgeList()
  ...
  local changePosX = Panel_Manufacture:GetPosX()
  if true == isKnowledgeOpened then
    changePosX = changePosX - _btn_funcBG:GetSizeX() - 30
    if changePosX < 0 then
      changePosX = 0
    end
  else
    changePosX = changePosX + _btn_funcBG:GetSizeX() + 30
    if getScreenSizeX() < changePosX + Panel_Manufacture:GetSizeX() then
      local gap = changePosX + Panel_Manufacture:GetSizeX() - getScreenSizeX()
      changePosX = changePosX - gap
    end
  end
  Panel_Manufacture:SetPosX(changePosX)
  _btn_funcBG:SetShow(isKnowledgeOpened)
end
...
list2:registEvent(__ePAUIList2EventType_LuaChangeContent, "Manufacture_ListControlCreate")
