PaGlobal_TransferLifeExp_All = {
  ...
  _STRINGTABLE = {
    ...
    _fitnessExpTitle = PAGetString(Defines.StringSheet_GAME, "LUA_TANSERFITNESSEXPERIENCE_TITLE"),
    _fitnessExpDesc = PAGetString(Defines.StringSheet_GAME, "LUA_TANSERFITNESSEXPERIENCE_DESC"),
    _levelString = PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_TEXT_CRAFTLEVEL"),
    lifeType = {
    ...
    fitnessType = {
      [0] = PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_FITNESS_STAMINA_TITLE_ONE"),
      [1] = PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_FITNESS_STRENGTH_TITLE_ONE"),
      [2] = PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_FITNESS_HEALTH_TITLE_ONE")
    }
  },
...
function PaGlobal_TransferLifeExp_All:registEventHandler(isConsole)
  ...
  registerEvent("FromClient_RequestUseExchangeFitnessExp", "FromClient_RequestUseExchangeFitnessExp")
  registerEvent("FromClient_ResponseExchangeFitnessExp", "FromClient_ResponseExchangeFitnessExp")
...
function PaGlobal_TransferLifeExp_All:update()
  local characterSize = getCharacterDataCount(__ePlayerCreateNormal)
  ...
  elseif nil == self._transferItemData.fitnessType then
    self._ui.txt_FromLifeValue:SetText("")
  else
    local typeName = self._STRINGTABLE.fitnessType[self._transferItemData.fitnessType]
    local playerGetter = getSelfPlayer():get()
    local level = playerGetter:getFitnessLevel(self._transferItemData.fitnessType)
    local currentExp = playerGetter:getCurrFitnessExperiencePoint(self._transferItemData.fitnessType)
    local maxExp = playerGetter:getDemandFItnessExperiencePoint(self._transferItemData.fitnessType)
    local rate = Int64toInt32(currentExp * toInt64(0, 100) / maxExp)
    if nil == rate then
      rate = 0
      _PA_LOG("\237\149\156\237\153\148\236\155\144", "rate : " .. tostring(rate))
    end
    local finalString = typeName .. " " .. self._STRINGTABLE._levelString .. level .. " " .. tostring(rate) .. "%"
    self._ui.txt_FromLifeValue:SetText(finalString)
  end
  ...
  if nil == self._transferItemData.lifeType then
    if nil == self._transferItemData.fitnessType then
      ...
      self._ui.txt_Title:SetText(self._STRINGTABLE._fitnessExpTitle)
      self._ui.txt_desc:SetText(self._STRINGTABLE._fitnessExpDesc)
      if false == self._isConsole then
        self._ui.btn_Confirm:addInputEvent("Mouse_LUp", "HandleEventLUp_TransferLifeExp_All_ConfirmTransfer(2)")
      end
    end
  else
...
function PaGlobal_TransferLifeExp_All:dataClear()
  ...
  self._transferType.fitnessType = nil
...
function HandleEventLUp_TransferLifeExp_All_SelectCharacter(idx)
  ...
  if true == PaGlobal_TransferLifeExp_All._isConsole then
    if nil == PaGlobal_TransferLifeExp_All._transferItemData.lifeType then
      if nil == PaGlobal_TransferLifeExp_All._transferItemData.fitnessType then
        HandleEventLUp_TransferLifeExp_All_ConfirmTransfer(1)
      else
        HandleEventLUp_TransferLifeExp_All_ConfirmTransfer(2)
      end
    else
...
function HandleEventLUp_TransferLifeExp_All_ConfirmTransfer(_transferType)
  ...
  if 0 == _transferType then
    ...
    local selectedCharacterData = getCharacterDataByIndex(PALifeExp._selectedCharacterIdx, __ePlayerCreateNormal)
  ...
  elseif 2 == _transferType then
    if nil ~= PALifeExp._transferItemData.lifeType or nil == PALifeExp._transferItemData.fitnessType then
      Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_TRANSFERLIFEEXPERIENCE_WRONGVALUE"))
      return
    end
    ToClient_requestExchangeFitnessExp(PALifeExp._transferItemData.where, PALifeExp._transferItemData.slotIdx, selIdx, PALifeExp._transferItemData.fitnessType)
  end
end
...
function FromClient_RequestUseExchangeBattleAndSkillExp(fromWhereType, fromSlotNo)
  ...
  PaGlobal_TransferLifeExp_All._transferItemData.fitnessType = nil
...
function FromClient_RequestUseExchangeFitnessExp(fromWhereType, fromSlotNo, fitnessType)
  PaGlobal_TransferLifeExp_All._transferItemData.where = fromWhereType
  PaGlobal_TransferLifeExp_All._transferItemData.slotIdx = fromSlotNo
  PaGlobal_TransferLifeExp_All._transferItemData.lifeType = nil
  PaGlobal_TransferLifeExp_All._transferItemData.fitnessType = fitnessType
  PaGlobal_TransferLifeExp_All:prepareOpen()
end
function FromClient_ResponseExchangeFitnessExp(fitnessType)
  Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_TRANSFERLIFEEXPERIENCE_SUCCESS_TRANSFER"))
  HandleEventLUp_TransferLifeExp_All_Close()
end
function FromClient_TransferLifeExp_ALL_UpdateContent(content, key)
  ...
  local characterData = getCharacterDataByIndex(key32, __ePlayerCreateNormal)
  if nil ~= characterData then
    ...
    PaGlobalFunc_Util_ChangeTextureClass(icon, classType)
