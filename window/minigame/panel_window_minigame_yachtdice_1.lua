...
function PaGlobal_MiniGame_YachtDice:clear()
  ...
  self._currentGameDbType = __eMiniGameDBType_Count
  local rollBtnBg = UI.getChildControl(self._ui._stc_rightBG, "Static_Roll_BG")
  rollBtnBg:SetShow(false)
  self._ui._btn_roll:SetShow(false)
  ...
  self._ui._stc_timer.txt_time:EraseAllEffect()
  self._soundTime = 10
  self._ui._stc_timer:SetShow(false)
  self._isPvPFirstTurn = false
  self:hideHandRankTooltip()
  self:hideMyDeckTooltip()
  self._isShowPlayerBubbleText = false
  self._isShowNpcBubbleText = false
  self._playerBubbleTextDeltaTime = 0
  self._npcBubbleTextDeltaTime = 0
  if nil ~= self._highPointEffectNo and nil ~= self._highPointControl then
    self._highPointControl:EraseEffect(self._highPointEffectNo)
  end
  self._highPointControl = nil
  self._highPointEffectNo = nil
...
function PaGlobal_MiniGame_YachtDice:initPanelPos()
...
function PaGlobal_MiniGame_YachtDice:prepareOpen(isOpenByNpc)
  if nil == isOpenByNpc then
    UI.ASSERT_NAME(false, "\237\152\184\236\182\156\234\178\189\235\161\156\234\176\128 \235\185\132\236\160\149\236\131\129\236\158\133\235\139\136\235\139\164! \236\149\188\235\165\180\235\165\188 \237\130\172 \236\136\152 \236\151\134\235\139\164.", "\236\157\180\236\163\188")
  end
  self._isOpenByNpc = isOpenByNpc
  self:showPopUpButton(not self._isOpenByNpc)
  self:open()
end
function PaGlobal_MiniGame_YachtDice:open()
  ...
  self:initPanelPos()
  if true == self._isOpenByNpc then
...
function PaGlobal_MiniGame_YachtDice:close()
  ...
  self._isOpenByNpc = false
  if true == Panel_Widget_MiniGame_YachtDice_Result:GetShow() then
    PaGlobal_MiniGame_YachtDice_Result:prepareClose()
  end
  self:hideHandRankTooltip()
  self:hideMyDeckTooltip()
  self:showSimpleNameTooltip(false, self.__eYachtPlayerType_Me)
  Panel_Window_MiniGame_YachtDice:CloseUISubApp()
  self._ui._btn_popUp:SetCheck(false)
...
function PaGlobal_MiniGame_YachtDice:validate()
  ...
  self._ui._btn_popUp:isValidate()
...
function PaGlobal_MiniGame_YachtDice:registerEvent()
  ...
  for ii = 0, __eYachtDiceCount - 1 do
    ...
    if true == self._isConsole then
      ...
      fieldCard.stc_numberImage:addInputEvent("Mouse_LUp", "PaGlobal_MiniGame_YachtDice:keepCardByConsole()")
    else
  ...
  registerEvent("FromClient_processYachtGameBubbleText", "FromClient_processYachtGameBubbleText")
...
function PaGlobal_MiniGame_YachtDice:updatePoint()
      ...
      if nil ~= slot.keyRaw then
        ...
        if true == isSet then
          ...
          if true == isSelf and rollCount > 0 and true == isMyTurn then
            ...
            if getableHighPoint < point then
              getableHighPoint = point
              if nil ~= self._highPointControl then
                if self._highPointControl == control then
                else
                  if nil ~= self._highPointEffectNo then
                    self._highPointControl:EraseEffect(self._highPointEffectNo)
                    self._highPointEffectNo = nil
                  end
                  self._highPointControl = control
                end
              else
                self._highPointControl = control
                if nil ~= self._highPointEffectNo then
  ...
  if 0 == getableHighPoint then
    if nil ~= self._highPointControl and nil ~= self._highPointEffectNo then
      self._highPointControl:EraseEffect(self._highPointEffectNo)
    end
    self._highPointControl = nil
    self._highPointEffectNo = nil
  elseif nil ~= self._highPointControl and nil == self._highPointEffectNo then
    self._highPointEffectNo = self._highPointControl:AddEffect("fUI_Mini_Game_11A", true, 0, 0)
  end
end
...
function PaGlobal_MiniGame_YachtDice:doCheckHandRank(keyRaw)
  if self._currentRenderEventType == self._renderEventType.DrawFieldCard then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantCheckHandRankWhileDrawCard"))
    return
  end
...
function PaGlobal_MiniGame_YachtDice:updateTurnBG()
  ...
  local rollBtnBg = UI.getChildControl(self._ui._stc_rightBG, "Static_Roll_BG")
  if true == isMyTurn then
    rollBtnBg:SetShow(true)
    self._ui._btn_roll:SetShow(true)
  ...
  self._highPointEffectNo = nil
  if true == isMyTurn then
    self._ui._stc_timer:SetPosition(self._timerConterPosToMe_X, self._timerConterPosToMe_Y)
  else
    self._ui._stc_timer:SetPosition(self._timerConterPosToYou_X, self._timerConterPosToYou_Y)
  end
  ...
  self._ui._stc_timer:SetShow(true)
...
function PaGlobal_MiniGame_YachtDice:updateMyUserInfo()
  ...
  turnBG:Set_UserInfoControls(true, userDataWrapper)
end
function PaGlobal_MiniGame_YachtDice:updateEnemyUserInfo()
  ...
  turnBG:Set_UserInfoControls(false, userDataWrapper)
end
...
function PaGlobal_MiniGame_YachtDice:initTitleBG()
  ...
  self._ui._btn_popUp = UI.getChildControl(self._ui._stc_titleBG, "CheckButton_TrayBTN")
  local phaseControl = {
    parent = nil,
    txt_title = nil,
    txt_desc = nil
  ...
  local msgGuideControl = UI.getChildControl(self._ui._stc_rightBG, "Static_MSG_Guide")
...
function PaGlobal_MiniGame_YachtDice:initLeftBG()
  ...
  self._ui._stc_turnBG[self.__eYachtPlayerType_Me] = self:initTurnBG("Static_MyTurnBG", self.__eYachtPlayerType_Me)
  self._ui._stc_turnBG[self.__eYachtPlayerType_You] = self:initTurnBG("Static_NPCTurnBG", self.__eYachtPlayerType_You)
  local myTurnBG = self._ui._stc_turnBG[self.__eYachtPlayerType_Me]
  local yourTurnBG = self._ui._stc_turnBG[self.__eYachtPlayerType_You]
  ...
  myTurnBG.stc_platformIcon = UI.getChildControl(stc_bottom, "Static_ConsolePartIcon")
  myTurnBG.txt_psOnlineID = UI.getChildControl(stc_bottom, "StaticText_ConsoleUserId")
  myTurnBG:isValidate()
  ...
  yourTurnBG.stc_platformIcon = UI.getChildControl(stc_top, "Static_ConsolePartIcon")
  yourTurnBG.txt_psOnlineID = UI.getChildControl(stc_top, "StaticText_ConsoleUserId")
...
function PaGlobal_MiniGame_YachtDice:initTurnBG(name, playerType)
  local turnBG = {
    parent = nil,
    stc_allBG = nil,
    stc_arrow = nil,
  ...
  function turnBG:ChangeTextureInfoName(texture)
    ...
    local x1, y1, x2, y2 = setTextureUV_Func(turnBG.stc_rightIcon, 0, 0, 360, 360)
    turnBG.stc_rightIcon:getBaseTexture():setUV(x1, y1, x2, y2)
    turnBG.stc_rightIcon:setRenderTexture(turnBG.stc_rightIcon:getBaseTexture())
    turnBG.stc_icon:ChangeTextureInfoName(texture)
    local x11, y11, x22, y22 = setTextureUV_Func(turnBG.stc_icon, 0, 0, 360, 360)
    turnBG.stc_icon:getBaseTexture():setUV(x11, y11, x22, y22)
    turnBG.stc_icon:setRenderTexture(turnBG.stc_icon:getBaseTexture())
  end
  function turnBG:isValidate()
    ...
    turnBG.txt_psOnlineID:isValidate()
    turnBG.stc_platformIcon:isValidate()
    turnBG:SetOriginPosInfo()
  end
  function turnBG:SetShowBubbleText(isShow)
    turnBG.txt_bubble:SetShow(isShow)
    if false == isShow then
      turnBG.txt_bubble:SetText("")
    else
      turnBG.txt_bubble:ResetVertexAni()
      turnBG.txt_bubble:SetVertexAniRun("Ani_Move_Pos_New", true)
      turnBG.txt_bubble:SetVertexAniRun("Ani_Scale_New", true)
    end
  end
  function turnBG:SetBubbleText(text)
    UI.ASSERT_NAME(nil ~= text, "bubbleText\234\176\128 nil\236\158\133\235\139\136\235\139\164.", "\236\157\180\236\163\188")
    turnBG.txt_bubble:SetTextMode(__eTextMode_AutoWrap)
    turnBG.txt_bubble:SetText(text)
    turnBG.txt_bubble:SetSize(turnBG.txt_bubble:GetSizeX(), turnBG.txt_bubble:GetTextSizeY() + 35)
  end
  function turnBG:SetOriginPosInfo()
    turnBG.info_originPos = {}
    turnBG.info_originPos.rightName = {}
    turnBG.info_originPos.rightName.x = turnBG.txt_rightName:GetPosX()
    turnBG.info_originPos.rightName.y = turnBG.txt_rightName:GetPosY()
    turnBG.info_originPos.rightWinRate = {}
    turnBG.info_originPos.rightWinRate.x = turnBG.txt_rightWinRate:GetPosX()
    turnBG.info_originPos.rightWinRate.y = turnBG.txt_rightWinRate:GetPosY()
  end
  function turnBG:Clear_UserInfoControls()
    turnBG.txt_rightName:SetPosXY(turnBG.info_originPos.rightName.x, turnBG.info_originPos.rightName.y)
    turnBG.txt_rightWinRate:SetPosXY(turnBG.info_originPos.rightWinRate.x, turnBG.info_originPos.rightWinRate.y)
    turnBG.txt_psOnlineID:SetShow(false)
    turnBG.txt_psOnlineID:SetText("-")
    turnBG.stc_platformIcon:SetShow(false)
  end
  function turnBG:Set_UserInfoControls(isBottomControls, userDataWrapper)
    if ToClient_isPS5() == false then
      return
    end
    local yachtActorType = userDataWrapper:getActorTypeRaw()
    if yachtActorType ~= __eActorType_Player then
      return
    end
    local function setPSOnlineID(platformType, platformNickname)
      if platformType ~= __eGAME_PLATFORM_TYPE_PS then
        return
      end
      if isBottomControls == true then
        turnBG.txt_rightName:SetPosY(turnBG.txt_rightName:GetPosY() - 10)
        turnBG.txt_psOnlineID:SetPosY(turnBG.txt_rightName:GetPosY() + turnBG.txt_rightName:GetSizeY() + 5)
        turnBG.txt_psOnlineID:SetText(ToClient_ConsoleUserNameString() .. ": " .. platformNickname)
        turnBG.txt_psOnlineID:SetShow(true)
        turnBG.txt_rightWinRate:SetPosY(turnBG.txt_psOnlineID:GetPosY() + turnBG.txt_psOnlineID:GetSizeY() + 5)
      elseif isBottomControls == false then
        turnBG.txt_rightName:SetPosY(turnBG.txt_rightName:GetPosY() - 20)
        turnBG.txt_psOnlineID:SetPosY(turnBG.txt_rightName:GetPosY() + turnBG.txt_rightName:GetSizeY() + 5)
        turnBG.txt_psOnlineID:SetText(ToClient_ConsoleUserNameString() .. ": " .. platformNickname)
        turnBG.txt_psOnlineID:SetShow(true)
        turnBG.txt_rightWinRate:SetPosY(turnBG.txt_psOnlineID:GetPosY() + turnBG.txt_psOnlineID:GetSizeY() + 5)
    ...
    local function setPlatformIcon(platformType)
      if platformType == __eGAME_PLATFORM_TYPE_PS then
        self.stc_platformIcon:ChangeTextureInfoName("combine/commonicon/crossplay_icon_ps.dds")
      else
        self.stc_platformIcon:ChangeTextureInfoName("combine/commonicon/crossplay_icon_other.dds")
      end
      if isBottomControls == true then
        local controlLeftPosX = self.txt_rightName:GetPosX()
        local textSizeX = self.txt_rightName:GetTextSizeX()
        local gap = 5
        local rightAligned_iconPosX = controlLeftPosX + textSizeX + gap
        self.stc_platformIcon:SetPosXY(rightAligned_iconPosX, self.txt_rightName:GetPosY())
      elseif isBottomControls == false then
        local controlRightPosX = self.txt_rightName:GetPosX() + self.txt_rightName:GetSizeX()
        local textSizeX = self.txt_rightName:GetTextSizeX()
        local iconSizeX = self.stc_platformIcon:GetSizeX()
        local gap = 5
        local leftAligned_iconPosX = controlRightPosX - textSizeX - iconSizeX - gap
        self.stc_platformIcon:SetPosXY(leftAligned_iconPosX, self.txt_rightName:GetPosY())
      end
      self.stc_platformIcon:SetShow(true)
    end
    local platformType = userDataWrapper:getPlatformType()
    local platformNickname = userDataWrapper:getPlatformNickname()
    setPSOnlineID(platformType, platformNickname)
    setPlatformIcon(platformType)
  end
  ...
  turnBG.txt_name:addInputEvent("Mouse_On", "PaGlobal_MiniGame_YachtDice:showSimpleNameTooltip(true," .. tostring(playerType) .. ")")
  turnBG.txt_name:addInputEvent("Mouse_Out", "PaGlobal_MiniGame_YachtDice:showSimpleNameTooltip(false," .. tostring(playerType) .. ")")
  return turnBG
end
function PaGlobal_MiniGame_YachtDice:showSimpleNameTooltip(isShow, playerType)
  if self.__eYachtPlayerType_Me ~= playerType and self.__eYachtPlayerType_You ~= playerType then
  ...
  if true == isShow then
    ...
    if self.__eYachtPlayerType_Me == playerType then
      userDataWrapper = ToClient_getMyYachtUserDataWrapper()
      if nil == userDataWrapper then
        return
      end
    else
      userDataWrapper = ToClient_getEnemyYachtUserDataWrapper()
      if nil == userDataWrapper then
        return
      end
    end
    if nil ~= userDataWrapper then
      TooltipSimple_Show(self._ui._stc_turnBG[playerType].txt_name, userDataWrapper:getUserNickname(), "")
    end
  else
    TooltipSimple_Hide()
  end
...
function PaGlobal_MiniGame_YachtDice:initRightBG()
  ...
  function timer:SetPosition(x, y)
    timer.parent:SetPosX(x)
    timer.parent:SetPosY(y)
  end
  ...
  local stc_top = UI.getChildControl(self._ui._stc_rightBG, "Static_Top")
  local stc_bottom = UI.getChildControl(self._ui._stc_rightBG, "Static_Bottom")
  self._timerConterPosToMe_X = stc_bottom:GetPosX() + 10
  self._timerConterPosToMe_Y = stc_bottom:GetPosY() - self._ui._stc_timer.parent:GetSizeY() - 10
  self._timerConterPosToYou_X = stc_top:GetPosX() + stc_top:GetSizeX() - self._ui._stc_timer.parent:GetSizeX() - 10
  self._timerConterPosToYou_Y = stc_top:GetPosY() + stc_top:GetSizeY() + 10
  local keyGuideBG = {
    parent = nil,
    pc_guide = nil,
    console_guide1 = nil,
  ...
  keyGuideBG.console_guide2 = UI.getChildControl(keyGuideBG.parent, "StaticText_CardSelect_LB_ConsoleUI")
  self._ui._stc_keyGuideBG = keyGuideBG
  if true == self._isConsole then
    self._ui._stc_keyGuideBG:SetShow(true)
    local keyGuides = {
      self._ui._stc_keyGuideBG.console_guide2,
      self._ui._stc_keyGuideBG.console_guide1
    }
    PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyGuides, self._ui._stc_keyGuideBG.parent, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHT)
    self._ui._stc_keyGuideBG:SetShow(false)
  else
    local panel = Panel_Window_MiniGame_YachtDice
    local panelRight = panel:GetPosX() + panel:GetSizeX()
    local guideTextSize = self._ui._stc_keyGuideBG.pc_guide:GetTextSizeX()
    self._ui._stc_keyGuideBG.pc_guide:SetPosX(panelRight - guideTextSize - self._ui._stc_keyGuideBG.pc_guide:GetSizeX())
    self._ui._stc_keyGuideBG.pc_guide:ComputePos()
  end
end
...
function PaGlobal_MiniGame_YachtDice:eventEndBeforeGameStart()
  ...
  self._isPvPFirstTurn = false
  local rollBtnBg = UI.getChildControl(self._ui._stc_rightBG, "Static_Roll_BG")
  if __eMiniGameDBType_YachtDice_PVP == self._currentGameDbType and 1 == ToClient_getCurrentTurnCount() then
    local isMyTurn = ToClient_isMyTurnMiniGameYachtDice()
    if true == isMyTurn then
      rollBtnBg:SetShow(true)
      self._ui._btn_roll:SetShow(true)
      ...
      self._ui._btn_roll:SetShow(false)
      self._ui._stc_msgPhase:SetTitle(PAGetString(Defines.StringSheet_GAME, "LUA_MINIGAME_PHASE_TITLE_0"))
      self._ui._stc_msgPhase:SetDesc(PAGetString(Defines.StringSheet_GAME, "LUA_MINIGAME_PHASE_START_ENEMY"))
    end
    FromClient_responseYachtDiceGameUpdateTurn()
  else
    rollBtnBg:SetShow(true)
    self._ui._btn_roll:SetShow(true)
    self._ui._stc_msgPhase:SetTitle(PAGetString(Defines.StringSheet_GAME, "LUA_MINIGAME_PHASE_TITLE_0"))
...
function PaGlobal_MiniGame_YachtDice:loadCompleteNpcAction()
  if __eMiniGameDBType_YachtDice_PVE ~= self._currentGameDbType then
    return
  end
...
function PaGlobal_MiniGame_YachtDice:showPlayerBubbleText(bubbleText)
  if __eMiniGameDBType_YachtDice_PVE ~= self._currentGameDbType then
    return
  end
  self._ui._stc_turnBG[self.__eYachtPlayerType_Me]:SetShowBubbleText(true)
  self._ui._stc_turnBG[self.__eYachtPlayerType_Me]:SetBubbleText(bubbleText)
  self._isShowPlayerBubbleText = true
  self._playerBubbleTextDeltaTime = 0
end
function PaGlobal_MiniGame_YachtDice:showNpcBubbleText(bubbleText)
  if __eMiniGameDBType_YachtDice_PVE ~= self._currentGameDbType then
    return
  end
  self._ui._stc_turnBG[self.__eYachtPlayerType_You]:SetShowBubbleText(true)
  self._ui._stc_turnBG[self.__eYachtPlayerType_You]:SetBubbleText(bubbleText)
  self._isShowNpcBubbleText = true
  self._npcBubbleTextDeltaTime = 0
end
function PaGlobal_MiniGame_YachtDice:updateAutoBubbleTextForPVE(deltaTime)
  if __eMiniGameDBType_YachtDice_PVE ~= self._currentGameDbType then
    return
  end
  if true == self._isShowPlayerBubbleText then
    self._playerBubbleTextDeltaTime = self._playerBubbleTextDeltaTime + deltaTime
    if self._bubbleTextShowTime < self._playerBubbleTextDeltaTime then
      self._ui._stc_turnBG[self.__eYachtPlayerType_Me]:SetShowBubbleText(false)
      self._isShowPlayerBubbleText = false
      self._playerBubbleTextDeltaTime = 0
    end
  end
  if true == self._isShowNpcBubbleText then
    self._npcBubbleTextDeltaTime = self._npcBubbleTextDeltaTime + deltaTime
    if self._bubbleTextShowTime < self._npcBubbleTextDeltaTime then
      self._ui._stc_turnBG[self.__eYachtPlayerType_You]:SetShowBubbleText(false)
      self._isShowNpcBubbleText = false
      self._npcBubbleTextDeltaTime = 0
    end
  end
...
function PaGlobal_MiniGame_YachtDice:updateTimer(deltaTime)
  ...
  if 0 == self._turnRemainTime then
    self._ui._stc_timer:SetShow(false)
  end
  ...
  local currentTurnMaxTime = 0
  local isMyTurn = ToClient_isMyTurnMiniGameYachtDice()
  if true == isMyTurn then
    currentTurnMaxTime = ToClient_getMiniGameYachtTurnMaxTime()
  elseif __eMiniGameDBType_YachtDice_PVP == self._currentGameDbType then
    currentTurnMaxTime = ToClient_getMiniGameYachtTurnMaxTime()
  elseif __eMiniGameDBType_YachtDice_PVE == self._currentGameDbType then
    currentTurnMaxTime = ToClient_getMiniGameYachtTurnMaxTimeNpc()
  else
    UI.ASSERT_NAME(false, "\235\185\132\236\160\149\236\131\129\236\158\133\235\139\136\235\139\164.", "\236\157\180\236\163\188")
  end
  local remainTime = ToClient_getMiniGameYachtTurnRemainTime()
  if 0 < self._turnRemainTime - remainTime then
    self._turnRemainTime = self._turnRemainTime - (self._turnRemainTime - remainTime)
  end
  local ratio = self._turnRemainTime / currentTurnMaxTime
...
function PaGlobal_MiniGame_YachtDice:showPopUpButton(isShow)
  if false == _ContentsGroup_PopUp then
    isShow = false
  end
  if true == isShow then
    self._ui._btn_popUp:SetShow(true)
    self._ui._btn_popUp:addInputEvent("Mouse_LUp", "HandleEventLUp_MiniGameYachtDice_CheckPopUpUI()")
    self._ui._btn_popUp:addInputEvent("Mouse_On", "HandleEventOnOut_MiniGameYachtDice_PopupUITooltip(true)")
    self._ui._btn_popUp:addInputEvent("Mouse_Out", "HandleEventOnOut_MiniGameYachtDice_PopupUITooltip(false)")
  else
    self._ui._btn_popUp:SetShow(false)
    self._ui._btn_popUp:addInputEvent("Mouse_LUp", "")
    self._ui._btn_popUp:addInputEvent("Mouse_On", "")
    self._ui._btn_popUp:addInputEvent("Mouse_Out", "")
  end
end
