...
function PaGlobal_TradeMarketItemList_All:updateBuySlot(contents, key32)
  ...
  if 0 == needLifeLevel or nil == needLifeLevel then
    ...
    if needLifeLevel < playerThisCraftLevel then
      ...
      if true == _ContentsGroup_ChinaFontColor then
        buyingConditionTxt:SetFontColor(Defines.Color.C_FFFF9A38)
      else
  ...
  PAGlobalFunc_SetItemTextColor(itemNameTxt, itemESSW)
  local icon = {}
  ...
  if 100 < tonumber(tostring(sellRateTxt)) then
    ...
    priceRate_Value = "(<PAColor0xFFD05D48>" .. priceRate_Value .. "<PAOldColor>\226\150\188)"
  end
  ...
  if 1 == self._currentNpcType then
    ...
    UI.setLimitAutoWrapTextAndAddTooltip(buyingConditionTxt, 3, buyingConditionValue, nil)
    ...
    if nil ~= tradeItemWrapper:getStaticStatus():get() then
      ...
      local remainMinuate = tradeItemWrapper:getStaticStatus():get()._expirationPeriod
      local day = remainMinuate / 1440
      local hour = remainMinuate % 1440 / 60
      local min = remainMinuate % 1440 % 60
      if day > 0 then
        remainTimeString = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORLD_MAP_SIEGE_DAY", "day", tostring(day))
      end
      if hour > 0 then
        remainTimeString = remainTimeString .. " " .. PAGetStringParam1(Defines.StringSheet_GAME, "BUFF_HOUR", "time_hour", tostring(hour))
      end
      if min > 0 then
        remainTimeString = remainTimeString .. " " .. PAGetStringParam1(Defines.StringSheet_GAME, "BUFF_MINUTE", "time_minute", tostring(min))
      end
      remainTimeIcon:SetText(remainTimeString)
  ...
  loyalSupplyIcon:SetShow(false)
  local territorySupplyKey = PaGloabl_TradeEventNotice_Renewal_All_GetTerritorySupplyIndex(itemKey)
...
function PaGlobal_TradeMarketItemList_All:updateSellSlot(contents, key32)
  ...
  PAGlobalFunc_SetItemTextColor(itemName, itemESSW)
  local priceRate_Value = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_TRADEMARKET_ALL_SELLRATES", "Percent", tostring(sellRateTxt))
  if 100 <= tonumber(tostring(sellRateTxt)) then
    priceRate_Value = "(<PAColor0xFF66CC33>" .. priceRate_Value .. "<PAOldColor>\226\150\178)"
  else
  ...
  if true == isTerritorySupplyCheck and nil == self._currShopType then
...
function PaGlobal_TradeMarketItemList_All:getCurrentNodeLevel()
  local nodeLevel = -1
  if nil == Panel_Dialog_Trade_MarketItemList_All then
    return nodeLevel
  end
  ...
  local wayPointKey = regionInfo:getExplorationKey()
  return ToClient_GetNodeLevel(wayPointKey)
end
