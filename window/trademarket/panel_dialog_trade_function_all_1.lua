function PaGlobal_TradeFunction_All:initialize()
  if true == self._initialize or nil == Panel_Dialog_Trade_Function_All then
    return
  end
  ...
  self._ui._tabGroups = Array.new()
  self._ui._tabGroups:push_back(self._ui._rdo_Sell)
  if self._isConsole == false then
    self._ui._tabGroups:push_back(self._ui._rdo_Exit)
  end
  self:validate()
...
function PaGlobal_TradeFunction_All:validate()
  if Panel_Dialog_Trade_Function_All == nil then
    return
  end
...
function PaGlobal_TradeFunction_All:onScreenResize()
  ...
  Panel_Dialog_Trade_Function_All:ComputePosAllChild()
end
...
function PaGlobal_TradeFunction_All:prepareOpen()
  ...
  if self._global._isTrading == false then
    ...
    self._currentTab = self._ENUM_TAB_SELL
  ...
  Panel_Dialog_Trade_Function_All:EraseAllEffect()
  Panel_Dialog_Trade_Function_All:AddEffect("fUI_Servant_Dialog_01A", true, 0, 0)
end
...
function PaGlobal_TradeFunction_All:merchantCheck()
  ...
  for key, value in pairs(self._ui._tabGroups) do
    if value ~= nil and value:GetShow() == true then
      btnTable:push_back(value)
      count = count + 1
    end
  end
  ...
  local posX = 0
  local jindex = 0
  for key, value in pairs(btnTable) do
    if value ~= nil then
      posX = startPosX + buttonSize * jindex
      jindex = jindex + 1
      value:SetSpanSize(posX, 0)
    end
  end
  self._ui._stc_KeyGuide_LB:SetShow(false)
  self._ui._stc_KeyGuide_RB:SetShow(false)
  local isKeyguideAvaliable = count > 0
  if self._isConsole == true and isKeyguideAvaliable == true then
    self._ui._stc_KeyGuide_LB:SetShow(isKeyguideAvaliable)
    self._ui._stc_KeyGuide_RB:SetShow(isKeyguideAvaliable)
    self._ui._stc_KeyGuide_LB:SetSpanSize(btnTable[1]:GetSpanSize().x - 50, self._ui._stc_KeyGuide_LB:GetSpanSize().y)
    self._ui._stc_KeyGuide_RB:SetSpanSize(btnTable[count]:GetSpanSize().x + btnTable[count]:GetSizeX() - 10, self._ui._stc_KeyGuide_RB:GetSpanSize().y)
  end
  local currentTab = self._ui._tabGroups[self._currentTab]
  ...
  self._ui._tabGroups[self._currentTab]:SetCheck(true)
end
...
function PaGlobal_TradeFunction_All:update()
  ...
  if -1 ~= self._currentTab then
    PaGlobalFunc_MarketItemList_All_ChangeTab(self._currentTab)
    PaGlobalFunc_TradeMyBasket_All_TabChange(self._currentTab)
  end
end
...
function PaGlobal_TradeFunction_All:isMainTown()
  local selfPlayer = getSelfPlayer()
  if selfPlayer == nil then
    return false
  end
  local nowRegionInfo = selfPlayer:getCurrentRegionInfo()
  if nowRegionInfo == nil then
    return false
  end
  return nowRegionInfo:get():isMainTown()
end
function PaGlobal_TradeFunction_All:isNormalTradeNPC()
  local characterStaticStatusWrapper = npcShop_getCurrentCharacterKeyForTrade()
