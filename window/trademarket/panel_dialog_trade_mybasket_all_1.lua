function PaGlobal_TradeMyBasket_All:initialize()
  ...
  self._ui._stc_MyItemDesertBuff_Template = UI.getChildControl(self._ui._list2_MyItemContent, "Static_DesertBuff")
...
function PaGlobal_TradeMyBasket_All:validate()
  ...
  self._ui._stc_MyItemDesertBuff_Template:isValidate()
...
function PaGlobal_TradeMyBasket_All:onScreenResize()
  ...
  self._ui._list2_MyBasket:ComputePos()
  self._ui._list2_MyItem:ComputePos()
  ...
  self._ui._list2_MyBasket:requestUpdateVisible()
  self._ui._list2_MyItem:requestUpdateVisible()
end
...
function PaGlobal_TradeMyBasket_All:prepareOpen()
  ...
  if nil ~= talker then
    ...
    if false == characterStaticStatus:isSmuggleMerchant() then
      ...
      local totalSellCount = mySellCount + myFishBagSellCount + vehicleSellCount + seaVehicleSellCount
...
function PaGlobal_TradeMyBasket_All:updateMyBasketData(content, key)
  ...
  if nil ~= self._currentCartItemTable[slotIndex] then
    ...
    PAGlobalFunc_SetItemTextColor(txt_itemName, shopItemSS)
...
function PaGlobal_TradeMyBasket_All:getMyItem()
  ...
  for itemIdx = 0, myFishBagSellCount - 1 do
    ...
    if shopItemWrapper ~= nil then
      local tradeItemInfo = {
        _itemFromWhere = self._ENUM_FISH_BAG,
        _shopSlotNo = itemIdx,
        _itemKey = shopItemWrapper:getStaticStatus():get()._key:get(),
        _isNodeLinked = false,
        _isFishBag = true
  ...
  for itemIdx = 0, myItemCount - 1 do
    ...
    elseif self._ENUM_INVENTORY == _slots._itemFromWhere then
      ...
      invenType = 0
    elseif self._ENUM_FISH_BAG == _slots._itemFromWhere then
      s64_TradeItemNo = npcShop_getItemNoInFishbagToSell(_slots._shopSlotNo)
      s64_inventoryItemCount = toInt64(0, 1)
      itemValueType = inventory:getBagItemByItemNo(s64_TradeItemNo)
      invenType = CppEnums.ItemWhereType.eInventoryBag
    end
    ...
    local isFishItemInFishBag = _slots._isFishBag
    local leftPeriod = FromClient_getTradeItemExpirationDate(itemExpiration, tradeItemWrapper:getStaticStatus():get()._expirationPeriod, isNewTradeItem, isFishItemInFishBag)
    ...
    if false == isSupplyMerchant and false == isFishSupplyMerchant then
      if nil ~= Panel_Dialog_Trade_BiddingGame_All then
        realPrice = getCalculateTradeItemPrice(tradeItemWrapper:getTradeSellPrice(), tradeItemWrapper:getStaticStatus():getCommerceType(), fromPosition, selfPlayerPosition, tradeItemWrapper:getTradeGroupType(), characterStaticStatus:getTradeGroupType(), leftPeriod, PaGlobalFunc_TradeBiddingGame_All_IsSuccess() or bigHand, isNewTradeItem)
      else
        realPrice = getCalculateTradeItemPrice(tradeItemWrapper:getTradeSellPrice(), tradeItemWrapper:getStaticStatus():getCommerceType(), fromPosition, selfPlayerPosition, tradeItemWrapper:getTradeGroupType(), characterStaticStatus:getTradeGroupType(), leftPeriod, isTradeGameSuccess() or bigHand, isNewTradeItem)
      end
    else
    ...
    bonusPercent = math.floor(fromToDistanceNavi / 100 * FromClient_getTradeBonusPercent(isNewTradeItem))
    bonusPercent = math.min(bonusPercent, FromClient_getTradeMaxDistancePercent())
    local isDesertBuff = selfPlayer:get():getSellTradeItemPricePercent()
    ...
    local isApplyDesertBuff = false
    if isDesertBuff > 0 and desertBuffPercent > 100 and fromToDistanceNavi >= desertBuffDistance then
      isApplyDesertBuff = true
    end
    local nodeNolinkPercent = getNotLinkNodeSellPercent()
    ...
    if false == isLinkedNode then
      ...
      realPrice = Int64toInt32(tradeItemWrapper:getStaticStatus():get()._originalPrice_s64) * nodeNolinkPercent / self._CONST_CALCULATE_PERCENT_VALUE / 100
      f_sellRate = nodeNolinkPercent / self._CONST_CALCULATE_PERCENT_VALUE
    ...
    if true == isSupplyMerchant or true == isFishSupplyMerchant then
      ...
      profitItemGold = toInt64(0, profitItemGold)
    end
    local itemData = {}
    ...
    itemData.desertBuff = isApplyDesertBuff
    itemData.isInFishBagItem = _slots._isFishBag
    self._myItemDataTable[itemIdx] = itemData
    self._realPriceList[itemIdx] = Int64toInt32(realPrice)
    self._totalProfit = self._totalProfit + profitItemGold * s64_inventoryItemCount
  end
  if totalSellCount > 0 then
...
function PaGlobal_TradeMyBasket_All:updateMyItemData(content, key)
  if self._myItemDataTable == nil or key == nil then
    return
  end
  ...
  local isFishSupplyMerchant = characterStaticStatus:isFishSupplyMerchant()
  local isSupplyMerchant = characterStaticStatus:isSupplyMerchant()
  local isLoyalFlag = isFishSupplyMerchant == true or isSupplyMerchant == true
  local itemWrapper
  if itemData.itemFromWhere == self._ENUM_LAND_VEHICLE then
    itemWrapper = npcShop_getVehicleItemWrapper(itemData.invenNum)
  elseif itemData.itemFromWhere == self._ENUM_SEA_VEHICLE then
    itemWrapper = npcShop_getShipItemWrapper(itemData.invenNum)
  elseif itemData.itemFromWhere == self._ENUM_INVENTORY then
    itemWrapper = npcShop_getItemWrapperByShopSlotNo(itemData.invenNum)
  elseif itemData.itemFromWhere == self._ENUM_FISH_BAG then
    itemWrapper = npcShop_getItemWrapperInFishBagByShopSlotNo(itemData.invenNum)
  else
  ...
  local stc_DesertBuff = UI.getChildControl(content, "Static_DesertBuff")
  btn_Sell:SetShow(not self._isConsole)
  local bonusFloat = bonusPercent / self._CONST_CALCULATE_PERCENT_VALUE
  local digitFloat = math.ceil(bonusFloat * 10 - 0.5) / 10
  if digitFloat <= 0 then
    digitFloat = 0
  end
  local distanceBonusString = digitFloat .. "%"
  ...
  if itemData.itemFromWhere == self._ENUM_LAND_VEHICLE or itemData.itemFromWhere == self._ENUM_SEA_VEHICLE then
  ...
  PAGlobalFunc_SetItemTextColor(txt_ItemName, itemSSW)
  ...
  if true == itemData.desertBuff then
    stc_DesertBuff:SetShow(true)
  else
    stc_DesertBuff:SetShow(false)
  end
  local mySellList = {
    _txt_ItemName = nil,
    _itemName = nil,
  ...
  mySellList._stc_DesertBuff = stc_DesertBuff
  ...
  if false == self._isConsole then
    ...
    local tooltipType = 0
    if itemData.itemFromWhere == self._ENUM_INVENTORY then
      tooltipType = 1
    elseif itemData.itemFromWhere == self._ENUM_LAND_VEHICLE then
      tooltipType = 2
    elseif itemData.itemFromWhere == self._ENUM_SEA_VEHICLE then
      tooltipType = 3
    elseif itemData.itemFromWhere == self._ENUM_FISH_BAG then
    ...
    stc_DesertBuff:addInputEvent("Mouse_On", "HandleEventOnOut_TradeMyBasket_All_SimpleTooltip(true, 7," .. key32 .. ")")
    ...
    stc_DesertBuff:addInputEvent("Mouse_Out", "HandleEventOnOut_TradeMyBasket_All_SimpleTooltip()")
  else
...
function PaGlobal_TradeMyBasket_All:checkVehicle(cartSlot, cartWeight)
  ...
  if nil ~= landServantInfoWrapper then
    ...
    if true == isAbleDistance then
      ...
      if cartSlot > 0 and true == self._ui._rdo_LandVehicle:IsCheck() then
  ...
  if true == ToClient_IsContentsGroupOpen("576") and ToClient_IsContentsGroupOpen("504") then
    ...
    if nil ~= seaServantInfoWrapper then
      ...
      if true == isAbleDistance then
        ...
        local freeInventorySlot = 0
        if cartSlot > 0 and true == self._ui._rdo_SeaVehicle:IsCheck() then
          freeInventorySlot = math.abs(maxInventorySlot - inventory:getFreeCount()) + cartSlot
        else
          freeInventorySlot = math.abs(maxInventorySlot - inventory:getFreeCount())
        end
