function PaGlobalFunc_TradeMarketItemToolTip_All_Show(itemKey, isWorldMap, isBuyAble)
  ...
  PAGlobalFunc_SetItemTextColor(currentUI._txt_ItemName, itemESSW)
  ...
  if false == isWorldMap or nil == isWorldMap then
    ...
    if true == _ContentsGroup_UsePadSnapping and nil ~= isBuyAble then
      PaGlobalFunc_TradeMarketItemList_All_ShowKeyGuideX(isBuyAble)
    end
    if true == _ContentsGroup_UsePadSnapping and false == isBuyAble then
      PaGlobal_TradeMarketItemList_All._ui._stc_KeyGuide_A:SetShow(false)
    else
      PaGlobal_TradeMarketItemList_All._ui._stc_KeyGuide_A:SetShow(true)
    end
  else
