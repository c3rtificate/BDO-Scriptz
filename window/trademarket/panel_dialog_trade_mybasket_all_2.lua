...
function PaGlobalFunc_TradeMyBasket_All_ConfirmToMyInventory()
  ...
  PaGlobalFunc_TradeMyBasket_All_Send_DoBuy(CppEnums.ItemWhereType.eMoneyInventory, CppEnums.ItemWhereType.eInventory)
end
function PaGlobalFunc_TradeMyBasket_All_ToMyVehicle()
  ...
  PaGlobalFunc_TradeMyBasket_All_Send_DoBuy(CppEnums.ItemWhereType.eMoneyInventory, CppEnums.ItemWhereType.eServantInventory)
end
function PaGlobalFunc_TradeMyBasket_All_GetBasketTotalWeight()
  local basketTotalWeight = 0
  if nil == Panel_Dialog_Trade_MyBasket_All then
    return basketTotalWeight
  end
  if nil == PaGlobal_TradeMyBasket_All._currentCartItemTable or nil == PaGlobal_TradeMyBasket_All._currentSlotCount then
    return basketTotalWeight
  end
  for i = 1, PaGlobal_TradeMyBasket_All._currentSlotCount do
    if nil ~= PaGlobal_TradeMyBasket_All._currentCartItemTable[i] and toInt64(0, 0) ~= PaGlobal_TradeMyBasket_All._currentCartItemTable[i]._count then
      ...
      local itemKey = PaGlobal_TradeMyBasket_All._currentCartItemTable[i]._itemKey
      local itemSSW = getItemEnchantStaticStatus(ItemEnchantKey(itemKey))
      basketTotalWeight = basketTotalWeight + itemSSW:get()._weight * itemCount32
    end
  end
  return basketTotalWeight
end
function PaGlobalFunc_TradeMyBasket_All_Send_DoBuy(fromWhere, toWhere)
  ...
  local servantType = 0
  local myMoney64 = getSelfPlayer():get():getMoneyInventory():getMoney_s64()
  ...
  ToClient_RequestTradeWareHouse(PaGlobal_Window_TradeWareHouse_All._currentRegionKey)
end
...
function PaGlobalFunc_TradeMyBasket_All_ConfirmSell(inputNumber, slotIdx)
  ...
  elseif PaGlobal_TradeMyBasket_All._ENUM_FISH_BAG == itemData.itemFromWhere then
    s64_TradeItemNo = npcShop_getItemNoInFishbagToSell(itemData.invenNum)
  ...
  if false == isLinkedNode and 0 ~= regionInfo._waypointKey then
    ...
    if true == isNodeFreeTrade then
      ...
      local nodeString = PAGetStringParam3(Defines.StringSheet_GAME, "Lua_TradeMarketSellList_NeedNodeLink", "exploreNode1", talker:getExplorationNodeName(), "exploreNode2", itemWrapper:getProductionRegion(), "sellPercent", nodeNolinkPercent / PaGlobal_TradeMyBasket_All._CONST_CALCULATE_PERCENT_VALUE)
      local messageboxData = {
    ...
    local rv
    if 5 == tradeType then
      rv = npcShop_doSellInTradeShop(itemData.invenNum, Int64toInt32(inputNumber), itemData.inventoryType, CppEnums.ItemWhereType.eGuildWarehouse)
    elseif PaGlobal_TradeMyBasket_All._ENUM_INVENTORY == itemData.itemFromWhere then
      rv = npcShop_doSellInTradeShop(itemData.invenNum, Int64toInt32(inputNumber), itemData.inventoryType, CppEnums.ItemWhereType.eMoneyInventory)
    elseif PaGlobal_TradeMyBasket_All._ENUM_FISH_BAG == itemData.itemFromWhere then
    elseif PaGlobal_TradeMyBasket_All._ENUM_LAND_VEHICLE == itemData.itemFromWhere then
      rv = npcShop_doSellInTradeShop(itemData.invenNum, Int64toInt32(inputNumber), itemData.inventoryType, CppEnums.ItemWhereType.eMoneyInventory, 0)
    elseif PaGlobal_TradeMyBasket_All._ENUM_SEA_VEHICLE == itemData.itemFromWhere then
      rv = npcShop_doSellInTradeShop(itemData.invenNum, Int64toInt32(inputNumber), itemData.inventoryType, CppEnums.ItemWhereType.eMoneyInventory, 1)
    end
  end
end
function PaGlobalFunc_TradeMyBasket_All_SellComplete()
  ...
  if 5 == sellData.tradeType then
    rv = npcShop_doSellInTradeShop(shopNumber, sellCount, toWhere, CppEnums.ItemWhereType.eGuildWarehouse)
  elseif PaGlobal_TradeMyBasket_All._ENUM_INVENTORY == itemFromWhere then
    rv = npcShop_doSellInTradeShop(shopNumber, sellCount, toWhere, CppEnums.ItemWhereType.eMoneyInventory)
  elseif PaGlobal_TradeMyBasket_All._ENUM_FISH_BAG == itemFromWhere then
    rv = npcShop_doSellInTradeShop(shopNumber, sellCount, toWhere, CppEnums.ItemWhereType.eMoneyInventory)
  elseif PaGlobal_TradeMyBasket_All._ENUM_LAND_VEHICLE == itemFromWhere then
    rv = npcShop_doSellInTradeShop(shopNumber, sellCount, toWhere, CppEnums.ItemWhereType.eMoneyInventory, 0)
  elseif PaGlobal_TradeMyBasket_All._ENUM_SEA_VEHICLE == itemFromWhere then
    rv = npcShop_doSellInTradeShop(shopNumber, sellCount, toWhere, CppEnums.ItemWhereType.eMoneyInventory, 1)
  end
  if 0 == rv then
...
function HandleEventLUp_TradeMyBasket_All_SellAll()
  ...
  local totalProfitString = makeDotMoney(PaGlobal_TradeMyBasket_All._totalProfit)
...
function PaGlobalFunc_TradeMyBasket_All_SellAllContinue()
  ...
  for goodsIdx = 0, myInventortySellAbleCount do
    ...
    if nil ~= shopItemWrapper then
      ...
      if 5 == tradeType then
        ...
        local myInvenMoney = Int64toInt32(getSelfPlayer():get():getMoneyInventory():getMoney_s64())
  ...
  local myFishBagSellAbleCount = npcShop_getSellCountInFishBag()
  for fishIdx = 0, myFishBagSellAbleCount - 1 do
    local shopItemWrapper = npcShop_getItemmInFishBagToSell(fishIdx)
    if shopItemWrapper ~= nil then
      local tradeType = shopItemWrapper:getStaticStatus():get()._tradeType
      s64_TradeItemNo = npcShop_getItemNoInFishbagToSell(fishIdx)
      s64_inventoryItemCount = toInt64(0, 1)
      if 5 == tradeType then
        rv = npcShop_doSellInTradeShop(fishIdx, Int64toInt32(s64_inventoryItemCount), CppEnums.ItemWhereType.eInventoryBag, 14)
        priceIndex = priceIndex + 1
        sellCount = sellCount + 1
      else
        local realPrice = PaGlobal_TradeMyBasket_All._realPriceList[itemIdx]
        ...
        rv = npcShop_doSellInTradeShop(fishIdx, Int64toInt32(s64_inventoryItemCount), CppEnums.ItemWhereType.eInventoryBag, CppEnums.ItemWhereType.eMoneyInventory)
        priceIndex = priceIndex + 1
        sellCount = sellCount + 1
      end
    end
          ...
          if not servertinventory:empty(slotCount) then
            ...
            if nil ~= s64_VehicleItemNo then
              ...
              if nil ~= servantItemWrapper then
                ...
                local limitCount = math.floor((PaGlobal_TradeMyBasket_All._LIMITED_SELLMONEY - myInvenMoney) / realPrice)
                priceIndex = priceIndex + 1
                rv = npcShop_doSellInTradeShop(slotCount - 2 - emptyCount, vehicleItemCount, 4, CppEnums.ItemWhereType.eMoneyInventory, servantType)
                sellCount = sellCount + 1
              end
            end
          else
...
function HandleEventLUp_TradeMyBasket_All_BiddingGameStart()
  ...
  if -1 ~= PaGlobalFunc_CharacterTag_All_GetDuelCharcterIndex() then
...
function HandleEventOnOut_TradeMyBasket_All_SimpleTooltip(isShow, tipType, index)
  ...
  elseif 7 == tipType then
    ...
    control = myItemSlotTable._stc_DesertBuff
