...
function PaGlobal_Window_Mail_All:registEventHandler()
  ...
  if false == self._isConsole then
    ...
    self._ui_pc.btn_allRecieve:SetTextMode(__eTextMode_LimitText)
    self._ui_pc.btn_allRecieve:addInputEvent("Mouse_On", "PaGlobal_Window_Mail_All_GetAllMoney(true)")
    self._ui_pc.btn_allRecieve:addInputEvent("Mouse_Out", "PaGlobal_Window_Mail_All_GetAllMoney()")
    PaGlobal_Util_RegistHelpTooltipEvent(self._ui_pc.btn_question, "\"Panelmail\"")
  else
    Panel_Window_Mail_All:registerPadEvent(__eConsoleUIPadEvent_Up_LB, "HandleEventLUp_Mail_All_ChangePage(false)")
    Panel_Window_Mail_All:registerPadEvent(__eConsoleUIPadEvent_Up_RB, "HandleEventLUp_Mail_All_ChangePage(true)")
...
function PaGlobal_Window_Mail_All:open()
  ...
  ToClient_UpdateOpenUICondition(__eCheckOpenUIType_Mail)
end
function PaGlobal_Window_Mail_All:prepareClose()
  ...
  if nil ~= PaGlobalFunc_Widget_Alert_SetUnReadMailCount then
    PaGlobalFunc_Widget_Alert_SetUnReadMailCount()
  end
  if nil ~= PaGlobalFunc_Widget_Alert_SetExpiredMailBubble then
...
function PaGlobal_Window_Mail_All:updateMailData()
  ...
  local pageNo = 0
  local pageIndex = 1
  local showTime = 604800
  local importantNearDateMailInfoList = {}
  local importantMailInfoList = {}
  local importantNoDateMailInfoList = {}
  local nearExpireDateMailInfoList = {}
  local expireDateMailInfoList = {}
  local noDateMailInfoList = {}
  local importantNearDateMailCount = 0
  local importantMailCount = 0
  local importantNoDateMailCount = 0
  local nearExpireDateMailCount = 0
  local expireDateMailCount = 0
  local noDateMailCount = 0
  ...
  for massMailIndex = 1, massMailCount do
    ...
    if massMailInfo ~= nil then
      local data = {
        ...
        _mail_GetExpireDate = massMailInfo:getMailExpireDateTime64(),
        _isMassMail = true
      }
      if massMailInfo:isImportantItem() == true then
        if data._mail_GetExpireDate ~= toInt64(0, 0) then
          if getLeftSecond_TTime64(data._mail_GetExpireDate) <= toInt64(0, showTime) then
            importantNearDateMailInfoList[importantNearDateMailCount] = data
            importantNearDateMailCount = importantNearDateMailCount + 1
          else
            importantMailInfoList[importantMailCount] = data
            importantMailCount = importantMailCount + 1
          end
        else
          importantNoDateMailInfoList[importantNoDateMailCount] = data
          importantNoDateMailCount = importantNoDateMailCount + 1
        end
      elseif data._mail_GetExpireDate ~= toInt64(0, 0) then
        if getLeftSecond_TTime64(data._mail_GetExpireDate) <= toInt64(0, showTime) then
          nearExpireDateMailInfoList[nearExpireDateMailCount] = data
          nearExpireDateMailCount = nearExpireDateMailCount + 1
        else
          expireDateMailInfoList[expireDateMailCount] = data
          expireDateMailCount = expireDateMailCount + 1
        end
      else
        noDateMailInfoList[noDateMailCount] = data
        noDateMailCount = noDateMailCount + 1
      end
    end
  end
  local mailCount = RequestMail_mailCount()
  for mailIndex = 1, mailCount do
    local mailInfo = RequestMail_getMailAt(mailIndex - 1)
    if mailInfo ~= nil then
      local data = {
        _indx = mailIndex - 1,
        _sender_Name = mailInfo:getSender(),
        _mail_Title = mailInfo:getTitle(),
        _mail_No = mailInfo:getMailNo(),
        _mail_GetItem = mailInfo:isExistItem(),
        _mail_GetDate = mailInfo:getMailReceiveDate(),
        _mail_GetExpireDate = mailInfo:getMailExpireDateTime64(),
        _isMassMail = false
      }
      if mailInfo:isImportantItem() == true then
        if data._mail_GetExpireDate ~= toInt64(0, 0) then
          if getLeftSecond_TTime64(data._mail_GetExpireDate) <= toInt64(0, showTime) then
            importantNearDateMailInfoList[importantNearDateMailCount] = data
            importantNearDateMailCount = importantNearDateMailCount + 1
  ...
  pageNo, pageIndex = self:updateMailDataByList(pageNo, pageIndex, importantNearDateMailInfoList, importantNearDateMailCount)
  pageNo, pageIndex = self:updateMailDataByList(pageNo, pageIndex, importantMailInfoList, importantMailCount)
  pageNo, pageIndex = self:updateMailDataByList(pageNo, pageIndex, importantNoDateMailInfoList, importantNoDateMailCount)
  pageNo, pageIndex = self:updateMailDataByList(pageNo, pageIndex, nearExpireDateMailInfoList, nearExpireDateMailCount)
  pageNo, pageIndex = self:updateMailDataByList(pageNo, pageIndex, expireDateMailInfoList, expireDateMailCount)
  pageNo, pageIndex = self:updateMailDataByList(pageNo, pageIndex, noDateMailInfoList, noDateMailCount)
  self._maxMailCount = massMailCount + mailCount
  self._ui.txt_mailCount:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_NO") .. "." .. tostring(self._maxMailCount))
  if 0 == pageNo then
    pageNo = 1
  end
  self._Page_Total = pageNo
end
function PaGlobal_Window_Mail_All:updateMailDataByList(pageNo, pageIndex, list, count)
  if list == nil or count == nil or pageNo == nil or pageIndex == nil then
    return 0, 0
  end
  for index = 0, count - 1 do
    local data = list[index]
    if data ~= nil then
      if (pageIndex - 1) % self._rowMax == 0 then
        pageNo = pageNo + 1
        pageIndex = 1
      end
...
function PaGlobal_Window_Mail_All:updateMailControl()
  ...
  for index = 1, self._rowMax do
    if nil ~= self._mailData[self._Page_Current] then
      if nil ~= self._mailData[self._Page_Current][index] then
        ...
        local _getExpireDate = self._mailData[self._Page_Current][index]._mail_GetExpireDate
        ...
        self._ui._mail[index].txt_sender:SetTextMode(__eTextMode_LimitText)
        ...
        if nil ~= _getExpireDate and toInt64(0, 0) ~= _getExpireDate then
          local showTime = 604800
          local hourTime = 86400
          local leftTime = getLeftSecond_TTime64(_getExpireDate)
          local expireText = ""
          if leftTime <= toInt64(0, hourTime) then
            expireText = convertStringFromDatetimeOverHourForMail(leftTime)
            self._ui._mail[index].txt_expirateDate:SetShow(true)
            self._ui._mail[index].txt_expirateDate:SetTextMode(__eTextMode_LimitText)
            self._ui._mail[index].txt_expirateDate:SetText(expireText)
          elseif leftTime <= toInt64(0, showTime) then
            expireText = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_MAILREMAIN_DAY", "day", tostring(calculateDayFromDateDay(leftTime)))
            self._ui._mail[index].txt_expirateDate:SetShow(true)
            self._ui._mail[index].txt_expirateDate:SetTextMode(__eTextMode_LimitText)
            self._ui._mail[index].txt_expirateDate:SetText(expireText)
          else
            self._ui._mail[index].txt_expirateDate:SetShow(false)
          end
        else
          self._ui._mail[index].txt_expirateDate:SetShow(false)
        end
  ...
  local massMailCount = Int64toInt32(ToClient_GetMassMailCount())
  if self._Page_Total > 0 or massMailCount > 0 then
    local pageCount = tostring(self._Page_Current) .. "/" .. tostring(self._Page_Total)
    self._ui.txt_pageNumber:SetText(pageCount)
  end
end
  ...
  for index = 1, self._rowMax do
    if nil == self._ui._mail[index] then
      ...
      local mail = {
        ...
        txt_expirateDate = UI.getChildControl(mailbutton, "StaticText_Date")
      }
      ...
      mail.txt_expirateDate:SetShow(false)
