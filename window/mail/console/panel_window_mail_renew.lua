...
function MailInfo:Init_Control()
  ...
  for ii = 1, self._rowMax do
    ...
    self._ui._Sender_Name[ii]:SetTextMode(__eTextMode_LimitText)
    self._ui._Mail_Title[ii] = UI.getChildControl(self._ui._List_BG[ii], "StaticText_Title")
    self._ui._Mail_Title[ii]:SetTextMode(__eTextMode_LimitText)
  ...
  self._ui.txt_keyGuideY = UI.getChildControl(self._ui.stc_bottomBG, "StaticText_KeyGuideY")
  self._ui.txt_keyGuideY:SetShow(_ContentsGroup_isPS4UI)
  local keyGuides = {
...
function MailInfo:registEventHandler()
  ...
  if _ContentsGroup_isPS4UI then
    _panel:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "MailList_RefreshPS4CacheMail()")
  end
  ...
  for index = 1, SlotMax do
    if nil ~= self._Data[pageNo] then
      if nil ~= self._Data[pageNo][index] then
        ...
        MailInfo._ui._Mail_Title[index]:SetText("<PAColor0xFFf3d900>" .. _mail_Title .. "<PAOldColor>")
        ...
        if true == _getItem then
          MailInfo._ui.stc_haveItemEnclosed[index]:SetColor(Defines.Color.C_FFFFCE22)
          MailInfo._ui._Mail_Title[index]:SetFontColor(Defines.Color.C_FFFFFFFF)
          MailInfo._ui._Mail_Title[index]:SetMonoTone(false)
        else
          MailInfo._ui.stc_haveItemEnclosed[index]:SetColor(Defines.Color.C_FFFFFFFF)
          MailInfo._ui._Mail_Title[index]:SetFontColor(Defines.Color.C_FFC4BEBE)
          MailInfo._ui._Mail_Title[index]:SetMonoTone(true)
        end
...
function PaGlobalFunc_Mail_GetDetail()
  ...
  RequestMail_getMailDetail(_mail_Data._Data[pageNo][index]._mailNo)
end
