function PaGlobal_Window_Mail_All_Open()
  if true == PaGlobal_Window_Mail_All._isWithReward then
...
function HandleEventLUp_Mail_All_GetDetail(index)
  ...
  if PaGlobal_Window_Mail_All._isConsole and false == MessageBox.isPopUp() then
    ToClient_padSnapSetTargetPanel(Panel_Window_MailDetail_All)
  end
  ...
  if true == isMassMail then
    ...
    RequestMail_getMailDetail(PaGlobal_Window_Mail_All._mailData[PaGlobal_Window_Mail_All._Page_Current][index]._mail_No)
  end
end
...
function PaGlobal_Window_Mail_All_GetAllMoney(isShow)
  if nil == isShow then
    TooltipSimple_Hide()
    return
  end
  local getTextMode = PaGlobal_Window_Mail_All._ui_pc.btn_allRecieve:IsLimitText()
  if true == isShow and true == getTextMode then
    local name = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_MAIL_MONEY_ALL_GET_BTN")
    local desc = ""
    local control = PaGlobal_Window_Mail_All._ui_pc.btn_allRecieve
