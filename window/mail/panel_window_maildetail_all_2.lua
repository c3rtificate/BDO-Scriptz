function PaGlobal_Window_MailDetail_All_Open(mailNo)
  ...
  elseif PaGlobal_Window_MailDetail_All:CheckItemGetAfterDelete(mailNo) == true then
    return
  end
...
function PaGlobal_Window_MailDetail_All_MoveScroll(isUp)
  if true == isUp then
    PaGlobal_Window_MailDetail_All._ui.frame_vScroll:ControlButtonUp()
  elseif false == isUp then
    PaGlobal_Window_MailDetail_All._ui.frame_vScroll:ControlButtonDown()
  end
  PaGlobal_Window_MailDetail_All._ui.frame_mailText:UpdateContentPos()
  PaGlobal_Window_MailDetail_All._ui.frame_mailText:UpdateContentScroll()
end
function HandleEventLUp_MailDetail_All_ReceiveItemConsole()
  if false == PaGlobal_Window_MailDetail_All._ui_console.txt_receiveItem:GetShow() then
    return
  end
...
function HandleEventLUp_MailDetail_All_Delete()
  if Panel_Window_NumberPad_All ~= nil and Panel_Window_NumberPad_All:GetShow() == true then
    return
  end
...
function HandleEventLUp_MailDetail_All_GetItemConsole()
  local itemESSW
  if PaGlobal_Window_MailDetail_All.isMassMail then
...
function HandleEventLUp_MailDetail_All_CheckAutoDeleteMail()
  if PaGlobal_Window_MailDetail_All._mainControl == nil then
    return
  end
  local control = PaGlobal_Window_MailDetail_All._ui.btn_checkAutoDelete
  if control == nil then
    return
  end
  local isCheck = control:IsCheck()
  ToClient_getGameUIManagerWrapper():setLuaCacheDataListBool(__eUseDeleteMailAfterGetItem, isCheck, CppEnums.VariableStorageType.eVariableStorageType_User)
end
function HandleEventLUp_MailDetail_All_ToggleAutoDelete()
  if PaGlobal_Window_MailDetail_All._mainControl == nil then
    return
  end
  local control = PaGlobal_Window_MailDetail_All._ui.btn_checkAutoDelete
  ...
  if control:GetShow() == false then
    return
  end
  local isCheck = not control:IsCheck()
...
function FromClient_MailDetail_All_ShowMassMailDetail(mailNo)
  ...
  local deleteDate = ToClient_GetSelectMassMailDeleteDate()
  if nil ~= deleteDate then
    ...
    PaGlobal_Window_MailDetail_All._ui.txt_mailExpirateDate:SetText(deleteDateStr)
    PaGlobal_Window_MailDetail_All._ui.stc_mailExpirateDate:SetShow(true)
  else
    PaGlobal_Window_MailDetail_All._ui.stc_mailExpirateDate:SetShow(false)
  end
  if true == PaGlobal_Window_MailDetail_All._ui.stc_mailExpirateDate:GetShow() then
    PaGlobal_Window_MailDetail_All._ui.frame_mailText:SetSize(PaGlobal_Window_MailDetail_All._ui.frame_mailText:GetSizeX(), PaGlobal_Window_MailDetail_All.mailTextSizeY - PaGlobal_Window_MailDetail_All._ui.stc_mailExpirateDate:GetSizeY())
  else
    PaGlobal_Window_MailDetail_All._ui.frame_mailText:SetSize(PaGlobal_Window_MailDetail_All._ui.frame_mailText:GetSizeX(), PaGlobal_Window_MailDetail_All.mailTextSizeY)
  end
  ...
  PaGlobal_Window_MailDetail_All._ui.btn_checkAutoDelete:SetShow(isExistItem)
  if PaGlobal_Window_MailDetail_All._isConsole == true then
    PaGlobal_Window_MailDetail_All._ui_console.txt_autoDelete:SetShow(isExistItem)
  end
  PaGlobal_Window_MailDetail_All:SetDynamicPanelSize()
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(PaGlobal_Window_MailDetail_All._ui_console.keyGuides, PaGlobal_Window_MailDetail_All._ui_console.stc_consoleBottomBG, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP)
end
function FromClient_MailDetail_All_ResponseMailGetItem(itemKey, itemCount_s64, immediateItem, isRelay)
  ...
  elseif 1 == itemKey and isRelay == true then
...
function FromClient_Window_MailDetail_All_ReSizePanel()
  ...
  PaGlobal_Window_MailDetail_All:reposition()
  if false == PaGlobal_Window_MailDetail_All._isConsole then
