...
function PaGlobal_Expirience_All:prepareOpen()
  ...
  if true == isUseTotalAccount() or true == isUseTotalAccountVer2() then
...
function PaGlobal_Expirience_All:SetUrlTotalAccount(token)
  ...
  if nil == self._directLinkUrl and true == self._isNoticeTapOpen then
    url = url .. "/NewsList"
    url = url .. "?_token=" .. tostring(token) .. "&_userNo=" .. tostring(userNo)
    if isUseTotalAccountVer2() == true then
      url = url .. "&_gmType=" .. tostring(isGm)
    else
      url = url .. "&isGm=" .. tostring(isGm)
    end
  elseif nil == self._directLinkUrl and true == self._isEventTabOpen then
    if isUseTotalAccountVer2() == true then
      url = url .. "/NewsList?categoryNo=3"
    else
  ...
  elseif nil ~= self._directLinkUrl and false == self._isWiki then
...
function PaGlobal_Expirience_All:prepareClose()
  if false == Panel_Window_ExpirienceWiki_All:IsShow() or true == Panel_Window_ExpirienceWiki_All:IsUISubApp() then
    return
  end
...
function FromClient_OpenAdventurerNote(token)
  ...
  if ToClient_WorldMapIsShow() then
    ...
    WorldMapPopupManager:push(Panel_Window_ExpirienceWiki_All, true)
  end
