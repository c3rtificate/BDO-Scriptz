...
function PaGlobal_ExpirienceWiki_All_Close_FromESC()
  if Panel_Window_ExpirienceWiki_All:IsUISubApp() == true then
    return
  end
  PaGlobal_Expirience_All:prepareClose()
end
...
function PaGlobal_ExpirienceWiki_GetShow()
  if Panel_Window_ExpirienceWiki_All == nil then
    return false
  end
  return Panel_Window_ExpirienceWiki_All:GetShow()
end
function PaGlobal_ExpirienceWiki_All_SetDirectLink(link)
  local eventS, eventE = string.find(link, "News")
  if nil ~= eventS and nil ~= eventE then
    PaGlobal_Expirience_All._isEventTabOpen = true
  end
  PaGlobal_Expirience_All._directLinkUrl = link
end
function PaGlobal_ExpirienceWiki_All_SetIsWiki()
  PaGlobal_Expirience_All._isWiki = true
end
function PaGlobal_ExpirienceWiki_All_EventNotifyOpen()
  PaGlobal_Expirience_All._isEventTabOpen = true
