...
function VillageTentPopup:initialize()
  ...
  if 0 == nationType then
    self._ui.txt_bottomDesc:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_HOUSE_INSTALLATIONMODE_VILLAGETENTPOPUP_DESC_NA"))
  elseif 1 == nationType then
    self._ui.txt_bottomDesc:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_HOUSE_INSTALLATIONMODE_VILLAGETENTPOPUP_DESC_EU"))
  else
    self._ui.txt_bottomDesc:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_HOUSE_INSTALLATIONMODE_VILLAGETENTPOPUP_DESC_ASIA"))
  end
  self._ui.txt_joinDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  for index = 0, maxCount - 1 do
    _dayControl[index] = {}
    temp[index] = {}
    temp[index]._day = UI.createControl(__ePAUIControl_StaticText, _panel, "StaticText_Day_" .. index)
    ...
    temp[index]._regionName = UI.createControl(__ePAUIControl_StaticText, _panel, "StaticText_RegionName_" .. index)
...
function VillageTent_SetText(count)
  ...
  for index = 0, maxCount - 1 do
    if index < count then
      ...
      tempString = PAGetString(Defines.StringSheet_GAME, tempString .. tostring(taxLevel + 1))
      if 0 == taxLevel then
        local grade = _regionInfoWrapper[index]:getSiegeOptionTypeByRegion()
