...
function HandleEventLUp_House_InstallationList_All_InstallFurniture(invenType, invenSlotNo, iscash, productNo)
  ...
  if true == PaGlobal_House_InstallationList_All._isMansion and PaGlobal_House_InstallationList_All._eTabMenu.BaseMaterial ~= PaGlobal_House_InstallationList_All._currentTabIndex then
    local itemSSW
    if true == iscash then
    ...
    if nil ~= itemSSW and nil ~= itemSSW:getCharacterStaticStatus() and nil ~= itemSSW:getCharacterStaticStatus():getObjectStaticStatus() then
      ...
      if installationType == CppEnums.InstallationType.eType_WallPaper or installationType == CppEnums.InstallationType.eType_FloorMaterial then
        Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_UI_MANTION_CRAFT"))
        return
      end
    end
  end
...
function HandleEventPad_House_InstallationList_All_LBorRB(isLB)
  local tabIndex = PaGlobal_House_InstallationList_All._currentTabIndex
  if true == isLB then
    tabIndex = tabIndex - 1
    if tabIndex < PaGlobal_House_InstallationList_All._eTabMenu.All then
      tabIndex = PaGlobal_House_InstallationList_All._eTabMenu.Tools
    end
  else
    tabIndex = tabIndex + 1
    if tabIndex > PaGlobal_House_InstallationList_All._eTabMenu.Tools then
      tabIndex = PaGlobal_House_InstallationList_All._eTabMenu.All
    end
  end
  if true == ToClient_VisitHouseIsMansionLand() and tabIndex == PaGlobal_House_InstallationList_All._eTabMenu.BaseMaterial then
    if true == isLB then
      tabIndex = PaGlobal_House_InstallationList_All._eTabMenu.Furniture
    else
      tabIndex = PaGlobal_House_InstallationList_All._eTabMenu.Tools
    end
  end
