function PaGlobal_House_Installation_All:initialize()
  ...
  self._ui._txt_pointBase = UI.getChildControl(self._ui._stc_titleGroup, "StaticText_InteriorPointBase")
  self._ui._txt_pointOption = UI.getChildControl(self._ui._stc_titleGroup, "StaticText_InteriorPointOption")
  self._ui._txt_pointBonus = UI.getChildControl(self._ui._stc_titleGroup, "StaticText_InteriorPointBonus")
  self._ui._txt_pointTotal = UI.getChildControl(self._ui._stc_titleGroup, "StaticText_InteriorPointTotal")
...
function PaGlobal_House_Installation_All:prepareOpen()
  ...
  if housing_isBuildMode() then
    ...
    if false == houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isFixedHouse() and false == houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isInnRoom() and false == houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isMansionLand() then
      self._eInstallMode = self._eInstallType.seed
      self:updateUseFarm()
    else
  ...
  local isSiegeKitMode = false
  if nil ~= Panel_House_InstallationMode_Siege_All and true == Panel_House_InstallationMode_Siege_All:GetShow() then
    isSiegeKitMode = true
  end
  if false == self._isInstallMode and false == isSiegeKitMode then
    self._ui._stc_consoleKeyBG:SetShow(self._isConsole)
  else
...
function PaGlobal_House_Installation_All:updateInteriorPoint()
  ...
  local houseWrapper = housing_getHouseholdActor_CurrentPosition()
  local isMansion = true
  if nil == houseWrapper then
    return
  end
  isMansion = houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isMansionLand()
...
function PaGlobal_House_Installation_All:updateUseFarm()
  ...
  if true == houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isFixedHouse() or true == houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isInnRoom() or true == houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isMansionLand() then
    return
  end
...
function PaGlobal_House_Installation_All:updateInteriorSensePoint()
  ...
  if interiorSensePoint > 0 and false == PaGlobal_House_InstallationMode_ObjectControl_All_GetShowPanel() then
...
function PaGlobal_House_Installation_All:setKeyGuide(modeType)
  ...
  if nil ~= houseWrapper then
    isFixed = houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isFixedHouse() or houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isInnRoom() or houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isMansionLand()
  end
  if nil ~= characterStaticWrapper then
