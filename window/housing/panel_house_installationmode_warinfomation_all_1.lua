function PaGlobal_WarInfomation_All:initialize()
  ...
  self._ui._btn_expand = UI.getChildControl(self._ui._stc_titleBg, "Button_Expand")
  self._ui._btn_small = UI.getChildControl(self._ui._stc_titleBg, "Button_Small")
  ...
  self._defaultTitleBgSizeX = self._ui._stc_titleBg:GetSizeX()
  if false == self._isConsole then
    self._ui._btn_expand:SetShow(not self._isExpandMode)
    self._ui._btn_small:SetShow(self._isExpandMode)
  else
    self._ui._btn_expand:SetShow(false)
...
function PaGlobal_WarInfomation_All:registEventHandler()
  ...
  self._ui._btn_expand:addInputEvent("Mouse_LUp", "PaGlobal_WarInfomation_All:updateExpandMode(true)")
  self._ui._btn_small:addInputEvent("Mouse_LUp", "PaGlobal_WarInfomation_All:updateExpandMode(false)")
end
function PaGlobal_WarInfomation_All:prepareOpen(buildingInfo)
  ...
  if nil ~= buildingInfo then
    self._isOpenFromWorldMap = true
    if false == self._isExpandMode then
      self._ui._stc_categoryBg:SetShow(true)
      self._ui._stc_detailGroup:SetShow(true)
    end
    self._ui._btn_expand:SetShow(false)
    self._ui._btn_small:SetShow(false)
  else
    self._isOpenFromWorldMap = false
    self:updateExpandMode(self._isExpandMode)
  end
...
function PaGlobal_WarInfomation_All:createInnerObject()
  self._innerObjectCount = 0
  self._innerObjectPool = {}
  for index = 0, ToClient_getSiegeObjectKindCount() - 1 do
    local characterWrapper = ToClient_getObjectStaticStatusByObjectKindbyIndex(index)
    if nil ~= characterWrapper and characterWrapper:isElfWarSpawnCharacter() == false then
      ...
      if nil ~= objectWrapper then
        ...
        self._innerObjectCount = self._innerObjectCount + 1
      end
    end
  end
...
function PaGlobal_WarInfomation_All:resize()
  ...
  if false == self._isExpandMode and false == self._isOpenFromWorldMap then
    Panel_House_InstallationMode_WarInfomation_All:SetSize(self._defualtPanelSizeX, titleBgSize + bottomBgSize)
  ...
  self._ui._stc_titleBg:SetSize(self._defaultTitleBgSizeX + self._addedNameSizeX, self._ui._stc_titleBg:GetSizeY())
  ...
  self._ui._btn_expand:ComputePos()
  ...
  if true == ToClient_IsRemoteInstallMode() then
    Panel_House_InstallationMode_WarInfomation_All:SetHorizonRight()
  end
end
...
function PaGlobal_WarInfomation_All:updateByBuildingInfo(buildingInfo)
  ...
  local templeteNameSizeX = self._ui._txt_templete[self._eCategory.name]:GetSizeX()
  local usedCount = 0
  for index = 0, self._innerObjectCount - 1 do
    local characterWrapper = ToClient_getObjectStaticStatusByObjectKindbyIndex(index)
    if nil ~= characterWrapper then
      ...
      if nil ~= objectWrapper then
        ...
        if maxNameLength < textSizeX and templeteNameSizeX < textSizeX then
          maxNameLength = textSizeX
        end
      end
    end
  end
  ...
  if 0 ~= maxNameLength then
    self._addedNameSizeX = maxNameLength - templeteNameSizeX
  end
end
function PaGlobal_WarInfomation_All:updateByHouseHoldActor()
  ...
  local maxNameLength = 0
  local templeteNameSizeX = self._ui._txt_templete[self._eCategory.name]:GetSizeX()
  local isRemoteMode = ToClient_IsRemoteInstallMode()
  local usedCount = 0
  for index = 0, self._innerObjectCount - 1 do
    local characterWrapper = ToClient_getObjectStaticStatusByObjectKindbyIndex(index)
    if nil ~= characterWrapper then
      ...
      if nil ~= objectWrapper then
        ...
        if true == isRemoteMode then
          local remoteBatchCount = ToClient_GetRemoteInstallCharacterCount(objectKind)
          objectCount = objectCount + remoteBatchCount
        end
        self._innerObjectPool[index].txt_value[self._eCategory.name]:SetText(characterWrapper:getName())
        self._innerObjectPool[index].txt_value[self._eCategory.count]:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_HOUSE_INSTALLATIONMODE_INNEROBJECTMAXCOUNT", "count", objectCount))
  ...
  if true == isRemoteMode then
    local remoteInsertedCount = ToClient_GetRemoteInsertedCount()
    allcount = allCount + remoteInsertedCount
  end
  self._ui._txt_objectCount:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_HOUSE_INSTALLATIONMODE_INNEROBJECTMAXCOUNT", "count", allCount))
...
function PaGlobal_WarInfomation_All:updateReposBuilding(buildingInfo)
  ...
  local posX = buildingInfo:GetPosX() + buildingInfo:GetSizeX()
...
function PaGlobal_WarInfomation_All:updateExpandMode(isExpandMode)
  if true == isExpandMode then
    self._ui._stc_categoryBg:SetShow(true)
    self._ui._stc_detailGroup:SetShow(true)
  else
    self._ui._stc_categoryBg:SetShow(false)
    self._ui._stc_detailGroup:SetShow(false)
  end
  self._isExpandMode = isExpandMode
  if false == self._isConsole then
    self._ui._btn_expand:SetShow(not self._isExpandMode)
    self._ui._btn_small:SetShow(self._isExpandMode)
  end
  PaGlobal_WarInfomation_All:resize()
end
