...
function PaGlobal_House_InstallationMode_ObjectControl_All:prepareOpen(installMode, posX, posY, isShow, isShowMove, isShowFix, isShowDelete, isShowCancel)
  ...
  PaGlobal_House_InstallationList_All_SetBlockFunctionBtn(true)
  if true == ToClient_VisitHouseIsMansion() and false == ToClient_VisitHouseIsMansionLand() then
    ...
    if nil ~= characterStaticWrapper then
      ...
      if nil ~= objectStaticWrapper then
        local installationType = characterStaticWrapper:getObjectStaticStatus():getInstallationType()
        if CppEnums.InstallationType.eType_MansionTree == installationType then
...
function PaGlobal_House_InstallationMode_ObjectControl_All:prepareClose()
  ...
  PaGlobal_House_InstallationList_All_SetBlockFunctionBtn(false)
  PaGlobal_House_InstallationMode_ObjectControl_All:hidePanel()
end
function PaGlobal_House_InstallationMode_ObjectControl_All:hidePanel()
...
function PaGlobal_House_InstallationMode_ObjectControl_All:initButtonSetting(installMode, isShow, isShowMove, isShowDelete, isShowCancel)
  ...
  if true == ToClient_IsRemoteInstallMode() and true == ToClient_isRemoteRemoveActor() then
    self._ui._btn_control[self._eButtonType.rotateRight]:SetEnable(false)
    self._ui._btn_control[self._eButtonType.rotateRight]:SetMonoTone(true)
    self._ui._btn_control[self._eButtonType.rotateRight]:SetIgnore(true)
    self._ui._btn_control[self._eButtonType.rotateLeft]:SetEnable(false)
    self._ui._btn_control[self._eButtonType.rotateLeft]:SetMonoTone(true)
    self._ui._btn_control[self._eButtonType.rotateLeft]:SetIgnore(true)
    self._ui._btn_control[self._eButtonType.move]:SetEnable(false)
    self._ui._btn_control[self._eButtonType.move]:SetMonoTone(true)
    self._ui._btn_control[self._eButtonType.move]:SetIgnore(true)
  else
    self._ui._btn_control[self._eButtonType.rotateRight]:SetEnable(true)
    self._ui._btn_control[self._eButtonType.rotateRight]:SetMonoTone(false)
    self._ui._btn_control[self._eButtonType.rotateRight]:SetIgnore(false)
    self._ui._btn_control[self._eButtonType.rotateLeft]:SetEnable(true)
    self._ui._btn_control[self._eButtonType.rotateLeft]:SetMonoTone(false)
    self._ui._btn_control[self._eButtonType.rotateLeft]:SetIgnore(false)
    self._ui._btn_control[self._eButtonType.move]:SetEnable(true)
    self._ui._btn_control[self._eButtonType.move]:SetMonoTone(false)
    self._ui._btn_control[self._eButtonType.move]:SetIgnore(false)
  end
...
function PaGlobal_House_InstallationMode_ObjectControl_All:buttonEventConfirm()
  ...
  elseif housing_isBuildMode() then
    ...
    if true == ToClient_IsRemoteInstallMode() then
      local remoteRegionKey = ToClient_GetRemoteRegionKey()
      ToClient_InsertRemoteInstall(remoteRegionKey:get())
...
function PaGlobal_House_InstallationMode_ObjectControl_All:buttonEventDelete()
  if true == ToClient_IsRemoteInstallMode() then
    ToClient_RemoveRemoteInstall()
  else
    local houseWrapper = housing_getHouseholdActor_CurrentPosition()
    local isSeedMode = false
    if nil ~= houseWrapper and false == houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isFixedHouse() and false == houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isInnRoom() and false == houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isMansionLand() then
      isSeedMode = true
    end
    if isSeedMode == true then
      local messageTitle = PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_NOTIFY")
      local messageContent = PAGetString(Defines.StringSheet_GAME, "LUA_HOUSE_INSTALLATIONMODE_TOOLTIP_SEEDDELETE")
