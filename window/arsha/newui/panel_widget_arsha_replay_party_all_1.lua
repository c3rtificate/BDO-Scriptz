...
function PaGlobal_Arsha_Replay_Party_All:open()
  ...
  if matchType ~= 0 or #self._teamMember > 2 then
...
function PaGlobal_Arsha_Replay_Party_All:createTeamUi()
  ...
  local leftTeamBottomPosY = 0
  local rightTeamBottomPosY = 0
  for teamNo, teamMemberList in pairs(self._teamMember) do
    ...
    for userIndex = 0, teamUserCount - 1 do
      if self._teamMemberUi[teamNo][userIndex] == nil then
        local temp = {}
        if false == isRight then
          ...
          temp._memberBg:SetPosY(self._ui._leftTeam.stc_memberTemplate:GetPosY() - self._replayMemberUiOffsetY + (temp._memberBg:GetSizeY() + gab) * userIndex)
          ...
          temp._memberBg:SetPosY(self._ui._rightTeam.stc_memberTemplate:GetPosY() - self._replayMemberUiOffsetY + (temp._memberBg:GetSizeY() + gab) * userIndex)
        ...
        temp._progress = UI.getChildControl(temp._memberBg, "Progress2_Big")
        temp._percent = UI.getChildControl(temp._memberBg, "StaticText_Percent")
        temp._stc_select = UI.getChildControl(temp._memberBg, "Static_Selected_BG")
        local baseProgress = UI.getChildControl(temp._memberBg, "Progress2_1")
        baseProgress:SetShow(false)
        temp._progress:SetShow(true)
        temp._isLeftTeam = not isRight
        temp._name:SetPosY(temp._name:GetPosY() - 10)
        temp._buffList = {}
      ...
      local tempPosY = Panel_Widget_Arsha_Party_All:GetPosY() + self._teamMemberUi[teamNo][userIndex]._memberBg:GetPosY() + self._teamMemberUi[teamNo][userIndex]._memberBg:GetSizeY() + 30
      if isRight == false then
        if leftTeamBottomPosY < tempPosY then
          leftTeamBottomPosY = tempPosY
        end
      elseif rightTeamBottomPosY < tempPosY then
        rightTeamBottomPosY = tempPosY
      end
    end
    if false == isRight then
  ...
  if nil ~= PaGlobal_Arsha_Hud_All then
    self._ui._leftTeam.txt_teamName:SetText(PaGlobal_Arsha_Hud_All._ui.txt_leftTeam:GetText())
    self._ui._rightTeam.txt_teamName:SetText(PaGlobal_Arsha_Hud_All._ui.txt_rightTeam:GetText())
  end
  self._ui._leftTeam.stc_teamNameBg:SetPosY(self._ui._leftTeam.stc_memberTemplate:GetPosY() - self._replayMemberUiOffsetY - gab - self._ui._leftTeam.stc_teamNameBg:GetSizeY())
  self._ui._rightTeam.stc_teamNameBg:SetPosY(self._ui._rightTeam.stc_memberTemplate:GetPosY() - self._replayMemberUiOffsetY - gab - self._ui._leftTeam.stc_teamNameBg:GetSizeY())
  ...
  ArshaSkillCooltime_SetPosY(leftTeamBottomPosY, rightTeamBottomPosY)
end
function PaGlobal_Arsha_Replay_Party_All:setting(teamNo)
  ...
  for index = 0, #self._teamMember[teamNo] do
    ...
    if nil ~= playerActorProxyWrapper and nil ~= userSlot then
      ...
      userSlot._memberBg:SetIgnore(false)
      userSlot._memberBg:addInputEvent("Mouse_LUp", "HandleEventLUp_Arsha_Replay_Party_All(" .. teamNo .. "," .. index .. "," .. tempActorKey .. ")")
...
function PaGlobal_Arsha_Replay_Party_All:clickPartyUI(teamNo, index, actorKeyRaw, isNotOff)
  if self._teamMemberUi[teamNo] ~= nil then
    for ii = 0, #self._teamMemberUi[teamNo] do
      if ii == index then
        if isNotOff == false and self._teamMemberUi[teamNo][ii]._stc_select:GetShow() == true then
          self._teamMemberUi[teamNo][ii]._stc_select:SetShow(false)
          ToClient_setReplayCompetitionShowSkillActorList(teamNo, 0)
        else
          self._teamMemberUi[teamNo][ii]._stc_select:SetShow(true)
          ToClient_setReplayCompetitionShowSkillActorList(teamNo, actorKeyRaw)
        end
      else
        self._teamMemberUi[teamNo][ii]._stc_select:SetShow(false)
      end
  ...
  for teamNo, teamMemberList in pairs(self._teamMember) do
    ...
    for userIndex = 0, teamUserCount - 1 do
      ...
      if nil ~= playerActorProxyWrapper and nil ~= userSlot then
        ...
        userSlot._progress:SetProgressRate(hpPercent)
        local hpString = string.format("%d/%d (%.2f", getHP, getMaxHP, hpPercent) .. "%)"
        userSlot._percent:SetText(hpString)
        if hpPercent <= 0 then
...
function PaGlobal_Arsha_Replay_Party_All:setMember(teamNo, actorKeyRaw)
  ...
  for ii = 0, #self._teamMember[teamNo] do
    if self._teamMember[teamNo][ii] == actorKeyRaw then
      return
    end
  end
...
function PaGlobal_Arsha_Replay_Party_All:updateScore(teamNo, score)
  ...
  if CppEnums.CompetitionMatchType.eCompetitionMatchMode_Personal ~= state_match then
    return
  end
...
function PaGlobal_Arsha_Replay_Party_All:attchToReplayCharacter(actorKeyRaw)
  local teamNoTemp = -1
  local indexTemp = -1
  for teamNo, teamMemberList in pairs(self._teamMember) do
    for userIndex = 1, #teamMemberList do
      if teamMemberList[userIndex] == actorKeyRaw then
        teamNoTemp = teamNo
        indexTemp = userIndex
      end
    end
  end
  if teamNoTemp == -1 or indexTemp == -1 then
    return
  end
  self:clickPartyUI(teamNoTemp, indexTemp - 1, actorKeyRaw, true)
end
function PaGlobal_Arsha_Replay_Party_All:registEventHandler()
  ...
  registerEvent("FromClient_ObserverAttachToReplayCharacter", "FromClient_Arsha_Replay_Party_All_AttachReplayCharacter")
end
...
function PaGlobal_Arsha_Replay_Party_All:changeTeamNameForReplay()
  if nil ~= PaGlobal_Arsha_Hud_All then
    self._ui._leftTeam.txt_teamName:SetText(PaGlobal_Arsha_Hud_All._ui.txt_leftTeam:GetText())
    self._ui._rightTeam.txt_teamName:SetText(PaGlobal_Arsha_Hud_All._ui.txt_rightTeam:GetText())
  end
