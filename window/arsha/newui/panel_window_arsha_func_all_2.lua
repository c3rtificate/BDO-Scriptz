...
function HandleEventOnOut_Arsha_Func_All_CommonTooltip(isShow, tipType)
  ...
  elseif 5 == tipType then
    ...
    local tempControl = PaGlobal_Arsha_Func_All._ui._attenderSet.btn_inviteList
    control = UI.getChildControl(tempControl, "Static_Icon")
...
function HandleEventLUp_Arsha_Func_All_TeamSubMenuShow(idx, controlType, userNo_s64, isAssistant)
  ...
  if nil ~= contents and shotBtnCount > 0 then
...
function FromClient_Arsha_Func_All_ChangeTeamName_ForReplay(readTeamName, BlueTeamName)
  PaGlobalFunc_Arsha_Hud_All_ChangeTeamNameForReplay(readTeamName, BlueTeamName)
  PaGlobalFunc_Arsha_Replay_Party_All_ChangeTeamNameForReplay()
end
...
function FromClient_InviteCompetitonAuthDenied()
  local messageBoxData = {
    title = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ALERT_NOTIFICATIONS"),
    content = PAGetString(Defines.StringSheet_GAME, "LUA_COMPETITION_REQ_FAIL"),
    priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW,
