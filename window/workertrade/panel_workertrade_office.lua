...
function WorkerTradeOffice_ExchangeMoneyToCompany()
  ...
  local function exchangeConfirm_TradeMoney(inputNumber)
    local function doExchange()
      ToClient_RequestExchangeWorkerTradeMoney(inputNumber, CppEnums.ItemWhereType.eMoneyInventory)
    end
...
function TradeLogListControlCreate(content, key)
  ...
  tradeLog:SetTextMode(__eTextMode_AutoWrap)
...
function workerTradeOffice:registerEvent()
  ...
  control._list2Log:registEvent(__ePAUIList2EventType_LuaChangeContent, "TradeLogListControlCreate")
  control._list2Log:createChildContent(__ePAUIList2ElementManagerType_List)
end
