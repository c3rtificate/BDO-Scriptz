function PaGlobal_CharacterTag_All:initialize()
  ...
  self._ui_console.stc_keyPrev = UI.getChildControl(self._ui_console.stc_keyGuideBg, "StaticText_Prev_Console")
  self._ui_console.stc_keyNext = UI.getChildControl(self._ui_console.stc_keyGuideBg, "StaticText_Next_Console")
  ...
  self._ui.btn_ItemCopy:ComputePos()
...
function PaGlobal_CharacterTag_All:setAlignKeyGuide()
  local keyGuideList = {
    self._ui_console.stc_keyPrev,
    self._ui_console.stc_keyNext,
    self._ui_console.stc_keyConfirm,
    self._ui_console.stc_keyCancle
  }
...
function PaGlobal_CharacterTag_All:loadMainFace()
  ...
  local characterCount = getCharacterDataCount(__ePlayerCreateNormal) - 1
  self._ui.txt_tagAreaValue:SetText(getRegionInfoByPosition(selfPos):getAreaName())
  self._ui.btn_charTAG:SetIgnore(false)
  self._ui.btn_charTAG:SetMonoTone(false)
  self._ui.btn_ItemCopy:SetIgnore(false)
  self._ui.btn_ItemCopy:SetMonoTone(false)
  if self.LOCAL_DEFINE.NODUEL == duelCharIndex then
    ...
    for idx = 0, characterCount do
      ...
      if ToClient_IsDuelCharacterByCharacterNo(characterData._characterNo_s64) == true then
        local duelChar_No_s64 = characterData._characterNo_s64
  ...
  if false == self._selfCharTag then
    ...
    self._ui.btn_charTAG:SetIgnore(true)
    self._ui.btn_charTAG:SetMonoTone(true)
    self._ui.btn_ItemCopy:SetIgnore(true)
    self._ui.btn_ItemCopy:SetMonoTone(true)
  end
end
function PaGlobal_CharacterTag_All:loadList()
  ...
  local duelCharIndex = PaGlobalFunc_CharacterTag_All_GetDuelCharcterIndex()
  local characterListMax = getCharacterDataCount(__ePlayerCreateNormal)
  local duelRegion_Key = ToClient_GetSelfPlayerTagRegionKey()
  for jj = 0, 5 do
  ...
  for idx = 0, 5 do
    ...
    if true == _ContentsGroup_UsePadSnapping then
      targetUI:registerPadEvent(__eConsoleUIPadEvent_Up_LB, "HandleEventScroll_CharacterTag_All_ScrollEvent(true)")
      targetUI:registerPadEvent(__eConsoleUIPadEvent_Up_RB, "HandleEventScroll_CharacterTag_All_ScrollEvent(false)")
    ...
    local workingText = PaGlobal_GameExit_All_GetWorkTypeString(characterData._pcWorkingType)
    ...
    elseif "" ~= workingText then
      ...
      self:setMonoToneIgnore(targetUI, targetImage, true)
    elseif ToClient_IsDuelCharacterByCharacterNo(characterData._characterNo_s64) == true then
      self:setMonoToneIgnore(targetUI, targetImage, true)
    ...
    if isSeasonCharacter == true then
      targetState:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_SEASONCONTENT_SEASONCHARACTER"))
      self:setMonoToneIgnore(targetUI, targetImage, true)
    end
...
function PaGlobal_CharacterTag_All:showManager(showModeNum, CharLV)
  ...
  if -1 == PaGlobalFunc_CharacterTag_All_GetDuelCharcterIndex() and true == self._currentTagState then
  ...
  if true == ToClient_doHaveDuelExp() then
    ...
    if true == _ContentsGroup_DuelCharacterCopyEquip then
      self._ui.btn_takeEXP:SetSpanSize(-140, self._ui.stc_titleBg:GetSizeY() + self._ui.stc_topBg:GetSizeY() - self._ui.btn_takeEXP:GetSizeY())
  ...
  if self.SHOW_TYPE.NORMAL == showModeNum or -1 == PaGlobalFunc_CharacterTag_All_GetDuelCharcterIndex() then
...
function PaGlobal_CharacterTag_All:faceSetting(targetImage, char_Type)
  if _ContentsGroup_isUsedNewCharacterInfo == false then
    ...
    targetImage:ChangeTextureInfoName(DefaultFace.path)
    local x1, y1, x2, y2 = setTextureUV_Func(targetImage, DefaultFace.x1, DefaultFace.y1, DefaultFace.x2, DefaultFace.y2)
    targetImage:getBaseTexture():setUV(x1, y1, x2, y2)
  end
end
function PaGlobal_CharacterTag_All:registEventHandler()
  ...
  registerEvent("FromClient_PrePareCopyMyEquipItem", "FromClient_PrePareCopyMyEquipItem")
...
function PaGlobal_CharacterTag_All:validate()
  ...
  self._ui_console.stc_keyPrev:isValidate()
  self._ui_console.stc_keyNext:isValidate()
end
