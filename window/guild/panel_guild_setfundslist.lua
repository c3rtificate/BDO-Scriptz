...
PaGlobal_Util_RegistHelpTooltipEvent(_btnQuestion, "\"PanelGuild\"")
if true == _ContentsOption_CH_Help then
  _btnQuestion:SetShow(false)
end
local Guild_Funds = {
...
function Guild_Funds:ResetControl()
  ...
  for index = 1, _memberCtrlCount do
    _guildList[index] = {}
    _guildList[index]._memberGrade = UI.createControl(__ePAUIControl_StaticText, _contentGuildList, "StaticText_Grade_" .. index - 1)
    _guildList[index]._memberName = UI.createControl(__ePAUIControl_StaticText, _contentGuildList, "StaticText_MemberName_" .. index - 1)
    _guildList[index]._memberContribution = UI.createControl(__ePAUIControl_StaticText, _contentGuildList, "StaticText_MemberContribution_" .. index - 1)
    _guildList[index]._memberFundsLimitEdit = UI.createControl(__ePAUIControl_Edit, _contentGuildList, "Edit_memberInputFunds_" .. index - 1)
    _guildList[index]._memberFundsIsUnlimited = UI.createControl(__ePAUIControl_CheckButton, _contentGuildList, "Check_memberFundsButton_" .. index - 1)
