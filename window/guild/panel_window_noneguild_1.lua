function PaGlobal_Guild_NoneGuild:init()
  ...
  self._ui._btn_createGuild = UI.getChildControl(leftArea, "Button_CreateGuild")
  self._ui._btn_search = UI.getChildControl(self._ui._txt_search, "Button_Search")
  self._ui._txt_KeyGuideX = UI.getChildControl(self._ui._txt_search, "StaticText_X_ConsoleUI")
  ...
  self._ui._txt_descTitle = UI.getChildControl(self._ui._stc_edge, "StaticText_DescTitle")
  ...
  self._ui._frame_desc = UI.getChildControl(self._ui._stc_edge, "Frame_Desc")
  ...
  self._ui._btn_guildRank = UI.getChildControl(rightArea, "Button_GuildRank")
  ...
  self._ui._btn_invite = UI.getChildControl(rightArea, "Button_Invite_Console")
  self._ui._txt_noSelectGuild = UI.getChildControl(rightArea, "StaticText_NoSelectGuild")
  self._ui._stc_horizonLine = UI.getChildControl(self._ui._stc_edge, "Static_HorizonLine")
  self._ui._stc_KeyGuideBG = UI.getChildControl(Panel_Guild_NoneGuild, "Static_KeyGuide_Console")
  self._ui._txt_KeyGuideA = UI.getChildControl(self._ui._stc_KeyGuideBG, "StaticText_Select_Console")
  self._ui._txt_KeyGuideB = UI.getChildControl(self._ui._stc_KeyGuideBG, "StaticText_Close_Console")
  local keyguides = {
  ...
  self._ui._txt_KeyGuideX:SetShow(_ContentsGroup_UsePadSnapping)
  self._ui._stc_KeyGuideBG:SetShow(_ContentsGroup_UsePadSnapping)
...
function PaGlobal_Guild_NoneGuild:SwitchPlatform()
  ...
  PaGlobal_Guild_NoneGuild:ConsoleSetting()
end
function PaGlobal_Guild_NoneGuild:ConsoleSetting()
  if _ContentsGroup_RenewUI == true then
    self._ui._btn_invite:SetShow(true)
    self._ui._btn_guildInfo:SetShow(false)
    self._ui._btn_guildRank:SetShow(false)
    self._ui._btn_applyJoin:SetSize(self._ui._stc_edge:GetSizeX(), self._ui._btn_applyJoin:GetSizeY())
  else
    self._ui._btn_invite:SetShow(false)
    self._ui._btn_guildRank:SetShow(true)
  end
end
function PaGlobal_Guild_NoneGuild:EventHandler()
  ...
  self._ui._btn_createGuild:addInputEvent("Mouse_LUp", "PaGlobalFunc_Guild_NoneGuild_CreateGuild()")
  self._ui._txt_search:setXboxVirtualKeyBoardEndEvent("PaGlobalFunc_Guild_NoneGuild_Search")
  self._ui._btn_invite:addInputEvent("Mouse_LUp", "FromClient_SimpleGuildInvite(" .. 0 .. ")")
  Panel_Guild_NoneGuild:registerPadEvent(__eConsoleUIPadEvent_Up_X, "PaGlobalFunc_Guild_NoneGuild_SetFocusEdit()")
  ...
  registerEvent("FromClient_CheckChinaProhibitedWord_SearchGuild", "FromClient_CheckChinaProhibitedWord_SearchGuild")
end
function PaGlobal_Guild_NoneGuild:open()
  ...
  PaGlobalFunc_Guild_NoneGuild_UpdateInviteCount(ToClient_getSimpleGuildInviteInfoSize())
end
function PaGlobal_Guild_NoneGuild:close()
  if nil == Panel_Guild_NoneGuild then
    return
  end
  if Panel_GuildWebInfo_All ~= nil and Panel_GuildWebInfo_All:GetShow() == true then
...
function PaGlobal_Guild_NoneGuild:changeTabForPad(moveDir)
  if Panel_Window_Guild_SimpleInvite:GetShow() == true then
    return
  end
...
function PaGlobal_Guild_NoneGuild:update()
  ...
  if guildListSize > 0 then
    local addGuildNoList = Array.new()
    for ii = 0, guildListSize - 1 do
      local wrapper
      if self._isSearchList == true then
        wrapper = ToClient_getSearchGuildPRListInfoWrapper(ii)
      else
        wrapper = ToClient_getGuildPRInfoWrapper(self._selectedIndex, ii)
      end
      if wrapper ~= nil then
        local guildNoRaw = wrapper:getGuildNo()
        local isAdded = false
        for key, value in pairs(addGuildNoList) do
          if value ~= nil and value == guildNoRaw then
            isAdded = true
            break
          end
        end
        if isAdded == false then
          ...
          addGuildNoList:push_back(guildNoRaw)
        end
      end
    end
  end
  ...
  if _ContentsGroup_RenewUI == true then
    self._ui._btn_applyJoinList:SetSize(225, 40)
    self._ui._btn_invite:SetShow(true)
  elseif 0 == guildListSize then
    self._ui._btn_applyJoinList:SetSize(460, 40)
  else
    self._ui._btn_applyJoinList:SetSize(225, 40)
  end
...
function PaGlobal_Guild_NoneGuild:SetRightAreaInfo(index)
  ...
  local stc_c_markbg = UI.getChildControl(stc_guildMark, "Static_GuildMarkBg_Console")
  local stc_c_mark = UI.getChildControl(stc_guildMark, "Static_GuildMark_Console")
  ...
  if false == ToClient_isConsole() and false == _ContentsOption_CH_NoCustomGuildMark then
    ...
    stc_mark:SetShow(true)
    stc_c_markbg:SetShow(false)
    stc_c_mark:SetShow(false)
  else
    if nil ~= PaGlobalFunc_ConsoleGuildMarkSet then
      PaGlobalFunc_ConsoleGuildMarkSet(wrapper:getGuildNo(), stc_c_mark, stc_c_markbg)
    end
    stc_c_markbg:SetShow(true)
    stc_c_mark:SetShow(true)
    stc_mark:SetShow(false)
  end
  txt_guildName:SetText(tostring(wrapper:getGuildName()))
  txt_guildMaster:SetText(tostring(wrapper:getGuildMasterUserNickName()))
  local introduceText = ToClient_GetGuildIntroductionByGuildNo(wrapper:getGuildNo())
  if 0 ~= string.len(introduceText) then
    PaGlobal_Guild_NoneGuild._ui._txt_desc:SetText(introduceText)
  else
  ...
  self._ui._btn_guildRank:addInputEvent("Mouse_LUp", "GuildRank_Web_Show()")
