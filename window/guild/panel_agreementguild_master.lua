...
function HandleClicked_AgreementGuild_Master_SetData(index)
  ...
  if _isJoin then
    ...
    self.maxBenefitValue = Int64toInt32(ToClient_GuildContractMaxBenefit(_memberIndex, periodValue[index], paymentPerDay[index]))
    self.maxpenaltyCostValue = Int64toInt32(ToClient_GuildContractMaxPenalty(_memberIndex, periodValue[index], cancellationCharge[index]))
...
function AgreementGuild_Master:Initialize()
  ...
  AgreementGuild_Master.btn_PayMax = UI.getChildControl(Panel_AgreementGuild_Master, "Button_PayMax")
  AgreementGuild_Master.btn_PayMin = UI.getChildControl(Panel_AgreementGuild_Master, "Button_PayMin")
  ...
  AgreementGuild_Master._frame_Summary:SetTextMode(__eTextMode_AutoWrap)
  ...
  for index = 0, 6 do
    ...
    AgreementGuild_Master._radioBtn_Period[index] = UI.createControl(__ePAUIControl_RadioButton, Panel_AgreementGuild_Master, "RadioButton_Period_" .. index)
...
function AgreementGuild_Master:SetShowContractPreSet(isShow)
  ...
  self.btn_PayMax:SetShow(not isShow)
  self.btn_PayMin:SetShow(not isShow)
  self.selectEditIndex = -1
end
function AgreementGuild_Master:SetHideContractControl()
  ...
  self.btn_PayMax:SetShow(false)
  self.btn_PayMin:SetShow(false)
  self.selectEditIndex = -1
end
...
function FGlobal_AgreementGuild_Master_Open(memberIndex, requesterMemberGrade, usableActivity)
  ...
  if _ContentsGroup_GuildAuthoritySeparation == true then
    isRenew = PaGlobalFunc_Guild_Authorization_RightCheck(__eGuildRightNew_Join)
  else
    isRenew = PaGlobalFunc_AgreementGuildMaster_RightCheck(__eGuildRightType_Contract, memberIndex, requesterMemberGrade)
  end
...
function PaGlobalFunc_AgreementGuildMaster_RightCheck(eRightType, memberIdx, requesterMemberGrade)
  local selfPlayer = getSelfPlayer()
  if nil == selfPlayer then
    return false
  end
  local memberInfo = ToClient_GetMyGuildInfoWrapper():getMember(memberIdx)
  if nil == memberInfo then
    _PA_ASSERT(false, "FGlobal_AgreementGuild_Master_Open \236\157\152 \235\169\164\235\178\132\236\157\184\235\141\177\236\138\164\234\176\128 \235\185\132\236\160\149\236\131\129\236\158\133\235\139\136\235\139\164 " .. tostring(memberIdx))
    return false
  end
  local function OriginalCheck()
    ...
    elseif 1 == requesterMemberGrade then
      ...
      bCheck = (2 == memberInfo:getGrade() or 5 == memberInfo:getGrade()) and true
    else
      bCheck = false
    end
    return bCheck
  ...
  local isRightContract = ToClient_IsCheckGuildRightType(eRightType)
  if false == isRightContract then
    return false
  end
  local memberGrade = memberInfo:getGrade()
  local myGradePosition = ToClient_GetGuildMemberPosition(requesterMemberGrade)
  local memberGradePosition = ToClient_GetGuildMemberPosition(memberGrade)
  if -1 == myGradePosition or -1 == memberGradePosition then
    return false
  end
  if __eGuildMemberGradeMaster == requesterMemberGrade then
    return true
  end
  if memberInfo:isSelf() == true and (__eGuildMemberGradeAdviser == requesterMemberGrade or __eGuildMemberGradeScribe == requesterMemberGrade or __eGuildMemberGradeChiefOfStaff == requesterMemberGrade) then
    return true
  end
...
function HandleClicked_AgreementGuild_Master_Renew()
  ...
  local checkedIndex = _AgreementGuild_Master_ReturnCheckedNum()
  self.maxBenefitValue = Int64toInt32(ToClient_GuildContractMaxBenefit(_memberIndex, periodValue[checkedIndex], paymentPerDay[checkedIndex]))
  self.maxpenaltyCostValue = Int64toInt32(ToClient_GuildContractMaxPenalty(_memberIndex, periodValue[checkedIndex], cancellationCharge[checkedIndex]))
  ...
  self.btn_PayMax:SetPosX(self.dailyPayment_edit:GetPosX() + self.dailyPayment_edit:GetSizeX() + 5)
  self.btn_PayMin:SetPosX(self.penaltyCost_edit:GetPosX() + self.penaltyCost_edit:GetSizeX() + 5)
  ...
  self.btn_Refuse:SetShow(true)
  self.btn_Close:SetShow(false)
  self.btn_PayMax:SetShow(true)
  self.btn_PayMin:SetShow(true)
  self.selectEditIndex = -1
  HandleClicked_AgreementGuild_Master_SetData(4)
end
function AgreementGuild_Master:registEventHandler()
  ...
  self.btn_PayMax:addInputEvent("Mouse_LUp", "HandleClicked_AgreementGuild_SetMaxCost()")
  self.btn_PayMin:addInputEvent("Mouse_LUp", "HandleClicked_AgreementGuild_SetMinCost()")
end
...
function AgreementGuild_SetMaxDailyPayment(checkIndex, benefitMax)
  ...
  local self = AgreementGuild_Master
  self.maxDailyPayment:SetText(PAGetStringParam2(Defines.StringSheet_GAME, "LUA_AGREEMENTGUILD_MASTER_MAXDAILYPAYMENT", "checkedIndex", makeDotMoney(toInt64(0, checkIndex)), "maxBenefitValue", makeDotMoney(benefitMax)))
end
function AgreementGuild_SetMaxPenalty(checkIndex, penaltyMax)
  ...
  local self = AgreementGuild_Master
  self.maxpenaltyCost:SetText(PAGetStringParam2(Defines.StringSheet_GAME, "LUA_AGREEMENTGUILD_MASTER_MAXDAILYPAYMENT", "checkedIndex", makeDotMoney(toInt64(0, checkIndex)), "maxBenefitValue", makeDotMoney(penaltyMax)))
end
function HandleClicked_AgreementGuild_SetMaxCost()
  local self = AgreementGuild_Master
  if -1 == self.selectEditIndex then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_AGREEMENTGUILD_MASTER_COSTBUTTON"))
    return
  end
  local maxBenefit = math.min(tonumber(self.maxBenefitValue), tonumber(CppEnums.GuildBenefit.eMaxContractedBenefit))
  local maxPenalty = math.min(tonumber(self.maxpenaltyCostValue), tonumber(CppEnums.GuildBenefit.eMaxContractedPenalty))
  if 0 == self.selectEditIndex then
    self.dailyPayment_edit:SetEditText(math.floor(maxBenefit))
  else
    self.penaltyCost_edit:SetEditText(math.floor(maxPenalty))
  end
end
function HandleClicked_AgreementGuild_SetMinCost()
  local self = AgreementGuild_Master
  if -1 == self.selectEditIndex then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_AGREEMENTGUILD_MASTER_COSTBUTTON"))
    return
  end
  local value_ContractPeriod = 0
  local value_DailyPayment = 0
  local value_PenaltyCost = 0
  for i = 0, #periodValue do
    if self._radioBtn_Period[i]:IsCheck() then
      value_ContractPeriod = periodValue[i]
      value_DailyPayment = paymentPerDay[i]
  ...
  local checkedIndex = _AgreementGuild_Master_ReturnCheckedNum()
  if 0 == self.selectEditIndex then
    self.dailyPayment_edit:SetEditText(paymentPerDay[checkedIndex])
  else
    self.penaltyCost_edit:SetEditText(cancellationCharge[checkedIndex])
  end
end
