...
function PaGlobal_PersonalBattle:update()
  ...
  local currentNation = ToClient_getEnteredNationString()
  ...
  for idx = 0, serverCount - 1 do
    if isUseTotalAccountVer2() == true and ToClient_SelfPlayerIsGM() == false then
      local serverData = getGameChannelServerDataByWorldNo(curChannelData._worldNo, idx)
      local getAccessNationServerString = serverData:getAccessibleNation()
      if getAccessNationServerString == "" or getAccessNationServerString == currentNation then
...
function PaGlobal_PersonalBattle:join(idx)
  ...
  local function joinPersonalBattle()
    ...
    while appliedBuff ~= nil do
      ...
      for idx = 1, #joinableDebuffTable do
        local joinableDebeffKey = joinableDebuffTable[idx]
      ...
      appliedBuff = playerWrapper:getAppliedBuffNext(true)
    end
