...
function PaGlobal_GuildMemberList_All:ControlAll_Init()
  ...
  self._ui.btn_guildFund = UI.getChildControl(self._ui.stc_guildFundBg, "Button_ActivityFundsOption")
  ...
  self._ui.btn_guildPoint = UI.getChildControl(self._ui.stc_bottomAreaBg, "Button_GuildPoint")
...
function PaGlobal_GuildMemberList_All:ControlPc_Init()
  ...
  self._ui_pc.stc_funcGradeBg = UI.getChildControl(Panel_GuildMemeberList_All, "Static_FunctionGradeBG")
  self._ui_pc.btn_func = UI.getChildControl(Panel_GuildMemeberList_All, "Button_Function_Temp")
  self._ui_pc.stc_line = UI.getChildControl(self._ui_pc.btn_func, "Static_HorizonLine")
...
function PaGlobal_GuildMemberList_All:ControlSetShow()
  ...
  if true == self._isConsole then
    ...
    self._ui.btn_guildPoint:SetShow(false)
    self._ui.btn_guildFund:SetShow(false)
  else
    ...
    self._ui.btn_warReward:SetShow(_ContentsGroup_Siege2024 == false)
    self._ui.btn_guildFundManage:SetShow(true)
    self._ui.btn_guildPoint:SetShow(true)
    self._ui.btn_guildFund:SetShow(true)
  end
  if false == _ContentsGroup_Siege then
    self._ui.btn_warReward:SetShow(false)
  end
...
function PaGlobal_GuildMemberList_All:CreateControl()
  ...
  local function createListInfo(pIndex)
    ...
    tempMember._documentBtn = UI.createAndCopyBasePropertyControl(self._ui.stc_listMember, "Button_Documents", tempMember._memberBg, "guildMemeberlist_Document_" .. pIndex)
    tempMember._approvalBtn = UI.createAndCopyBasePropertyControl(self._ui.stc_listMember, "Button_Approval", tempMember._memberBg, "guildMemeberlist_Approval_" .. pIndex)
    ...
    tempMember._approvalBtn:SetShow(false)
  ...
  local function createMenuButton(pIndex)
    if _ContentsGroup_GuildAuthoritySeparation == true then
      if nil == self._MenuButtonStrNew[pIndex] then
        return
      end
    elseif nil == self._MenuButtonStr[pIndex] then
      return
    end
    local tempMenuButton = {}
    tempMenuButton._btn = UI.createAndCopyBasePropertyControl(Panel_GuildMemeberList_All, "Button_Function_Temp", self._ui_pc.stc_funcBg, "guildMemeberList_MenuButton_" .. pIndex)
    tempMenuButton._line = UI.createAndCopyBasePropertyControl(self._ui_pc.btn_func, "Static_HorizonLine", tempMenuButton._btn, "guildMemeberList_MenuLine_" .. pIndex)
    tempMenuButton._btn:SetPosX(5)
    tempMenuButton._btn:SetShow(false)
    if true == isGameTypeTH() or isGameTypeASIA() or true == isGameTypeID() then
      tempMenuButton._btn:SetSize(self._menuButtonSize.x + 10, self._menuButtonSize.y)
    else
      tempMenuButton._btn:SetSize(self._menuButtonSize.x, self._menuButtonSize.y)
    end
    if _ContentsGroup_GuildAuthoritySeparation == true then
      tempMenuButton._btn:SetText(self._MenuButtonStrNew[pIndex])
      tempMenuButton._btn:addInputEvent("Mouse_LUp", "HandleEventLUp_GuildMemberList_All_SelectMenuButtonNew(" .. pIndex .. ")")
    else
      tempMenuButton._btn:SetText(self._MenuButtonStr[pIndex])
      tempMenuButton._btn:addInputEvent("Mouse_LUp", "HandleEventLUp_GuildMemberList_All_SelectMenuButton(" .. pIndex .. ")")
    end
    return tempMenuButton
  end
  local function createGradeButton(index)
    if self._GradeButtonStr[index] == nil then
      return
    end
    local tempButton = {}
    tempButton._btn = UI.createAndCopyBasePropertyControl(Panel_GuildMemeberList_All, "Button_Function_Temp", self._ui_pc.stc_funcGradeBg, "guildMemeberList_GradeButton_" .. index)
    tempButton._line = UI.createAndCopyBasePropertyControl(self._ui_pc.btn_func, "Static_HorizonLine", tempButton._btn, "guildMemeberList_MenuLine_" .. index)
    tempButton._btn:SetPosX(5)
    tempButton._btn:SetShow(false)
    tempButton._line:SetShow(false)
    if true == isGameTypeTH() or isGameTypeASIA() or true == isGameTypeID() then
      tempButton._btn:SetSize(self._menuButtonSize.x + 10, self._menuButtonSize.y)
    else
      tempButton._btn:SetSize(self._menuButtonSize.x, self._menuButtonSize.y)
    end
    tempButton._btn:addInputEvent("Mouse_LUp", "HandleEventLUp_GuildMemberList_All_SelectGradeButton(" .. index .. ")")
    tempButton._btn:SetText(self._GradeButtonStr[index])
    return tempButton
  end
  for index = 0, self._config.listMaxCount do
    self._ui._memberlist[index] = createListInfo(index)
  end
  local menuBtnTextSizeX = UI.getChildControl(Panel_GuildMemeberList_All, "Button_Function_Temp"):GetSizeX()
  if _ContentsGroup_GuildAuthoritySeparation == true then
    for index = 0, self._MenuButtonTypeNew.COUNT do
      self._ui._buttonlist[index] = createMenuButton(index)
      if nil ~= self._ui._buttonlist[index] then
        menuBtnTextSizeX = math.max(menuBtnTextSizeX, self._ui._buttonlist[index]._btn:GetTextSizeX() + 10)
      end
    end
    for index = 0, self._MenuButtonTypeNew.COUNT do
      if nil ~= self._ui._buttonlist[index] then
        self._ui._buttonlist[index]._btn:SetSize(menuBtnTextSizeX, self._ui._buttonlist[index]._btn:GetSizeY())
      end
    end
    for index = 0, self._GradeButtonType.COUNT - 1 do
      self._ui._gradelist[index] = createGradeButton(index)
      if self._ui._gradelist[index] ~= nil then
        menuBtnTextSizeX = math.max(menuBtnTextSizeX, self._ui._gradelist[index]._btn:GetTextSizeX() + 10)
      end
    end
    for index = 0, self._GradeButtonType.COUNT - 1 do
      if nil ~= self._ui._gradelist[index] then
        self._ui._gradelist[index]._btn:SetSize(menuBtnTextSizeX, self._ui._gradelist[index]._btn:GetSizeY())
      end
    end
  else
...
function PaGlobal_GuildMemberList_All:prepareOpen()
  ...
  self._ui_pc.stc_line:SetShow(false)
...
function PaGlobal_GuildMemberList_All:registEventHandler()
  ...
  self._ui.btn_guildFundManage:addInputEvent("Mouse_LUp", "PaGlobalFunc_GuildIncentiveOption_All_Open()")
  self._ui.btn_guildPoint:addInputEvent("Mouse_LUp", "PaGlobal_GuildPointMenu_All_Open()")
  ...
  self._ui.btn_guildFund:addInputEvent("Mouse_LUp", "PaGlobal_GuildActivityFunds_List_All_Open()")
  registerEvent("FromClient_ChangeUserNicknameDirectly", "FromClient_ChangeUserNicknameDirectly_GuildMemberList")
end
...
function PaGlobal_GuildMemberList_All:resize()
  ...
  if true == self._contentOpen.voice then
    ...
    self._ui._titlelist[self._SortType.NAME]:SetSpanSize(300, 0)
    self._ui._titlelist[self._SortType.NAME]:ComputePos()
    self._ui._titlelist[self._SortType.APOINT]:SetSpanSize(630, 0)
    self._ui._titlelist[self._SortType.APOINT]:ComputePos()
    self._ui._titlelist[self._SortType.WPOINT]:SetSpanSize(270, 0)
    self._ui._titlelist[self._SortType.WPOINT]:ComputePos()
    self._ui._titlelist[self._SortType.WGRADE]:SetSpanSize(150, 0)
    self._ui._titlelist[self._SortType.WGRADE]:ComputePos()
    self._ui._titlelist[self._SortType.CONTACT]:SetSpanSize(30, 0)
    self._ui._titlelist[self._SortType.CONTACT]:ComputePos()
    for index = 0, self._config.listMaxCount - 1 do
      local memberlistControl = self._ui._memberlist[index]
      memberlistControl._charName:SetSpanSize(280, 0)
      memberlistControl._activity:SetSpanSize(600, 0)
      memberlistControl._contributedTendency:SetSpanSize(740, 0)
      memberlistControl._warState:SetSpanSize(180, 0)
      memberlistControl._warGrade:SetSpanSize(180, 0)
      memberlistControl._contractBtn:SetSpanSize(65, 0)
      memberlistControl._documentBtn:SetSpanSize(40, 0)
      memberlistControl._approvalBtn:SetSpanSize(15, 0)
    end
  end
  local gabX = 10
  for index = 0, self._config.listMaxCount - 1 do
...
function PaGlobal_GuildMemberList_All:setDocumentButton(index, guildMember, documentBtn, approvalBtn, dataIdx)
  if nil == PaGlobalFunc_GuildMain_All_GetBgShow then
    return
  end
  if false == PaGlobalFunc_GuildMain_All_GetBgShow(PaGlobal_GuildMain_All._eTABINDEX._MEMBERLIST) then
  ...
  documentBtn:addInputEvent("Mouse_On", "HandleEventOnOut_GuildMemeberList_All_PointDetailTooltip(true, " .. dataIdx .. "," .. index .. ")")
  documentBtn:addInputEvent("Mouse_Out", "HandleEventOnOut_GuildMemeberList_All_PointDetailTooltip(false)")
  documentBtn:addInputEvent("Mouse_LUp", "HandleEventLUp_GuildMemberList_All_PointDetailClick( " .. dataIdx .. " )")
  approvalBtn:addInputEvent("Mouse_On", "HandleEventOnOut_GuildMemeberList_All_ApprovalTooltip( true, " .. dataIdx .. "," .. index .. ")")
  approvalBtn:addInputEvent("Mouse_Out", "HandleEventOnOut_GuildMemeberList_All_ApprovalTooltip(false)")
  approvalBtn:addInputEvent("Mouse_LUp", "HandleEventLUp_GuildMemberList_All_ApprovalClick( " .. dataIdx .. " )")
  if guildMember:isSelf() == false then
    approvalBtn:SetIgnore(true)
    approvalBtn:SetMonoTone(true)
  else
    approvalBtn:SetIgnore(false)
    approvalBtn:SetMonoTone(false)
  end
end
...
function PaGlobal_GuildMemberList_All:updateBottomInfo()
  ...
  if true == PaGlobalFunc_GuildMain_All_CheckIsMecernary() then
    ...
    self._ui.btn_warReward:SetShow(false == self._isConsole and true == _ContentsGroup_Siege and _ContentsGroup_Siege2024 == false)
  end
  if self._isConsole == false then
    if _ContentsGroup_GuildAuthoritySeparation == true then
      rightGrade = PaGlobalFunc_Guild_Authorization_RightCheck(__eGuildRightNew_SetMemberLimitPrice)
      self._ui.btn_guildFund:SetShow(rightGrade)
      rightGrade = PaGlobalFunc_Guild_Authorization_RightCheck(__eGuildRightNew_PayIncentive)
      self._ui.btn_guildFundManage:SetShow(rightGrade)
      rightGrade = PaGlobalFunc_Guild_Authorization_RightCheck(__eGuildRightNew_PointSetting)
      self._ui.btn_guildPoint:SetShow(rightGrade)
    else
      rightGrade = ToClient_IsCheckGuildRightType(__eGuildRightType_SetMemberLimitPrice)
      self._ui.btn_guildFund:SetShow(rightGrade)
      rightGrade = ToClient_IsCheckGuildRightType(__eGuildRightType_PayIncentive)
      self._ui.btn_guildFundManage:SetShow(rightGrade)
      rightGrade = ToClient_IsCheckGuildRightType(__eGuildRightType_PointSetting)
      self._ui.btn_guildPoint:SetShow(rightGrade)
    end
  end
end
...
function PaGlobal_GuildMemberList_All:updateListMemeber(guildMember, index, dataIdx)
  ...
  self:setDocumentButton(index, guildMember, guildMemberControl._documentBtn, guildMemberControl._approvalBtn, dataIdx)
  ...
  if true == PaGlobalFunc_GuildMain_All_CheckIsMecernary() then
    ...
    guildMemberControl._documentBtn:SetMonoTone(true)
    guildMemberControl._documentBtn:addInputEvent("Mouse_On", "")
    guildMemberControl._documentBtn:addInputEvent("Mouse_Out", "")
    guildMemberControl._documentBtn:addInputEvent("Mouse_LUp", "")
    guildMemberControl._approvalBtn:SetMonoTone(true)
    guildMemberControl._approvalBtn:addInputEvent("Mouse_On", "")
    guildMemberControl._approvalBtn:addInputEvent("Mouse_Out", "")
    guildMemberControl._approvalBtn:addInputEvent("Mouse_LUp", "")
  end
  ...
  if true == isSelf or userNo == myUserNo then
    ...
    self._ui.btn_warReward:SetMonoTone(not isTakableSiegeReward)
    self._ui.btn_warReward:SetEnable(isTakableSiegeReward)
    guildMemberControl._approvalBtn:SetShow(true)
  else
    guildMemberControl._approvalBtn:SetShow(false)
  end
  if true == self._isConsole then
    ...
    guildMemberControl._memberBg:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandlePadEvent_GuildMemberlist_All_FullNameTooltip(" .. tostring(index) .. ")")
...
function GuildMemberCountSet()
  local self = PaGlobal_GuildMemberList_All
  local myGuildInfo = ToClient_GetMyGuildInfoWrapper()
  if nil == myGuildInfo then
    return
  end
  local _isOnlineMember = 0
  local _joinMemCount = 0
  local memberCount = myGuildInfo:getMemberCount()
  for index = 1, memberCount do
    local tempGuildList = {}
    local myGuildMemberInfo = myGuildInfo:getMember(index - 1)
    if nil == myGuildMemberInfo then
      return
    end
    if true == myGuildMemberInfo:isOnline() and false == myGuildMemberInfo:isGhostMode() then
      _isOnlineMember = _isOnlineMember + 1
    end
    if true == myGuildMemberInfo:isSiegeParticipant() then
      _joinMemCount = _joinMemCount + 1
    else
    end
  end
  if true == PaGlobal_GuildMemberList_All._isConsole then
    local textSizeX = self._ui.txt_guildFund:GetTextSizeX()
    if textSizeX > 260 then
      self._ui.stc_guildFundBg:SetSize(textSizeX, 40)
      self._ui.txt_guildFund:SetSize(textSizeX, 20)
    else
      self._ui.stc_guildFundBg:SetSize(260, 40)
      self._ui.txt_guildFund:SetSize(260, 20)
    end
  else
    self._ui.txt_guildFund:SetTextMode(__eTextMode_LimitText)
    self._ui.stc_guildFundBg:SetSize(260, 40)
    self._ui.txt_guildFund:SetSize(260, 20)
  end
  local isAlliance = false
  local allianceJoinMemberCount = 0
  local allianceMemberCount = 0
  local myAllianceWrapper = ToClient_GetMyGuildAllianceWrapper()
  if myAllianceWrapper ~= nil then
    isAlliance = true
    ...
    for index = 0, guildCount - 1 do
      ...
      if guildWrapper ~= nil then
        ...
        allianceMemberCount = allianceMemberCount + guildMembercount
        for idx = 0, guildMembercount - 1 do
          local memberWrapper = guildWrapper:getMember(Int64toInt32(idx))
          if memberWrapper ~= nil and memberWrapper:isSiegeParticipant() == true then
            allianceJoinMemberCount = allianceJoinMemberCount + 1
          end
        end
      end
    end
  end
  if _joinMemCount > 0 or allianceJoinMemberCount > 0 then
    if _ContentsGroup_ElfWar == true and ToClient_IsElfWarServer() == true and ToClient_GetElfWarState() ~= __eElfWar_GuildApplying and ToClient_GetElfWarState() ~= __eElfWar_Stop and ToClient_GetElfWarState() ~= __eElfWar_Count then
      if isAlliance == true then
        self._ui.txt_guildFund:SetText(PAGetStringParam6(Defines.StringSheet_GAME, "LUA_GUILDMEMBERCOUNT_ELFWAR_1", "onlineMember", _isOnlineMember, "memberCount", memberCount, "joinMemCount", _joinMemCount, "memberCount2", memberCount, "onlineMember1", allianceJoinMemberCount, "memberCount1", allianceMemberCount))
      else
        self._ui.txt_guildFund:SetText(PAGetStringParam4(Defines.StringSheet_GAME, "LUA_GUILDMEMBERCOUNT_ELFWAR", "onlineMember", _isOnlineMember, "memberCount", memberCount, "joinMemCount", _joinMemCount, "memberCount2", memberCount))
      end
    elseif isAlliance == true then
      self._ui.txt_guildFund:SetText(PAGetStringParam6(Defines.StringSheet_GAME, "LUA_GUILDMEMBERCOUNT_JOIN_1", "onlineMember", _isOnlineMember, "memberCount", memberCount, "joinMemCount", _joinMemCount, "memberCount2", memberCount, "onlineMember1", allianceJoinMemberCount, "memberCount1", allianceMemberCount))
    else
      self._ui.txt_guildFund:SetText(PAGetStringParam4(Defines.StringSheet_GAME, "LUA_GUILDMEMBERCOUNT_JOIN", "onlineMember", _isOnlineMember, "memberCount", memberCount, "joinMemCount", _joinMemCount, "memberCount2", memberCount))
    end
  else
    self._ui.txt_guildFund:SetText(PAGetStringParam2(Defines.StringSheet_GAME, "LUA_GUILDMEMBERCOUNT_NORMAL", "onlineMember", _isOnlineMember, "memberCount", memberCount))
  end
  self._ui.stc_guildFundBg:ComputePos()
  self._ui.txt_guildFund:SetIgnore(true)
  if true == self._ui.txt_guildFund:IsLimitText() then
    self._ui.txt_guildFund:SetIgnore(false)
    self._ui.txt_guildFund:addInputEvent("Mouse_On", "HandleEventOnOut_GuildMemberList_All_Showtooltip(true)")
    self._ui.txt_guildFund:addInputEvent("Mouse_Out", "HandleEventOnOut_GuildMemberList_All_Showtooltip()")
  else
    self._ui.txt_guildFund:SetIgnore(true)
    self._ui.txt_guildFund:addInputEvent("Mouse_On", "")
    self._ui.txt_guildFund:addInputEvent("Mouse_Out", "")
  end
end
function HandleEventOnOut_GuildMemberList_All_Showtooltip(isShow)
  local self = PaGlobal_GuildMemberList_All
  ...
  local name = ""
  local desc = self._ui.txt_guildFund:GetText()
  TooltipSimple_Show(self._ui.txt_guildFund, name, desc)
end
