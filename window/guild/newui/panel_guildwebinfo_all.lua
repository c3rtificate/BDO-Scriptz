...
function PaGlobalFunc_GuildWebInfo_All_GuildWebInfoOpen(listIdx)
  ...
  local isRightWebInfoEdit = false
  if selfPlayerGuildNo_64 == guildNo_str then
    if _ContentsGroup_GuildAuthoritySeparation == true then
      isRightWebInfoEdit = PaGlobalFunc_Guild_Authorization_RightCheck(__eGuildRightNew_Notice)
    else
      isRightWebInfoEdit = PaGlobalFunc_GuildWebInfo_All_RightCheck(__eGuildRightType_Notice)
    end
  end
  ...
  url = url .. "&isMaster=" .. tostring(isRightWebInfoEdit)
...
function PaGlobalFunc_GuildWebInfo_All_RightCheck(eRightType)
  local selfPlayer = getSelfPlayer()
  if nil == selfPlayer then
    return false
  end
  local function OriginalCheck()
    ...
    if __eGuildRightType_Notice == eRightType then
      return isMasters
    end
    return false
  end
