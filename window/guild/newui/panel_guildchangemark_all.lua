...
function PaGlobal_GuildChangeMark_All:initialize()
  ...
  local titleArea = UI.getChildControl(Panel_GuildChangeMark_All, "Static_TitleArea")
  ...
  self._ui.btn_Confirm = UI.getChildControl(Panel_GuildChangeMark_All, "Button_1_Import")
  ...
  self._ui.btn_Confirm:SetShow(not _ContentsGroup_UsePadSnapping)
  if true == _ContentsGroup_UsePadSnapping then
    Panel_GuildChangeMark_All:SetSize(Panel_GuildChangeMark_All:GetSizeX(), self._consolePanelSize)
...
function PaGlobal_GuildChangeMark_All:registerEvent()
  ...
  self._ui.btn_Close:addInputEvent("Mouse_LUp", "PaGlobalFunc_GuildChangeMark_All_Close()")
  self._ui.btn_Confirm:addInputEvent("Mouse_LUp", "HandleEventPadPress_GuildChangeMark_All_MarkConfirm()")
  for idx = 1, self._selectableCount do
    if true == _ContentsGroup_UsePadSnapping then
      ...
      self._ui.stc_BgTable[idx]:addInputEvent("Mouse_LUp", "HandleEventPadPress_GuildChangeMark_All_SelectBg( " .. idx .. " )")
      self._ui.stc_IconTable[idx]:addInputEvent("Mouse_LUp", "HandleEventPadPress_GuildChangeMark_All_SelectIcon( " .. idx .. " )")
    end
  end
end
...
function PaGlobalFunc_GuildMark_SetGuildMarkControl(bgControl, iconControl, actorKey)
  ...
  if nil ~= bgControl and bgIdx >= 0 and bgIdx <= PaGlobal_GuildChangeMark_All._selectableCount then
  ...
  if nil ~= iconControl and iconIdx >= 0 and iconIdx <= PaGlobal_GuildChangeMark_All._selectableCount then
...
function HandleEventPadPress_GuildChangeMark_All_SelectBg(idx)
  if nil == PaGlobal_GuildChangeMark_All._ui.stc_BgTable[idx] or nil == idx then
    if true == _ContentsOption_CH_NoCustomGuildMark then
      idx = 0
      PaGlobal_GuildChangeMark_All._ui.stc_CurBgEffect:SetShow(false)
      PaGlobal_GuildChangeMark_All._ui.stc_CurrentBg:SetShow(false)
    end
    return
  end
  PaGlobal_GuildChangeMark_All._ui.stc_CurrentBg:SetShow(true)
...
function HandleEventPadPress_GuildChangeMark_All_SelectIcon(idx)
  if nil == idx or nil == PaGlobal_GuildChangeMark_All._ui.stc_IconTable[idx] then
    if true == _ContentsOption_CH_NoCustomGuildMark then
      ...
      PaGlobal_GuildChangeMark_All._ui.stc_CurrentMark:SetShow(false)
    end
    return
  end
  PaGlobal_GuildChangeMark_All._ui.stc_CurrentMark:SetShow(true)
  PaGlobal_GuildChangeMark_All._ui.stc_CurIconEffect:SetShow(idx ~= 0)
