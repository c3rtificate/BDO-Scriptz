...
function PaGlobal_GuildIncentiveOption_All:ControlPc_Init()
  ...
  self._ui_pc.btn_setting = UI.getChildControl(self._ui.stc_bottomBg, "Button_Option")
end
...
function PaGlobal_GuildIncentiveOption_All:ControlSetShow()
  ...
  if true == self._isConsole then
    ...
    self._ui_pc.btn_setting:SetShow(false)
    ...
    self._ui_pc.btn_setting:SetShow(true)
...
function PaGlobal_GuildIncentiveOption_All:CreateControl()
  ...
  local function createListInfo(pIndex)
    ...
    tempMember._radio_All:SetCheck(false)
    tempMember._radio_Personal:SetCheck(true)
...
function PaGlobal_GuildIncentiveOption_All:prepareOpen()
  if nil == Panel_GuildIncentiveOption_All then
    return
  end
  ...
  self._moneyValue = 0
  ToClient_InitGuildIncentiveListClear()
...
function PaGlobal_GuildIncentiveOption_All:registEventHandler()
  ...
  if true == self._isConsole then
    ...
    self._ui_pc.btn_setting:addInputEvent("Mouse_LUp", "PaGlobal_GuildIncentiveSetting_All_Open()")
  ...
  Panel_GuildIncentiveOption_All:registerPadEvent(__eConsoleUIPadEvent_Up_X, "PaGlobal_GuildIncentiveSetting_All_Open()")
end
...
function PaGlobal_GuildIncentiveOption_All:setGuildMemberData()
  ...
  for index = 1, memberCount do
    ...
    tempGuildIncentive = {
      ...
      ap = myGuildMemberInfo:getActivityPoint(),
...
function PaGlobal_GuildIncentiveOption_All:updateListInfo()
  ...
  for index = 0, self._listMaxCount do
    ...
    self._ui._memberlist[index]._radio_All:SetShow(false)
    self._ui._memberlist[index]._radio_Personal:SetShow(false)
  end
  local myGuildListInfo = ToClient_GetMyGuildInfoWrapper()
  if nil == myGuildListInfo then
    return
  ...
  for i = 1, memberCount do
    ...
    elseif __eGuildMemberGradeAdviser == gradeType then
      self._ui._memberlist[index]._grade:ChangeTextureInfoName("Combine/Etc/Combine_Etc_Guild_00.dds")
      local x1, y1, x2, y2 = setTextureUV_Func(self._ui._memberlist[index]._grade, 68, 1, 110, 27)
      self._ui._memberlist[index]._grade:getBaseTexture():setUV(x1, y1, x2, y2)
    ...
    elseif __eGuildMemberGradeGunner == gradeType then
      self._ui._memberlist[index]._grade:ChangeTextureInfoName("Combine/Etc/Combine_Etc_Guild_00.dds")
      local x1, y1, x2, y2 = setTextureUV_Func(self._ui._memberlist[index]._grade, 68, 28, 110, 54)
      self._ui._memberlist[index]._grade:getBaseTexture():setUV(x1, y1, x2, y2)
    elseif __eGuildMemberGradeScribe == gradeType then
      self._ui._memberlist[index]._grade:ChangeTextureInfoName("Combine/Etc/Combine_Etc_Guild_00.dds")
      local x1, y1, x2, y2 = setTextureUV_Func(self._ui._memberlist[index]._grade, 500, 44, 543, 70)
      self._ui._memberlist[index]._grade:getBaseTexture():setUV(x1, y1, x2, y2)
    elseif __eGuildMemberGradeChiefOfStaff == gradeType then
      self._ui._memberlist[index]._grade:ChangeTextureInfoName("Combine/Etc/Combine_Etc_Guild_00.dds")
      local x1, y1, x2, y2 = setTextureUV_Func(self._ui._memberlist[index]._grade, 500, 114, 543, 140)
      self._ui._memberlist[index]._grade:getBaseTexture():setUV(x1, y1, x2, y2)
    elseif __eGuildMemberGradeMissionOfficer == gradeType then
      self._ui._memberlist[index]._grade:ChangeTextureInfoName("Combine/Etc/Combine_Etc_Guild_00.dds")
      local x1, y1, x2, y2 = setTextureUV_Func(self._ui._memberlist[index]._grade, 500, 141, 543, 167)
      self._ui._memberlist[index]._grade:getBaseTexture():setUV(x1, y1, x2, y2)
    ...
    if true == guildMemberInfo:isOnline() then
      ...
      local usableActivity = guildMemberInfo:getActivityPoint()
      self._ui._memberlist[index]._activity:SetText(tostring(usableActivity))
    else
    ...
    if true == self._isConsole then
      ...
      self._ui._memberlist[index]._memberBg:registerPadEvent(__eConsoleUIPadEvent_Up_A, "HandleEventLUp_GuildIncentiveOption_All_Grade(" .. index .. ")")
    else
