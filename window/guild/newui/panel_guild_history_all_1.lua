function PaGlobal_Guild_History_All:initialize()
  ...
  self._ui.stc_KeyGuide_LT = UI.getChildControl(self._ui.stc_topArea, "Static_LT_ConsoleUI")
  self._ui.stc_KeyGuide_RT = UI.getChildControl(self._ui.stc_topArea, "Static_RT_ConsoleUI")
  ...
  self._ui.txt_noLog = UI.getChildControl(self._ui.stc_mainArea, "StaticText_NoLog")
  PaGlobal_Guild_History_All._isConsole = _ContentsGroup_UsePadSnapping
  local startSpan = 200
  ...
  for index = 1, 12 do
    ...
    self._ui._monthControl[index]:SetSpanSize(startSpan + (self._ui._monthControl[index]:GetSizeX() + 10) * index, 0)
    if false == self._isConsole then
      self._ui._monthControl[index]:addInputEvent("Mouse_LUp", "HandleEventLUp_Guild_History_All_MonthCheck(" .. index .. ")")
    else
      self._ui._monthControl[index]:SetIgnore(true)
    end
...
function PaGlobal_Guild_History_All:registEventHandler()
  ...
  if true == self._isConsole then
    PaGlobal_GuildMain_All._ui.stc_History_Bg:registerPadEvent(__eConsoleUIPadEvent_Up_LT, "HandleEventLUp_Guild_History_All_ChangeCurrentValue(" .. -1 .. ")")
    PaGlobal_GuildMain_All._ui.stc_History_Bg:registerPadEvent(__eConsoleUIPadEvent_Up_RT, "HandleEventLUp_Guild_History_All_ChangeCurrentValue(" .. 1 .. ")")
  end
...
function PaGlobal_Guild_History_All:switchPlatform()
  if true == self._isConsole then
    self._ui.stc_KeyGuide_LT:SetShow(true)
    self._ui.stc_KeyGuide_RT:SetShow(true)
    self._ui.btn_left:SetShow(false)
    self._ui.btn_right:SetShow(false)
    self._ui.txt_titleYear:SetShow(false)
  else
  end
end
