...
function PaGlobal_GuildIntro_All:prepareOpen()
  ...
  local checkButtonImgSize = 30
  local gabX = 10
  local maxTextSizeX = self._ui._activityControl[1]:GetSizeX() - (checkButtonImgSize + gabX * 3)
  for i = 1, self._maxActivityTypeCount do
    self._ui._activityControl[i]:SetCheck(false)
    if maxTextSizeX < self._ui._activityControl[i]:GetTextSizeX() then
      self._ui._activityControl[i]:SetTextHorizonLeft()
      self._ui._activityControl[i]:SetTextSpan(checkButtonImgSize, self._ui._activityControl[i]:GetTextSpan().y)
      UI.setLimitTextAndAddTooltip(self._ui._activityControl[i])
    end
  end
...
function PaGlobal_GuildIntro_All:registIntroduction()
  ...
  local dailyPayString = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_GUILDINTRODUCEREGIST_SALARY") .. " " .. self._ui.txt_Salary:GetText()
