...
function HandleEventLUp_Guild_History_All_YearCheck(index)
  ...
  if false == PaGlobal_Guild_History_All._isConsole then
...
function HandleEventLUp_Guild_History_All_ChangeCurrentValue(moveDir)
  if nil == PaGlobal_GuildMain_All._ui.stc_History_Bg or false == PaGlobal_GuildMain_All._ui.stc_History_Bg:GetShow() then
    return
  end
  local nextYear = PaGlobal_Guild_History_All._year
  local nextMonth = PaGlobal_Guild_History_All._month
  ...
  if nextYear < PaGlobal_Guild_History_All._firstLogYearValue or nextYear > ToClient_GetThisYear() then
    return
  end
  if nextYear == PaGlobal_Guild_History_All._year and nextMonth == PaGlobal_Guild_History_All._month then
    return
  end
  for idx = 1, #PaGlobal_Guild_History_All._ui._monthControl do
    if idx == nextMonth then
      PaGlobal_Guild_History_All._ui._monthControl[idx]:SetCheck(true)
    else
      PaGlobal_Guild_History_All._ui._monthControl[idx]:SetCheck(false)
    end
  end
  PaGlobal_Guild_History_All._year = nextYear
  PaGlobal_Guild_History_All._month = nextMonth
  local isLeftHide = PaGlobal_Guild_History_All._firstLogYearValue == nextYear and 1 == nextMonth
  local isRightHide = ToClient_GetThisYear() == nextYear and 12 == nextMonth
  PaGlobal_Guild_History_All._ui.stc_KeyGuide_LT:SetShow(not isLeftHide)
  PaGlobal_Guild_History_All._ui.stc_KeyGuide_RT:SetShow(not isRightHide)
  if ToClient_GetThisYear() <= PaGlobal_Guild_History_All._year and ToClient_GetThisMonth() < PaGlobal_Guild_History_All._month then
    PaGlobal_Guild_History_All._day = 0
  end
  if ToClient_GetThisMonth() == PaGlobal_Guild_History_All._month then
    ...
    PaGlobal_Guild_History_All._day = PaGlobal_Guild_History_All._dayCount[PaGlobal_Guild_History_All._month]
  end
  PaGlobal_Guild_History_All:dataUpdate()
end
function FromClient_Guild_History_All_Update()
  ...
  HandleEventOnOut_Guild_History_All_ShowTooltip(false)
  local newContent = {
  ...
  local noLogText = UI.cloneControl(PaGlobal_Guild_History_All._ui.txt_noLog, PaGlobal_Guild_History_All._ui.frameContent, "StaticText_NoLog")
  noLogText:SetPosY(315)
  ...
  if 0 == PaGlobal_Guild_History_All._day or nil == PaGlobal_Guild_History_All._day then
    return
  end
  local sizeY = 20
  local lineGap = 30
  ...
  for dayIndex = PaGlobal_Guild_History_All._day, 1, -1 do
    ...
    for i = PaGlobal_Guild_History_All._listCount - 1, 0, -1 do
      ...
      if nil ~= _journalInfo[i] and dayIndex == _journalInfo[i]:getJournalDay() then
        ...
        local spanDir = 1
        if 1 == dayLogCount % 2 then
          local _dayHistoryLeftValue = UI.cloneControl(PaGlobal_Guild_History_All._ui.txt_leftHistory, PaGlobal_Guild_History_All._ui.frameContent, "StaticText_GuildHistory_" .. i)
          PaGlobal_Guild_History_All._ui._dayHistoryValue[i] = _dayHistoryLeftValue
          spanDir = -1
        else
          local _dayHistoryRightValue = UI.cloneControl(PaGlobal_Guild_History_All._ui.txt_rightHistory, PaGlobal_Guild_History_All._ui.frameContent, "StaticText_GuildHistory_" .. i)
