function PaGlobalFunc_GuildIncentiveOption_All_Open()
  PaGlobal_GuildIncentiveOption_All:prepareOpen()
end
...
function PaGlobal_GuildIncentiveOption_All_IncentiveInfo()
  if nil == Panel_GuildIncentiveOption_All then
    return false
  end
  if Panel_GuildIncentiveOption_All:GetShow() == false then
    return
  end
...
function HandleEventLUp_GuildIncentiveOption_All_ListSort(sortType)
  ...
  elseif 1 == PaGlobal_GuildIncentiveOption_All._selectSortType then
    if false == PaGlobal_GuildIncentiveOption_All._listSort.ap then
      PaGlobal_GuildIncentiveOption_All._ui._title.txt_activity:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_WINDOW_GUILD_ATTRIBUTEREPORT_POINTTITLE") .. "\226\150\178")
      PaGlobal_GuildIncentiveOption_All._listSort.ap = true
    else
      PaGlobal_GuildIncentiveOption_All._ui._title.txt_activity:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_WINDOW_GUILD_ATTRIBUTEREPORT_POINTTITLE") .. "\226\150\188")
...
function PaGlobalFunc_GuildIncentiveOption_All_SetMoneyValue(selectPercent)
  if selectPercent == nil then
    return
  end
  if nil == Panel_GuildIncentiveOption_All then
    return
  ...
  local businessFundsResult = businessFunds * toInt64(0, selectPercent) / toInt64(0, 100)
  PaGlobal_GuildIncentiveOption_All._moneyValue = businessFundsResult
  ToClient_InitGuildIncentiveList(PaGlobal_GuildIncentiveOption_All._moneyValue)
  PaGlobal_GuildIncentiveOption_All:update()
end
function PaGlobalFunc_GuildIncentiveOption_All_TitleLineReset()
  ...
  PaGlobal_GuildIncentiveOption_All._ui._title.txt_activity:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_WINDOW_GUILD_ATTRIBUTEREPORT_POINTTITLE"))
end
