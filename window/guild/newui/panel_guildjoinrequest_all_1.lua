...
function PaGlobal_GuildJoinRequestList_All:ControlAll_Init()
  ...
  if ToClient_isPS5() == true then
    self._ui._title.txt_level:SetSpanSize(25, 0)
...
function PaGlobal_GuildJoinRequestList_All:Refresh()
  ...
  if self._isConsole == true then
    if PaGlobal_GuildJoinRequestList_All._isResetStart == true then
      return
    end
    PaGlobal_GuildJoinRequestList_All._isResetStart = true
    self._ui.list2_request:getElementManager():clearKey()
    Panel_GuildJoinRequestList_All:RegisterUpdateFunc("PaGlobalFunc_GuildJoinRequestList_All_PerFrameUpdate_All")
  end
...
function PaGlobal_GuildJoinRequestList_All:showGuildApplicantPR(index)
  if nil == PaGlobalFunc_GuildMain_All_GetBgShow then
    return
  end
  if false == PaGlobalFunc_GuildMain_All_GetBgShow(PaGlobal_GuildMain_All._eTABINDEX._RECRUIT) then
  ...
  PaGlobal_GuildIntroduceMySelf_ApplicantOpen(index)
end
function PaGlobal_GuildJoinRequestList_All:createList(content, index)
  ...
  local stc_platformIcon = UI.getChildControl(content, "Static_CrossPlayIcon")
  ...
  btn_whisper:SetShow(not _ContentsGroup_RenewUI)
  if true == _ContentsGroup_NewGuildNone then
    btn_delete:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_LOBBY_SELECTCLASS_KEYGUIDE_SHOW"))
    btn_delete:addInputEvent("Mouse_LUp", "PaGlobal_GuildJoinRequestList_All:showGuildApplicantPR(" .. key .. ")")
  ...
  if ToClient_isPS5() == true then
    txt_level:SetPosX(stc_platformIcon:GetPosX() + (stc_platformIcon:GetSizeX() - 5))
    local iconTextureName = "combine/commonicon/crossplay_icon_other.dds"
    local platformNickname = "-"
    if applicantInfoWrapper:getPlatformType() == __eGAME_PLATFORM_TYPE_PS then
      iconTextureName = "combine/commonicon/crossplay_icon_ps.dds"
      platformNickname = applicantInfoWrapper:getPlatformNickname()
      if platformNickname == nil or platformNickname == "" then
        platformNickname = "-"
      end
    end
    stc_platformIcon:ChangeTextureInfoName(iconTextureName)
    stc_platformIcon:SetShow(true)
    txt_charName:SetText(txt_charName:GetText() .. " / ( " .. platformNickname .. " )")
    txt_charName:ComputePos()
  end
end
