...
function PaGlobalFunc_GuildAlliance_All_RegisterUpdateSkillCoolTime()
  if nil ~= PaGlobalFunc_GuildSkill_All_CoolTimeUpdatePerFrame then
    Panel_GuildMain_All:RegisterUpdateFunc("PaGlobalFunc_GuildSkill_All_CoolTimeUpdatePerFrame")
  end
end
function HandleEventPadUp_GuildMain_All_PressOtherTab(isUp)
  if true == _ContentsGroup_UsePadSnapping and false == _ContentsGroup_RenewUI and nil ~= Panel_Tooltip_Item_hideTooltip then
...
function PaGlobalFunc_GuildMain_All_RightCheck(eRightType)
  ...
  local function OriginalCheck()
    ...
    if __eGuildRightType_Recruitment == eRightType then
      return isMasters
    end
    return false
  end
