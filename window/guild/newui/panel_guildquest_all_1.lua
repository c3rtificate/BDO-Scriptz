function PaGlobal_GuildQuest_All:initialize()
  ...
  self._ui.stc_ArbitratioBG = UI.getChildControl(self._ui.stc_TopBG, "Static_Arbitration_BG")
  self._ui.txt_ArbitrationQuestTime = UI.getChildControl(self._ui.stc_ArbitratioBG, "StaticText_QuestTime")
  self._ui.txt_ArbitrationQuestName = UI.getChildControl(self._ui.stc_ArbitratioBG, "StaticText_QusetName")
  self._ui.txt_ArbitrationQuestCondition = UI.getChildControl(self._ui.stc_ArbitratioBG, "StaticText_QuestCondition")
  self._ui.txt_ArbitrationDesc = UI.getChildControl(self._ui.stc_ArbitratioBG, "StaticText_Desc")
  self._ui.txt_ArbitrationTitle = UI.getChildControl(self._ui.stc_ArbitratioBG, "StaticText_Title")
  for i = 0, self._progressQuestConditionMaxCount - 1 do
    self._arbitrationQuestCondition[i] = UI.createAndCopyBasePropertyControl(self._ui.stc_ArbitratioBG, "StaticText_QuestCondition", self._ui.stc_ArbitratioBG, "StaticText_QuestCondition_" .. i)
    self._arbitrationQuestCondition[i]:SetPosY(self._arbitrationQuestCondition[i]:GetPosY() + (self._arbitrationQuestCondition[i]:GetSizeY() + 5) * i)
  end
  ...
  self._ui.txt_ReAcceptTime = UI.getChildControl(self._ui.stc_TopBG, "StaticText_ReacceptTime")
  ...
  self._ui.stc_QuestRefreshTime = UI.getChildControl(self._ui.stc_MainArea, "StaticText_QuestRefreshTime")
...
function PaGlobal_GuildQuest_All:registEventHandler(isConsole)
  ...
  registerEvent("FromClient_ResponsGuildQuestLastRefreshTickCount", "FromClient_ResponsGuildQuestLastRefreshTickCount")
  registerEvent("FromClient_ResponsGuildQuestRemainRestrictTickCount", "FromClient_GuildQuest_ResponsGuildQuestRemainRestrictTickCount")
  for i = 0, self._qeustListMaxShowCount - 1 do
...
function PaGlobal_GuildQuest_All:updateProgressQuest()
  ...
  local isArbitrationQuest = false
  for i = 0, self._progressQuestConditionMaxCount - 1 do
    ...
    self._arbitrationQuestCondition[i]:SetShow(false)
    self._arbitrationQuestCondition[i]:SetText("")
    self._arbitrationQuestCondition[i]:SetLineRender(false)
    self._arbitrationQuestCondition[i]:SetFontColor(Defines.Color.C_FFC4BEBE)
  end
  self._ui.txt_ProgressQuestName:SetShow(false)
  self._ui.txt_ProgressQuestTime:SetShow(false)
  self._ui.stc_ArbitratioBG:SetShow(false)
  self._ui.txt_ArbitrationQuestName:SetShow(false)
  self._ui.txt_ArbitrationQuestTime:SetShow(false)
  self._ui.txt_ArbitrationDesc:SetShow(false)
  self._ui.txt_ArbitrationTitle:SetShow(false)
  ...
  if ToClient_getCurrentGuildQuestType() == __eGuildQuestType_Arbitration or self._guildQuestType == __eGuildQuestType_Arbitration then
    isArbitrationQuest = true
    self._ui.stc_ArbitratioBG:SetShow(true)
    questName = self._ui.txt_ArbitrationQuestName
    questTime = self._ui.txt_ArbitrationQuestTime
    questConditionList = self._arbitrationQuestCondition
  end
  ...
  if isProgressing then
    ...
    if isArbitrationQuest == true then
      questName:SetText(PAGetStringParam2(Defines.StringSheet_GAME, "LUA_GUILD_QUEST_LIMIT", "currentGuildQuestName", currentGuildQuestName, "requireGuildRankStr", requireGuildRankStr))
    else
    ...
    if true == isRightQuestComplete then
    ...
    elseif isArbitrationQuest == true then
      self._ui.txt_ArbitrationDesc:SetShow(true)
      questConditionList[0]:SetShow(false)
      questName:SetShow(false)
      self._ui.txt_ArbitrationTitle:SetShow(true)
    else
  ...
  for index = 0, self._qeustListMaxShowCount - 1 do
    ...
    if questIndex < #self._currentQuestListData then
      ...
      if nil ~= guildQuestList then
        ...
        if true == isRightQuestAccept and false == isProgressing then
          if enableQuestCount > 0 or self._guildQuestType == __eGuildQuestType_Arbitration then
...
function PaGlobal_GuildQuest_All:setQuestRefreshTime(tickCount_s64)
  if false == self._parentBG:GetShow() then
    return
  end
  local tick32 = Int64toInt32(tickCount_s64)
  tick32 = tick32 / 1000
  local sec = tick32 % 60
  local minute = tick32 / 60
  local timeString = ""
  if tick32 >= 0 and minute <= 60 and sec > 0 then
    timeString = PAGetStringParam2(Defines.StringSheet_GAME, "LUA_REFRESH_GUILDQUEST_REMAINTIME", "remainMinute", string.format("%d", minute), "remainSecond", string.format("%d", sec))
  end
  self._ui.stc_QuestRefreshTime:SetText(timeString)
end
function PaGlobal_GuildQuest_All:setQuestRestrictTime(tickCount_s64)
  if false == self._parentBG:GetShow() then
    return
  end
  local tick32 = Int64toInt32(tickCount_s64)
  ...
  local minute = tick32 / 60
  local timeString = ""
  if tick32 <= 0 or minute >= 60 or minute < 0 or 0 >= self._enableQuestCount then
    self._ui.txt_ReAcceptTime:SetText(timeString)
    return
  end
  if minute <= 1 then
    minute = 1
  end
  timeString = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_RESTRICT_GUILDQUEST_REMAINTIME", "remainMinute", string.format("%d", minute))
  self._ui.txt_ReAcceptTime:SetShow(true)
  self._ui.txt_ReAcceptTime:SetText(timeString)
end
