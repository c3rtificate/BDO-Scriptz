function PaGlobal_GuildAlliance_All:initialize()
  ...
  self._ui.edit_NoticeCover = UI.getChildControl(self._ui.stc_Right, "MultilineEdit_CoverBox")
  ...
  self._ui.btn_Mark:SetShow(not self._isConsole and _ContentsOption_CH_NoCustomGuildMark == false)
  ...
  if true == _ContentsGroup_AllianceMemberMaxChange then
    self._ui.txt_Desc:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_GUILDALLIANCEINFO_GUILDALLIANCE_RULES_TEXT_NEW"))
  else
...
function PaGlobal_GuildAlliance_All:registerInputEvent_PC()
  ...
  self._ui.edit_NoticeCover:addInputEvent("Mouse_LUp", "HandleEventLUp_GuildAlliance_All_SetEditText()")
...
function PaGlobal_GuildAlliance_All:buttonCheck()
  ...
  self._ui.btn_Mark:SetShow(isChairman and _ContentsOption_CH_NoCustomGuildMark == false)
  ...
  self._ui.edit_NoticeCover:SetIgnore(not isChairman)
end
...
function PaGlobal_GuildAlliance_All:AllianceInfoUpdate(myAllianceWrapper)
  ...
  local memberCount = ""
  if true == _ContentsGroup_AllianceMemberMaxChange then
    memberCount = tostring(myAllianceWrapper:getUserCount()) .. " / 150"
  else
    memberCount = tostring(myAllianceWrapper:getUserCount()) .. " / 100"
  end
