...
function PaGlobal_GuildVolunteerList_All:ControlAll_Init()
  ...
  self._ui.btn_warReward = UI.getChildControl(self._ui.stc_bottomBg, "Button_WarReward")
  ...
  self._ui.btn_warReward:SetShow(PaGlobalFunc_GuildMain_All_CheckIsMecernary() and _ContentsGroup_Siege2024 == false)
end
...
function PaGlobal_GuildVolunteerList_All:prepareOpen()
  ...
  if nil ~= getSelfPlayer() then
    local isRightVolunteerHire = false
    if _ContentsGroup_GuildAuthoritySeparation == true then
      isRightVolunteerHire = PaGlobalFunc_Guild_Authorization_RightCheck(__eGuildRightNew_Volunteer)
    else
      isRightVolunteerHire = PaGlobalFunc_GuildVolunteerList_All_RightCheck(__eGuildRightType_Volunteer)
    end
...
function PaGlobal_GuildVolunteerList_All:registEventHandler()
  ...
  registerEvent("FromClient_ResponseTakableSiegeReward", "FromClient_GuildVolunteerList_All_ResponseTakableSiegeReward")
  ...
  self._ui.btn_warReward:addInputEvent("Mouse_LUp", "ToClient_resquestTakeSiegeMercenaryMedalReward()")
end
...
function PaGlobal_GuildVolunteerList_All:updateListInfo()
  ...
  for index = 0, volunteerCount - 1 do
    ...
    local userNo = volunteerMemberInfo:getUserNo()
    local myUserNo = getSelfPlayer():get():getUserNo()
    if true == volunteerMemberInfo:isSelf() or userNo == myUserNo then
      local isTakableSiegeReward = volunteerMemberInfo:isTakableSiegeReward()
...
function FromClient_GuildVolunteerList_All_ResponseTakableSiegeReward()
  PaGlobal_GuildVolunteerList_All:updateListInfo()
end
function PaGlobal_GuildVolunteerList_All:updateListMemeber(member, index)
  ...
  local isRightVolunteerMenu = false
  if _ContentsGroup_GuildAuthoritySeparation == true then
    isRightVolunteerMenu = PaGlobalFunc_Guild_Authorization_RightCheck(__eGuildRightNew_SetMemberLimitPrice)
  else
    isRightVolunteerMenu = PaGlobalFunc_GuildVolunteerList_All_RightCheck(__eGuildRightType_SetMemberLimitPrice)
  end
  if false == member:isSelf() and nil ~= getSelfPlayer() then
    if true == isRightVolunteerMenu then
  ...
  local isRightContract = false
  if _ContentsGroup_GuildAuthoritySeparation == true then
    isRightContract = PaGlobalFunc_Guild_Authorization_RightCheck(__eGuildRightNew_Join)
  else
    isRightContract = PaGlobalFunc_GuildVolunteerList_All_RightCheck(__eGuildRightType_Contract)
  end
  local padding = 10
  if true == member:isSelf() or true == isRightContract then
  ...
  if true == isRightContract or true == member:isSelf() then
