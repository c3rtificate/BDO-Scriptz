...
function CreateListContent_GuildBattle_EntryMember(content, index)
  if nil == Panel_GuildBattle_EntryList then
    return
  end
  ...
  static_ClassIcon:ChangeTextureInfoName(classSymbomInfo.path)
  local x1, y1, x2, y2 = setTextureUV_Func(static_ClassIcon, classSymbomInfo.x1, classSymbomInfo.y1, classSymbomInfo.x2, classSymbomInfo.y2)
...
function PaGlobal_GuildBattle_SelectEntry:Initialize()
  ...
  local ui = self._ui
  ui._btn_WinClose = UI.getChildControl(Panel_GuildBattle_EntryList, "Button_Win_Close")
  ui._staticText_Title = UI.getChildControl(Panel_GuildBattle_EntryList, "StaticText_Title")
  ui._static_TopBg = UI.getChildControl(Panel_GuildBattle_EntryList, "Static_TopBg")
  ui._static_Bg = UI.getChildControl(Panel_GuildBattle_EntryList, "Static_Bg")
  ui._static_BottomBg = UI.getChildControl(Panel_GuildBattle_EntryList, "Static_BottomBg")
...
function PaGlobal_GuildBattle_SelectEntry:registEventHandler()
  if nil == Panel_GuildBattle_EntryList then
    return
  end
  self._ui._btn_Reload:addInputEvent("Mouse_LUp", "PaGlobal_GuildBattle_SelectEntry:UpdateMemberInfo()")
  self._ui._btn_ConfirmEntry:addInputEvent("Mouse_LUp", "PaGlobal_GuildBattle_SelectEntry:ConfirmEntryMember()")
  self._ui._list_GuildMembers:registEvent(__ePAUIList2EventType_LuaChangeContent, "CreateListContent_GuildBattle_EntryMember")
  self._ui._list_GuildMembers:createChildContent(__ePAUIList2ElementManagerType_List)
end
function PaGlobal_GuildBattle_SelectEntry:registMessageHandler()
...
function PaGlobal_GuildBattle_SelectEntry:Show(isShow)
  if true == isShow then
    PaGlobal_GuildBattleSelectEntry_CheckLoadUI()
    self:UpdateMemberInfo()
    self:UpdateRemainTime()
  else
    PaGlobal_GuildBattleSelectEntry_CheckCloseUI()
  end
end
...
function PaGlobal_GuildBattleSelectEntry_CheckLoadUI()
  if false == _ContentsGroup_PanelReload_Develop then
    Panel_GuildBattle_EntryList:SetShow(true)
    return
  end
  local rv = reqLoadUI("UI_Data/Window/Guild/Panel_GuildBattle_EntryList.XML", "Panel_GuildBattle_EntryList", Defines.UIGroup.PAGameUIGroup_Widget, PAUIRenderModeBitSet({
    Defines.RenderMode.eRenderMode_Default
  }))
  if nil ~= rv then
    Panel_GuildBattle_EntryList = rv
    rv = nil
    PaGlobal_GuildBattleSelectEntry_Init()
  end
  Panel_GuildBattle_EntryList:SetShow(true)
end
function PaGlobal_GuildBattleSelectEntry_CheckCloseUI()
  if false == PaGlobal_GuildBattle_SelectEntry:IsShow() then
    return
  end
  if false == _ContentsGroup_PanelReload_Develop then
    Panel_GuildBattle_EntryList:SetShow(false)
  else
    reqCloseUI(Panel_GuildBattle_EntryList)
  end
end
function FromClient_luaLoadComplete_GuildBattle_SelectEntry()
  PaGlobal_GuildBattleSelectEntry_Init()
  PaGlobal_GuildBattle_SelectEntry:registMessageHandler()
end
function PaGlobal_GuildBattleSelectEntry_Init()
  PaGlobal_GuildBattle_SelectEntry:Initialize()
  PaGlobal_GuildBattle_SelectEntry:registEventHandler()
end
