...
function guildRanking:Update()
  ...
  for listIdx = startSlotNo, self._listCount - 1 do
    ...
    list.guild:addInputEvent("Mouse_LUp", "PaGlobalFunc_GuildWebInfo_All_GuildWebInfoOpen( " .. listIdx .. " )")
...
function PaGlobal_GuildRank:initialize()
  if nil == Panel_GuildRank_Web then
    return
  end
  Panel_GuildRank_Web:SetShow(false, false)
  self._ui.titleBG = UI.getChildControl(Panel_GuildRank_Web, "Static_TitleBG")
  self._ui.webClose = UI.getChildControl(self._ui.titleBG, "Button_Win_Close")
  self._ui.webHelp = UI.getChildControl(self._ui.titleBG, "Button_Question")
  self._ui.webRefresh = UI.getChildControl(self._ui.titleBG, "Button_Refresh")
  self._ui.rankBg = UI.getChildControl(Panel_GuildRank_Web, "Static_RankListBG")
  self._ui.webClose:SetShow(not _ContentsGroup_UsePadSnapping)
  self._ui.webRefresh:SetShow(_ContentsGroup_ResetCoherent)
  self._ui.rankBg:SetShow(false)
  self._ui.webHelp:SetShow(false)
  self._Web = UI.createControl(__ePAUIControl_WebControl, Panel_GuildRank_Web, "WebControl_GuildRank_WebLink")
...
function PaGlobal_GuildRank:registerEventHandler()
  if nil == Panel_GuildRank_Web then
    return
  end
  PaGlobal_Util_RegistWebResetControl(self._ui.webRefresh)
  self._ui.webClose:addInputEvent("Mouse_LUp", "GuildRank_Web_Close()")
...
function GuildRank_Web_Show()
  if not Panel_GuildRank_Web:GetShow() then
    ...
    PaGlobal_GuildRank._Web:ResetUrl()
    ...
    PaGlobal_GuildRank._Web:SetUrl(PaGlobal_GuildRank._webSizeX, PaGlobal_GuildRank._webSizeY, url, false, true)
    PaGlobal_GuildRank._Web:SetIME(true)
    audioPostEvent_SystemUi(1, 0)
    _AudioPostEvent_SystemUiForXBOX(1, 0)
  end
end
...
function FromClient_GuildRank_Initialize()
  PaGlobal_GuildRank:initialize()
end
function FromClient_resetCoherentUIForGuildRank()
  if nil ~= Panel_GuildRank_Web and Panel_GuildRank_Web:GetShow() then
    TooltipSimple_Hide()
    GuildRank_Web_Close()
  end
end
registerEvent("FromClient_luaLoadComplete", "FromClient_GuildRank_Initialize")
registerEvent("FromClient_resetCoherentUI", "FromClient_resetCoherentUIForGuildRank")
