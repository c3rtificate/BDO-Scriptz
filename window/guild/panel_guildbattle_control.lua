...
local ShowBattleStateChangeMessage = function(state)
  ...
  local msg = {
    ...
    sub = PaGlobal_GuildBattle_GetTitle(),
    addMsg = ""
  }
...
function FGlobal_GuildBattle_UpdatePerFrame(deltaTime)
  if nil == ToClient_GuildBattle_IsInGuildBattle or nil == PaGlobal_GuildBattle_SelectEntry or nil == PaGlobal_GuildBattle_SelectAttend or nil == PaGlobal_GuildBattlePoint or nil == PaGlobal_GuildBattle_Control then
    return
  end
  if nil == PaGlobal_Guild_Battle_All then
    return
  end
  ...
  if PaGlobal_GuildBattle_Control._elapsedTime >= 0.2 then
    ...
    if true == PaGlobalFunc_Guild_Battle_All_IsShow() then
      PaGlobal_Guild_Battle_All_UpdateRemainTime()
    end
...
function GuildBattle_UpdatePerFrame(deltaTime)
  ...
  if nil == GuildWatchMode._remainTime then
    ...
    FromClient_DeadMessage_All_NotifySiegeShowWatchPanel(false)
...
function PaGlobal_GuildBattle_GetTitle()
  if true == ToClient_isPersonalBattle() then
