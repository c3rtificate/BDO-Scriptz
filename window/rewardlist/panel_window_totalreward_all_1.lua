function PaGlobal_TotalReward_All:initialize()
  ...
  self._ui.stc_eventBanner = UI.getChildControl(Panel_Window_TotalReward_All, "Static_EventBanner")
  self._ui.stc_eventBanner:SetShow(_ContentsGroup_DrawEventNotice)
...
function PaGlobal_TotalReward_All:prepareOpen()
  ...
  if true == _ContentsGroup_RenewUI then
    Panel_Window_TotalReward_All:ComputePos()
  end
...
function PaGlobal_TotalReward_All:receiveSilver()
  if Panel_Window_TotalReward_All == nil then
    return
  end
  if PaGlobal_TotalReward_All._rewardSilver == nil or PaGlobal_TotalReward_All._rewardSilver == toInt64(0, 0) then
    return
  end
...
function PaGlobal_TotalReward_All:receiveAll()
  ...
  if true == PaGlobal_TotalReward_All._isConsole then
    MessageBox.showMessageBox(messageBoxData, "top", false, true)
  else
    MessageBox.showMessageBox(messageBoxData, "top", false, false)
  end
end
function PaGlobal_TotalReward_All:receiveItem(rewardKey, index, toWarehouse, isAllReceive)
  if nil == Panel_Window_TotalReward_All then
    return
  end
  ...
  ToClient_ReceivePendingReward(itemIdx, toWarehouse, isAllReceive)
end
          ...
          if nil ~= itemStatic and 1 ~= itemKey then
            ...
            if true == PaGlobal_TotalReward_All._isConsole then
              ...
              if true == _ContentsGroup_RenewUI_Tooltip then
                ...
                uiSlot.iconBg:addInputEvent("Mouse_Out", "HandleEventOn_TotalReward_All_SlotTooltip(" .. rewardKey .. "," .. index .. "," .. slotIndex .. ", false)")
                uiSlot.iconBg:registerPadEvent(__eConsoleUIPadEvent_X, "HandleEventOn_TotalReward_All_SlotTooltip(" .. rewardKey .. "," .. index .. "," .. slotIndex .. ", true)")
              end
            else
