...
function GoodsTooltipInfo:Initialize()
  ...
  self.Name:SetTextMode(__eTextMode_AutoWrap)
  self.Desc:SetAutoResize(true)
  self.Desc:SetTextMode(__eTextMode_AutoWrap)
  self.Desc:SetFontColor(UI_color.C_FFC4A68A)
  self.BindType:SetShow(false)
  self.BindType:SetTextMode(__eTextMode_AutoWrap)
  self.ClassType:SetShow(false)
  self.EnchantLimitedType:SetShow(false)
  self.SalesPeriod:SetShow(false)
  self.SalesPeriod:SetTextMode(__eTextMode_AutoWrap)
  self.SalesPeriod:SetAutoResize(true)
  self.DiscountPeriod:SetShow(false)
  self.DiscountPeriod:SetTextMode(__eTextMode_AutoWrap)
  self.DiscountPeriod:SetAutoResize(true)
  ...
  for itemIdx = 0, self.itemSlotMaxCount do
    local tempSlot = {}
    local CreatePackingItemBG = UI.createControl(__ePAUIControl_Static, self.ItemListTitle, "CashShop_GoodsTooltipInfo_PackingItemBG_" .. itemIdx)
    ...
    local CreatePackingItem = UI.createControl(__ePAUIControl_StaticText, CreatePackingItemBG, "CashShop_GoodsTooltipInfo_PackingItem_" .. itemIdx)
    ...
    local CreatePackingItemName = UI.createControl(__ePAUIControl_StaticText, CreatePackingItemBG, "CashShop_GoodsTooltipInfo_PackingItemName_" .. itemIdx)
    ...
    CreatePackingItemName:SetTextMode(__eTextMode_AutoWrap)
...
function GoodsTooltipInfo:Update()
  ...
  if nil ~= tradeDesc then
    self.Warning:SetTextMode(__eTextMode_AutoWrap)
  ...
  local slotPos = self.Slot:GetPosY() + self.Slot:GetSizeY()
  local namePos = self.Name:GetPosY() + self.Name:GetSizeY()
  if slotPos > namePos then
    controlPosY = controlPosY + slotPos + 5
  else
    controlPosY = controlPosY + namePos + 5
  end
  if self.BindType:GetShow() then
    self.BindType:SetPosY(controlPosY)
    if self.BindType:GetSizeY() < self.BindType:GetTextSizeY() then
      controlPosY = controlPosY + self.BindType:GetTextSizeY() + 5
    else
