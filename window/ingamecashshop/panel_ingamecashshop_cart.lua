...
local function updateUI(uiGroup, selectItem, uiSlot)
  ...
  prductNameUI:SetTextMode(__eTextMode_LimitText)
  discountMsgUI:SetTextMode(__eTextMode_LimitText)
  ...
  if selectItem:isDiscountApplied() then
    ...
    discountMsgUI:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_DISCOUNT_SIMPLE", "endDiscountTime", endDiscountTime))
...
function CashShopCart:initialize(gapY)
  CashShopCart._defaultUI = {
    ...
    [3] = {
      ...
      _type = __ePAUIControl_StaticText,
      _owner = 0
    },
    [4] = {
...
function CashShopCart:fillSlots()
  ...
  if Panel_IngameCashShop_MakePaymentsFromCart:IsShow() == true then
    FGlobal_IngameCashShop_MakePaymentsFromCart_Update()
  end
end
    ...
    if cumulatedTime > 1 then
      ...
      for index = self._currentSlotIndex, cartListCount - 1 do
        ...
        if selectItem:isDiscountApplied() then
          ...
          discountMsgUI:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_DISCOUNT_SIMPLE", "endDiscountTime", remainTime))
          discountMsgUI:SetShow(true)
        else
          discountMsgUI:SetText(cashProductSSW:getDescription())
          discountMsgUI:SetShow(true)
