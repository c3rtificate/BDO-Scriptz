...
local cashCoupon = {
  ...
  savedCouponProductNoRaw = nil,
  savedCouponProductCount = nil
}
local selectKey = -1
function IngameCashShopCoupon_Init()
  local self = cashCoupon
  self._list2:changeAnimationSpeed(10)
  self._list2:registEvent(__ePAUIList2EventType_LuaChangeContent, "IngameCashShopCoupon_ListUpdate")
  ...
  self._list2_Unused:registEvent(__ePAUIList2EventType_LuaChangeContent, "IngameCashShopCoupon_ListUpdate_Unused")
  self._list2_Unused:createChildContent(__ePAUIList2ElementManagerType_List)
...
function IngameCashShopCoupon_ListUpdate(contents, key)
  ...
  if nil ~= couponWrapper then
    ...
    txt_PearlIcon:SetPosY(74)
    txt_CategoryDesc:SetShow(true)
    txt_CategoryDesc:SetTextMode(__eTextMode_LimitText)
    txt_CategoryDesc:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COUPON_USECATEGORY") .. " : " .. tostring(couponCategory))
    if txt_CategoryDesc:IsLimitText() then
      txt_CategoryDesc:SetIgnore(false)
      txt_CategoryDesc:addInputEvent("Mouse_On", "IngameCashShopCoupon_SimpletooltipCategoryDesc(true, " .. idx .. ")")
      txt_CategoryDesc:addInputEvent("Mouse_Out", "IngameCashShopCoupon_SimpletooltipTitle()")
    else
      txt_CategoryDesc:SetIgnore(true)
      txt_CategoryDesc:addInputEvent("Mouse_On", "")
      txt_CategoryDesc:addInputEvent("Mouse_Out", "")
    end
...
function IngameCashShopCoupon_SavedCouponIndex(index)
  ...
  btn_Coupon:setRenderTexture(btn_Coupon:getBaseTexture())
  txt_Rate:SetFontColor(4293914607)
  if savedIndex ~= index then
    if savedIndex ~= selectKey then
      FGlobal_IngameShopBuy_Update(true)
    else
      FGlobal_IngameShopBuy_Update(false)
    end
  elseif savedIndex == selectKey then
    FGlobal_IngameShopBuy_Update(false)
    savedIndex = -1
    selectKey = -1
    self.savedCouponIndex = nil
    IngameCashShopCoupon_Update(self.savedCouponProductNoRaw, self.savedCouponProductCount)
  end
end
...
function IngameCashShopCoupon_SimpletooltipCategoryDesc(isShow, index)
  if nil == isShow then
    TooltipSimple_Hide()
    return
  end
  ...
  local name, desc, control
  local contents = self._list2:GetContentByKey(toInt64(0, index))
  local coupon_CategoryDesc = UI.getChildControl(contents, "StaticText_CouponCategoryDesc")
  local couponWrapper = ToClient_GetCouponInfoWrapper(index)
  if nil ~= couponWrapper then
    local couponName = couponWrapper:getCouponCategoryExpression()
    name = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_COUPON_TITLE")
    desc = couponName
    control = coupon_CategoryDesc
    TooltipSimple_Show(control, name, desc)
  end
end
...
function PaGlobalFunc_IngameCashShopCoupon_ResetValue()
  local self = cashCoupon
  savedIndex = -1
  selectKey = -1
  self.savedCouponIndex = nil
...
function IngameCashShopCoupon_Open(openType, productNoRaw, productCount)
  ...
  self._list2:SetShow(true)
  self._list2_Unused:SetShow(false)
  self.savedCouponProductNoRaw = productNoRaw
  self.savedCouponProductCount = productCount
  ...
  if true == Panel_IngameCashShop_Coupon:GetShow() and nil ~= Panel_IngameCashShop_MoonBlessing_All and true == Panel_IngameCashShop_MoonBlessing_All:GetShow() then
    Panel_IngameCashShop_Coupon:SetPosX(Panel_IngameCashShop_MoonBlessing_All:GetPosX() - Panel_IngameCashShop_Coupon:GetSizeX())
    Panel_IngameCashShop_BuyOrGift:SetPosX(Panel_IngameCashShop_Coupon:GetPosX() - Panel_IngameCashShop_Coupon:GetSizeX())
  end
end
