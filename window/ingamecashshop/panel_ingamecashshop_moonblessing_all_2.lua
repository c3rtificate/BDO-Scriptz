...
function HandleEvent_IngameCashShopMoonBlessing_All_ApplyReward(itemKey)
  ...
  if PaGlobal_IngameCashShopMoonBlessing_All._mileageInfo[itemKey] == nil then
    return
  end
  ...
  local messageBoxData = {
    ...
    content = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_MILEAGE_CONFIRM_MESSAGE"),
...
function PaGlobal_CashMileage_IsOpenCheck()
  ...
  if _ContentsGroup_PearlShopMileage == true then
    local tempCnt = 0
...
function PaGlobal_CashMileage_ChangeConsumePearl(pearlValue)
  ...
  if self._ui._list2_mileage[self.MILEAGE_TYPE.CONSUMED] == nil then
    return
  end
