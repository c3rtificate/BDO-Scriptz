...
local inGameShop = {
  ...
  _ch_banner = UI.getChildControl(Panel_IngameCashShop, "Static_1"),
  ...
  desc = {
    ...
    _staticText_GoodsSalesPeriod = nil,
    _staticText_PolicyDesc = nil,
    ...
    _btn_BigCart = nil,
    _btn_BigGift = nil,
    _btn_BigStyleAlbum = nil,
    _btn_BigECart = nil,
    _btn_BigECartSlot = nil,
    _btn_SideDetail = nil,
    _staticText_GuildGiveaway = nil
  },
  ...
  _pricePosX = 0,
  _savedProductNo = nil,
  _savedSelectedIndex = nil,
  _savedSelectedbValue = nil,
  _savedTabIndex = nil,
  _savedMainTab = nil,
  _savedSubTab = nil,
  _currentIndex = 0,
...
inGameShop._scrollBTN_IngameCash:addInputEvent("Mouse_UpScroll", "InGameShop_ScrollEvent( true )")
inGameShop._scrollBTN_IngameCash:addInputEvent("Mouse_DownScroll", "InGameShop_ScrollEvent( false )")
inGameShop._scroll_IngameCash:addInputEvent("Mouse_UpScroll", "InGameShop_ScrollEvent( true )")
inGameShop._scroll_IngameCash:addInputEvent("Mouse_DownScroll", "InGameShop_ScrollEvent( false )")
...
inGameShop._nowPearlIcon = UI.getChildControl(inGameShop._pearlBox, "Static_NowPearlIcon")
inGameShop._staticText_PearlCount = UI.getChildControl(inGameShop._pearlBox, "StaticText_NowPearlCount")
inGameShop._btn_BuyPearl = UI.getChildControl(inGameShop._pearlBox, "Button_BuyPearl")
inGameShop._nowCash = UI.getChildControl(inGameShop._cashBox, "Static_CashIcon")
inGameShop._staticText_CashCount = UI.getChildControl(inGameShop._cashBox, "StaticText_NowCashCount")
inGameShop._btn_BuyDaum = UI.getChildControl(inGameShop._cashBox, "Button_BuyDaum")
inGameShop._btn_RefreshCash = UI.getChildControl(inGameShop._cashBox, "Button_RefreshCash")
...
local tabId = {
  ...
  history = inGameShop._tabCount + 2
}
...
function InGameShop_GameTypeCheck()
  ...
  if isGameTypeTaiwan() then
    isTaiwanNation = true
  elseif isGameTypeTR() or isGameTypeTH() or isGameTypeASIA() or isGameTypeID() then
    isTaiwanNation = true
...
local function tag_changeTexture(slot, tagType)
  ...
  if tagType >= 8 then
    control:ChangeTextureInfoName("new_ui_common_forlua/window/ingamecashshop/CashShop_05.dds")
  elseif 7 == tagType then
...
function inGameShop:init()
  ...
  for ii = 0, classCount - 1 do
    ...
    if nil ~= className and "" ~= className and " " ~= className and PaGlobalFunc_Util_PearlShopClassTypeButtonOpenCheck(classType) == true then
  ...
  for dd = 1, getCashMainCategorySize() do
    ...
    if nil ~= mainTabInfo then
      tabIndexList[dd] = {
        ...
        mainTabInfo:isDisplay()
      }
      if false == mainTabInfo:isDisplay() then
        self._hideTabCount = self._hideTabCount + 1
      end
      if true == hidePearlBox and UI_CCC.eCashProductCategory_Pearl == mainTabInfo:getCategoryType() then
        self._hideTabCount = self._hideTabCount + 1
        tabIndexList[dd][8] = false
      end
    end
  end
  self:sortTapData()
  for ii = 1, self._tabCount do
    ...
    tab.text:SetTextMode(__eTextMode_AutoWrap)
    tab.static:SetText("")
    ...
    for jj = 1, subCategoryCount do
      ...
      subtab.text:SetTextMode(__eTextMode_LimitText)
  ...
  if isGameServiceTypeConsole == false then
    promotionTab.static:SetShow(false)
  end
  ...
  for ii = 0, maxSlotCount - 1 do
    ...
    slot.eventCartIcon = UI.createAndCopyBasePropertyControl(Panel_IngameCashShop, "TemplateList_Static_EventCart", slot.static, "InGameShop_Slot_EventCart_" .. ii)
    ...
    slot.discount:SetTextMode(__eTextMode_Limit_AutoWrap)
    slot.name:SetTextMode(__eTextMode_Limit_AutoWrap)
    ...
    slot.eventCartIcon:SetShow(false)
    slot.eventCartIcon:SetIgnore(false)
    slot.eventCartIcon:addInputEvent("Mouse_LUp", "IngameCashShop_ShowEventIconEffect()")
  ...
  if _ContentsGroup_RefundCashProduct == true then
    local myHistoryTab = {}
    myHistoryTab.static = UI.createAndCopyBasePropertyControl(Panel_IngameCashShop, "RadioButton_CategoryTab_0", Panel_IngameCashShop, "InGameShop_MyHistoryTab")
    myHistoryTab.text = UI.createAndCopyBasePropertyControl(Panel_IngameCashShop, "StaticText_Category", myHistoryTab.static, "InGameShop_Text")
    myHistoryTab.icon = UI.createAndCopyBasePropertyControl(Panel_IngameCashShop, "Static_ButtonIcon_0", myHistoryTab.static, "InGameShop_MyHistoryTab_Icon")
    myHistoryTab.static:SetPosX(tabConfig._startX)
    myHistoryTab.static:SetPosY(tabConfig._startY + tabConfig._gapY * (tabId.history - 1))
    myHistoryTab.text = UI.createAndCopyBasePropertyControl(Panel_IngameCashShop, "StaticText_Category", myHistoryTab.static, "StaticText_HistoryText")
    myHistoryTab.text:SetShow(true)
    myHistoryTab.text:SetHorizonLeft()
    myHistoryTab.text:SetVerticalMiddle()
    myHistoryTab.text:SetSpanSize(myHistoryTab.icon:GetSizeX() + 5, 0)
    myHistoryTab.text:ComputePos()
    myHistoryTab.text:SetTextMode(__eTextMode_AutoWrap)
    myHistoryTab.text:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_HISTORY_TAB_TITLE"))
    myHistoryTab.static:SetText("")
    myHistoryTab.static:SetShow(true)
    _ingameCash_SetTabIconTexture(myHistoryTab.icon, tabId.history, 0)
    self._historyTab = myHistoryTab
    myHistoryTab.static:ChangeTextureInfoTextureIDAsync("IngameCashShop_CashCategory_Frame_Normal", 0)
    myHistoryTab.static:ChangeTextureInfoTextureIDAsync("IngameCashShop_CashCategory_Frame_Over", 1)
    myHistoryTab.static:ChangeTextureInfoTextureIDAsync("IngameCashShop_CashCategory_Frame_Click", 2)
    myHistoryTab.icon:ChangeTextureInfoTextureIDAsync("IngameCashShop_CashCategory_Icon_Normal", 0)
    myHistoryTab.icon:ChangeTextureInfoTextureIDAsync("IngameCashShop_CashCategory_Icon_Over", 1)
    myHistoryTab.icon:ChangeTextureInfoTextureIDAsync("IngameCashShop_CashCategory_Icon_Click", 2)
    myHistoryTab.static:setRenderTexture(myHistoryTab.static:getBaseTexture())
    myHistoryTab.icon:setRenderTexture(myHistoryTab.icon:getBaseTexture())
  end
  ...
  self._promotionWeb = UI.createControl(__ePAUIControl_WebControl, Panel_IngameCashShop, "IngameCashShop_PromotionWebLink")
  ...
  self._categoryWeb = UI.createControl(__ePAUIControl_WebControl, self._static_PromotionBanner, "IngameCashShop_CategoryWebLink")
  ...
  elseif isGameTypeSA() then
    cashIcon_changeTexture(self._nowCash, contry.sa)
  ...
  if true == _ContentsGroup_MileageShop then
    ...
    self._mileageBox:SetShow(false)
    self._mileage:SetShow(false)
    self._staticText_MileageCount:SetShow(false)
    self._pearlBox:SetSize(self._pearlBox:GetSizeX() + self._mileageBox:GetSizeX() / 2, self._pearlBox:GetSizeY())
    self._cashBox:SetSize(self._cashBox:GetSizeX() + self._mileageBox:GetSizeX() / 2, self._cashBox:GetSizeY())
  end
  ...
  inGameShop:serviceTypeCheckControl()
end
function inGameShop:isCheckOverlapText(slot)
  local sizeX = slot.pearlIcon:GetPosX() + self.defaultPearlIconSize + slot.originalPrice:GetTextSizeX() + self.defaultPriceArrowSize
...
function inGameShop:serviceTypeCheckControl()
...
function inGameShop:getMaxPosition()
  ...
  if -1 ~= self._openProductKeyRaw and nil ~= self._openProductKeyRaw then
    maxPosition = (self._listCount - 1) * self._config._slot._gapY + self._slots[0].static:GetSizeY() - self._static_ScrollArea:GetSizeY() + self._goodDescBG:GetSizeY()
  else
    maxPosition = (self._listCount - 1) * self._config._slot._gapY + self._slots[0].static:GetSizeY() - self._static_ScrollArea:GetSizeY()
  end
...
function inGameShop:setElement(index, productNoRaw, slot)
  ...
  local cashProductBuySpeedUserType = cashProduct:getSpeedUserType(speedUserType)
  local isForSpeedUserCashItem = cashProduct:isForSpeedUserCashItem()
  ...
  if isForSpeedUserCashItem == true and cashProductBuySpeedUserType == false then
    slot.discount:SetText(cashProduct:getDescription())
  elseif cashProduct:isApplySellTimeLimit() then
    local startSellTimeValue = PATime(cashProduct:getStartSellTime():get_s64())
    local endSellTimeValue = PATime(cashProduct:getEndSellTime():get_s64())
    ...
    local startSellTime = tostring(startSellTimeValue:GetYear()) .. "." .. tostring(startSellTimeValue:GetMonth()) .. "." .. tostring(startSellTimeValue:GetDay())
    local yearValue = endSellTimeValue:GetYear()
    if yearValue < __eInvisibleCashProductPeriodYear then
      local resType = getGameServiceResType()
      if true == isGameTypeTH() or CppEnums.ServiceResourceType.eServiceResourceType_TH == resType then
        yearValue = 543 + yearValue
      end
      local endSellTime = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GOODSDETAILINFO_SELLTIME", "GetYear", tostring(yearValue), "GetMonth", formattedMonth, "GetDay", tostring(endSellTimeValue:GetDay())) .. " " .. string.format("%.02d", endSellTimeValue:GetHour()) .. ":" .. string.format("%.02d", endSellTimeValue:GetMinute())
      local countryKind = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GOODSDETAILINFO_SELLTIME", "GetYear", tostring(yearValue), "GetMonth", formattedMonth, "GetDay", tostring(endSellTimeValue:GetDay())) .. " " .. string.format("%.02d", endSellTimeValue:GetHour()) .. ":" .. string.format("%.02d", endSellTimeValue:GetMinute())
      local remainTime = convertStringFromDatetime(cashProduct:getRemainSellTime())
      if true == disCountSetUse then
        countryKind = remainTime
      else
        countryKind = endSellTime
      end
      slot.discount:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_SELL", "endSellTime", countryKind))
    end
  elseif cashProduct:isApplyDiscount() then
    ...
    local monthValue = tostring(endDiscountTimeValue:GetMonth())
    local formattedMonth = PAGetString(Defines.StringSheet_GAME, "LUA_DATEINFO_MONTH_" .. monthValue)
    ...
    local yearValue = endDiscountTimeValue:GetYear()
    if yearValue < __eInvisibleCashProductPeriodYear then
      local resType = getGameServiceResType()
      if true == isGameTypeTH() or CppEnums.ServiceResourceType.eServiceResourceType_TH == resType then
        yearValue = 543 + yearValue
      end
      local endDiscountTime = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GOODSDETAILINFO_DISCOUNTTIME", "GetYear", tostring(yearValue), "GetMonth", formattedMonth, "GetDay", tostring(endDiscountTimeValue:GetDay())) .. " " .. string.format("%.02d", endDiscountTimeValue:GetHour()) .. ":" .. string.format("%.02d", endDiscountTimeValue:GetMinute())
      local countryKind = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GOODSDETAILINFO_DISCOUNTTIME", "GetYear", tostring(yearValue), "GetMonth", formattedMonth, "GetDay", tostring(endDiscountTimeValue:GetDay())) .. " " .. string.format("%.02d", endDiscountTimeValue:GetHour()) .. ":" .. string.format("%.02d", endDiscountTimeValue:GetMinute())
  ...
  if cashProduct:isApplyDiscount() then
    ...
    slot.priceArrow:SetSpanSize(slot.price:GetSpanSize().x + slot.price:GetTextSizeX(), slot.priceArrow:GetSpanSize().y)
    slot.originalPrice:SetSpanSize(slot.priceArrow:GetSpanSize().x + slot.priceArrow:GetSizeX() + slot.originalPrice:GetTextSizeX(), slot.originalPrice:GetSpanSize().y)
    slot.pearlIcon:SetSpanSize(slot.originalPrice:GetSpanSize().x + slot.pearlIcon:GetSizeX(), slot.pearlIcon:GetSpanSize().y)
  else
    slot.pearlIcon:SetSpanSize(slot.price:GetSpanSize().x + slot.price:GetTextSizeX(), slot.pearlIcon:GetSpanSize().y)
  end
  ...
  local eventTab = inGameShop._currentTab
  if true == self._searchedAll then
    eventTab = PaGlobal_InGameShop_FindTabIndex(cashProduct:getMainCategory())
  end
  if eventTab > 0 and eventTab <= getCashMainCategorySize() and true == FGlobal_IngameCashShopEventCart_IsContentsOpen() then
    local isOnListButton = 0 == evantType or 1 == evantType
    local isOnSlotButton = 0 == evantType or 2 == evantType
    local eventListKey = tabIndexList[eventTab][6]
    local eventSlotKey = tabIndexList[eventTab][7]
    ...
    if eventListKey > 0 and true == isOpenEventList and eventSlotKey > 0 and true == isOpenEventSlot then
      slot.eventCartIcon:SetShow(isOnListButton)
      ...
      if self._currentPos < pos + slot.static:GetSizeY() + self._goodDescBG:GetSizeY() and pos + slot.static:GetSizeY() < self._currentPos + areaSizeY and 1 < self._goodDescBG:GetSizeY() then
        ...
        if 0 < inGameShop._currentTab and inGameShop._currentTab <= getCashMainCategorySize() and true == FGlobal_IngameCashShopEventCart_IsContentsOpen() then
          if 0 < tabIndexList[inGameShop._currentTab][6] and 0 < tabIndexList[inGameShop._currentTab][7] then
            self.desc._btn_BigStyleAlbum:SetSpanSize(142, 60)
            self.desc._btn_BigGift:SetSpanSize(102, 60)
            self.desc._btn_BigECart:SetSpanSize(62, 60)
            self.desc._btn_BigECartSlot:SetSpanSize(22, 60)
          elseif 0 < tabIndexList[inGameShop._currentTab][7] then
            self.desc._btn_BigStyleAlbum:SetSpanSize(102, 60)
            self.desc._btn_BigGift:SetSpanSize(62, 60)
            self.desc._btn_BigECartSlot:SetSpanSize(22, 60)
          elseif 0 < tabIndexList[inGameShop._currentTab][6] then
            self.desc._btn_BigStyleAlbum:SetSpanSize(102, 60)
            self.desc._btn_BigGift:SetSpanSize(62, 60)
            self.desc._btn_BigECart:SetSpanSize(22, 60)
          else
            self.desc._btn_BigStyleAlbum:SetSpanSize(102, 60)
            self.desc._btn_BigGift:SetSpanSize(62, 60)
            self.desc._btn_BigECart:SetSpanSize(22, 60)
        ...
        if false == self.desc._btn_BigGift:GetShow() then
          self.desc._btn_BigStyleAlbum:SetSpanSize(self.desc._btn_BigGift:GetSpanSize().x, self.desc._btn_BigGift:GetSpanSize().y)
        end
        self.desc._btn_BigCart:SetSpanSize(22, 60)
        ...
        self.desc._static_PearlPrice:SetSpanSize(30, 105)
        self.desc._static_PearOriginalPrice:SetSpanSize(self.desc._static_PearlPrice:GetSpanSize().x + self.desc._static_PearlPrice:GetTextSizeX(), 105)
        self.desc._static_PearlIcon:SetSpanSize(self.desc._static_PearOriginalPrice:GetSpanSize().x + self.desc._static_PearOriginalPrice:GetTextSizeX() + 5, 105)
        if true == _ContentsGroup_SelectBox and false == isGameServiceTypeConsole() then
          if true == self.desc._btn_BigGift:GetShow() then
            self.desc._btn_SideDetail:SetSpanSize(self.desc._btn_BigGift:GetSpanSize().x + self.desc._btn_SideDetail:GetSizeX(), 60)
          else
            self.desc._btn_SideDetail:SetSpanSize(self.desc._btn_BigCart:GetSpanSize().x + self.desc._btn_SideDetail:GetSizeX(), 60)
          end
        end
      end
      pos = pos + self._goodDescBG:GetSizeY()
    end
    if slot.eventCartIcon:GetShow() == true then
      slot.name:SetSize(390 - slot.eventCartIcon:GetSizeX() - 10, slot.name:GetSizeY())
    else
      slot.name:SetSize(390, slot.name:GetSizeY())
    end
...
function inGameShop:initTabPos()
  ...
  for ii = 1, self._tabCount do
    ...
    local posY = tabConfig._startY + tabConfig._gapY * (tabPosIndex - 1)
    tab.static:SetPosX(tabConfig._startX)
    tab.static:SetPosY(posY)
    tab.text:SetPosX(40)
    tab.text:SetPosY(5)
    if ii == self._hotDealTabIndex or false == tabIndexList[ii][8] then
...
function inGameShop:SetSearchAll(isSearchAll)
  if nil == inGameShop then
    return
  end
  if nil == Panel_IngameCashShop then
    return
  end
  if false == Panel_IngameCashShop:GetShow() then
    return
  end
  local cashMall = getIngameCashMall()
  if nil == cashMall then
    return
  end
  cashMall:setIsSearchAll(isSearchAll)
end
function inGameShop:GetSearchAll()
  if nil == inGameShop then
    return false
  end
  if nil == Panel_IngameCashShop then
  ...
  return cashMall:getIsSearchAll()
end
function inGameShop:registMessageHandler()
  ...
  if _ContentsGroup_RefundCashProduct == true then
    self._historyTab.static:addInputEvent("Mouse_LUp", "InGameShop_HistoryToggle()")
    self._historyTab.static:addInputEvent("Mouse_On", "InGameShop_ShowSimpleToolTip( true, " .. 2 .. " )")
    self._historyTab.static:addInputEvent("Mouse_Out", "InGameShop_ShowSimpleToolTip( false, " .. 2 .. "  )")
  end
...
function CheckCashProductInEasyPayment(cashProduct)
  if cashProduct:isBuyable() == false then
    return false
  end
  return true
end
function InGameShop_TabEvent(tab)
  ...
  if tab > 0 and tab <= self._tabCount then
    ...
    if _ContentsGroup_RefundCashProduct == true and not self._historyTab.static:IsCheck() and PaGlobal_CashShopBuyHistory_Close ~= nil then
      PaGlobal_CashShopBuyHistory_Close()
    end
  end
  if nil ~= Panel_Window_SelectBox and true == Panel_Window_SelectBox:GetShow() then
...
function PaGlobal_InGameShop_GetTabIndex(categoryType)
  for ii = 1, inGameShop._tabCount do
...
function PaGlobal_InGameShop_GoToTab(categoryType)
  inGameShop:RadioReset()
  inGameShop._tabs[categoryType].static:SetCheck(true)
end
function PaGlobal_InGameShop_FindTabIndex(categoryNum)
  if nil == inGameShop then
    return nil
  end
  for ii = 1, inGameShop._tabCount do
    if nil ~= tabIndexList[ii][2] and tabIndexList[ii][2] == categoryNum then
      return ii
    end
  end
  return nil
...
function InGameShop_CloseHotAndNewList()
  ...
  self._static_ScrollArea:SetPosY(self._radioButton_Sale:GetPosY() + 50)
  self._scroll_IngameCash:SetPosY(self._static_ScrollArea:GetPosY())
  self._static_TopLineBG:SetPosY(self._radioButton_Sale:GetPosY())
  self._edit_Search:SetPosY(self._radioButton_Sale:GetPosY() + 5)
  self._button_Search:SetPosY(self._radioButton_Sale:GetPosY() + 5)
  self._combo_Class:SetPosY(self._radioButton_Sale:GetPosY() + 8)
  local remainingSizeY = _ingameCashShop_SetViewListCount()
  self._scroll_IngameCash:SetSize(self._scroll_IngameCash:GetSizeX(), remainingSizeY * 0.98)
  self._static_ScrollArea:SetSize(self._static_ScrollArea:GetSizeX(), remainingSizeY * 0.98)
  self._static_ScrollArea1:SetPosY(0)
...
function setPosOpenSubTab(tabIndex)
  ...
  for jj = tabIndex + 1, self._tabCount do
    ...
    local posY = lastPosY + tabConfig._gapY * (tabPosIndex - (tabIndex + 1))
    tab.static:SetPosX(tabConfig._startX)
    tab.static:SetPosY(posY)
    tab.text:SetPosX(40)
    tab.text:SetPosY(5)
    if jj == self._hotDealTabIndex or false == tabIndexList[jj][8] then
      tab.static:SetShow(false)
    else
      tab.static:SetShow(true)
      tabPosIndex = tabPosIndex + 1
  ...
  self._myCartTab.static:SetPosY(tabConfig._startY + subTabSize + tabConfig._gapY * (tabId.cart - (2 + self._hideTabCount)))
  if _ContentsGroup_RefundCashProduct == true then
    self._historyTab.static:SetPosX(tabConfig._startX)
    self._historyTab.static:SetPosY(tabConfig._startY + subTabSize + tabConfig._gapY * (tabId.history - (2 + self._hideTabCount)))
  end
...
function setPosCloseSubTab()
  ...
  self:initTabPos()
  self._myCartTab.static:SetPosX(tabConfig._startX)
  self._myCartTab.static:SetPosY(tabConfig._startY + tabConfig._gapY * (tabId.cart - (2 + self._hideTabCount)))
  if _ContentsGroup_RefundCashProduct == true then
    self._historyTab.static:SetPosX(tabConfig._startX)
    self._historyTab.static:SetPosY(tabConfig._startY + tabConfig._gapY * (tabId.history - (2 + self._hideTabCount)))
  end
...
function InGameShop_SubTabEvent(mainTab, subTab)
  ...
  self._currentPos = 0
  self._position = 0
  self._hasMyClassItem = false
  self._scroll_IngameCash:SetControlPos(0)
  self._savedMainTab = mainTab
  self._savedSubTab = subTab
  if self._tabs[mainTab]._subTab[subTab] == nil then
    return
  end
...
function InGameShop_RadioReset()
  inGameShop:RadioReset()
end
function inGameShop:RadioReset()
  ...
  if _ContentsGroup_RefundCashProduct == true then
    self._historyTab.static:SetCheck(false)
  end
end
...
function InGameShop_SetSearchAll(isSearchAll)
  if nil == inGameShop then
    return
  end
  inGameShop:SetSearchAll(isSearchAll)
end
function InGameShop_Search()
  ...
  FGlobal_IngameCashShop_SelectedItemReset()
  InGameShop_SetSearchAll(true)
  self:initData()
  self:update()
  if true == inGameShop:GetSearchAll() then
    inGameShop._searchedAll = true
  end
  InGameShop_SetSearchAll(false)
end
...
function InGameShop_HistoryToggle()
  if _ContentsGroup_RefundCashProduct == false then
    return
  end
  local self = inGameShop
  self._previousTab = self._currentTab
  self._currentTab = tabId.history
  if self._promotionWeb:GetShow() then
  ...
  if _ContentsGroup_RefundCashProduct == true then
    if not self._historyTab.static:IsCheck() then
      _ingameCash_SetTabIconTexture(self._historyTab.icon, tabId.history, 0)
    else
      _ingameCash_SetTabIconTexture(self._historyTab.icon, tabId.history, 2)
    end
    if PaGlobal_CashShopBuyHistory_Close ~= nil then
      PaGlobal_CashShopBuyHistory_Open()
    end
    Panel_IngameCashShop:SetChildIndex(self._historyTab.static, 9900)
  end
  self._scroll_IngameCash:SetShow(false)
  InGameShop_CloseHotAndNewList()
  IngameCashShopEventCart_Close()
  makeSubTab(tabId.history)
end
...
function InGameShop_ShowSimpleToolTip(isShow, buttonType)
  ...
  elseif 1 == buttonType then
    ...
    tabIdx = tabId.cart
  elseif 2 == buttonType and _ContentsGroup_RefundCashProduct == true then
    name = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_HISTORY_TAB_TITLE")
    desc = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_HISTORY_TAB_DESC")
    uiControl = self._historyTab.static
    tabIdx = tabId.history
  end
  if true == isShow then
...
function _ingameCash_SetTabIconTexture(control, tabIdx, status)
  ...
  elseif tabId.history == tabIdx then
    return
  end
...
function FGlobal_InGameShop_OpenByEventAlarm()
  ...
  if curChannelData._isMultiNationSiegeSever == true then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrNoDisabledBySystem"))
    return false
  end
  ...
  local categoryUrl = ""
  local promotionUrl = ""
  local portalUrl = ToClient_getInGamePortalURL()
  if nil ~= portalUrl and "" ~= portalUrl then
    promotionUrl = portalUrl .. "/CashShop/Product"
    categoryUrl = portalUrl .. "/CashShop/Banner"
  else
    promotionUrl, categoryUrl = PaGlobalFunc_InGameCashShop_PromotionBannerUrlOldSet()
  end
...
function PaGlobalFunc_InGameShop_GoLastProduct()
  if true == PaGlobalFunc_SaveNotEnoughMoneyCheck() then
    ToClient_RequestShowProduct(inGameShop._savedProductNo, nil, PaGlobal_IngameCashShop_SavedbValue())
    PaGlobalFunc_SaveNotEnoughMoneyValueCheck(false)
  end
end
function IngameCashShop_PearlBoxFilter(slotNo, itemWrapper, count, inventoryType)
  if itemWrapper:getStaticStatus():isPearlBox() then
    return false
  elseif CppEnums.ContentsEventType.ContentsType_Coupon == itemWrapper:getStaticStatus():getContentsEventType() then
    return false
  else
    return true
  end
end
function PaGlobalFunc_InGameShop_OpenInventory(filter)
  Inventory_SetFunctor(filter, IngameCashShop_PearlBox_Open, nil, nil)
  InventoryWindow_Show(true, true)
end
function IngameCashShop_GuildGiveawayFilter(slotNo, itemWrapper, count, inventoryType)
  if __eContentsType_GuildGiveaway == itemWrapper:getStaticStatus():getContentsEventType() then
    return false
  end
  return true
end
function IngameCashShop_PearlBox_Open(slotNo, itemWrapper, count, inventoryType)
  ...
  if CppEnums.ContentsEventType.ContentsType_Coupon == itemWrapper:getStaticStatus():getContentsEventType() then
    doOpen()
  else
    ...
    if __eContentsType_GuildGiveaway == itemWrapper:getStaticStatus():getContentsEventType() then
      messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_OPEN_PEARLBOX_NOCANCEL")
    end
...
function IngameCashShop_Descinit()
  ...
  self._btn_BigStyleAlbum = UI.getChildControl(inGameShop._goodDescBG, "Button_BigStyleAlbum")
  ...
  self._btn_SideDetail = UI.getChildControl(inGameShop._goodDescBG, "Button_SideDetail")
  self._staticText_GoodsSalesPeriod = UI.getChildControl(inGameShop._goodDescBG, "StaticText_GoodsSalesPeriod")
  self._staticText_PolicyDesc = UI.getChildControl(inGameShop._goodDescBG, "StaticText_PolicyDesc")
  if true == _ContentsGroup_ChinaFontColor then
    self._staticText_PolicyDesc:SetFontColor(UI_color.C_FFFFAD38)
  end
  self._staticText_GuildGiveaway = UI.getChildControl(inGameShop._goodDescBG, "StaticText_GuildRewardItemListTitle")
  self._staticText_Title:SetAutoResize(true)
  self._static_Desc:SetAutoResize(true)
  self._staticText_Title:SetTextMode(__eTextMode_AutoWrap)
  self._static_Desc:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._staticText_GoodsSalesPeriod:SetFontColor(UI_color.C_FFFFEF82)
  ...
  for ii = 0, inGameShop._itemCount - 1 do
    ...
    slot.iconBG:SetPosX(itemConfig._startX + itemConfig._gapX * (ii % inGameShop._config._itemCountPerLine))
    slot.iconBG:SetPosY(itemConfig._startY + math.floor(ii / inGameShop._config._itemCountPerLine) * itemConfig._gapY)
    inGameShop._items[ii] = slot
  end
  for ii = 0, __eGuildGiveawaySelectedItemMaxCount - 1 do
    local slot = {}
    slot.iconBG = UI.createAndCopyBasePropertyControl(inGameShop._goodDescBG, "Static_ItemSlotBG", self._staticText_GuildGiveaway, "GuildGiveawayItem_" .. ii)
    slot.icon = UI.createAndCopyBasePropertyControl(inGameShop._goodDescBG, "Static_ItemSlot", slot.iconBG, "GuildGiveawayItemIcon_" .. ii)
    slot.iconBG:SetPosX(itemConfig._startX + itemConfig._gapX * (ii % inGameShop._config._itemCountPerLine))
    slot.iconBG:SetPosY(itemConfig._startY + math.floor(ii / inGameShop._config._itemCountPerLine) * itemConfig._gapY)
    inGameShop._guildGiveawayItems[ii] = slot
  end
...
function IngameCashShop_DescUpdate()
  ...
  for ii = 0, itemListCount - 1 do
    ...
    slot.icon:SetPosX((slot.iconBG:GetSizeX() - slot.icon:GetSizeX()) / 2)
    slot.icon:SetPosY((slot.iconBG:GetSizeY() - slot.icon:GetSizeY()) / 2)
  ...
  self._staticText_GoodsSalesPeriod:SetText("")
  self._staticText_PolicyDesc:SetText("")
  ...
  self._static_Slot:SetPosX(self._static_SlotBG:GetPosX() + (self._static_SlotBG:GetSizeX() - self._static_Slot:GetSizeX()) / 2)
  self._static_Slot:SetPosY(self._static_SlotBG:GetPosY() + (self._static_SlotBG:GetSizeY() - self._static_Slot:GetSizeY()) / 2)
  ...
  if cashProduct:isApplySellTimeLimit() then
    local startSellTimeValue = PATime(cashProduct:getStartSellTime():get_s64())
    local endSellTimeValue = PATime(cashProduct:getEndSellTime():get_s64())
    local monthValue = tostring(endSellTimeValue:GetMonth())
    local formattedMonth = PAGetString(Defines.StringSheet_GAME, "LUA_DATEINFO_MONTH_" .. monthValue)
    ...
    if yearValue < __eInvisibleCashProductPeriodYear then
      ...
      local endSellTime = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GOODSDETAILINFO_SELLTIME", "GetYear", tostring(yearValue), "GetMonth", formattedMonth, "GetDay", tostring(endSellTimeValue:GetDay())) .. " " .. string.format("%.02d", endSellTimeValue:GetHour()) .. ":" .. string.format("%.02d", endSellTimeValue:GetMinute())
      if true == disCountSetUse then
        endSellTime = convertStringFromDatetime(cashProduct:getRemainSellTime())
      else
        endSellTime = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GOODSDETAILINFO_SELLTIME", "GetYear", tostring(yearValue), "GetMonth", formattedMonth, "GetDay", tostring(endSellTimeValue:GetDay())) .. " " .. string.format("%.02d", endSellTimeValue:GetHour()) .. ":" .. string.format("%.02d", endSellTimeValue:GetMinute())
      end
      self._staticText_GoodsSalesPeriod:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GOODSDETAILINFO_SELLPERIODDESC", "endSellTime", endSellTime))
      local remainDay = calculateDayFromDateDay(cashProduct:getRemainSellTime())
      if 0 == Int64toInt32(remainDay) and false == disCountSetUse then
        local remainStr = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_SELL_REMAINTIME", "remainTime", convertStringFromDatetime(cashProduct:getRemainSellTime()))
        self._staticText_GoodsSalesPeriod:SetFontColor(UI_color.C_FFFF0000)
        self._staticText_GoodsSalesPeriod:SetText(remainStr)
      end
      self._staticText_GoodsSalesPeriod:SetPosY(optionDesc_PosY + descConfig._gapY * descCount)
      descCount = descCount + 1
    end
  end
  if cashProduct:isApplyDiscount() then
    ...
    if yearValue < __eInvisibleCashProductPeriodYear then
      ...
      local startDiscountTime = tostring(startDiscountTimeValue:GetYear()) .. "." .. tostring(startDiscountTimeValue:GetMonth()) .. "." .. tostring(startDiscountTimeValue:GetDay())
      local endSellTime = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GOODSDETAILINFO_DISCOUNTTIME", "GetYear", tostring(yearValue), "GetMonth", formattedMonth, "GetDay", tostring(endDiscountTimeValue:GetDay())) .. " " .. string.format("%.02d", endDiscountTimeValue:GetHour()) .. ":" .. string.format("%.02d", endDiscountTimeValue:GetMinute())
      if true == disCountSetUse then
        endSellTime = convertStringFromDatetime(cashProduct:getRemainDiscountTime())
      else
        endSellTime = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GOODSDETAILINFO_DISCOUNTTIME", "GetYear", tostring(yearValue), "GetMonth", formattedMonth, "GetDay", tostring(endDiscountTimeValue:GetDay())) .. " " .. string.format("%.02d", endDiscountTimeValue:GetHour()) .. ":" .. string.format("%.02d", endDiscountTimeValue:GetMinute())
      end
      self._static_DiscountPeriodDesc:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_GOODSDETAILINFO_DISCOUNTPERIODDESC", "endDiscountTime", endSellTime))
  ...
  if true == cashProduct:isDisplayCashProductPolicy() then
    self._staticText_PolicyDesc:SetTextMode(__eTextMode_AutoWrap)
    self._staticText_PolicyDesc:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_INGAMECASHSHOP_RELAUNCH"))
    self._staticText_PolicyDesc:SetPosY(optionDesc_PosY + descConfig._gapY * descCount)
    descCount = descCount + 1
    if self._staticText_PolicyDesc:GetSizeY() < self._staticText_PolicyDesc:GetTextSizeY() then
      descCount = descCount + 1
    end
  end
  ...
  if true == cashProduct:getGuildGiveawayKey():isDefined() then
    self._staticText_GuildGiveaway:SetPosY(optionDesc_PosY + descConfig._gapY * descCount)
    descCount = descCount + 1
    optionDesc_PosY = optionDesc_PosY + inGameShop._guildGiveawayItems[0].iconBG:GetSizeY() + 20
  else
    self._staticText_GuildGiveaway:SetShow(false)
  end
  if true == _ContentsGroup_SelectBox and false == isGameServiceTypeConsole() then
    local isHaveSelectBoxItem = cashProduct:isHaveSelectBoxItem()
    self._btn_SideDetail:SetShow(isHaveSelectBoxItem)
    if false == isHaveSelectBoxItem and nil ~= Panel_Window_SelectBox and true == Panel_Window_SelectBox:GetShow() then
      PaGlobal_SelectBox:prepareClose()
    end
  else
    self._btn_SideDetail:SetShow(false)
  end
  local addSizeY = 0
  if subItemListCount > inGameShop._config._itemCountPerLine then
    addSizeY = 46
  end
  inGameShop.itemDescDetailSize = optionDesc_PosY + descConfig._gapY * descCount
  inGameShop._maxDescSize = inGameShop.itemDescDetailSize + addSizeY
  local limitType = cashProduct:getCashPurchaseLimitType()
  if UI_CCC.eCashProductCategory_Pearl == cashProduct:getMainCategory() then
  ...
  elseif UI_CCC.eCashProductCategory_Mileage == cashProduct:getMainCategory() then
    ...
    self._btn_BigBuy_C:SetSpanSize(20, 10)
    self._btn_BigBuy_M:SetSpanSize(20, 10)
  elseif UI_CCC.eCashProductCategory_StepUp == cashProduct:getMainCategory() then
    self._btn_BigGift:SetMonoTone(true)
    self._btn_BigCart:SetMonoTone(true)
    self._btn_BigGift:SetEnable(false)
    self._btn_BigCart:SetEnable(false)
  ...
  local isBuyable = cashProduct:isBuyable()
  ...
  if false == isBuyShow and false == isBuyable then
    self._btn_BigBuy:addInputEvent("Mouse_LUp", "InGameShop_BuyConditionGuideMessageOpen(" .. inGameShop._openProductKeyRaw .. ")")
    self._btn_BigBuy_C:addInputEvent("Mouse_LUp", "InGameShop_BuyConditionGuideMessageOpen(" .. inGameShop._openProductKeyRaw .. ")")
    self._btn_BigBuy_M:addInputEvent("Mouse_LUp", "InGameShop_BuyConditionGuideMessageOpen(" .. inGameShop._openProductKeyRaw .. ")")
    self._btn_BigBuy_Silver:addInputEvent("Mouse_LUp", "InGameShop_BuyConditionGuideMessageOpen(" .. inGameShop._openProductKeyRaw .. ")")
  else
    self._btn_BigBuy:addInputEvent("Mouse_LUp", "InGameShop_prepareDescSelectedBuyItem(" .. inGameShop._openProductKeyRaw .. ")")
    self._btn_BigBuy_C:addInputEvent("Mouse_LUp", "InGameShop_prepareDescSelectedBuyItem(" .. inGameShop._openProductKeyRaw .. ")")
    self._btn_BigBuy_M:addInputEvent("Mouse_LUp", "InGameShop_prepareDescSelectedBuyItem(" .. inGameShop._openProductKeyRaw .. ")")
    self._btn_BigBuy_Silver:addInputEvent("Mouse_LUp", "InGameShop_prepareDescSelectedBuyItem(" .. inGameShop._openProductKeyRaw .. ")")
  end
  self._btn_BigGift:addInputEvent("Mouse_LUp", "IngameCashShop_DescSelectedGiftItem(" .. inGameShop._openProductKeyRaw .. ")")
  self._btn_BigCart:addInputEvent("Mouse_LUp", "InGameShop_prepareDescSelectedCartItem(" .. inGameShop._openProductKeyRaw .. ")")
  ...
  if true == _ContentsGroup_SelectBox and false == isGameServiceTypeConsole() then
    self._btn_SideDetail:addInputEvent("Mouse_LUp", "InGameShop_ProcessSideDetailFunc(" .. inGameShop._openProductKeyRaw .. ")")
  end
  if _ContentsOption_CH_GameType == true and UI_CCC.eCashProductCategory_Mileage == cashProduct:getMainCategory() then
    self._btn_BigCart:SetShow(false)
    self._btn_BigGift:SetShow(false)
  end
  if cashProduct:getMainCategory() == CppEnums.CashProductCategory.eCashProductCategory_StepUp then
    self._btn_BigCart:SetShow(false)
    self._btn_BigGift:SetShow(false)
  end
  if UI_CCC.eCashProductCategory_Costumes == cashProduct:getMainCategory() then
    local cashProduct = getIngameCashMall():getCashProductStaticStatusByProductNoRaw(inGameShop._openProductKeyRaw)
    if nil ~= cashProduct and true == cashProduct:getIsEquipItemByIndex(0) then
      self._btn_BigStyleAlbum:SetShow(true)
      self._btn_BigStyleAlbum:addInputEvent("Mouse_LUp", "IngameCashShop_ShowStyleAlbum(" .. inGameShop._openProductKeyRaw .. ")")
    else
      self._btn_BigStyleAlbum:SetShow(false)
    end
  else
    self._btn_BigStyleAlbum:SetShow(false)
  end
  if isJapan == true then
    if self._btn_BigECart:GetShow() == true then
      self._btn_BigGift:SetShow(false)
      self._btn_BigBuy:SetMonoTone(true)
      self._btn_BigBuy:SetIgnore(true)
    else
      self._btn_BigBuy:SetMonoTone(false)
      self._btn_BigBuy:SetIgnore(false)
    end
  elseif self._btn_BigECart:GetShow() == true then
    self._btn_BigBuy:SetMonoTone(true)
    self._btn_BigBuy:SetIgnore(true)
  else
    self._btn_BigBuy:SetMonoTone(false)
    ...
    if nil ~= inGameShop._comboList[ii] then
      ...
      if nil ~= subProduct then
        ...
        if 1 == subProduct:getDisplayFilterKey() or 11 == subProduct:getDisplayFilterKey() then
          ...
          inGameShop._subItemButton[ii].static:SetTextMode(__eTextMode_LimitText)
          ...
          inGameShop._subItemButton[ii + inGameShop._skipCount].static:SetTextMode(__eTextMode_LimitText)
...
function InGameShop_ShowGuildGiveawayItemToolTip(isShow, index)
  local self = inGameShop
  if true == isShow then
    local cashProduct = getIngameCashMall():getCashProductStaticStatusByProductNoRaw(self._openProductKeyRaw)
    local key = cashProduct:getGuildGiveawayKey()
    if false == key:isDefined() then
      return
    end
    local itemEnchantKey = ToClient_getGuildGiveawaySelectedItemEnchantKey(key:get(), index)
    local itemSSW = getItemEnchantStaticStatus(itemEnchantKey)
    if nil ~= itemSSW then
      Panel_Tooltip_Item_Show(itemSSW, self._guildGiveawayItems[index].icon, true, false, nil)
    end
  else
    Panel_Tooltip_Item_hideTooltip()
  end
...
function IngameCashShop_SelectedItem(index, bValue)
  ...
  local prevIndex = -1
  if 4 ~= self._currentTab then
    self._savedSelectedIndex = index
    self._savedSelectedbValue = bValue
    self._savedProductNo = slot.productNoRaw
  end
  if Panel_IngameCashShop_BuyOrGift:GetShow() then
    _InGameShopBuy_Confirm_Cancel()
  end
  ...
  if true == _ContentsGroup_SelectBox and false == isGameServiceTypeConsole() then
    ...
    if nil ~= cashProduct then
      local isHaveSelectBoxItem = cashProduct:isHaveSelectBoxItem()
      if true == isHaveSelectBoxItem and nil ~= Panel_Window_SelectBox and true == Panel_Window_SelectBox:GetShow() and true == self._isClick then
        PaGlobal_SelectBox:prepareClose()
      end
    end
  end
  ...
  local isForcePositionSet
  if true == self._hasMyClassItem and _ContentsOption_CH_GameType == false then
    isForcePositionSet = true
  end
  if true == self._searchedAll then
    local cashProduct = getIngameCashMall():getCashProductStaticStatusByProductNoRaw(slot.productNoRaw)
    if nil ~= cashProduct then
      isForcePositionSet = false
      ...
      if nil ~= tabIndex then
        ...
        if nil == tabIndexList[tabIndex] then
          return
        end
        eventListKey = tabIndexList[tabIndex][6]
        eventSlotKey = tabIndexList[tabIndex][7]
        if (0 < eventListKey or 0 < eventSlotKey) and true == FGlobal_IngameCashShopEventCart_IsContentsOpen() then
          local eventListWrapper = ToClient_GetEventCategoryStaticStatusWrapperByKeyRaw(eventListKey)
          local eventSlotWrapper = ToClient_GetEventCategoryStaticStatusWrapperByKeyRaw(eventSlotKey)
          if nil ~= eventListWrapper or nil ~= eventSlotWrapper then
        ...
        setPosCloseSubTab()
        if nil ~= tabIndex and tabIndex > 0 and tabIndex <= getCashMainCategorySize() then
          local subCategoryCount = getCashMiddleCategorySize(tabIndexList[tabIndex][2])
          if 0 ~= subCategoryCount then
            self._checkTab = true
            setPosOpenSubTab(tabIndex)
        ...
        if nil ~= self._tabs[tabIndex].static then
          Panel_IngameCashShop:SetChildIndex(self._tabs[tabIndex].static, 9900)
        end
        if subTabIndex >= 0 and nil ~= self._tabs[tabIndex] and nil ~= self._tabs[tabIndex]._subTab[subTabIndex] and nil ~= self._tabs[tabIndex]._subTab[subTabIndex].static then
          local selectPosX = self._tabs[tabIndex]._subTab[subTabIndex].static:GetPosX()
          local selectPosY = self._tabs[tabIndex]._subTab[subTabIndex].static:GetPosY()
          self._subTapSelect.static:SetPosX(selectPosX)
          self._subTapSelect.static:SetPosY(selectPosY)
  ...
  IngameCashShop_SelectedItemXXX(slot.productNoRaw, isForcePositionSet, bValue)
  local isEventIconShow = nil ~= slot.eventCartIcon and true == slot.eventCartIcon:GetShow()
  if true == isEventIconShow then
    IngameCashShop_ShowEventIconEffect()
  end
  ...
  if nil ~= cashProduct then
    ...
    if true == cashProduct:getGuildGiveawayKey():isDefined() then
      local key = cashProduct:getGuildGiveawayKey()
      for ii = 0, __eGuildGiveawaySelectedItemMaxCount - 1 do
        local itemEnchantKey = ToClient_getGuildGiveawaySelectedItemEnchantKey(key:get(), ii)
        local itemCount = ToClient_getGuildGiveawaySelectedItemCount(key:get(), ii)
        local itemSSW = getItemEnchantStaticStatus(itemEnchantKey)
        local slot = self._guildGiveawayItems[ii]
        if nil ~= itemSSW then
          slot.icon:ChangeTextureInfoName("Icon/" .. tostring(itemSSW:getIconPath()))
          slot.icon:SetText(tostring(itemCount))
          slot.icon:addInputEvent("Mouse_On", "InGameShop_ShowGuildGiveawayItemToolTip(true, " .. ii .. " )")
          slot.icon:addInputEvent("Mouse_Out", "InGameShop_ShowGuildGiveawayItemToolTip(false, " .. ii .. " )")
          slot.iconBG:SetShow(true)
          slot.icon:SetPosX((slot.iconBG:GetSizeX() - slot.icon:GetSizeX()) / 2)
          slot.icon:SetPosY((slot.iconBG:GetSizeY() - slot.icon:GetSizeY()) / 2)
        else
...
function IngameCashShop_SelectedItemXXX(productNoRaw, isForcePositionSet, bValue)
  ...
  self._hasMyClassItem = false
  local myClassProductNo, myClassItemIdx = IngameCashShop_FindInnerMyClassItem(productNoRaw)
  local offergroup = cashProduct:getOfferGroup()
  if myClassProductNo ~= productNoRaw and -1 ~= myClassItemIdx and 0 ~= myClassProductNo then
    if 0 ~= offergroup then
      InGameShop_subItemEvent(myClassItemIdx)
      for ii = 1, inGameShop._subItemCount do
        if nil ~= inGameShop._subItemButton[ii].static and nil ~= inGameShop._subItemButton[ii] then
          if ii == myClassItemIdx then
            inGameShop._subItemButton[ii].static:SetCheck(true)
          else
            inGameShop._subItemButton[ii].static:SetCheck(false)
          end
    ...
    elseif true == cashProduct:isChooseCash() and false == _isChangeShowWindow then
      InGameShop_ChooseCashView(myClassProductNo)
      _isChangeShowWindow = true
    end
  end
  _isChangeShowWindow = false
...
function IngameCashShop_SelectedItemXXXForScreenShotSNS(productNoRaw, isForcePositionSet, bValue)
  local self = inGameShop
  local cashProduct = getIngameCashMall():getCashProductStaticStatusByProductNoRaw(productNoRaw)
  if nil == cashProduct then
    return
...
function IngameCashShop_CartItem(index)
  ...
  local isMatchedClass = PaGlobalFunc_IngameCashShop_isActualUsableClass(cashProduct)
  local productName = cashProduct:getName()
  if false == isMatchedClass then
    ...
    local messageBoxMemo = "<PAColor0xffd0ee68>[" .. PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_MATHCLASS") .. "]<PAOldColor> \n" .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_CARTITEM_MSGMEMO", "getName", productName)
    messageBoxData = {
...
function InGameShop_prepareDescSelectedCartItem(productNoRaw)
  if nil == Panel_IngameCashShop then
    return
  end
  if nil == inGameShop then
  ...
  local checkNickItem = PaGlobal_InGameCashShop_CheckNickNameItem(productNoRaw)
  if true == checkNickItem then
    return
  end
  IngameCashShop_DescSelectedCartItem(productNoRaw)
end
...
function IngameCashShop_ShowStyleAlbum(productNoRaw)
  if nil == Panel_IngameCashShop then
    return
  end
  ...
  PaGlobal_ScreenShotSNS_Open(productNoRaw)
end
...
function InGameShop_prepareDescSelectedBuyItem(productNoRaw)
  if nil == Panel_IngameCashShop then
    return
  end
  if nil == inGameShop then
  ...
  IngameCashShop_DescSelectedBuyItem(productNoRaw)
end
function InGameShop_ProcessSideDetailFunc(productNoRaw)
  if nil == Panel_IngameCashShop then
    return
  end
  ...
  PaGlobal_ShowSelectBoxInfo_OnlyReadMode(productNoRaw)
end
function IngameCashShop_DescSelectedBuyItem(productNoRaw)
  ...
  if UI_CCC.eCashProductCategory_Pearl == categoryType or UI_CCC.eCashProductCategory_Mileage == categoryType or true == cashProduct:isMoneyPrice() or true == cashProduct:getGuildGiveawayKey():isDefined() then
    IngameCashShopCoupon_Close()
  else
...
function InGameShop_BuyConditionGuideMessageOpen(productNoRaw)
  local messageBoxData = {
    title = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_ALERT"),
    content = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYCONDITIONGUDE_DESC"),
  ...
  if cashProduct:getMainCategory() == CppEnums.CashProductCategory.eCashProductCategory_StepUp then
    messageBoxData.content = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYCONDITION_STEPUP")
  end
  MessageBox.showMessageBox(messageBoxData)
end
...
function InGameShop_Resize()
  ...
  if isGameServiceTypeConsole() == true then
    ...
    tabConfig._startY = self._promotionTab.static:GetPosY()
  end
  self._myCartTab.static:SetPosY(tabConfig._startY + tabConfig._gapY * (tabId.cart - 1))
  if _ContentsGroup_RefundCashProduct == true then
    self._historyTab.static:SetPosY(tabConfig._startY + tabConfig._gapY * (tabId.history - 1))
  end
  self:initTabPos()
  local remainingSizeY = _ingameCashShop_SetViewListCount()
  self._static_GradationBottom:SetPosX(slotConfig._startX)
...
function _ingameCashShop_SetViewListCount()
  ...
  local bannerEndGap = self._radioButton_Sale:GetPosY() - (self._static_PromotionBanner:GetPosY() + self._static_PromotionBanner:GetSizeY())
...
function InGameCashshopDescUpdate(deltaTime)
  ...
  elseif self._isClick then
    if self._goodDescBG:GetSizeY() > 1.5 then
      ...
      self._goodDescBG:SetSize(self._goodDescBG:GetSizeX(), 1)
      if self._isSubItemClick and false == self._hasMyClassItem then
      ...
      elseif true == self._hasMyClassItem then
        self._isSubItemClick = false
      end
    end
  ...
  for _, control in pairs(inGameShop.desc) do
    local checkSize = control:GetPosY() + control:GetSizeY()
    local isShow = checkSize > 0 and checkSize < self._goodDescBG:GetSizeY()
...
function InGameShop_Open()
  ...
  if nil ~= PaGloabl_IntroMovie_GetIsIntroMoviePlay and true == PaGloabl_IntroMovie_GetIsIntroMoviePlay() then
    return
  end
  local curChannelData = getCurrentChannelServerData()
  if curChannelData._isMultiNationSiegeSever == true then
...
function InGameShop_OpenActual()
  ...
  if not isGameServiceTypeConsole() then
    FGlobal_SetCandidate()
    self._categoryWeb:SetUrl(self._categoryWeb:GetSizeX(), self._categoryWeb:GetSizeY(), categoryUrl, false, isTaiwanNation)
    self._categoryWeb:SetShow(false)
    self._categoryWeb:SetIME()
  ...
  if isGameServiceTypeConsole() == true then
    ...
    if temporaryWrapper ~= nil then
      ...
      if 1 ~= userType and 2 ~= userType then
        self:clearSubCategoryForFirstOpen()
      end
    end
  else
    self:clearSubCategoryForFirstOpen()
  ...
  inGameShop:SetSearchAll(false)
  inGameShop._searchedAll = false
  if true == _ContentsGroup_RenewUI and false == ToClient_isOnceOpenCouponUI() then
    local count = ToClient_GetCouponInfoUsableCount()
    if count > 0 and nil ~= PaGlobal_PearlShopCoupon_Open then
      PaGlobal_PearlShopCoupon_Open()
      ToClient_setOnceOpenCouponUI(true)
    end
  end
  if isGameServiceTypeConsole() == false then
    self:RadioReset()
...
function InGameShop_GoToCostumeTab()
  if true == _ContentsGroup_RenewUI_PearlShop and true == _ContentsGroup_XB_Obt then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_NOTAVAILABLE_NOW"))
    return false
  end
...
function inGameShop:clearSubCategoryForFirstOpen()
  if isFirstRespone == true then
    for ii = 1, self._tabCount do
      ...
      for jj = 1, subCategoryCount do
        ...
        if false == ToClient_isUseDynamicBSS(__eDynamicBSSType_CashProduct) then
          getIngameCashMall():setCashProductNoRawFilterList()
        else
          getIngameCashMall():setDynamicBssFilterList()
        end
        if getIngameCashMall():getCashProductFilterListCount() <= 0 or getIngameCashMall():isExistStepUpProduct() == true then
...
function PaGlobal_InGameCashShop_CheckClose()
  if not inGameShop:isClearToClose() then
    return
  end
  local OpenMoonBlessing = function()
    HandleEventLUp_IngameCashShopMoonBlessing_All_Open()
  end
  if nil ~= PaGlobalFunc_IngameCashShopMoonBlessing_All_CheckMileageRewarded then
    PaGlobalFunc_IngameCashShopMoonBlessing_All_CheckMileageRewarded()
  end
  if true == inGameShop._isCheckedMoonBlessing then
    local messageBoxTitle = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_ALERT")
    local messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_CHECKMOONBLESSING")
    local messageBoxData = {
      title = messageBoxTitle,
      content = messageBoxMemo,
      functionYes = OpenMoonBlessing,
      functionNo = InGameShop_Close,
      priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
    }
    MessageBox.showMessageBox(messageBoxData)
  else
...
function PaGlobal_InGameCashShop_SetCheckMoonBlessing(tempCnt)
  if tempCnt > 0 then
    inGameShop._isCheckedMoonBlessing = true
  else
    inGameShop._isCheckedMoonBlessing = false
  end
end
function InGameShop_Close()
  ...
  inGameShop._savedNotEnoughMoney = false
  PaGlobalFunc_FullScreenFade_RunAfterFadeIn(InGameShop_CloseActual)
  if _ContentsGroup_RefundCashProduct == true then
    inGameShop._historyTab.static:SetCheck(false)
    if PaGlobal_CashShopBuyHistory_Close ~= nil then
      PaGlobal_CashShopBuyHistory_Close()
    end
  end
...
function InGameShop_CloseActual()
  ...
  elseif Panel_Dialog_ServantList_All ~= nil and Panel_Dialog_ServantList_All:GetShow() == true then
  ...
  if true == isInvenOpen and false == _ContentsGroup_RenewUI then
  ...
  if false == _ContentsGroup_RemasterUI_Main_Alert and false == _ContentsGroup_UsePadSnapping then
  ...
  if nil ~= Panel_Npc_Dialog_All and Panel_Npc_Dialog_All:GetShow() then
  ...
  self._isCheckedMoonBlessing = false
  ClearFocusEdit()
  if _ContentsGroup_RenewUI_PearlShop then
  ...
  if nil ~= PaGlobal_ScreenShotSNS_Close then
...
function ToClient_RequestShowProductForSceenShotSnS(productNo, price, bValue)
  local self = inGameShop
  ...
  if nil ~= cashProduct then
    ...
    IngameCashShop_SelectedItemXXXForScreenShotSNS(productNo, true, bValue)
    for ii = 1, inGameShop._listComboCount do
      if nil ~= inGameShop._subItemButton[ii] and inGameShop._subItemButton[ii].productNo == productNo then
        inGameShop._cashProductIndex = ii
      end
    end
...
function ToClient_RequestShowProduct(productNo, price, bValue)
  ...
  if nil ~= cashProduct then
    ...
    inGameShop._cashProductNoData = productNo
    inGameShop._isClick = false
    IngameCashShop_SelectedItemXXX(productNo, true, bValue)
    for ii = 1, inGameShop._listComboCount do
      if inGameShop._subItemButton[ii].productNo == productNo then
        inGameShop._cashProductIndex = ii
      end
    end
    inGameShop._subItemButton[1].static:SetCheck(false)
...
function InGameShop_MoneyIcon_Tooltip(isShow, tipType)
  ...
  elseif 1 == tipType then
    ...
    if CppEnums.CountryType.KOR_ALPHA == getGameServiceType() or CppEnums.CountryType.KOR_REAL == getGameServiceType() then
      desc = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMESHOP_MONEYICON_TOOLTIP_MILEAGE_DESC_KR")
    else
...
function PaGlobal_IngameCashShop_OtherPanels_Close()
  ...
  if nil ~= PaGlobal_FriendList_All_Close then
  ...
  if true == _ContentsGroup_NewUI_XXX then
    if nil ~= Panel_ItemWarp_All and Panel_ItemWarp_All:GetShow() then
      Panel_ItemWarp_All_Close()
    end
    if nil ~= Panel_Change_Nickname_All and Panel_Change_Nickname_All:GetShow() then
      HandleEventLUp_ChangeNickNameAll_Close()
    end
  elseif nil ~= Panel_ItemWarp and Panel_ItemWarp:GetShow() then
    Panel_ItemWarp_Close()
  end
end
function PaGlobalFunc_InGameCashShop_PromotionBannerUrlOldSet()
  local category_url = ""
  local promotion_url = ""
  local SALangType = "pt"
  if UI_SERVICE_RESOURCE.eServiceResourceType_ES == getGameServiceResType() then
  ...
  if isGameTypeKorea() then
    if CppEnums.CountryType.KOR_ALPHA == getGameServiceType() then
      promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_DEV_URL_PROMOTIONURL")
      category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_DEV_URL_CATEGORYURL")
    elseif CppEnums.CountryType.KOR_REAL == getGameServiceType() then
      promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL")
      category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL")
    end
  elseif isGameTypeTaiwan() then
    if CppEnums.CountryType.TW_ALPHA == getGameServiceType() then
      promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_TW_ALPHA")
      category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_TW_ALPHA")
    elseif CppEnums.CountryType.TW_REAL == getGameServiceType() then
      promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_TW")
      category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_TW")
    end
  elseif _ContentsOption_CH_GameType == true then
    promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_KR2")
    category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_KR2")
  elseif CppEnums.CountryType.SA_ALPHA == getGameServiceType() then
    promotion_url = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_SA_ALPHA", "lang", SALangType)
    category_url = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_SA_ALPHA", "lang", SALangType)
  elseif CppEnums.CountryType.SA_REAL == getGameServiceType() then
    promotion_url = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_SA", "lang", SALangType)
    category_url = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_SA", "lang", SALangType)
  elseif CppEnums.CountryType.TR_ALPHA == getGameServiceType() then
    promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_TR_ALPHA")
    category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_TR_ALPHA")
  elseif CppEnums.CountryType.TR_REAL == getGameServiceType() then
    promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_TR")
    category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_TR")
  elseif CppEnums.CountryType.TH_ALPHA == getGameServiceType() then
    promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_TH_ALPHA")
    category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_TH_ALPHA")
  elseif CppEnums.CountryType.TH_REAL == getGameServiceType() then
    promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_TH")
    category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_TH")
  elseif CppEnums.CountryType.ID_ALPHA == getGameServiceType() then
    promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_ID_ALPHA")
    category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_ID_ALPHA")
  elseif CppEnums.CountryType.ID_REAL == getGameServiceType() then
    promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_ID")
    category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_ID")
  elseif CppEnums.CountryType.RUS_ALPHA == getGameServiceType() then
    promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_RUS_ALPHA")
    category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_RUS_ALPHA")
  elseif CppEnums.CountryType.RUS_REAL == getGameServiceType() then
    promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_RUS")
    category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_RUS")
  elseif CppEnums.CountryType.JPN_ALPHA == getGameServiceType() then
    promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_JP_ALPHA")
    category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_JP_ALPHA")
  elseif CppEnums.CountryType.JPN_REAL == getGameServiceType() then
    promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL_JP")
    category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL_JP")
  else
    promotion_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_PROMOTIONURL")
    category_url = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_URL_CATEGORYURL")
  end
  return promotion_url, category_url
end
function IngameCashShop_FindInnerMyClassItem(productNoRaw)
  local cashProduct = getIngameCashMall():getCashProductStaticStatusByProductNoRaw(productNoRaw)
  local selfPlayer = getSelfPlayer()
  if nil ~= cashProduct and nil ~= selfPlayer then
    do
      ...
      if 0 ~= offergroup then
        count = getIngameCashMall():getCashProductStaticStatusGroupListCount(offergroup)
        for ii = 0, count - 1 do
          local offerCashProduct = getIngameCashMall():getCashProductStaticStatusGroupByIndex(offergroup, ii)
          if nil ~= offerCashProduct and IngameCashShop_filterData(offerCashProduct) and true == IngameCashShop_FindInnerMyClassInnerItem(offerCashProduct:getNoRaw()) then
            inGameShop._hasMyClassItem = true
            return offerCashProduct:getNoRaw(), ii + 1
          end
        end
      elseif true == cashProduct:isChooseCash() then
        local validChooseCashProduct = cashProduct:chooseCashCount()
        for ii = 0, validChooseCashProduct - 1 do
          local chooseCashProduct = cashProduct:getChooseCashByIndex(ii)
          if nil ~= chooseCashProduct and true == chooseCashProduct:isClassTypeUsable(myClass) then
            inGameShop._hasMyClassItem = true
            return chooseCashProduct:getNoRaw(), ii + 1
          end
        end
      end
    end
...
function IngameCashShop_FindInnerMyClassInnerItem(productNoRaw)
  local cashProduct = getIngameCashMall():getCashProductStaticStatusByProductNoRaw(productNoRaw)
  local selfPlayer = getSelfPlayer()
  if nil ~= cashProduct and nil ~= selfPlayer then
    local itemCount = cashProduct:getInnerItemListCount()
    local itemType
    local listClass = {}
    for itemIdx = 0, itemCount - 1 do
      local itemSSW = cashProduct:getInnerItemByIndex(itemIdx)
      if nil ~= itemSSW then
        itemType = itemSSW:getItemType()
        if __eEquipSlotNoLantern == itemSSW:getEquipSlotNo() then
        elseif itemSSW:isUsableServant() then
        elseif itemSSW:isEquipable() and true == itemSSW:get()._usableClassType:isOn(selfPlayer:getClassType()) then
          return true
        end
      else
      end
...
function IngameCashShop_ShowEventIconEffect()
  if false == ToClient_IsContentsGroupOpen("379") then
    return
  end
  if nil == Panel_IngameCashShop_EventCart or false == Panel_IngameCashShop_EventCart:GetShow() then
    return
  end
  PaGlobalFunc_IngameCashShop_EventCart_ShowEffect()
end
function PaGlobalFunc_IngameCashShop_CheckAppliedNickNameChange()
  if nil == Panel_IngameCashShop then
    return false
  end
...
function PaGlobalFunc_IngameCashShop_isChangeNickItemByCashProduct(cashProduct)
  if nil == Panel_IngameCashShop then
    return false
  end
  if nil == cashProduct then
...
function PaGlobal_InGameCashShop_CheckNickNameItem(productNoRaw)
  ...
  local isAppliedNickNameChange = PaGlobalFunc_IngameCashShop_CheckAppliedNickNameChange()
  if true == isAppliedNickNameChange then
    return false
  end
  local isChangeNickItem = PaGlobalFunc_IngameCashShop_isChangeNickItemByCashProduct(cashProduct)
...
function PaGlobalFunc_IngameCashShop_isActualUsableClass(cashProduct)
  if nil == Panel_IngameCashShop then
    return false
  end
  if nil == cashProduct then
  ...
  if true == cashProduct:isChooseCash() then
    cashProduct = cashProduct:getChooseCashByIndex(selectedIndex)
    if nil == cashProduct then
      return
    end
  end
  local cashProductItemCount = cashProduct:getItemListCount()
  local subCashProductItemCount = cashProduct:getSubItemListCount()
  if cashProductItemCount <= 0 and subCashProductItemCount <= 0 then
    return false
  end
  local itemEnchantSSW = cashProduct:getItemByIndex(0)
  local subitemEnchantSSW = cashProduct:getSubItemByIndex(0)
  if itemEnchantSSW == nil and subitemEnchantSSW == nil then
    return false
  end
  local itemSS, subItemSS
  if itemEnchantSSW ~= nil then
    itemSS = itemEnchantSSW:get()
  end
  if subitemEnchantSSW ~= nil then
    subItemSS = subitemEnchantSSW:get()
  end
  if itemSS == nil and subItemSS == nil then
    return false
  end
  local selfPlayer = getSelfPlayer()
  if nil == selfPlayer then
  ...
  local isMatchClass = false
  local isHasClassFilter = cashProduct:doHaveDisplayClass()
  local isShowClassFilter = cashProduct:isClassTypeUsable(selfPlayer:getClassType())
  local isUseableClass = false
  if itemSS ~= nil then
    isUseableClass = itemSS._usableClassType:isOn(selfPlayer:getClassType())
  end
  if subItemSS ~= nil then
    isUseableClass = subItemSS._usableClassType:isOn(selfPlayer:getClassType())
  end
  if false == isHasClassFilter then
    isMatchClass = true
  elseif true == isShowClassFilter and true == isUseableClass then
    isMatchClass = true
  end
  if false == isMatchClass then
    isMatchClass = IngameCashShop_FindInnerMyClassInnerItem(cashProduct:getNoRaw())
  end
  return isMatchClass
end
function PaGlobal_IsOpenSelectBoxContents()
  if false == _ContentsGroup_SelectBox then
    return false
  end
  if true == isGameServiceTypeConsole() then
  ...
  if true == PaGlobal_GetShowSelectBoxPanel() then
    return true
  end
  return false
end
function PaGlobal_IngameCashShop_SavedSelectIndex()
  if nil == inGameShop._savedSelectedIndex then
    return
  end
  return inGameShop._savedSelectedIndex
end
function PaGlobal_IngameCashShop_SavedbValue()
  if nil == inGameShop._savedSelectedbValue then
    return
  end
  return inGameShop._savedSelectedbValue
end
function PaGlobal_IngameCashShop_SavedTabIndex()
  if nil == inGameShop._savedTabIndex then
    return
  end
  return inGameShop._savedTabIndex
end
function PaGlobal_IngameCashShop_SavedWingTabIndex()
  local rv = inGameShop._savedMainTab
  return rv
end
function PaGlobal_InGameCashShop_OpenByIndex(mainTabIndex, subTabIndex)
  if mainTabIndex ~= nil then
    if InGameShop_OpenActual() == false then
      return
    end
    InGameShop_RadioReset()
    PaGlobal_InGameShop_GoToTab(mainTabIndex)
    InGameShop_TabEvent(mainTabIndex)
  end
  if subTabIndex ~= nil then
    InGameShop_SubTabEvent(mainTabIndex, subTabIndex)
  end
end
