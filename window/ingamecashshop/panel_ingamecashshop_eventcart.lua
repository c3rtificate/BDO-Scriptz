...
local isEventCartOpen = _ContentsGroup_CashShopEventCart
...
local EventCart = {
  ...
  stc_consoleKeyGuideBg = UI.getChildControl(Panel_IngameCashShop_EventCart, "Static_Console_KeyPad"),
  _effectCtrl = UI.getChildControl(Panel_IngameCashShop_EventCart, "Static_effect"),
  stc_eventCartIcon = nil,
  txt_eventCartEmpty = nil,
  maxSaleTxt = nil,
  maxSaleTxt = nil,
  nextSaleTxt = nil,
  ...
  _panelOriginHalf = Panel_IngameCashShop_EventCart:GetSizeY() * 0.5
}
...
function IngameCashShopEventCart_Init()
  ...
  self.isConsole = _ContentsGroup_UsePadSnapping
  if false == _ContentsGroup_PearlShopGift then
  end
  ...
  self.stc_eventCartIcon = UI.getChildControl(self._list2, "Static_EventIcon")
  self.txt_eventCartEmpty = UI.getChildControl(self._list2, "StaticText_Empty_Info")
  ...
  self.cautionTxt:SetTextMode(__eTextMode_AutoWrap)
  ...
  self.keyGuideConfirm = UI.getChildControl(self.stc_consoleKeyGuideBg, "StaticText_Console_A")
  self.keyGuideViewPercent = UI.getChildControl(self.stc_consoleKeyGuideBg, "StaticText_Console_X")
  self.keyGuideClose = UI.getChildControl(self.stc_consoleKeyGuideBg, "StaticText_Console_B")
  self.keyGuideDelete = UI.getChildControl(self.stc_consoleKeyGuideBg, "StaticText_Console_Y")
  self.keyGuideResetAll = UI.getChildControl(self.stc_consoleKeyGuideBg, "StaticText_Console_LT")
  self.keyGuideGift = UI.getChildControl(self.stc_consoleKeyGuideBg, "StaticText_Console_RT")
  self.keyGuideGift:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_BUYORGIFT_DOGIFT"))
  if true == self.isConsole then
    Panel_IngameCashShop_EventCart:SetSize(Panel_IngameCashShop_EventCart:GetSizeX(), Panel_IngameCashShop_EventCart:GetSizeY() - self.btn_Confirm:GetSizeY() - 5)
  end
  ...
  IngameCashShopEventCart_PadKeyguideSet()
  IngameCashShopEventCart_registEventHandler()
end
function IngameCashShopEventCart_registEventHandler()
  local self = EventCart
  registerEvent("FromClient_buyCompleteCashShopEventCart", "FromClient_buyCompleteCashShopEventCart")
  if true == self.isConsole then
    Panel_IngameCashShop_EventCart:registerPadEvent(__eConsoleUIPadEvent_A, "IngameCashShopEventCart_Buy()")
    Panel_IngameCashShop_EventCart:registerPadEvent(__eConsoleUIPadEvent_LTPress_Y, "IngameCashShopEventCart_Clear( true )")
    Panel_IngameCashShop_EventCart:registerPadEvent(__eConsoleUIPadEvent_X, "IngameCashShopEventCart_ShowPointToggle()")
  else
  ...
  self._list2:registEvent(__ePAUIList2EventType_LuaChangeContent, "IngameCashShopEventCart_ListUpdate")
...
function ToolBgSet()
  ...
  local wTextSizeX = 0
  local hTextSizeY = 0
  local txt_tooltipTitle = UI.getChildControl(self.point_Tooltip, "StaticText_PointTitle")
  for ii = 0, size - 1 do
    local eventText = {}
    if nil == self.point_TooltipList[ii] then
      ...
      eventText:SetPosY(gabY + ii * eventText:GetTextSizeY())
      eventText:SetShow(true)
      self.point_TooltipList[ii] = eventText
      hTextSizeY = eventText:GetTextSizeY()
      if 0 == wTextSizeX or wTextSizeX < eventText:GetTextSizeX() then
        wTextSizeX = eventText:GetTextSizeX()
      end
    else
      hTextSizeY = self.point_TooltipList[ii]:GetTextSizeY()
      if 0 == wTextSizeX or wTextSizeX < self.point_TooltipList[ii]:GetTextSizeX() then
        wTextSizeX = self.point_TooltipList[ii]:GetTextSizeX()
      end
    end
  end
  self.point_Tooltip:SetSize(wTextSizeX + gabX, gabY + size * hTextSizeY)
  txt_tooltipTitle:SetSize(self.point_Tooltip:GetSizeX(), txt_tooltipTitle:GetSizeY())
  txt_tooltipTitle:ComputePos()
  return true
end
function PaGlobal_Event_Point_Tooltip(isShow)
  ...
  local gabX = 10
  local setPosX = -1 * (point_TooltipSizeX + gabX)
  local setPosY = 0
  if eventCartPosX - point_TooltipSizeX < 0 then
    setPosX = eventCartSizeX + gabX
  end
  if screenSizeX < eventCartPosX + eventCartSizeX then
    setPosX = -1 * (point_TooltipSizeX + gabX)
  end
  ...
  self.point_Tooltip:SetSpanSize(setPosX, self.point_Tooltip:GetSpanSize().y)
  self.point_Tooltip:SetPosY(setPosY)
  self.point_Tooltip:SetShow(isShow)
end
function IngameCashShopEventCart_ShowPointToggle()
  local self = EventCart
  if nil == self.point_Tooltip then
    return
  end
  if true == self.point_Tooltip:GetShow() then
    PaGlobal_Event_Point_Tooltip(false)
  else
    PaGlobal_Event_Point_Tooltip(true)
  end
end
function IngameCashShopEventCart_ListUpdate(contents, key)
  local self = EventCart
  local idx = Int64toInt32(key)
  local stc_Bg = UI.getChildControl(contents, "Static_BG")
  ...
  if nil ~= cashProduct then
    ...
    if cashProduct:isApplyDiscount() == true then
      txt_Price:SetText(makeDotMoney(cashProduct:getDiscountPrice()))
    else
    ...
    btn_Delete:SetShow(false == _ContentsGroup_RenewUI)
    btn_Delete:addInputEvent("Mouse_LUp", "IngameCashShopEventCart_Delete(" .. idx .. ")")
    if true == self.isConsole then
      stc_Bg:registerPadEvent(__eConsoleUIPadEvent_Y, "IngameCashShopEventCart_Delete(" .. idx .. ")")
    end
  end
end
...
function IngameCashShopEventCart_Update(productNoRaw, index)
  ...
  elseif true == Panel_PearlShop_ProductBuy:GetShow() then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantInsertOrDeleteEventCartItemOnPopUp"))
    return false
  end
  local self = EventCart
  ...
  if Panel_IngameCashShop_EventCart:IsShow() then
    ...
    IngameCashShopEventCart_CheckListEmpty()
  end
end
...
function IngameCashShopEventCart_Open(eventKey, eventSlotKey)
  local self = EventCart
  if not isEventCartOpen then
    return
  end
  if false == _ContentsGroup_RenewUI then
    if false == Panel_IngameCashShop:IsShow() then
      return
    end
  elseif false == Panel_PearlShop:GetShow() then
    return
  end
  if 0 == eventKey and 0 == eventSlotKey then
    return
  end
  local listOpen = true
  local slotOpen = true
  local eventListWrapper = ToClient_GetEventCategoryStaticStatusWrapperByKeyRaw(eventKey)
...
function IngameCashShopEventCart_CheckListEmpty()
  local self = EventCart
  local cartSize = Int64toInt32(EventCart._list2:getElementManager():getSize())
  if cartSize < 0 or 0 == cartSize then
    self.stc_eventCartIcon:SetShow(true)
    self.txt_eventCartEmpty:SetShow(true)
  else
    self.stc_eventCartIcon:SetShow(false)
    self.txt_eventCartEmpty:SetShow(false)
  end
end
function IngameCashShopEventCart_PadKeyguideSet()
  local self = EventCart
  if false == self.isConsole then
    self.stc_consoleKeyGuideBg:SetShow(false)
    self.btn_Confirm:SetShow(true)
    self.btn_Cancle:SetShow(true)
    self.btn_X:SetShow(true)
    self.point_Btn_Question:SetShow(true)
  else
    self.stc_consoleKeyGuideBg:SetShow(true)
    self.stc_consoleKeyGuideBg:ComputePos()
    self.btn_Confirm:SetShow(false)
    self.btn_Cancle:SetShow(false)
    self.btn_X:SetShow(false)
    self.point_Btn_Question:SetShow(false)
    self.keyGuideGift:SetShow(false)
    local keyguide = {
      self.keyGuideResetAll,
      self.keyGuideClose,
      self.keyGuideViewPercent,
      self.keyGuideConfirm,
      self.keyGuideDelete
    }
    PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyguide, self.stc_consoleKeyGuideBg, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP)
  end
end
function IngameCashShopEventCart_Close()
  local self = EventCart
  if true == Panel_IngameCashShop_EventCart:IsShow() then
    Panel_IngameCashShop_EventCart:EraseAllEffect()
...
function IngameCashShopEventCart_UpdatePrice()
  ...
  if true == isSellinPeriod and true == isDiscountPeriod then
    ...
    self.nextSaleTxt:SetTextMode(__eTextMode_AutoWrap)
...
function FGlobal_IngameCashShopEventCart_Confirm(currentTab)
  local self = EventCart
  getIngameCashMall():requestBuyItemByEventCart(-1, 0, self.eventKey[currentTab], currentTab, true)
end
...
function PaGlobalFunc_IngameCashShop_EventCart_ShowEffect()
  if nil == EventCart._effectCtrl then
    return
  end
  EventCart._effectCtrl:EraseAllEffect()
  EventCart._effectCtrl:ComputePos()
  EventCart._effectCtrl:AddEffect("fUI_CashShop_01A", false, 0, EventCart._panelOriginHalf + 3)
end
