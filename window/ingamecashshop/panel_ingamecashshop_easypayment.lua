...
function PaGlobal_EasyBuy:Init()
  ...
  registerEvent("FromClient_NotifyPearlCount", "PaGlobal_EasyBuy_MyPearlUpdate")
  registerEvent("FromClient_NotifyBuyCashItemForMe", "PaGlobal_EasyBuy_MyPearlUpdate")
  self._ui._list2:changeAnimationSpeed(10)
  self._ui._list2:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobal_EasyBuy_Update")
...
function PaGlobal_EasyBuy_MyPearlUpdate()
...
function PaGlobal_EasyBuy:Open(uniqueNo, waypointKey, immediatelyUse, tempCashItemSSW)
  if isGameTypeGT() then
    return
  end
  if false == _ContentsGroup_EasyBuy then
    return
  end
  ...
  if Panel_IngameCashShop_EasyPayment:GetShow() and false == Panel_Dialog_ServantInfo_All:GetShow() and nil ~= Panel_Dialog_ServantInfo_All then
