local pearlShopProductInfo = {
  ...
  _originFramePosY = 0,
  _originBottomControlPosY = 0,
  _originTitleControlPosY = 0,
  _keyGuideAlign = {},
  _backendNeedUpdateItemKeyList = Array.new()
}
function pearlShopProductInfo:initialize()
  ...
  self._originMainBgSizeY = self._ui._mainBgControl:GetSizeY()
  self._ui._frame = UI.getChildControl(self._ui._mainBgControl, "Frame_Tooltip")
  self._originFramePosY = self._ui._frame:GetPosY()
  ...
  self._ui._bannerControl = UI.getChildControl(self._ui._content, "Static_ProductBanner")
  self._ui._titleControl = UI.getChildControl(self._ui._content, "StaticText_ProductName")
  self._originTitleControlPosY = self._ui._titleControl:GetPosY()
  ...
  self._ui._effectDescControl = UI.getChildControl(self._ui._content, "StaticText_EffectDesc")
  self._ui._effectDescControl:SetTextMode(__eTextMode_AutoWrap)
  self._ui._extractDescControl = UI.getChildControl(self._ui._content, "StaticText_ExtractDesc")
  self._ui._extractDescControl:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._originBottomControlPosY = self._ui._bottomControl:GetPosY()
...
function pearlShopProductInfo:open(productInfo, chooseProduct)
  if not productInfo then
    return
  end
  self._productNoRaw = productInfo:getNoRaw()
  self._isChooseProduct = chooseProduct
  self:focus(0)
  _AudioPostEvent_SystemUiForXBOX(8, 14)
  self._panel:SetShow(true)
end
function PaGlobalFunc_PearlShopProductInfoOpen(productInfo, chooseProduct)
  pearlShopProductInfo:open(productInfo, chooseProduct)
end
function pearlShopProductInfo:close()
  PaGlobalFunc_PearlShopSetBKeyGuide()
  pearlShopProductInfo._backendNeedUpdateItemKeyList = Array.new()
...
function pearlShopProductInfo:focus(controlIndex)
  ...
  if _ContentsGroup_ItemBackEndLoading == true then
    for idx = 1, #pearlShopProductInfo._backendNeedUpdateItemKeyList do
      local savedInfo = pearlShopProductInfo._backendNeedUpdateItemKeyList[idx]
      if savedInfo ~= nil then
        local forcusedIndex = savedInfo._focusedIndex
        if forcusedIndex ~= self._focusedIndex then
          table.remove(pearlShopProductInfo._backendNeedUpdateItemKeyList, idx)
        end
      end
    end
  end
...
function pearlShopProductInfo:addItemSlotControlList(count)
  local itemSlotGroup = UI.getChildControl(self._ui._mainBG, "Static_ItemSlotGroup")
  local slotOriginalPosY = self._ui._itemSlotBgTemplateControl:GetPosY()
...
function pearlShopProductInfo:update()
  ...
  for i = 0, self._ui._itemSlotControlListSize - 1 do
    ...
    if showFlag then
      ...
      if i == self._focusedIndex then
        ...
        if true == self._isChooseProduct then
          self._ui._bannerControl:SetShow(true)
          self._ui._bannerControl:ChangeTextureInfoName(info:getPackageIcon())
        else
          self._ui._bannerControl:SetShow(false)
        end
        self._ui._titleControl:SetText(focusedItemInfo:getName())
        local itemdesc = ""
        if "" ~= focusedItemInfo:getDescription() then
          itemdesc = focusedItemInfo:getDescription()
        else
          itemdesc = PAGetString(Defines.StringSheet_GAME, "GAME_MESSAGE_WAIT_PROCESS")
          if _ContentsGroup_ItemBackEndLoading == true then
            local infoList = {
              _focusedIndex = self._focusedIndex,
              _itemEnchantKey = itemInfo:get()._key:get()
            }
            pearlShopProductInfo._backendNeedUpdateItemKeyList:push_back(infoList)
          end
        end
        self._ui._descControl:SetText(itemdesc)
        self._ui._effectDescControl:SetText(focusedItemInfo:getEnchantDescription())
        valksCount = focusedItemInfo:getExtractionCount_s64()
        cronsCount = focusedItemInfo:getCronCount_s64()
        if focusedItemInfo:get():isItemInstallation() then
          local interiorPoint = focusedItemInfo:getCharacterStaticStatus():getObjectStaticStatus():getInteriorSensPoint()
          if 0 ~= interiorPoint then
            ...
            self._ui._descControl:SetText(interiorPointStr .. self._ui._descControl:GetText())
          end
        end
      end
    end
  ...
  local addBannerSizeY = 0
  if true == self._isChooseProduct then
    addBannerSizeY = self._ui._bannerControl:GetSizeY() + 10
  end
  self._ui._titleControl:SetPosY(self._originTitleControlPosY + addBannerSizeY)
  ...
  self._ui._content:SetSize(self._ui._content:GetSizeX(), self._ui._effectDescControl:GetPosY() + self._ui._effectDescControl:GetSizeY() + 10)
  self._ui._bottomControl:ComputePos()
  if self._ui._frame:GetSizeY() < self._ui._content:GetSizeY() then
  ...
  self._ui._frame:SetPosY(self._originFramePosY + plusPosY)
  self._ui._bottomControl:SetPosY(self._originBottomControlPosY + plusPosY)
  self._ui._mainBgControl:SetSize(self._ui._mainBgControl:GetSizeX(), self._originMainBgSizeY + plusPosY)
  ...
  self._ui._vScroll:SetInterval(self._ui._content:GetSizeY() / 100 * 1.1)
...
function FromClient_PearlShopProductInfoInit()
  ...
  if _ContentsGroup_ItemBackEndLoading == true then
    registerEvent("FromClient_ItemBackEndLoadComplete", "FromClient_PearlShopProduct_BackEndLoadComplete")
  end
end
function FromClient_PearlShopProduct_BackEndLoadComplete(loadedItemKey, loadedEnchantLevel)
  if _ContentsGroup_ItemBackEndLoading == false then
    return
  end
  if Panel_PearlShop_ProductInfo:GetShow() == false then
    return
  end
  if pearlShopProductInfo._backendNeedUpdateItemKeyList == nil then
    return
  end
  ...
  for idx = 1, #pearlShopProductInfo._backendNeedUpdateItemKeyList do
    ...
    if savedInfo ~= nil then
      ...
      local focusedIndex = savedInfo._focusedIndex
      if focusedIndex == pearlShopProductInfo._focusedIndex and loadedEnchantKey:get() == savedItemkey then
        local itemESSW = getItemEnchantStaticStatus(loadedEnchantKey)
        if itemESSW ~= nil then
          pearlShopProductInfo:update()
          table.remove(pearlShopProductInfo._backendNeedUpdateItemKeyList, idx)
        end
      else
        table.remove(pearlShopProductInfo._backendNeedUpdateItemKeyList, idx)
