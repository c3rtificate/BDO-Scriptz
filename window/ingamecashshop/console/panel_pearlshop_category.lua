local pearlShopCategory = {
  ...
  _ui = {
    ...
    _bottomControl = nil,
    _bottomKeyGuideBg = nil,
    _selectControl = nil,
    _couponControl = nil,
    _rateViewControl = nil,
    _pearlControl = nil,
...
if _ContentsGroup_RefundCashProduct == true then
  pearlShopCategory._ui._buyHistroyIndex = pearlShopCategory._ui._mainCategoryControlListSize + 1
end
local isOn = false
function pearlShopCategory:initMainCategory()
  ...
  if _ContentsGroup_RefundCashProduct == true then
    local control = UI.cloneControl(template, group, "Static_MainMenu" .. self._ui._mainCategoryControlListSize)
    local buttonControl = UI.getChildControl(control, "RadioButton_MainMenu_Category")
    local title = PAGetString(Defines.StringSheet_GAME, "LUA_INGAMECASHSHOP_HISTORY_TAB_TITLE")
    tempControlList[self._ui._mainCategoryControlListSize] = control
    buttonControl:addInputEvent("Mouse_LUp", "PaGlobalFunc_PearlShopCategorySelectMainCategory(" .. self._ui._mainCategoryControlListSize .. ")")
    buttonControl:addInputEvent("Mouse_On", "PaGlobalFunc_PearlShopCategoryFocusMainCategory(" .. self._ui._mainCategoryControlListSize .. ")")
    buttonControl:addInputEvent("Mouse_Out", "PaGlobalFunc_PearlShopCategoryFocusOutMainCategory(" .. self._ui._mainCategoryControlListSize .. ")")
    buttonControl:ChangeTextureInfoTextureIDAsync("IngameCashShop_CashCategory_Icon_Normal", 0)
    buttonControl:ChangeTextureInfoTextureIDAsync("IngameCashShop_CashCategory_Icon_Over", 1)
    buttonControl:ChangeTextureInfoTextureIDAsync("IngameCashShop_CashCategory_Icon_Click", 2)
    buttonControl:setRenderTexture(buttonControl:getBaseTexture())
    buttonControl:SetText(" " .. title)
    local sortInfo = {}
    sortInfo._index = self._ui._mainCategoryControlListSize
    sortInfo._displayOrder = self._ui._buyHistroyIndex
    if nil ~= sortInfo._displayOrder then
      table.insert(indexList, sortInfo)
    end
    self._ui._buyHistroyControl = control
  end
...
function pearlShopCategory:initBannerArea()
  ...
  for ii = 0, #self._ui.stc_bannerBGs do
    self._ui.web_banners[ii] = UI.createControl(__ePAUIControl_WebControl, self._ui.stc_bannerBGs[ii], "web_Banner" .. ii)
...
function pearlShopCategory:updateKeyGuideAlign()
...
function pearlShopCategory:initialize()
  ...
  self:initBannerArea()
  self._ui._bottomControl = UI.getChildControl(self._panel, "Static_BottomBg")
  self._ui._bottomKeyGuideBg = UI.getChildControl(self._ui._bottomControl, "Static_KeyGuide_Bg")
  self._ui._selectControl = UI.getChildControl(self._ui._bottomKeyGuideBg, "StaticText_Select_ConsoleUI")
  self._ui._exitControl = UI.getChildControl(self._ui._bottomKeyGuideBg, "StaticText_Exit_ConsoleUI")
  self._ui._couponControl = UI.getChildControl(self._ui._bottomKeyGuideBg, "StaticText_Coupon_ConsoleUI")
  self._ui._rateViewControl = UI.getChildControl(self._ui._bottomKeyGuideBg, "StaticText_LBX_ConsoleUI")
  local pearlBgControl = UI.getChildControl(self._ui._bottomControl, "Static_MoneyType2BG")
  self._ui._pearlControl = UI.getChildControl(pearlBgControl, "StaticText_MoneyType_Price1")
  local loyaltyBgControl = UI.getChildControl(self._ui._bottomControl, "Static_MoneyType2BG_2")
  ...
  self._keyGuideAlign:push_back(self._ui._rateViewControl)
  self._keyGuideAlign:push_back(self._ui._couponControl)
  self._keyGuideAlign:push_back(self._ui._selectControl)
  self._keyGuideAlign:push_back(self._ui._exitControl)
...
function pearlShopCategory:back()
  ...
  self:close()
  InGameShop_Close()
  if nil ~= Panel_Window_DailyStamp_All and self._isDailyStampShow then
    self._isDailyStampShow = false
    PaGlobalFunc_DailyStamp_All_Open(false)
  end
end
...
function pearlShopCategory:focusMainCategory(index)
  if true == _ContentsGroup_CashShopEventCart and self._selectedMainCategoryIndex ~= index then
...
function pearlShopCategory:selectMainCategory(index)
  if self:checkValidMainCategory(index) or _ContentsGroup_RefundCashProduct == true and self._selectedMainCategoryIndex == self._ui._mainCategoryControlListSize then
  else
    return
  end
  if 0 < getCashMiddleCategorySize(index + 1) then
...
function PaGlobalFunc_PearlShopCategory_GetMainCategoryIndex()
  if not pearlShopCategory:checkValidMainCategory(pearlShopCategory._selectedMainCategoryIndex) then
    return nil
  end
  return pearlShopCategory._selectedMainCategoryIndex
end
...
function pearlShopCategory:gotoNextStep(productNo64)
  if not self:checkValidMainCategory(self._selectedMainCategoryIndex) then
    if _ContentsGroup_RefundCashProduct == true and self._selectedMainCategoryIndex == self._ui._mainCategoryControlListSize then
  ...
  if isGameServiceTypeConsole() and 0 == self._selectedMainCategoryIndex then
...
function pearlShopCategory:updateSubCategory()
  ...
  if not self:checkValidMainCategory(self._selectedMainCategoryIndex) then
    if _ContentsGroup_RefundCashProduct == true and self._selectedMainCategoryIndex == self._ui._mainCategoryControlListSize then
      ...
      mainGrupFocusBG:SetPosY(self._ui._buyHistroyControl:GetPosY() - 2)
    end
    return
  end
  ...
  if subCategorySize > 0 then
    if nil ~= PaGlobalFunc_PearlShopCheckShow and false == PaGlobalFunc_PearlShopCheckShow() then
...
function pearlShopCategory:showXXX(flag)
  ...
  self._ui._bottomControl:SetSize(getScreenSizeX(), self._ui._bottomControl:GetSizeY())
  ...
  if true == _ContentsGroup_Console_WebBanner then
    ...
    if ToClient_isUsingProductManagerAPI() == true then
      if nil == domainURL or "" == domainURL then
        domainURL = "https://dev-game-portal.xbox.playblackdesert.com/Banner?bannerType=1"
      else
        domainURL = "https://" .. domainURL .. "/Console/Game/InGame/PositionBanner?bannerType=1"
      end
    ...
    if true == initIndexFlag then
      ...
      if temporaryWrapper ~= nil then
        ...
        if userType == __eEAdmissionSpeedChannelType_Return or userType == __eEAdmissionSpeedChannelType_Newbie then
          self._selectedMainCategoryIndex = 9
          if userType == __eEAdmissionSpeedChannelType_Return then
            self._selectedSubCategoryIndex = 1
          else
            self._selectedSubCategoryIndex = 0
          end
          getIngameCashMall():setCurrentCategory(self._selectedMainCategoryIndex + 1)
          getIngameCashMall():setCurrentSubTab(self._selectedSubCategoryIndex + 1)
          PaGlobalFunc_PearlShop_ClassFilter_Hidden()
          self:showXXX(false)
          PaGlobalFunc_PearlShopOpen(true)
          showBanner = false
        end
      end
      ...
      self._bannerIsReady = {}
    elseif self._selectedMainCategoryIndex == 0 then
      local mainGroup = UI.getChildControl(self._panel, "Static_MainMenuGroup")
      local mainGrupFocusBG = UI.getChildControl(mainGroup, "Static_FocusBG")
      mainGrupFocusBG:SetShow(false)
      self._ui._subCategoryGroupControl:SetShow(false)
    end
    self._ui.stc_bannerArea:SetShow(showBanner)
...
function pearlShopCategory:requestProductByProductNo(productNo64)
  ...
  if nil ~= cashProductWrapper then
    ...
    if not isAdultPeople and isAdultProduct then
      ...
      if true == _ContentsGroup_CashShopEventCart and self._selectedMainCategoryIndex ~= cashProductWrapper:getMainCategory() - 1 then
        IngameCashShopEventCart_Clear()
      end
