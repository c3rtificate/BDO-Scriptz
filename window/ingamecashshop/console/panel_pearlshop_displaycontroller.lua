...
function pearlShopDisplayController:setCharacterEquip()
  ...
  for itemIdx = 0, itemCount - 1 do
    ...
    if itemSSW:isUsableServant() then
      ...
      if itemSSW:get():isServantTypeUsable(CppEnums.ServantKind.Type_Ship, 0) or itemSSW:get():isServantTypeUsable(CppEnums.ServantKind.Type_Raft, 0) then
        servantType = CppEnums.ServantKind.Type_Ship
      elseif itemSSW:get():isServantTypeUsable(CppEnums.ServantKind.Type_TwoWheelCarriage, 0) or itemSSW:get():isServantTypeUsable(CppEnums.ServantKind.Type_FourWheeledCarriage, 0) then
        servantType = CppEnums.ServantKind.Type_FourWheeledCarriage
      elseif itemSSW:get():isServantTypeUsable(CppEnums.ServantKind.Type_Horse, usableType) then
        servantType = CppEnums.ServantKind.Type_Horse
      elseif itemSSW:get():isServantTypeUsable(CppEnums.ServantKind.Type_FishingBoat, 0) then
        servantType = CppEnums.ServantKind.Type_FishingBoat
      elseif itemSSW:get():isServantTypeUsable(CppEnums.ServantKind.Type_Camel, 0) then
        servantType = CppEnums.ServantKind.Type_Camel
      elseif itemSSW:get():isServantTypeUsable(CppEnums.ServantKind.Type_Donkey, usableType) then
        servantType = CppEnums.ServantKind.Type_Donkey
      elseif itemSSW:get():isServantTypeUsable(CppEnums.ServantKind.Type_PersonalTradeShip, 0) then
        servantType = CppEnums.ServantKind.Type_PersonalTradeShip
      end
  ...
  if true == hasEquipment and false == hasUsableServant then
    ...
    if isSocialAction == true then
      local socialActionCPSSW = ToClient_GetCashProductStaticStatusWrapperByKeyRaw(self._productNoRaw)
  ...
  elseif true == hasUsableServant then
    ...
    if servantType == CppEnums.ServantKind.Type_Horse then
      if self._productNoRaw == 102090 then
        characterKeyRaw = 9889
        getIngameCashMall():clearEquipViewList()
      elseif self._productNoRaw == 102091 then
        characterKeyRaw = 9891
        getIngameCashMall():clearEquipViewList()
      elseif self._productNoRaw == 102092 or self._productNoRaw == 102093 then
        characterKeyRaw = 31539
        getIngameCashMall():clearEquipViewList()
      elseif self._productNoRaw == 102094 then
        characterKeyRaw = 9888
        getIngameCashMall():clearEquipViewList()
      elseif self._productNoRaw == 102095 then
        characterKeyRaw = 31557
        getIngameCashMall():clearEquipViewList()
      elseif self._productNoRaw == 102096 then
        characterKeyRaw = 9887
        getIngameCashMall():clearEquipViewList()
      elseif self._productNoRaw == 102097 then
...
function pearlShopDisplayController:showAvailableMenu()
  ...
  if self._categoryNo_Outfit == self._curMainCategory or true == self._isAvatarProduct then
    local isAwakenWeaponContentsOpen = PaGlobalFunc_Util_GetIsAwakenWeaponContentsOpen(self._nowSetClass)
    if nil == isAwakenWeaponContentsOpen then
      isAwakenWeaponContentsOpen = false
    end
    self._ui.stc_outfitBtnList[self.OUTFIT_BUTTON_TYPE.IS_AWAKEN]:SetShow(isAwakenWeaponContentsOpen)
    if true == isAwakenWeaponContentsOpen then
      ...
      local btnSizeX = self._ui.stc_outfitBtnList[self.OUTFIT_BUTTON_TYPE.POSE_BATTLE]:GetSizeX()
      self._ui.stc_outfitBtnA:SetPosX(self._ui.stc_outfitBtnList[self.OUTFIT_BUTTON_TYPE.POSE_BATTLE]:GetPosX() + btnSizeX)
    end
