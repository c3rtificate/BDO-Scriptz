...
function PearlCharge:update()
  ...
  for productIdx = 0, productCount - 1 do
    ...
    if product ~= nil then
      ...
      slot.price:SetText(makeDotMoney(product:getProductPrice()))
...
function _IngameCashShop_PearlCharge_ConfirmDO()
  local self = PearlCharge
  local cashProduct = getIngameCashMall():getCashProductStaticStatusByProductNoRaw(self.selected_productNoRaw)
  ToClient_requestBuyItem(self.selected_productNoRaw, 1, cashProduct:getProductPrice(), CppEnums.BuyItemReqTrType.eBuyItemReqTrType_UiUpdate, toInt64(0, -1), 0, CppEnums.ItemWhereType.eInventory, false, CppEnums.ItemWhereType.eCount, __eTInventorySlotNoUndefined)
end
