...
local CashShopSetEquip = {
  ...
  BTN_Percent = UI.getChildControl(Panel_IngameCashShop_SetEquip, "Button_Percent"),
  SlotUIPool = {},
  beforProductNoRaw = -1,
  nowProductNoRaw = -1,
  beforSetClass = -1,
...
function CashShopSetEquip:Initialize()
  ...
  for index, value in pairs(self.SetEquipSlotNo) do
    local tempSlot = {}
    local CreateItemBG = UI.createControl(__ePAUIControl_Static, Panel_IngameCashShop_SetEquip, "CashShop_SetEquip_ItemBG_" .. value)
    ...
    local CreateItemName = UI.createControl(__ePAUIControl_StaticText, CreateItemBG, "CashShop_SetEquip_ItemName_" .. value)
    ...
    local CreateBTN_UnSet = UI.createControl(__ePAUIControl_Button, CreateItemBG, "CashShop_SetEquip_UnSetBTN_" .. value)
  ...
  if ToClient_isConsole() == true then
    self.BTN_Percent:SetShow(false)
  else
    if self.BTN_Percent:GetSizeX() < self.BTN_Percent:GetTextSizeX() then
      self.BTN_Percent:SetSize(self.BTN_Percent:GetTextSizeX() + 15, self.BTN_Percent:GetSizeY())
    end
    if self.BTN_QNA:GetShow() == true then
      self.BTN_Percent:SetPosX(self.BTN_QNA:GetPosX() - self.BTN_Percent:GetSizeX() - 10)
    else
      self.BTN_Percent:SetPosX(self.BTN_QNA:GetPosX() - self.BTN_Exit:GetSizeX() - 10)
    end
    self.BTN_Percent:SetShow(true)
  end
end
...
function CashShopSetEquip:Return_ServantType(itemSSW, servantType)
  local returnValue
  local usable = itemSSW:getUsableServantSubType()
  if CppEnums.ServantKind.Type_Ship == servantType then
    returnValue = itemSSW:get():isServantTypeUsable(CppEnums.ServantKind.Type_Ship, 0) or itemSSW:get():isServantTypeUsable(CppEnums.ServantKind.Type_Raft, 0)
  elseif CppEnums.ServantKind.Type_FourWheeledCarriage == servantType then
    returnValue = itemSSW:get():isServantTypeUsable(CppEnums.ServantKind.Type_TwoWheelCarriage, 0) or itemSSW:get():isServantTypeUsable(CppEnums.ServantKind.Type_FourWheeledCarriage, 0)
  elseif CppEnums.ServantKind.Type_Carrack == servantType then
    returnValue = itemSSW:get():isServantTypeUsable(servantType, usable)
  else
    returnValue = itemSSW:get():isServantTypeUsable(servantType, usable)
  end
  return returnValue
...
function FGlobal_CashShop_SetEquip_Update(productNoRaw)
  ...
  getIngameCashMall():setSocialActionView(false)
  ...
  local isSocialAction = false
  local usableServantKind = 1
  for itemIdx = 0, itemCount - 1 do
    local itemSSW = CPSSW:getInnerItemByIndex(itemIdx)
    ...
    elseif itemSSW:isUsableServant() then
      ...
      elseif self:Return_ServantType(itemSSW, CppEnums.ServantKind.Type_PersonalTradeShip) then
        self.hasServantType = CppEnums.ServantKind.Type_PersonalTradeShip
      elseif self:Return_ServantType(itemSSW, CppEnums.ServantKind.Type_Carrack) then
        self.hasServantType = CppEnums.ServantKind.Type_Carrack
        usableServantKind = itemSSW:getUsableServantSubType()
      end
    elseif itemSSW:isEquipable() then
      ...
      for classType, _ in pairs(listClass) do
        ...
        if false == itemSSW:isBlackDesertClassType(classType) then
          listClass[classType] = nil
        end
      end
    elseif true == itemSSW:isCustomizedItemBox() then
      local customizedItemKey = CustomizedItemKey(itemSSW:getContentsEventParam1())
      local customizedItemSSW = ToClient_getCustomizedItemStaticStatusWrapper(customizedItemKey)
      if nil ~= customizedItemSSW then
        local matchedItemSSW = customizedItemSSW:getMatchedItem()
        if nil ~= matchedItemSSW then
          hasEquipment = true
        end
      end
    else
      if itemSSW:getContentsEventType() == __eContentsType_SocialAction then
        hasEquipment = true
        isSocialAction = true
        listClass[myClass] = true
      else
      end
    end
  end
  ...
  if (true ~= hasEquipment or false ~= hasLantern) and true == hasUsableServant then
    ...
    if self.hasServantType == CppEnums.ServantKind.Type_Carrack then
      characterKeyRaw = 31054 + (usableServantKind - 1)
      getIngameCashMall():clearEquipViewList()
    end
    if self.hasServantType == CppEnums.ServantKind.Type_Horse then
      if productNoRaw == 102090 then
        characterKeyRaw = 9889
        getIngameCashMall():clearEquipViewList()
      elseif productNoRaw == 102091 then
        characterKeyRaw = 9891
        getIngameCashMall():clearEquipViewList()
      elseif productNoRaw == 102092 or productNoRaw == 102093 then
        characterKeyRaw = 31539
        getIngameCashMall():clearEquipViewList()
      elseif productNoRaw == 102094 then
        characterKeyRaw = 9888
        getIngameCashMall():clearEquipViewList()
      elseif productNoRaw == 102095 then
        characterKeyRaw = 31557
        getIngameCashMall():clearEquipViewList()
      elseif productNoRaw == 102096 then
        characterKeyRaw = 9887
        getIngameCashMall():clearEquipViewList()
      elseif productNoRaw == 102097 then
        characterKeyRaw = 31575
        getIngameCashMall():clearEquipViewList()
      end
    end
    CashShopController_ForceOffAllButton()
  ...
  elseif true == hasEquipment and false == hasLantern then
    self.beforSetClass = self.nowSetClass
    self.nowSetClass = checkClass
    local doEquipProcess = true
    if self.beforSetClass == -1 and self.nowSetClass == -1 then
      doEquipProcess = false
    elseif self.beforSetClass == self.nowSetClass then
    ...
    if isSocialAction == true then
      local socialActionCPSSW = ToClient_GetCashProductStaticStatusWrapperByKeyRaw(productNoRaw)
      if socialActionCPSSW == nil then
        return
      end
      getIngameCashMall():setSocialAction(socialActionCPSSW)
    end
    if doEquipProcess == true then
...
function CashSHop_IsUnderWearItem(productNoRaw)
  ...
  for key = 0, count - 1 do
    ...
    if __eEquipSlotNoAvatarUnderwear == itemType then
      return true
    end
  end
  return false
...
function CashShop_SetEquip_AutoUpHair(productNoRaw)
  ...
  for key = 0, count - 1 do
    ...
    local itemType = itemSSW:getEquipSlotNo()
    if __eEquipSlotNoFaceDecoration1 == itemType or __eEquipSlotNoFaceDecoration2 == itemType or __eEquipSlotNoFaceDecoration3 == itemType then
      isUpHairMode = true
      break
    end
  end
...
function HandleClicked_QNAWebLink_Open()
  if _ContentsOption_CH_GameType == true then
    PaGlobal_URL_OpenChCsCenter(Defines.ChCsCenterOpenType.CsCenterOpen_PearlShop)
  else
    FGlobal_QnAWebLink_Open()
  end
end
function HandleClicked_PercentWebLink_Open()
...
function CashShopSetEquip:registEventHandler()
  ...
  self.BTN_Percent:addInputEvent("Mouse_LUp", "HandleClicked_PercentWebLink_Open()")
...
local CashShopController = {
  ...
  btn_GuildGiveaway = UI.getChildControl(Panel_IngameCashShop_Controller, "Button_GuildReward"),
...
function CashShopController:Initialize()
  ...
  if ToClient_isAdultUser() == false or _ContentsOption_CH_NoUnderWear == true then
    self.btn_AllDoff:SetPosX(self.Slider_Endurance:GetPosX() - 50 + self.btn_AllDoff:GetSizeX() / 2)
  ...
  local spanGabX = 5
  if self.btn_Mileage:GetSizeX() < self.btn_Mileage:GetTextSizeX() + spanGabX then
    self.btn_Mileage:SetTextHorizonLeft()
    self.btn_Mileage:SetTextMode(__eTextMode_AutoWrap)
    self.btn_Mileage:SetTextSpan(spanGabX, 0)
    self.btn_Mileage:SetText(self.btn_Mileage:GetText())
  end
  self:updateButtonImageAwakenWeapon()
end
...
function CashShopController:Open()
  ...
  PaGlobalFunc_IngameCashShopSetEquip_RightTopBtnAlign()
  ...
  CashShopController.btn_OpenHelm:SetMonoTone(false)
  self.isShowUI = true
...
function CashShopController:Close()
  ...
  self.BTN_EyeSee:SetIgnore(false)
  self.BTN_EyeSee:SetMonoTone(false, false)
end
...
function FGlobal_CashShop_SetEquip_SelectedItem(productNoRaw)
  ...
  self.txt_petLookNameMain:SetTextMode(__eTextMode_AutoWrap)
  self.txt_petLookNameSub:SetTextMode(__eTextMode_AutoWrap)
...
function CashShopController:SetPosition()
  ...
  self.btn_GuildGiveaway:ComputePos()
...
function HandleClicked_CashShopController_ToggleOpenHelm()
  local self = CashShopController
  if true == CashShopController.btn_WarStance:IsCheck() then
    self.btn_OpenHelm:SetCheck(not self.btn_OpenHelm:IsCheck())
    return
  end
...
function HandleClicked_CashShopController_ToggleWarStance()
  ...
  if true == isChecked then
    getIngameCashMall():setIsShowBattleHelmet(true)
    CashShopController.btn_OpenHelm:SetMonoTone(true)
  else
    CashShopController.btn_OpenHelm:SetMonoTone(false)
    getIngameCashMall():setIsShowBattleHelmet(CashShopController.btn_OpenHelm:IsCheck())
  end
end
...
function PaGlobalFunc_IngameCashShopSetEquip_MileageBtnEffect(rewardCnt)
  CashShopController.btn_Mileage:EraseAllEffect()
  if rewardCnt > 0 then
    CashShopController.btn_Mileage:AddEffect("fUI_MoonBless_01A", true, 0, 0)
  end
end
function PaGlobalFunc_IngameCashShopSetEquip_GuildRewardBtnEffect(rewardCnt)
  CashShopController.btn_GuildGiveaway:EraseAllEffect()
  if rewardCnt > 0 then
    CashShopController.btn_GuildGiveaway:AddEffect("fUI_MoonBless_01A", true, 0, 0)
  else
  end
end
function PaGlobalFunc_IngameCashShopSetEquip_RightTopBtnAlign()
  if nil == Panel_IngameCashShop_Controller then
    return
  end
  local btnList = {
    [1] = CashShopController.btn_Coupon,
    [2] = CashShopController.btn_FirstIgnore,
    [3] = CashShopController.btn_SaleGoods,
    [4] = CashShopController.btn_Mileage,
    [5] = CashShopController.btn_GuildGiveaway
  }
  if true == _ContentsGroup_PearlShopMileage and true == PaGlobal_CashMileage_IsOpenCheck() then
    CashShopController.btn_Mileage:SetShow(true)
  else
    CashShopController.btn_Mileage:SetShow(false)
  end
  if true == _ContentsGroup_GuildGiveaway then
    CashShopController.btn_GuildGiveaway:SetShow(getSelfPlayer():get():isGuildMember())
  else
    CashShopController.btn_GuildGiveaway:SetShow(false)
  end
  local specialMoveCheck = CashShopController.btn_SpecialMove:IsCheck()
  local sunShineCheck = CashShopController.SunIcon:IsCheck()
  local spanSize = {x = 10, y = 90}
  if sunShineCheck or specialMoveCheck then
    spanSize.y = spanSize.y + CashShopController.btn_SpecialMove1:GetSizeY() + 5
  end
  for ii = 1, #btnList do
    if nil ~= btnList[ii] and true == btnList[ii]:GetShow() then
      btnList[ii]:SetSpanSize(spanSize.x, spanSize.y)
      spanSize.y = spanSize.y + btnList[ii]:GetSizeY() + 5
    end
  end
end
...
function _cashShopController_ButtonTooltip(isShow, buttonType)
  ...
  elseif buttonType == 8 then
    if CashShopSetEquip.nowSetClass == __eClassType_ShyWaman then
...
function HandleClicked_SpecialMoveSetting(moveType)
  ...
  CashShopController.BTN_EyeSee:SetCheck(false)
  CashShopController.BTN_EyeSee:SetIgnore(true)
  CashShopController.BTN_EyeSee:SetMonoTone(true, true)
...
function FGlobal_SpecialMoveSettingCheck()
  ...
  self.BTN_EyeSee:SetIgnore(false)
  self.BTN_EyeSee:SetMonoTone(false, false)
  self.BTN_EyeSee:SetCheck(self.BTN_EyeSee:IsCheck())
  PaGlobalFunc_IngameCashShopSetEquip_RightTopBtnAlign()
end
...
function CashShopController:updateButtonImageAwakenWeapon()
  local classType = CashShopSetEquip.nowSetClass
  if classType < 0 then
    classType = getSelfPlayer():getClassType()
  end
  if __eClassType_ShyWaman == classType then
    self.btn_AwakenWeapon:ChangeTextureInfoName("Renewal/PcRemaster/Remaster_Btn_01.dds")
    local x1, y1, x2, y2 = setTextureUV_Func(self.btn_AwakenWeapon, 263, 1, 295, 33)
    self.btn_AwakenWeapon:getBaseTexture():setUV(x1, y1, x2, y2)
    self.btn_AwakenWeapon:ChangeOnTextureInfoName("Renewal/PcRemaster/Remaster_Btn_01.dds")
    local x1, y1, x2, y2 = setTextureUV_Func(self.btn_AwakenWeapon, 296, 1, 328, 33)
    self.btn_AwakenWeapon:getOnTexture():setUV(x1, y1, x2, y2)
    self.btn_AwakenWeapon:ChangeClickTextureInfoName("Renewal/PcRemaster/Remaster_Btn_01.dds")
    local x1, y1, x2, y2 = setTextureUV_Func(self.btn_AwakenWeapon, 329, 1, 361, 33)
    self.btn_AwakenWeapon:getClickTexture():setUV(x1, y1, x2, y2)
  else
    self.btn_AwakenWeapon:ChangeTextureInfoName("Renewal/PcRemaster/Remaster_Btn_00.dds")
    local x1, y1, x2, y2 = setTextureUV_Func(self.btn_AwakenWeapon, 202, 242, 234, 274)
    self.btn_AwakenWeapon:getBaseTexture():setUV(x1, y1, x2, y2)
    self.btn_AwakenWeapon:ChangeOnTextureInfoName("Renewal/PcRemaster/Remaster_Btn_00.dds")
    local x1, y1, x2, y2 = setTextureUV_Func(self.btn_AwakenWeapon, 235, 242, 267, 274)
    self.btn_AwakenWeapon:getOnTexture():setUV(x1, y1, x2, y2)
    self.btn_AwakenWeapon:ChangeClickTextureInfoName("Renewal/PcRemaster/Remaster_Btn_00.dds")
    local x1, y1, x2, y2 = setTextureUV_Func(self.btn_AwakenWeapon, 268, 242, 300, 274)
    self.btn_AwakenWeapon:getClickTexture():setUV(x1, y1, x2, y2)
  end
  if true == getIngameCashMall():getAwakenWeaponView() then
    self.btn_AwakenWeapon:setRenderTexture(self.btn_AwakenWeapon:getClickTexture())
  else
    self.btn_AwakenWeapon:setRenderTexture(self.btn_AwakenWeapon:getBaseTexture())
  end
end
function CashShopController:registEventHandler()
  ...
  self.btn_Mileage:addInputEvent("Mouse_LUp", "HandleEventLUp_IngameCashShopMoonBlessing_All_Open()")
  if true == _ContentsGroup_GuildGiveaway then
    self.btn_GuildGiveaway:addInputEvent("Mouse_LUp", "PaGlobal_IngameCashshop_GuildGiveaway_Open()")
  end
end
function CashShopController:registMessageHandler()
  ...
  registerEvent("FromClient_ChangeCashshopBattle", "FromClient_ChangeCashshopBattle")
end
