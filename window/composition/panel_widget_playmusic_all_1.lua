...
function PaGlobal_PlayMusic_All:initialize()
  ...
  self._ui._playTime = UI.getChildControl(Panel_Widget_PlayMusic_All, "StaticText_PlayTime")
...
function PaGlobal_PlayMusic_All:prepareOpen()
  ...
  self:resize()
  self:setPos()
  Panel_Widget_PlayMusic_All:RegisterUpdateFunc("PaGlobal_PlayMusic_All_RemainTime")
  if 0 < PaGlobal_MusicSetting_All._playTime then
    local endSellTimeValue = PATime(PaGlobal_MusicSetting_All._playTime)
    local timeStr = string.format("%.02d", endSellTimeValue:GetMinute()) .. ":" .. string.format("%.02d", endSellTimeValue:GetSecond())
    self._ui._playTime:SetText(timeStr)
    self._ui._playTime:SetShow(true)
  else
    self._ui._playTime:SetShow(false)
  end
...
function PaGlobal_PlayMusic_All:prepareClose()
  ...
  Panel_Widget_PlayMusic_All:ClearUpdateLuaFunc()
...
function PaGlobal_PlayMusic_All:PlayMusic_RemainTime(deltaTime)
  PaGlobal_MusicSetting_All._playTime = PaGlobal_MusicSetting_All._playTime - deltaTime
  local endSellTimeValue = PATime(PaGlobal_MusicSetting_All._playTime)
  local timeStr = string.format("%.02d", endSellTimeValue:GetMinute()) .. ":" .. string.format("%.02d", endSellTimeValue:GetSecond())
  self._ui._playTime:SetText(timeStr)
end
