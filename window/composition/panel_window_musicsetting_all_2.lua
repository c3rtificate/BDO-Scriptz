function PaGlobal_MusicSetting_All_ShowTooltip(index, trackType)
  ...
  TooltipSimple_Show(button, PAGetString(Defines.StringSheet_GAME, "AUDIO_MIDI_INSTRUMENT_" .. trackType), nil, false)
end
...
function PaGlobal_MusicSetting_All_List2Update(content, key)
  ...
  for ii = 1, 2 do
    ...
    if nil ~= characterList[ii] then
      characterData = getCharacterDataByIndex(characterList[ii], __ePlayerCreateNormal)
    end
...
function PaGlobal_MusicSetting_All_ResembleRequest()
  ...
  PaGlobal_MusicSetting_All:clearButtonEvent(false)
  PaGlobal_MusicSetting_All:openResemble()
...
function PaGlobal_MusicSetting_All_PlayMusicSolo()
  ...
  if false == _ContentsGroup_InstrumentActivation or -1 == PaGlobal_MusicSetting_All._selectIndex or PaGlobal_MusicSetting_All._selfPlayerIndex == PaGlobal_MusicSetting_All._selectIndex then
...
function PaGlobal_MusicSetting_All_Open(musicIndex, instrumentType)
  PaGlobal_MusicBoard_All_Close()
  PaGlobal_MusicSetting_All:prepareOpen(musicIndex, instrumentType)
end
function PaGlobal_MusicSetting_All_SetMusicIndex(musicIndex)
  PaGlobal_MusicSetting_All._musicIndex = musicIndex
end
