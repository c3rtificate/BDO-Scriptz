  ...
  for index = 0, totalTrackCount - 1 do
    ...
    if nil ~= tempContent then
      ...
      if nil ~= tempTitle then
        ...
        self:setTextureControl(self._controlTable[index].content, trackType, index)
        ...
        self._controlTable[index].content:SetEnableArea(0, 0, self._controlTable[index].content:GetSizeX() + self._controlTable[index].title:GetTextSizeX() + 25, self._controlTable[index].content:GetSizeY())
        self._controlTable[index].content:addInputEvent("Mouse_On", "PaGlobal_SelectInstruments_All:setNameControl(" .. index .. "," .. trackType .. ", true)")
        self._controlTable[index].content:addInputEvent("Mouse_Out", "PaGlobal_SelectInstruments_All:setNameControl(" .. index .. "," .. trackType .. ", false)")
      end
    end
  end
  if false == _ContentsGroup_MarnisInstrument then
    self._titleTable[__eeInstrumentAdvancedType_Synth]:SetShow(false)
  end
end
function PaGlobal_SelectInstruments_All:updateControl(musicInfo)
  ...
  if true == _ContentsGroup_AdvancedMusic then
    for advancedType = __eInstrumentAdvancedType_Novice, __eInstrumentAdvancedType_Count - 1 do
      ...
      nextPos = nextPos + self._titleTable[advancedType]:GetSizeY() + gap
        ...
        if false == musicInfo:isEmpty(trackType) and true == Toclient_isHaveInstrument(trackType) and advancedType == ToClient_getAdvancedTypeByIndex(trackType) then
          for ii = 0, #self._controlTable do
            if trackType == self._controlTable[ii].trackType then
              ...
              self._controlTable[ii].title:SetFontColor(4292723614)
              nextPos = nextPos + self._controlTable[ii].content:GetSizeY() + gap
            end
          end
        end
      end
    ...
    nextPos = nextPos + self._titleTable[__eInstrumentAdvancedType_Novice]:GetSizeY() + gap
    for index = 0, instrumentTrackListSize - 1 do
      local trackType = musicInfo:getTrackTypeByIndex(index)
...
function PaGlobal_SelectInstruments_All:prepareOpen(musicIndex)
  ...
  PaGlobal_MusicSetting_All_SetMusicIndex(musicIndex)
...
function PaGlobal_SelectInstruments_All:setTextureControl(control, trackType, index)
  if nil == Panel_Window_SelectInstruments_All then
    return
  end
  ...
  if nil == normalUV or nil == onUV or nil == clickUV then
    ...
    elseif trackType >= 20 and trackType <= 23 then
      hasUV = true
    ...
    elseif trackType >= 24 and trackType <= 27 then
      hasUV = true
    ...
    elseif trackType >= 28 and trackType <= 31 then
      hasUV = true
    ...
    elseif trackType >= 32 and trackType <= 35 then
      hasUV = true
...
function PaGlobal_SelectInstruments_All:setNameControl(control, trackType, isShow)
  if nil == Panel_Window_SelectInstruments_All then
    return
  end
  if isShow ~= nil then
    if self._controlTable[control] ~= nil then
      self._controlTable[control].title:SetText(PAGetString(Defines.StringSheet_GAME, "AUDIO_MIDI_INSTRUMENT_" .. tostring(trackType)))
      if isShow == false then
        self._controlTable[control].title:SetFontColor(4292723614)
      else
        self._controlTable[control].title:SetFontColor(4294967295)
      end
    end
    return
  end
  control:SetText(PAGetString(Defines.StringSheet_GAME, "AUDIO_MIDI_INSTRUMENT_" .. tostring(trackType)))
  control:SetFontColor(4292723614)
end
...
function PaGlobal_SelectInstruments_All:selectTrackIndex(trackIndex)
  ...
  PaGlobal_MusicSetting_All_SelectTrack(trackIndex)
end
