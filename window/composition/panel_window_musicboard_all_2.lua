function PaGlobal_MusicBoard_All_ChangeFilterType(type)
  if nil == Panel_Window_MusicBoard_All then
    return
  end
  PaGlobal_MusicBoard_All._current_InstrumentType = type
  PaGlobal_MusicBoard_All:filterPosInit()
  PaGlobal_MusicBoard_All:update()
end
function HandleEventOnOut_MusicBoard_All_SearchResetTooltip(isShow)
  if nil == isShow or false == isShow then
    TooltipSimple_Hide()
    return
  end
  local uiControl = PaGlobal_MusicBoard_All._ui._btn_searchReset
...
function PaGlobal_MusicBoard_All_FilterTooltipShow(index, midiType)
  ...
  local control = PaGlobal_MusicBoard_All._ui._checkBox_Filter[index].control
  if nil == control then
    return
  end
  TooltipSimple_Show(control, PAGetString(Defines.StringSheet_GAME, "AUDIO_MIDI_INSTRUMENT_" .. midiType))
end
function PaGlobal_MusicBoard_All_ListTooltipShow(key, index, midiType)
  if nil == Panel_Window_MusicBoard_All then
    return
  end
  ...
  local count = PaGlobal_MusicBoard_All._controlCount
  if PaGlobal_MusicBoard_All._current_InstrumentType ~= __eeInstrumentAdvancedType_Synth then
    count = 10
  end
  local control = UI.getChildControl(content, "Radiobutton_Instrument_" .. index % count + 1)
  if nil == control then
    return
  end
  TooltipSimple_Show(control, PAGetString(Defines.StringSheet_GAME, "AUDIO_MIDI_INSTRUMENT_" .. midiType))
...
function PaGlobal_MusicBoard_All_ListEtcTooltip(key)
  ...
  TooltipSimple_Show(control, PAGetString(Defines.StringSheet_GAME, "LUA_INSTRUMENT_ETC"), nil, false)
end
...
function FromClient_OpenMusicBoardToPartyMusic(ensembleMusicName)
  if nil == Panel_Window_MusicBoard_All then
    return
  end
  local function agree()
    PaGlobal_MusicBoard_All._ensembleMusicName = ensembleMusicName
...
function FromClient_OpenMessageBoxAutoRefuse()
  if nil == Panel_Window_MusicBoard_All then
    return
  end
  local messageBoxMemo = PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantFindEnsembleMusic")
  local messageBoxData = {
    title = PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_NOTIFY"),
    content = messageBoxMemo,
...
function PaGlobal_MusicBoard_All_PlayMusic_OpenSetting(musicIndex, instrumentType)
  if nil == Panel_Window_MusicBoard_All then
    return
  end
  PaGlobal_MusicSetting_All_Open(musicIndex, instrumentType)
end
