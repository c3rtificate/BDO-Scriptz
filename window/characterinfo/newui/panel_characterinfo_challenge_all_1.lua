function PaGlobal_CharInfoChallenge_All:initialize()
  ...
  local bannerArea = UI.getChildControl(Panel_CharacterInfoChallenge_All, "Static_BannerArea")
  self._ui.stc_eventBanner = UI.getChildControl(bannerArea, "Static_EventBanner")
  self._ui.stc_eventBanner:SetShow(true)
  if _ContentsGroup_ChinaOnly == true then
    self._ui.stc_eventBanner:SetShow(false)
  end
  self._ui.stc_eventBanner:addInputEvent("Mouse_LUp", "PaGlobalFunc_InGameEventNotice()")
  self._ui.stc_solareBanner = UI.getChildControl(bannerArea, "Static_Solare_Banner")
  self._ui.stc_solareBanner:SetShow(_ContentsGroup_SolareMatching)
  self._ui.stc_solareBanner:addInputEvent("Mouse_LUp", "PaGlobalFunc_InGameBannerSolare()")
  if self._ui.stc_eventBanner:GetShow() then
    self._ui.stc_solareBanner:SetSpanSize(self._ui.stc_solareBanner:GetSpanSize().x, self._ui.stc_eventBanner:GetPosY() + self._ui.stc_eventBanner:GetSizeY() + 10)
  end
  ...
  UI.ASSERT_NAME(nil ~= self._selfPlayer, " Panel_CharacterInfo_FootPrint_All_1.lua / getSelfPlayer() is nil", "-")
  ...
  local completeBg = UI.getChildControl(Panel_CharacterInfoChallenge_All, "Static_SubListBG")
  self._ui.stc_complete_CH = UI.getChildControl(completeBg, "MultilineText_Noreward")
  self._ui.stc_complete_CH:SetShow(false)
...
function PaGlobal_CharInfoChallenge_All:tabMenuSetting()
  ...
  if isGameTypeEnglish() or isGameTypeTaiwan() or isGameTypeSA() or _ContentsOption_CH_GameType or isGameTypeTR() or isGameTypeTH() or isGameTypeASIA() or isGameTypeID() or self._isConsole then
    countrySet = false
  else
    countrySet = true
  end
  ...
  for idx = 0, self._MAXBGCOUNT - 1 do
    do
      ...
      for subidx = 0, self._MAXSUBSLOT - 1 do
        ...
        subItem.enchantText:SetSize(35, 35)
        subItem.enchantText:SetPosX(0)
        subItem.enchantText:SetPosY(0)
...
function PaGlobal_CharInfoChallenge_All:update()
  ...
  local remainReward_Count = ToClient_GetChallengeRemainRewardCount()
  ...
  if currentTab == self._eTAB.COMPLETE then
    ...
    self._ui.stc_complete_CH:SetShow(0 >= self._currentTaskCount)
  ...
  elseif currentTab == self._eTAB.REWARD then
    self._prevRewardTaskCount = self._currentTaskCount
    self._currentTaskCount = PaGlobal_CharInfoChallenge_All:getRewardGroupCount()
    if self._prevRewardTaskCount ~= self._currentTaskCount then
      self._currentSelected._subItemIdx = nil
      self._currentSelected._challengeIdx = nil
    end
    UIScroll.SetButtonSize(self._ui.stc_Scroll, self._MAXBGCOUNT - 1, self._currentTaskCount)
    ...
    for idx = 0, self._MAXBGCOUNT - 1 do
      ...
      if nil ~= rewardWrapper then
        ...
        if existRewardCount > 0 then
          ...
          self._ui.stc_RewardTable[uiCount]._receiveBtn:addInputEvent("Mouse_LUp", "PaGlobalFunc_CharInfoChallenge_All_ReciveAllReward_Try( " .. dataIdx .. "," .. existRewardCount .. "," .. uiCount .. ")")
...
function PaGlobal_CharInfoChallenge_All:setRewardSlotInfo(baseReward, subReward, challengeIdx, uiIdx, isAcceptAbleReward)
  ...
  for idx = 0, self._MAXSUBSLOT - 1 do
    ...
    self._ui.stc_RewardTable[uiIdx]._selectItem[idx]._slot.icon:EraseAllEffect()
    if idx < selectRewardCount then
      ...
      if self._eTAB.REWARD == self._currentSelected._tabIdx and true == isAcceptAbleReward or true == self._isConsole then
        ...
        if self._currentSelected._challengeIdx == challengeIdx and self._currentSelected._subItemIdx ~= nil then
          HandleEventLUp_CharInfoChallenge_All_SelectItemClick(challengeIdx, uiIdx, self._currentSelected._subItemIdx)
        end
      end
      if true == self._isConsole then
        if 0 == uiIdx then
...
function PaGlobal_CharInfoChallenge_All:eraseRewardIconEffect(selectRewardCount)
  for idx = 0, self._MAXBGCOUNT - 1 do
    for index = 0, self._MAXSUBSLOT - 1 do
      self._ui.stc_RewardTable[idx]._selectItem[index]._slot.icon:EraseAllEffect()
      self._ui.stc_RewardTable[idx]._selectItem[index]._slot.icon:EraseAllEffect()
    end
  end
end
function PaGlobal_CharInfoChallenge_All:remindRewardIconEffect(uiIdx, selectRewardCount)
  for index = 0, self._MAXSUBSLOT - 1 do
    if index < selectRewardCount then
      self._ui.stc_RewardTable[uiIdx]._selectItem[index]._slot.icon:AddEffect("fUI_Tuto_ItemHp_01A", false, 0, 0)
    end
  end
end
