function PaGlobal_CharInfoHistory_All:initialize()
  ...
  self._ui.txt_Year_Console = UI.getChildControl(Panel_CharacterInfoHistory_All, "StaticText_YearConsole")
  ...
  self._ui.stc_KeyGuide_LT = UI.getChildControl(stc_MonthTabBg, "Static_LT_ConsoleUI")
  self._ui.stc_KeyGuide_RT = UI.getChildControl(stc_MonthTabBg, "Static_RT_ConsoleUI")
  ...
  self._ui.txt_NoLog = UI.getChildControl(self._ui.frame_ContentTemp, "StaticText_NoLog")
  ...
  if true == self._isConsole then
    self._ui.btn_YearLeft:SetShow(false)
    self._ui.btn_YearRight:SetShow(false)
    self._ui.txt_Year:SetShow(false)
    for idx = 0, #self._pastMonth_DayCount - 1 do
      local spanSize = self._ui.rdo_MonthTable[idx]:GetSpanSize()
      self._ui.rdo_MonthTable[idx]:SetSpanSize(spanSize.x - 50, spanSize.y)
      self._ui.rdo_MonthTable[idx]:SetIgnore(true)
    end
    self._ui.stc_KeyGuide_LT:SetShow(self._isConsole)
    self._ui.stc_KeyGuide_RT:SetShow(self._isConsole)
    self._ui.txt_Year_Console:SetShow(self._isConsole)
    self._ui.stc_keyGuideBG = UI.getChildControl(Panel_CharacterInfoHistory_All, "Static_KeyGuideBg_Console")
    self._ui.stc_keyGuideB = UI.getChildControl(self._ui.stc_keyGuideBG, "StaticText_B_ConsoleUI")
  end
  UI.ASSERT_NAME(nil ~= self._selfPlayer, " Panel_CharacterInfo_Title_All_1.lua / getSelfPlayer() is nil", "-")
...
function PaGlobal_CharInfoHistory_All:registEventHandler(console)
  ...
  if true == console then
    PaGlobal_CharInfo_All._ui.stc_HistoryBg:registerPadEvent(__eConsoleUIPadEvent_Up_LT, "HandleEventLUp_CharInfoHistory_All_ChangeCurrentValue(" .. -1 .. ")")
    PaGlobal_CharInfo_All._ui.stc_HistoryBg:registerPadEvent(__eConsoleUIPadEvent_Up_RT, "HandleEventLUp_CharInfoHistory_All_ChangeCurrentValue(" .. 1 .. ")")
  else
    for idx = 0, #self._pastMonth_DayCount - 1 do
...
function PaGlobal_CharInfoHistory_All:updateFrame()
  ...
  if true == self._isConsole then
    local changeText = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORLD_MAP_SIEGE_YEAR", "year", self._currentValue._year) .. " " .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WORLD_MAP_SIEGE_MONTH", "month", self._currentValue._month)
    self._ui.txt_Year_Console:SetText(changeText)
  end
  ...
  local noLogText = UI.cloneControl(self._ui.txt_NoLog, self._ui.frame_Content, "StaticText_NoLog")
...
function PaGlobal_CharInfoHistory_All:monthUpdate(index, audioFlag)
  ...
  self._haveInfoMonth = self._currentValue._month
  ToClient_RequestJournalList(self._currentValue._year, self._currentValue._month, self._currentValue._myHistory)
end
function PaGlobal_CharInfoHistory_All:changeCurrentValue(moveDir)
  local nextYear = self._currentValue._year
  local nextMonth = self._currentValue._month
  nextMonth = nextMonth + moveDir
  if nextMonth < 1 then
    nextMonth = 12
    nextYear = nextYear - 1
  ...
  if nextYear < self._firstLogYearValue or nextYear > ToClient_GetThisYear() then
    return
  end
  if nextYear == self._currentValue._year and nextMonth == self._currentValue._month then
    return
  end
  for idx = 0, #self._pastMonth_DayCount - 1 do
    if idx == nextMonth - 1 then
      self._ui.rdo_MonthTable[idx]:SetCheck(true)
      self._currentidx = idx
    else
      self._ui.rdo_MonthTable[idx]:SetCheck(false)
  ...
  self._ui.stc_SelectLine:SetSpanSize(self._ui.rdo_MonthTable[self._currentidx]:GetSpanSize().x, self._ui.stc_SelectLine:GetSpanSize().y)
  self._currentValue._year = nextYear
  self._currentValue._month = nextMonth
  local isLeftHide = self._firstLogYearValue == nextYear and 1 == nextMonth
  local isRightHide = ToClient_GetThisYear() == nextYear and 12 == nextMonth
  self._ui.stc_KeyGuide_LT:SetShow(not isLeftHide)
  self._ui.stc_KeyGuide_RT:SetShow(not isRightHide)
  if ToClient_GetThisMonth() == self._currentValue._month then
    self._currentValue._day = ToClient_GetToday()
  else
    self._currentValue._day = self._pastMonth_DayCount[nextMonth]
  end
  ToClient_RequestJournalList(self._currentValue._year, self._currentValue._month, self._currentValue._myHistory)
end
function PaGlobal_CharInfoHistory_All:validate()
  ...
  self._ui.txt_Year_Console:isValidate()
