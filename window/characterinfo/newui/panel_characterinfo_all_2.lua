...
function HandleEventLUp_ChracterInfo_All_ClickOtherTab(idx)
  UI.ASSERT_NAME(nil ~= PaGlobal_CharInfo_All._tabData[idx], "Panel_CharacterInfo_All.lua / self._tabData[idx] is nil ", "-")
  if nil == idx then
    return
  end
...
function PaGlobalFunc_ChracterInfo_All_ChangeOpenPosForReward()
  Panel_CharacterInfo_All:SetSpanSize(-200, 0)
  Panel_CharacterInfo_All:ComputePos()
end
...
function PaGlobalFunc_ChracterInfo_All_GetBgShow(idx)
  ...
  if nil ~= PaGlobal_CharInfo_All._tabData[idx].bg then
    ...
    UI.ASSERT_NAME(nil ~= PaGlobal_CharInfo_All._tabData[idx].bg, "Panel_CharacterInfo_All.lua / PaGlobal_CharInfo_All._tabData[idx].bg[" .. idx .. "] is nil ", "-")
  end
end
...
function PaGlobalFunc_ChracterInfo_All_ShowTooltip_Console(idx)
  ...
  if nil == idx or true == _ContentsGroup_RenewUI_Tooltip and true == Panel_Widget_Tooltip_Renew:GetShow() then
    return
...
function PaGlobalFunc_ChracterInfo_All_Link_Title(linkTitleKey, linkCategory)
  ...
  HandleEventLUp_ChracterInfo_All_ClickOtherTab(1)
  PaGlobal_CharinfoTitle_LinkAcquireTitle(linkTitleKey, linkCategory)
end
