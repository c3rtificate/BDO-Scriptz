...
function PaGlobal_CharInfoLife_All:initialize()
  ...
  for idx = 0, self._LIFETYPE_COUNT - 1 do
    ...
    if nil ~= parent then
      ...
      if idx == eLIFE.gather then
        self._ui.stc_collectMiniGameIcon = UI.getChildControl(lifeUI._title, "Static_Icon")
        self._ui.btn_collectMiniGameOnOff = UI.getChildControl(lifeUI._title, "CheckButton_OnOff")
        self._ui.stc_collectMiniGameIcon:SetShow(_ContentsGroup_MorningLand)
        self._ui.btn_collectMiniGameOnOff:SetShow(_ContentsGroup_MorningLand)
      end
  ...
  UI.ASSERT_NAME(nil ~= self._selfPlayer, " Panel_CharacterInfo_Life_All_1.lua / getSelfPlayer() is nil", "-")
...
function PaGlobal_CharInfoLife_All:updateKeyguides()
  ...
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyGuideList, self._ui.stc_Console_KeyGuideBg, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHT, nil, nil)
end
...
function PaGlobal_CharInfoLife_All:setUnChangedData()
  for idx = 0, self._LIFETYPE_COUNT - 1 do
    if nil ~= self._ui.stc_life[idx] then
      ...
      if idx == eLIFE.gather then
        local spanGapX = self._ui.stc_life[idx]._title:GetTextSizeX() + 5
        self._ui.stc_collectMiniGameIcon:SetSpanSize(self._ui.stc_collectMiniGameIcon:GetSpanSize().x + spanGapX, self._ui.stc_collectMiniGameIcon:GetSpanSize().y)
        self._ui.btn_collectMiniGameOnOff:SetSpanSize(self._ui.btn_collectMiniGameOnOff:GetSpanSize().x + spanGapX, self._ui.btn_collectMiniGameOnOff:GetSpanSize().y)
      end
      self._ui.stc_life[idx]._parent:SetIgnore(false)
      if false == self._isConsole then
        self._ui.stc_life[idx]._parent:addInputEvent("Mouse_On", "HandleEventOnOut_CharInfoLife_All_LifeType_Tooltip(true," .. tostring(idx) .. ", -1 ,false)")
      ...
      elseif false == _ContentsGroup_RenewUI_Tooltip then
        self._ui.stc_life[idx]._parent:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandleEventOnOut_CharInfoLife_All_LifeType_Tooltip(true," .. tostring(idx) .. ",-1,false)")
        self._ui.stc_life[idx]._parent:addInputEvent("Mouse_On", "HandleEventOnOut_CharInfoLife_All_LifeType_Area(true," .. tostring(idx) .. ")")
        self._ui.stc_life[idx]._parent:addInputEvent("Mouse_Out", "HandleEventOnOut_CharInfoLife_All_LifeType_Tooltip(false)")
      else
        self._ui.stc_life[idx]._parent:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandleEventOnOut_CharInfoLife_All_LifeType_Tooltip(true," .. tostring(idx) .. ",-1, true)")
        self._ui.stc_life[idx]._parent:addInputEvent("Mouse_On", "HandleEventOnOut_CharInfoLife_All_LifeType_Area(true," .. tostring(idx) .. ")")
        self._ui.stc_life[idx]._parent:addInputEvent("Mouse_Out", "HandleEventOnOut_CharInfoLife_All_LifeType_Tooltip(false)")
      end
      if _ContentsGroup_MorningLand == true and idx == 0 then
        self._ui.stc_life[idx]._parent:addInputEvent("Mouse_LUp", "HandleEventLUp_CharInfoLife_All_UpdateCollectMiniGameUseable()")
      end
        ...
        for subidx = 1, #self._ui.stc_life[idx]._subLifeType do
          ...
          if nil ~= stringTable then
            ...
            if false == self._isConsole then
              ...
              self._ui.stc_life[idx]._subLifeType[subidx]._subParent:addInputEvent("Mouse_On", "HandleEventOnOut_CharInfoLife_All_LifeType_Tooltip(true," .. tostring(idx) .. "," .. tostring(subidx) .. ",false)")
            ...
            elseif false == _ContentsGroup_RenewUI_Tooltip then
              self._ui.stc_life[idx]._subLifeType[subidx]._subtitle:SetIgnore(true)
              self._ui.stc_life[idx]._subLifeType[subidx]._subParent:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandleEventOnOut_CharInfoLife_All_LifeType_Tooltip(true," .. tostring(idx) .. "," .. tostring(subidx) .. ",false)")
              self._ui.stc_life[idx]._subLifeType[subidx]._subParent:addInputEvent("Mouse_Out", "HandleEventOnOut_CharInfoLife_All_LifeType_Tooltip(false)")
            elseif true == _ContentsGroup_RenewUI then
              self._ui.stc_life[idx]._subLifeType[subidx]._subtitle:SetIgnore(true)
              self._ui.stc_life[idx]._subLifeType[subidx]._subParent:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandleEventOnOut_CharInfoLife_All_LifeType_Tooltip(true," .. tostring(idx) .. "," .. tostring(subidx) .. ",true)")
              self._ui.stc_life[idx]._subLifeType[subidx]._subParent:addInputEvent("Mouse_Out", "HandleEventOnOut_CharInfoLife_All_LifeType_Tooltip(false)")
            end
          end
        end
...
function PaGlobal_CharInfoLife_All:update()
    ...
    if nil ~= self._ui.stc_life[idx] then
      ...
      if true == _ContentsGroup_EnhanceCollect then
        if idx == eLIFE.gather or idx == eLIFE.manufacture then
          ...
          commonPoint = math.max(commonPoint, 0)
  ...
  local isNotUseableMiniGame = ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(__eCollectMiniGameNotUseable)
  self._ui.btn_collectMiniGameOnOff:SetCheck(not isNotUseableMiniGame)
  self._ui.stc_collectMiniGameIcon:SetMonoTone(isNotUseableMiniGame)
  local isLearnKnowledge = ToClient_IsLearnedKnowledge(PaGlobal_CharInfoLife_All._miniGameMentalCardKey)
  self._ui.btn_collectMiniGameOnOff:SetShow(isLearnKnowledge)
  self._ui.stc_collectMiniGameIcon:SetMonoTone(isLearnKnowledge == false)
end
function PaGlobal_CharInfoLife_All:validate()
  ...
  self._ui.stc_collectMiniGameIcon:isValidate()
  self._ui.btn_collectMiniGameOnOff:isValidate()
