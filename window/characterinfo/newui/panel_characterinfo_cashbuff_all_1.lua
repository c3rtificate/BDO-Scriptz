...
function PaGlobal_CharInfoCashBuff_All:update()
  ...
  local isAppliedElementalMonsterBuff = isAppliedMonsterBuffLand or isAppliedMonsterBuffMoon or isAppliedMonsterBuffSun
  local isAppliedThousandInsam = player:isApplyChargeSkill(CppEnums.UserChargeType.eUserChargeType_ThousandInsam)
  local isAppliedSiege2024 = player:isApplyChargeSkill(CppEnums.UserChargeType.eUserChargeType_Siege2024)
  local currentServerData = getCurrentChannelServerData()
  if nil ~= currentServerData then
    applyNewOlviaBuff = currentServerData._isSpeedChannel
  end
  flags[self._eBUFF_TYPE.GuildWar] = self._localNodeInvestment
  flags[self._eBUFF_TYPE.RussiaMonthly] = false
  if applyStarter then
    local isVaild = starter > 0
    flags[self._eBUFF_TYPE.Kamasylvia] = isVaild
  else
    flags[self._eBUFF_TYPE.Kamasylvia] = false
  end
  if applyPremium then
    local isVaild = premium > 0
  ...
  if true == isPremiumPcRoom then
    if not isGameTypeRussia() and not isGameTypeEnglish() and not isGameTypeSA() and not isGameTypeTR() then
  ...
  flags[self._eBUFF_TYPE.HopeBuff] = _ContentsGroup_HopeGauge
  flags[self._eBUFF_TYPE.HappyBlackSpiritBuff] = true == _contentsGroup_HappyBlackSpiritBuff and true == applyTransferEventBuff
  flags[self._eBUFF_TYPE.NewOlviaBuff] = applyNewOlviaBuff
  flags[self._eBUFF_TYPE.EventBuff] = _ContentsGroup_TransferEventBuff
  flags[self._eBUFF_TYPE.ElementalMonsterBuff] = isAppliedElementalMonsterBuff
  flags[self._eBUFF_TYPE.ThousandInsam] = isAppliedThousandInsam
  flags[self._eBUFF_TYPE.Siege2024] = isAppliedSiege2024
  self._buffData = {}
  self:updateBuffData()
  self._listShowBuff = {}
  local uiIndex = 1
...
function PaGlobal_CharInfoCashBuff_All:updateBuffData()
  ...
  local thousandInsamTime = player:getUserChargeTime(CppEnums.UserChargeType.eUserChargeType_ThousandInsam)
  local siege2024Time = player:getUserChargeTime(CppEnums.UserChargeType.eUserChargeType_Siege2024)
  ...
  for buffType = 1, self._buffTypeCount do
    ...
    elseif buffType == self._eBUFF_TYPE.FeverBuff then
      ...
      if true == ToClient_isConsole() then
        ...
        desc = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_AGRISSPOINT_ICON_TOOLTIP", "addFeverPoint", addFeverPoint, "maxFeverPoint", maxFeverPoint, "feverDropRate", feverDropRate)
      end
      if feverPoint > 0 then
    ...
    elseif buffType == self._eBUFF_TYPE.HopeBuff then
      ...
      desc = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_HOPEGAUGE_TOOLTIP_DESC", "grade", grade, "drop", drop, "count", count)
      leftTime = Util.Time.timeFormatting(Int64toInt32(player:getItemCollectionScrollPoint()))
    elseif buffType == self._eBUFF_TYPE.HappyBlackSpiritBuff then
      ...
      leftTime = nil
    elseif buffType == self._eBUFF_TYPE.NewOlviaBuff then
      ...
      leftTime = nil
    elseif buffType == self._eBUFF_TYPE.EventBuff then
      ...
      leftTime = nil
    elseif buffType == self._eBUFF_TYPE.ElementalMonsterBuff then
      local isAppliedMonsterBuffSun = ToClient_isAppliedElementalMonsterBuff(self._elementalMonsterBuffSun)
      local isAppliedMonsterBuffMoon = ToClient_isAppliedElementalMonsterBuff(self._elementalMonsterBuffMoon)
      local isAppliedMonsterBuffLand = ToClient_isAppliedElementalMonsterBuff(self._elementalMonsterBuffLand)
      ...
      if isAppliedMonsterBuffSun == true then
        ...
        local isSelfPlayer = selfPlayer:getUserNickname() == userNickname or selfPlayer:getOriginalName() == userNickname
    ...
    elseif buffType == self._eBUFF_TYPE.ThousandInsam then
      ...
      leftTime = convertStringFromDatetime(toInt64(0, thousandInsamTime))
    elseif buffType == self._eBUFF_TYPE.Siege2024 then
      ...
      leftTime = convertStringFromDatetime(toInt64(0, siege2024Time))
    end
