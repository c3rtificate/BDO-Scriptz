function PaGlobal_CharInfoTitle_All:initialize()
  ...
  local btn_Question = UI.getChildControl(stc_totalProgBg, "Button_Question")
  self._ui.stc_Question = UI.getChildControl(btn_Question, "Static_QuestionMark")
  self._ui.stc_TitleTooltip = UI.getChildControl(Panel_CharacterInfoTitle_All, "Static_TitleToolTip")
  local titleTooltipBG = UI.getChildControl(self._ui.stc_TitleTooltip, "Static_ToolTipBG")
  local titleTooltipText = UI.getChildControl(titleTooltipBG, "StaticText_TitleInfo")
  titleTooltipText:SetTextMode(__eTextMode_AutoWrap)
  titleTooltipText:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CHARACTERINFO_TITLE_TOOLTIP_DESC"))
  local titleToolTipTextSizeY = titleTooltipText:GetTextSizeY()
  local titleToolTipSizeDiff = self._ui.stc_TitleTooltip:GetSizeY() - titleTooltipBG:GetSizeY()
  local titleToolTipBGSizeY = titleTooltipBG:GetSizeY() + (titleToolTipTextSizeY - titleTooltipBG:GetSizeY())
  self._ui.stc_TitleTooltip:SetSize(self._ui.stc_TitleTooltip:GetSizeX(), titleToolTipBGSizeY + titleToolTipSizeDiff)
  titleTooltipBG:SetSize(titleTooltipBG:GetSizeX(), titleToolTipBGSizeY)
  self._ui.titleTooltipPosX = self._ui.stc_TitleTooltip:GetPosX()
  self._ui.titleTooltipPosY = self._ui.stc_TitleTooltip:GetPosY()
  ...
  for idx = 0, 3 do
    ...
    if true == _ContentsGroup_ChinaFontColor and idx == self._eTITLETYPE._COMBAT then
      titleProg._title:SetFontColor(Defines.Color.C_FFFFAD38)
    end
  ...
  self._ui.stc_KeyGuide_LS = UI.getChildControl(self._ui.stc_Console_KeyGuides, "StaticText_EffectLSButton")
  ...
  self._ui.stc_KeyGuide_LS:SetShow(self._isConsole)
  if CppEnums.ServiceResourceType.eServiceResourceType_KR ~= serviceType and CppEnums.ServiceResourceType.eServiceResourceType_CN ~= serviceType and CppEnums.ServiceResourceType.eServiceResourceType_Dev ~= serviceType then
...
function PaGlobal_CharInfoTitle_All:registEventHandler(console)
  ...
  registerEvent("FromClient_Aquire_LastTitle", "FromClient_CharInfoTitle_AquireTitle")
  ...
  if true == console then
    ...
    PaGlobal_CharInfo_All._ui.stc_TitleBg:registerPadEvent(__eConsoleUIPadEvent_LSClick, "HandleEventOn_CharInfoTitle_QuestionTooltip()")
  else
    ...
    self._ui.stc_Question:addInputEvent("Mouse_On", "HandleEventOn_CharInfoTitle_QuestionTooltip(true)")
    self._ui.stc_Question:addInputEvent("Mouse_Out", "HandleEventOn_CharInfoTitle_QuestionTooltip(false)")
    for idx = 0, 3 do
...
function PaGlobal_CharInfoTitle_All:dataClear()
  ...
  self._linkFindIndex = -1
  self._linkTitleKeyRaw = 0
  self._linkTitleCategory = 0
  if true == self._isConsole then
    local keyguide = {
      self._ui.stc_KeyGuide_LS,
      self._ui.stc_KeyGuide_Y,
      self._ui.stc_KeyGuide_A,
      self._ui.stc_KeyGuide_B
    }
    PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyguide, self._ui.stc_Console_KeyGuides, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP)
  end
  self._ui.stc_TitleTooltip:SetShow(false)
end
