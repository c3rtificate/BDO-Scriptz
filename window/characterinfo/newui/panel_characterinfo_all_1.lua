...
function PaGlobal_CharInfo_All:switchPlatForm(console)
  ...
  self._ui.btn_Resize:SetShow(not console)
  self._ui.rdo_History:SetShow(true)
...
function PaGlobal_CharInfo_All:prepareOpen()
  if false == self._initialize then
    local function setTabData(index, btn, panel, bg, updateFunc)
      ...
      UI.ASSERT_NAME(nil ~= index, "ChracterInfo_All_1.lua / index is nil", "-")
      UI.ASSERT_NAME(nil ~= btn, "ChracterInfo_All_1.lua / btn is nil", "-")
      UI.ASSERT_NAME(nil ~= panel, "ChracterInfo_All_1.lua / panel is nil", "-")
      UI.ASSERT_NAME(nil ~= bg, "ChracterInfo_All_1.lua / bg is nil", "-")
      UI.ASSERT_NAME(nil ~= self._tabData, "ChracterInfo_All_1.lua / table is nil", "-")
      UI.ASSERT_NAME(nil ~= updateFunc, "ChracterInfo_All_1.lua / updateFunc is nil", "-")
...
function PaGlobal_CharInfo_All:otherTabOpen(idx)
  UI.ASSERT_NAME(nil ~= self._tabData[idx], "Panel_CharacterInfo_All_1.lua / self._tabData[idx] is nil ", "-")
  ...
  UI.ASSERT_NAME(nil ~= self._tabData[idx].updateFunc, "Panel_CharacterInfo_All_1.lua / idx =" .. tostring(idx) .. "\tupdateFunc is nil ", "-")
  ...
  if true == self._isConsole then
    ...
    elseif self._TABINDEX._footPrint == idx then
      Panel_CharacterInfo_All:registerPadEvent(__eConsoleUIPadEvent_Up_RT, "HandleEventPadPress_CharInfoFootPrint_All_TabChange(true)")
      Panel_CharacterInfo_All:registerPadEvent(__eConsoleUIPadEvent_Up_LT, "HandleEventPadPress_CharInfoFootPrint_All_TabChange(false)")
    else
...
function PaGlobal_CharInfo_All:setKeyGuideAlign(idx)
  ...
  if true == PaGlobalFunc_ChracterInfo_All_GetBgShow(PaGlobal_CharInfo_All._TABINDEX._basic) then
    keyguide = {
      ...
      PaGlobal_CharInfoBasic_All._ui.stc_KeyGuide_RS,
  ...
  elseif true == PaGlobalFunc_ChracterInfo_All_GetBgShow(PaGlobal_CharInfo_All._TABINDEX._history) then
    keyguide = {
      PaGlobal_CharInfoHistory_All._ui.stc_keyGuideB
    }
  end
