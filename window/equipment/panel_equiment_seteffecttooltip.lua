Panel_Equipment_SetEffect_Tooltip:SetShow(false, false)
Panel_Equipment_SetEffect_Tooltip:ActiveMouseEventEffect(true)
Panel_Equipment_SetEffect_Tooltip:setMaskingChild(true)
Panel_Equipment_SetEffect_Tooltip:setGlassBackground(true)
PaGlobal_EquipmentTooltip = {
  _ui = {
    _Title = UI.getChildControl(Panel_Equipment_SetEffect_Tooltip, "StaticText_Title"),
    _NoEffect = UI.getChildControl(Panel_Equipment_SetEffect_Tooltip, "StaticText_NoEffect"),
    _BaseGroupTitle = UI.getChildControl(Panel_Equipment_SetEffect_Tooltip, "StaticText_EffectGroupTitle"),
    _BasePartTitle = UI.getChildControl(Panel_Equipment_SetEffect_Tooltip, "StaticText_EffectPartTitle"),
    _BaseSetDesc = UI.getChildControl(Panel_Equipment_SetEffect_Tooltip, "StaticText_EffectDesc"),
    _Desc = UI.getChildControl(Panel_Equipment_SetEffect_Tooltip, "StaticText_Desc"),
    _GroupTitle = {},
    _PartTitle = {},
    _PartDesc = {},
  ...
  _isDetail = false,
  _panel = Panel_Equipment_SetEffect_Tooltip,
  _info = {}
}
function PaGlobal_EquipmentTooltip:createAllGroupTitleUI()
  for index = 0, self._maxSetEffectCount - 1 do
    ...
    local control = UI.createControl(__ePAUIControl_StaticText, self._panel, controlName)
...
function PaGlobal_EquipmentTooltip:createPartTitleUI()
  local control = UI.createControl(__ePAUIControl_StaticText, self._panel, "PartTitle_" .. #self._ui._PartTitle)
...
function PaGlobal_EquipmentTooltip:createPartDescUI()
  local control = UI.createControl(__ePAUIControl_StaticText, self._panel, "PartDesc_" .. #self._ui._PartDesc)
...
function PaGlobal_EquipmentTooltip:SetPos()
  ...
  local posY = equipPosY + control:GetSizeY() - self._panel:GetSizeY() - 10
  if posY < 0 then
    posY = 0
...
function PaGlobal_EquipmentTooltip:updateInfo()
  ...
  for i = 0, #sortedInfoList do
    ...
    local skillPoint = sortedInfoList[i]:getPoint()
...
function PaGlobal_EquipmentTooltip:initUI()
  ...
  self._ui._Title:SetTextMode(__eTextMode_AutoWrap)
...
function PaGlobal_EquipmentTooltip:toggleDetail()
  ...
  self:updateUI()
  if true == self._panel:IsUISubApp() then
    self._panel:CloseUISubApp()
    self:SetPos()
    self._panel:OpenUISubApp()
  end
