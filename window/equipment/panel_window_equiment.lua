...
function equip_checkUseableSlot(index)
  ...
  if index == UnUsedEquipNo_01 or index == nUsedEquipNo_02 or index == __eEquipSlotNoCount or index == __eEquipSlotNoExplorationBonus0 or index == __eEquipSlotNoInstallation4 or index == __eEquipSlotNoBody or index == __eEquipSlotNoAvatarBody then
    returnValue = false
  end
  return returnValue
end
function equip:initControl()
  ...
  for v = EquipNoMin, EquipNoMax do
    if true == equip_checkUseableSlot(v) then
      ...
      if true == ToClient_EquipSlot_CheckItemLock(v, whereType) then
        ToClient_Inventory_RemoveItemLock(v, whereType)
      end
      if 15 == v then
      ...
      if nil ~= self.avataEquipSlotId[v] then
        ...
        if __eEquipSlotNoAvatarAwakenWeapon == v then
  ...
  if __eClassType_ShyWaman == getSelfPlayer():getClassType() then
    ...
    local loopSlot = {__eEquipSlotNoBelt, __eEquipSlotNoNecklace}
  ...
  if false == ToClient_isAdultUser() then
    ...
    self.checkUnderwear:SetShow(false == _ContentsOption_CH_NoUnderWear)
  end
...
function FGlobal_Equipment_SetFunctionButtonHide(isShow)
  ...
  if true == _ContentsOption_CH_NoUnderWear or true == ToClient_isAdultUser() then
...
function Equipment_updateSlotData()
  ...
  for v = EquipNoMin, EquipNoMax do
    if true == equip_checkUseableSlot(v) then
      ...
      if nil ~= itemWrapper then
        ...
        slot.icon:SetEnable(true)
        slotBG:SetShow(true)
        if __eEquipSlotNoAwakenWeapon == v then
        ...
        elseif __eEquipSlotNoAvatarAwakenWeapon == v then
          local isShow = false
          if __eClassType_ShyWaman ~= getSelfPlayer():getClassType() then
  ...
  if false == ToClient_IsContentsGroupOpen("335") then
    ...
    self.slots[__eEquipSlotNoQuestBook].icon:addInputEvent("Mouse_LUp", "Input_Equipment_QuestBook()")
  end
  local alchemyStoneQuickKeyString = keyCustom_GetString_UiKey(__eUiInputType_AlchemyStone)
  alchemyStoneQuickKey:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_ALCHEMYSTONE_QUICKCOMMAND", "keyString", alchemyStoneQuickKeyString))
end
...
function FGlobal_AccSlotNo(itemWrapper, isChange)
  ...
  elseif 92 == equipType then
    ...
    equipItemWrapper = ToClient_getEquipmentItem(32)
    equipItemWrapper2 = ToClient_getEquipmentItem(33)
    if nil ~= equipItemWrapper and nil ~= equipItemWrapper2 then
      acc = 32 + equip.slotArtifactIndex
      if true == isChange then
        if 0 == equip.slotArtifactIndex then
          equip.slotArtifactIndex = 1
        else
          equip.slotArtifactIndex = 0
        end
      end
