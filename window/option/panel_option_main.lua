...
function PaGlobal_Option:InitUi()
  ...
  self._ui._staticCategoryDesc:SetTextMode(__eTextMode_LimitText)
  ...
  for specindex = 1, specCount do
    self._ui.spec[specindex] = UI.getChildControl(self._ui._staticSpecBG, "Button_Spec" .. specindex)
    self._ui.spec[specindex]:addInputEvent("Mouse_LUp", "PaGlobal_Option:ClickedSpecSetting(" .. specindex .. ")")
    local specName = UI.getChildControl(self._ui.spec[specindex], "StaticText_Name")
    specName:SetTextMode(__eTextMode_AutoWrap)
    specName:SetText(specName:GetText())
    self._ui._specDescTable[specindex] = UI.getChildControl(self._ui.spec[specindex], "StaticText_Desc")
    self._ui._specDescTable[specindex]:SetTextMode(__eTextMode_AutoWrap)
    ...
    self._ui.spec[specindex]:addInputEvent("Mouse_On", "PaGlobal_Option:MouseOnSpec(" .. specindex .. ")")
    self._ui.spec[specindex]:addInputEvent("Mouse_Out", "PaGlobal_Option:MouseOutSpec(" .. specindex .. ")")
  end
  ...
  self._ui._categoryTitleTable.Performance:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui._categoryTitleTable.Graphic:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui._categoryTitleTable.Sound:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui._categoryTitleTable.Function:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui._categoryTitleTable.Interface:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui._listSearchBg:registEvent(__ePAUIList2EventType_LuaChangeContent, "FGlobal_Option_List2SearchElementCreate")
  self._ui._listSearchBg:createChildContent(__ePAUIList2ElementManagerType_List)
  ...
  for customIndex = 0, 1 do
    ...
    self._ui._customDescTable[customIndex]:SetTextMode(__eTextMode_AutoWrap)
    ...
    customName:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui._btn_uiScale = UI.getChildControl(self._ui._staticSpecBG, "Button_UIScale")
  self._ui._btn_uiScale:addInputEvent("Mouse_LUp", "PaGlobal_Option:optimizationUIScale()")
  self._ui._btn_uiScale:addInputEvent("Mouse_On", "PaGlobal_Option:MouseOnUIScale()")
  self._ui._btn_uiScale:addInputEvent("Mouse_Out", "PaGlobal_Option:MouseOutUIScale()")
  self._ui._uiScaleDesc = UI.getChildControl(self._ui._btn_uiScale, "StaticText_Desc")
  local scaleName = UI.getChildControl(self._ui._btn_uiScale, "StaticText_Name")
  scaleName:SetTextMode(__eTextMode_AutoWrap)
  scaleName:SetText(scaleName:GetText())
  self._ui._uiScaleDesc:SetPosY(self._ui._uiScaleDesc:GetPosY() + scaleName:GetTextSizeY() - scaleName:GetSizeY())
  self._ui._buttonQuestion:addInputEvent("Mouse_LUp", "Panel_WebHelper_ShowToggle( \"UIGameOption\" )")
  PaGlobal_Util_RegistHelpTooltipEvent(self._ui._buttonQuestion, "\"UIGameOption\"")
...
function PaGlobal_Option:Open()
  ...
  self._prevUIScale = ToClient_GetUIScale()
  self._prevResolutionX = getResolutionSizeX()
  self._prevResolutionY = getResolutionSizeY()
end
...
function PaGlobal_Option:getOptionFromName(strElementName)
  for elementName, option in pairs(self._elements) do
    if strElementName == elementName then
      return option
    end
  end
end
function PaGlobal_Option:ClickedApplyOption()
  ...
  self:SetXXX("GraphicOption", self._elements.GraphicOption._curValue)
  for elementName, option in pairs(self._elements) do
...
function FGlobal_Option_ChangeDisplayApply()
  ...
  saveGameOption(true)
  reloadGameUI()
  if ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(__eUiInterfaceSizeMessagePanel) == false and ToClient_GetUIScale() == PaGlobal_Option._prevUIScale and (PaGlobal_Option._prevResolutionX ~= getResolutionSizeX() or PaGlobal_Option._prevResolutionY ~= getResolutionSizeY()) then
    PaGlobal_Option:optimizationUIScale()
  end
end
...
function PaGlobal_Option:MouseOnSpec(category)
  local control = self._ui.spec[category]
  local name = self._ui._specDescTable[category]:GetText()
...
function PaGlobal_Option:MouseOnCustom(index)
  local control = self._ui["_buttonLoadCustom" .. index]
  local name = self._ui._customDescTable[index]:GetText()
  TooltipSimple_Show(control, name)
end
function PaGlobal_Option:MouseOutCustom(index)
  TooltipSimple_Hide()
end
function PaGlobal_Option:MouseOnUIScale()
  local control = self._ui._btn_uiScale
  local name = self._ui._uiScaleDesc:GetText()
  TooltipSimple_Show(control, name)
end
function PaGlobal_Option:MouseOutUIScale()
  TooltipSimple_Hide()
end
...
function PaGlobal_Option:SelectOptionFrame(category, detail)
  ...
  self._ui._staticSearchNoResult:SetShow(false)
  if "Graphic" == category and "Window" == detail then
    PaGlobal_Option:SetRecommendScale()
  end
end
function PaGlobal_Option:ListInit()
  local tree2 = self._ui._list2
  tree2:changeAnimationSpeed(11)
  tree2:registEvent(__ePAUIList2EventType_LuaChangeContent, "FGlobal_Option_CategoryUpdate")
...
function FGlobal_Option_CategoryUpdate(contents, key)
  ...
  if indexMap._isMain then
    ...
    categorySubBar:SetCheck(PaGlobal_Option._list2._selectedSubKey == idx)
  end
end
...
function PaGlobal_Option:optimizationUIScale()
  ...
  local messageboxData = {
    ...
    content = messageboxMemo,
    functionYes = PaGlobal_Option_OptimizationUIScaleYes,
    functionNo = MessageBox_Empty_function,
    priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
  }
  MessageBox.showMessageBox(messageboxData, nil, false, true, 3)
end
function PaGlobal_Option_OptimizationUIScaleYes()
  local option = PaGlobal_Option:getOptionFromName("UIScale")
  if nil == option then
    return
  end
  local sliderValue = PaGlobal_Option:FromRealValueToSliderValue(PaGlobal_Option:getRecommendUIScale(), option._sliderValueMin, option._sliderValueMax)
  option._curValue = sliderValue
  PaGlobal_Option:ClickedApplyOption()
end
function PaGlobal_Option_InitCheckUIScale()
  ...
  local currentValue = PaGlobal_Option:FromSliderValueToRealValue(option._initValue, option._sliderValueMin, option._sliderValueMax)
  currentValue = math.floor(currentValue + 0.5)
  if 100 ~= currentValue then
    return
  end
  local sizeY = Panel_CharacterInfo_All:GetSizeY()
...
function PaGlobal_Option:SetContentsOption()
  ...
  if false == _ContentsGroup_InstrumentPerformance then
    local soundBG = UI.getChildControl(self._frames.Sound.OnOff._uiFrameContent, "StaticText_BgOrder0_Import")
    local button = UI.getChildControl(soundBG, "CheckButton_EnableAroundPlayMusic")
    local buttonStr = UI.getChildControl(soundBG, "StaticText_EnableAroundPlayMusic_Desc")
    button:SetShow(false)
    buttonStr:SetShow(false)
    self._elements.EnableAroundPlayMusic = nil
    soundBG:SetSize(soundBG:GetSizeX(), soundBG:GetSizeY() - 35)
    local bg1 = UI.getChildControl(self._frames.Sound.OnOff._uiFrameContent, "StaticText_BgOrder1_Import")
    local bg2 = UI.getChildControl(self._frames.Sound.OnOff._uiFrameContent, "StaticText_BgOrder2_Import")
    local bg3 = UI.getChildControl(self._frames.Sound.OnOff._uiFrameContent, "StaticText_BgOrder3_Import")
    bg1:SetPosY(bg1:GetPosY() - 35)
    bg2:SetPosY(bg2:GetPosY() - 35)
    bg3:SetPosY(bg3:GetPosY() - 35)
  end
  ...
  if false == isNeedGameOptionFromServer() then
    if true == _ContentsOption_CH_WaterMark then
      local frameGraphic = UI.getChildControl(Panel_Window_cOption, "Frame_Graphic_ScreenShot")
      local frameGraphicContent = frameGraphic:GetFrameContent()
      local _bg2 = UI.getChildControl(frameGraphicContent, "StaticText_BgOrder2_Import")
      local _bg3 = UI.getChildControl(frameGraphicContent, "StaticText_BgOrder3_Import")
      local _bg4 = UI.getChildControl(frameGraphicContent, "StaticText_BgOrder4_Import")
      local _bg5 = UI.getChildControl(frameGraphicContent, "StaticText_BgOrder5_Import")
      _bg2:SetShow(false)
      _bg3:SetShow(false)
      _bg4:SetShow(false)
      _bg5:SetShow(false)
      local frameNation = UI.getChildControl(Panel_Window_cOption, "Frame_Function_Nation")
      local frameNationContent = frameGraphic:GetFrameContent()
      local _bg1 = UI.getChildControl(frameGraphicContent, "StaticText_BgOrder1_Import")
      _bg1:SetShow(false)
    end
    return
  end
  ...
  if false == availableExchangeRefuse and nil ~= self._frames.Function.Etc then
    local bg = UI.getChildControl(self._frames.Function.Etc._uiFrameContent, "StaticText_BgOrder3_Import")
  ...
  local availableUIFontType = isGameServiceTypeDev() or isGameTypeKorea() or isGameTypeEnglish()
  if false == availableUIFontType then
    local bg = UI.getChildControl(self._frames.Function.Convenience._uiFrameContent, "StaticText_BgOrder3_Import")
    bg:SetShow(false)
    self._elements.UIFontType = nil
  end
  if false == _ContentsGroup_isFairy then
  ...
  if _ContentsGroup_isConsoleTest then
    ...
    UI.getChildControl(bg00, "CheckButton_EnableAroundMusicSoundOnly"):SetShow(false)
    UI.getChildControl(bg00, "StaticText_EnableAroundPlayMusic_Desc"):SetShow(false)
    ...
    if false == ToClient_isConsolePlatformSpec() then
  ...
  if true == _ContentsOption_CH_GameType then
    ...
    PaGlobal_Option:SetHideOption("UiProductionNote", true)
    PaGlobal_Option:SetHideOption("UiGuildServant", true)
    PaGlobal_Option:SetHideOption("UiBlackGift", true)
    local bg = UI.getChildControl(self._frames.Interface.UI._uiFrameContent, "StaticText_BgOrder1_Import")
    UI.getChildControl(bg, "StaticText_UI11"):SetShow(false)
    UI.getChildControl(bg, "StaticText_UI26"):SetShow(false)
    UI.getChildControl(bg, "StaticText_UiBlackGift"):SetShow(false)
    local bg0 = UI.getChildControl(self._frames.Function.Alert._uiFrameContent, "StaticText_BgOrder0_Import")
    local bg2 = UI.getChildControl(self._frames.Function.Alert._uiFrameContent, "StaticText_BgOrder2_Import")
    UI.getChildControl(bg0, "CheckButton_AlertRoyalTrade"):SetShow(false)
    UI.getChildControl(bg0, "StaticText_AlertRoyalTrade_Desc"):SetShow(false)
    UI.getChildControl(bg0, "CheckButton_AlertItemMarket"):SetShow(false)
    UI.getChildControl(bg0, "StaticText_AlertItemMarket_Desc"):SetShow(false)
    UI.getChildControl(bg2, "CheckButton_AlertTerritoryWar"):SetShow(false)
    UI.getChildControl(bg2, "StaticText_AlertTerritoryWar_Desc"):SetShow(false)
    local _bg2 = UI.getChildControl(self._frames.Graphic.ScreenShot._uiFrameContent, "StaticText_BgOrder2_Import")
    local _bg3 = UI.getChildControl(self._frames.Graphic.ScreenShot._uiFrameContent, "StaticText_BgOrder3_Import")
    local _bg4 = UI.getChildControl(self._frames.Graphic.ScreenShot._uiFrameContent, "StaticText_BgOrder4_Import")
    local _bg5 = UI.getChildControl(self._frames.Graphic.ScreenShot._uiFrameContent, "StaticText_BgOrder5_Import")
    _bg2:SetShow(false)
    _bg3:SetShow(false)
    _bg4:SetShow(false)
    _bg5:SetShow(false)
    local frameMinusY = _bg2:GetSizeY() + _bg3:GetSizeY() + _bg4:GetSizeY() + _bg5:GetSizeY()
    self._frames.Graphic.ScreenShot._uiFrameContent:SetSize(self._frames.Graphic.ScreenShot._uiFrameContent:GetSizeX(), self._frames.Graphic.ScreenShot._uiFrameContent:GetSizeY() - frameMinusY)
    local _bg1 = UI.getChildControl(self._frames.Function.Nation._uiFrameContent, "StaticText_BgOrder1_Import")
    _bg1:SetShow(false)
    local _bgSound2 = UI.getChildControl(self._frames.Sound.OnOff._uiFrameContent, "StaticText_BgOrder2_Import")
    _bgSound2:SetShow(false)
    local npcVoiceTypeTitle = UI.getChildControl(_bgSound2, "StaticText_Name"):SetShow(false)
    local _bgView7 = UI.getChildControl(self._frames.Function.View._uiFrameContent, "StaticText_BgOrder7_Import")
    _bgView7:SetShow(false)
    local TentTitle = UI.getChildControl(_bgView7, "StaticText_Name"):SetShow(false)
    if false == _ContentsOption_CH_OptionUseAlchemyStone then
      local _bgInterface = UI.getChildControl(self._frames.Interface.Action._uiFrameContent, "StaticText_BgOrder2_Import")
      UI.getChildControl(_bgInterface, "StaticText_UseAlchemyStone"):SetShow(false)
      PaGlobal_Option:SetHideOption("UiAlchemyStone", true)
    end
  end
  if false == _ContentsGroup_VoiceChat then
    local OptionBG = UI.getChildControl(self._frames.Interface.Function._uiFrameContent, "StaticText_BgOrder1_Import")
    UI.getChildControl(OptionBG, "RadioButton_ActionPushToTalk"):SetShow(false)
    UI.getChildControl(OptionBG, "StaticText_Function9"):SetShow(false)
  end
  local OptionBG = UI.getChildControl(self._frames.Performance.Npc._uiFrameContent, "StaticText_BgOrder2_Import")
  OptionBG:SetShow(true)
  if true == _ContentsGroup_AutomaticGuildeSmooth then
    local OptionBG1 = UI.getChildControl(self._frames.Function.Etc._uiFrameContent, "StaticText_BgOrder1_Import")
    local OptionBG2 = UI.getChildControl(self._frames.Function.Etc._uiFrameContent, "StaticText_BgOrder2_Import")
    OptionBG1:SetShow(true)
    OptionBG2:SetShow(true)
  else
    local OptionBG1 = UI.getChildControl(self._frames.Function.Etc._uiFrameContent, "StaticText_BgOrder1_Import")
    local OptionBG2 = UI.getChildControl(self._frames.Function.Etc._uiFrameContent, "StaticText_BgOrder2_Import")
    OptionBG1:SetShow(false)
    OptionBG2:SetShow(false)
    local bg3 = UI.getChildControl(self._frames.Function.Etc._uiFrameContent, "StaticText_BgOrder3_Import")
    local bg4 = UI.getChildControl(self._frames.Function.Etc._uiFrameContent, "StaticText_BgOrder4_Import")
    local padding = OptionBG2:GetPosY() - OptionBG1:GetPosY() - OptionBG1:GetSizeY()
    bg3:SetPosY(bg3:GetPosY() - (OptionBG1:GetSizeY() + OptionBG2:GetSizeY()) - padding * 2)
    bg4:SetPosY(bg4:GetPosY() - (OptionBG1:GetSizeY() + OptionBG2:GetSizeY()) - padding * 2)
  end
  if false == _ContentsGroup_ExpirienceWiki and false == _ContentsGroup_ExpirienceWiki_KorVersion then
    PaGlobal_Option:SetHideOption("UiExpirienceWiki", true)
    local bg6 = UI.getChildControl(self._frames.Interface.UI._uiFrameContent, "StaticText_BgOrder1_Import")
    UI.getChildControl(bg6, "StaticText_UiExpirienceWiki"):SetShow(false)
  end
end
...
function FGlobal_IsChecked_GetSkillComboSetting()
  return PaGlobal_Option:Get("SkillComboGuide")
end
...
local recommendBaseValue = 1080
function PaGlobal_Option:SetRecommendScale()
  local frameBG = UI.getChildControl(self._frames.Graphic.Window._uiFrameContent, "StaticText_BgOrder3_Import")
  local txt_uiSizeDesc = UI.getChildControl(frameBG, "StaticText_Name")
  local screenSizeY = getResolutionSizeY()
  if screenSizeY < recommendBaseValue then
    local recommendUIScale = self:getRecommendUIScale()
    txt_uiSizeDesc:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_NEWGAMEOPTION_UIScale") .. " (" .. PAGetString(Defines.StringSheet_GAME, "LUA_OPTION_RECOMMANDATION") .. " : <PAColor0xffddcd82>" .. recommendUIScale .. "<PAOldColor>)")
  else
    txt_uiSizeDesc:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_NEWGAMEOPTION_UIScale"))
  end
end
function PaGlobal_Option:getRecommendUIScale()
  return math.floor(getResolutionSizeY() / recommendBaseValue * 100)
end
