...
function PaGlobal_Option:ListInit()
  ...
  tree2:registEvent(__ePAUIList2EventType_LuaChangeContent, "FGlobal_Option_ListTreeUpdate")
      ...
      if count > 1 then
        for index2, frame in pairs(_detailOrder[index]) do
          ...
          if true == self:CheckAvailableDetail(category, detail) then
            ...
            if ToClient_isPS5() == true and string.find(category, "Interface") ~= nil and string.find(detail, "Pad") ~= nil then
              self._list2._tree2IndexMap[keyIndex]._string = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_PLAYSTATION5_PADNAME")
            end
...
function FromClient_Option_PadSnapChangeTarget(fromControl, toControl)
  ...
  if nil ~= toControl then
    if toControl:GetType() == __ePAUIControl_Slider then
...
function FGlobalFunc_GameOption_SetSkillCommand(isShow)
...
function GameOption_ShowFriendLoginMessage()
  ...
  local gameOptionSetting = ToClient_getGameOptionControllerWrapper()
  return PaGlobal_Option._elements.ShowFriendLoginMessage:get(gameOptionSetting)
end
