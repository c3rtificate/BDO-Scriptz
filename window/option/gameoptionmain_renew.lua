...
function PaGlobal_Option:SetTitleAndDescription(groupBg, option)
  ...
  if nil ~= option._title then
    title:SetTextMode(__eTextMode_AutoWrap)
  ...
  for index, option in ipairs(frameInfo) do
    ...
    if nil ~= group then
      ...
      if nil ~= option._desc then
        ...
        txt_Desc:SetText(PAGetString(Defines.StringSheet_RESOURCE, option._desc))
      end
      if nil == txt_Desc:GetText() or "" == txt_Desc:GetText() then
        txt_Desc:SetTextMode(__eTextMode_AutoWrap)
        txt_Desc:SetText(PAGetString(Defines.StringSheet_GAME, option._desc))
      end
      if txt_Desc:GetSizeY() < txt_Desc:GetTextSizeY() then
        local sizeY = txt_Desc:GetTextSizeY() - txt_Desc:GetSizeY()
        txt_Desc:SetSize(txt_Desc:GetSizeX(), txt_Desc:GetSizeY() + sizeY)
        group:SetSize(group:GetSizeX(), group:GetSizeY() + sizeY + 20)
      end
...
function PaGlobal_Option:SetControlSetting(option, value)
  if OPTION_TYPE.CHECKBUTTON == option._type then
    ...
    local checkButtonName = UI.getChildControl(checkButton, "Button_Name")
    if checkButton:IsCheck() then
      if nil ~= option._checkButtonMapping and nil ~= option._checkButtonMapping[0] and nil ~= option._checkButtonMapping[0]._string then
        checkButtonName:SetText(PAGetString(Defines.StringSheet_RESOURCE, option._checkButtonMapping[0]._string))
      else
        checkButtonName:SetText(PaGlobal_Option._radioButtonOnString)
      end
    elseif nil ~= option._checkButtonMapping and nil ~= option._checkButtonMapping[1] and nil ~= option._checkButtonMapping[1]._string then
      checkButtonName:SetText(PAGetString(Defines.StringSheet_RESOURCE, option._checkButtonMapping[1]._string))
    else
      checkButtonName:SetText(PaGlobal_Option._radioButtonOffString)
    end
...
function FGlobal_Option_luaLoadComplete()
  ...
  FGlobal_Option_ResetUnUseOption()
...
function FGlobal_Option_ResetUnUseOption()
  setMaidView(0)
  local WorldMapAnimationStyle_NoAnimation = 3
  setWorldmapOpenType(WorldMapAnimationStyle_NoAnimation)
end
