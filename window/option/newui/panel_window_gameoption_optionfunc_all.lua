...
function FromClient_ReturnGraphicOptionByStartSequence(graphicValue, textureQuality)
  local graphicOption = graphicValue
  local textureOption = textureQuality
  PaGlobal_GameOption_All:SetCurrentValue("RadioButton_GraphicOption", 0, graphicOption)
  PaGlobal_GameOption_All:SetCurrentValue("RadioButton_TextureQuality", 0, textureQuality)
  PaGlobal_GameOption_All:ClickedConfirmOption()
end
function FromClient_ChangeGraphicOptionByStartSequence()
  PaGlobal_GameOption_All:SetCurrentValue("RadioButton_GraphicOption", 0, 7)
  PaGlobal_GameOption_All:SetCurrentValue("RadioButton_TextureQuality", 0, 2)
  PaGlobal_GameOption_All:ClickedConfirmOption()
end
registerEvent("FromClient_ChangeGraphicOptionByStartSequence", "FromClient_ChangeGraphicOptionByStartSequence")
registerEvent("FromClient_ReturnGraphicOptionByStartSequence", "FromClient_ReturnGraphicOptionByStartSequence")
...
function PaGlobal_GameOption_All._elements.GraphicOption:EventException(value, beforeValue)
  ...
  if nil ~= option._curValue then
    ...
    if GraphicEnum.UltraHigh == value then
      ...
      MessageBox.showMessageBox(messageBoxData)
    elseif value == GraphicEnum.UltraLow and ToClient_GetSequenceRemasterOption() == false then
      local messageBoxData = {
        title = PAGetString(Defines.StringSheet_GAME, "LUA_OPTION_GRAPHICMODE_ALERTTITLE"),
...
function PaGlobal_GameOption_SetValue(controlName, cacheNo, order)
  PaGlobal_GameOption_All:SetCurrentValue(controlName, cacheNo, order)
end
