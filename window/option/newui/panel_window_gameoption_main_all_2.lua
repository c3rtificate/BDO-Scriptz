...
function PaGlobal_GameOption_All_GetSkillComboSetting()
  return PaGlobal_GameOption_All:getOptionValue("SkillComboGuide")
end
...
function PaGlobal_GameOption_All_ChangeDisplayApply()
  ...
  saveGameOption(true)
  reloadGameUI()
  if ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(__eUiInterfaceSizeMessagePanel) == false and ToClient_GetUIScale() == PaGlobal_GameOption_All._prevUIScale and (PaGlobal_GameOption_All._prevResolutionX ~= getResolutionSizeX() or PaGlobal_GameOption_All._prevResolutionY ~= getResolutionSizeY()) then
...
function PaGlobal_GameOption_All_isHideFullUiByAttacked()
  local gameOptionSetting = ToClient_getGameOptionControllerWrapper()
  if nil ~= gameOptionSetting then
    local isHide = gameOptionSetting:getHideFullUiByAttacked()
    return isHide
  end
  return true
end
function PaGlobal_GameOption_All_SetOtherPanelWhenKeyChange()
  if PaGlobal_GalleyBoost_All_setKeyChange ~= nil then
    PaGlobal_GalleyBoost_All_setKeyChange()
  end
end
