...
function PaGlobal_GameOption_All:Close()
  ...
  if true == PaGlobal_GameOption_All._isShortcutOpen then
    FGlobal_KeyboardHelpShow()
    PaGlobal_GameOption_All._isShortcutOpen = false
  end
end
...
function PaGlobal_GameOption_All_GameOptionShortCutOpen(isShortcutOpen)
  if nil == isShortcutOpen then
    PaGlobal_GameOption_All._isShortcutOpen = false
  end
  if false == PaGlobal_GameOption_All:isOpen() then
    showGameOption()
  end
  PaGlobal_GameOption_All._isShortcutOpen = isShortcutOpen
  PaGlobal_GameOption_All:movePage(PaGlobal_GameOption_All.PAGE.Category)
  PaGlobal_GameOption_All:GoCategory("Interface", "UI")
  KeyboardHelpHide()
end
...
function PaGlobal_GameOption_All_ShowHomeTooltip(isShow, tooltipName)
  ...
  if true == isShow then
    TooltipSimple_CommonShow(tooltipData.control:GetText(), tooltipData.name)
...
function PaGlobal_GameOption_All_ShowPictureToolTip(isShow, elementName)
  ...
  local parentPosY = PaGlobal_GameOption_All._ui.stc_pictureTooltip:GetParentPosY()
  local sizeY = PaGlobal_GameOption_All._ui.stc_pictureTooltip:GetSizeY()
  local gapY = PaGlobal_GameOption_All._ui.stc_pictureTooltip:GetPosY()
  if screenY < parentPosY + sizeY then
    gapY = gapY - (parentPosY + sizeY - screenY) - 5
  end
  PaGlobal_GameOption_All._ui.stc_pictureTooltip:SetPosY(gapY)
end
...
function PaGlobal_GameOption_All:ChangeUpScaleModeDesc(isShow, cacheNo, elementName, index)
  if nil == isShow then
    isShow = false
  end
  if "UpscaleEnable" ~= elementName then
    return
  end
  local groupBg = self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.RADIOVERTICAL][cacheNo]
  local desc = UI.getChildControlNoneAssert(groupBg, "StaticText_Desc")
  if nil == desc then
    return
  end
  local FSRText = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_NEWGAMEOPTION_GRAPHIC_QUALITY_UpscaleFSREnable_DESC")
  local descText = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_NEWGAMEOPTION_GRAPHIC_QUALITY_UpscaleEnable_DESC")
  if true == isShow then
    if nil ~= index and 2 == index then
      descText = FSRText
    end
  else
...
function GameOption_ShowFriendLoginMessage()
  if ToClient_cutsceneIsPlay() == true then
    return false
  end
  local gameOptionSetting = ToClient_getGameOptionControllerWrapper()
  return PaGlobal_GameOption_All._elements.ShowFriendLoginMessage:get(gameOptionSetting)
end
