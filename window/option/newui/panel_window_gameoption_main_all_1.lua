function PaGlobal_GameOption_All:ClickedMainCategory(key, category)
  ...
  if key + 1 then
    self._list2._selectedSubKey = key + 1
    self:SelectOptionFrame(category, onlyOnDetail)
  end
...
function FGlobal_Option_ResetFrame()
  ...
  for index, option in ipairs(resetElements) do
    if nil ~= option._element then
      ...
      elseif nil ~= curOption._defaultValue then
        ...
        if self._elements.GraphicOption == curOption or self._elements.AntiAliasing == curOption then
          isGraphicChanged = true
        end
  ...
  if true == _ContentsGroup_UseAMDFSR and true == isGraphicChanged then
    ...
    local graphicOption = PaGlobal_GameOption_All:getOptionValue("GraphicOption")
    if graphicOption >= PaGlobal_GameOption_All.GRAPHIC.UltraLow then
      isGraphicReady = true
    end
    local AAValue = PaGlobal_GameOption_All:getOptionValue("AntiAliasing")
    if AAValue >= 2 then
      isTAAReady = true
    end
    if false == isGraphicReady or false == AAValue then
      local upscaleType = PaGlobal_GameOption_All._UpScaleType.OFF
...
function PaGlobal_GameOption_All:ClickedApplyOption()
  ...
  if true == displayChange then
    refreshAllSubAppWindowSize()
    self:DisplayChanged()
    self:Close()
    return
  end
  if nil ~= QuickSlot_OnceUpdateByGameOption then
    QuickSlot_OnceUpdateByGameOption()
  end
  ...
  self:ApplyButtonEnable(false)
  ClearFocusEdit()
  PaGlobal_GameOption_All_SetOtherPanelWhenKeyChange()
end
function PaGlobal_GameOption_All:ClickedConfirmOption()
  if isNeedGameOptionFromServer() == true then
    if FGlobal_QuestWindowRateSetting ~= nil then
...
function PaGlobal_GameOption_All:ClickedSpecSetting(value)
  PaGlobalFunc_GameModeSettingNotice_All_Open(value)
...
function PaGlobal_GameOption_All_GetHomeSpecName(value)
  if nil == value then
    return ""
  end
  local specName = ""
  if value <= 4 then
    specName = PaGlobal_GameOption_All._homeTooltip["Spec" .. tostring(value)].control:GetText()
  elseif value <= 8 then
    specName = PaGlobal_GameOption_All._homeTooltip["War" .. tostring(value - 4)].control:GetText()
  end
  return specName
end
function PaGlobal_GameOption_All_GetSpecSetting(value)
  if nil == value then
    return nil
  end
  if value < PaGlobal_GameOption_All.SPEC.LowNormal or value > PaGlobal_GameOption_All.SPEC.HighestSiege then
    return nil
  end
  if nil == PaGlobal_GameOption_All._specSettingOption or nil == PaGlobal_GameOption_All._specSettingOption[value] then
    return nil
  end
  return PaGlobal_GameOption_All._specSettingOption[value]
end
