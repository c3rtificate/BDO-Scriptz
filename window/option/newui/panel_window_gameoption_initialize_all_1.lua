function PaGlobal_GameOption_All:CreateFrame(category, detail, searchElement)
  ...
  local subRadioButtonCacheNo = 0
  local subCheckButtonCacheNo = 0
  local etcPageButtonCacheNo = 0
    ...
    if nil ~= option._element then
      if (true == option._element._isShow or nil == option._element._isShow) and (true == option._contentsOption or nil == option._contentsOption) then
        ...
        elseif __ePAUIControl_RadioButton == option._element._controlType then
          ...
          elseif true == option._element._isSubRadio then
            eventGroup = self:CreateControl_SubRadioButton(subRadioButtonCacheNo, index, option._element)
            subRadioButtonCacheNo = subRadioButtonCacheNo + 1
          else
        ...
        elseif __ePAUIControl_CheckButton == option._element._controlType then
          if true == option._element._isSubCheck then
            eventGroup = self:CreateControl_SubCheckButton(subCheckButtonCacheNo, index, option._element)
            subCheckButtonCacheNo = subCheckButtonCacheNo + 1
          else
        ...
        elseif __ePAUIControl_ComboBox == option._element._controlType then
          ...
          if __ePAUIControl_Button == option._element._controlType then
            if option._element._name == "EtcOpenBrowser" then
              eventGroup = self:CreateControl_EtcPageButton(etcPageButtonCacheNo, index, option._element)
              etcPageButtonCacheNo = etcPageButtonCacheNo + 1
            else
...
function PaGlobal_GameOption_All:AfterFixFrameException(category, detail)
  if "GraphicQuality" == detail or "Quality" == detail then
    ...
    local upscaleTypeValue = PaGlobal_GameOption_All._elements.UpscaleEnable._curValue
    if nil == upscaleTypeValue then
      upscaleTypeValue = PaGlobal_GameOption_All:getOptionValue("UpscaleEnable")
    end
    if true == _ContentsGroup_UseAMDFSR then
      local UpscaleFSRTypeOption = PaGlobal_GameOption_All._elements.UpscaleFSRType
      if PaGlobal_GameOption_All._UpScaleType.OFF == upscaleTypeValue or PaGlobal_GameOption_All._UpScaleType.UPSCALE == upscaleTypeValue then
...
function PaGlobal_GameOption_All:SetTitleAndDescription(groupBg, option)
  ...
  if option._controlType == __ePAUIControl_RadioButton then
    if nil ~= option.uiInputType or nil ~= option.actionInputType then
      ...
      if "WatermarkPosition" == option._name or "ShowNavPathEffectType" == option._name or "WorldMapOpenType" == option._name or "UpscaleFSRType" == option._name then
        local radioButtonSizeX = 230
        local radioButtonSizeY = 50
        local startPosX = 20
        local radioButtonTemplate = UI.getChildControl(groupBg, "RadioButton_0")
        if nil ~= option._isSubRadio and true == option._isSubRadio then
          startPosX = startPosX + radioButtonTemplate:GetSpanSize().x
          if "UpscaleFSRType" == option._name then
        ...
        for ii = 0, option._radioButtonMapping._count - 1 do
          ...
          radioButton:SetPosX(startPosX + xIndex * radioButtonSizeX)
          radioButton:SetPosY(startPosY + yIndex * radioButtonSizeY)
        end
      end
      ...
      if "UpscaleEnable" == option._name then
        local radioButton = UI.getChildControl(groupBg, "RadioButton_0")
        desc:SetHorizonRight()
        desc:ComputePos()
        desc:SetSize(groupBg:GetSizeX() * 0.5 - 30, radioButton:GetSizeY() * option._radioButtonMapping._count + 5)
        desc:SetPosX(groupBg:GetSizeX() * 0.5 + 30)
        desc:SetPosY(radioButton:GetPosY())
        local upscaleTypeValue = PaGlobal_GameOption_All._elements.UpscaleEnable._curValue
        if nil == upscaleTypeValue then
          upscaleTypeValue = PaGlobal_GameOption_All:getOptionValue("UpscaleEnable")
        end
        local upScaleTxt = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_NEWGAMEOPTION_GRAPHIC_QUALITY_UpscaleEnable_DESC")
        if PaGlobal_GameOption_All._UpScaleType.AMDFSR == upscaleTypeValue then
          upScaleTxt = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_NEWGAMEOPTION_GRAPHIC_QUALITY_UpscaleFSREnable_DESC")
        end
        desc:SetText(upScaleTxt)
      end
    end
  ...
  elseif option._controlType == __ePAUIControl_Button then
    if option._name == "EtcOpenBrowser" then
      local button1 = UI.getChildControl(groupBg, "Button_Template1")
      local button2 = UI.getChildControl(groupBg, "Button_Template2")
      local button3 = UI.getChildControl(groupBg, "Button_Template3")
      local button4 = UI.getChildControl(groupBg, "Button_Template4")
      button1:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_NEWGAMEOPTION_CH_OpenWebBrowser_0"))
      button2:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_NEWGAMEOPTION_CH_OpenWebBrowser_1"))
      button3:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_NEWGAMEOPTION_CH_OpenWebBrowser_2"))
      button4:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_NEWGAMEOPTION_CH_OpenWebBrowser_3"))
    else
      local button1 = UI.getChildControl(groupBg, "Button_Template1")
      local button2 = UI.getChildControl(groupBg, "Button_Template2")
    ...
    if nil == option.uiInputType and nil == option.actionInputType then
      for ii = 0, option._radioButtonMapping._count do
        ...
        if nil ~= control then
          ...
          elseif "ScreenShotQuality" ~= option._name and "GraphicOption" ~= option._name and "UpscaleEnable" ~= option._name then
...
function PaGlobal_GameOption_All:CreateControl_RadioButton(cacheNo, order, option)
  ...
  for ii = 0, MaxRadioButton do
    if ii < option._radioButtonMapping._count then
      ...
      if option._name == "PetRender" and isAbyssOne == true then
        radioButton:SetMonoTone(true)
        radioButton:SetIgnore(true)
      end
      if option._name == "FairyRender" and isAbyssOne == true then
        radioButton:SetMonoTone(true)
        radioButton:SetIgnore(true)
      end
...
function PaGlobal_GameOption_All:CreateControl_RadioButtonVertical(cacheNo, order, option)
  ...
  for ii = 0, MaxRadioButton do
    if ii < option._radioButtonMapping._count then
      ...
      if "UpscaleEnable" == option._name then
        radioButton:addInputEvent("Mouse_On", "PaGlobal_GameOption_All:ChangeUpScaleModeDesc(true, " .. tostring(cacheNo) .. ", \"" .. option._name .. "\"," .. tostring(ii) .. ")")
        radioButton:addInputEvent("Mouse_Out", "PaGlobal_GameOption_All:ChangeUpScaleModeDesc(false, " .. tostring(cacheNo) .. ", \"" .. option._name .. "\"," .. tostring(ii) .. ")")
      end
    else
      local radioButton = UI.getChildControlNoneAssert(self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.RADIOVERTICAL][cacheNo], "RadioButton_" .. tostring(ii))
      if nil == radioButton then
...
function PaGlobal_GameOption_All:CreateControl_SubCheckButton(cacheNo, order, option)
  if nil == self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.SUBCHKBUTTON][cacheNo] then
    local groupTemplate = UI.getChildControl(self._ui._frameContent, "Static_TemplateSubCheckBtnGroup")
    local group = UI.cloneControl(groupTemplate, self._ui._frameContent, "Static_SubCheckButtonGroup" .. tostring(cacheNo))
    self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.SUBCHKBUTTON][cacheNo] = group
  end
  local checkButtonTemplate = UI.getChildControl(self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.SUBCHKBUTTON][cacheNo], "CheckButton_Template")
  ...
  local title = UI.getChildControl(self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.SUBCHKBUTTON][cacheNo], "StaticText_Title")
  local desc = UI.getChildControl(self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.SUBCHKBUTTON][cacheNo], "StaticText_Desc")
  title:SetMonoTone(false)
  desc:SetMonoTone(false)
  return self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.SUBCHKBUTTON][cacheNo]
end
function PaGlobal_GameOption_All:CreateControl_SubRadioButton(cacheNo, order, option)
  if nil == self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.SUBRADIOBUTTON][cacheNo] then
    local groupTemplate = UI.getChildControl(self._ui._frameContent, "Static_TemplateSubRadioBtnGroup")
    local group = UI.cloneControl(groupTemplate, self._ui._frameContent, "Static_SubRadioButtonGroup" .. tostring(cacheNo))
    self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.SUBRADIOBUTTON][cacheNo] = group
  end
  local groupTemplate = UI.getChildControl(self._ui._frameContent, "Static_TemplateSubRadioBtnGroup")
  local radioButtonTemplate = UI.getChildControl(groupTemplate, "RadioButton_Template")
  local MaxRadioButton = 20
  ...
  for ii = 0, MaxRadioButton do
    if ii < option._radioButtonMapping._count then
      ...
      local yIndex = math.floor(ii / 3)
      lastYIndex = yIndex
      local radioButton = UI.getChildControlNoneAssert(self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.SUBRADIOBUTTON][cacheNo], "RadioButton_" .. tostring(ii))
      if nil == radioButton then
        radioButton = UI.cloneControl(radioButtonTemplate, self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.SUBRADIOBUTTON][cacheNo], "RadioButton_" .. tostring(ii))
      end
      radioButton:addInputEvent("Mouse_LUp", "PaGlobal_GameOption_All:SetCurrentValue( \"" .. tostring("RadioButton_" .. option._name) .. "\", " .. tostring(cacheNo) .. ", " .. tostring(ii) .. ")")
      local radioButtonSizeX = 220
      local radioButtonSizeY = 50
      ...
      radioButton:SetMonoTone(false)
      radioButton:SetIgnore(false)
      radioButton:SetGroup(30 + cacheNo)
      if "ScreenShotQuality" == option._name then
        radioButton:addInputEvent("Mouse_On", "PaGlobal_GameOption_All:ScreenShotQualityTooltip(true," .. tostring(cacheNo) .. "," .. tostring(ii) .. ", \"" .. option._name .. "\")")
        radioButton:addInputEvent("Mouse_Out", "PaGlobal_GameOption_All:ScreenShotQualityTooltip(false," .. tostring(cacheNo) .. "," .. tostring(ii) .. ", \"" .. option._name .. "\")")
      end
  ...
  self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.SUBRADIOBUTTON][cacheNo]:SetSize(groupTemplate:GetSizeX(), groupTemplate:GetSizeY() + 50 * lastYIndex)
  return self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.SUBRADIOBUTTON][cacheNo]
end
...
function PaGlobal_GameOption_All:CreateControl_PageButton(cacheNo, order, option)
  local templateName = ""
  local buttonGroupName = ""
  templatename = "Static_TemplatePageBtnGroup"
  buttonGroupName = "Static_PageButtonGroup"
  if nil == self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.BUTTON][cacheNo] then
    local groupTemplate = UI.getChildControl(self._ui._frameContent, templatename)
    local group = UI.cloneControl(groupTemplate, self._ui._frameContent, buttonGroupName .. tostring(cacheNo))
...
function PaGlobal_GameOption_All:CreateControl_EtcPageButton(cacheNo, order, option)
  local templateName = ""
  local buttonGroupName = ""
  templatename = "Static_TemplateEtcOpenBrowserBtnGroup"
  buttonGroupName = "Static_EtcPageButtonGroup"
  if nil == self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.ETCBUTTON][cacheNo] then
    local groupTemplate = UI.getChildControl(self._ui._frameContent, templatename)
    local group = UI.cloneControl(groupTemplate, self._ui._frameContent, buttonGroupName .. tostring(cacheNo))
    self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.ETCBUTTON][cacheNo] = group
  end
  local buttonTemplate1 = UI.getChildControl(self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.ETCBUTTON][cacheNo], "Button_Template1")
  local buttonTemplate2 = UI.getChildControl(self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.ETCBUTTON][cacheNo], "Button_Template2")
  local buttonTemplate3 = UI.getChildControl(self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.ETCBUTTON][cacheNo], "Button_Template3")
  local buttonTemplate4 = UI.getChildControl(self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.ETCBUTTON][cacheNo], "Button_Template4")
  buttonTemplate1:addInputEvent("Mouse_LUp", "PaGlobal_GameOption_All:EtcOpenBrowser(1)")
  buttonTemplate2:addInputEvent("Mouse_LUp", "PaGlobal_GameOption_All:EtcOpenBrowser(2)")
  buttonTemplate3:addInputEvent("Mouse_LUp", "PaGlobal_GameOption_All:EtcOpenBrowser(3)")
  buttonTemplate4:addInputEvent("Mouse_LUp", "PaGlobal_GameOption_All:EtcOpenBrowser(4)")
  buttonTemplate1:SetShow(true)
  buttonTemplate2:SetShow(true)
  buttonTemplate3:SetShow(true)
  buttonTemplate4:SetShow(true)
  return self._controlCache[PaGlobal_GameOption_All.OPTION_TYPE.ETCBUTTON][cacheNo]
end
