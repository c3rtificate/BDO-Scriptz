...
function PaGlobal_GameOption_All_InitCheckUIScale()
  ...
  if Panel_CharacterInfo_All ~= nil then
...
function PaGlobal_GameOption_All:controlInit()
  ...
  local specIcon = UI.getChildControl(self._ui.stc_firstBG, "StaticText_Spec")
  self._ui.txt_SpecTitle = UI.getChildControl(specIcon, "StaticText_Title")
  local warIcon = UI.getChildControl(self._ui.stc_secondBG, "StaticText_War")
  self._ui.txt_WarTitle = UI.getChildControl(warIcon, "StaticText_Title")
  local personalIcon = UI.getChildControl(self._ui.stc_thirdBG, "StaticText_Personal")
  self._ui.txt_Personal = UI.getChildControl(personalIcon, "StaticText_Title")
  self._ui.txt_SpecTitle:SetTextMode(__eTextMode_AutoWrap)
  self._ui.txt_SpecTitle:SetText(self._ui.txt_SpecTitle:GetText())
  self._ui.txt_WarTitle:SetTextMode(__eTextMode_AutoWrap)
  self._ui.txt_WarTitle:SetText(self._ui.txt_WarTitle:GetText())
  self._ui.txt_Personal:SetTextMode(__eTextMode_AutoWrap)
  self._ui.txt_Personal:SetText(self._ui.txt_Personal:GetText())
  self._ui.btn_spec = {}
  self._ui.btn_war = {}
...
function PaGlobal_GameOption_All:registEventHandler()
  ...
  if ToClient_isInAbyssOne() == true then
    self._ui.btn_saveSetting:SetEnable(false)
    self._ui.btn_saveSetting:SetMonoTone(true)
  else
    self._ui.btn_saveSetting:SetEnable(true)
    self._ui.btn_saveSetting:SetMonoTone(false)
...
function PaGlobal_GameOption_All:setDefaultValue()
  ...
  PaGlobal_GameOption_All._elements.RemasterUI._defaultValue = false == ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(__eSwapRemasterUISetting)
end
...
function PaGlobal_GameOption_All:setContentsOption()
  ...
  if false == _ContentsGroup_FamilyInventory then
    self._elements.UiFamilyInven._isShow = false
  end
  ...
  if true == ToClient_isAvailableChangeServiceType() then
    if isGameTypeThisCountry(CppEnums.ContryCode.eContryCode_NA) or true == isGameTypeSA() or true == isGameTypeTH() or true == isGameTypeID() or true == isGameTypeASIA() or CppEnums.GameServiceType.eGameServiceType_DEV == getGameServiceType() then
  ...
  elseif nil ~= self._frames.Function.Nation then
    ...
    if true == isGameTypeTH() or true == isGameTypeASIA() then
      self._elements.UseChattingFilter._isShow = true
    end
  end
  if true == isGameTypeSA() or true == isGameTypeRussia() or true == _ContentsOption_CH_GameType then
  ...
  if _ContentsGroup_isConsoleTest then
    ...
    self._elements.AlertWaitItemMarket._isShow = false
    self._elements.AlertHp._isShow = false
    ...
    self._elements.UsePadRenewerUI._isShow = false
  ...
  if ToClient_GetOnStartCloudGame() == true then
    self._elements.ScreenMode._isShow = false
    self._elements.ScreenResolution._isShow = false
    self._elements.GraphicOption._isShow = false
    self._elements.Dof._isShow = false
    self._elements.AntiAliasing._isShow = false
    self._elements.SSAO._isShow = false
    self._elements.Tessellation._isShow = false
    self._elements.TextureQuality._isShow = false
    self._elements.UIScale._isShow = false
    self._elements.PostFilter._isShow = false
    self._elements.Representative._isShow = false
    self._elements.CharacterEffect._isShow = false
    self._elements.SelfPlayerOnlyEffect._isShow = false
    self._elements.NearestPlayerOnlyEffect._isShow = false
    self._elements.UpscaleEnable._isShow = false
    self._elements.SnowPoolOnlyInSafeZone._isShow = false
    self._elements.AutoOptimizationFrameLimit._isShow = false
  end
end
...
function PaGlobal_GameOption_All:ListInit()
  ...
  if true == isNeedGameOptionFromServer() then
    ...
    elseif true == _ContentsOption_CH_GameType then
      ...
      table.remove(self._frames.Interface.UI, 29)
      table.remove(self._frames.Interface.UI, 31)
    else
  ...
  elseif true == _ContentsOption_CH_GameType then
    ...
    table.remove(self._frames.Interface.Action, 13)
    table.remove(self._frames.Interface.UI, 4)
    table.remove(self._frames.Interface.UI, 29)
    table.remove(self._frames.Interface.UI, 31)
  else
...
function PaGlobal_GameOption_All:CheckAvailableDetail(category, detail)
  if PaGlobal_GameOption_All._frames[category][detail] == nil then
    return false
  end
  local count = 0
