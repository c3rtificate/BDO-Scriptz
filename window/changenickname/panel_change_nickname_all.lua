...
function PaGlobal_ChangeNickName_All:initailze()
  ...
  if true == _ContentsGroup_ChinaFontColor then
    self._ui.stc_Illust:ChangeTextureInfoName("new_ui_common_forlua/window/lobby/lobby_accountnickname_bg.dds")
    local x1, y1, x2, y2 = setTextureUV_Func(self._ui.stc_Illust, 0, 0, 200, 252)
    self._ui.stc_Illust:getBaseTexture():setUV(x1, y1, x2, y2)
    self._ui.stc_Illust:setRenderTexture(self._ui.stc_Illust:getBaseTexture())
  end
...
function PaGlobal_ChangeNickName_All:registEventHandler(isConsole)
  ...
  registerEvent("FromClient_QuickChangeUserNickNameAck", "FromClient_QuickChangeUserNickNameAck")
...
function PaGlobal_ChangeNickName_All:prepareClose()
  ...
  PaGlobal_ChangeNickName_All._isCreate = false
  ClearFocusEdit()
  CheckChattingInput()
...
function HandleEventLUp_ChangeNickNameAll_ChangeNickName()
  ...
  if false == PaGlobal_ChangeNickName_All._isValidChangeName or userInput ~= PaGlobal_ChangeNickName_All._checkedValidChangeName then
    if _ContentsGroup_UsePadSnapping == true then
      PaGlobal_ChangeNickName_All_UnSetFocusOnEditBox()
    end
    local messageBoxData = {
      title = msgTitle,
      content = PAGetString(Defines.StringSheet_GAME, "LUA_DUPLICATIONNAME_CHECK_ALARM"),
      functionApply = PaGlobal_ChangeNickName_All_SetFocusOnEditBox,
  ...
  local messageBoxData = {
    ...
    functionNo = PaGlobal_ChangeNickName_All_SetFocusOnEditBox,
    priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
  }
  if false == PaGlobal_ChangeNickName_All._isConsole then
...
function HandleEventLUp_ChangeNickNameAll_CheckOverlap()
  ...
  ToClient_CheckOverlapChangeName(userInput, nameType, PaGlobal_ChangeNickName_All._isCreate, false)
end
...
function FromClient_ChangeNickname_All_Show(param)
  ...
  if changeNameType._characterName == param then
    ...
    if isGameTypeKorea() or isGameTypeTaiwan() or isGameTypeTH() or isGameTypeASIA() or isGameTypeRussia() or isGameTypeID() or isGameTypeTR() or isGameTypeJapan() or isGameTypeEnglish() then
...
<PAColor0xFFF26A6A>]] .. PAGetString(Defines.StringSheet_GAME, "LUA_MAKENAME_WARNING") .. "<PAOldColor>"
  ...
  elseif changeNameType._guildName == param then
    if _ContentsGroup_GuildAuthoritySeparation == true then
      local isRight = PaGlobalFunc_Guild_Authorization_RightCheck(__eGuildRightNew_SetGuildNameMark)
      if isRight == false then
        PaGlobalFunc_Guild_Authorization_ShowErrMessage(__eGuildRightNew_SetGuildNameMark)
        return
      end
    else
...
<PAColor0xFFF26A6A>]] .. PAGetString(Defines.StringSheet_GAME, "LUA_MAKENAME_WARNING") .. "<PAOldColor>"
    elseif true == PaGlobal_ChangeNickName_All._isConsole then
      isNameChangeText = PAGetString(Defines.StringSheet_GAME, "LUA_CHANGENAME_CONSOLE_TEXTBIND")
    else
      isNameChangeText = defaultNameChangeText
    end
    txt_Condition:SetText(isNameChangeText)
...
function FromClient_ChangeNickname_All_NotifyChangeNameResult(type, isCreate, isPossible)
  if true == isCreate and type ~= 2 then
    return
  end
  local messageBoxMemo = ""
  local msgTitle = ""
...
function FromClient_QuickChangeUserNickNameAck()
...
function PaGlobal_ChangeNickName_All_OpenQuickNameChange()
  ...
  PaGlobal_ChangeNickName_All._isCreate = true
...
function PaGlobal_ChangeNickName_All_UnSetFocusOnEditBox()
  if Panel_Change_Nickname_All == nil or Panel_Change_Nickname_All:GetShow() == false then
    return
  end
  ClearFocusEdit(PaGlobal_ChangeNickName_All._ui.edit_Nickname)
end
function PaGlobal_ChangeNickName_All_SetFocusOnEditBox()
  if Panel_Change_Nickname_All == nil or Panel_Change_Nickname_All:GetShow() == false then
    return
  end
  if _ContentsGroup_UsePadSnapping == true then
