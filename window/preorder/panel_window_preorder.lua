...
function preOrder:open(parentPanel)
  if nil ~= parentPanel then
    ...
    if nil ~= selfPlayer then
      isSeasonCharacter = selfPlayer:get():isSeasonCharacter()
    end
    if true == _ContentsGroup_SeasonContents and true == isSeasonCharacter then
      parentPanel:registerPadEvent(__eConsoleUIPadEvent_RTPress_X, "PaGlobalFunc_PreOrder_OpenSeasonPass()")
      _panel:ChangeTextureInfoName("Combine/CommonIcon/Console_SeasonServer_Banner.dds")
...
function PaGlobalFunc_PreOrder_OpenSeasonPass()
  if nil ~= PaGlobalFunc_InventoryInfo_GetShow and true == PaGlobalFunc_InventoryInfo_GetShow() then
