...
local Achievement = {
  _ui = {
    ...
    _stc_stamp = nil,
    _stc_stamp2 = nil,
    _btn_pagePrevious = nil,
    _btn_pagePreview = nil,
    _stc_keyGuide = nil,
    _stc_objectiveBG = {},
    _stc_objectiveBG2 = {},
    _txt_objective = {},
...
function Achievement:initialize()
  self._ui._btn_pagePrevious = UI.getChildControl(_panel, "Button_PagePrevious")
  self._ui._btn_pagePreview = UI.getChildControl(_panel, "Button_PagePreview")
  ...
  self._ui._btn_pagePrevious:addInputEvent("Mouse_LUp", "input_Achievement_LUp_PageChange(false)")
  self._ui._btn_pagePrevious:addInputEvent("Mouse_UpScroll", "input_Achievement_LUp_PageChange(false)")
  self._ui._btn_pagePrevious:addInputEvent("Mouse_DownScroll", "input_Achievement_LUp_PageChange(true)")
  self._ui._btn_pagePreview:addInputEvent("Mouse_LUp", "input_Achievement_LUp_PageChange(true)")
  self._ui._btn_pagePreview:addInputEvent("Mouse_UpScroll", "input_Achievement_LUp_PageChange(false)")
  self._ui._btn_pagePreview:addInputEvent("Mouse_DownScroll", "input_Achievement_LUp_PageChange(true)")
...
function HandlerEventLUp_Achievement_BookMark(isOpen)
  local bookGroup, bookChapter = PaGlobalFunc_Achievement_BookShelf_All_GetBookInfoForBookmark()
...
function PaGlobalFunc_Achievement_OpenBookShelfQuest(questGroup, questNo)
  ...
  local journalKey = ToClient_GetJournalKeyByQuestNo(questGroup, questNo)
  if journalKey == nil or journalKey < 0 then
    return
  end
  local chapterKey = ToClient_GetChapterKeyByQuestNo(questGroup, questNo)
  if chapterKey == nil or chapterKey < 0 then
    return
  end
  local journalType = ToClient_GetJournalType(journalKey, chapterKey)
  ...
  elseif journalType == __eQuestJournalType_MorningLand then
    ...
    local pageIndex = ToClient_GetJournalBookPageIndex(journalKey, chapterKey, questGroup, questNo)
    PaGlobalFunc_Achievement_MorningLand_Open(journalKey, chapterKey, pageIndex)
  else
...
function Achievement:open_from_bookShelf(questGroup)
  ...
  if Panel_Window_Achievement_BookShelf_All:GetShow() and _ContentsGroup_UsePadSnapping then
...
function Achievement:setCurrentPage()
  ...
  for index = 0, #stringList - 1 do
    ...
    if true == self._nationKoreaCheck then
      ...
      self._ui._stc_questImage:setRenderTexture(self._ui._stc_questImage:getBaseTexture())
      self._ui._frm_frame:UpdateContentPos()
      if self._ui._txt_descTitle_Date:GetSizeY() + 10 + self._ui._txt_descTitle_Date:GetTextSizeY() + targetLeftControlSizeY > 505 then
        self._ui._frm_vScroll:SetShow(true)
        self._ui._btn_pagePrevious:SetIgnore(true)
      else
        self._ui._frm_vScroll:SetShow(false)
        self._ui._btn_pagePrevious:SetIgnore(false)
      end
    end
  end
...
function Achievement:setOtherPage(isNext)
  ...
  for index = 0, #stringList2 - 1 do
    ...
    if true == self._nationKoreaCheck then
      ...
      self._ui._stc_questImage2:setRenderTexture(self._ui._stc_questImage2:getBaseTexture())
      self._ui._frm_frame2:UpdateContentPos()
      if self._ui._txt_descTitle_Date2:GetSizeY() + 10 + self._ui._txt_descTitle_Date2:GetTextSizeY() + targetLeftControlSizeY2 > 505 then
...
function Achievement:getRewardText(baseReward)
  ...
  if __eRewardFamilyStat == rewardType then
    ...
    elseif __eFamilySpecialInfoParamType_InvenCount == statType then
    ...
    elseif __eFamilySpecialInfoParamType_StackLimitExceed == statType then
  ...
  elseif __eRewardCharacterStat == rewardType then
    ...
    if __eCharacterSpecialInfoParamType_OffencePoint == statType then
    ...
    elseif __eCharacterSpecialInfoParamType_DefencePoint == statType then
    ...
    elseif __eCharacterSpecialInfoParamType_Inventory == statType then
    ...
    elseif __eCharacterSpecialInfoParamType_StackLimitExceed == statType then
...
function PaGlobal_Achievement_RewardButtonEffect(controlType)
  ...
  if __eRewardFamilyStat == rewardType then
    ...
    elseif __eFamilySpecialInfoParamType_Weight == statType or __eFamilySpecialInfoParamType_Stack == statType then
