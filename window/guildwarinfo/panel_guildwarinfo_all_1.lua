function PaGlobal_GuildWarInfo_All:initialize()
  ...
  for index = 0, self._MAXTERRITORYCOUNT - 1 do
    ...
    self._ui.btn_Territory[index]:SetShow(true)
  end
...
function PaGlobal_GuildWarInfo_All:setKeyGuide()
  ...
  if false == self._isRankOpen then
    txt_KeyGuideY:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_GUILDWARINFO_POINT_VIEW_TOOLTIP_NAME"))
  else
    txt_KeyGuideY:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_GUILDWARINFO_INFO_VIEW_TOOLTIP_NAME"))
  end
  local keyGuides = {
    txt_KeyGuideY,
    txt_KeyGuideX,
...
function PaGlobal_GuildWarInfo_All:selectTab(index)
  ...
  ToClient_RequestListSiegeGuild(self._selectedTerritoryKey, true)
  if true == PaGlobal_GuildWarInfo_All:IsRankClose() then
