function PaGlobal_GuildWarInfoSmall_All:initialize()
  ...
  self._ui.stc_ComboBoxList = UI.getChildControl(self._ui.stc_ComboBox, "Combobox_1_List")
  self._originPanelSizeY = Panel_GuildWarInfoSmall_All:GetSizeY()
  if true == _ContentsOption_CH_TerritoryCount then
    self._MAXTERRIROTYCOUNT = 4
    self._ui.stc_ComboBoxList:SetSize(self._ui.stc_ComboBoxList:GetSizeX(), 30 * self._MAXTERRIROTYCOUNT)
  end
  self:initGuildInfo()
...
function PaGlobal_GuildWarInfoSmall_All:prepareOpen(selectedTerritoryKey)
  ...
  self:initComboBox()
  self:update(self._territoryBoxIndex[selectedTerritoryKey])
  self:open()
end
...
function PaGlobal_GuildWarInfoSmall_All:update(selectedIndex)
  if nil == Panel_GuildWarInfoSmall_All then
    return
  end
  if nil == selectedIndex then
    selectedIndex = self._ui.stc_ComboBox:GetSelectIndex()
  end
  self._ui.stc_ComboBox:SetSelectItemIndex(selectedIndex)
  self._selectedTerritoryKey = self._indexToTerritory[selectedIndex]
...
function PaGlobal_GuildWarInfoSmall_All:initComboBox()
  ...
  for index = 0, self._MAXTERRIROTYCOUNT - 1 do
    ...
    if true == ToClient_isStartMajorSiege(index) then
      ...
      self._territoryBoxIndex[index] = count
      self._indexToTerritory[count] = index
      count = count + 1
    end
  end
