function PaGlobalFunc_GuildWarInfo_All_Open(territoryKey)
  ...
  if PaGlobalFunc_GuildWarInfo_All_GetShow() then
    return
  end
...
function PaGlobalFunc_GuildWarInfo_All_UpdateSecondRoundRankList(content, index)
  if nil == Panel_GuildWarInfo_All then
    return
  end
  if nil == content or nil == index then
  ...
  PaGlobal_GuildWarInfo_All:updateSecondRoundRankList(content, index)
end
...
function HandleEventOnOut_GuildWarInfo_All_ShowRefreshTooltip(isShow)
  ...
  TooltipSimple_Show(uiControl, name, nil, false)
end
function HandleEventOnOut_GuildWarInfo_All_ShowRankTooltip(isShow)
  if nil == Panel_GuildWarInfo_All then
    return
  end
  if false == isShow then
  ...
  if false == PaGlobal_GuildWarInfo_All._isRankOpen then
    local name = PAGetString(Defines.StringSheet_GAME, "LUA_GUILDWARINFO_POINT_VIEW_TOOLTIP_NAME")
    local desc = PAGetString(Defines.StringSheet_GAME, "LUA_GUILDWARINFO_POINT_VIEW_TOOLTIP_DESC")
    local uiControl = PaGlobal_GuildWarInfo_All._ui.btn_Rank
    TooltipSimple_Show(uiControl, name, desc, false)
  else
    local name = PAGetString(Defines.StringSheet_GAME, "LUA_GUILDWARINFO_INFO_VIEW_TOOLTIP_NAME")
    local desc = PAGetString(Defines.StringSheet_GAME, "LUA_GUILDWARINFO_INFO_VIEW_TOOLTIP_DESC")
    local uiControl = PaGlobal_GuildWarInfo_All._ui.btn_Rank
    TooltipSimple_Show(uiControl, name, desc, false)
  end
end
...
function HandleEventLUp_GuildWarInfo_All_SecondRoundRankToggle()
  PaGlobal_GuildWarInfo_All:secondRoundRankToggle()
end
...
function FromClient_GuildWarInfo_All_LoadSiegeSecondRoundScoreList()
  if nil == Panel_GuildWarInfo_All then
    return
  end
  PaGlobal_GuildWarInfo_All:secondRoundRankOpen()
end
