...
function PaGlobalFunc_PartyList_All_WithdrawPartyList()
  PaGlobalFunc_PartyList_All_Update(true)
end
function PaGlobalFunc_PartyList_All_InsertPartyList()
  PaGlobalFunc_PartyList_All_Update(false)
end
function PaGlobalFunc_PartyList_All_CreatePartyList()
  PaGlobalFunc_PartyList_All_Update(false)
end
function PaGlobalFunc_PartyList_All_ChangePartyLeader()
  PaGlobalFunc_PartyList_All_Update(true)
end
function PaGlobalFunc_PartyList_All_Update(neeedRequestPartyList)
  ...
  if true == neeedRequestPartyList then
...
function HandleEventLUp_PartyList_All_DoSearch(str)
  ...
  if _ContentsOption_CH_CheckProhibitedWord == true then
    ToClient_CheckChinaProhibitedWord(msg, __eSceneIdSearchParty)
    return
  end
  local serverNo = 0
...
function PadEventXUp_PatyList_All_RecruiteShow()
  ...
  _AudioPostEvent_SystemUiForXBOX(50, 0)
  PaGlobalFunc_FindPartyRecruite_Show(false)
end
...
function FromClient_PartyList_All_RequestPartyJoin(guestActorKey, characterName, level, classType, partyType, platformType, platformNickname)
  if nil == Panel_PartyList_All then
    return
  end
...
function FromClient_PartyList_All_PartyLeaderIsNotLogined(partyType)
  if partyType == nil then
    return
  end
  local partyLeaderText = ""
  if partyType == CppEnums.PartyType.ePartyType_Large then
    partyLeaderText = PAGetString(Defines.StringSheet_GAME, "LUA_LARGE_PARTY_READER_WORD")
  else
    partyLeaderText = PAGetString(Defines.StringSheet_GAME, "LUA_PARTY_READER_WORD")
  end
  local messageData = {
    title = PAGetString(Defines.StringSheet_RESOURCE, "MESSAGEBOX_TEXT_TITLE"),
    content = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_PARTY_READER_AFK", "partyReader", partyLeaderText),
    functionYes = nil,
    functionNo = nil,
    functionApply = MessageBox_Empty_function,
    priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
...
function FromClient_CheckChinaProhibitedWord_SearchParty(filteredText)
  if _ContentsOption_CH_CheckProhibitedWord == false then
    return
  end
  local self = PaGlobal_PartyList_All
  if self == nil then
    return
  end
  local serverNo = 0
  ToClient_FilteredPartyRecruitmentList(serverNo, filteredText)
  ClearFocusEdit()
end
