function PaGlobalFunc_GuildPartyList_All_Open()
  PaGlobal_GuildPartyList_All:prepareOpen()
end
function PaGlobalFunc_GuildPartyList_All_Close()
  PaGlobal_GuildPartyList_All:prepareClose()
end
function PaGlobalFunc_GuildPartyList_All_Update()
  if nil == Panel_GuildPartyList_All then
    return
  end
  if false == Panel_GuildPartyList_All:GetShow() then
    return
  end
...
function HandleEventLUp_GuildPartyList_All_SearchEdit()
  if nil == Panel_GuildPartyList_All then
    return
  end
  if false == Panel_GuildPartyList_All:GetShow() then
  ...
  SetFocusEdit(PaGlobal_GuildPartyList_All._ui.edit_search)
  PaGlobal_GuildPartyList_All._ui.edit_search:SetEditText(PaGlobal_GuildPartyList_All._ui.edit_search:GetEditText(), true)
end
function HandleEventLUp_GuildPartyList_All_DoSearch(str)
  if nil == Panel_GuildPartyList_All then
    return
  end
  if false == Panel_GuildPartyList_All:GetShow() then
  ...
  local msg = ""
  if PaGlobal_GuildPartyList_All._isPadSnap then
    if nil == str then
      msg = PaGlobal_GuildPartyList_All._ui.edit_search:GetEditText()
    else
      msg = str
      PaGlobal_GuildPartyList_All._ui.edit_search:SetEditText(str)
    end
    ClearFocusEdit()
  else
    msg = PaGlobal_GuildPartyList_All._ui.edit_search:GetEditText()
...
function HandleEventLUp_GuildPartyList_All_Reset()
  if nil == Panel_GuildPartyList_All then
    return
  end
  local serverNo = 0
  local msg = ""
  PaGlobal_GuildPartyList_All._ui.edit_search:SetEditText(msg)
  ToClient_FilteredPartyRecruitmentList(serverNo, msg)
end
function PadEventXUp_GuildPartyList_All_RecruiteShow()
  ...
  if false == PaGlobal_GuildPartyList_All._canInvite then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_ITEMMARKET_FAVORITE_ALREADYREGIST"))
    return
  end
  _AudioPostEvent_SystemUiForXBOX(50, 0)
  PaGlobalFunc_FindPartyRecruite_Show(true)
end
function HandleEventOn_GuildPartyList_All_OnSelectButton(id, eType)
  if nil == Panel_GuildPartyList_All then
    return
  end
  local partyWrapper = PaGlobal_GuildPartyList_All._partySortTable[id]
  if nil == partyWrapper then
    return
  end
  PaGlobal_GuildPartyList_All:setButtonText(eType)
end
function HandleEventLUp_GuildPartyList_All_ClickSelectButton(index, eType, param1, param2)
  if nil == Panel_GuildPartyList_All then
    return
  end
  local partyWrapper = PaGlobal_GuildPartyList_All._partySortTable[index]
  if nil == partyWrapper then
    return
  end
  PaGlobal_GuildPartyList_All:doAction(index, eType, param1, param2)
end
function HandleEventLUp_GuildPartyList_All_Advertising(index)
  if nil == Panel_GuildPartyList_All then
    return
  end
  if nil == index then
  ...
  PaGlobal_GuildPartyList_All._isDisable = true
  if true == PaGlobal_GuildPartyList_All._isConsole then
    PaGlobal_GuildPartyList_All._ui_console.list2_party:requestUpdateVisible()
  else
    PaGlobal_GuildPartyList_All._ui_pc.list2_party:requestUpdateVisible()
  end
  local partyWrapper = ToClient_GetRecruitmentPartyListAt(index)
  if nil ~= partyWrapper then
    ToClient_SetLinkedPartyRecruitInfo(index)
    chatting_sendMessageNoMatterEmpty("", partyWrapper:getTitle(), CppEnums.ChatType.Guild)
  end
end
function HandleEventLUp_GuildPartyList_All_Cancle()
  if nil == Panel_GuildPartyList_All then
    return
  end
...
function HandleEventLUp_GuildPartyList_All_Support(index)
  if nil == Panel_GuildPartyList_All then
    return
  end
  if nil == index then
...
function PaGlobalFunc_GuildPartyList_All_IsBlockedLeader(xuid)
  return ToClient_IsBlockedLeaderFromMe(xuid)
end
function HandleEventLUp_GuildPartyList_All_WhisperToLeader(index)
  if nil == Panel_GuildPartyList_All then
    return
  end
  if nil == index then
...
function HandleEventLUp_GuildPartyList_All_MoveServer(serverNo, partyNo)
  if nil == Panel_GuildPartyList_All then
    return
  end
  ...
  local isMoveAbleServer = PaGlobalFunc_GuildPartyList_All_IsMoveAbleServer(serverIndex)
...
function PaGlobalFunc_GuildPartyList_All_IsMoveAbleServer(serverIndex)
  if nil == serverIndex then
    return false
  end
...
function FromClient_GuildPartyList_All_ResponsePartyRecruitmentInfo(param1)
  if nil == Panel_GuildPartyList_All then
    return
  end
  if 0 == param1 then
...
function FromClient_GuildPartyList_All_RequestPartyJoin(guestActorKey, characterName, level, classType, partyType, platformType, platformNickname)
  if nil == Panel_GuildPartyList_All then
    return
  end
  ...
  local className = getClassName(classType, __ePlayerCreateNormal)
  ...
  if ToClient_isPS5() == true then
    if platformType ~= __eGAME_PLATFORM_TYPE_PS or platformNickname == nil or platformNickname == "" then
      ...
      displayName = characterName .. "[ " .. platformNickname .. " ]"
    end
  end
  local memoContent = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_PARTYLISTRECRUITE_ALERTCONTENT", "name", displayName, "class", className, "level", level)
  local messageBoxData = {
    title = memoTitle,
    content = memoContent,
...
function FromClient_GuildPartyList_All_Resize()
  if nil == Panel_GuildPartyList_All then
    return
  end
  PaGlobal_GuildPartyList_All:resize()
end
function PaGlobalFunc_GuildPartyList_All_CheckUiEdit(targetUI)
  if nil == Panel_GuildPartyList_All then
    return false
  end
  ...
  return targetUI:GetKey() == PaGlobal_GuildPartyList_All._ui.edit_search:GetKey()
end
function PaGlobalFunc_GuildPartyList_All_ClearFocusEdit()
  ClearFocusEdit()
  CheckChattingInput()
end
function PaGlobalFunc_GuildPartyList_All_SetLinkedParty(recruitKey64)
  if nil == Panel_GuildPartyList_All then
    return
  end
  ...
  PaGlobal_GuildPartyList_All._clickedPartyRecruitKey = recruitKey64
end
function PaGlobalFunc_GuildPartyList_All_IsBtnDisableSet()
  if nil == Panel_GuildPartyList_All then
    return false
  end
  return PaGlobal_GuildPartyList_All._isDisable
end
function PaGlobal_GuildPartyList_All_UpdateFunc(deltaTime)
  if nil == Panel_GuildPartyList_All then
    return
  end
  PaGlobal_GuildPartyList_All._currentTime = PaGlobal_GuildPartyList_All._currentTime + deltaTime
  if PaGlobal_GuildPartyList_All._adBtnDisableTime <= PaGlobal_GuildPartyList_All._currentTime then
    PaGlobal_GuildPartyList_All._isDisable = false
    PaGlobal_GuildPartyList_All._currentTime = 0
    PaGlobal_GuildPartyList_All:update()
  end
end
