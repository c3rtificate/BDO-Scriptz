...
function PaGlobal_GrowthPass:initialize_categoryArea()
  ...
  if false == self._isConsole then
    ...
    self._ui._btn_linkGuide:SetShow(false)
  end
...
function PaGlobal_GrowthPass:initialize_mainArea()
  self._ui._stc_questLoading_icon = UI.getChildControl(self._ui._frame_mainBG, "Static_Loading")
  self._ui._stc_questLoading_texture = UI.getChildControl(self._ui._frame_mainBG, "Static_Loading_BlackSpirit")
  self._ui._stc_questLoading_desc = UI.getChildControl(self._ui._frame_mainBG, "StaticText_Loading_Desc")
  self._ui._stc_questLoading_icon:SetShow(false)
  self._ui._stc_questLoading_texture:SetShow(false)
  self._ui._stc_questLoading_desc:SetShow(false)
          ...
          if nil ~= cellWrapper then
            ...
            elseif __eGrowthPass_CellType_Value == cellData.type then
              ...
              if nil ~= questWrapper then
                ...
                cellData.questName:SetText(ToClient_getReplaceDialog(questWrapper:getTitle()))
                ...
                if noteLinkSize >= 1 then
                  ...
                  if false == self._isPadMode then
                    ...
                    cellData.guideLink1:addInputEvent("Mouse_On", "PaGlobalFunc_GrowthPass_ShowAuthorSimpleTooltip(" .. categoryKeyRaw .. "," .. row .. "," .. col .. "," .. "0, true)")
                    cellData.guideLink1:addInputEvent("Mouse_Out", "PaGlobalFunc_GrowthPass_ShowAuthorSimpleTooltip(nil, nil, nil, nil, false)")
                  end
                end
                if noteLinkSize >= 2 then
                  ...
                  if false == self._isPadMode then
                    ...
                    cellData.guideLink2:addInputEvent("Mouse_On", "PaGlobalFunc_GrowthPass_ShowAuthorSimpleTooltip(" .. categoryKeyRaw .. "," .. row .. "," .. col .. "," .. "1, true)")
                    cellData.guideLink2:addInputEvent("Mouse_Out", "PaGlobalFunc_GrowthPass_ShowAuthorSimpleTooltip(nil, nil, nil, nil, false)")
                  end
                end
                if 3 == noteLinkSize then
                  ...
                  if false == self._isPadMode then
                    ...
                    cellData.guideLink3:addInputEvent("Mouse_On", "PaGlobalFunc_GrowthPass_ShowAuthorSimpleTooltip(" .. categoryKeyRaw .. "," .. row .. "," .. col .. "," .. "2, true)")
                    cellData.guideLink3:addInputEvent("Mouse_Out", "PaGlobalFunc_GrowthPass_ShowAuthorSimpleTooltip(nil, nil, nil, nil, false)")
                  end
                end
              else
...
function PaGlobal_GrowthPass:initialize_keyguideArea()
  if nil == Panel_Window_GrowthPass then
    return
  end
  if false == self._isPadMode then
  ...
  self._consoleUi._stc_keyguideArea:SetShow(true)
  self._consoleUi._txt_keyguide_A = UI.getChildControl(self._consoleUi._stc_keyguideArea, "StaticText_A_ConsoleUI")
  self._consoleUi._txt_keyguide_B = UI.getChildControl(self._consoleUi._stc_keyguideArea, "StaticText_B_ConsoleUI")
  self.keyGuides = {
    self._consoleUi._txt_keyguide_A,
    self._consoleUi._txt_keyguide_B
  }
  for index = 1, #self.keyGuides do
    self.keyGuides[index]:SetShow(true)
  end
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(self.keyGuides, self._consoleUi._stc_keyguideArea, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHT)
end
function PaGlobal_GrowthPass:registEventHandler()
  ...
  if _ContentsGroup_QuestBackEndLoading == true then
    registerEvent("FromClient_QuestBackEndLoadComplete", "FromClient_GrowthPass_QuestBackEndLoadComplete")
  end
end
function PaGlobal_GrowthPass:resize()
  if nil == Panel_Window_GrowthPass then
    return
  end
...
function PaGlobal_GrowthPass:recalcPanelPosition()
  if nil == Panel_Window_GrowthPass then
    return
  end
  local panel = Panel_Window_GrowthPass
  ...
  if true == self._isPadMode then
    panelSizeY = panelSizeY + self._consoleUi._stc_keyguideArea:GetSizeY()
  end
  ...
  Panel_Window_GrowthPass:SetPosX(calcPosX)
  Panel_Window_GrowthPass:SetPosY(calcPosY)
end
...
function PaGlobal_GrowthPass:showIntroScreen(isShow)
  ...
  self:hideIntroScreenBackground(isShow)
  if true == isShow then
    ToClient_padSnapSetTargetPanel(self._ui._stc_startBG)
  end
end
function PaGlobal_GrowthPass:hideIntroScreenBackground(isHide)
  if nil == Panel_Window_GrowthPass then
    return
  end
  self._ui._stc_mainTabTypeBG:SetShow(not isHide)
  self._ui._stc_categoryReward:SetShow(not isHide)
  self._ui._frame_mainBG:SetShow(not isHide)
end
...
function PaGlobal_GrowthPass:close()
  ...
  if false == self._isConsole then
    PaGlobalFunc_GrowthPass_ShowAuthorSimpleTooltip(nil, nil, nil, nil, false)
  end
...
function PaGlobal_GrowthPass:validate()
  ...
  self._ui._stc_questLoading_icon:isValidate()
  self._ui._stc_questLoading_texture:isValidate()
  self._ui._stc_questLoading_desc:isValidate()
  self._ui._stc_startBG:isValidate()
  self._ui._stc_totalRewardBG:isValidate()
  if true == self._isPadMode then
    for index = 1, #self.keyGuides do
      self.keyGuides[index]:isValidate()
    end
  end
end
    ...
    for col = 0, totalColCount - 1 do
      ...
      if nil ~= cellRef then
        if nil ~= cellRef.control then
          ...
          if true == isShow and false == isFocused and true == self._isPadMode and cellRef.type == __eGrowthPass_CellType_Value then
            ToClient_padSnapChangeToTarget(cellRef.control)
            isFocused = true
          end
        end
      else
      ...
      if nil ~= layoutWrapper and nil ~= currentCategorySSW then
        ...
        for rewardIndex = 0, self._categoryRewardCount - 1 do
          ...
          if nil ~= rewardDataRef then
            if true == self._consoleUi then
              rewardDataRef.control:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobalFunc_GrowthPass_OnClickedCategoryRewardIcon(" .. tostring(categoryKeyRaw) .. "," .. tostring(rewardIndex) .. ")")
            else
              rewardDataRef.control:removeInputEvent("Mouse_LUp")
              rewardDataRef.control:addInputEvent("Mouse_LUp", "PaGlobalFunc_GrowthPass_OnClickedCategoryRewardIcon(" .. tostring(categoryKeyRaw) .. "," .. tostring(rewardIndex) .. ")")
            end
            ...
            if true == self._consoleUi then
              rewardDataRef.itemSlot.icon:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobalFunc_GrowthPass_OnClickedCategoryRewardIcon(" .. tostring(categoryKeyRaw) .. "," .. tostring(rewardIndex) .. ")")
            else
            ...
            if nil ~= rewardDataRef.controlEffect and nil ~= rewardDataRef.control then
              rewardDataRef.control:EraseEffect(rewardDataRef.controlEffect)
              rewardDataRef.controlEffect = nil
            end
            if true == ToClient_checkGetableGrowthPassCategoryRewardItem(value.categoryKey, rewardIndex) then
              ...
              rewardDataRef.controlEffect = rewardDataRef.control:AddEffect("fUI_GrowthPass_Box_03A", true, 0, 0)
            else
    ...
    if key == self._currentSelectedCategoryIndex then
      ...
      if nil ~= layoutWrapper then
        ...
        for row = 0, totalRowCount - 1 do
          for col = 0, totalColCount - 1 do
            ...
            if nil ~= cellData then
              if nil ~= cellData.controlEffect and nil ~= cellData.reward then
                cellData.reward:EraseEffect(cellData.controlEffect)
                cellData.controlEffect = nil
              end
              if __eGrowthPass_CellType_Value == cellData.type then
                local cellWrapper = layoutWrapper:getCellObjectWrapper(row, col)
                if nil ~= cellWrapper then
                  local cell_questGroup = cellWrapper:getQuestGroup()
                  local cell_questId = cellWrapper:getQuestId()
                  if self:isHaveBackEndData(cell_questGroup, cell_questId) == true then
                    local currentQuestState = ToClient_getGrowthPassQuestState(cell_questGroup, cell_questId)
                    ...
                    elseif __eGrowthPass_QuestState_Complete == currentQuestState then
                      ...
                      cellData.clearQuest:SetShow(false)
                      cellData.completeIcon:SetShow(false)
                      cellData.controlEffect = cellData.reward:AddEffect("fUI_GrowthPass_Box_01A", true, 0, 0)
                    ...
                    self:pushQuestLoadingData(self._currentSelectedCategoryIndex, cell_questGroup, cell_questId)
                  end
                end
              end
            end
        ...
        if self:isExistLoadingDataAnyOne() == true then
          self:showQuestLoading(true)
        else
          self:showQuestLoading(false)
        end
      end
...
function PaGlobal_GrowthPass:updateTotalRewardItemState()
  ...
  for index = 0, totalRewardCount - 1 do
    ...
    if nil ~= self._ui.totalRewardDataList[index].controlEffect and nil ~= self._ui.totalRewardDataList[index].baseItemSlot.icon then
      self._ui.totalRewardDataList[index].baseItemSlot.icon:EraseEffect(self._ui.totalRewardDataList[index].controlEffect)
      self._ui.totalRewardDataList[index].controlEffect = nil
    end
    ...
    if true == ToClient_checkGetableGrowthPassTotalRewardItem(totalRewardKey) then
      ...
      self._ui.totalRewardDataList[index].controlEffect = self._ui.totalRewardDataList[index].baseItemSlot.icon:AddEffect("fUI_GrowthPass_Box_02A", true, 0, 0)
    else
...
function PaGlobal_GrowthPass:showQuestLoading(isShow)
  if Panel_Window_GrowthPass == nil or _ContentsGroup_QuestBackEndLoading == false then
    return
  end
  local frameContents = UI.getChildControl(self._ui._frame_mainBG, "Frame_1_Content")
  frameContents:SetShow(not isShow)
  self._ui._stc_questLoading_icon:SetShow(isShow)
  self._ui._stc_questLoading_texture:SetShow(isShow)
  self._ui._stc_questLoading_desc:SetShow(isShow)
end
function PaGlobal_GrowthPass:clearQuestLoadingTable()
  if Panel_Window_GrowthPass == nil or _ContentsGroup_QuestBackEndLoading == false then
    return
  end
  self._questLoadingDataList = Array.new()
end
function PaGlobal_GrowthPass:isHaveBackEndData(questGroup, questId)
  if Panel_Window_GrowthPass == nil or _ContentsGroup_QuestBackEndLoading == false then
    return true
  end
  local questSSW = questList_getQuestStatic(questGroup, questId)
  if questSSW == nil then
    return true
  end
  return questSSW:isHaveBackEndData()
end
function PaGlobal_GrowthPass:pushQuestLoadingData(currentCategoryIdx, questGroup, questId)
  if Panel_Window_GrowthPass == nil or _ContentsGroup_QuestBackEndLoading == false or currentCategoryIdx == nil or questGroup == nil or questId == nil then
    return
  end
  ...
  local newLoadingData = {
    _categoryIndex = currentCategoryIdx,
    _questNoRaw = questSSW:getKey()
  }
  if self:isExistLoadingDataAnyOne() == true then
    for index = #self._questLoadingDataList, 1, -1 do
      ...
      if value ~= nil then
        ...
        elseif value._categoryIndex ~= newLoadingData._categoryIndex then
          table.remove(self._questLoadingDataList, index)
        end
      end
    end
...
function PaGlobal_GrowthPass:isExistLoadingDataAnyOne()
  if Panel_Window_GrowthPass == nil or _ContentsGroup_QuestBackEndLoading == false then
    return false
  end
  if self._questLoadingDataList == nil then
