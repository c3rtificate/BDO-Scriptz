function PaGlobalFunc_GrowthPass_Open()
  ...
  if true == isSeasonCharacter then
...
function PaGlobalFunc_GrowthPass_ShowItemTooltip(categoryKeyRaw, rewardIndex, isShow)
  if nil == Panel_Window_GrowthPass or nil == PaGlobal_GrowthPass then
    return
  end
  local categoryKey = GrowthPassCategoryKey(categoryKeyRaw)
  ...
  if false == self._isConsole then
    if true == isShow then
      Panel_Tooltip_Item_Show(rewardItemWrapper, self._ui._categoryRewardDataList[rewardIndex].itemSlot.icon, true, false)
    else
      Panel_Tooltip_Item_hideTooltip()
    end
  elseif true == isShow then
    PaGlobalFunc_TooltipInfo_Open(Defines.TooltipDataType.ItemSSWrapper, rewardItemWrapper, Defines.TooltipTargetType.Item, 0, getScreenSizeX())
  else
...
function PaGlobalFunc_GrowthPass_ShowTotalRewardItemTooltip(totalRewardIndex, isShow)
  ...
  elseif true == isShow then
    PaGlobalFunc_TooltipInfo_Open(Defines.TooltipDataType.ItemSSWrapper, baseRewardItemWrapper, Defines.TooltipTargetType.Item, 0, getScreenSizeX())
  else
    PaGlobalFunc_TooltipInfo_Close()
  end
end
...
function PaGlobalFunc_GrowthPass_AdventurerNotes(questGroup, questId, idx)
  ...
  _PA_LOG("\236\181\156\237\149\156\236\167\132", "" .. tostring(noteLink))
...
function PaGlobalFunc_GrowthPass_ShowAuthorSimpleTooltip(categoryKeyRaw, row, col, idx, isShow)
  if nil == Panel_Window_GrowthPass then
    return
  end
  if false == Panel_Window_GrowthPass:GetShow() then
    ...
    if nil ~= cellWrapper then
      ...
      if nil ~= questInfoWrapper then
        ...
        if nil ~= cellData then
          ...
          local questWrapper = ToClient_getQuestWrapper(questNo)
          local noteAuthor = ""
          noteAuthor = questWrapper:getNoteAuthor(idx)
          _PA_LOG("\236\181\156\237\149\156\236\167\132", "" .. tostring(noteAuthor))
          if 0 == idx then
            uiControl = cellData.guideLink1
          elseif 1 == idx then
            uiControl = cellData.guideLink2
          elseif 2 == idx then
            uiControl = cellData.guideLink3
          else
            return ""
          end
          noteAuthor = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_GROWTHPASS_LINK_AUTHOR", "name", noteAuthor)
          TooltipAuthor_Show(uiControl, nil, noteAuthor)
        end
      end
    end
  else
...
function FromClient_GrowthPass_QuestBackEndLoadComplete(s32_questNoRaw)
  if Panel_Window_GrowthPass == nil or _ContentsGroup_QuestBackEndLoading == false then
    return
  end
  if Panel_Window_GrowthPass:GetShow() == false then
    return
  end
  if PaGlobal_GrowthPass:isExistLoadingDataAnyOne() == false then
    return
  end
  local updateTargetCategoryIndex = -1
  for index = 1, #PaGlobal_GrowthPass._questLoadingDataList do
    local value = PaGlobal_GrowthPass._questLoadingDataList[index]
    if value ~= nil and value._questNoRaw == s32_questNoRaw then
      updateTargetCategoryIndex = value._categoryIndex
      table.remove(PaGlobal_GrowthPass._questLoadingDataList, index)
      break
    end
  end
  if PaGlobal_GrowthPass:isExistLoadingDataAnyOne() == false then
    if updateTargetCategoryIndex == -1 then
      return
    end
    PaGlobal_GrowthPass:showQuestLoading(false)
    PaGlobalFunc_GrowthPass_OnClickedCategoryButton(updateTargetCategoryIndex)
    return
  end
end
