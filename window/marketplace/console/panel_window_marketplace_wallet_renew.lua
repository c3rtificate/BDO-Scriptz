...
function MarketPlace_Wallet:initControl()
  ...
  self._ui.stc_walletSortBg = UI.getChildControl(self._ui.stc_LeftBg, "Static_WalletSortBg")
  self._ui.btn_sortOption = UI.getChildControl(self._ui.stc_walletSortBg, "Button_Sort_Set")
  self._ui.chk_sort = UI.getChildControl(self._ui.stc_walletSortBg, "CheckButton_ItemSort")
...
function MarketPlace_Wallet:initEvent()
  ...
  self._ui.btn_sortOption:addInputEvent("Mouse_LUp", "PaGlobalFunc_MarketPlace_MyWalletInfo_ToggleSortOptionView()")
...
function PaGlobalFunc_MarketPlace_Wallet_CreateControlMyBiddingList(contents, key)
  ...
  if self._selectedBiddingTabIndex == self._biddingTabIdx.sell then
    ...
    txt_ItemName:SetPosY(txt_registerTime:GetPosY() - txt_ItemName:GetSizeY() - 7)
    local waitTimeStr
    if waitTime == nil or waitTime <= toInt64(0, 0) then
      waitTimeStr = "-"
    else
      local dataInfo = utcTimestampToDate(Int64toInt32(waitTime), true)
      waitTimeStr = string.format("%04d %02d-%02d %02d:%02d", dataInfo._year, dataInfo._month, dataInfo._day, dataInfo._hour, dataInfo._minute)
    end
  ...
  elseif self._selectedBiddingTabIndex == self._biddingTabIdx.buy then
    ...
    local baseMinPrice = itemInfo:getBaseMinPrice()
    ...
    txt_ItemName:SetTextMode(__eTextMode_AutoWrap)
    txt_ItemName:SetText(itemNameStr)
    txt_ItemName:SetVerticalMiddle()
...
function PaGlobalFunc_MarketPlace_Wallet_ShowToolTip(slotNo, slotIdx)
  local self = MarketPlace_Wallet
  local itemMyWalletInfo = getWorldMarketWaitListShowingSellBiddingListByIdx(slotNo)
  if itemMyWalletInfo == nil then
    return
  end
  local itemWrapper = itemMyWalletInfo:getItemEnchantStaticStatusWrapper()
...
function MarketPlace_Wallet:updateBiddingItemList()
  ...
  if nil ~= self._ui.txt_ReserveValue and nil ~= self._ui.txt_CompleteValue then
    self._ui.txt_CompleteValue:ComputePos()
    self._ui.txt_CompleteTitle:SetSpanSize(self._ui.txt_CompleteValue:GetSpanSize().x + self._ui.txt_CompleteValue:GetTextSizeX() + 10, self._ui.txt_CompleteTitle:GetSpanSize().y)
    self._ui.txt_ReserveValue:SetSpanSize(self._ui.txt_CompleteTitle:GetSpanSize().x + self._ui.txt_CompleteTitle:GetTextSizeX() + 10, self._ui.txt_ReserveValue:GetSpanSize().y)
    self._ui.txt_ReserveTitle:SetSpanSize(self._ui.txt_ReserveValue:GetSpanSize().x + self._ui.txt_ReserveValue:GetTextSizeX() + 10, self._ui.txt_ReserveTitle:GetSpanSize().y)
  end
...
function PaGlobalFunc_Wallet_GetScrollPos()
  local self = MarketPlace_Wallet
  self._vscroll = self._ui.list2_BiddingItem:GetVScroll()
  self._toIndex = self._ui.list2_BiddingItem:getCurrenttoIndex()
  if 1 == self._vscroll:GetControlPos() and 0 ~= self._toIndex then
    self._toIndex = self._toIndex - 1
  end
  self._scrollValue = self._ui.list2_BiddingItem:GetVScroll()
  if false == self._ui.list2_BiddingItem:IsIgnoreVerticalScroll() then
    self._scrollValue = self._vscroll:GetControlPos()
  end
end
function MarketPlace_Wallet:setScrollPos()
  if nil == self._ui.list2_BiddingItem or nil == self._vscroll then
    return
  end
  self._ui.list2_BiddingItem:setCurrenttoIndex(self._toIndex)
  if false == self._ui.list2_BiddingItem:IsIgnoreVerticalScroll() then
...
function PaGlobalFunc_MarketPlace_MyWalletInfo_ToggleSortOptionView()
  local self = MarketPlace_Wallet
  if self == nil then
    return
  end
...
function PaGlobalFunc_MarketPlace_Wallet_TabToggle()
  ...
  self._toIndex = 0
  self._scrollValue = 0
  self._ui.list2_BiddingItem:moveTopIndex()
  self._ui.list2_BiddingItem:requestUpdateVisible()
  self._ui.list2_BiddingItem:updateContentPos()
end
...
function PaGlobal_SellBidding_Calculate(idx)
  ...
  PaGlobalFunc_Wallet_GetScrollPos()
...
function PaGlobalFunc_Retry_SellBidding(itemkey, enchantLevel, isSealed, isBuy)
  ToClient_requestDetailOneItemByWorldMarket(ItemEnchantKey(itemkey, enchantLevel), isSealed, true, isBuy)
end
function PaGlobalFunc_Retry_BuyBidding(itemkey, enchantLevel, isSealed, isBuy)
  ToClient_requestDetailOneItemByWorldMarket(ItemEnchantKey(itemkey, enchantLevel), isSealed, true, isBuy)
...
function MarketPlaceWallet_updateSlotData_Console()
  local self = MarketPlace_Wallet
  if self == nil then
    return
  end
...
function PaGlobalFunc_Wallet_Open()
  ...
  self._startInvenSlotIndex = 0
  self._ui.scroll_ItemSlot:SetControlPos(0)
  PaGlobalFunc_AutoSortFunctionPanel_LinkPanel(InvenSortLinkPanelIndex.MarketPlaceWalletMain, _panel, self._ui.btn_sortOption, self._ui.chk_sort, nil, nil, "Static_SortComponent_MyWalletInfo")
  PaGlobalFunc_AutoSortFunctionPanel_LoadSortOptionAndDoSort(InvenSortLinkPanelIndex.MarketPlaceWalletMain, false)
