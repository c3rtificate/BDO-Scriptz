...
local MarketPlace = {
  _ui = {
    ...
    txt_TitleBg = UI.getChildControl(_panel, "StaticText_TopTitle"),
    txt_Title = nil,
...
function MarketPlace:init()
  ...
  self._ui.txt_Title = UI.getChildControl(self._ui.txt_TitleBg, "StaticText_TitleIcon")
  self:registEvent()
end
...
function MarketPlace:open(tabIdx)
  ...
  self._ui.txt_Title:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_MARKETPLACE_TITLE") .. " " .. version)
...
function PaGlobalFunc_MarketPlace_CloseAllCheck(isCheckClose)
  PaGlobalFunc_MarketPlace_Close(isCheckClose)
end
function PaGlobalFunc_MarketPlace_Close(isCheckClose)
  local self = MarketPlace
  if nil == self then
    _PA_ASSERT(false, "\237\140\168\235\132\144\236\157\180 \236\161\180\236\158\172\237\149\152\236\167\128 \236\149\138\236\138\181\235\139\136\235\139\164!! : MarketPlace")
    return
  ...
  local function closePanel()
    ...
    elseif true == self._isOpenByMaid then
      ToClient_CallHandlerMaid("_maidLogOut")
      self._isOpenByMaid = false
    end
  end
  if nil == isCheckClose or false == isCheckClose then
    closePanel()
    return
  end
  if nil ~= PaGlobalFunc_TooltipInfo_GetShow and true == PaGlobalFunc_TooltipInfo_GetShow() and nil ~= PaGlobalFunc_TooltipInfo_Close then
...
function PaGlobalFunc_MarketPlace_OpenFromDialog()
  ...
  if nil ~= PaGlobal_Warehouse_All_Close then
...
function PaGlobalFunc_MarketPlace_CloseToDialog()
  ...
  PaGlobalFunc_MarketPlace_Close(true)
end
...
function PaGlobalFunc_MarketPlace_SimpleToolTip(isShow, index)
  ...
  if 0 < getWorldMarketAddWeight() then
    local s64_addWeight_div = toInt64(0, getWorldMarketAddWeight())
    local str_AddWeight = string.format("%.1f", Int64toInt32(s64_addWeight_div) / 10)
    desc = desc .. [[


]] .. PAGetString(Defines.StringSheet_GAME, "LUA_MARKETWEIGHT_ADDWEIGHT") .. " " .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_MARKETPLACE_VOLUME_VT_PARAM_NONCOLOR", "Weight", str_AddWeight)
  end
  if 1 == index then
