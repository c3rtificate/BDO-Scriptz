...
local MarketPlace_MyInven = {
  _ui = {
    ...
    stc_InvenSortBg = UI.getChildControl(_panel, "Static_WalletSortBg"),
    txt_Weight = nil,
    btn_sortOption = nil,
    chk_sort = nil
  },
...
function MarketPlace_MyInven:initControl()
  self._ui.txt_Weight = UI.getChildControl(self._ui.stc_InvenSortBg, "StaticText_WeightIcon")
  self._ui.btn_sortOption = UI.getChildControl(self._ui.stc_InvenSortBg, "Button_Sort_Set")
  self._ui.chk_sort = UI.getChildControl(self._ui.stc_InvenSortBg, "CheckButton_ItemSort")
  ...
  self._ui.btn_WareHouseDeposit:SetShow(false)
  self:setUsingTab()
  self:initData()
...
function MarketPlace_MyInven:initEvent()
  ...
  self._ui.btn_sortOption:addInputEvent("Mouse_LUp", "PaGlobalFunc_MarketPlace_Inventory_ToggleSortOptionView()")
end
function PaGlobalFunc_MarketPlace_Inventory_ToggleSortOptionView()
  local self = MarketPlace_MyInven
  if self == nil then
    return
  end
...
function MarketPlace_MyInven:selectTab(idx)
  ...
  if nil == invenType then
    ...
    tempWrapper:setCurrentMarketPlaceWhereType(CppEnums.ItemWhereType.eInventory)
    return
  else
    self._currentInvenType = invenType
    tempWrapper:setCurrentMarketPlaceWhereType(invenType)
  end
...
function MarketPlace_MyInven:updateInventory()
  ...
  if isWareHouse == true then
    if PaGlobalFunc_AutoSortFunctionPanel_IsSorted(InvenSortLinkPanelIndex.MarketPlaceInventory) == true then
      ...
      for ii = 1, invenUseSize - 1 do
        slotNoList[ii] = selfPlayer:getRealInventorySlotNoNew(__eInventorySortWhereType_MarketPlaceInventory, ii)
      end
    end
  elseif PaGlobalFunc_AutoSortFunctionPanel_IsSorted(InvenSortLinkPanelIndex.MarketPlaceInventory) == true then
    PaGlobalFunc_AutoSortFunctionPanel_LoadSortOptionAndDoSort(InvenSortLinkPanelIndex.MarketPlaceInventory, false)
  ...
  moneyValue = makeDotMoney(getSelfPlayer():get():getMoneyInventory():getMoney_s64())
...
function MarketPlaceMyInventory_updateSlotData_Console()
  local self = MarketPlace_MyInven
  if self == nil then
    return
  end
...
function PaGlobalFunc_MarketPlace_MyInven_Open()
  ...
  PaGlobalFunc_AutoSortFunctionPanel_LinkPanel(InvenSortLinkPanelIndex.MarketPlaceInventory, mainPanel, self._ui.btn_sortOption, self._ui.chk_sort, nil, nil, "Static_SortComponent_Inventory")
  PaGlobalFunc_AutoSortFunctionPanel_LoadSortOptionAndDoSort(InvenSortLinkPanelIndex.MarketPlaceInventory, false)
  self._ui.scroll_ItemSlot:SetControlPos(0)
  self._startInvenSlotIndex = 0
  self:setUsingTab()
...
function MarketPlace_MyInven:getWarehouseWrapper()
  ...
  local plantWayKey = getCurrentWaypointKey()
