...
function PaGlobalFunc_Wallet_Init()
  ...
  self:initControl()
  self:initEvent()
  if self._isOpenedMileageFunction == true and self._ui.txt_MarketTaxBonus:GetShow() == true then
    self._ui.txt_MarketTaxBonus:SetPosX(self._ui.txt_MarketTaxBonus:GetPosX() - 15)
  end
end
function MarketPlace_Wallet:initControl()
  ...
  self._ui.txt_MileageValue = UI.getChildControl(self._ui.stc_WalletStatBg, "StaticText_Tax_CH")
  ...
  self._ui.txt_registItemDesc:SetTextMode(__eTextMode_AutoWrap)
  self._ui.txt_registItemDesc:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_MARKETPLACE_WALLET_BLOCKBG_DESC"))
  self._ui.edit_search = UI.getChildControl(self._ui.stc_ItemListBg, "Edit_GoodsName")
  self._ui.btn_sortOption = UI.getChildControl(self._ui.stc_ItemListBg, "Button_Sort_Set")
  self._ui.chk_sort = UI.getChildControl(self._ui.stc_ItemListBg, "CheckButton_ItemSort")
  self._ui.txt_editDefault = UI.getChildControl(self._ui.edit_search, "StaticText_DefaultSearchText")
  self._ui.btn_editReset = UI.getChildControl(self._ui.edit_search, "Button_Reset")
  self._ui.edit_search_wallet = UI.getChildControl(self._ui.stc_Main, "Edit_GoodName")
  self._ui.txt_editDefault_wallet = UI.getChildControl(self._ui.edit_search_wallet, "StaticText_DefaultSearchText")
  self._ui.btn_editReset_wallet = UI.getChildControl(self._ui.edit_search_wallet, "Button_Reset")
  ...
  self._ui.btn_editReset:SetShow(false)
  self._ui.txt_MileageValue:SetShow(self._isOpenedMileageFunction)
  self._ui.txt_MarketTaxBonus:SetShow(not self._isOpenedMileageFunction)
  ...
  for ii = 0, self._config.slotCount - 1 do
    local slot = {}
    slot.background = UI.createControl(__ePAUIControl_Static, self._ui.stc_ItemSlotBg, "ItemSlotBg_" .. ii)
...
function MarketPlace_Wallet:refreshRevieveAllButtonText(sellCount, tabIndex)
  if false == self._isOpenedMileageFunction then
    return
  end
  if nil == tabIndex then
    return
  end
  local defaultButtonSizeX = 180
  if tabIndex == self._biddingTabIdx.buy then
    self._ui.btn_RecieveAll:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_MARKETPLACE_MAIN_RECIEVEALL_BUTTON"))
    self._ui.btn_RecieveAll:SetSize(defaultButtonSizeX, self._ui.btn_RecieveAll:GetSizeY())
  elseif tabIndex == self._biddingTabIdx.sell then
    local needTotalMileage = 100 * sellCount
    if needTotalMileage < 0 then
      needTotalMileage = 0
    end
    self._ui.btn_RecieveAll:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_MARKETPLACE_BTN_TAX_CH", "tax", needTotalMileage))
    self._ui.btn_RecieveAll:SetSize(self._ui.btn_RecieveAll:GetTextSizeX() + 20, self._ui.btn_RecieveAll:GetSizeY())
  else
    return
  end
  self._ui.btn_RecieveAll:ComputePos()
end
...
function PaGlobalFunc_Wallet_UpdatePearlItemLimitCount()
  ...
  self._ui.txt_LimitDesc:SetTextMode(__eTextMode_LimitText)
...
function MarketPlace_Wallet:initEvent()
  ...
  self._ui.list2_BiddingItem:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_MarketPlace_Wallet_CreateControlMyBiddingList")
  self._ui.list2_BiddingItem:createChildContent(__ePAUIList2ElementManagerType_List)
  self._ui.list2_WalletItem:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_MarketPlace_Wallet_CreateControlWalletItemList")
  self._ui.list2_WalletItem:createChildContent(__ePAUIList2ElementManagerType_List)
  self._ui.list2_History:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_MarketPlace_Wallet_CreateControlHistroyList")
  self._ui.list2_History:createChildContent(__ePAUIList2ElementManagerType_List)
  ...
  self._ui.edit_search:addInputEvent("Mouse_LUp", "PaGlobalFunc_MarketPlace_Wallet_SetFocusSearchText()")
  self._ui.edit_search:RegistReturnKeyEvent("PaGlobalFunc_MarketPlace_Wallet_SearchItem()")
  self._ui.btn_sortOption:addInputEvent("Mouse_LUp", "HandleLUp_MarketPlace_Wallet_ToggleSortOptionView()")
  self._ui.edit_search_wallet:addInputEvent("Mouse_LUp", "PaGlobalFunc_MarketPlace_Wallet_SetFocusSearchTextBidding()")
  self._ui.edit_search_wallet:RegistReturnKeyEvent("PaGlobalFunc_MarketPlace_Wallet_SearchItemBidding()")
  self._ui.btn_editReset_wallet:addInputEvent("Mouse_LUp", "PaGlobalFunc_MarketPlace_Wallet_ClearFocusSearchTextBidding()")
  self._ui.btn_editReset:addInputEvent("Mouse_LUp", "HandleEventLUp_MarketPlace_Wallet_ClearSearch()")
  self._ui.btn_editReset:addInputEvent("Mouse_On", "HandleEventOnOut_MarketPlace_Wallet_SearchResetTooltip(true)")
  self._ui.btn_editReset:addInputEvent("Mouse_Out", "HandleEventOnOut_MarketPlace_Wallet_SearchResetTooltip(false)")
  ...
  if true == self._isOpenedMileageFunction then
    self._ui.txt_MarketTaxBonus:removeInputEvent("Mouse_On")
    self._ui.txt_MarketTaxBonus:removeInputEvent("Mouse_Out")
  end
end
...
function PaGlobalFunc_MarketPlace_Wallet_SetFocusSearchText()
  SetFocusEdit(MarketPlace_Wallet._ui.edit_search)
  MarketPlace_Wallet._ui.txt_editDefault:SetShow(false)
end
function PaGlobalFunc_MarketPlace_Wallet_SetFocusSearchTextBidding()
  SetFocusEdit(MarketPlace_Wallet._ui.edit_search_wallet)
  MarketPlace_Wallet._ui.txt_editDefault_wallet:SetShow(false)
end
function HandleEventLUp_MarketPlace_Wallet_ClearSearch()
  PaGlobalFunc_MarketPlace_Wallet_ClearFocusSearchText()
  MarketPlace_Wallet:updateWallet()
end
function PaGlobalFunc_MarketPlace_Wallet_ClearFocusSearchText()
  ClearFocusEdit()
  MarketPlace_Wallet._filterText = ""
  MarketPlace_Wallet._isSearch = false
  MarketPlace_Wallet._searchItemEffectOff = {}
  MarketPlace_Wallet._ui.edit_search:SetEditText("")
  MarketPlace_Wallet._ui.txt_editDefault:SetShow(true)
  MarketPlace_Wallet._ui.btn_editReset:SetShow(false)
  TooltipSimple_Hide()
end
function PaGlobalFunc_MarketPlace_Wallet_ClearFocusSearchTextBidding()
  ClearFocusEdit()
  MarketPlace_Wallet._ui.edit_search_wallet:SetEditText("")
  MarketPlace_Wallet._ui.txt_editDefault_wallet:SetShow(true)
  MarketPlace_Wallet._ui.btn_editReset_wallet:SetShow(false)
  TooltipSimple_Hide()
end
function PaGlobalFunc_MarketPlace_Wallet_SearchItem()
  if false == CheckChattingInput() then
    ClearFocusEdit()
  end
  MarketPlace_Wallet._isSearch = true
  MarketPlace_Wallet._filterText = MarketPlace_Wallet._ui.edit_search:GetEditText()
  MarketPlace_Wallet._ui.btn_editReset:SetShow(true)
  MarketPlace_Wallet:findSearchItem()
end
function PaGlobalFunc_MarketPlace_Wallet_SearchItemBidding()
  if false == CheckChattingInput() then
    ClearFocusEdit()
  end
  local self = MarketPlace_Wallet
  local searchText = MarketPlace_Wallet._ui.edit_search_wallet:GetEditText()
  MarketPlace_Wallet._ui.btn_editReset_wallet:SetShow(true)
  ...
  for ii = 0, itemListCount - 1 do
    local slotNo = ii
    if self._selectedBiddingTabIndex == self._biddingTabIdx.buy then
      itemMyWalletInfo = getWorldMarketBuyBiddingListByIdx(slotNo)
    else
      itemMyWalletInfo = getWorldMarketSellBiddingListByIdx(slotNo)
    end
    if nil ~= itemMyWalletInfo then
      ...
      if nil ~= itemWrapper then
        ...
        if true == stringSearch(itemName, searchText) then
          MarketPlace_Wallet._ui.list2_BiddingItem:moveIndex(slotNo)
          isFindItem = true
        end
      end
    end
  end
  if self._selectedBiddingTabIndex == self._biddingTabIdx.sell and isFindItem == false then
    local waitItemListCount = getWorldMarketWaitListShowingSellBiddingListCount()
    for ii = 0, waitItemListCount - 1 do
      local slotNo = ii
      local itemMyWalletInfo = getWorldMarketWaitListShowingSellBiddingListByIdx(slotNo + itemListCount)
      if itemMyWalletInfo ~= nil then
        local itemWrapper = itemMyWalletInfo:getItemEnchantStaticStatusWrapper()
        if itemWrapper ~= nil then
          local itemName = itemWrapper:getName()
          if true == stringSearch(itemName, searchText) then
            MarketPlace_Wallet._ui.list2_BiddingItem:moveIndex(slotNo + itemListCount)
            isFindItem = true
          end
        end
      end
  ...
  if false == isFindItem then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_ALCHEMY_NO_SEARCH_RESULT"))
  end
end
function MarketPlace_Wallet:findSearchItem()
  local isEmptyText = nil == self._filterText or "" == self._filterText or self._filterText:len() <= 0
  if false == self._isSearch and true == isEmptyText then
    return
  end
  if true == self._isSearch then
    self._searchItemEffectOff = {}
    if true == isEmptyText then
      self._ui.txt_editDefault:SetShow(true)
      TooltipSimple_Hide()
    end
  end
  self:updateWallet()
  if true == self._isSearch and false == isEmptyText and false == self._isSearchFind then
    self:reScanSearchItem()
  end
end
function MarketPlace_Wallet:reScanSearchItem()
  ...
  local startSlot = 0
  local isSorted = PaGlobalFunc_AutoSortFunctionPanel_IsSorted(InvenSortLinkPanelIndex.MarketPlaceWalletMain)
  ...
  if true == isSearchFind then
    if self._viewType.slot == self._selectedViewType then
      ...
      self._ui.scroll_ItemSlot:SetControlPos(scrollPos)
      self._startInvenSlotIndex = startSlot
      self:updateWallet_Slot()
    else
      self._ui.list2_WalletItem:moveIndex(startSlot)
    end
  else
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_ALCHEMY_NO_SEARCH_RESULT"))
  end
end
function HandleEventOnOut_MarketPlace_Wallet_SearchResetTooltip(isShow)
  if nil == isShow or false == isShow then
    TooltipSimple_Hide()
    return
  end
  local uiControl = MarketPlace_Wallet._ui.btn_editReset
...
function PaGlobalFunc_MarketPlace_Wallet_CreateControlHistroyList(contents, key)
  ...
  txt_Content:SetTextMode(__eTextMode_LimitText)
...
function MarketPlace_Wallet:setHistoryColor(historyType)
  ...
  elseif historyType == __eWorldMarket_HistoryType_SellWait then
    color = Defines.Color.C_FFF26A6A
  elseif historyType == __eWorldMarket_HistoryType_SellWaitFail then
  else
...
function PaGlobalFunc_MarketPlace_Wallet_CreateControlWalletItemList(contents, key)
  ...
  local originalIdx = Int64toInt32(key)
  local idx = originalIdx
  if PaGlobalFunc_AutoSortFunctionPanel_IsSorted(InvenSortLinkPanelIndex.MarketPlaceWalletMain) == true then
    idx = selfPlayer:getRealInventorySlotNoNew(__eInventorySortWhereType_MarketPlaceWallet, idx)
  end
  ...
  if nil ~= itemWrapper then
    ...
    slot.icon:addInputEvent("Mouse_On", "PaGlobalFunc_MarketPlace_Wallet_WalletListSlotToolTip(" .. originalIdx .. ")")
    ...
    local itemNameStr = PaGlobalFunc_MarketPlace_setNameAndEnchantLevel(itemWrapper:get()._key:getEnchantLevel(), itemWrapper:getItemType(), itemWrapper:getName(), itemWrapper:getItemClassify(), itemWrapper:isSpecialEnchantItem())
    txt_Name:SetText(itemNameStr)
    if true == txt_Name:IsLimitText() then
      btn_Slot:addInputEvent("Mouse_On", "PaGlobalFunc_Wallet_WalletListTextToolTipShow(" .. originalIdx .. ")")
      btn_Slot:addInputEvent("Mouse_Out", "TooltipSimple_Hide()")
      btn_Slot:setTooltipEventRegistFunc("PaGlobalFunc_Wallet_WalletListTextToolTipShow(" .. originalIdx .. ")")
    end
    if true == self._isSearch then
      local itemName = itemWrapper:getName()
      if true == stringSearch(itemName, self._filterText) then
        slot.icon:SetAlpha(1)
        txt_Name:SetMonoTone(false)
        local isEmptyText = nil == self._filterText or "" == self._filterText or 0 >= self._filterText:len()
        if nil == self._searchItemEffectOff[idx] and false == isEmptyText then
          slot.icon:AddEffect("fUI_Tuto_ItemHp_01A", true, 0, 0)
        end
        if false == isEmptyText then
          self._isSearchFind = true
          if 0 <= originalIdx - 3 then
            self._prevScrollIndex = originalIdx - 3
          else
...
function PaGlobalFunc_MarketPlace_GetWalletItemCount(itemEnchantKey)
  local walletItemListCount = getWorldMarketMyWalletListCount()
  for idx = 0, walletItemListCount - 1 do
    local itemMyWalletInfo = getWorldMarketMyWalletListByIdx(idx)
    if nil == itemMyWalletInfo then
      _PA_ASSERT(false, "\236\160\149\235\179\180\234\176\128 \236\152\172\235\176\148\235\165\180\236\167\128 \236\149\138\236\138\181\235\139\136\235\139\164!! : getWorldMarketMyWalletListByIdx(idx)")
      return
    end
    local walletItemEnchantKey = itemMyWalletInfo:getEnchantKey():get()
    local walletItemCount = itemMyWalletInfo:getItemCount()
    if itemEnchantKey == walletItemEnchantKey then
      return walletItemCount
    end
  end
  return 0
end
function PaGlobalFunc_MarketPlace_Wallet_CreateControlMyBiddingList(contents, key)
  ...
  local txt_registerTime = UI.getChildControl(contents, "Template_StaticText_RegisterTime")
  ...
  local btn_Retry = UI.getChildControl(contents, "Template_Button_Retry")
  local ringbuffIcon = UI.getChildControl(contents, "Template_Static_BigTrader")
  ...
  if self._selectedBiddingTabIndex == self._biddingTabIdx.sell then
    local itemInfo = getWorldMarketWaitListShowingSellBiddingListByIdx(idx)
    if nil == itemInfo then
      _PA_ASSERT(false, "\236\160\149\235\179\180\234\176\128 \236\152\172\235\176\148\235\165\180\236\167\128 \236\149\138\236\138\181\235\139\136\235\139\164!! : getWorldMarketWaitListShowingSellBiddingListByIdx( idx )")
      return
    end
    local itemSSW = itemInfo:getItemEnchantStaticStatusWrapper()
    local itemKey = itemSSW:get()._key:getItemKey()
    local itemEnchantKey = itemInfo:getEnchantKey():get()
    ...
    local isRingBuff = itemInfo:isRingBuff()
    local waitTime = itemInfo:getRegisterOrWaitEndTime()
    slot:setItemByStaticStatus(itemSSW, 0, -1, false, itemSSW:get():isCash(), false, 0, 0, nil, true, isSealed)
    slot.isEmpty = false
    local walletItemCount = PaGlobalFunc_MarketPlace_GetWalletItemCount(itemEnchantKey)
    local nameColor = PaGlobalFunc_MarketPlace_SetNameColor(nameColorGrade)
    txt_ItemName:SetFontColor(nameColor)
    local itemNameStr = PaGlobalFunc_MarketPlace_setNameAndEnchantLevel(chooseEnchantLevel, itemSSW:getItemType(), itemSSW:getName(), itemSSW:getItemClassify(), itemSSW:isSpecialEnchantItem())
    txt_ItemName:SetTextMode(__eTextMode_AutoWrap)
    txt_ItemName:SetText(itemNameStr)
    if nil ~= ToClient_getGameOptionControllerWrapper() and 0 < ToClient_getGameOptionControllerWrapper():getUIFontSizeType() then
      UI.setLimitTextAndAddTooltip(txt_ItemName)
    end
    local waitTimeStr
    if waitTime == nil or waitTime <= toInt64(0, 0) then
      waitTimeStr = "-"
    else
      waitTimeStr = os.date("%Y %m-%d %H:%M", Int64toInt32(waitTime))
    end
    ringbuffIcon:SetShow(isRingBuff)
    ...
    btn_Retry:addInputEvent("Mouse_LUp", "PaGlobalFunc_Retry_SellBidding(" .. itemKey .. "," .. chooseEnchantLevel .. ", " .. tostring(isSealed) .. ", " .. tostring(false) .. ")")
    ...
    btn_Retry:SetShow(true)
    txt_registerTime:SetShow(true)
    txt_registerTime:SetText(waitTimeStr)
    if Int64toInt32(sellNo) == 0 then
      btn_Withdraw:SetShow(false)
      btn_Retry:SetShow(false)
      txt_registerTime:SetFontColor(Defines.Color.C_FFD05D48)
    else
      txt_registerTime:SetFontColor(Defines.Color.C_FFF5BA3A)
    end
    if 0 == Int64toInt32(leftCount) and 0 == Int64toInt32(walletItemCount) then
      btn_Retry:SetMonoTone(true)
      btn_Retry:SetEnable(false)
    else
      btn_Retry:SetMonoTone(false)
      btn_Retry:SetEnable(true)
    end
  elseif self._selectedBiddingTabIndex == self._biddingTabIdx.buy then
    ...
    local itemSSW, itemKey, enchantLevel, chooseEnchantLevel, nameColorGrade, pricePerOne, baseMinPrice, leftCount, buyNo, boughtCount, registerMoneyCount
    if true == PaGlobal_TutorialPhase_IsTutorial() and nil ~= PaGlobal_TutorialPhase_MarketPlace_GetItemSSW() then
      ...
      itemKey = itemSSW:get()._key:getItemKey()
      ...
      baseMinPrice = itemInfo:getBaseMinPrice()
    ...
    local silverItemCount = silverInfo:getItemCount()
    local totalSilverItemCount = silverItemCount + pricePerOne
    ...
    btn_Retry:addInputEvent("Mouse_LUp", "PaGlobalFunc_Retry_BuyBidding(" .. itemKey .. "," .. chooseEnchantLevel .. ", " .. tostring(false) .. ", " .. tostring(true) .. ")")
    ...
    btn_Retry:SetShow(true)
    txt_registerTime:SetShow(false)
    if baseMinPrice > totalSilverItemCount then
      btn_Retry:SetMonoTone(true)
      btn_Retry:SetEnable(false)
    else
      btn_Retry:SetMonoTone(false)
...
function MarketPlace_Wallet:updateMyInfo()
  ...
  if self._ui.txt_MileageValue:GetShow() == true and self._isOpenedMileageFunction == true then
    local mileageInfo = getWorldMarketMileageInfo()
    self._ui.txt_MileageValue:SetText(makeDotMoney(mileageInfo:getItemCount()))
  end
  ...
  if self._ui.txt_MarketTaxBonus:GetShow() == true then
    ...
    if true == self._isOpenedMileageFunction then
      self._ui.txt_MarketTaxBonus:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_MAIL_DETAIL_SILVER_COUNT_TITLE", "value", 100))
    end
  end
  PaGlobalFunc_MarketPlace_WalletInventoryMoney(silverInfo)
end
function MarketPlaceWallet_updateSlotData()
  local self = MarketPlace_Wallet
  if self == nil then
    return
  end
...
function MarketPlace_Wallet:updateWallet()
  if PaGlobalFunc_MarketPlace_GetShow() == false then
    return
  end
  if PaGlobalFunc_WorldMarket_IsCurrentTabWallet() == false then
    return
  end
  if self._viewType.slot == self._selectedViewType then
    self:updateWallet_Slot()
  ...
  self._ui.txt_BuyCount:SetSize(math.max(25, self._ui.txt_BuyCount:GetTextSizeX() + 12), self._ui.txt_BuyCount:GetSizeY())
  self._ui.txt_BuyCount:ComputePos()
  self._ui.txt_BuyCount:SetShow(boughtCount > 0)
  self._ui.txt_SellCount:SetText(soldCount)
  self._ui.txt_SellCount:SetSize(math.max(25, self._ui.txt_SellCount:GetTextSizeX() + 12), self._ui.txt_SellCount:GetSizeY())
  self._ui.txt_SellCount:ComputePos()
...
function MarketPlace_Wallet:updateWallet_Slot()
  ...
  self._isSearchFind = false
  local isSorted = PaGlobalFunc_AutoSortFunctionPanel_IsSorted(InvenSortLinkPanelIndex.MarketPlaceWalletMain)
  if isSorted == true then
    PaGlobalFunc_AutoSortFunctionPanel_LoadSortOptionAndDoSort(InvenSortLinkPanelIndex.MarketPlaceWalletMain, false)
  end
  for slotIdx = 0, self._config.slotCount - 1 do
    ...
    if walletItemCount > originalSlotNo then
      ...
      if true == self._isSearch and nil ~= itemWrapper then
        local itemName = itemWrapper:getName()
        if true == stringSearch(itemName, self._filterText) then
          slot.icon:SetAlpha(1)
          if nil == self._searchItemEffectOff[slotNo] and false == isEmptyText then
            slot.icon:AddEffect("fUI_Tuto_ItemHp_01A", true, 0, 0)
          end
          if false == isEmptyText then
            self._isSearchFind = true
...
function PaGlobalFunc_MarketPlace_Wallet_BiddingListSlotToolTip(idx, biddingType)
  ...
  if self._biddingTabIdx.sell == biddingType then
    itemMyWalletInfo = getWorldMarketWaitListShowingSellBiddingListByIdx(idx)
...
function PaGlobalFunc_MarketPlace_Wallet_WalletListSlotToolTip(idx)
  ...
  local sortIndex = idx
  if PaGlobalFunc_AutoSortFunctionPanel_IsSorted(InvenSortLinkPanelIndex.MarketPlaceWalletMain) == true then
    sortIndex = selfPlayer:getRealInventorySlotNoNew(__eInventorySortWhereType_MarketPlaceWallet, idx)
  end
  local itemMyWalletInfo = getWorldMarketMyWalletListByIdx(sortIndex)
  if itemMyWalletInfo == nil then
    return
  end
  local itemWrapper = itemMyWalletInfo:getItemEnchantStaticStatusWrapper()
  ...
  local icon = UI.getChildControl(slot, "Static_Wallet_InvenItemList")
  if nil ~= icon then
    icon:EraseAllEffect()
  end
  self._searchItemEffectOff[idx] = true
  PaGlobalFunc_MarketPlace_ShowToolTip(itemWrapper, slot, false)
end
...
function MarketPlace_Wallet:updateBiddingItemList(isIndexNotChanged)
  local itemListCount = 0
  local sellAndWaitListItemCount = 0
  self._prevItemIdx = self._ui.list2_BiddingItem:getCurrenttoIndex()
  self._ui.list2_BiddingItem:getElementManager():clearKey()
  self._ui.btn_Buy:SetCheck(false)
  self._ui.btn_Sell:SetCheck(false)
  if self._selectedBiddingTabIndex == self._biddingTabIdx.sell then
    itemListCount = getWorldMarketSellBiddingListCount()
    sellAndWaitListItemCount = getWorldMarketWaitListShowingSellBiddingListCount()
    local soldCount, leftCount = self:getSoldBiddingCount()
    self._ui.btn_RecieveAll:SetShow(soldCount > 0)
    self:refreshRevieveAllButtonText(soldCount, self._biddingTabIdx.sell)
  ...
  elseif self._selectedBiddingTabIndex == self._biddingTabIdx.buy then
    ...
    self:refreshRevieveAllButtonText(boughtCount, self._biddingTabIdx.buy)
  ...
  if itemListCount + sellAndWaitListItemCount > 0 then
  ...
  if self._selectedBiddingTabIndex == self._biddingTabIdx.sell then
    for idx = itemListCount, itemListCount + sellAndWaitListItemCount - 1 do
      self._ui.list2_BiddingItem:getElementManager():pushKey(toInt64(0, idx))
    end
  end
  self._ui.list2_BiddingItem:requestUpdateVisible()
  if true == isIndexNotChanged and -1 ~= self._prevItemIdx then
    self._ui.list2_BiddingItem:moveIndex(self._prevItemIdx)
  end
end
...
function HandleLUp_MarketPlace_Wallet_ToggleSortOptionView()
  local self = MarketPlace_Wallet
  if self == nil then
    return
  end
  PaGlobalFunc_AutoSortFunctionPanel_ShowToggle(InvenSortLinkPanelIndex.MarketPlaceWalletMain)
end
function PaGlobalFunc_MarketPlcae_Wallet_RecieveAll()
  ...
  elseif false == isPremiumUser and false == _ContentsGroup_OldMoonSmelting then
...
function FromClient_MarketPlace_Wallet_noticeMyBiddingList(itemEnchantKey, noticeType)
  ...
  local itemNameStr = FGlobal_ChangeItemNameColorByGrade(ItemEnchantKey(itemEnchantKey))
  local str
  ...
  local logTime = tostring(paTime:GetHour()) .. ":" .. tostring(paTime:GetMinute())
  PaGlobalFunc_Widget_Alert_Check_MarketPlaceMyBiddingUpdate(str, logTime)
  FGlobal_ItemMarket_AlarmIcon_Reload()
end
...
function FromClient_MarketPlace_Wallet_responseCalculateSellBidding()
  ...
  self:updateHistory()
  self:updateMyInfo()
  local lastChangeSilver = ToClient_getLastVariedSilver()
  if lastChangeSilver <= toInt64(0, 0) then
    return
  elseif lastChangeSilver > toInt64(0, 0) and lastChangeSilver < toInt64(0, 1000000) then
    audioPostEvent_SystemUi(6, 3)
  elseif lastChangeSilver >= toInt64(0, 1000000) and lastChangeSilver < toInt64(0, 10000000) then
    audioPostEvent_SystemUi(6, 5)
  elseif lastChangeSilver >= toInt64(0, 10000000) and lastChangeSilver < toInt64(0, 50000000) then
    audioPostEvent_SystemUi(6, 6)
  elseif lastChangeSilver >= toInt64(0, 50000000) and lastChangeSilver < toInt64(0, 200000000) then
    audioPostEvent_SystemUi(6, 7)
  elseif lastChangeSilver >= toInt64(0, 200000000) and lastChangeSilver < toInt64(0, 500000000) then
...
function MarketPlace_Wallet:biddingOpen(tabIdx)
  local self = MarketPlace_Wallet
  local isIndexNotChanged = false
  if self._selectedBiddingTabIndex == tabIdx then
    isIndexNotChanged = true
  end
  self._selectedBiddingTabIndex = tabIdx
  self:updateBiddingItemList(isIndexNotChanged)
end
function PaGlobal_SellBidding_Calculate(idx)
  ...
  if false == isPremiumUser and false == itemSSW:get():isCash() and false == _ContentsGroup_OldMoonSmelting then
    FromClient_ValuePackageIcon()
    local messageBoxTitle = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ALERT_NOTIFICATIONS")
...
function PaGlobalFunc_Wallet_HistoryUpdate()
  local self = MarketPlace_Wallet
  if nil == self then
    _PA_ASSERT(false, "\237\140\168\235\132\144\236\157\180 \236\161\180\236\158\172\237\149\152\236\167\128 \236\149\138\236\138\181\235\139\136\235\139\164!! : MarketPlace")
    return
...
function PaGlobalFunc_Wallet_Open()
  ...
  PaGlobalFunc_AutoSortFunctionPanel_LinkPanel(InvenSortLinkPanelIndex.MarketPlaceWalletMain, _panel, self._ui.btn_sortOption, self._ui.chk_sort, nil, nil)
  PaGlobalFunc_AutoSortFunctionPanel_LoadSortOptionAndDoSort(InvenSortLinkPanelIndex.MarketPlaceWalletMain, false)
  PaGlobalFunc_MarketPlace_Wallet_ClearFocusSearchText()
  PaGlobalFunc_MarketPlace_Wallet_ClearFocusSearchTextBidding()
...
function PaGlobalFunc_Wallet_Close()
  ...
  if PaGlobalFunc_AutoSortFunctionPanel_GetShow(InvenSortLinkPanelIndex.MarketPlaceWalletMain) == true then
...
function PaGlobal_MarketPlace_Wallet_All_FindItemSlot(findItemKey)
  local self = MarketPlace_Wallet
  local selfPlayerWrapper = getSelfPlayer()
  if selfPlayerWrapper == nil then
    return
  ...
  if self._viewType.list == self._selectedViewType then
    local key64 = toInt64(0, targetSlotNo)
    local contents = self._ui.list2_WalletItem:GetContentByKey(key64)
    targetControl = contents
  end
