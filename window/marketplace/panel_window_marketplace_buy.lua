...
function MarketPlaceBuy:createControl()
  ...
  self._ui._walletCount = UI.getChildControl(self._ui._stc_TotalPriceBg, "StaticText_MyWalletHasCount")
  ...
  self._ui._txtBuyDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  local rdoDaysGroup = UI.getChildControl(Panel_Window_MarketPlace_BuyManagement, "Static_RadioBtnGroup")
  self._ui._rdo_days = {
    [1] = {
  ...
  for ii = 1, #self._ui._rdo_days do
    ...
    self._ui._rdo_days[ii].control:addInputEvent("Mouse_LUp", " HandleLClick_MarketPlace_Buy_SelectDays(" .. ii .. ")")
  end
...
function PaLocal_MarketPlace_GraphTest(input)
  if 1 == input then
    for i = 1, 10 do
      graphTest[i] = UI.createControl(__ePAUIControl_StaticText, _panel, "StaticText_TestPoint_" .. i)
...
function PaGlobal_MarketPlaceBuy_SelectCount()
  ...
  if nil ~= itemSSW then
    ...
    local sellCountBiddingList = ToClient_GetSellCountBiddingList()
    local addBuyRefCount = itemSSW:getAddBuyRefCountForWorldMarket()
    local addBuyCount = itemSSW:getAddBuyCountForWorldMarket()
    if addBuyRefCount ~= nil and addBuyRefCount > toInt64(0, 0) and sellCountBiddingList >= addBuyRefCount then
      maxRegisterCount = addBuyCount
    end
...
function PaGlobal_MarketPlaceBuy_Req()
  ...
  if true == self._isOtpItem and lastOtpConfirm < utcTime and true == _ContentsGroup_WorldMarketOTP then
    messageBoxMemo = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_MARKETPLACE_OTP_BUY_MSG", "itemName", tostring(self._itemnameColor .. self._buyitemName .. "<PAOldColor>"), "itemCount", tostring(self._selectCount), "itemSumPrice", makeDotMoney(needTotalPrice))
  end
...
function MarketPlaceBuy:initData(itemKey, minEnchantLevel, maxEnchantLevel, standardPrice_s64, itemCount_s64, biddingBuyListCount, lastPrice_s64, lastTime_utc, totalCount_s64, minPrice, maxPrice, isInit)
  ...
  if standardPrice_s64 < silverInfo:getItemCount() + ToClient_GetRetryRemainderSilver() then
    self._maxItemCount_s64 = (silverInfo:getItemCount() + ToClient_GetRetryRemainderSilver()) / standardPrice_s64
  else
    self._maxItemCount_s64 = toInt64(0, 0)
  end
  local itemSSW = getItemEnchantStaticStatus(ItemEnchantKey(self._itemKey, self._itemMinLevel))
  local maxRegisterCount = toInt64(0, 0)
  if nil ~= itemSSW then
  ...
  local hasCount = getHasCount(ItemEnchantKey(self._itemKey, self._itemMinLevel), minEnchantLevel)
  self._ui._walletCount:SetText(makeDotMoney(hasCount))
...
function FromClient_responseBuyItemToWorldMarket(mySilver)
  ...
  ToClient_requestGetBiddingList(__eWorldMarketKeyType_Item, self._itemKey, self._itemMinLevel, true, false, PaGlobalFunc_ItemMarket_GetCurrentCategory())
end
...
function HandleLClick_MarketPlace_CountButton(type, isPlus)
  ...
  elseif 2 == type then
    ...
    if resultValue >= self._itemMinLevel and resultValue <= self._itemMaxLevel then
      ...
      local hasCount = getHasCount(ItemEnchantKey(self._itemKey, self._itemMinLevel), resultValue)
      self._ui._walletCount:SetText(makeDotMoney(hasCount))
    end
  elseif 3 == type then
...
function FromClient_MarketPlaceBuy_responseMarketPriceInfo()
  if true == PaGlobal_MarketPlace_DrawGraph(MarketPlaceBuy, Panel_Window_MarketPlace_BuyManagement) then
    PaGlobalFunc_MarketPlaceBuy_SelectPriceByIndex(0)
    PaGlobalFunc_MarketPlacePriceList_SetDefaultIndex(false)
  end
end
function PaGlobal_MarketPlace_DrawGraph(metaTable, panel)
  ...
  local selectIndex = 0
  for ii = 1, #self._ui._rdo_days do
    if true == self._ui._rdo_days[ii].control:IsCheck() then
      selectIndex = ii
    end
  end
  ...
  if false == HandleLClick_MarketPlace_SelectDays(self, selectIndex) then
    return false
  end
  panel:SetShow(true)
  return true
end
function PaGlobal_MarketPlace_PriceMinMax(days, maxDays)
  local initV2 = ToClient_GetWorldMarketPriceGraphPosAt(maxDays - days)
  local minPrice = initV2.y
  local maxPrice = initV2.y
  for i = 1, days - 1 do
    local v2 = ToClient_GetWorldMarketPriceGraphPosAt(maxDays - days + i)
...
function HandleLClick_MarketPlace_Buy_SelectDays(index)
  HandleLClick_MarketPlace_SelectDays(MarketPlaceBuy, index)
end
function HandleLClick_MarketPlace_SelectDays(metaTable, index)
  local self = metaTable
  if nil == self or nil == self._ui._rdo_days[index] then
    return false
  end
  self._graph.panel:ClearGraphList()
  local maxDays = 90
  local days = self._ui._rdo_days[index].days
  local minPrice, maxPrice = PaGlobal_MarketPlace_PriceMinMax(days, maxDays)
  ...
  local dateIntervalX = self._graph.panelSizeX / (days - 1)
  ...
  self._graph.panel:SetSize(dateIntervalX * (days - 1), self._graph.panel:GetSizeY())
  self._graph.panel:ComputePos()
  for i = 1, 5 do
    self._graph[i].txt_Price:SetText(makeDotMoney(math.floor(avgPrice + avgPrice * avgInterval[i] + 0.5)))
    self._graph[6 - i].txt_Date:SetText(ToClient_GetWorldMarketBerforeDay((i - 2) * days / 3))
  end
  for i = 0, days - 1 do
    local vector2_DrawPos = ToClient_GetWorldMarketPriceGraphPosAt(maxDays - days + i)
