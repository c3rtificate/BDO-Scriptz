...
local MarketPlace_MyInven = {
  _ui = {
    ...
    btn_Deposit_Mileage = UI.getChildControl(_panel, "Button_Deposit_Tax_CH"),
...
function MarketPlace_MyInven:initControl()
  ...
  self._ui.txt_MileageValue = UI.getChildControl(self._ui.btn_Deposit_Mileage, "StaticText_MileageValue")
  ...
  self._ui.btn_Deposit_Mileage:SetShow(self._useMileageFunction)
end
function MarketPlace_MyInven:initEvent()
  ...
  self._ui.btn_sortOption:addInputEvent("Mouse_LUp", "HandleLUp_MarketPlace_MyInven_ToggleSortOptionView()")
  ...
  self._ui.btn_Deposit_Mileage:addInputEvent("Mouse_LUp", "InputMLUp_MarketWallet_RegisterMileage()")
  self._ui.btn_Deposit_Mileage:addInputEvent("Mouse_On", "MarketWallet_DepositMileage_TooltipShow()")
  self._ui.btn_Deposit_Mileage:addInputEvent("Mouse_Out", "MarketWallet_DepositMileage_TooltipHide()")
...
function MarketPlace_MyInven:initPanelSize()
  if true == self._useMileageFunction then
    self._ui.btn_Deposit:SetSpanSize(self._ui.btn_Deposit:GetSpanSize().x, self._ui.btn_Deposit_Mileage:GetSizeY() + self._ui.btn_Deposit_Mileage:GetSpanSize().y + 5)
    _panel:SetSize(_panel:GetSizeX(), _panel:GetSizeY() + self._ui.btn_Deposit_Mileage:GetSizeY())
    self._ui.btn_Deposit:ComputePos()
    self._ui.btn_Deposit_Mileage:ComputePos()
  end
end
...
function MarketWallet_DepositMileage_TooltipShow()
  local uiControl = MarketPlace_MyInven._ui.btn_Deposit_Mileage
  local name = PAGetString(Defines.StringSheet_GAME, "LUA_MARKETPLACE_M_DEPOSIT_TITLE")
  local desc = PAGetString(Defines.StringSheet_GAME, "LUA_MARKETPLACE_M_DEPOSIT_DESC")
  TooltipSimple_Show(uiControl, name, desc)
end
function MarketWallet_DepositMileage_TooltipHide()
  TooltipSimple_Hide()
end
...
function MarketPlaceMyInventory_updateSlotData()
  local self = MarketPlace_MyInven
  if self == nil then
    return
  end
...
function HandleLUp_MarketPlace_MyInven_ToggleSortOptionView()
  local self = MarketPlace_MyInven
  if self == nil then
    return
  end
  PaGlobalFunc_AutoSortFunctionPanel_ShowToggle(InvenSortLinkPanelIndex.MarketPlaceInventory)
end
...
function MarketPlace_MyInven:findSearchItem(isSearch)
  ...
  for slotIdx = 0, self._config.slotCount - 1 do
    ...
    if slot ~= nil and slot.icon ~= nil and slot.slotNo ~= nil and slotBg ~= nil and slotBg.lock ~= nil and slotBg.lock:GetShow() == false then
      if true == isSearch then
...
function MarketPlace_MyInven:reScanSearchItem()
  ...
  local isSorted = PaGlobalFunc_AutoSortFunctionPanel_IsSorted(InvenSortLinkPanelIndex.MarketPlaceInventory)
  local slotNoList = Array.new()
  if isSorted == true then
    PaGlobalFunc_AutoSortFunctionPanel_LoadSortOptionAndDoSort(InvenSortLinkPanelIndex.MarketPlaceInventory, false)
    slotNoList:fill(useStartSlot, invenMaxSize - 1)
    for ii = 1, invenUseSize - useStartSlot do
      slotNoList[ii] = selfPlayer:getRealInventorySlotNoNew(__eInventorySortWhereType_MarketPlaceInventory, ii + sortIndexValue)
    end
  end
...
function PaGlobalFunc_MarketPlace_SelectTab(tabIdx)
  ...
  tempWrapper:setCurrentMarketPlaceWhereType(tabIdx)
...
function PaGlobalFunc_MarketPlace_MyInven_GetMoney()
  ...
  if false == PaGlobalFunc_MarketPlace_GetWareHouseCheck() then
    moneyValue = getSelfPlayer():get():getMoneyInventory():getMoney_s64()
  else
...
function MarketPlace_MyInven:update()
  ...
  local selfPlayerWrapper = getSelfPlayer()
  local selfPlayer = selfPlayerWrapper:get()
  local moneyTitle = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_MYMONEY")
  local moneyValue = makeDotMoney(selfPlayer:getMoneyInventory():getMoney_s64())
  self._ui.txt_MoneyTitle:SetText(moneyTitle)
  self._ui.txt_MoneyValue:SetText(moneyValue)
  if true == self._useMileageFunction then
    local mileageValue = makeDotMoney(selfPlayer:getMileage())
...
function MarketPlace_MyInven:updateInventory()
  ...
  local slotNoList = Array.new()
  slotNoList:fill(useStartSlot, invenMaxSize - 1)
  if PaGlobalFunc_AutoSortFunctionPanel_IsSorted(InvenSortLinkPanelIndex.MarketPlaceInventory) == true then
    PaGlobalFunc_AutoSortFunctionPanel_LoadSortOptionAndDoSort(InvenSortLinkPanelIndex.MarketPlaceInventory, false)
    for ii = 1, invenUseSize - 2 do
      slotNoList[ii] = selfPlayer:getRealInventorySlotNoNew(__eInventorySortWhereType_MarketPlaceInventory, ii - 1)
    end
  end
  for slotIdx = 0, self._config.slotCount - 1 do
    local slotBg = self._slotInvenBgData[slotIdx]
...
function MarketPlace_MyInven:updateWareHouseInventory()
  ...
  if PaGlobalFunc_AutoSortFunctionPanel_IsSorted(InvenSortLinkPanelIndex.MarketPlaceInventory) == true then
    ...
    for ii = 1, useMaxCount - 1 do
      slotNoList[ii] = selfPlayer:getRealInventorySlotNoNew(__eInventorySortWhereType_MarketPlaceInventory, ii)
    end
  end
  for slotIdx = 0, self._config.slotCount - 1 do
    local slotBg = self._slotInvenBgData[slotIdx]
...
function PaGlobalFunc_MarketPlace_MyInven_Open(isWaretype)
  ...
  _panel:SetPosX(scrSizeX - panelSizeX - 10)
  _panel:SetPosY(scrSizeY / 2 - panelSizeY / 2)
  if 10 > _panel:GetPosY() then
    _panel:SetPosY(10)
  end
  PaGlobalFunc_AutoSortFunctionPanel_LinkPanel(InvenSortLinkPanelIndex.MarketPlaceInventory, _panel, self._ui.btn_sortOption, self._ui.checkBtn_ItemSort, nil, nil)
  PaGlobalFunc_AutoSortFunctionPanel_LoadSortOptionAndDoSort(InvenSortLinkPanelIndex.MarketPlaceInventory)
  PaGlobalFunc_MarketPlace_ClearFocusSearchText()
  self:open()
...
function PaGlobalFunc_MarketPlace_MyInven_Close()
  ...
  if PaGlobalFunc_AutoSortFunctionPanel_GetShow(InvenSortLinkPanelIndex.MarketPlaceInventory) == true then
    PaGlobalFunc_AutoSortFunctionPanel_Hide()
  end
  local tempWrapper = getTemporaryInformationWrapper()
  tempWrapper:setCurrentMarketPlaceWhereType(CppEnums.ItemWhereType.eCount)
  PaGlobalFunc_MarketPlace_ClearFocusSearchText()
  self:close()
  if nil ~= Panel_Window_MarketPlace_Function and false == Panel_Window_MarketPlace_Function:GetShow() and nil ~= Panel_Window_MarketPlace_WalletInventory and false == Panel_Window_MarketPlace_WalletInventory:GetShow() then
  ...
  if nil ~= self._newItemData and self._newItemData._whereType == self._currentInvenType and nil ~= self._newItemData._slotNo then
    ...
    if nil ~= self._newItemData._itemKey and nil ~= self._slotInvenBgData[savedSlotBgIdx] and nil ~= self._slotInvenItemData[savedSlotBgIdx] then
      if false == self._slotInvenItemData[savedSlotBgIdx].isEmpty then
        ...
        elseif nil ~= self._slotInvenBgData[savedSlotBgIdx] then
...
function PaGlobal_MarketPlace_MyInven_All_FindItemSlot(findItemKey)
  local self = MarketPlace_MyInven
  local selfPlayerWrapper = getSelfPlayer()
  if nil == selfPlayerWrapper then
    return
  ...
  if true == isSearchFind then
    ...
    for ii = 0, self._config.slotCount - 1 do
      local slot = self._slotInvenItemData[ii]
      if slot ~= nil and slot.icon ~= nil then
        local slotNo = ii + 2 + startSlot
        if isSorted == true then
          slotNo = selfPlayer:getRealInventorySlotNoNew(__eInventorySortWhereType_MarketPlaceInventory, ii + startSlot)
        end
