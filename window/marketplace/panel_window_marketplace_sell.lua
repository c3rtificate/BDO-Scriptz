...
local MarketPlaceSell = {
  _ui = {
    ...
    _txt_applyRingBuff = UI.getChildControl(_panel, "StaticText_BigTrader"),
...
function MarketPlaceSell:createControl()
  ...
  if _ContentsOption_CH_GameType == true then
    self._ui.btn_maxPrice:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_MARKETPLACE_BUTTON_MAX"))
    self._ui.btn_minPrice:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_MARKETPLACE_BUTTON_MIN"))
  end
  ...
  self._ui._txtSellDesc:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui._chk_ringbuff = UI.getChildControl(self._ui._txt_applyRingBuff, "CheckButton_BigTrader")
  local slot = {}
  ...
  local rdoDaysGroup = UI.getChildControl(Panel_Window_MarketPlace_SellManagement, "Static_RadioBtnGroup")
  self._ui._rdo_days = {
    [1] = {
      control = UI.getChildControl(rdoDaysGroup, "RadioButton_1"),
      days = 30
    },
    [2] = {
      control = UI.getChildControl(rdoDaysGroup, "RadioButton_2"),
      days = 90
    }
  }
  for ii = 1, #self._ui._rdo_days do
    if ii == 1 then
      self._ui._rdo_days[ii].control:SetCheck(true)
    else
      self._ui._rdo_days[ii].control:SetCheck(false)
    end
    self._ui._rdo_days[ii].control:addInputEvent("Mouse_LUp", " HandleLClick_MarketPlace_Sell_SelectDays(" .. ii .. ")")
  end
  self._ui._stc_ItemName:SetTextMode(__eTextMode_AutoWrap)
...
function MarketPlaceSell:graphControlInitialize()
  ...
  self._graph.panel = UI.createControl(__ePAUIControl_Graph, self._ui.stc_GraphBg, "Graph_Panel")
  ...
  for i = 1, 5 do
    ...
    tmpGraph.txt_Price = UI.createControl(__ePAUIControl_StaticText, self._ui.stc_GraphBg, "StaticText_Price_" .. i)
    tmpGraph.stc_PriceLine = UI.createControl(__ePAUIControl_Static, self._ui.stc_GraphBg, "Static_PriceLine_" .. i)
    tmpGraph.txt_Date = UI.createControl(__ePAUIControl_StaticText, self._ui.stc_GraphBg, "StaticText_Date_" .. i)
...
function PaGlobal_MarketPlaceSell_SelectCount()
  ...
  if nil ~= itemSSW then
    local maxRegisterCount = itemSSW:getSellCountForWorldMarket()
...
function MarketPlaceSell:open(itemKey, enchantLevel, enchantMinLevel, standardPrice_s64, itemCount_s64, isSealed, biddingSellListCount, lastPrice_s64, lastTime_utc, totalCount_s64, minPrice, maxPrice)
  ...
  if nil == ringCount or ringCount <= 0 or 55 == itemSSW:get():getItemMarketMainCategoryNo() then
    self._ui._txt_applyRingBuff:SetShow(false)
  elseif ringCount >= 1 then
    self._ui._txt_applyRingBuff:SetShow(true)
    local ringBuffString = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WINDOW_MARKETPLACE_RINGBUFF_COUNT", "count", tostring(ringCount))
    self._ui._txt_applyRingBuff:SetText(ringBuffString)
  end
  ...
  if nil ~= itemSSW then
    maxRegisterCount = itemSSW:getSellCountForWorldMarket()
    if true == itemSSW:get():isCash() then
      maxRegisterCount = toInt64(0, 100)
    end
...
function PaGlobal_MarketPlaceSell_Req()
  ...
  self._isOtpItem = ToClient_IsOTPWorldMarketItem(enchantKey, self._standardPrice_s64)
  ...
  local function sellExcute()
    ...
    local lastOtpConfirm = getWorldMarketLastOtpConfirm()
    local utcTime = Int64toInt32(getUtc64())
    if nil == ringCount or 0 == ringCount then
      isRingBuff = false
    elseif -1 == ringCount then
      isRingBuff = true
    elseif ringCount >= 1 then
      isRingBuff = self._ui._chk_ringbuff:IsCheck()
    end
    if 55 == itemSSW:get():getItemMarketMainCategoryNo() then
      isRingBuff = false
    end
    local function requestSellItemToWorldMarket()
      if self._isOtpItem == true and lastOtpConfirm < utcTime and _ContentsGroup_WorldMarketOTP == true then
    ...
    if isPremiumUser == false and itemSSW:get():isCash() == false and _ContentsGroup_OldMoonSmelting == false then
      local itemListCount = getWorldMarketSellBiddingListCount()
      local soldCount = toInt64(0, 0)
      for idx = 0, itemListCount - 1 do
        local itemInfo = getWorldMarketSellBiddingListByIdx(idx)
        if itemInfo == nil then
          _PA_ASSERT(false, "PaGlobal_MarketPlaceSell_Req:: getWorldMarketSellBiddingListByIdx( idx ) itemInfo\235\165\188 \234\176\128\236\160\184\236\152\164\235\138\148\235\141\176 \236\139\164\237\140\168\237\150\136\236\138\181\235\139\136\235\139\164. ")
          return
        end
        local currentItemSSW = itemInfo:getItemEnchantStaticStatusWrapper()
        if currentItemSSW == nil then
          _PA_ASSERT(false, "PaGlobal_MarketPlaceSell_Req:: itemInfo:getItemEnchantStaticStatusWrapper() WorldTradeMarketMySellBiddingInfo\235\161\156\235\182\128\237\132\176 ItemEnchantStaticStatusWrapper\235\165\188 \234\176\128\236\160\184\236\152\164\235\138\148\235\141\176 \236\139\164\237\140\168\237\150\136\236\138\181\235\139\136\235\139\164.")
          return
        end
        if currentItemSSW:get()._key:get() == self._itemKey then
          soldCount = itemInfo:getSoldCount()
          break
        end
      end
      if soldCount > toInt64(0, 0) then
        local messageBoxTitle = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ALERT_NOTIFICATIONS")
        local messageBoxMemo = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_MARKETPLACE_SELL_PCROOM_MEMO2", "forPremium", requestGetRefundPercentForPremiumPackage())
        local messageBoxData = {
          title = messageBoxTitle,
          content = messageBoxMemo,
          functionYes = requestSellItemToWorldMarket,
  ...
  local function checkbox_sell()
    ...
    local lastOtpConfirm = getWorldMarketLastOtpConfirm()
    local utcTime = Int64toInt32(getUtc64())
    if true == self._isOtpItem and lastOtpConfirm < utcTime and true == _ContentsGroup_WorldMarketOTP then
      messageBoxMemo = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_MARKETPLACE_OTP_SELL_MSG", "itemName", tostring(self._itemnameColor .. self._sellitemName .. "<PAOldColor>"), "itemCount", tostring(self._selectCount), "itemSumPrice", makeDotMoney(needTotalPrice))
    end
    local messageBoxTitle = PAGetString(Defines.StringSheet_GAME, "LUA_MARKETPLACE_SELL_TITLE")
    local messageBoxDesc = PAGetString(Defines.StringSheet_GAME, "LUA_MARKETPLACE_VALUEPACKAGE_WARNING_MSG")
    ...
    if lastOtpConfirm < utcTime and true == _ContentsGroup_WorldMarketOTP then
...
function FromClient_responseSellItemToWorldMarket(mySilver)
  ...
  PaGlobalFunc_Wallet_HistoryUpdate()
  if nil == self then
    return
  end
  local self = MarketPlaceSell
...
function FromClient_MarketPlaceSell_responseMarketPriceInfo()
  if true == PaGlobal_MarketPlace_DrawGraph(MarketPlaceSell, Panel_Window_MarketPlace_SellManagement) then
    PaGlobalFunc_MarketPlaceSell_SelectPriceByIndex(0)
...
function Update_MarketPlaceSell_HoverPrice(deltatime)
  ...
  if self._selectDays < 0 then
    return
  end
  local dateIntervalX = self._graph.panelSizeX / (self._selectDays - 1)
...
function HandleLClick_MarketPlace_Sell_SelectDays(index)
  HandleLClick_MarketPlace_SelectDays(MarketPlaceSell, index)
end
