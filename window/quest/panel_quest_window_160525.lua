...
local regionString = {
  ...
  [QuestRegionType.eQuestRegionType_Odilita] = PAGetString(Defines.StringSheet_GAME, "LUA_QUESTLIST_HISTORY_TXT_REGION_11")
}
local regionOpen = {
  ...
  [QuestRegionType.eQuestRegionType_Odilita] = true,
...
local questSortArrayRegion = {
  ...
  [QuestRegionType.eQuestRegionType_Odilita] = {}
}
local questArrayRegionCount = {
  ...
  [QuestRegionType.eQuestRegionType_Odilita] = 0
}
...
local questArrayRegionSort = {
  ...
  [10] = QuestRegionType.eQuestRegionType_Odilita,
  [11] = QuestRegionType.eQuestRegionType_None
}
    ...
    if QuestTabType.QuestTabType_Progress == progressingActiveTab then
      if true == questGroupInfo:isGroupQuest() then
        ...
        if 1 == questCount then
          ...
          if false == uiQuestInfo:isSpecialType() and false == uiQuestInfo:isRegionMonsterKillType() and false == uiQuestInfo:isSupplyServantType() and false == uiQuestInfo:isSeasonPassType() and false == uiQuestInfo:isGrowthPassType() then
        ...
        for questIdx = startIdx, questCount - 1 do
          ...
          local dataRegionIdx = uiQuestInfo:getQuestRegion()
          if dataRegionIdx >= QuestRegionType.eQuestRegionType_Count or dataRegionIdx < QuestRegionType.eQuestRegionType_None then
        ...
        for index = 0, regionCount do
          do
            ...
            local function doInsertData()
              ...
              if false == _ContentsGroup_Othilita and QuestRegionType.eQuestRegionType_Odilita - 1 == index then
                return
              end
              useArray[arrayIdx] = {
                isQuest = false,
  ...
  for questIdx = QuestWindow.config.startSlotIndex, self.config.listCount do
    ...
    if 0 < self.config.listCount then
      ...
      if false == useArray[questIdx].isQuest then
        ...
        ToClient_ChangeQuestIconByQuestType(self.uiPool.groupTitle[uiCount].typeIcon, iconType)
        ...
        ToClient_ChangeQuestIconByQuestType(self.uiPool.listMain[uiCount].typeIcon, useArray[questIdx].questType)
        if QuestTabType.QuestTabType_Progress == progressingActiveTab then
        ...
        if not useArray[questIdx].isNext then
          ...
          if useArray[questIdx].isCleared then
            ...
            self.uiPool.listMain[uiCount].name:SetTextMode(__eTextMode_LimitText)
            ...
            if QuestTabType.QuestTabType_Progress == progressingActiveTab and isGroup then
              ...
              if beforIdx >= 0 then
                ToClient_ChangeQuestIconByQuestType(self.uiPool.listMain[beforIdx].typeIcon, useArray[questIdx].questType)
...
function QuestWindow:registEventHandler()
  ...
  PaGlobal_Util_RegistHelpTooltipEvent(self.ui.btn_Question, "\"PanelQuestHistory\"")
  if true == _ContentsOption_CH_Help then
    self.ui.btn_Question:SetShow(false)
    self.ui.checkPopUp:SetSpanSize(self._ui._btn_Question:GetSpanSize().x, self._ui._btn_Question:GetSpanSize().y)
  end
