...
function rewardTooltip_ForQuestWidgetInfo(rewardType, isShow, questtype, index, key, value)
  ...
  elseif "ExploreExp" == rewardType then
    name = PAGetString(Defines.StringSheet_GAME, "LUA_QUESTREWARD_SIMPLE_TOOLTIP_EXPLOREEXP")
    desc = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_QUESTREWARD_SIMPLE_TOOLTIP_EXPLOREEXP_DESC", "exploreExp", key)
  elseif "FamilyStat" == rewardType then
    name = PaGlobalFunc_GetFamiltyStatRewardToolipText_Title()
    desc = PaGlobalFunc_GetFamiltyStatRewardToolipText_Desc(key, value)
  end
...
function HandleEventLUp_QuestInfo_All_AgrisAcceptCheck(isAccept, groupId, questId)
  if false == isAccept then
    PaGlobalFunc_QuestInfo_All_Close()
    return
  end
  if false == _ContentsGroup_FeverQuest then
    return
  end
  if false == questList_isAcceptableQuest(groupId, questId) then
    return
  end
  ToClient_AcceptQuest(groupId, questId)
  PaGlobalFunc_QuestInfo_All_Close()
end
function FromClient_QuestInfo_All_RandomQuestClicked(groupId, questId)
  if nil == Panel_Window_QuestInfo_All then
    return
  end
  ...
  PaGlobal_QuestInfo_All:openInfoSave(groupId, questId, nil, "nil", 0, nil, nil, nil, nil, nil, nil, true)
...
function PaGlobalFunc_QuestInfo_All_Detail(groupId, questId, uiCondition, groupTitle, questGroupCount, fromQuestWidget, isRecommand, isNextQuest, isCleared, isAccepted, uiIdx)
  ...
  if Panel_Window_QuestInfo_All:GetShow() == false then
    PaGlobalFunc_QuestInfo_All_Open()
  else
    PaGlobalFunc_QuestInfo_All_Update()
  end
...
function HandleEventLOnOut_QuestInfo_All_ButtonTooltip(isShow, buttonIndex)
  if PaGlobal_QuestInfo_All == nil then
    return
  end
  ...
  if buttonIndex == 0 then
    targetControl = PaGlobal_QuestInfo_All._ui_pc.btn_giveUp
  elseif buttonIndex == 1 then
    targetControl = PaGlobal_QuestInfo_All._ui_pc.chk_questNavi
  elseif buttonIndex == 2 then
    targetControl = PaGlobal_QuestInfo_All._ui_pc.chk_autoNavi
  else
    return
  end
  ...
  local txt = UI.getChildControl(targetControl, "StaticText_ButtonName")
  if txt == nil then
    return
  end
  name = txt:GetText()
