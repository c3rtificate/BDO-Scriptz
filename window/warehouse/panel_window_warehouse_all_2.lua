function HandleEventLUp_Warehouse_All_PopMoney()
  local slotNo = getWarehouseSlotNo()
  if nil == slotNo then
    return
  end
...
function HandleEventRUp_Warehouse_All_Item(index)
  ...
  if Panel_Window_GuildPresent:GetShow() == true then
    ...
    PaGlobal_GuildPresent_All_PushItem(slotNo)
    return
  end
...
function HandleEventLUp_Warehouse_All_TotalSearch()
  if Panel_Window_TotalSearch == nil or PaGlobalFunc_TotalSearch_GetShow() == true then
    return
  end
  PaGlobalFunc_TotalSearch_OpenByWareHouse()
end
...
function HandleEvent_Warehouse_All_CargoSellOpen()
  if true == _ContentsGroup_CargoSell then
    PaGlobal_WareHouse_All:prepareClose()
    if PaGlobal_WareHouse_All._isConsole == false then
...
function HandleEventLUp_Warehouse_All_GuildWareHouseHistory()
  if Panel_Window_GuildPresent:GetShow() == true then
    PaGlobal_GuildPresent_All_Close()
  end
...
function HandleEventLUp_Warehouse_All_LDownClick(index)
  ...
  if true == _ContentsOption_CH_GameType and 0 == PaGlobal_WareHouse_All.savedWayPointKey and CppEnums.WarehoouseFromType.eWarehoouseFromType_Npc == PaGlobal_WareHouse_All._fromType then
    PaGlobal_WareHouse_All.savedWayPointKey = PaGlobal_WareHouse_All._currentWaypointKey
  end
...
function HandleEvent_Warehouse_All_Scroll(isUp)
  ...
  if PaGlobal_WareHouse_All:isGuildHouse() == true and warehouseWrapper:isExistGuildPresentItem() == false and Panel_Window_GuildPresent:GetShow() == true then
    return
  end
...
function HandleEventMOut_Warehouse_All_IconOut(index)
  local isGuildWare = CppEnums.WarehoouseFromType.eWarehoouseFromType_GuildItemShare == PaGlobal_WareHouse_All._fromType or CppEnums.WarehoouseFromType.eWarehoouseFromType_GuildHouse == PaGlobal_WareHouse_All._fromType or CppEnums.WarehoouseFromType.eWarehoouseFromType_GuildSafe == PaGlobal_WareHouse_All._fromType
  ...
  if false == isGuildWare and nil ~= PaGlobal_WareHouse_All._slotFilterEffect[index] then
    slot.icon:EraseEffect(PaGlobal_WareHouse_All._slotFilterEffect[index])
    PaGlobal_WareHouse_All._slotFilterEffect[index] = slot.icon:AddEffect("UI_Inventory_Filtering", true, 0, 0)
  end
  ...
  if slot._isLockedItem == true then
...
function PaGlobalFunc_WareHouse_All_GetSortControlPosition()
  if PaGlobal_WareHouse_All == nil then
    return
  end
  return PaGlobal_WareHouse_All._ui._chk_itemSort:GetParentPosX(), PaGlobal_WareHouse_All._ui._chk_itemSort:GetParentPosY()
end
function HandleEventLUp_WareHouse_All_ToggleCheckSortButton()
  if PaGlobal_WareHouse_All == nil then
    return
  end
  HandleLUp_AutoSortFunctionPanel_ToggleUseSort(InvenSortLinkPanelIndex.WareHouse, true)
end
function HandleEventLUp_Warehouse_All_ShowSortOption()
  if PaGlobal_WareHouse_All == nil then
    return
  end
  PaGlobalFunc_AutoSortFunctionPanel_ShowToggle(InvenSortLinkPanelIndex.WareHouse)
end
...
function FromClient_Warehouse_All_GuildUpdate(bForcedOpen, bForcedClose, whereType)
  ...
  if false == _ContentsGroup_GuildShareItemWareHouse then
    if CppEnums.ItemWhereType.eGuildWarehouse == whereType then
    ...
    elseif CppEnums.ItemWhereType.eGuildSafe == whereType then
      PaGlobal_WareHouse_All._ui._stc_title:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WAREHOUSE_ALL_TITLE_GUILD", "guildName", guildName))
      PaGlobal_WareHouse_All.guildWareHouseFilterFunc = PaGlobal_Warehouse_All_GuildItemWareHouseFilter
      PaGlobal_Warehouse_All_OpenWithInventory(PaGlobal_Warehouse_All_GuildItemInvenFilter, isWareHouseOpen)
      PaGlobal_WareHouse_All._fromType = CppEnums.WarehoouseFromType.eWarehoouseFromType_GuildSafe
    end
  elseif CppEnums.ItemWhereType.eGuildWarehouse == whereType then
    PaGlobal_WareHouse_All._ui._stc_title:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WAREHOUSE_ALL_TITLE_GUILD", "guildName", guildName))
    PaGlobal_WareHouse_All.guildWareHouseFilterFunc = PaGlobal_Warehouse_All_GuildItemWareHouseFilter
    PaGlobal_Warehouse_All_OpenWithInventory(PaGlobal_Warehouse_All_GuildItemInvenFilter, isWareHouseOpen)
    PaGlobal_WareHouse_All._fromType = CppEnums.WarehoouseFromType.eWarehoouseFromType_GuildHouse
  elseif CppEnums.ItemWhereType.eBorrowInventory == whereType then
  ...
  if false == PaGlobal_WareHouse_All._isConsole then
    for ii = 0, CppEnums.ServantType.Type_Count - 1 do
      if true == servant_checkDistance(ii) then
        if false == PaGlobal_WareHouse_All._isConsole then
          PaGlobal_WareHouse_All._ui._pc._chk_horseBag:SetCheck(not PaGlobal_WareHouse_All._isHideHorseBag)
        end
        PaGlobal_WareHouse_All._ui._pc._chk_horseBag:SetShow(true)
        break
      end
    end
    if false == PaGlobal_WareHouse_All._isHideHorseBag and nil == isScrollHold or false == isScrollHold then
      ServantInventory_OpenAll()
    elseif nil ~= ServantInventory_Close then
      ServantInventory_Close(true)
    end
  end
  if _ContentsOption_CH_GameType == true then
    PaGlobal_WareHouse_All._ui._pc._btn_question:addInputEvent("Mouse_LUp", "Panel_WebHelper_ShowToggle( \"PanelGuild\" )")
    PaGlobal_Util_RegistHelpTooltipEvent(PaGlobal_WareHouse_All._ui._pc._btn_question, "\"PanelGuild\"")
  end
end
...
function FromClient_Warehouse_All_FurnitureUpdate()
  ...
  if false == PaGlobal_WareHouse_All._isConsole then
    ...
    PaGlobal_WareHouse_All._ui._btn_wareHouseHistory:SetShow(false)
...
function PaGlobal_Warehouse_All_UpdateSearchMenuList(contents, key)
  ...
  local territoryIcon = UI.getChildControl(radioButton, "Static_Territory_Icon")
...
function PaGlobal_Warehouse_All_BasicItemFilter(slotNo, notUse_itemWrappers, whereType)
  ...
  if itemWrapper:getStaticStatus():isForJustTrade() == true and PaGlobal_WareHouse_All._fromType == CppEnums.WarehoouseFromType.eWarehoouseFromType_Maid then
    return true
  end
  return false
end
...
function PaGlobal_Warehouse_All_OtherTownWareHouseItemFilter(slotNo, notUse_itemWrappers, whereType)
  local itemWrapper = getInventoryItemByType(whereType, slotNo)
  if itemWrapper == nil then
    return true
  end
  ...
  if itemWrapper:getStaticStatus():isGuildStockable() == true then
    return true
  end
  if itemWrapper:getStaticStatus():isForJustTrade() == true then
    return true
  end
  if itemWrapper:getStaticStatus():isBarterItem() == true then
    return true
  end
  if itemWrapper:movableOtherTown() == false then
    return true
  end
  if CppEnums.ContentsEventType.ContentsType_InventoryBag == itemWrapper:getStaticStatus():getContentsEventType() then
    ...
    for index = 0, bagSize - 1 do
      ...
      if itemWrapper:getStaticStatus():get():isCash() then
        whereType = CppEnums.ItemWhereType.eCashInventory
      end
      bagItemWrapper = getInventoryBagItemByType(whereType, slotNo, index)
      if nil ~= bagItemWrapper and bagItemWrapper:movableOtherTown() == false then
        return true
      end
    end
  end
...
function PaGlobal_Warehouse_All_CanNotMovableOtherHouseItemFilter(slotNo, notUse_itemWrappers, whereType)
  return true
end
...
function PaGlobal_Warehouse_All_MoveNonStackItemCheck()
  ...
  return PaGlobal_WareHouse_All:isNpc() == true or PaGlobal_WareHouse_All:isMaid() == true
end
        ...
        if false == itemSSW:isMoney() then
          ...
          if true == PaGlobal_WareHouse_All._isUseMultiMaid then
            if false == _ContentsGroup_MoveNonStackItem or true == itemSSW:isStackable() then
              ...
              local nonStackItemCount = ToClient_getNonStackItemCount(CppEnums.ItemWhereType.eWarehouse, slotNo, false, PaGlobal_WareHouse_All._currentWaypointKey)
              local availableMaidCount = ToClient_getUsableMaidCountByType(false)
              if 0 == availableMaidCount then
                Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCantFindUseableMaid"))
                return
...
function PaGlobal_Warehouse_All_SetIgnoreMoneyButton(setIgnore)
  if setIgnore == true then
    if true == PaGlobal_WareHouse_All._buttonData.guildLog then
      PaGlobal_WareHouse_All._ui._console._txt_keyGuideMarketInvenOpen:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_GUILDWAREHOUSE_USELOG_BUTTON"))
      ...
      PaGlobal_WareHouse_All._ui._console._txt_keyGuideMarketInvenOpen:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_MARKETPLACE_MARKETINVEN"))
...
function HandleEventLUp_WareHouse_All_OpenGuildPresent()
  if PaGlobal_WareHouse_All:isGuildHouse() == false then
    return
  end
  if Panel_Window_GuildWareHouseHistory:GetShow() == true then
    PaGlobal_GuildWareHouseHistory:close()
  end
  PaGlobal_GuildPresent_All_Open()
...
function PaGlobal_Warehouse_All_PopItemFromItemSearch(itemCount_s64, slotNo, actorKey, waypointKey)
  if warehouse_popToInventoryByMaid ~= nil then
    warehouse_popToInventoryByMaid(slotNo, itemCount_s64, actorKey, false, waypointKey)
    if Panel_Window_Warehouse:GetShow() == true and PaGlobal_WareHouse_All._selectWaypointKey ~= waypointKey then
      HandleEventLUp_WareHouse_All_OtherTownsWareHouse(waypointKey)
      PaGlobal_WareHouse_All._selectWaypointKey = waypointKey
    end
  end
end
