...
function InputMouse_Palette_All_ColorToolTip(isShow, itemEnchantKey, rowsIdx, colsIdx)
  if true == isShow then
    ...
    local itemName = getItemName(itemEnchantSS)
    local desc
    local icon = PaGlobal_Palette_All.slot[rowsIdx][colsIdx].color
...
function PaGlobal_Palette_All_IsShow()
  if Panel_Window_Palette_All == nil then
    return nil
  end
  return Panel_Window_Palette_All:GetShow()
end
