...
function FGlobal_Panel_Dye_ReNew_AddEvent()
  ...
  if true == _ContentsOption_CH_GameType then
    RadioButton_Tab_ALL:SetShow(false)
    RadioButton_Tab_My:SetShow(false)
    RadioButton_Tab_Pearl:SetSize(345, 32)
    RadioButton_Tab_Pearl:SetSpanSize(0, 3)
  end
end
function HandleReset_DyeReNew_AmpuleTab_BySelectCharacterType()
  ...
  if true == _ContentsOption_CH_GameType then
    ...
    RadioButton_Tab_My:SetCheck(false)
    RadioButton_Tab_Pearl:SetCheck(true)
  else
    RadioButton_Tab_ALL:SetCheck(false)
...
function HandleClicked_LUp_SelectEquipItem(equipSlotNo, equipSlotIndex)
  ...
  elseif true == dyeCombine._ui.toggleAwakenWeapon:GetShow() and (29 == equipSlotNo or 30 == equipSlotNo) then
    isWeaponEquip = true
...
function HandleOnOut_DeyReNew_Ampule_Item(isShow, itemEnchantKey, UIIndex)
  local self = FGlobal_DyeReNew_GetInstance()
  local uiControl = self._arrEquipPartSlot[UIIndex]
  local enchantKey = ItemEnchantKey(itemEnchantKey)
  if false == enchantKey:isDefined() then
    TooltipSimple_Hide()
    return
  end
  local itemEnchantSSW = getItemEnchantStaticStatus(enchantKey)
...
function HandleClicked_LUp_Ampule_SelectedType(isShowAll, isPearl)
  ...
  if true == _ContentsGroup_RgbUIPickForItem then
    if true == isShowAll and true == isPearl then
      SetShow_ColorChoiceButton()
    else
      SetHide_ColorChoiceButton()
    end
  end
...
function SetShow_ColorChoiceButton()
  local Static_BG = UI.getChildControl(Panel_Dye_ReNew, "Static_BG")
  local UIAmpuleTargetBG = UI.getChildControl(Static_BG, "Static_AmpuleTartget_BG")
  local RadioButton_Tab_RGB = UI.getChildControl(UIAmpuleTargetBG, "Button_ColorChoice")
  RadioButton_Tab_RGB:SetShow(true)
  RadioButton_Tab_RGB:addInputEvent("Mouse_LUp", "PaGlobal_ColorPalette_All_Open(" .. __eRGBPaletteTypeItem .. ")")
end
function SetHide_ColorChoiceButton()
  local Static_BG = UI.getChildControl(Panel_Dye_ReNew, "Static_BG")
  local UIAmpuleTargetBG = UI.getChildControl(Static_BG, "Static_AmpuleTartget_BG")
  local RadioButton_Tab_RGB = UI.getChildControl(UIAmpuleTargetBG, "Button_ColorChoice")
  RadioButton_Tab_RGB:SetShow(false)
end
...
function PaGlobal_DeyReNew_Palette_SelectColor_Renew(paletteType, hslInfo)
  local self = FGlobal_DyeReNew_GetInstance()
  if true == self._checkButtonDyeAll:IsCheck() then
    local infoCount = ToClient_getDyeingTargetInformationCount()
    if 0 == infoCount then
