...
function DyeingPartList:initialize()
  ...
  self._ui._stc_checkButtonBG = UI.getChildControl(self._ui.stc_listBG, "Static_CheckButtonList")
  _LISTBG_ORIGINAL_SIZE = self._ui.stc_listBG:GetSizeY()
  ...
  for idx = 0, self._maxToggleCount - 1 do
    self._ui._chk_buttons[idx] = UI.getChildControl(self._ui._stc_checkButtonBG, "CheckButton_" .. idx)
    self._ui._chk_buttons[idx]:addInputEvent("Mouse_LUp", "PaGlobalFunc_DyeingMain_ToggleOption( " .. idx .. " )")
  end
end
...
function DyeingPartList:close()
  if nil ~= Panel_Window_DyeingPalette_Renew then
    PaGlobalFunc_DyeingPalette_CleanData()
  end
...
function DyeingPartList:update()
  ...
  for ii = 1, equipSlotCount do
    ...
    if nil ~= itemWrapper then
      ...
      if nil ~= targetData.pictogramUV then
        ...
        local x1, y1, x2, y2
        if classType == __eClassType_ShyWaman and 2 == ii then
          pictogram:ChangeTextureInfoName("New_UI_Common_forLua/Window/Dye/Dye_New_00.dds")
          x1, y1, x2, y2 = setTextureUV_Func(pictogram, 422, 35, 450, 63)
        else
          x1, y1, x2, y2 = setTextureUV_Func(pictogram, targetData.pictogramUV[slotNo][1], targetData.pictogramUV[slotNo][2], targetData.pictogramUV[slotNo][3], targetData.pictogramUV[slotNo][4])
        end
  ...
  for ii = 1, #targetData.costumeSlotNoList do
    ...
    if nil ~= itemWrapper then
      ...
      if nil ~= targetData.pictogramUV then
        ...
        if classType == __eClassType_ShyWaman and 2 == ii then
          pictogram:ChangeTextureInfoName("New_UI_Common_forLua/Window/Dye/Dye_New_00.dds")
          x1, y1, x2, y2 = setTextureUV_Func(pictogram, 422, 6, 450, 34)
        else
          x1, y1, x2, y2 = setTextureUV_Func(pictogram, targetData.pictogramUV[slotNo][1], targetData.pictogramUV[slotNo][2], targetData.pictogramUV[slotNo][3], targetData.pictogramUV[slotNo][4])
        end
        pictogram:getBaseTexture():setUV(x1, y1, x2, y2)
  ...
  if self._currentTargetIndex == TARGET_INDEX.CHARACTER then
    self._ui.stc_listBG:SetSize(self._ui.stc_listBG:GetSizeX(), _LISTBG_ORIGINAL_SIZE)
    self._ui.txt_keyGuideA:SetPosY(lastSlot:GetParentPosY() + 134)
    self._ui._stc_checkButtonBG:SetShow(true)
    for idx = 0, self._maxToggleCount - 1 do
      self._ui._chk_buttons[idx]:SetCheck(false)
    end
  else
    self._ui.stc_listBG:SetSize(self._ui.stc_listBG:GetSizeX(), _LISTBG_ORIGINAL_SIZE - 134)
    self._ui.txt_keyGuideA:SetPosY(lastSlot:GetParentPosY() + 70)
    self._ui._stc_checkButtonBG:SetShow(false)
  end
end
function Input_DyeingPartList_SelectEquip(slotNo)
  ...
  local isHideHelmet = DyeingPartList._ui._chk_buttons[DyeingPartList._ENUM_TOGGLE_TYPE.Helmet]:IsCheck()
  if (6 == slotNo or 17 == slotNo) and true == isHideHelmet then
    DyeingPartList._ui._chk_buttons[DyeingPartList._ENUM_TOGGLE_TYPE.Helmet]:SetCheck(false)
    PaGlobalFunc_DyeingMain_ToggleOption(DyeingPartList._ENUM_TOGGLE_TYPE.Helmet)
  end
  ToClient_padSnapSetTargetPanel(Panel_Window_DyeingPalette_Renew)
end
function PaGlobalFunc_DyeingMain_ToggleOption(clickedType)
  if DyeingPartList._ui._chk_buttons[clickedType] == nil then
    return
  end
  local checked = DyeingPartList._ui._chk_buttons[clickedType]:IsCheck()
  if clickedType == DyeingPartList._ENUM_TOGGLE_TYPE.Underwear then
    ToClient_RequestToggleShowUnderwear()
    if DyeingPartList._ui._chk_buttons[DyeingPartList._ENUM_TOGGLE_TYPE.WarStance]:IsCheck() == true then
      ToClient_RequestSetBattleView(true)
    end
  elseif clickedType == DyeingPartList._ENUM_TOGGLE_TYPE.Avatar then
    DyeingPartList._ui._chk_buttons[DyeingPartList._ENUM_TOGGLE_TYPE.Underwear]:SetCheck(false)
    ToClient_RequestToggleHideAvatar()
  elseif clickedType == DyeingPartList._ENUM_TOGGLE_TYPE.Helmet then
    ToClient_RequestHideHelmet(not checked)
  elseif clickedType == DyeingPartList._ENUM_TOGGLE_TYPE.FaceViewHair then
    ToClient_setFaceViewHair(checked)
  elseif clickedType == DyeingPartList._ENUM_TOGGLE_TYPE.FaceGuard then
    ToClient_RequestHideBattleHelmet(checked)
  elseif clickedType == DyeingPartList._ENUM_TOGGLE_TYPE.Cloak then
    ToClient_RequestHideCloak(not checked)
  elseif clickedType == DyeingPartList._ENUM_TOGGLE_TYPE.WarStance then
    ToClient_RequestSetBattleView(checked)
  elseif clickedType == DyeingPartList._ENUM_TOGGLE_TYPE.AwakenWeapon then
    ToClient_SetAwakenWeaponShow(checked)
  end
end
