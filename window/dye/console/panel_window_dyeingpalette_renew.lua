...
local shaiWeaponActionIndex = {
  Tring = __eDyeingAwakenBattleActionIndex_Lute,
  Tutu = __eDyeingAwakenBattleActionIndex_Flute,
  Tocong = __eDyeingAwakenBattleActionIndex_Djembe
}
local DyeingPalette = {
  _ui = {
...
function DyeingPalette:initialize()
  ...
  self._ui.stc_selectAllPart = UI.getChildControl(self._ui.stc_partSelectBG, "StaticText_SelectAllPart")
  self._ui.chk_selectAllPart = UI.getChildControl(self._ui.stc_partSelectBG, "CheckBox_SelectAllPart")
  self._ui.stc_shaiAwakenWeaponBG = UI.getChildControl(self._ui.stc_partSelectBG, "Static_ShyWakenWeaponDyeArea")
  for key, value in pairs(shaiWeaponActionIndex) do
    self._ui.rdo_weapon[value] = UI.getChildControl(self._ui.stc_shaiAwakenWeaponBG, "RadioButton_Weapon_" .. tostring(value))
    self._ui.rdo_weapon[value]:addInputEvent("Mouse_LUp", "InputMLUp_DyeingPalette_SelectShaiWeaponActionIndex(" .. tostring(value) .. ")")
  end
  self._ui.stc_colorPartBG = UI.getChildControl(self._ui.stc_partSelectBG, "Static_ColorPartBG")
  for ii = 1, 18 do
    self._ui.rdo_color[ii] = UI.getChildControl(self._ui.stc_colorPartBG, "Radiobutton_ColorBg_" .. ii)
  ...
  self:SetRightKeyGuideAlign()
  self:SetLeftKeyGuideAlign()
  self:initAmpuleList()
  self:registEventHandler()
end
function DyeingPalette:SetRightKeyGuideAlign()
  local keyGuideList = {
...
function DyeingPalette:SetLeftKeyGuideAlign()
...
function DyeingPalette:registEventHandler()
  ...
  _panel:registerPadEvent(__eConsoleUIPadEvent_Up_LSClick, "Input_DyeingPalette_SelectPartAll()")
end
...
function DyeingPalette:open(targetType, slotNo)
  ...
  self._ui.stc_colorPartBG:ComputePos()
  if 29 == slotNo and __eClassType_ShyWaman == self._classType then
    InputMLUp_DyeingPalette_SelectShaiWeaponActionIndex(__eDyeingAwakenBattleActionIndex_Base)
    self._ui.stc_shaiAwakenWeaponBG:SetShow(true)
    self._ui.stc_colorPartBG:SetPosY(self._ui.stc_colorPartBG:GetPosY() + 40)
  else
    self._ui.stc_shaiAwakenWeaponBG:SetShow(false)
  end
...
function Input_DyeingPalette_DyePart(partID, slotID, uiIdx)
  ...
  self._ui.chk_selectAllPart:SetCheck(false)
  self:updatePalette()
end
function Input_DyeingPalette_SelectPartAll()
  for ii = 1, #DyeingPalette._ui.rdo_color do
    DyeingPalette._ui.rdo_color[ii]:SetCheck(false)
  end
  DyeingPalette._currentDyePartIndex = -1
  DyeingPalette._ui.chk_selectAllPart:SetCheck(not DyeingPalette._ui.chk_selectAllPart:IsCheck())
end
...
function Input_DyeingPalette_Ampule(index)
  local self = DyeingPalette
  local targetSlotNo = self._slotNo
  local targetPartID = self._nowClickPartId
  local targetPartSlotId = self._nowClickPartSlotId
  local targetDyeCount = 0
  local bSelectAll = self._ui.chk_selectAllPart:IsCheck()
  if nil ~= self._currentDyePartIndex and 0 < self._currentDyePartIndex then
    targetDyeCount = 1
  end
  if true == bSelectAll then
    targetDyeCount = ToClient_getDyeingTargetInformationCount()
  end
  if 0 == targetDyeCount then
    return
  end
  for ii = 1, targetDyeCount do
    self._nowPaletteAmpuleIndex = index
    if true == bSelectAll then
      if nil == self._partDyeInfo[ii] then
        _PA_ASSERT_NAME(false, "\236\151\188\236\131\137 \235\182\128\236\156\132 \236\160\149\235\179\180\234\176\128 \236\151\134\236\138\181\235\139\136\235\139\164", "\236\161\176\236\158\172\236\167\132")
        return
      end
      targetPartID = self._partDyeInfo[ii][1]
      targetPartSlotId = self._partDyeInfo[ii][2]
    end
    if -1 == targetSlotNo or -1 == targetPartID then
      return
    end
    _AudioPostEvent_SystemUiForXBOX(50, 1)
    local targetPaletteCategoryIndexForClient = self._nowPaletteCategoryIndex - 1
    local targetPaletteAmpuleIndexForClient = self._nowPaletteAmpuleIndex - 1
    local DyeingPaletteCategoryInfo = ToClient_requestGetPaletteCategoryInfo(targetPaletteCategoryIndexForClient, self._paletteShowAll)
    if nil ~= DyeingPaletteCategoryInfo then
      local ampuleCount = DyeingPaletteCategoryInfo:getCount(targetPaletteAmpuleIndexForClient, true)
      local convertCount = Int64toInt32(ampuleCount)
      ...
      ToClient_RequestSelectedDyeingPalette(targetSlotNo, targetPartID, targetPartSlotId, targetPaletteCategoryIndexForClient, targetPaletteAmpuleIndexForClient, self._paletteShowAll)
      if 0 < ToClient_RequestGetPartDyeingSlotCount(targetSlotNo, 0) then
        table.insert(self._selectedDyePart, targetSlotNo)
      end
    end
  end
  ToClient_RequestUpdatCharacterView(targetSlotNo)
  self:updateDyeParts()
  self:updatePalette()
end
...
function InputScroll_DyeingPalette_Scroll(isUp)
  ...
  if oldAmount ~= self._currentScrollAmount and (_ContentsGroup_UsePadSnapping or ToClient_IsDevelopment()) then
...
function InputMLUp_DyeingPalette_SelectShaiWeaponActionIndex(actionIndex)
  local self = DyeingPalette
  for key, value in pairs(shaiWeaponActionIndex) do
    self._ui.rdo_weapon[value]:SetCheck(value == actionIndex)
  end
