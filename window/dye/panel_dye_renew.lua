...
function FGlobal_DyeReNew_GetEnableKR2()
  return _ContentsOption_CH_GameType == true
end
...
function DyeReNew:Open()
  ...
  if true == _ContentsOption_CH_GameType then
    HandleClicked_LUp_Ampule_SelectedType(true, true)
  else
...
function DyeReNew:Reset_Position()
  ...
  for ii = 0, enControlValue.MaxEquipSlotCount - 1 do
    ...
    if true == _ContentsOption_CH_NoUnderWear and 14 == ii then
...
function DyeReNew:Change_EquipIcon()
  ...
  for idx, equipSlotIdx in pairs(targetSlot) do
    ...
    if enCharacterType.Character == characterType and isClassType == __eClassType_ShyWaman then
      if 3 == idx then
...
function FromClient_updateDyeingTargetList()
  ...
  for idx, equipSlotWarraper in pairs(EquipSlotIcon[self._selected_CharacterTarget]) do
    ...
    if true == isDyeAble and nil ~= itemWrapper and true == self:GetUIEquipBGByIndex(idx - 1):GetShow() then
...
function DyeReNew:checkShowAwakenItemSlot()
  if true == self._enableAwaken then
    return
  end
  local isCharacterItem = enCharacterType.Character == self._selected_CharacterTarget
  local awakenWeaponIndex = {3, 10}
  local startSlotPosX = self:GetUIEquipBGByIndex(0):GetPosX()
  local slotIntervalSizeX = 50
  local rowCnt = 0
  for idx, equipSlotWarraper in pairs(EquipSlotIcon[self._selected_CharacterTarget]) do
    local targetSlot = self:GetUIEquipBGByIndex(idx - 1)
    for index = 1, #awakenWeaponIndex do
      if awakenWeaponIndex[index] == idx then
        if true == isCharacterItem then
          targetSlot:SetShow(self._enableAwaken)
        else
          targetSlot:SetShow(true)
        end
      end
    end
    if true == targetSlot:GetShow() then
      targetSlot:SetPosX(startSlotPosX + slotIntervalSizeX * rowCnt)
      rowCnt = rowCnt + 1
    end
    if rowCnt > enControlValue.MaxEquipSlotRowCount - 1 or 0 == idx % enControlValue.MaxEquipSlotRowCount then
      rowCnt = 0
    end
  end
end
...
function DyeReNew:Update_Part()
  ...
  for infoIdx = 0, infoCount - 1 do
    ...
    local itemKey = ToClient_getDyeItemKeyByDyedItem(equipSlotNo, partIdx, slotIdx)
    PartSlot:addInputEvent("Mouse_On", "HandleOnOut_DeyReNew_Ampule_Item( true, " .. itemKey .. "," .. DyeSlotIdx .. ")")
    PartSlot:addInputEvent("Mouse_Out", "HandleOnOut_DeyReNew_Ampule_Item( false, " .. itemKey .. "," .. DyeSlotIdx .. ")")
...
function FGlobal_DyeReNew_IsShowable()
  ...
  if true == ToClient_CheckEquipMajorSiegeGuildCloth() then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_SEIGECLOTHES_WARNING"))
    return false
  end
...
function FGlobal_Panel_DyeReNew_Show()
  ...
  if true == _ContentsGroup_NewUI_Inventory_All then
    isInventoryOpen = Panel_Window_Inventory_All:IsShow()
  else
...
function Dye_ReNew_IsDyeableEquipment(equipSlotNo)
  ...
  elseif Dye_ReNew_IsNormal(equipSlotNo) then
    return true
  elseif __eEquipSlotNoAvatarUnderwear == equipSlotNo and enCharacterType.Character == self._selected_CharacterTarget then
    return true
  elseif (__eEquipSlotNoFaceDecoration1 == equipSlotNo or __eEquipSlotNoFaceDecoration2 == equipSlotNo or __eEquipSlotNoFaceDecoration3 == equipSlotNo) and enCharacterType.Character == self._selected_CharacterTarget then
    return true
  end
  return false
end
function Dye_ReNew_IsAvatar(equipSlotNo)
  ...
  elseif enCharacterType.Horse == self._selected_CharacterTarget and (__eEquipSlotNoAvatarChest == equipSlotNo or __eEquipSlotNoAvatarGlove == equipSlotNo or __eEquipSlotNoAvatarBoots == equipSlotNo or __eEquipSlotNoAvatarHelm == equipSlotNo) then
    return true
  end
  return false
end
...
function PaGlobal_Dye_ReNew_OtherPanels_Close()
  ...
  elseif nil ~= Panel_DyePalette and Panel_DyePalette:GetShow() and nil ~= FGlobal_DyePalette_Close then
