...
function HandleEventLUp_Socket_All_SelectItem(slotIdx, key, isEquip)
  ...
  if true == socketInfo._setEquipItem then
    ...
    PaGlobalFunc_Socket_All_SetTab(PaGlobal_Socket_All._TAB.CRYSCTAL)
  else
...
function PaGlobalFunc_Socket_All_UpdateList(control, key)
  ...
  if true == isDuplicatedItem and nil ~= slot.duplicatedForDuelIcon then
  ...
  if true == isOriginalForDuel and nil ~= slot.originalForDuelIcon then
