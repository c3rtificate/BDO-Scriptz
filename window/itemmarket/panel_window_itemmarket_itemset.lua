...
function ItemMarketItemSet:Initialize()
  ...
  self._list2:registEvent(__ePAUIList2EventType_LuaChangeContent, "ItemmarketItemSet_ListUpdate")
...
function ItemmarketItemSet_ListUpdate(contents, key)
  ...
  if nil ~= myItemInfo then
    ...
    if nil ~= iess then
      ...
      elseif enchantLevel > 0 and CppEnums.ItemClassifyType.eItemClassify_Accessory == iess:getItemClassify() and (iess:isSpecialEnchantItem() == false or iess:isKingAccessory() == true) then
...
function ItemMarketItemSet_UpdateMoneyByWarehouse()
  if false == Panel_Window_ItemMarket_ItemSet:GetShow() then
    return
  end
  ItemMarketItemSet.invenMoney:SetText(makeDotMoney(getSelfPlayer():get():getMoneyInventory():getMoney_s64()))
