...
PaGlobal_Util_RegistHelpTooltipEvent(_buttonQuestion, "\"ItemMarket\"")
  ...
  if 0 < self.config.totalItemCount then
    for slotIdx = self.config.startIndex, self.config.totalItemCount - 1 do
      ...
      if 0 < ToClient_getSpecialEnchantDisplayLevel(itemSSW:get()._key) then
        enchantLevel = ToClient_getSpecialEnchantDisplayLevel(iitemSSW:get()._key)
      end
      ...
      if CppEnums.ItemClassifyType.eItemClassify_Accessory == itemSSW:getItemClassify() and (itemSSW:isSpecialEnchantItem() == false or itemSSW:isKingAccessory() == true) then
        ...
        elseif 6 == enchantLevel then
          slot.enchantLevel:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_6"))
          slot.enchantLevel:SetShow(true)
        elseif 7 == enchantLevel then
          slot.enchantLevel:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_7"))
          slot.enchantLevel:SetShow(true)
        elseif 8 == enchantLevel then
          slot.enchantLevel:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_8"))
          slot.enchantLevel:SetShow(true)
        elseif 9 == enchantLevel then
          slot.enchantLevel:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_9"))
          slot.enchantLevel:SetShow(true)
        elseif 10 == enchantLevel then
          slot.enchantLevel:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_10"))
          slot.enchantLevel:SetShow(true)
        end
      end
