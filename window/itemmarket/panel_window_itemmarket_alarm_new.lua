...
local PaGlobal_ItemMarket_Alarm = {
  ...
  _eBiddingEvnetType = {BuyBidding = 1, SellBidding = 6},
  _currentCount = 0,
  _unreadCount = 0,
  _maxCount = 20,
  _lastIndex = 0,
...
function PaGlobal_ItemMarket_Alarm:Init()
  ...
  local popupOpen = _ContentsGroup_PopUp
  ...
  slot:createChild()
  self._ui._alramList:registEvent(__ePAUIList2EventType_LuaChangeContent, "ItemMarket_AlarmList_ControlCreate")
  self._ui._alramList:createChildContent(__ePAUIList2ElementManagerType_List)
...
function ItemMarket_AlarmList_ControlCreate(content, key)
  ...
  local info = ToClient_GetAlarmInfoByIndex(index)
  local slot = {}
  ...
  local alertType = 23
  slot.icon:addInputEvent("Mouse_LUp", "PaGlobalFunc_DirectWallet_HandleLClick(" .. alertType .. ")")
  desc:SetIgnore(false)
  desc:addInputEvent("Mouse_LUp", "PaGlobalFunc_DirectWallet_HandleLClick(" .. alertType .. ")")
  local itemName = FGlobal_ChangeItemNameColorByGrade(info:getItemEnchantKey())
  local modifyCount = info:getModifyCount()
  local infoType = info:getType()
  local itemString = tostring(modifyCount)
  alarmType:SetTextMode(__eTextMode_AutoWrap)
  local modifyString = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_MARKETPLACE_WALLET_VALUE_COUNT", "count", tostring(modifyCount))
  local descString = ""
  if self._eBiddingEvnetType.SellBidding == infoType then
    alarmType:SetText("[" .. PAGetString(Defines.StringSheet_GAME, "LUA_ITEMMARKET_ITEMSET_SELLCOMPLETE") .. "]")
    descString = PAGetString(Defines.StringSheet_GAME, "LUA_MARKETPLACE_SELL_TITLE")
  elseif self._eBiddingEvnetType.BuyBidding == infoType then
    alarmType:SetText("[" .. PAGetString(Defines.StringSheet_GAME, "LUA_MARKETPLACE_WALLET_BUY_COMPLATE") .. "]")
    descString = PAGetString(Defines.StringSheet_GAME, "LUA_NPCSHOP_BUY_ALERT_TITLE")
  end
  desc:SetText(" " .. itemName .. " " .. modifyString .. " " .. descString)
  recentBg:SetShow(index >= PaGlobal_ItemMarket_Alarm._lastIndex)
end
function PaGlobalFunc_DirectWallet_HandleLClick(alertType)
...
function PaGlobal_ItemMarket_Alarm:Reload()
  ...
  local listSize = ToClient_GetAlarmInfoListSize()
  for index = listSize - 1, 0, -1 do
    local hisotoryBidding = ToClient_GetAlarmInfoByIndex(index)
    if nil ~= hisotoryBidding then
...
function FGlobal_ItemMarket_AlarmIcon_Reload()
  PaGlobal_ItemMarket_Alarm._unreadCount = 0
  FGlobal_ItemMarket_SetCount()
  PaGlobal_ItemMarket_Alarm:Reload()
end
function ItemMarket_AlarmList_Close()
  ...
  Panel_Tooltip_Item_hideTooltip()
  PaGlobal_ItemMarket_Alarm._lastIndex = ToClient_GetAlarmInfoListSize()
end
...
function FGlobal_ItemMarketAlarm_LuaLoadComplete()
  ...
  PaGlobal_ItemMarket_Alarm._unreadCount = 0
  PaGlobal_ItemMarket_Alarm._lastIndex = -1
