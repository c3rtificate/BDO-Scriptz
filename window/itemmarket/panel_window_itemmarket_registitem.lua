...
function itemMarket_afterRegist()
  ...
  self.itemName:SetTextMode(__eTextMode_AutoWrap)
...
function FGlobal_ItemMarketRegistItem_Open(isOpenWarehouse, isByMaid)
  ...
  self.guideText:SetTextMode(__eTextMode_AutoWrap)
  ...
  local invenSizeX = false
  if true == _ContentsGroup_NewUI_Inventory_All then
    invenSizeX = Panel_Window_Inventory_All:GetSizeX()
  else
    invenSizeX = Panel_Window_Inventory:GetSizeX()
  end
  if getScreenSizeX() / 2 < invenSizeX + Panel_Window_ItemMarket_RegistItem:GetSizeX() / 2 then
    Panel_Window_ItemMarket_RegistItem:SetPosX(getScreenSizeX() - invenSizeX - Panel_Window_ItemMarket_RegistItem:GetSizeX())
  end
  local registedItemCount = getItemMarketMyItemsCount()
  self.sellItemTitle:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_ITEMMARKET_REGISTITEM_ENABLE_REGISTCOUNT", "itemCount", 30 - registedItemCount))
end
...
function FGlobal_ItemMarketRegistItem_Close(isItemMarketItemSet_Show)
  ...
  if not Panel_Window_Warehouse:GetShow() then
    ...
    PaGlobal_Warehouse_All_ResetFilter()
    Inventory_SetFunctor(nil, FGlobal_PopupMoveItem_InitByInventory, PaGlobal_Warehouse_All_Close, nil)
...
function FGlobal_ItemMarketRegistItemFromInventory(s64_count, slotNo, inventoryType, waypointKey)
  ...
  elseif enchantLevel > 0 and CppEnums.ItemClassifyType.eItemClassify_Accessory == itemSS:getItemClassify() and (itemSS:isSpecialEnchantItem() == false or itemSSW:isKingAccessory() == true) then
