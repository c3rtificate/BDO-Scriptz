...
function PaGlobal_SeasonBanner:prepareOpen()
  ...
  if true == self._isConsole then
    Panel_Window_SeasonBanner:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "HandleEventLUp_SeasonBanner_ChangePage(true)")
    self._ui.stc_keyGuideY:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_DIALOGLIST_ALL_NEXT"))
    self:alignKeyGuide()
  end
...
function PaGlobal_SeasonBanner:changePage(nextPage)
  ...
  if true == self._isConsole then
    if self._endPageNo == currentPageNo then
      Panel_Window_SeasonBanner:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "HandleEventLUp_SeasonBanner_SetSeasonCharacter()")
      self._ui.stc_keyGuideY:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_WINDOW_SEASONBANNER_CREATEBTN"))
    else
      Panel_Window_SeasonBanner:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "HandleEventLUp_SeasonBanner_ChangePage(true)")
      self._ui.stc_keyGuideY:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_DIALOGLIST_ALL_NEXT"))
    end
