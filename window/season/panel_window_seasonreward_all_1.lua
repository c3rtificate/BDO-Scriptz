function PaGlobal_SeasonReward_All:initialize()
  ...
  self._ui.stc_keyguideBG = UI.getChildControl(Panel_Window_SeasonReward_All, "Static_KeyGuide")
  self._ui.stc_keyguideY = UI.getChildControl(self._ui.stc_keyguideBG, "Static_GuideY_C")
  self._ui.stc_keyguideX = UI.getChildControl(self._ui.stc_keyguideBG, "Static_GuideX_C")
  ...
  if true == self._isConsole then
    Panel_Window_SeasonReward_All:SetSize(Panel_Window_SeasonReward_All:GetSizeX(), Panel_Window_SeasonReward_All:GetSizeY() - self._ui.btn_exchange:GetSizeY() + 10)
    self._ui.list2_season:SetSize(self._ui.list2_season:GetSizeX(), self._ui.list2_season:GetSizeY() - self._ui.btn_exchange:GetSizeY() + 10)
    self._ui.stc_centerBG:SetSize(self._ui.stc_centerBG:GetSizeX(), self._ui.stc_centerBG:GetSizeY() - self._ui.btn_exchange:GetSizeY() + 10)
    Panel_Window_SeasonReward_All:ComputePosAllChild()
    self._ui.stc_bottomBG:SetSpanSize(self._ui.stc_bottomBG:GetSpanSize().x, self._ui.stc_bottomBG:GetSpanSize().y - self._ui.btn_exchange:GetSizeY())
    PaGlobal_SeasonReward_All:keyAlign()
  end
  local rewardSlot = {}
...
function PaGlobal_SeasonReward_All:registEventHandler()
  ...
  if true == self._isConsole then
    ...
    Panel_Window_SeasonReward_All:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "PaGlobal_SeasonReward_All_ReceiveReward()")
    self._ui.stc_itemSlot:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandleEventOnOut_SeasonReward_All_ShowRewardItemTooltip_Console()")
    self._ui.stc_itemSlot:addInputEvent("Mouse_Out", "HandleEventOnOut_SeasonReward_All_ShowRewardItemTooltip_Console(false)")
  else
...
function PaGlobal_SeasonReward_All:keyAlign()
...
function PaGlobal_SeasonReward_All:listUpdate()
  ...
  if ToClient_isRewardRepeatableSeasonExist() == true then
    for index = 1, seasonCount - 1 do
      self._ui.list2_season:getElementManager():pushKey(toInt64(0, index))
    end
    self._ui.list2_season:getElementManager():pushKey(toInt64(0, self._seasonNoRewardRepeatable))
  else
...
function PaGlobal_SeasonReward_All:contentUpdate()
  ...
  if true == isReceivedReward and self._selectedSeasonNo ~= self._seasonNoRewardRepeatable then
    isReceivable = false
  else
    isReceivable = true
  end
  ...
  self._receivedItemIndex = nil
  if seasonRewardCount >= 2 then
  ...
  local rewardItemEnchantKey
  if self._receivedItemIndex ~= nil then
    rewardItemEnchantKey = ToClient_getRewardByIndex(self._selectedSeasonNo, self._receivedItemIndex - 1)
  else
    rewardItemEnchantKey = ToClient_getRewardByIndex(self._selectedSeasonNo, self._selectedItemIndex - 1)
  end
  ...
  if enchantLevel > 0 then
    ...
    elseif 1 == itemSSW:getItemType() and enchantLevel > 15 then
...
function PaGlobal_SeasonReward_All:rewardListUpdate(rewardCount, isReceivable)
  for index = 1, self._maxRewardCount do
    ...
    if rewardCount < index then
      ...
      if ToClient_getSelectedRewardSeasonNo(rewardItemEnchantKey) ~= 0 then
        self._receivedItemIndex = index
      end
      if false == self._isConsole then
        ...
        slot.bg:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandleEventOnOut_SeasonReward_All_ShowListItemTooltip(true, " .. index .. "," .. rewardItemEnchantKey:getItemKey() .. "," .. rewardItemEnchantKey:getEnchantLevel() .. ")")
        slot.bg:addInputEvent("Mouse_Out", "HandleEventOnOut_SeasonReward_All_ShowListItemTooltip(false)")
      end
...
function PaGlobal_SeasonReward_All:prepareOpen()
  ...
  if true == self._isConsole then
    Panel_Window_SeasonReward_All:ignorePadSnapMoveToOtherPanelUpdate(true)
  end
...
function PaGlobal_SeasonReward_All:prepareClose()
  ...
  if true == self._isConsole then
    Panel_Window_SeasonReward_All:ignorePadSnapMoveToOtherPanelUpdate(false)
  end
