...
function FromClient_SetGameUIMode()
  ...
  if ToClient_IsEnterMirrorField(__eMirrorFieldContentsKey_BlackTemple) == true then
...
function changePositionBySever(panel, panelId, isShow, isChangePosition, isChangeSize)
  if panel == nil or panelId == nil or isShow == nil or isChangePosition == nil or isChangeSize == nil then
    return
  end
...
function PaGlobalFunc_CalcPositionByRelativePos(calcPos)
  if math.ceil(calcPos) - calcPos < 1.0E-4 then
    return math.ceil(calcPos)
  end
  return calcPos
end
