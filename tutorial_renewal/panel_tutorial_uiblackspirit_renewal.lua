...
function PaGlobal_TutorialUiBlackSpirit:setSpiritPos(x, y)
  self._ui._static_BottomBg:SetPosX(x)
  self._ui._static_BottomBg:SetPosY(y)
end
function PaGlobal_TutorialUiBlackSpirit:resetSpiritPos(x, y)
  self._ui._static_BottomBg:ComputePos()
end
function PaGlobal_TutorialUiBlackSpirit_setSpiritPos(x, y)
  PaGlobal_TutorialUiBlackSpirit:setSpiritPos(x, y)
end
function PaGlobal_TutorialUiBlackSpirit_resetSpiritPos()
  PaGlobal_TutorialUiBlackSpirit:resetSpiritPos()
end
function FromClient_CutsceneKeyGuideShow(posX, posY, keyType, isShow)
  if true ~= PaGlobal_TutorialUiBlackSpirit.isInit then
    PaGlobal_TutorialUiBlackSpirit:initialize()
  end
  Panel_Tutorial_Renew:SetShow(isShow, isShow)
  PaGlobal_TutorialUiBlackSpirit:setShowAll(false)
  PaGlobal_TutorialUiBlackSpirit._ui._static_BottomBg:SetShow(true)
  PaGlobal_TutorialUiBlackSpirit._ui._static_KeyGuideBg:SetShow(true)
  local stepBg = UI.getChildControl(Panel_Tutorial_Renew, "Static_StepBg")
  stepBg:SetShow(false)
  for ii = 0, #PaGlobal_TutorialUiBlackSpirit._keyGuide do
    if nil ~= PaGlobal_TutorialUiBlackSpirit._keyGuide[ii] then
      PaGlobal_TutorialUiBlackSpirit._keyGuide[ii]:SetShow(true)
    end
  end
  PaGlobal_TutorialUiBlackSpirit:setSpiritKeyGuideShow(keyType)
end
