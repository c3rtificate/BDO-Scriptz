...
function PaGlobal_TutorialUiMasking:showMasking(posX, posY)
  if not _ContentsGroup_UsePadSnapping then
    self._ui._maskBg_Quest:SetShow(true)
    local _posX = 0
    local _posY = 0
    if true == _ContentsGroup_NewUI_Inventory_All then
      _posX = Panel_Window_Inventory_All:GetPosX() + posX - 130
      _posY = Panel_Window_Inventory_All:GetPosY() + posY - 115
    else
      _posX = Panel_Window_Inventory:GetPosX() + posX - 130
      _posY = Panel_Window_Inventory:GetPosY() + posY - 115
    end
...
function PaGlobal_TutorialUiMasking:showSpiritMasking()
  ...
  local _posX = getScreenSizeX() - 100
  local _posY = getScreenSizeY() - 70
