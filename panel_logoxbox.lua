...
function Panel_LogoXBox_Init()
  ...
  if CppEnums.ServiceResourceType.eServiceResourceType_KR == resType or CppEnums.ServiceResourceType.eServiceResourceType_JP == resType then
    local pressStr = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_LOGOXBOX_PRESSBUTTON")
    txt_keyGuideA:SetTextSpan(50, 7)
    txt_keyGuideA:SetText(pressStr)
    local totalSizeX = txt_keyGuideA:GetSizeX() + txt_keyGuideA:GetTextSizeX()
    local posX = (button_Xbox:GetSizeX() - totalSizeX) / 2
    txt_keyGuideA:SetPosX(posX)
  end
...
function Panel_LogoXbox_InitWeb()
  ...
  if nil == web_movie then
    web_movie = UI.createControl(__ePAUIControl_WebControl, stc_webBG, "WebControl_BGMovie")
  end
...
function FromClient_ShowRestartMessage(strMessage)
  local requestRestart = function()
    ToClient_RestartApplivation()
  end
  local titleText = PAGetString(Defines.StringSheet_GAME, "LUA_MESSAGEBOX_NOTIFY")
  local messageboxData = {
    title = titleText,
    content = message,
    functionApply = requestRestart,
...
registerEvent("FromClient_ShowRestartMessage", "FromClient_ShowRestartMessage")
