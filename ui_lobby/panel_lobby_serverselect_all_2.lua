function HandleEventLUp_ServerSelect_All_EnterMainServer()
  ...
  if 0 ~= restrictedServerNo and true == ToClient_IsMoveableChannelRemainTime(worldServerIdx, mainServerNo) then
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_SERVERSELECT_MAINSERVERTITLE"),
...
function HandleEventLUp_ServerSelect_All_EnterButton(key32)
  ...
  if restrictedServerNo ~= 0 then
    local moveAbleAccessTime = 0
    if channelServerData._isSeasonChannel then
      moveAbleAccessTime = getSpecialChannelMoveableTime(worldServerData._worldNo)
    else
      moveAbleAccessTime = getChannelMoveableTime(worldServerData._worldNo)
    end
    if channelServerData._serverNo ~= restrictedServerNo and worldServerData._restrictReasonType == __eRestrictServerReasonType_PCWorking then
      local canAccessServerName = getChannelName(worldServerData._worldNo, restrictedServerNo)
      if nil ~= canAccessServerName then
        local messageBoxData = {
          title = PAGetString(Defines.StringSheet_GAME, "LUA_ALERT_DEFAULT_TITLE"),
          content = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_RESTRICT_SERVER_FOR_READBOOK", "serverName", canAccessServerName),
          functionApply = MessageBox_Empty_function,
          priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
        }
        MessageBox.showMessageBox(messageBoxData)
  ...
  if true == channelServerData._isSpeedChannel then
    ...
    if nil ~= tempWrapper and 0 == tempWrapper:getMyAdmissionToSpeedServer() and false == ToClient_IsDevelopment() then
      local resetSelectChannel = function()
        HandleEventLUp_ServerSelect_All_SelectChannel(nil)
      end
      local messageBoxData = {
        title = PAGetString(Defines.StringSheet_GAME, "LUA_ALERT_DEFAULT_TITLE"),
        content = PAGetString(Defines.StringSheet_GAME, "LUA_SERVERSELECT_NEWOLVIA_ALERT"),
        functionApply = resetSelectChannel,
        priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
      }
      MessageBox.showMessageBox(messageBoxData)
      return
...
function HandleEventLUp_ServerSelect_All_SelectChannel(key32)
  ...
  if nil == key32 then
    ...
    if true == _ContentsGroup_SeasonContents then
      ...
      PaGlobal_ServerSelect_All._ui._stc_channelDesc[PaGlobal_ServerSelect_All._CHANNEL_TYPE.SEASON_NEWBIE_CHANNEL]:SetShow(false)
    end
    PaGlobal_ServerSelect_All._ui._stc_channelDesc[PaGlobal_ServerSelect_All._CHANNEL_TYPE.HADUM]:SetShow(false)
    return
  end
  ...
  if false == PaGlobal_ServerSelect_All._isConsole and (PaGlobal_ServerSelect_All._selectedWorldIndex ~= worldServerIdx or PaGlobal_ServerSelect_All._selectedChannelIndex ~= channelServerIdx) then
    ...
    if channelServerData._isSeasonChannel and true == _ContentsGroup_SeasonContents then
      if channelServerData._isSeasonChannel and channelServerData:isPVPServer() then
        channelType = PaGlobal_ServerSelect_All._CHANNEL_TYPE.SEASON_ARSHA_CHANNEL
      elseif channelServerData._isSeasonNewbieChannel then
        channelType = PaGlobal_ServerSelect_All._CHANNEL_TYPE.SEASON_NEWBIE_CHANNEL
      else
        channelType = PaGlobal_ServerSelect_All._CHANNEL_TYPE.SEASONCHANNEL
      end
    elseif channelServerData._isHardCoreChannel == true then
      channelType = PaGlobal_ServerSelect_All._CHANNEL_TYPE.HARDCORE_CHANNEL
    ...
    elseif channelServerData:isPVPServer() then
      ...
      PaGlobal_ServerSelect_All:updateChannelDesc(PaGlobal_ServerSelect_All._CHANNEL_TYPE.ARSHA_CHANNEL, channelServerData._PVPServerType)
    elseif channelServerData._isSiegeChannel then
      channelType = PaGlobal_ServerSelect_All._CHANNEL_TYPE.WAR_INFO
    elseif channelServerData._isHadumChannel then
      channelType = PaGlobal_ServerSelect_All._CHANNEL_TYPE.HADUM
    elseif channelServerData._elfWarGameNo == true then
      channelType = PaGlobal_ServerSelect_All._CHANNEL_TYPE.ELfWAR
    end
    for ii = 2, #PaGlobal_ServerSelect_All._ui._stc_channelDesc do
      PaGlobal_ServerSelect_All._ui._stc_channelDesc[ii]:SetShow(false)
    end
    ...
    PaGlobal_ServerSelect_All._agree = nil
    ...
    if _ContentsGroup_HadumField == false and channelServerData._isHadumChannel == true and PaGlobal_ServerSelect_All._agree == nil then
      PaGlobal_Popup_Hadum_Warning_Open(key32)
      return
    end
    ...
    if true == channelServerData._isSpeedChannel and false == isGMClient then
      local tempWrapper = getTemporaryInformationWrapper()
      if nil ~= tempWrapper and 0 == tempWrapper:getMyAdmissionToSpeedServer() and false == ToClient_IsDevelopment() then
        local resetSelectChannel = function()
          HandleEventLUp_ServerSelect_All_SelectChannel(nil)
...
function HandleEventLUp_ServerSelect_All_EnterChannel(key32)
  ...
  if channelServerData._isHardCoreChannel == true then
    local function selectCurrentServerGroup()
      selectServerGroup(worldIndex - 1, serverIdx - 1)
    end
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_ALERT_DEFAULT_TITLE"),
      content = PAGetString(Defines.StringSheet_GAME, "LUA_HARDCORECHANNEL_SELECT"),
      functionApply = selectCurrentServerGroup,
      priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
    }
    MessageBox.showMessageBox(messageBoxData)
    return
  elseif channelServerData:isPVPServer() then
    ...
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_ALERT_DEFAULT_TITLE"),
      content = PAGetString(Defines.StringSheet_GAME, "LUA_ARSHASERVER_SELECT"),
      functionApply = selectCurrentServerGroup,
      priority = CppEnums.PAUIMB_PRIORITY.PAUIMB_PRIORITY_LOW
    }
    MessageBox.showMessageBox(messageBoxData)
  ...
  if channelServerData._isMultiNationSiegeSever == true and PaGlobal_AsiaChampionshop_Agreement_Open ~= nil then
    PaGlobal_AsiaChampionshop_Agreement_Open(worldIndex - 1, serverIdx - 1)
    return
  end
  selectServerGroup(worldIndex - 1, serverIdx - 1)
end
function HandleEventOn_ServerSelect_All_OverChannelControl(key32)
  ...
  if true == PaGlobal_ServerSelect_All._isConsole then
    ...
    elseif true == _ContentsGroup_SeasonContents and true == channelServerData._isSeasonChannel then
      if true == channelServerData._isSeasonNewbieChannel then
        channelType = PaGlobal_ServerSelect_All._CHANNEL_TYPE.SEASON_NEWBIE_CHANNEL
      else
        channelType = PaGlobal_ServerSelect_All._CHANNEL_TYPE.SEASONCHANNEL
      end
...
function PaGlobal_ServerSelect_All_HadumEnterAgree(key32)
  PaGlobal_ServerSelect_All._agree = true
  HandleEventLUp_ServerSelect_All_SelectChannel(key32)
end
...
function PaGlobal_ServerSelect_All_CheckAndSwitchPadUI()
  if nil == Panel_Lobby_ServerSelect_All then
    return
  end
  PaGlobal_ServerSelect_All:CheckAndSwitchPadUI()
end
function FromClient_ServerSelect_All_Init_FromReload()
  if Panel_Lobby_ServerSelect_All == nil then
    return
  end
  if PaGlobal_ServerSelect_All._initialize == true then
    return
  end
  FromClient_ServerSelect_All_Init()
end
