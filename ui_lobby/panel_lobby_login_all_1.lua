function PaGlobal_Login_All:initialize()
  ...
  self._isNormalBI = isGameTypeKorea()
  ...
  self._ui._stc_ageInfo = UI.getChildControl(Panel_Lobby_Login_All, "Static_AgeInfo")
  self._ui._txt_authorization = UI.getChildControl(Panel_Lobby_Login_All, "StaticText_Authorization")
  ...
  self._ui._txt_authorization:SetShow(false)
  if isGameTypeKR2() == true then
    self._ui._txt_authorization:SetShow(true)
  end
  ...
  if true == _ContentsGroup_MountainOfEternalWinter then
    ...
    self._moviePath = {
      "UI_Movie/Loading/Winter/login_03.bk2",
      "UI_Movie/Loading/Winter/login_04.bk2",
      "UI_Movie/Loading/Winter/login_01.bk2",
      "UI_Movie/Loading/Winter/login_02.bk2",
      "UI_Movie/Loading/Winter/login_05.bk2",
      "UI_Movie/Loading/Winter/login_06.bk2"
    }
    self._movieOrder = {
      1,
      2,
  ...
  if true == _ContentsOption_CH_LOGO then
    self:initLoginMovie()
    self._ui._stc_ageInfo:SetShow(true)
    self._ui._stc_ageInfo:SetIgnore(false)
  else
    self:initLoginMovie()
    self._ui._stc_ageInfo:SetShow(false)
  end
  setPresentLock(true)
...
function PaGlobal_Login_All:registEventHandler()
  ...
  self._ui._stc_ageInfo:addInputEvent("Mouse_LUp", "HandleEventLUp_Login_All_OpenAgeInfo()")
  ...
  registerEvent("FromClient_ConnectToAuthServerTimerStart", "FromClient_ConnectToAuthServerTimerStart")
  if true == self._isConsole then
...
function PaGlobal_Login_All:validate()
  ...
  self._ui._txt_authorization:isValidate()
end
function PaGlobal_Login_All:initLoginImage()
  ...
  if true == _ContentsOption_CH_LOGO then
    local CHTable = {
      [1] = "New_UI_Common_forLua/Window/Lobby/bgValencia_4.dds",
      [2] = "New_UI_Common_forLua/Window/Lobby/bgValenciaSea_2.dds",
      [3] = "New_UI_Common_forLua/Window/Lobby/bgValenciaSea_1.dds",
      [4] = "New_UI_Common_forLua/Window/Lobby/bgDragan_3.dds",
      [5] = "New_UI_Common_forLua/Window/Lobby/bgKamasilvia_2.dds"
    }
    for index = 1, 5 do
      ...
      targetControl:ChangeTextureInfoName(CHTable[index])
...
function PaGlobal_Login_All:onScreenResize()
  ...
  self._ui._txt_authorization:SetPosX((getScreenSizeX() - self._ui._txt_authorization:GetSizeX()) / 2)
  self._ui._txt_authorization:SetPosY(getScreenSizeY() - 50)
  ...
  if isGameTypeKorea() or isGameTypeTaiwan() or isGameTypeGT() or isGameTypeTR() or isGameTypeTH() or isGameTypeID() or true == self._isConsole or isGameTypeRussia() or isGameTypeJapan() or isGameTypeEnglish() or isGameTypeSA() or isGameTypeASIA() then
  ...
  elseif _ContentsOption_CH_LOGO then
    self._ui._stc_daumCI:SetShow(false)
    self._ui._stc_pearlabyssCI:SetShow(false)
    self._ui._stc_ageInfo:ComputePos()
  else
