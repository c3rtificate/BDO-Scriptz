...
function UpdateMeshIndex()
  if not FGlobal_IsInGameMode() and not isNormalCustomizingIndex(selectedClassType, selectedListParamType, selectedListParamIndex, paramValueList[selectedItemIndex + 1]) then
...
function UpdateMeshIndexInternal()
  ...
  local function wizardFaceHairCheck()
    ...
    if __eClassType_WizardMan == selectedClassType and (wizardLastFaceHairIdx == wizardPrevFaceTypeIndex and wizardLastFaceHairIdx ~= selectedParamValue or wizardLastFaceHairIdx ~= prevFaceTypeIndex and wizardLastFaceHairIdx == selectedParamValue) then
  ...
  if 1 == selectedListParamType and 0 == selectedListParamIndex then
    applyDefaultCustomizationMakeupValue(selectedClassType, selectedParamValue)
  end
end
...
function PaGlobalFunc_CustomizationMesh_GetParamValue(itemIndex)
  local luaSelectedItemIndex = itemIndex + 1
  local selectedParamValue = paramValueList[luaSelectedItemIndex]
  if nil == selectedParamValue then
    return 0
  end
