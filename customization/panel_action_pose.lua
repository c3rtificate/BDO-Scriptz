...
function createMotionList()
  ...
  for itemIdx = 0, count - 1 do
    if MotionTable[itemIdx] == social and true == checkMotionContentsType(itemIdx) then
      local tempContentImage = UI.createControl(__ePAUIControl_Static, Frame_Content, "Frame_Image_" .. uiIdx)
      CopyBaseProperty(Frame_ContentImage, tempContentImage)
      tempContentImage:addInputEvent("Mouse_LUp", "UpdateMotion(" .. itemIdx .. ", " .. uiIdx .. ")")
      ...
      tempContentImage:SetPosX(uiIdx % ColumnCount * columnWidth + contentsOffsetX)
      tempContentImage:SetPosY(math.floor(uiIdx / ColumnCount) * columnHeight + contentsOffsetY)
      tempContentImage:setRenderTexture(tempContentImage:getBaseTexture())
      tempContentImage:SetShow(true)
      ContentImage[uiIdx] = tempContentImage
      uiIdx = uiIdx + 1
    end
  end
...
function UpdateMotion(index, uiIndex)
  applyMotion(index)
  UpdateMarkPosition(uiIndex)
end
