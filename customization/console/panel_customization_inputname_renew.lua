...
function PaGlobalFunc_Customization_InputName_Confirm(str)
  ...
  if true == _ContentsGroup_SelectStartPosition and FGlobal_GetPlayerCreateType() == __ePlayerCreateNormal and nil ~= Panel_Window_StartBranch then
  ...
  PaGlobalFunc_ClassSelect_CharacterCreate(nameStr, __eQuestTutorialBranch_0)
end
function PaGlobalFunc_Customization_InputName_SelectStartBranch(tutorialBranch)
  local self = Customization_InputNameInfo
  if false == self._createSync then
    return
  end
  ...
  local nameStr = self._ui._edit_InputName:GetEditText()
  _AudioPostEvent_SystemUiForXBOX(50, 1)
  PaGlobalFunc_ClassSelect_CharacterCreate(nameStr, tutorialBranch)
end
...
function Customization_InputNameInfo:InitEvent()
  ...
  Panel_Customizing_InputName:RegisterUpdateFunc("CustomizationInPutName_UpdatePerFrame")
...
function PaGlobalFunc_Customization_InputName_Open(clearStr)
  ...
  if true == self._ui._chk_season:GetShow() then
    if nil ~= PaGlobal_Season_Character_Create._isSeasonCharacter then
      self._ui._chk_season:SetCheck(PaGlobal_Season_Character_Create._isSeasonCharacter)
    else
...
function PaGlobalFunc_Customization_InputName_CanMakeSeasonCharacter()
  if nil ~= FGlobal_GetPlayerCreateType and FGlobal_GetPlayerCreateType() ~= __ePlayerCreateNormal then
    return false
  end
...
function CustomizationInPutName_UpdatePerFrame(deltaTime)
  if isPadUp(__eJoyPadInputType_LeftThumb) == true then
    ClearFocusEdit()
    PaGlobalFunc_Customization_InputName_CheckDuplicate()
  end
end
