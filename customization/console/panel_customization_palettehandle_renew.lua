...
function PaGlobalFunc_Customization_PaletteHandle_CreateEyePalette(colorTempate, colorParent, focusBox, classType, paramType, paramIndex, paramIndex2, PaletteIndex, checkBoxLeft, checkBoxRight)
  ...
  for colorIndex = 0, count - 1 do
    local tempStatic = UI.createControl(__ePAUIControl_Button, colorParent, "Static_Color_" .. colorIndex)
    ...
    tempStatic:addInputEvent("Mouse_On", "PaGlobalFunc_Customization_PaletteHandle_SetSelectButton(" .. colorIndex .. ",false)")
...
function PaGlobalFunc_Customization_PaletteHandle_UpdateEyePalette(classType, paramType, paramIndex, paramIndex2, colorIndex)
  ...
  PaGlobalFunc_Customization_PaletteHandle_SetSelectButton(colorIndex, false)
end
function PaGlobalFunc_Customization_PaletteHandle_CreateCommonPalette(colorTempate, colorParent, focusBox, classType, paramType, paramIndex, PaletteIndex, uiId)
  local self = Customization_PaletteHandleInfo
  self:ClearPalette()
  self._focusBox = focusBox
  local count = getPaletteColorCount(PaletteIndex)
  for colorIndex = 0, count - 1 do
    ...
    local colorTemp = getPaletteColor(PaletteIndex, colorIndex)
    tempStatic:SetColor(Int64toInt32(colorTemp))
    if 3 == uiId and true == _ContentsGroup_RgbUIPick then
      tempStatic:addInputEvent("Mouse_On", "PaGlobalFunc_Customization_PaletteHandle_SetSelectButton(" .. colorIndex .. ",true)")
      tempStatic:addInputEvent("Mouse_LUp", "PaGlobalFunc_Customization_PaletteHandle_UpdateCommonPalette(" .. classType .. "," .. paramType .. "," .. paramIndex .. "," .. colorIndex .. ",true)")
    else
      tempStatic:addInputEvent("Mouse_On", "PaGlobalFunc_Customization_PaletteHandle_SetSelectButton(" .. colorIndex .. ",false)")
      tempStatic:addInputEvent("Mouse_LUp", "PaGlobalFunc_Customization_PaletteHandle_UpdateCommonPalette(" .. classType .. "," .. paramType .. "," .. paramIndex .. "," .. colorIndex .. ",false)")
    end
    self._colorStatic[colorIndex] = tempStatic
  end
  local verticalCount = count / self._config._columnCount + 1
...
function PaGlobalFunc_Customization_PaletteHandle_UpdateCommonPalette(classType, paramType, paramIndex, colorIndex, forColorPalette)
  local self = Customization_PaletteHandleInfo
  self._currentColorIndex = colorIndex
  self._focusBox:SetPosX(self._colorStatic[colorIndex]:GetPosX() - 2)
  self._focusBox:SetPosY(self._colorStatic[colorIndex]:GetPosY() - 2)
  self._focusBox:SetShow(true)
  setParam(classType, paramType, paramIndex, colorIndex)
  PaGlobalFunc_Customization_PaletteHandle_SetSelectButton(colorIndex, forColorPalette)
end
...
function PaGlobalFunc_Customization_PaletteHandle_SetSelectButton(colorIndex, forColorPalette)
  if nil == forColorPalette then
    return
  end
  local self = Customization_PaletteHandleInfo
  ...
  elseif true == forColorPalette and true == _ContentsGroup_RgbUIPick then
    PaGlobalFunc_Customization_SetKeyGuide(8)
  else
