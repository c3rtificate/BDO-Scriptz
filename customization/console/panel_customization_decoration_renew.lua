...
function Customization_DecoInfo:InitControl()
  ...
  self._originSliderSizeX = self._ui._slider[0]:GetSizeX()
  self._originSliderPosX = self._ui._slider[0]:GetPosX()
end
function Customization_DecoInfo:InitEvent()
  ...
  self._ui._checkBox_LeftEye:addInputEvent("Mouse_LUp", "PaGlobalFunc_Customization_Deco_setLeftEyeDecoSlider()")
  self._ui._checkBox_RightEye:addInputEvent("Mouse_LUp", "PaGlobalFunc_Customization_Deco_setRightEyeDecoSlider()")
end
...
function PaGlobalFunc_Customization_Deco_UpdateDecorationPose()
  ...
  Panel_Window_MessageBox_All:SetShow(false)
end
function PaGlobalFunc_Customization_Deco_UpdateDecorationContents(contentsIndex, currentClassType, currentuiId)
  ...
  if listCount == 1 then
    ...
    for itemIndex = 0, paramMax do
      ...
      if nil == tempContentImage then
        tempContentImage = UI.createControl(__ePAUIControl_RadioButton, self._ui._static_TypeGroup, "Frame_Image_" .. itemIndex .. "_" .. self._currentUiId)
  ...
  for sliderIndex = 0, sliderCount - 1 do
    ...
    if 3 == self._currentUiId and true == _ContentsGroup_RgbUIPick then
      self._ui._slider[sliderIndex]:addInputEvent("Mouse_On", "PaGlobalFunc_Customization_Deco_SliderFocusOn(true)")
    else
      self._ui._slider[sliderIndex]:addInputEvent("Mouse_On", "PaGlobalFunc_Customization_Deco_SliderFocusOn(false)")
    end
    ...
    self._ui._slider[sliderIndex]:ComputePos()
    if 12 == self._currentUiId then
      self._ui._slider[sliderIndex]:SetSize(180, self._ui._slider[sliderIndex]:GetSizeY())
      self._ui._slider[sliderIndex]:SetPosX(self._originSliderPosX + 20)
      self._ui._sliderProgress[sliderIndex]:SetSize(180, self._ui._sliderProgress[sliderIndex]:GetSizeY())
    else
      self._ui._slider[sliderIndex]:SetSize(self._originSliderSizeX, self._ui._slider[sliderIndex]:GetSizeY())
      self._ui._slider[sliderIndex]:SetPosX(self._originSliderPosX)
      self._ui._sliderProgress[sliderIndex]:SetSize(self._originSliderSizeX, self._ui._sliderProgress[sliderIndex]:GetSizeY())
    end
  ...
  if paletteCount == 1 then
    ...
    PaGlobalFunc_Customization_PaletteHandle_CreateCommonPalette(self._ui._colorTemplate, self._ui._static_ColorGroup, self._ui._colorSelect, self._currentClassType, paletteParamType, paletteParamIndex, paletteIndex, self._currentUiId)
    if true == _ContentsGroup_RgbUIPick then
      if 3 == self._currentUiId then
        Panel_Customizing_CommonDecoration:registerPadEvent(__eConsoleUIPadEvent_X, "PaGlobalFunc_Customization_Deco_ColorPaletteToggle(" .. self._currentClassType .. ", " .. paletteParamType .. ", " .. paletteParamIndex .. ")")
      else
        Panel_Customizing_CommonDecoration:registerPadEvent(__eConsoleUIPadEvent_X, "")
      end
    end
...
function PaGlobalFunc_Customization_Deco_ColorPaletteToggle(classType, paramType, paramIndex)
  if nil == Panel_Window_ColorPalette_All then
    return
  end
  if false == _ContentsGroup_RgbUIPick then
    return
  end
  if false == Panel_Window_ColorPalette_All:IsShow() then
    PaGlobalFunc_Customization_KeyGuideSetShow(false)
    PaGlobal_ColorPalette_All_Open(__eRGBPaletteTypeCustomization)
    PaGlobal_ColorPalette_All_SetDecorationPart(classType, paramType, paramIndex)
    ToClient_SetColorPaletteOpen(true)
  else
    PaGlobalFunc_Customization_KeyGuideSetShow(true)
    PaGlobal_ColorPalette_All_Close()
    ToClient_SetColorPaletteOpen(false)
  end
end
...
function PaGlobalFunc_Customization_Deco_OpenEyeDecorationUi(classType, uiId)
  ...
  self._currentUiId = uiId
  self:ClearRadioButton()
  local isShai = getSelfPlayer():getClassType() == __eClassType_ShyWaman
...
function PaGlobalFunc_Customization_Deco_UpdateEyeDecoContents(contentsIndex, addHistory, currentclassType, currentuiId)
  ...
  if listCount == 1 then
    ...
    for itemIndex = 0, paramMax do
      local tempContentImage = self._typeList[itemIndex]
      if nil == tempContentImage then
        tempContentImage = UI.createControl(__ePAUIControl_RadioButton, self._ui._static_TypeGroup, "Frame_Image_" .. itemIndex)
        CopyBaseProperty(self._ui._typeTemplate, tempContentImage)
      end
...
function PaGlobalFunc_Customization_Deco_SliderFocusOn(forColorPalette)
  local self = Customization_DecoInfo
  if true == forColorPalette and true == _ContentsGroup_RgbUIPick then
...
function PaGlobalFunc_Customization_Deco_setLeftEyeDecoSlider()
  local self = Customization_DecoInfo
  local sliderCount = getUiSliderCount(self._currentClassType, self._currentUiId, contentsIndex)
  if self._ui._checkBox_LeftEye:IsCheck() then
    for sliderIndex = 0, sliderCount - 1 do
  ...
  elseif self._ui._checkBox_RightEye:IsCheck() then
    for sliderIndex = 0, sliderCount - 1 do
...
function PaGlobalFunc_Customization_Deco_setRightEyeDecoSlider()
  local self = Customization_DecoInfo
  local sliderCount = getUiSliderCount(self._currentClassType, self._currentUiId, contentsIndex)
  if self._ui._checkBox_RightEye:IsCheck() then
    for sliderIndex = 0, sliderCount - 1 do
      local param = getParam(self._sliderParamType[sliderIndex], self._sliderParamIndex2[sliderIndex])
      PaGlobalFunc_CustomIzationCommon_SetSliderValue(self._ui._slider[sliderIndex], param, self._sliderParamMin[sliderIndex], self._sliderParamMax[sliderIndex])
      PaGlobalFunc_Customization_Deco_UpdateEyeSlider(sliderIndex, self._sliderParamIndex2[sliderIndex])
    end
  elseif self._ui._checkBox_LeftEye:IsCheck() then
    for sliderIndex = 0, sliderCount - 1 do
      local param = getParam(self._sliderParamType[sliderIndex], self._sliderParamIndex[sliderIndex])
      PaGlobalFunc_CustomIzationCommon_SetSliderValue(self._ui._slider[sliderIndex], param, self._sliderParamMin[sliderIndex], self._sliderParamMax[sliderIndex])
      PaGlobalFunc_Customization_Deco_UpdateEyeSlider(sliderIndex, self._sliderParamIndex[sliderIndex])
...
function PaGlobalFunc_Customization_Deco_Close()
  if nil ~= Panel_Window_ColorPalette_All and true == _ContentsGroup_RgbUIPick and true == Panel_Window_ColorPalette_All:IsShow() then
    PaGlobal_ColorPalette_All_Close()
    ToClient_SetColorPaletteOpen(false)
    return
  end
...
function PaGlobalFunc_Customization_Deco_MoveTab(isLeft)
  ...
  if nil ~= self._buttonList[self._currentTabIdx] then
