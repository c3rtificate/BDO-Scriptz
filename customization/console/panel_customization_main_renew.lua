...
local CustomizationMain = {
  ...
  _config = {
    ...
    _slideFocus = 6,
    _subPanel = 7,
    _colorButtonColorPalette = 8,
    _sliderFocusColorPalette = 9
  },
  _mainMenuConfig = {
    _Customizing = 0,
    _Zodiac = 1,
  ...
  _isCameraMode = false,
  _isCustomEditable = true,
  _customAuthorUserNo = -1,
  _showSeasonBanner = false,
  _keyGuidesText = {
    ...
    _strColorPalette = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_COLORPALETTE_TITLE")
  }
}
...
function PaGlobalFunc_Customization_ClickedZodiac(index)
  ...
  if zodiacDescription ~= nil then
    self._ui._staticText_ZodiacDesc:SetTextMode(__eTextMode_AutoWrap)
...
function CustomizationMain:MainMenuOpen()
  local customSetIgnore = false
  local selfPlayerUserNo = Defines.s64_const.s64_m1
  if true == ToClient_isLobbyProcessor() then
    selfPlayerUserNo = getUserNoByLobby()
  else
    selfPlayerUserNo = getSelfPlayer():get():getUserNo()
  end
  if false == self._isCustomEditable and (selfplayerUserNo == authorUserNo or Defines.s64_const.s64_m1 == authorUserNo) then
    customSetIgnore = false
  end
  ...
  for index, mainButton in pairs(self._ui._currentMainMenu) do
    ...
    if self._mainMenuConfig._Customizing == index and true == customSetIgnore then
      mainButton:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CUSTOMIZATION_NOEDIT"))
      mainButton:SetIgnore(true)
    elseif self._mainMenuConfig._Customizing == index then
      mainButton:SetText(PAGetString(Defines.StringSheet_RESOURCE, "INTERACTION_BTN_CHANGELOOK"))
    end
...
function CustomizationMain:InitControl()
  ...
  self._ui._static_KeyGuideBg:SetPosXY(Panel_Customizing:GetPosX(), Panel_Customizing:GetPosY() + Panel_Customizing:GetSizeY() - self._ui._static_KeyGuideBg:GetSizeY())
  self._ui._static_BottomBg:SetPosXY(0, 0)
  ...
  for index = 0, self._config._CustomizingMaxSubTree - 1 do
    local control = UI.createControl(__ePAUIControl_RadioButton, self._ui._static_SubMenuBg, "radioButton_Customizing_" .. index)
  ...
  for index = 0, count - 1 do
    local control = UI.createControl(__ePAUIControl_RadioButton, self._ui._static_SubMenuBg, "radioButton_Zodiac_" .. index)
    CopyBaseProperty(self._ui._subMenuTemplate, control)
  ...
  for index = 0, self._config._ActionMaxSubTree - 1 do
    local control = UI.createControl(__ePAUIControl_RadioButton, self._ui._static_SubMenuBg, "radioButton_Action_" .. index)
    CopyBaseProperty(self._ui._subMenuTemplate, control)
    control:SetPosX(self._focusMenuPosX)
    control:SetPosY(self._focusMenuPosY + (control:GetSizeY() + 30) * (index + 1))
  ...
  for index = 0, self._config._maxLeafTree - 1 do
    local control = UI.createControl(__ePAUIControl_RadioButton, self._ui._static_SubMenuBg, "leafButton_" .. index)
  ...
  self._ui._web_RandomBeauty = UI.createControl(__ePAUIControl_WebControl, Panel_Customizing, "WebControl_RandomCustomization_Renew")
end
function PaGlobalFunc_Customization_SetKeyGuide(state)
  ...
  if true == _ContentsGroup_UsePadSnapping and false == _ContentsGroup_RenewUI then
    if self._currentKeyGuideIndex == self._config._boneSlideFocus or self._currentKeyGuideIndex == self._config._slideFocus then
      ToClient_SetEnableCameraPostUpdate(false)
    else
      ToClient_SetEnableCameraPostUpdate(true)
    end
    if self._currentKeyGuideIndex == self._config._boneCamera then
...
function CustomizationMain:MakeKeyGuide(state)
  ...
  keyGuideTableTemplete._bg = UI.createControl(__ePAUIControl_Static, self._ui._static_KeyGuideBg, "Static_Custom_bottomBg_" .. #self._KeyGuideTable)
  ...
  elseif self._config._colorButtonColorPalette == state then
    self:addKeyGuide(keyGuideTableTemplete, {
      self._keyGuideTextureConfig._B
    }, keyText._strBack)
    self:addKeyGuide(keyGuideTableTemplete, {
    ...
    }, keyText._strColorPalette)
    self:addKeyGuide(keyGuideTableTemplete, {
      self._keyGuideTextureConfig._RSClick
    }, keyText._strCameramode)
  elseif self._config._sliderFocusColorPalette == state then
    self:addKeyGuide(keyGuideTableTemplete, {
      self._keyGuideTextureConfig._B
    }, keyText._strBack)
    self:addKeyGuide(keyGuideTableTemplete, {
...
function CustomizationMain:addKeyGuide(keyGuideTable, iconKeyList, str)
  ...
  local strControl = UI.createControl(__ePAUIControl_StaticText, keyGuideTable._bg, "StaticText_Custom_KeyGuide_" .. #self._KeyGuideTable .. "_" .. self._currentKeyGuideCount)
  ...
  if nil == keyGuideTable[self._currentKeyGuideCount - 1] then
    startPosX = getResolutionSizeX() - strControl:GetTextSizeX() - 50
  else
  ...
  for index = #iconKeyList - 1, 0, -1 do
    ...
    local control = UI.createControl(__ePAUIControl_Button, keyGuideTable._bg, "Button_Custom_KeyGuide_" .. #self._KeyGuideTable .. "_" .. self._currentKeyGuideCount .. "_" .. index)
...
function PaGlobalFunc_Customization_Back()
  local self = CustomizationMain
  _AudioPostEvent_SystemUiForXBOX(50, 3)
  local isCustomizingAlbumOpen = false
  if nil ~= FGlobal_CustomizingAlbum_GetShow then
    isCustomizingAlbumOpen = FGlobal_CustomizingAlbum_GetShow()
  end
  if true == isCustomizingAlbumOpen then
    if true == _ContentsGroup_RenewUI and nil ~= PaGlobalFunc_CustomizationAlbum_Escape and false == PaGlobalFunc_CustomizationAlbum_Escape() then
...
function CustomizationMain:InitKeyGuide()
  ...
  self._KeyGuideTable[self._config._colorButtonColorPalette] = {}
  self._KeyGuideTable[self._config._colorButtonColorPalette] = self:MakeKeyGuide(self._config._colorButtonColorPalette)
  self._KeyGuideTable[self._config._sliderFocusColorPalette] = {}
  self._KeyGuideTable[self._config._sliderFocusColorPalette] = self:MakeKeyGuide(self._config._sliderFocusColorPalette)
end
...
function PaGlobalFunc_FromClient_Customization_ShowAllUI(isShow)
  ...
  PaGlobalFunc_Customization_OpenSeasonBanner(isShow)
end
...
function PaGlobalFunc_Customization_SetShow(isShow, isAni)
  Panel_Customizing:SetPosXY(5, 5)
  Panel_Customizing:SetSize(getScreenSizeX() - 10, getScreenSizeY() - 10)
  if true == isShow then
    local self = CustomizationMain
    self._ui._static_KeyGuideBg:SetPosXY(Panel_Customizing:GetPosX(), Panel_Customizing:GetPosY() + Panel_Customizing:GetSizeY() - self._ui._static_KeyGuideBg:GetSizeY())
    self._ui._static_BottomBg:SetPosXY(0, 0)
...
function FromClient_CustomizationIsEditable(isEditable)
  CustomizationMain:IsEditable(isEditable)
end
function CustomizationMain:IsEditable(isEditable)
  self._customAuthorUserNo = ToClient_getOriginalAuthorUserNoTmp()
  self._isCustomEditable = isEditable
  if 0 == self._currentDepth then
    self:MainMenuOpen()
  end
end
function PaGlobalFunc_Customization_OpenSeasonBanner(isShow)
  if false == isShow then
    return
  end
  ...
  if true == CustomizationMain._showSeasonBanner then
    return
  end
  local temporaryWrapper = getTemporaryInformationWrapper()
  ...
  if (1 == userType or 2 == userType) and nil ~= PaGlobalFunc_SeasonBanner_Open then
    ...
    CustomizationMain._showSeasonBanner = true
  end
end
