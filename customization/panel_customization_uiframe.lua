...
function CloseFrame()
  if true == _ContentsGroup_RgbUIPick and true == ToClient_isLobbyProcessor() and nil ~= Panel_Window_ColorPalette_All and true == Panel_Window_ColorPalette_All:IsShow() then
    return
  end
...
function PaGloabl_getFacehairCheckBtnValue()
  return CheckButton_UseFaceCustomizationHair:IsCheck()
end
function PaGloabl_setFacehairCheckBtnValue(state)
  setUseFaceCustomizationHair(state)
end
function OpenCustomizationUiGroupFrame(classType, uiGroupIndex)
  ...
  local isShy = __eClassType_ShyWaman == getSelfPlayer():getClassType()
  for uiPartIndex = 0, partNum - 1 do
    if ChekcTatoo_PossibleContry(uiPartIndex, false) then
      local luaUiPartIndex = uiPartIndex + 1
      local partName = getUiPartDescName(classType, uiGroupIndex, uiPartIndex)
      local tempGroup = {button, text}
      local tempStaticText = UI.createControl(__ePAUIControl_StaticText, Panel_CustomizationFrame, "StaticText_Category_" .. uiPartIndex)
      ...
      local tempButton = UI.createControl(__ePAUIControl_Button, Panel_CustomizationFrame, "Button_ShowDetail_" .. uiPartIndex)
      ...
      tempButton:SetIgnore(isBlock and isShy)
      tempButton:SetMonoTone(isBlock and isShy)
      tempStaticText:SetMonoTone(isBlock and isShy)
...
function ChekcTatoo_PossibleContry(uiPartIndex, isContainer)
  ...
  local isTatoo = false == isTatooGroup or false == _ContentsOption_CH_TatooCheck or tatooIndex + plusIndex ~= uiPartIndex and true == _ContentsOption_CH_TatooCheck
  return isTatoo
end
