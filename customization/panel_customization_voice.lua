...
function createVoiceList(inGameMode, classType)
  ...
  for itemIdx = 0, VoiceCount - 1 do
    ...
    local tempContentImage = UI.createControl(__ePAUIControl_Static, Panel_CustomizationVoice, "Frame_Image_" .. itemIdx)
    CopyBaseProperty(Frame_ContentImage, tempContentImage)
    local tempVoiceCount = UI.createControl(__ePAUIControl_StaticText, tempContentImage, "Frame_VoiceCount_" .. itemIdx)
    CopyBaseProperty(voiceCountText, tempVoiceCount)
    local staticPayMark = UI.createControl(__ePAUIControl_Static, tempContentImage, "Static_PayMark_" .. itemIdx)
    ...
    if not FGlobal_IsInGameMode() and not isNormalCustomizingIndex(classType, NoCashType.eCustomizationNoCashType_Voice, NoCashVoice.eCustoimzationNoCashVoice_Type, itemIdx) then
  ...
  if Const_UsebleMotionbyServiceArea then
    ...
    for itemIdx = 0, count - 1 do
      if MotionTable[itemIdx] == social then
        local tempContentImage = UI.createControl(__ePAUIControl_Static, Frame_Content, "Frame_MotionImage_" .. createIndex)
...
function showVoiceUI(show)
  ...
  PaGlobal_SetIgnoreCameraLook(show)
  if true == show then
    CloseFrameForPoseUI()
