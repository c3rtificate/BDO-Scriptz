...
local stc_TopLine1 = UI.getChildControl(stc_Zodiac, "Static_TopHorizonLine1")
local stc_BottomLine1 = UI.getChildControl(stc_Zodiac, "Static_BottomHorizonLine1")
...
local _ContentsGroup_newCreateButtonLayout = true
local default_Area = UI.getChildControl(Panel_CustomizationMain, "Static_NewCreateButtonArea")
local btn_backHome = UI.getChildControl(default_Area, "Button_BackHome")
local chk_seasonCreateGo = UI.getChildControl(default_Area, "CheckButton_SeasonCreate")
local stc_seasonCreateCount = UI.getChildControl(chk_seasonCreateGo, "StaticText_CreateCount")
local btn_createCharacterButton = UI.getChildControl(default_Area, "Button_CreateCharacter")
local txt_familyNameValue = UI.getChildControl(default_Area, "StaticText_FamilyName_Value")
local edt_NameInput = UI.getChildControl(default_Area, "Edit_CharacterName_Input")
local btn_duplicationChk = UI.getChildControl(default_Area, "Button_DuplicationCheck")
local txt_duplicationChkComment = UI.getChildControl(default_Area, "StaticText_CheckComment")
local btn_Rule = UI.getChildControl(default_Area, "Button_Rule")
txt_duplicationChkComment:SetText("")
local txt_authorInfo = UI.getChildControl(Panel_CustomizationMain, "StaticText_AuthorInfo")
txt_authorInfo:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CUSTOMIZATIONMAIN_CUSTOMIZING_INFO"))
local txt_authorInfoValue = UI.getChildControl(Panel_CustomizationMain, "StaticText_AuthorInfoValue")
local line_HorizonLine = UI.getChildControl(stc_Zodiac, "Static_BottomHorizonLine1")
...
Button_Duplication:addInputEvent("Mouse_LUp", "HandleClicked_DuplicateName(false)")
Button_Char:addInputEvent("Mouse_LUp", "HandleClicked_DuplicateName(true)")
btn_backHome:addInputEvent("Mouse_LUp", "HandleClicked_CustomizationMain_SelectClass()")
Button_Back:addInputEvent("Mouse_LUp", "HandleClicked_CustomizationMain_Back()")
btn_Rule:addInputEvent("Mouse_LUp", "HandleClicked_RuleShow()")
...
local function updateComputePos()
  ...
  txt_authorInfo:ComputePos()
  txt_authorInfoValue:ComputePos()
...
function SelectZodiac(zodiacIndex)
  ...
  if zodiacDescription ~= nil then
    ...
    stc_Zodiac:SetSize(210, StaticText_ZodiacDescription:GetTextSizeY() + StaticText_ZodiacName:GetSizeY() + Static_ZodiacImage:GetSizeY() + 50)
      ...
      if false == _ContentsGroup_isConsolePadControl then
        ...
        if isGameTypeKR2() == true then
          ...
          if idx == 2 then
            ...
            mainButtonInfo[idx].button:AddEffect("UI_Customize_Button01", true, 0, 0)
          else
            mainButtonInfo[idx].button:EraseAllEffect()
...
function CustomizationStatic_UpdatePerFrame(deltaTime)
  ...
  if isKeyUpFor(CppEnums.VirtualKeyCode.KeyCode_RIGHT) then
    if false == Edit_CharName:GetFocusEdit() and false == FileExplorer_IsOpen() and false == edit_SeasonCharName:GetFocusEdit() and false == edt_NameInput:GetFocusEdit() then
      customHistoryDo()
    end
  elseif isKeyUpFor(CppEnums.VirtualKeyCode.KeyCode_LEFT) and false == Edit_CharName:GetFocusEdit() and false == FileExplorer_IsOpen() and false == edit_SeasonCharName:GetFocusEdit() and false == edt_NameInput:GetFocusEdit() then
    customHistoryUnDo()
  end
  ...
  if initialized == true then
    for index = 1, mainButtonNum do
      if index ~= 3 then
        ...
        if true == mainButtonInfo[index].isOpen and distance > 170 and isGameTypeKR2() == false then
...
function InitCustomizationMainUI()
  ...
  CreateHistoryButton()
  ClearFocusEdit()
  txt_duplicationChkComment:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERCREATE_MAKEANAME"))
  InitZodiac()
  btn_createCharacterButton:SetSpanSize(10, 45)
  if false == _ContentsGroup_newCreateButtonLayout then
    chracterCraeation_SeasonSetting()
  else
    chracterCraeation_SeasonSetting_New()
  end
...
function chracterCraeation_SeasonSetting()
  if false == _ContentsGroup_SeasonContents and false == _ContentsGroup_PreSeason then
    ...
    chk_seasonCreateGo:SetShow(false)
    default_Area:SetShow(false)
    return
  end
  if nil == FGlobal_GetPlayerCreateType or nil == PaGlobalFunc_CharacterSelect_All_CanMakeSeason then
    return
  end
  ...
  default_Area:SetShow(true)
  btn_duplicate_season:SetShow(true)
  btn_duplicationChk:SetShow(true)
  local canMakeSeason = true == PaGlobalFunc_CharacterSelect_All_CanMakeSeason() and FGlobal_GetPlayerCreateType() == __ePlayerCreateNormal
  ...
  chk_seasonCreateGo:SetShow(canMakeSeason)
  if nil ~= PaGlobal_Season_Character_Create._isSeasonCharacter then
    chk_Season:SetCheck(PaGlobal_Season_Character_Create._isSeasonCharacter)
    chk_seasonCreateGo:SetCheck(PaGlobal_Season_Character_Create._isSeasonCharacter)
  else
    chk_Season:SetCheck(false)
    chk_seasonCreateGo:SetCheck(false)
  end
  default_Area:ComputePosAllChild()
  ...
  btn_CreateSeason:addInputEvent("Mouse_LUp", "HandleClicked_DuplicateName(true)")
  btn_createCharacterButton:addInputEvent("Mouse_LUp", "HandleClicked_DuplicateName(true)")
  btn_duplicate_season:addInputEvent("Mouse_LUp", "HandleClicked_DuplicateName(false)")
  btn_duplicationChk:addInputEvent("Mouse_LUp", "HandleClicked_DuplicateName(false)")
  chk_Season:addInputEvent("Mouse_LUp", "PaGlobalFunc_Customization_SeasonBtnTextureChange()")
  chk_seasonCreateGo:addInputEvent("Mouse_LUp", "PaGlobalFunc_Customization_SeasonBtnTextureChange()")
  ...
  edt_NameInput:SetMaxInput(getGameServiceTypeCharacterNameLength())
  edt_NameInput:SetEditText("")
  if nil ~= PaGlobal_Season_Character_Create._isSeasonCharacter then
    if FGlobal_GetPlayerCreateType() == __ePlayerCreatePremium then
      btn_CreateSeason:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERSELECT_CREATENEWCHARACTER_BTN"))
    end
  elseif FGlobal_GetPlayerCreateType() ~= __ePlayerCreateNormal then
    btn_CreateSeason:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERSELECT_CREATENEWCHARACTER_BTN"))
  else
  ...
  txt_familyNameValue:SetText(getFamilyName())
  stc_seasonCreateCount:SetText(PaGetStringParam1(Defines.StringSheet_RESOURCE, "PANEL_CREATE_SEASONCHARACTER_COUNT", "NUM", tostring(getUsableSeasonCharacterSlotCount())))
end
function chracterCraeation_SeasonSetting_New()
  if nil == FGlobal_GetPlayerCreateType or nil == PaGlobalFunc_CharacterSelect_All_CanMakeSeason then
    return
  end
  Button_Duplication:SetShow(false)
  ...
  btn_createCharacterButton:SetSpanSize(10, 20)
  chk_seasonCreateGo:SetShow(canMakeSeason)
  if PaGlobalFunc_Customization_ForceFirstChracterSeason() == true then
    chk_seasonCreateGo:SetCheck(true)
  end
  default_Area:ComputePosAllChild()
  btn_createCharacterButton:addInputEvent("Mouse_LUp", "HandleClicked_DuplicateName(true)")
  btn_duplicationChk:addInputEvent("Mouse_LUp", "HandleClicked_DuplicateName(false)")
...
function showAllUI(show)
  ...
  if _ContentsGroup_isConsolePadControl then
    ...
    txt_authorInfo:SetShow(false)
    txt_authorInfoValue:SetShow(false)
...
function ToggleUi()
  if CheckButton_ToggleUi:IsCheck() then
    ...
    elseif 4 == hideUIIndex then
      Panel_CustomizationVoice:SetShow(true)
    ...
    elseif Panel_CustomizationFrame:GetShow() then
      ...
      hideUIIndex = 1
    elseif Panel_CustomizationVoice:GetShow() then
...
function CreateHistoryButton()
  if false == _ContentsGroup_isConsolePadControl then
    ...
    for index = 0, 9 do
      ...
      tempButton:SetPosX(PosX + Button_SaveHistory:GetSizeX() + 5 + (tempButton:GetSizeX() + 2.8) * index)
      ...
      if index < count then
        ...
        tempButton:SetText(index + 1)
      else
        local x1, y1, x2, y2 = setTextureUV_Func(tempButton, 1, 1, 41, 41)
...
function HandleClicked_DuplicateName(isCreateCharacterAfterDuplicateCheck)
  ...
  ToClient_CheckOverlapChangeName(userInput, 0, true, isCreateCharacterAfterDuplicateCheck)
end
function PaGlobal_EnterInputKey()
  local isSeason = false
  local userInput = ""
  if isGameTypeKR2() == true then
    local matchString = "<PAColor0xffb7e65d>" .. PAGetString(Defines.StringSheet_GAME, "LUA_DUPLICATIONNAME_CREATE_POSSIBLE") .. "<PAOldColor>"
    if txt_duplicationChkComment:GetText() == matchString then
      HandleClicked_DuplicateName(true)
      return
    end
  end
  if false == _ContentsGroup_SeasonContents and false == _ContentsGroup_PreSeason then
  ...
  if "" == userInput then
    ...
    SetFocusEdit(edt_NameInput)
    edt_NameInput:EraseAllEffect()
    edt_NameInput:AddEffect("fUI_InGame_Light_04A", false, 50, 0)
    return
  end
  ToClient_CheckOverlapChangeName(userInput, 0, true, false)
  ClearFocusEdit()
end
...
function ShowCharacterCustomization(customizationData, classIndex, isInGame)
  ...
  if isInGame == true then
    ...
    btn_Rule:SetShow(false)
    btn_RandomBeauty:SetShow(_ContentsGroup_webAlbumOpen)
    Static_CharName:SetShow(false)
    Edit_CharName:SetShow(false)
    staticMainImage[5]:SetShow(false)
    stc_TopLine1:SetShow(false)
    stc_BottomLine1:SetShow(false)
    ...
    btn_Rule:SetShow(true)
    btn_RandomBeauty:SetShow(_ContentsGroup_webAlbumOpen)
    Static_CharName:SetShow(true)
    Edit_CharName:SetShow(true)
    stc_TopLine1:SetShow(true)
    stc_BottomLine1:SetShow(true)
  ...
  if (isGameTypeEnglish() or isGameTypeTaiwan() or isGameTypeGT() or isGameTypeTR() or isGameTypeTH() or isGameTypeID() or isGameTypeASIA()) and not isInGame then
    btn_Rule:SetShow(true)
  else
    btn_Rule:SetShow(false)
  end
  ...
  if false == _ContentsGroup_newCreateButtonLayout then
    ...
    txt_authorInfo:SetShow(true)
    txt_authorInfoValue:SetShow(true)
  end
  if _ContentsOption_CH_Customization_Body == true then
    txt_authorInfo:SetShow(false)
    txt_authorInfoValue:SetShow(false)
  end
end
function CustomizationMain_OpenSeasonBanner(isShow)
  ...
  if nil ~= FGlobal_GetPlayerCreateType and FGlobal_GetPlayerCreateType() == __ePlayerCreatePremium then
    return
  end
...
function EventSelectClass()
  ...
  PaGlobalFunc_Customization_ResetUrlRandBeauty()
  changeCreateCharacterMode_SelectClass(FGlobal_GetPlayerCreateType())
...
function CustomizationAuthorName(authorName)
  if nil == authorName then
    ...
    txt_authorInfoValue:SetText("-")
  else
    StaticText_AuthorName:SetText(authorName)
    txt_authorInfoValue:SetText("<PAColor0xffa78e6a>" .. PAGetString(Defines.StringSheet_GAME, "LUA_CUSTOMIZATIONMAIN_CUSTOMIZING_AUTHOR") .. "<PAOldColor> <PAColor0xffd8ad70>" .. authorName .. "<PAOldColor>")
  end
end
...
function PaGlobal_CustomizationIsEditable(isEditable, authorUserNo)
  ...
  if setIgnore == true then
    local warnPosX = txt_authorInfoValue:GetPosX() + txt_authorInfoValue:GetTextSizeX()
    btn_Warn:SetSpanSize(warnPosX, 107)
    btn_Warn:ComputePos()
...
function FromClient_VaildCheckResultForCharacterName(type, isCreate, isPossible)
  ...
  if true == isPossible then
    ...
    txt_duplicationChkComment:SetText("<PAColor0xffb7e65d>" .. messageBoxMemo .. "<PAOldColor>")
    local isSeason = false
    if false == _ContentsGroup_SeasonContents and false == _ContentsGroup_PreSeason then
      isSeason = false
    else
    ...
    if true == _ContentsGroup_newCreateButtonLayout then
      cachedInputCharacterName = edt_NameInput:GetEditText()
      txt_duplicationChkComment:EraseAllEffect()
      txt_duplicationChkComment:AddEffect("fUI_InGame_Light_05A", false, 50, 0)
    end
    isValidInputCharacterName = true
  else
    messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_DUPLICATIONNAME_CREATE_IMPOSSIBLE")
    txt_duplicationChkComment:SetText("<PAColor0xffd05d48>" .. messageBoxMemo .. "<PAOldColor>")
    cachedInputCharacterName = ""
    isValidInputCharacterName = false
  end
  if false == _ContentsGroup_newCreateButtonLayout then
...
function PaGlobalFunc_Customization_ForceFirstChracterSeason()
  ...
  if FGlobal_GetPlayerCreateType() ~= __ePlayerCreateNormal then
    return false
  end
  local temporaryWrapper = getTemporaryInformationWrapper()
...
function PaGlobalFunc_Customization_SeasonBtnTextureChange()
  ...
  if PaGlobalFunc_Customization_ForceFirstChracterSeason() == true then
    chk_seasonCreateGo:SetCheck(true)
    local title = ""
    local contenet = PAGetString(Defines.StringSheet_GAME, "LUA_SEASONCHARACTER_CREATE_MSG")
    local messageBoxData = {
      title = title,
      content = contenet,
      functionYes = MessageBox_Empty_function,
  ...
  if true == chk_Season:GetShow() and true == chk_Season:IsCheck() then
...
function PaGlobalFunc_Customization_IsSeasonBtnClick()
  ...
  if false == _ContentsGroup_newCreateButtonLayout then
    ...
    if false == chk_seasonCreateGo:GetShow() then
      return false
    end
    return chk_seasonCreateGo:IsCheck()
  end
end
...
function PaGlobalFunc_Customization_GetInputName()
  if fasle == _ContentsGroup_newCreateButtonLayout then
    return ""
  end
  return edt_NameInput:GetEditText()
end
