...
function CreateCommonPalette(FrameTemplate, ftCollision, classType, paramType, paramIndex, PaletteIndex, isBlend)
  ...
  for colorIndex = 0, count - 1 do
    ...
    local tempStatic = UI.createControl(__ePAUIControl_Static, ftContent, "Static_Color_" .. colorIndex)
    ...
    tempStatic:SetColor(Uint64toUint32(colorTemp))
  ...
  if true == _ContentsGroup_RgbUIPick and (__eRgbPartLimitationOnHairBase == paramIndex or __eRgbPartLimitationOnHairEnd == paramIndex or __eRgbPartLimitationOnHairIn == paramIndex) then
    isPaletteOpen = true
  end
  if false == isPaletteOpen then
    FrameTemplate:SetSize(imageSize * 10 + 9, math.floor(count / ColumnCount + 0.5) * imageSize + math.floor(count / ColumnCount + 0.5))
  else
    local Frame_Button = UI.getChildControl(Frame_Content, "Frame_Button")
    if nil ~= Frame_Button then
      if nil == colorSelectionButton then
        colorSelectionButton = UI.createControl(__ePAUIControl_Button, FrameTemplate, "Button_ColorSelection")
        CopyBaseProperty(Frame_Button, colorSelectionButton)
      end
      local yPosOffSet = 5
      colorSelectionButton:SetTextMode(__eTextMode_AutoWrap)
      colorSelectionButton:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_COLORPALETTE_TITLE"))
      colorSelectionButton:SetPosY(ftCollision:GetSizeY() + yPosOffSet)
      colorSelectionButton:SetShow(true)
      colorSelectionButton:addInputEvent("Mouse_LUp", "OpenColorPaletteToggle(" .. classType .. "," .. paramType .. "," .. paramIndex .. ")")
      if nil ~= PaGlobal_ColorPalette_All_SetDecorationPart then
        PaGlobal_ColorPalette_All_SetDecorationPart(classType, paramType, paramIndex)
        PaGlobal_ColorPalette_All_ClearLastValueCached()
      end
      FrameTemplate:SetSize(ftCollision:GetSizeX(), colorSelectionButton:GetPosY() + colorSelectionButton:GetSizeY() + yPosOffSet)
    end
  end
  FrameTemplate:UpdateContentScroll()
  FrameTemplate:UpdateContentPos()
end
function OpenColorPaletteToggle(classType, paramType, paramIndex)
  if nil == PaGlobal_ColorPalette_All_Open then
    return
  end
  if nil == PaGlobal_ColorPalette_All_SetDecorationPart then
    return
...
function clearPalette()
  ...
  rgbArr = {}
  if nil ~= colorSelectionButton then
    colorSelectionButton:SetShow(false)
    UI.deleteControl(colorSelectionButton)
    colorSelectionButton = nil
  end
end
