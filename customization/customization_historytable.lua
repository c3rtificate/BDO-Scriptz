...
if true == _ContentsGroup_ChinaFontColor then
  selectColor = Defines.Color.C_FFE4A84A
end
...
function SetHistroyList()
  ...
  list2_HistoryList:moveBottomIndex()
end
...
function registEventHandler()
  ...
  list2_HistoryList:registEvent(__ePAUIList2EventType_LuaChangeContent, "HistoryListControlCreate")
  list2_HistoryList:createChildContent(__ePAUIList2ElementManagerType_List)
end
...
function calculateText(GroupIndex, PartIndex)
  ...
  getUiPartDescName(classtype, GroupIndex, PartIndex)
  if 0 == GroupIndex then
    text = PAGetString(Defines.StringSheet_RESOURCE, "UI_CUSTOMIZATION_MAIN_HAIR") .. " - "
    partStr = getUiPartDescName(classtype, GroupIndex, PartIndex)
