...
function PaGlobalFunc_Camp_All_CloseAll()
  ...
  if true == _ContentsGroup_UsePadSnapping then
    ...
    if nil ~= Panel_Dialog_Repair_Function_All and Panel_Dialog_Repair_Function_All:GetShow() and PaGlobalFunc_RepairFunc_All_IsCampingType() then
...
function HandleClickedLUp_Camp_All_OpenMenuFuction(menuNo)
  ...
  if true == _ContentsGroup_DeleteMaxEnchanterName then
    PaGlobal_DeleteMaxEnchanterName_Close()
  end
...
function FromClient_Camp_All_OpenByActorKeyRaw(actorKeyRaw)
  ...
  if false == _ContentsGroup_UsePadSnapping then
    ...
    if nil ~= PaGlobalFunc_NPCShop_ALL_SetIsCamping then
...
function FromClient_Camp_All_OpenCampingShop()
  ...
  PaGlobal_Camp_All._isCamping = true
  if nil ~= FromClient_NPCShop_ALL_Open then
...
function PaGlobalFunc_Camp_All_UpdateSnapTarget(deltaTime)
  if nil == Panel_Window_Camp_All then
    return
  end
  if false == _ContentsGroup_UsePadSnapping or true == ToClient_isConsole() or nil == PaGlobal_Camp_All._ui.btn_Menu[4] or false == PaGlobal_Camp_All._ui.btn_Menu[4]:GetShow() then
    Panel_Window_Camp_All:ClearUpdateLuaFunc()
  end
  PaGlobal_Camp_All._curSnapDelayTime = PaGlobal_Camp_All._curSnapDelayTime + deltaTime
  if PaGlobal_Camp_All._curSnapDelayTime > 0.05 then
    PaGlobal_Camp_All._curSnapDelayTime = 0
    ToClient_padSnapResetControl()
    ToClient_padSnapChangeToTarget(PaGlobal_Camp_All._ui.btn_Menu[4])
    Panel_Window_Camp_All:ClearUpdateLuaFunc()
  end
end
