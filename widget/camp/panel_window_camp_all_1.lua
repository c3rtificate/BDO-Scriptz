...
function PaGlobal_Camp_All:prepareOpen()
  ...
  if true == _ContentsGroup_UsePadSnapping and false == ToClient_isConsole() then
    PaGlobal_Camp_All._curSnapDelayTime = 0
    Panel_Window_Camp_All:RegisterUpdateFunc("PaGlobalFunc_Camp_All_UpdateSnapTarget")
  end
end
    ...
    if isUnseal then
      ...
      if nil ~= itemWrapper then
        if 6 ~= value then
          ...
          if 4 == value then
            ToClient_padSnapChangeToTarget(self._ui.btn_Menu[value])
          end
        end
