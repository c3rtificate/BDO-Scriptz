...
local isOpenInGameCashShop = false
...
local _btn_Refresh = UI.getChildControl(Panel_ScreenShotAlbum_FullScreen, "Button_Refresh")
...
function Panel_ScreenShotAlbum_FullScreen_Initialize()
  _screenshotAlbumWeb = UI.createControl(__ePAUIControl_WebControl, Panel_ScreenShotAlbum_FullScreen, "WebControl_ScreenshotAlbum")
...
function ScreenshotAlbum_FullScreen_Open(addUrl)
  if nil ~= Panel_IngameCashShop and true == Panel_IngameCashShop:GetShow() then
    isOpenInGameCashShop = true
  end
...
function ScreenshotAlbum_FullScreen_Close()
  ...
  _screenshotAlbumWeb:ResetUrl()
  Panel_ScreenShotAlbum_FullScreen:SetShow(false, false)
  if false == isOpenInGameCashShop then
    FGlobal_ScreenshotAlbum_ShowByScreenShotFrame()
  end
  isOpenInGameCashShop = false
end
