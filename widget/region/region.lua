...
function Region_OnChangeRegion(regionData)
  if nil == regionData then
    ...
    if Panel_ElfWar_Region ~= nil and Panel_ElfWar_Region:GetShow() == true then
      return
    end
    if true == regionData:get():isAtoraxion() then
      if PaGlobalFunc_Atoraxion_RegionInfo_Open ~= nil then
        PaGlobalFunc_Atoraxion_RegionInfo_Open(regionData)
      end
      if FromClient_Atoraxion_RegionChange_Open ~= nil then
        FromClient_Atoraxion_RegionChange_Open(regionData)
        PaGlobalFunc_PartyRecruitIcon_Show()
      end
      return
    elseif PaGlobalFunc_AtoraxionInfo_Close ~= nil then
      PaGlobalFunc_AtoraxionInfo_Close()
    end
