...
function FromClient_OpenExplorer_CustomizingCharacter(title, defaultName, paramList)
  ...
  for key, value in pairs(paramList) do
    ...
    elseif ".bdc" == string.sub(value, 1, 4) then
      if "_0" == string.sub(value, 5, 6) then
        isEditable = false
      elseif "_1" == string.sub(value, 5, 6) then
        isEditable = true
      end
    end
  ...
  if false == _ContentsGroup_RenewUI_Customization then
    ToClient_UploadWebCustomizingCharacter(false == Panel_CustomizationMain:GetShow(), subNameStringBDC, isEditable)
  else
    ToClient_UploadWebCustomizingCharacter(false == PaGlobalFunc_Customization_GetShow(), subNameStringBDC, isEditable)
  end
end
...
local screenShotFromPhotoGallery = false
function FromClient_OpenExplorer_FullScreenSizeScreenShot(title, defaultName, paramList)
  ...
  screenShotFromPhotoGallery = true
  ToClient_SetShowPhotoGallery(true)
end
...
function ScreenShotFrame_Show()
  ...
  if true == _ContentsGroup_UsePadSnapping then
    btnIncrease:SetShow(false)
    btnDecrease:SetShow(false)
    btnScreenShot:SetShow(false)
    ScreenShotFramedesc:SetShow(false)
  else
    btnIncrease:SetShow(true)
    btnDecrease:SetShow(true)
    btnScreenShot:SetShow(true)
    ScreenShotFramedesc:SetShow(true)
  end
  btnIncrease:SetShow(true)
  btnDecrease:SetShow(true)
...
function ScreenShotSave_ForWeb_Renew()
  ToClient_ProgressCapture(0, 0, width, height, subNameString)
end
function FromClient_CustomizedCaptureComplete()
  if screenShotFromPhotoGallery == true then
    screenShotFromPhotoGallery = false
...
function ScreenShotFrame_RePos()
  ...
  if true == _ContentsGroup_UsePadSnapping then
    local leftStickX = getPadLeftStickMoveX() * 5
    local leftStickY = getPadLeftStickMoveY() * 5
    Panel_Widget_ScreenShotFrame:SetPosX(Panel_Widget_ScreenShotFrame:GetPosX() + leftStickX)
    Panel_Widget_ScreenShotFrame:SetPosY(Panel_Widget_ScreenShotFrame:GetPosY() - leftStickY)
  else
    local leftX = 0
    local leftY = 0
    if true == isKeyPressed(CppEnums.VirtualKeyCode.KeyCode_W) then
      leftY = 5
    elseif true == isKeyPressed(CppEnums.VirtualKeyCode.KeyCode_S) then
      leftY = -5
    elseif true == isKeyPressed(CppEnums.VirtualKeyCode.KeyCode_A) then
      leftX = -5
    elseif true == isKeyPressed(CppEnums.VirtualKeyCode.KeyCode_D) then
      leftX = 5
    end
    Panel_Widget_ScreenShotFrame:SetPosX(Panel_Widget_ScreenShotFrame:GetPosX() + leftX)
    Panel_Widget_ScreenShotFrame:SetPosY(Panel_Widget_ScreenShotFrame:GetPosY() - leftY)
  end
...
function FGlobal_ScreenShotFrame_IsShow()
  return Panel_Widget_ScreenShotFrame:GetShow()
end
...
registerEvent("FromClient_SaveScreenShotForWeb", "ScreenShotSave_ForWeb_Renew")
