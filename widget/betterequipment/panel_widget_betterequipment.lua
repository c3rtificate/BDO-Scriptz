...
function BetterEquipment:initialize()
  ...
  for index = 1, self._maxEquipCount do
    ...
    self._ui.txt_nameList[index]:SetTextMode(__eTextMode_AutoWrap)
  ...
  if true == _ContentsGroup_UsePadSnapping then
    tempControl:SetText(keyCustom_GetString_ActionPad(CppEnums.ActionInputType.ActionInputType_Interaction))
  else
...
function BetterEquipment:isSpecialWeaponEnable(itemWrapper)
  if nil == itemWrapper then
    return false
  end
  local itemSSW = itemWrapper:getStaticStatus()
  ...
  if ToClient_isHadumChannel() == false or ToClient_IsEnterHadumField() == false then
    return false
  end
  if false == itemSSW:get():isSpecialWeapon() then
    return false
  end
...
function BetterEquipment:checkBetterItem(itemWrapper, slotNo)
  ...
  if false == PaGlobalFunc_Util_CheckSeasonEquip(itemSSW) then
    return false
  end
  ...
  if not isServantEquip and true == isEquip and true == itemWrapper:checkConditions() then
    ...
    if equipSlotNo ~= __eEquipSlotNoRightHand and equipSlotNo ~= __eEquipSlotNoLeftHand and equipSlotNo ~= __eEquipSlotNoAlchemyStone and equipSlotNo ~= __eEquipSlotNoChest and equipSlotNo ~= __eEquipSlotNoGlove and equipSlotNo ~= __eEquipSlotNoBoots and equipSlotNo ~= __eEquipSlotNoHelm and equipSlotNo ~= __eEquipSlotNoNecklace and equipSlotNo ~= __eEquipSlotNoRing1 and equipSlotNo ~= __eEquipSlotNoRing2 and equipSlotNo ~= __eEquipSlotNoEaring1 and equipSlotNo ~= __eEquipSlotNoEaring2 and equipSlotNo ~= __eEquipSlotNoBelt and equipSlotNo ~= __eEquipSlotNoArtifact1 and equipSlotNo ~= __eEquipSlotNoArtifact2 and equipSlotNo ~= __eEquipSlotNoAwakenWeapon then
      return ret
    end
    ...
    if true == self:isSpecialWeaponEnable(itemWrapper) then
      return true
    end
    ...
    if true == _ContentsGroup_NewUI_Inventory_All then
      offencePoint, defencePoint, equipOffencePoint, equipDefencePoint, matchEquip, isAccessory = PaGlobalFunc_Inventory_All_CompareSpec(self._whereType, slotNo, isAccessory)
...
function Input_BetterEquipment_EquipKeyUp()
  local self = BetterEquipment
  if true == _ContentsGroup_UsePadSnapping then
    if true == Panel_Widget_PanelInteraction_Renew:IsShow() or true == Panel_Interaction_House:IsShow() then
      return
    end
    if true == _ContentsGroup_UsePadSnapping then
      ...
      if true == _ContentsGroup_LightStoneBag and PaGlobalFunc_LightStoneBag_IsShow ~= nil and PaGlobalFunc_LightStoneBag_IsShow() == true then
        return
      end
    end
    if _ContentsGroup_AccessoryStepUp == true and Panel_Window_Accessory_StepUp_All ~= nil and Panel_Window_Accessory_StepUp_All:GetShow() == true then
      return
    end
  elseif Panel_Interaction:IsShow() or Panel_Interaction_House:IsShow() then
    return
  end
  ...
  elseif true == PcEnduranceToggle() or PaGlobalFunc_Endurance_InvenSlot_All_GetShow() then
    ...
    if nil ~= tempPanel then
      if invenSlotIcon:GetShow() then
        _panel:SetPosY(invenSlotIcon:GetPosY() + invenSlotIcon:GetSizeY() + 80)
      else
        _panel:SetPosY(tempPanel:GetPosY() + tempPanel:GetSizeY() + 80)
      end
    end
  elseif Panel_Widget_TownSimpleNavi_All ~= nil and Panel_Widget_TownSimpleNavi_All:GetShow() == true then
    tempPanel = Panel_Widget_TownSimpleNavi_All
...
function FromClient_BetterEquipment_GetEquipableItem(itemEnchantKey, slotNo)
  local self = BetterEquipment
  if true == _ContentsGroup_UsePadSnapping and true == _ContentsGroup_Season_EquipmentUpgrade and true == PaGlobal_SeasonEquipment_Upgrade_GetShow() then
    return
  end
  local itemWrapper = getInventoryItemByType(self._whereType, slotNo)
  if nil ~= itemWrapper and itemWrapper:get():getKey():get() == itemEnchantKey and true == self:checkBetterItem(itemWrapper, slotNo) then
    for ii = 1, #self._equipSlotList do
      if self._equipSlotList[ii] == slotNo then
        table.remove(self._equipSlotList, ii)
      end
    end
...
function PaGlobal_BetterEquipment_IsAwakenQuestClear()
  local classType = getSelfPlayer():getClassType()
  if __eClassType_RangerMan == classType then
    return true
  end
