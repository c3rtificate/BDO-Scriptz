...
local TitleTable = {
  ...
  "",
  "",
  PAGetString(Defines.StringSheet_GAME, "ACQUIRE_TITLEMESSAGE_34")
}
local objectMessage = {
...
local function Acquire_SetData(notifyMsg)
  ...
  elseif Acquire_Enum.GetLostItemByPet == arcType then
    local itemEnchantSSW = notifyMsg:getItemEnchantStaticStatusWrapper()
    local arcItemKey = itemEnchantSSW:get()._key:getItemKey()
    if nil ~= itemEnchantSSW then
      arcIconPath = itemEnchantSSW:getIconPath()
  ...
  elseif Acquire_Enum.AcquiredTitle == arcType then
    local acquiredTitleMsg = notifyMsg:getAcquiredTitle()
    local acquiredTitleName = ""
    if acquiredTitleMsg ~= nil then
      acquiredTitleName = acquiredTitleMsg:getName()
    end
    arcObjectMsg = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_ACQUIRE_MESSAGE_GET_TITLE_1", "titleName", acquiredTitleName)
...
function Acquire_ProcessMessage()
  ...
  if not Panel_Acquire:GetShow() and false == Acquire_UI.mainPanel:isPlayAnimation() then
    local notifyMsg = Acquire_getNextData()
    if nil ~= notifyMsg then
      if Panel_RewardSelect_NakMessage:GetShow() == true or Panel_Event_TicketShop_Nak:GetShow() == true then
  ...
  if enableSkill_UpdateData ~= nil and _ContentsGroup_RenewUI_Skill == false then
