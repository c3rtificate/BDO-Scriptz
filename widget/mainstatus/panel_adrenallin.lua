...
if _ContentsGroup_BlackSpiritLock then
  if true == _ContentsGroup_NewUI_BlackSpiritSkillLock_All then
    _transLockButton:addInputEvent("Mouse_LUp", "PaGlobalFunc_BlackSpiritSkillLock_All_Open(0)")
  else
...
function panel_Adrenalin_unRenderInInstance()
  if nil == Panel_Adrenallin then
    _PA_ASSERT_NAME(false, "Panel_Adrenallin\236\157\180 nil \236\158\133\235\139\136\235\139\164.", "\234\176\156\235\176\156\236\158\144")
    return
  end
  ...
  if true == bHide then
    Panel_Adrenallin:SetRenderAndEventHide(true)
  else
    Panel_Adrenallin:SetRenderAndEventHide(false)
  end
end
function UseableBlackSpritSkill()
  ...
  local isUseRage = selfPlayer:isUseableBlackSpritSkill()
...
function Panel_adrenallin_SetAlphaAllChild(alphaValue)
  ...
  _transLockButton:SetAlpha(alphaValue)
  _staticLockButton:SetAlpha(alphaValue)
end
...
function FromClient_UpdateAdrenalin()
  ...
  if true == _ContentsGroup_RenewUI_Main then
    ...
    Panel_Adrenallin:SetShow(getSelfPlayer():isEnableAdrenalin() and not isRecordMode and not PaGlobalFunc_IsRemasterUIOption() and false == ToClient_IsPlayingReplay() and false == ToClient_IsReplayStreamingMode())
  end
  adrenallin_Update()
end
...
registerEvent("FromClient_ApplyUISettingPanelInfo", "Panel_Adrenallin_OnSreenResize")
...
function PaGlobalFunc_Adrenallin_LuaLoadComplete()
  ...
  panel_Adrenalin_unRenderInInstance()
end
function FromClient_StartSequencePlayerControlable_Adrenallin()
  Panel_Adrenallin:SetAlpha(0)
  local aniInfo = UIAni.AlphaAnimation(1, Panel_Adrenallin, 0, 0.8)
  aniInfo:SetDisableWhileAni(true)
end
registerEvent("FromClient_luaLoadComplete", "PaGlobalFunc_Adrenallin_LuaLoadComplete")
registerEvent("FromClient_StartSequencePlayerControlable", "FromClient_StartSequencePlayerControlable_Adrenallin")
function Panel_Adrenallin_SetShow(isShow, isAni, isForce)
