...
function partyWidget:initControl()
  ...
  self._ui._radioButton_Free:SetTextMode(__eTextMode_AutoWrap)
  self._ui._radioButton_Turn:SetTextMode(__eTextMode_AutoWrap)
  self._ui._radioButton_Random:SetTextMode(__eTextMode_AutoWrap)
  self._ui._radioButton_LeaderOnly:SetTextMode(__eTextMode_AutoWrap)
...
function partyWidget:createControl()
  ...
  for index = 1, self._config._maxPartyMemberCount do
    ...
    info._button_Leave:SetTextMode(__eTextMode_AutoWrap)
    info._button_SetLeader:SetTextMode(__eTextMode_AutoWrap)
    info._button_ForceOut:SetTextMode(__eTextMode_AutoWrap)
...
function partyWidget:updatePartyList(isCreate)
  if not self._isInitailized then
    return
  end
  local partyMemberCount = RequestParty_getPartyMemberCount()
  ...
  elseif PaGlobal_RaidParty_GetShow() and CppEnums.PartyType.ePartyType_Large == self._partyType then
    Panel_Widget_Party:SetShow(false)
    PaGlobal_RaidParty_Update()
  elseif false == Panel_Widget_Party:GetShow() and partyMemberCount > 0 and not isCreate then
...
function partyWidget:setClassMpBar(control, class)
  ...
  if 0 == colorBlindMode then
    ...
    control:ChangeTextureInfoName(CppEnums.ClassType_MpBarTexture.path)
...
function partyWidget:registEventHandler()
  ...
  registerEvent("FromClient_ApplyUISettingPanelInfo", "PaGlobalFunc_PartyWidget_SetScreenSize")
