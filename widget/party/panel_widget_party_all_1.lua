function PaGlobal_Party_All:initialize()
  ...
  PaGlobal_Party_All:hidePanelConditionCheck()
...
function PaGlobal_Party_All:registEventHandler()
  ...
  self._ui_pc.chk_dropItem:addInputEvent("Mouse_LUp", "HandleEventLUp_Party_All_SetLootingOption(false)")
  self._ui_pc.btn_Grade:addInputEvent("Mouse_LUp", "HandleEventLUp_Party_All_ShowGradeListToggle()")
  self._ui_pc.btn_changeType:addInputEvent("Mouse_LUp", "HandleEventLUp_Party_All_ChangeType(true)")
  ...
  self._ui_pc.chk_dropItem_Mini:addInputEvent("Mouse_LUp", "HandleEventLUp_Party_All_SetLootingOption(true)")
  self._ui_pc.btn_changeType_Mini:addInputEvent("Mouse_LUp", "HandleEventLUp_Party_All_ChangeType(false)")
  self._ui_pc.radio_free_Mini:addInputEvent("Mouse_LUp", "HandleEventLUp_Party_All_SelectLootingType(" .. CppEnums.PartyLootType.LootType_Free .. ")")
  self._ui_pc.radio_turn_Mini:addInputEvent("Mouse_LUp", "HandleEventLUp_Party_All_SelectLootingType(" .. CppEnums.PartyLootType.LootType_Shuffle .. ")")
  self._ui_pc.radio_rando_Mini:addInputEvent("Mouse_LUp", "HandleEventLUp_Party_All_SelectLootingType(" .. CppEnums.PartyLootType.LootType_Random .. ")")
  for index = 1, self._config.maxPartyCnt do
...
function PaGlobal_Party_All:showRegistItem(isShow)
  ...
  if true == self._ui_pc.stc_optionBG_Mini:GetShow() then
    posY = self._ui.stc_members_Mini[self._partyMemberCount].control:GetPosY() + self._ui_pc.stc_marketOptionBg:GetSizeY() * 0.8
  end
...
function PaGlobal_Party_All:updatePartyList(isCreate)
  ...
  if true == PaGlobalFunc_Widget_Raid_All_GetShow() and CppEnums.PartyType.ePartyType_Large == ToClient_GetPartyType() then
    PaGlobal_Party_All:prepareClose()
    PaGlobalFunc_Widget_Raid_All_Update()
    return
  end
  local partyMemberCount = RequestParty_getPartyMemberCount()
  if false == Panel_Widget_Party_All:GetShow() and partyMemberCount > 0 and not isCreate and false == Panel_Widget_Arsha_Party_All:GetShow() then
    PaGlobal_Party_All:createPartyList()
    PaGlobal_Party_All:open()
    return
  end
  if false == Panel_Widget_Party_All:GetShow() or CppEnums.PartyType.ePartyType_Normal ~= ToClient_GetPartyType() then
    return
  end
  self:resetPartyData()
  self:setPartyMember(partyMemberCount)
  self:setMemberTexture(partyMemberCount, self._ui.stc_members)
  self:setMemberTexture(partyMemberCount, self._ui.stc_members_Mini)
...
function PaGlobal_Party_All:resetPartyData()
  ...
  for index = 1, self._config.maxPartyCnt do
    ...
    self._ui.stc_members_Mini[index].control:SetShow(false)
  end
end
...
function PaGlobal_Party_All:findPartyDataByActorKeyRaw(actorKeyRaw)
...
function PaGlobal_Party_All:setMemberTexture(partyMemberCount, table)
  local isSelfMaster = RequestParty_isLeader()
  for index = 1, partyMemberCount do
    local member = table[index]
    if nil == member then
      return
    end
    ...
    PaGlobal_Party_All:setClassHpBar(member._progress_hp, partyData._class)
...
function PaGlobal_Party_All:setClassIcon(control, class)
  ...
  control:ChangeTextureInfoName(iconTexture.path)
...
function PaGlobal_Party_All:setClassMpBar(control, class)
  ...
  if 0 == colorBlindMode then
    PaGlobalFunc_Util_SetClassMpBar(control, class)
    local mpBarTexture = CppEnums.ClassType_MpBarTexture[class]
    local texturePath = CppEnums.ClassType_MpBarTexture.path
    if true == self._ui_pc.stc_optionBG_Mini:GetShow() then
  ...
  elseif colorBlindMode >= 1 then
    if false == self._ui_pc.stc_optionBG_Mini:GetShow() then
      control:ChangeTextureInfoName("Combine/Progress/Combine_Progress_02.dds")
      local xx1, yy1, xx2, yy2 = setTextureUV_Func(control, 327, 560, 500, 564)
      control:getBaseTexture():setUV(xx1, yy1, xx2, yy2)
      control:setRenderTexture(control:getBaseTexture())
    else
      control:ChangeTextureInfoName("Combine/Progress/Combine_Progress_03.dds")
...
function PaGlobal_Party_All:setClassHpBar(control, class)
  if nil == control then
    return
  end
  local colorBlindMode = ToClient_getGameUIManagerWrapper():getLuaCacheDataListNumber(__eColorBlindMode)
  ...
  elseif false == self._ui_pc.stc_optionBG_Mini:GetShow() then
    control:ChangeTextureInfoName("Combine/Progress/Combine_Progress_02.dds")
...
function PaGlobal_Party_All:selectPartyOption(index)
  ...
  if self._lastSelectUser == index then
    ...
    if partyData._isSelf == true then
      ...
      control_mini._btn_leave:addInputEvent("Mouse_LUp", "HandleEventLUp_Party_All_SelectWithdrawMemeber(" .. partyData._index .. ")")
      control_mini._btn_leave:SetShow(true)
    elseif RequestParty_isLeader() == true then
      ...
      control_mini._btn_forceOut:addInputEvent("Mouse_LUp", "HandleEventLUp_Party_All_SelectBanishMember(" .. partyData._index .. ")")
      control_mini._btn_setLeader:addInputEvent("Mouse_LUp", "HandleEventLUp_Party_All_SelectChangeLeader(" .. partyData._index .. ")")
