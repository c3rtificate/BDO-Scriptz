...
function PaGlobal_MaidList_All:consoleKeyGuideSet(showMarket, showStorage)
  ...
  self._ui_console.stc_guideIconX:SetShow(showStorage)
  local isShowUseMultiMaidClick = _ContentsGroup_UseMultiMaid
  ...
  if true == isShowUseMultiMaidClick then
    Panel_Window_MaidList_All:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandleEventLUp_MaidList_All_UseMultiMaidClick()")
  ...
  local isMaidCheck = ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(__eUseMultiMaid, true, CppEnums.VariableStorageType.eVariableStorageType_User)
  PaGlobal_MaidList_All:keyGuideAlign(isMaidCheck)
end
function PaGlobal_MaidList_All:keyGuideAlign(useMaidChief)
  if false == self._isConsole then
    return
  end
  if true == useMaidChief then
    self._ui_console.stc_guideIconX:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_MAIDLIST_TOGETHERORDER_UNSET"))
  else
    self._ui_console.stc_guideIconX:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_MAIDLIST_TOGETHERORDER_SET"))
  end
  local tempBtnGroup = {
...
function PaGlobal_MaidList_All:registEventHandler()
  ...
  if false == self._isConsole then
    ...
    self._ui.chk_UseMultiMaid:addInputEvent("Mouse_LUp", "HandleEventLUp_MaidList_All_UseMultiMaidClick()")
...
function PaGlobal_MaidList_All:prepareOpen()
  ...
  local isMaidCheck = ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(__eUseMultiMaid, true, CppEnums.VariableStorageType.eVariableStorageType_User)
  if false == self._isConsole then
    self._ui.chk_UseMultiMaid:SetIgnore(false == _ContentsGroup_UseMultiMaid)
    self._ui.chk_UseMultiMaid:SetShow(true == _ContentsGroup_UseMultiMaid)
    self._ui.chk_UseMultiMaid:SetCheck(isMaidCheck)
  else
    PaGlobal_MaidList_All:keyGuideAlign(isMaidCheck)
  end
...
function PaGlobal_MaidList_All:LogInMaidShow()
  if ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_LocalWar) == true then
    return
  end
...
function PaGlobal_MaidList_All:updateData(tab)
  ...
  elseif tab == TAB_TYPE.ALL and nil ~= self._maidData then
    local tempTable = {}
    local index = #tempTable + 1
