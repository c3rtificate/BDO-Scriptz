...
function FriendHouseRank:Initialize()
  ...
  self._tabStartPosX = self._tab[0]:GetPosX()
end
...
function FriendHouseRank:Update()
  ...
  if 0 >= self._listCount then
    ...
    for listIdx = self._startIndex, self._listCount - 1 do
      ...
      list.name:SetShow(true)
      if true == isRequestMansion then
        list.name:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_INTERACTION_FRIENDHOUSELIST_DELETE", "getUserNickname", VisitableHouseWrapper:getUserNickname()))
      else
...
function FGlobal_FriendHouseRank_Open()
  ...
  if not Panel_Interaction_FriendHouseList:GetShow() then
    ...
    for index = 0, 3 do
      if nil ~= self._tab[index] then
        self._tab[index]:SetCheck(false)
      end
    end
    if true == isRequestMansion then
      if nil ~= self._tab[1] then
        self._tab[1]:SetCheck(true)
      end
    elseif nil ~= self._tab[0] then
      self._tab[0]:SetCheck(true)
    end
  end
  if true == isRequestMansion then
    self._tab[0]:SetShow(false)
  else
    self._tab[0]:SetShow(true)
  end
  local tabBtnSizeX = self._tab[0]:GetSizeX()
  local gabX = 5
  local tabCnt = 0
  for index = 0, 3 do
    if true == self._tab[index]:GetShow() then
      self._tab[index]:SetPosX(self._tabStartPosX + tabBtnSizeX * tabCnt + (gabX + tabCnt))
      tabCnt = tabCnt + 1
    end
  end
  self:Update()
