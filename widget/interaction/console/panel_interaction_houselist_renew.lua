...
function Interaction_HouseListInfo:InitControl()
  ...
  self._baseRadioButtonSpanX = self._ui._radioButton_Tab[0]:GetSpanSize().x
end
...
function PaGlobalFunc_Interaction_HouseList_Open()
  ...
  if true == actorWrapper:isMansionLand() then
    self._minTabIndex = 1
    self._ui._radioButton_Tab[self._config._rank]:SetShow(false)
    local radioButtonGapX = 150
    for index = 1, self._config._count - 1 do
      self._ui._radioButton_Tab[index]:SetSpanSize(self._baseRadioButtonSpanX + radioButtonGapX * (index - 1), self._ui._radioButton_Tab[index]:GetSpanSize().y)
    end
    if self._config._rank == self._currentTabIndex then
      self._currentTabIndex = self._config._rank + 1
    end
  else
    self._minTabIndex = 0
    self._ui._radioButton_Tab[self._config._rank]:SetShow(true)
    local radioButtonGapX = 100
    for index = 0, self._config._count - 1 do
      self._ui._radioButton_Tab[index]:SetSpanSize(self._baseRadioButtonSpanX + radioButtonGapX * index, self._ui._radioButton_Tab[index]:GetSpanSize().y)
    end
  end
  self:UpdateTabButton()
  self:UpdateList()
