...
function IngameCustomize_Show()
  ...
  if true == _ContentsGroup_RadioPlayer and nil ~= PaGlobal_SaveRadioPlayerPos then
...
function IngameCustomize_Hide()
  ...
  if true == gameExitPhoto then
    ...
    if false == _ContentsGroup_RenewUI_ExitGame then
      PaGlobal_GameExit_All_UpdateSlot(CharacterSlotIndex + 1)
    end
  end
...
function HandleClicked_CloseIngameCustomizationActual()
  ...
  if true == _ContentsGroup_RadioPlayer and nil ~= PaGlobal_ApplyRadioPlayerPos then
    PaGlobal_ApplyRadioPlayerPos()
  end
end
