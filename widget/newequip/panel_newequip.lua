...
local partsTooltip = UI.createControl(__ePAUIControl_StaticText, Panel_NewEquip, "newEquip_Tooltip")
...
function Panel_NewEquip_ScreenResize()
  if true == _ContentsGroup_UsePadSnapping then
    ...
    Panel_NewEquip:SetPosX(FGlobal_GetPersonalIconPosX(4) - 120)
    return
  end
  Panel_NewEquip:ComputePos()
  local posX, posY
...
local function initialize()
  ...
  if true == _ContentsGroup_UsePadSnapping and Panel_NewEquip:GetSizeX() < widgetTitle:GetTextSizeX() then
...
registerEvent("FromClient_ApplyUISettingPanelInfo", "Panel_NewEquip_ScreenResize")
