...
function PaGlobal_MiniGame_Find_All:updateValue(commercialPercent, RClickCount, isDynamic, emptyCount, subObjCount, trapCount)
  ...
  if _ContentsGroup_LifeEquipmentSlots == true then
    itemWrapper = ToClient_getEquipmentItem(__eEquipSlotNoHoe)
  elseif ToClient_CheckEquipUnionBelt() == false then
    itemWrapper = ToClient_getEquipmentItem(__eEquipSlotNoSubTool)
  else
    itemWrapper = ToClient_GetCollectToolInCollectToolBag(__eCollectToolType_Hoe)
  end
