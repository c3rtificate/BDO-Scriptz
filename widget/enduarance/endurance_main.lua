PaGlobalPlayerEnduranceList = {
  ...
  isEffectSound = false,
  brokenSubToolNo = __eEquipSlotNoCount,
...
function PaGlobalPlayerEnduranceList:initialize()
  for index = 0, __eEquipSlotNoCount - 1 do
    ...
    elseif 2 == index then
      self.enduranceInfo[index].control = UI.getChildControl(self.panel, "Static_SubTool")
    elseif 3 == index then
    ...
    elseif 32 == index then
      self.enduranceInfo[index].control = nil
      local artifactControl = UI.getChildControl(self.panel, "Static_Artifacts1")
      if artifactControl ~= nil then
        artifactControl:SetShow(false)
      end
    elseif 33 == index then
      self.enduranceInfo[index].control = nil
      local artifactControl = UI.getChildControl(self.panel, "Static_Artifacts2")
      if artifactControl ~= nil then
        artifactControl:SetShow(false)
      end
    elseif __eEquipSlotNoAxe == index then
      self.enduranceInfo[index].control = UI.getChildControl(self.panel, "Static_Axe")
    elseif __eEquipSlotNoSyringe == index then
      self.enduranceInfo[index].control = UI.getChildControl(self.panel, "Static_Sap")
    elseif __eEquipSlotNoHoe == index then
      self.enduranceInfo[index].control = UI.getChildControl(self.panel, "Static_Sickle")
    elseif __eEquipSlotNoButcheryKnife == index then
      self.enduranceInfo[index].control = UI.getChildControl(self.panel, "Static_SlaughterKnife")
    elseif __eEquipSlotNoSkinKnife == index then
      self.enduranceInfo[index].control = UI.getChildControl(self.panel, "Static_TanningKnife")
    elseif __eEquipSlotNoPickAx == index then
      self.enduranceInfo[index].control = UI.getChildControl(self.panel, "Static_Pickax")
    elseif __eEquipSlotNoFishingRod == index then
      self.enduranceInfo[index].control = UI.getChildControl(self.panel, "Static_FishingRod")
    elseif __eEquipSlotNoBobber == index then
      self.enduranceInfo[index].control = UI.getChildControl(self.panel, "Static_Fishbobber")
    elseif __eEquipSlotNoFishingHarpoon == index then
      self.enduranceInfo[index].control = UI.getChildControl(self.panel, "Static_Harpoon")
    end
...
function Panel_PlayerEndurance_Position()
  ...
  self.radarSizeX = FGlobal_Panel_Radar_GetSizeX()
  self.panel:SetPosX(getScreenSizeX() - self.radarSizeX - keyboardGuideX - self.panel:GetSizeX() - 15)
  ...
  if Panel_Widget_TownSimpleNavi_All ~= nil and Panel_Widget_TownSimpleNavi_All:GetShow() == true then
    self.panel:SetPosY(Panel_Widget_TownSimpleNavi_All:GetSizeY() + Panel_Widget_TownSimpleNavi_All:GetPosY() + 10)
  end
