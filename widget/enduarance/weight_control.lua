function FGlobal_Inventory_WeightCheck()
  ...
  if sumtotalWeight - decreaseFairyWeight >= 90 then
    ...
    if true == _ContentsGroup_ChinaFontColor then
      self.weight:SetText("<PAColor0xFFF5B327>" .. totalWeight .. "%" .. "<PAOldColor>")
      self.weight:SetColor(Defines.Color.C_FFF5B327)
    else
...
function FGlobal_Inventory_WeightCheck_Reposition()
  ...
  if Panel_Widget_KeyboardGuide ~= nil and Panel_Widget_KeyboardGuide:GetShow() or Panel_HorseEndurance:GetShow() or Panel_CarriageEndurance:GetShow() or Panel_ShipEndurance:GetShow() or Panel_SailShipEndurance:GetShow() then
    self.weight:SetPosY(self.panel:GetSizeY() + 50)
  end
