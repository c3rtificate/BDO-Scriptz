...
local ui = {
  ...
  _button_Z = UI.getChildControl(Panel_Movie_KeyViewer, "StaticText_Btn_Z"),
  _button_X = UI.getChildControl(Panel_Movie_KeyViewer, "StaticText_Btn_X"),
...
local ui_2 = {
  ...
  _button_Z = UI.getChildControl(Panel_Movie_KeyViewer, "StaticText_Btn_Z_2"),
  _button_X = UI.getChildControl(Panel_Movie_KeyViewer, "StaticText_Btn_X_2"),
...
local prevIsShowKeyViewer = false
function Panel_KeyViewer_ScreenRePosition()
  ...
  local relativePosX = ToClient_GetUiInfo(CppEnums.PAGameUIType.PAGameUIPanel_KeyViewer, 0, CppEnums.PanelSaveType.PanelSaveType_RelativePositionX)
  local relativePosY = ToClient_GetUiInfo(CppEnums.PAGameUIType.PAGameUIPanel_KeyViewer, 0, CppEnums.PanelSaveType.PanelSaveType_RelativePositionY)
  ...
  elseif relativePosX == 0 and relativePosY == 0 then
    ...
    Panel_Movie_KeyViewer:SetRelativePosX(relativePosX)
    Panel_Movie_KeyViewer:SetRelativePosY(relativePosY)
    Panel_Movie_KeyViewer:SetPosX(scrX * relativePosX - Panel_Movie_KeyViewer:GetSizeX() / 2)
    Panel_Movie_KeyViewer:SetPosY(scrY * relativePosY - Panel_Movie_KeyViewer:GetSizeY() / 2)
  end
  if 0 < ToClient_GetUiInfo(CppEnums.PAGameUIType.PAGameUIPanel_KeyViewer, 0, CppEnums.PanelSaveType.PanelSaveType_IsSaved) then
    ...
    prevIsShowKeyViewer = Panel_Movie_KeyViewer:GetShow()
  end
...
local forMovieRecord = function()
  ...
  PaGlobalFunc_Panel_TimeBar_Show(false)
...
function PaGlobal_MovieKeyViewer_Init()
  if nil == Panel_Movie_KeyViewer then
    _PA_LOG("\236\161\176\236\158\172\236\167\132", "KeyViewer \237\140\168\235\132\144\236\157\180 \236\151\134\236\138\181\235\139\136\235\139\164 \237\153\149\236\157\184\235\176\148\235\158\141\235\139\136\235\139\164.")
    return
  end
  if true == _ContentsGroup_UsePadSnapping then
    return
  ...
  registerEvent("FromClient_ApplyUISettingPanelInfo", "Panel_KeyViewer_ScreenRePosition")
  Panel_Movie_KeyViewer:RegisterUpdateFunc("Panel_KeyViewer_KeyUpdate")
end
function FromClient_CutsceneKeyGuideShow(posX, posY, keyType, isShow)
  if nil == Panel_Movie_KeyViewer then
    return
  end
  if true == isShow then
    ...
    PaGlobal_MovieKeyViewer_CutsceneKeyAniShow(keyType, true)
  else
    PaGlobal_MovieKeyViewer_CutsceneKeyAniReset()
    ButtonToggleAll(false)
    if true == prevIsShowKeyViewer then
      Panel_KeyViewer_Show()
    else
...
registerEvent("FromClient_CutsceneKeyGuideShow", "FromClient_CutsceneKeyGuideShow")
function PaGlobal_MovieKeyViewer_CutsceneKeyAniShow(key, isOn)
  if nil == Panel_Movie_KeyViewer then
    return
  end
  local keyValueToIndex = {
  ...
  local aUI = ui[keyValueToIndex[key + 1]]
  local aUI2 = ui_2[keyValueToIndex[key + 1]]
  if nil ~= aUI and nil ~= aUI2 then
    aUI:ResetVertexAni()
    aUI2:ResetVertexAni()
    aUI:SetShow(true)
    aUI2:SetShow(true)
...
function PaGlobal_MovieKeyViewer_CutsceneKeyAniReset()
  if nil == Panel_Movie_KeyViewer then
    return
  end
  local keyValueToIndex = {
  ...
  for key, value in pairs(keyValueToIndex) do
    local aUI = ui[key]
    local aUI2 = ui_2[key]
    if nil ~= aUI and nil ~= aUI2 then
      aUI:ResetVertexAni()
