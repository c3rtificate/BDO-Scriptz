...
function FromClient_EventSelfPlayerUsedSkill()
  local autoUIPosY = 0
  local isPlayReplay = ToClient_IsPlayingReplay() == true and ToClient_IsAbusingReplay() == true
  local isSequencePlay = ToClient_isPlayingSequence()
  ...
  if isPlayReplay == false and ToClient_isInAbyssOne() == false and PaGlobalFunc_UiSet_GetSkillLogIsShow ~= nil and PaGlobalFunc_UiSet_GetSkillLogIsShow() == false then
    ...
    if PaGlobal_ArousalTutorial_Manager ~= nil then
  ...
  if nil ~= selfPlayer and isPlayReplay == false and isSequencePlay == false then
    ...
    if nil ~= regionInfo then
      ...
      if true == isSafeZone and false == isLocalWar then
        local skillWrapper = selfPlayerUsedSkillFront()
        if PaGlobal_TutorialManager ~= nil then
          PaGlobal_TutorialManager:handleEventSelfPlayerUsedSkill(skillWrapper)
        end
  ...
  if nil ~= skillWrapper then
    ...
    _displayRunningTime = 0
    index = index + 1
    if SkillCommand_Effect_Cutscene ~= nil then
      SkillCommand_Effect_Cutscene(skillWrapper)
    end
    if SkillCommand_Effect_Sequence ~= nil and isSequencePlay == true then
      SkillCommand_Effect_Sequence(skillWrapper)
    end
  end
end
...
function Panel_Widget_SkillLog_SetPos()
  ...
  if (relativePosX == -1 and relativePosY) == -1 or relativePosX == 0 and relativePosY == 0 then
    ...
    local bias = 0.9
    local bposY = math.floor(scrY * relativePosY - Panel_Widget_SkillLog:GetSizeY() / 2 + bias)
    ...
    Panel_Widget_SkillLog:SetPosY(bposY)
  end
end
