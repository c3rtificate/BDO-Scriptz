function PaGlobal_FarmingTimer:initialize()
  ...
  self._ui.btn_StartShortcut = UI.getChildControl(Panel_Widget_FarmingTimer_All, "Button_Start_Shortcut")
  self:validate()
...
function PaGlobal_FarmingTimer:registEventHandler()
  ...
  if true == _ContentsGroup_FarmingList then
    self._ui.btn_StartShortcut:setButtonShortcutsWithEvent("PaGlobalFunc_SetCheckShortcut()", "PANEL_GUILD_MANUFACTURE_START")
    self._ui.btn_Stop:setButtonShortcuts("PANEL_WINDOW_REPLAYCONTROLLER_STOP")
    self._ui.btn_Reset:setButtonShortcuts("PANEL_UI_SETTING_BTN_RESET")
  end
end
function PaGlobalFunc_SetCheckShortcut()
  local self = PaGlobal_FarmingTimer
  if false == self._ui.chk_Start:IsCheck() then
    self._ui.chk_Start:SetCheck(true)
  else
    self._ui.chk_Start:SetCheck(false)
  end
  PaGloabl_FarmingTimer_StartToggle()
end
