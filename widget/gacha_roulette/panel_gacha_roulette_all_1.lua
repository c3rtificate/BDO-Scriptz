...
function PaGlobal_RandomBoxSelect_All:prepareOpen()
  ...
  local rv = PaGlobal_RandomBoxSelect_All:checkInvenSize()
  if 0 ~= rv then
    if rv == -1 then
      Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrNoNormalInventoryNotExistFreeSlot"))
    elseif rv == -2 then
      Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrNoCashInventoryNotExistFreeSlot"))
    end
