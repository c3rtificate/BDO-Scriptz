...
local function MiniGame_Manual_Timing_0(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Timing_0 == true then
    ...
    _ui.txt_Purpose:SetTextMode(__eTextMode_AutoWrap)
...
local function MiniGame_Manual_Timing_1(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Timing_1 == true then
    ...
    if _ContentsGroup_isConsolePadControl then
      ...
      _panel:SetShow(true)
      _ui.txt_HorseInfo:SetTextMode(__eTextMode_AutoWrap)
...
local function MiniGame_Manual_Cart_Hallowin(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Cart_0 == true then
    ...
    UIAni.AlphaAnimation(1, _panel, 0, 0.22)
    if _ContentsGroup_isConsolePadControl then
      _ui.txt_Purpose:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_HALLOWEEN_CART_TITLE"))
    else
      _ui.txt_Purpose:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_HALLOWEEN_CART_TITLE"))
    end
    _ui.txt_Purpose:SetShow(true)
    _ui.txt_Purpose:AddEffect("UI_QustComplete01", false, 0, 0)
    ui_Value.isFirstTime_Manual_Cart_0 = false
...
local function MiniGame_Manual_HorseDrop_Hallowin(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_HorseDrop_0 == true then
    ...
    _panel:SetAlpha(0)
    UIAni.AlphaAnimation(1, _panel, 0, 0.22)
    _ui.txt_Purpose:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_HALLOWEEN_HORSEDROP_0"))
    _ui.txt_Purpose:SetShow(true)
    _ui.txt_Purpose:AddEffect("UI_QustComplete01", false, 0, 0)
    ui_Value.isFirstTime_Manual_HorseDrop_0 = false
    GlobalValue_MiniGame_Value_HorseDrop = true
...
function FGlobal_BulletCheck()
  ...
  if nil == itemWrapper then
    return PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_RELOADALERT")
  else
...
local ConsoleKeyGuide_CattleGate = function()
  PaGlobalFunc_ConsoleKeyGuide_SetActionKeyGuide(Defines.ConsoleKeyGuideType.cattlegate)
end
...
local function Panel_Global_Manual_End(actorKeyRaw, isSelf)
  ...
  elseif Panel_PowerGauge:GetShow() then
...
ActionChartEventBindFunction(415, ConsoleKeyGuide_CattleGate)
