...
local function MiniGame_Manual_Timing_0(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Timing_0 == true then
    ...
    if true == _ContentsGroup_UsePadSnapping then
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_GLOBALMANUAL_CONSOLE_TIMING_0"))
    else
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_GLOBALMANUAL_TIMING_0"))
    end
    ui._horse_Icon_Title:SetShow(true)
    ui._horseCancel:SetText(PAGetString(Defines.StringSheet_RESOURCE, "PANEL_GLOBALMANUAL_DROPROPE"))
...
local function MiniGame_Manual_Timing_1(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Timing_1 == true then
    ...
    ui._purposeText:AddEffect("UI_QustComplete01", false, 0, 0)
    if true == _ContentsGroup_UsePadSnapping then
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_GLOBALMANUAL_CONSOLE_TIMING_1"))
    else
...
local function MiniGame_Manual_Timing_2(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Timing_2 == true then
    ...
    if false == _ContentsGroup_UsePadSnapping then
      ...
      PaGlobal_Global_Manual_KeyTextUpdate()
    end
...
local function MiniGame_Manual_Timing_3(actorKeyRaw, isSelf)
  ...
  uiPress._button_Space:AddEffect("fUI_Horse_Space_02A", false, 0, 0)
  UIAni.AlphaAnimation(1, uiPress._button_Space, 0.5, 0.75)
end
local MiniGame_Manual_TamingHorse_Sugar_Comment = function(actorKeyRaw, isSelf)
  local sugarString = {
    [0] = PAGetString(Defines.StringSheet_GAME, "LUA_HORSE_TAMING_SUGAR_MENT0"),
    PAGetString(Defines.StringSheet_GAME, "LUA_HORSE_TAMING_SUGAR_MENT1"),
    PAGetString(Defines.StringSheet_GAME, "LUA_HORSE_TAMING_SUGAR_MENT2"),
    PAGetString(Defines.StringSheet_GAME, "LUA_HORSE_TAMING_SUGAR_MENT3")
  }
  local getSugarString = getRandomValue(0, 3)
  PaGlobal_GlobalNak_Open(CppEnums.EChatNoticeType.MasterlessHorse, 0, sugarString[getSugarString])
end
local function MiniGame_Manual_Cart_0(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Cart_0 == true then
    ...
    if true == _ContentsGroup_UsePadSnapping then
      ui._purposeText2:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_MINIGAME_CART_TITLE"))
    else
...
local isCartHallowin = false
local function MiniGame_Manual_Cart_Hallowin(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Cart_0 == true then
    for _, v in pairs(ui) do
      v:SetShow(false)
      v:ComputePos()
    ...
    ui._purposeText2:AddEffect("UI_QustComplete01", false, 0, 0)
    if true == _ContentsGroup_UsePadSnapping then
      ui._purposeText2:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_HALLOWEEN_CART_TITLE"))
    else
      ui._purposeText2:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_HALLOWEEN_CART_TITLE"))
    end
    ui._cart_Icon_Title:SetShow(true)
    if false == _ContentsGroup_UsePadSnapping then
      uiPress._m0:SetShow(true)
...
function FGlobal_isPlaying_CartHallowin()
  return isCartHallowin
end
...
local function MiniGame_Manual_HorseDrop_0(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_HorseDrop_0 == true then
    ...
    ui._purposeText:AddEffect("UI_QustComplete01", false, 0, 0)
    if true == _ContentsGroup_UsePadSnapping then
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_GLOBALMANUAL_HORSEDROP_0"))
    else
...
local isDropHallowin = false
local function MiniGame_Manual_HorseDrop_Hallowin(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_HorseDrop_0 == true then
    for _, v in pairs(ui) do
      v:SetShow(false)
      v:ComputePos()
    ...
    ui._purposeText:AddEffect("UI_QustComplete01", false, 0, 0)
    if true == _ContentsGroup_UsePadSnapping then
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_HALLOWEEN_HORSEDROP_0"))
    else
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_HALLOWEEN_HORSEDROP_0"))
    end
    ui._horse_Icon_Title:SetShow(true)
    ui_Value.isFirstTime_Manual_HorseDrop_0 = false
    GlobalValue_MiniGame_Value_HorseDrop = true
...
function FGlobal_isPlaying_DropHallowin()
  return isDropHallowin
end
local function MiniGame_Manual_Flute_0(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Flute_0 == true and not IsHideMiniGameManual() then
    ...
    ui._purposeText:AddEffect("UI_QustComplete01", false, 0, 0)
    if true == _ContentsGroup_UsePadSnapping then
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_GLOBALMANUAL_RHYTHM_0"))
    else
...
local function MiniGame_Manual_Flute_1(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Flute_1 == true then
    ...
    ui._purposeText:AddEffect("UI_QustComplete01", false, 0, 0)
    if true == _ContentsGroup_UsePadSnapping then
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_GLOBALMANUAL_RHYTHM_1"))
    else
...
local function MiniGame_Manual_Drum_0(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Drum_0 == true and not IsHideMiniGameManual() then
    ...
    ui._purposeText:AddEffect("UI_QustComplete01", false, 0, 0)
    if true == _ContentsGroup_UsePadSnapping then
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_GLOBALMANUAL_DRUM_0"))
    else
...
local function MiniGame_Manual_Drum_1(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Drum_1 == true then
    ...
    ui._purposeText:AddEffect("UI_QustComplete01", false, 0, 0)
    if true == _ContentsGroup_UsePadSnapping then
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_GLOBALMANUAL_DRUM_1"))
    else
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_GLOBALMANUAL_RHYTHM_1"))
    end
    ui._flute_Icon_Title:SetShow(true)
...
local function MiniGame_Manual_Instrument_0(actorKeyRaw, isSelf)
  ...
  if not IsHideMiniGameManual() then
    ...
    ui._purposeText:SetShow(true)
    if true == _ContentsGroup_UsePadSnapping then
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_GLOBALMANUAL_RHYTHM_2"))
    else
...
local function MiniGame_Manual_HerbMachine_0(actorKeyRaw, isSelf)
  ...
  PaGlobal_MiniGame_HerbMachine_All_SetGameEndCount(5)
...
local MiniGame_Manual_HerbMachine = function()
  PaGlobal_MiniGame_HerbMachine_All_ResetCount()
end
local function MiniGame_Manual_Hammer_0(actorKeyRaw, isSelf)
  ui_Value.isFirstTime_Manual_HerbMachine_0 = true
  PaGlobal_MiniGame_HerbMachine_All_SetGameEndCount(1)
  if ui_Value.isFirstTime_Manual_HerbMachine_0 == true then
    for _, v in pairs(ui) do
      v:SetShow(false)
      v:ComputePos()
...
local function MiniGame_Manual_Buoy_1(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Buoy_1 == true then
    ...
    PaGlobal_MiniGame_Buoy_All_WaitInitMode()
  end
end
...
local MiniGame_Manual_Reset_Buoy = function()
  PaGlobal_MiniGame_Buoy_All_ResetCount()
end
function PaGlobalFunc_PressSpace_SetShow(isShow)
  uiPress._button_Space:SetShow(isShow)
end
local function MiniGame_Manual_Milky_0(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Milky_0 == true then
    ...
    Panel_Global_Manual:SetShow(true)
    Panel_Global_Manual:SetAlpha(0)
    PaGlobal_PowerControl_All_GameOpen()
    UIAni.AlphaAnimation(1, Panel_Global_Manual, 0, 0.22)
    ui._purposeText:SetShow(true)
    ui._purposeText:AddEffect("UI_QustComplete01", false, 0, 0)
    if true == _ContentsGroup_UsePadSnapping then
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_MINIGAME_MILKCOW_READY1"))
    else
...
local function MiniGame_Manual_Milky_1(actorKeyRaw, isSelf)
  if ui_Value.isFirstTime_Manual_Milky_1 == true then
    ...
    ui._purposeText:AddEffect("UI_QustComplete01", false, 0, 0)
    if true == _ContentsGroup_UsePadSnapping then
      ui._purposeText:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_XBOX1_MINIGAME_MILKCOW_READY2"))
    else
...
function Parking_PLZ_Update(variableName, value)
  if variableName == "IsParking" and value == 0 then
    ...
    elseif getSelfPlayer():getRegionInfoWrapper():get():isSafeZone() == false then
...
local KeyGuide_InWater = function()
  ...
  if PaGlobal_SkillCommand_CheckRegion ~= nil then
...
local KeyGuide_Seat = function(actorKeyRaw, isSelf)
  if nil == getSelfPlayer() then
    return
  end
  local targetPanel = getSelfPlayer():get():getUIPanel()
...
local KeyGuide_SnowBoard = function(actorKeyRaw, isSelf)
  if nil == getSelfPlayer() then
    return
  end
  local targetPanel = getSelfPlayer():get():getUIPanel()
  ...
  PaGlobal_CommandGuide_Show(true, 6)
end
local function Panel_Global_Manual_End(actorKeyRaw, isSelf)
  if false == isSelf then
    return
  end
  if true == _ContentsGroup_NewUI_Fishing_All and PaGlobal_Fishing_All_FishingEnd ~= nil then
    PaGlobal_Fishing_All_FishingEnd()
  elseif Panel_Fishing_End ~= nil then
    Panel_Fishing_End()
  end
  HideUseTab_Func()
  if true == _ContentsGroup_NewUI_MiniGame_Sniper_All then
    if PaGlobalFunc_SniperGame_All_EndSniperGame ~= nil then
      PaGlobalFunc_SniperGame_All_EndSniperGame()
    end
    if PaGlobal_MiniGame_SniperReload_All_Close ~= nil then
      PaGlobal_MiniGame_SniperReload_All_Close()
    end
  else
    if PaGlobalFunc_SniperGame_EndSniperGame ~= nil then
      PaGlobalFunc_SniperGame_EndSniperGame()
    end
    if PaGlobalFunc_SniperReload_Close ~= nil then
      PaGlobalFunc_SniperReload_Close()
    end
  end
  uiPress._button_Space:ComputePos()
  Panel_Global_Manual:SetShow(false)
  if PaGlobal_PowerControl_All_GameEnd ~= nil then
  ...
  if _ContentsGroup_NewUI_Fishing_All == true then
    if PaGlobal_PowerGauge_All_Close ~= nil then
      PaGlobal_PowerGauge_All_Close()
    end
  elseif Panel_PowerGauge ~= nil and Panel_PowerGauge:GetShow() == true then
    FGlobal_PowerGauge_Close()
  end
  if Panel_CannonGauge ~= nil and Panel_CannonGauge:GetShow() == true then
    FGlobal_CannonGauge_Close()
  end
  if Panel_CommandGuide ~= nil and Panel_CommandGuide:GetShow() == true then
  ...
  isCartRace = false
  isCartHallowin = false
  isDropHallowin = false
end
local function MiniGame_Manual_Instrument_1(actorKeyRaw, isSelf)
  Panel_Global_Manual_End(actorKeyRaw, isSelf)
end
function PaGlobal_Global_Manual_KeyTextUpdate()
  uiPress._button_W:SetText(keyCustom_GetString_ActionKey(CppEnums.ActionInputType.ActionInputType_MoveFront))
  ui._button_W:SetText(keyCustom_GetString_ActionKey(CppEnums.ActionInputType.ActionInputType_MoveFront))
  uiPress._button_A:SetText(keyCustom_GetString_ActionKey(CppEnums.ActionInputType.ActionInputType_MoveLeft))
  ui._button_A:SetText(keyCustom_GetString_ActionKey(CppEnums.ActionInputType.ActionInputType_MoveLeft))
  uiPress._button_S:SetText(keyCustom_GetString_ActionKey(CppEnums.ActionInputType.ActionInputType_MoveBack))
  ui._button_S:SetText(keyCustom_GetString_ActionKey(CppEnums.ActionInputType.ActionInputType_MoveBack))
  uiPress._button_D:SetText(keyCustom_GetString_ActionKey(CppEnums.ActionInputType.ActionInputType_MoveRight))
...
ActionChartEventBindFunction(205, MiniGame_Manual_TamingHorse_Sugar_Comment)
ActionChartEventBindFunction(221, MiniGame_Manual_Cart_0)
ActionChartEventBindFunction(222, MiniGame_Manual_Cart_Race)
ActionChartEventBindFunction(223, MiniGame_Manual_Cart_Hallowin)
...
ActionChartEventBindFunction(233, MiniGame_Manual_HorseDrop_Hallowin)
...
ActionChartEventBindFunction(417, KeyGuide_Seat)
ActionChartEventBindFunction(418, KeyGuide_SnowBoard)
...
function TestAction_RiderSkill()
  ToClient_testAction("Rider_F_Start")
end
