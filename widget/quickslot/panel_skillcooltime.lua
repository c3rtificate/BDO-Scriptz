...
local skillCooltime = {
  ...
  _stc_Wifi = UI.getChildControl(Panel_SkillCooltime, "StaticText_1")
}
...
function SkillCooltime_OnResize()
  ...
  elseif relativePosX == 0 and relativePosY == 0 then
    ...
    local bias = 0.95
    local bposY = math.floor(sizeY * relativePosY - Panel_SkillCooltime:GetSizeY() / 2 + bias)
    ...
    Panel_SkillCooltime:SetPosY(bposY)
  end
...
function skillCooltime:registMessageHandler()
  ...
  registerEvent("FromClient_StartSequencePlayerControlable", "FromClient_StartSequencePlayerControlable_SkillCooltime")
end
function FromClient_StartSequencePlayerControlable_SkillCooltime()
  Panel_SkillCooltime:SetAlpha(0)
  local aniInfo = UIAni.AlphaAnimation(1, Panel_SkillCooltime, 0, 0.8)
...
function FromClient_QuickSlotWifi(value)
  if skillCooltime == nil then
    return
  end
  if value < 0 then
    skillCooltime._stc_Wifi:SetShow(false)
    return
  end
  if skillCooltime._stc_Wifi:GetShow() == false then
    skillCooltime._stc_Wifi:SetShow(true)
  end
  skillCooltime._stc_Wifi:SetText(value .. "ms")
end
registerEvent("FromClient_RenderModeChangeState", "SkillCooltime_OnResize")
registerEvent("FromClient_QuickSlotWifi", "FromClient_QuickSlotWifi")
