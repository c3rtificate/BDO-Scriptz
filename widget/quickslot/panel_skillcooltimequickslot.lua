...
function PaGlobal_SkillCoolTimeQuickSlot:init()
  ...
  for panelIdx = 0, self._config.maxPanelCount - 1 do
    ...
    local panel = UI.createPanelAndSetPanelRenderMode("Panel_SkillCoolTimeQuickSlot_" .. tostring(panelIdx), Defines.UIGroup.PAGameUIGroup_MainUI, PAUIRenderModeBitSet({
    ...
    slot.SlotBG:SetIgnore(true)
    slot.memo = UI.createAndCopyBasePropertyControl(Panel_SkillCoolTimeQuickSlot, "StaticText_Memo", slot.Panel, "SkillCoolTimeQuickSlot_" .. panelIdx .. "_SlotMemo")
  ...
  for panelIdx = 0, self._config.maxPanelCount - 1 do
    ...
    if 0 == skillNo then
      ...
      if _ContentsGroup_SkillCoolTimeMemo == true then
        ...
        slot.memo:SetTextMode(__eTextMode_AutoWrap)
...
function FromClient_StartSequencePlayerControlable_SkillCoolTimeQuickSlot()
  for panelIdx = 0, PaGlobal_SkillCoolTimeQuickSlot._config.maxPanelCount - 1 do
    local panel = PaGlobal_SkillCoolTimeQuickSlot._panelPool[panelIdx]
    panel.Panel:SetAlpha(0)
    local aniInfo = UIAni.AlphaAnimation(1, panel.Panel, 0, 0.8)
...
registerEvent("FromClient_StartSequencePlayerControlable", "FromClient_StartSequencePlayerControlable_SkillCoolTimeQuickSlot")
