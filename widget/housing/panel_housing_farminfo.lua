...
function farmInfo:init()
  ...
  if true == _ContentsOption_CH_Help then
    self._ui.button_Question:SetShow(false)
  end
...
function FarmInfo_Change_Texture(isChange)
  ...
  if not isGameTypeKorea() then
    self.staticText_CropsIcon_PruningDesc:SetTextMode(__eTextMode_Limit_AutoWrap)
    self.staticText_CropsIcon_InsectDamegeDesc:SetTextMode(__eTextMode_Limit_AutoWrap)
