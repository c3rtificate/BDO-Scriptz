...
function PaGlobal_ResidenceList:registEventHandler()
  ...
  PaGlobal_ResidenceList._ui._list2_residence:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobal_ResidenceList_addResidenceListContent")
  PaGlobal_ResidenceList._ui._list2_residence:createChildContent(__ePAUIList2ElementManagerType_List)
end
...
function PaGlobal_ResidenceList:update()
  ...
  local count = ToClient_getMyDwellingCount() + 2
  for index = 0, count do
    local dwellingSSW = ToClient_getMyDwelling(index)
    if nil ~= dwellingSSW and nil ~= dwellingSSW:getName() and nil ~= dwellingSSW:getObjectStaticStatus() and false == dwellingSSW:getObjectStaticStatus():isMansionLand() then
  ...
  if nil ~= guildHouseSSW and nil ~= guildHouseSSW:getName() then
    PaGlobal_ResidenceList._totalResidenceCount = PaGlobal_ResidenceList._totalResidenceCount + 1
    local residenceData = {
      _residenceIndex = PaGlobal_ResidenceList._totalResidenceCount,
