...
function Panel_Widget_HousingName_info:registerMessageHandler()
  ...
  registerEvent("FromClient_ChangeSwimmingSuitShowMode", "FromClient_HousingName_ChangeUnderwearModeInHouse")
...
function Panel_Widget_HousingName_info:initValue(isPetAndMadeInit)
  if true == isPetAndMadeInit then
...
function PaGlobalFunc_HousingName_ToggleHideWear()
  ...
  local underWear = selfPlayer:get():getSwimmingSuitMode() or selfPlayer:get():getUnderwearModeInhouse()
  Panel_Widget_HousingName_info:setUnderWear(underWear)
end
...
function PaGlobalFunc_HousingName_EventHousingShowVisitHouse(isShow, houseName, userNickname, point, isMine)
  ...
  local isShowUnderwear = selfPlayer:get():getSwimmingSuitMode() or selfPlayer:get():getUnderwearModeInhouse()
  ...
  if false == self._value.isMyHouse and (true == ToClient_isPS() or true == isVisitHousePS or true == isMansionLand) then
    self._ui.static_Dpad_Up:SetShow(false)
    self._ui.static_InstallMode:SetShow(false)
    Panel_HouseName:registerPadEvent(__eConsoleUIPadEvent_Up_DpadUp, "")
    self._ui.static_Dpad_Left:SetSpanSize(self._value.changeIconSpanX, self._ui.static_Dpad_Left:GetSpanSize().y)
    self._ui.staticText_UnderWear:SetSpanSize(self._value.changeIconSpanX, self._ui.staticText_UnderWear:GetSpanSize().y)
    self._ui.static_DpadDown:SetSpanSize(self._value.changeIconSpanX + self._value.baseSpanSizeX, self._ui.static_DpadDown:GetSpanSize().y)
    self._ui.static_Inventory:SetSpanSize(self._value.changeIconSpanX + self._value.baseSpanSizeX, self._ui.static_Inventory:GetSpanSize().y)
    self._ui.static_DpadRight:SetSpanSize(self._value.changeIconSpanX + self._value.baseSpanSizeX * 2, self._ui.static_DpadRight:GetSpanSize().y)
    self._ui.staticText_PetAndMade:SetSpanSize(self._value.changeIconSpanX + self._value.baseSpanSizeX * 2, self._ui.staticText_PetAndMade:GetSpanSize().y)
  else
    self._ui.static_Dpad_Up:SetShow(true)
    self._ui.static_InstallMode:SetShow(true)
    self._ui.static_Dpad_Left:SetSpanSize(self._value.baseIconSpanX, self._ui.static_Dpad_Left:GetSpanSize().y)
    self._ui.staticText_UnderWear:SetSpanSize(self._value.baseIconSpanX, self._ui.staticText_UnderWear:GetSpanSize().y)
    self._ui.static_DpadDown:SetSpanSize(self._value.baseIconSpanX + self._value.baseSpanSizeX, self._ui.static_DpadDown:GetSpanSize().y)
    self._ui.static_Inventory:SetSpanSize(self._value.baseIconSpanX + self._value.baseSpanSizeX, self._ui.static_Inventory:GetSpanSize().y)
    self._ui.static_DpadRight:SetSpanSize(self._value.baseIconSpanX + self._value.baseSpanSizeX * 2, self._ui.static_DpadRight:GetSpanSize().y)
    self._ui.staticText_PetAndMade:SetSpanSize(self._value.baseIconSpanX + self._value.baseSpanSizeX * 2, self._ui.staticText_PetAndMade:GetSpanSize().y)
  end
  if true == isMansionLand then
    self._ui.staticText_Host_Title:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_HOUSING_HOUSENAME_BIGHOUSE", "user_nickname", userNickname))
  else
  ...
  self._ui.staticText_Adress:SetShow(true)
  self._ui.staticText_HousingPoint:SetShow(true)
...
function PaGlobalFunc_HousingName_isMyHouse()
  return Panel_Widget_HousingName_info._value.isMyHouse
end
...
function PaGlobalFunc_HousingName_InputModeChange(prevMode, currentMode)
  ...
  local isMansionLand = false
  local houseWrapper = housing_getHouseholdActor_CurrentPosition()
  if nil ~= houseWrapper then
    isMansionLand = houseWrapper:getStaticStatusWrapper():getObjectStaticStatus():isMansionLand()
  end
  if false == self._value.isMyHouse and (true == ToClient_isPS() or true == isVisitHousePS or true == isMansionLand) then
    self._ui.static_Dpad_Up:SetShow(false)
    self._ui.static_InstallMode:SetShow(false)
