...
function housing:Init()
  ...
  for ii = 0, housing._maxInstallationSlotCount - 1 do
    local newSlot = {}
    newSlot._staticBackSlot = UI.createControl(__ePAUIControl_Static, housing._staticBackInstallations, "Static_Installation_SlotBack_" .. ii)
    ...
    newSlot._staticImgSlot = UI.createControl(__ePAUIControl_Static, newSlot._staticBackSlot, "Static_Installation_SlotImg_" .. ii)
...
function housing:ShowInstallationMenu(isShow, posX, posY, isHarvest, isShowMove, isShowFix, isShowDelete, isShowCancel)
  ...
  if true == _ContentsGroup_NewUI_InstallMode_All then
    if true == isHarvest then
      PaGlobal_FarmInfo_All_InstallModeOpen()
    else
...
function Panel_Housing_CancelModeFromKeyBinder()
  ...
  if true == _ContentsGroup_UsePadSnapping and false == _ContentsGroup_RenewUI then
    ToClient_setMousePosition(100, 100)
  end
end
...
function Panel_Housing_Update_ShowingInstallationList_Sub_2()
  ...
  for ii = 0, loop do
    ...
    if nil ~= installationActorWrapper then
      ...
      if isAdd and (isAll or isFloor or isWall or isTable) then
        local tempItem = {}
        tempItem._slotNo = __eTInventorySlotNoUndefined
...
function FromClient_ChangeCurrentHousehold()
  if true == _ContentsGroup_NewUI_InstallMode_All then
    if false == PaGlobal_House_Installation_All_GetPanelShow() then
      return
    end
  elseif false == Panel_House_InstallationMode:IsShow() then
    return
  end
  Event_Housing_ShowHousingModeUI(false)
  housing_changeHousingMode(false)
