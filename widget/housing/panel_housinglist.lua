...
function Panel_HousingList_Update()
  ...
  if _myDwellingCount > 0 then
    local dwellingCount = _myDwellingCount + 2
    local diffCount = 0
    for idx = 0, dwellingCount - 1 do
      local characterStaticStatusWrapper = ToClient_getMyDwelling(idx + diffCount)
      if nil ~= characterStaticStatusWrapper and nil ~= characterStaticStatusWrapper:getObjectStaticStatus() then
        if true == characterStaticStatusWrapper:getObjectStaticStatus():isMansionLand() then
          diffCount = diffCount + 1
        elseif characterStaticStatusWrapper:getName() ~= nil then
          local slotIndex = idx - diffCount
          ...
          self._housePos[slotIndex] = housePos
          local regionWrapper = ToClient_getRegionInfoWrapperByPosition(housePos)
          if slotIndex ~= 0 then
            _PosY = self._Territory:GetSizeY() + 7 + _PosY
          end
          self.listArray[slotIndex]._Territory:SetText(regionWrapper:getTerritoryName())
          self.listArray[slotIndex]._Territory:SetPosX(50)
          self.listArray[slotIndex]._Territory:SetPosY(_PosY)
          self.listArray[slotIndex]._Territory:SetShow(true)
          self.listArray[slotIndex]._TownName:SetText(regionWrapper:getAreaName())
          self.listArray[slotIndex]._TownName:SetPosX(180)
          self.listArray[slotIndex]._TownName:SetPosY(_PosY)
          self.listArray[slotIndex]._TownName:SetShow(true)
          self.listArray[slotIndex]._Address:SetText(characterStaticStatusWrapper:getName())
          self.listArray[slotIndex]._Address:SetPosX(330)
          self.listArray[slotIndex]._Address:SetPosY(_PosY)
          self.listArray[slotIndex]._Address:SetShow(true)
          self.listArray[slotIndex]._Navi:SetPosX(615)
          self.listArray[slotIndex]._Navi:SetPosY(_PosY + 2)
          self.listArray[slotIndex]._Navi:SetShow(true)
          self.listArray[slotIndex]._Navi:addInputEvent("Mouse_LUp", "_HousingListNavigatorStart(" .. slotIndex .. "," .. dwellingCount .. ")")
        end
      end
    end
  end
