function PaGlobal_HouseName_All:initialize()
  ...
  self._ui._chk_hideDurability = UI.getChildControl(self._ui._pc._stc_btnBg, "CheckButton_ToggleHideDurability")
  self._ui._stc_hideDurabilityHideIcon = UI.getChildControl(self._ui._chk_hideDurability, "Static_Icon_HideButton")
  self._ui._stc_hideDurabilityShowIcon = UI.getChildControl(self._ui._chk_hideDurability, "Static_Icon_ShowButton")
  ...
  if true == _ContentsGroup_ChinaFontColor then
    self._ui._txt_noticeLighting:SetFontColor(Defines.Color.C_FFFFAD38)
  end
  ...
  self._baseConsoleInventoryIconSpanX = self._ui._console._stc_inventory:GetSpanSize().x
  self._originPetIconSpanX = self._ui._chk_hidePet:GetSpanSize().x
  self._originDurabilityIconSpanX = self._ui._chk_hideDurability:GetSpanSize().x
  ...
  if nil ~= ToClient_callEventHousingShowVisitHouse then
    ToClient_callEventHousingShowVisitHouse()
  end
  local isHideDurability = ToClient_getGameUIManagerWrapper():getLuaCacheDataListBool(__eHideHousingDurability)
  self._ui._chk_hideDurability:SetCheck(isHideDurability)
  HandleEventLUp_HouseName_All_SetDurabilityHideToggle()
end
function PaGlobal_HouseName_All:registEventHandler()
  ...
  self._ui._chk_hideDurability:addInputEvent("Mouse_LUp", "HandleEventLUp_HouseName_All_SetDurabilityHideToggle()")
  self._ui._chk_hideDurability:addInputEvent("Mouse_On", "HandleEventOn_HouseName_All_ButtonTooltip(" .. self._eButtonType.durability .. ")")
  self._ui._chk_hideDurability:addInputEvent("Mouse_Out", "TooltipSimple_Hide()")
  ...
  registerEvent("FromClient_responseRepairToInstallation", "FromClient_responseRepairToInstallation")
  registerEvent("FromClient_UpdateUIOnce_NotReloadUI", "FromClient_UpdateUIOnce_NotReloadUI_CloseVisitHouse")
end
function FromClient_UpdateUIOnce_NotReloadUI_CloseVisitHouse()
  FromClient_HouseName_All_EventHousingShowVisitHouse(false, "", "", "", "")
end
