...
function GameTipsInit()
  ...
  if true == _ContentsOption_CH_WepHelperControl then
    gameTipsWindow.helpBTN:SetShow(false)
    gameTipsWindow.closeBTN:SetSpanSize(0, gameTipsWindow.closeBTN:GetSpanSize().y)
    gameTipsWindow.closeBTN:ComputePos()
  end
end
function GameTipsListControlCreate(content, key)
  ...
  tipsDesc:SetTextMode(__eTextMode_AutoWrap)
...
function FGlobal_GameTips_GetShow()
  if Panel_Window_GameTips == nil then
    return false
  end
  return Panel_Window_GameTips:GetShow()
end
function gameTipsWindow:registEventHandler()
  ...
  self.list2:registEvent(__ePAUIList2EventType_LuaChangeContent, "GameTipsListControlCreate")
  self.list2:createChildContent(__ePAUIList2ElementManagerType_List)
end
