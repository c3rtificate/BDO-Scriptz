function HandleEventLUp_Fishing_All_SelectGrade(grade)
  ...
  if grade >= PaGlobal_Fishing_All._eGradeList._count or grade < PaGlobal_Fishing_All._eGradeList._none then
    grade = PaGlobal_Fishing_All._eGradeList._none
  end
...
function HandleEventOn_Fishing_All_ShowGradeTooltip(grade)
  local name = PaGlobal_Fishing_All._gradeString[grade].name
  local desc = PaGlobal_Fishing_All._gradeString[grade].desc
...
function HandleEventLUp_Fishing_All_NaviEventArea()
  local position = ToClient_GetNearEventNavigationPosition(Defines.EventNaviType.EVENT_FISHING)
  if position == float3(0, 0, 0) then
    return
  end
  PaGlobal_Fishing_All:hideEventAreaIcon()
  PaGlobal_Fishing_All._checkEventAreaIcon = true
  ToClient_WorldMapNaviStart(position, NavigationGuideParam(), true, true)
  local message = PAGetString(Defines.StringSheet_GAME, "LUA_FISHINGEVENT_SYSTEMMSG")
...
function PaGlobal_Fishing_All_ShowEventAreaIcon(eventNaviType)
  PaGlobal_Fishing_All:showEventAreaIcon(eventNaviType)
end
...
function PaGlobal_Fishing_All_UpdatePerFrameFunc(deltaTime)
  ...
  if true == PaGlobal_Fishing_All._isSpreading then
    if isPadUp(__eJoyPadInputType_X) then
      local nextGrade = PaGlobal_Fishing_All._currentSelectGrade + 1
      HandleEventLUp_Fishing_All_SelectGrade(nextGrade)
