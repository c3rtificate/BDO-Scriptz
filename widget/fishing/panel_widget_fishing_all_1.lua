function PaGlobal_Fishing_All:initialize()
  ...
  self._ui._stc_naviEventAreaIcon = UI.getChildControl(Panel_Widget_Fishing_All, "Static_NaviEventAreaIcon")
  self._ui._txt_naviEventAreaText = UI.getChildControl(self._ui._stc_naviEventAreaIcon, "StaticText_Desc")
  ...
  UI.setLimitAutoWrapTextAndAddTooltip(self._ui._txt_naviEventAreaText, 3, self._ui._txt_naviEventAreaText:GetText(), nil)
...
function PaGlobal_Fishing_All:registEventHandler()
  ...
  self._ui._stc_naviEventAreaIcon:addInputEvent("Mouse_LUp", "HandleEventLUp_Fishing_All_NaviEventArea()")
  self._ui._txt_naviEventAreaText:addInputEvent("Mouse_LUp", "HandleEventLUp_Fishing_All_NaviEventArea()")
  registerEvent("FromClient_EventNavigationShow", "PaGlobal_Fishing_All_ShowEventAreaIcon")
end
function PaGlobal_Fishing_All:prepareOpen()
  ...
  if nil ~= PaGlobal_FishingEventIcon_Close then
    PaGlobal_FishingEventIcon_Close()
  end
...
function PaGlobal_Fishing_All:fishing_Start()
  if _ContentsGroup_LifeEquipmentSlots == true and self._stateValue._isJaksal_0 == false then
    self._stateValue._isJaksal_0 = true
  end
  if self._stateValue._isFishing_Start == true and false == self:isHideMiniGameManual() then
    ...
    PaGlobal_Fishing_All_ShowEventAreaIcon(Defines.EventNaviType.EVENT_FISHING)
  end
end
...
function PaGlobal_Fishing_All:jaksal_0()
  if _ContentsGroup_LifeEquipmentSlots == true and self._stateValue._isFishing_0 == false then
    self._stateValue._isFishing_0 = true
  end
...
function PaGlobal_Fishing_All:showEventAreaIcon(eventNaviType)
  if Panel_Widget_Fishing_All:GetShow() == false then
    return
  end
  if Defines.EventNaviType.EVENT_FISHING ~= eventNaviType then
    return
  end
  PaGlobal_Fishing_All:hideEventAreaIcon()
  local stringKey = ToClient_GetEventNavigationStringKey(eventNaviType)
  if stringKey == nil then
    return
  end
  if false == self._checkEventAreaIcon and false == isGameTypeGT() then
    self._ui._stc_naviEventAreaIcon:SetShow(true)
    self._ui._txt_naviEventAreaText:SetText(PAGetString(Defines.StringSheet_GAME, stringKey))
    UI.setLimitAutoWrapTextAndAddTooltip(self._ui._txt_naviEventAreaText, 3, self._ui._txt_naviEventAreaText:GetText(), nil)
  end
end
function PaGlobal_Fishing_All:hideEventAreaIcon()
  self._ui._stc_naviEventAreaIcon:SetShow(false)
  TooltipSimple_Hide()
end
