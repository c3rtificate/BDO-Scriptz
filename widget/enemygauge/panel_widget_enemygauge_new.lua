...
local function updateStunGauge(targetActor, stun, maxStun, spanY)
  local percents = stun / maxStun * 100
  if targetActor:hasStun() and stun > 0 then
    EnemyBuffListBoarder:SetSpanSize(-15, spanY)
...
function Panel_MonsterInfo_UpdateStun(actorKey, stun, maxStun)
  if targetActorKey == actorKey then
    ...
    local buffSpanY = 15
    if targetActor:get():isMonster() and 4 == targetActor:getCharacterGradeType() then
      buffSpanY = 0
    elseif targetActor:get():isPlayer() then
      buffSpanY = 0
    end
    updateStunGauge(targetActor:get(), stun, maxStun, buffSpanY)
  end
end
function targetHpInfo_Update_Monster(actorKey, nowHP)
  ...
  if 4 ~= targetActor:getCharacterGradeType() then
    if checkActiveCondition(targetActor:getCharacterKey()) == true then
      ...
      if nowHP < prevHP then
        ...
        if _ContentsOption_CH_HPMP_Bar_Color == true then
  ...
  if not isXbox and playerLevel >= 10 and playerLevel < 61 then
    if not ToClient_GetMessageFilter(10) and FromClient_ContactOfStrongMonster() and not regionInfo:get():isSafeZone() then
      ...
      if Panel_Monster_Bar ~= nil and Panel_Monster_Bar:GetShow() == true then
...
function panel_Update_Monster_Info(actorKey)
  ...
  updateStunGauge(targetActor:get(), targetActor:get():getStun(), targetActor:get():getMaxStun(), buffSpanY)
  ...
  while nil ~= appliedBuff do
    ...
    appliedBuff_DATAPool[appliedBuff_Idx] = appliedBuffz
  ...
  if targetActor:get():isPlayer() then
    ...
    if playerActorProxyWrapper:get():isVolunteer() then
      ...
      local playerName = targetActor:getName()
      if true == ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_Solare) then
        playerName = playerActorProxyWrapper:getUserNickname()
      end
      if true == isKnowledgeGradeShow then
        lua_TargetInfo_Name:SetText(playerName .. " " .. mentalLevelString)
      else
        lua_TargetInfo_Name:SetText(playerName)
      end
      if ToClient_namelessPVPServer() == true then
        local actor = getActor(actorKey)
        if actor ~= nil and actor:IsNeedHideNameForPVPServer() == true then
          lua_TargetInfo_Name:SetText(PaGlobalFunc_getAnonymousNameString(0))
        end
      end
    end
  elseif true == isKnowledgeGradeShow then
    ...
    local isPVPNameLess = ToClient_namelessPVPServer()
    if instanceMapKey == __eInstanceLocalWar_PapuCrio and targetActor:get():isVehicle() then
      local vehicleActor = getVehicleActor(actorKey)
      if nil ~= vehicleActor then
        local vehicleActorGet = vehicleActor:get()
        if nil ~= vehicleActorGet then
          local playerActorProxyWrapper = getPlayerActor(vehicleActorGet:getOwnerActorKey())
          if nil ~= playerActorProxyWrapper then
            lua_TargetInfo_Name:SetText(playerActorProxyWrapper:getName())
          end
        end
      end
    end
    if isPVPNameLess == true and targetActor:get():isVehicle() then
      local vehicleActor = getVehicleActor(actorKey)
      if nil ~= vehicleActor then
        local vehicleActorGet = vehicleActor:get()
        if nil ~= vehicleActorGet then
          lua_TargetInfo_Name:SetText("")
        end
      end
    end
  end
  if isGameServiceTypeDev() == true then
    ...
    if elementSSW ~= nil then
      local part = elementSSW:getMorningLandMonsterPartRaw()
      if part == __eMorningLandMosterPart1 then
  ...
  elseif targetActor:get():isMonster() then
    if 4 == targetActor:getCharacterGradeType() then
      ...
      EnemyBuffListBoarder:SetSpanSize(-15, buffSpanY)
    else
  ...
  local monsterHiddenHp = targetActor:get():getCharacterStaticStatus()._isHiddenHp
  local monsterHiddenName = targetActor:get():isHiddenName()
  lua_EnemyTypeIcon:SetShow(false)
  lua_EnemyTypeText:SetShow(false)
  if targetActor:get():isMonster() == true and (monsterHiddenHp == false or monsterHiddenName == false) then
...
function _recentTargetInfo_TendencyOnDead_MSG(targetActor, actorKey)
  ...
  if targetActor:get():isVehicle() == true then
    local vehicleActor = getVehicleActor(actorKey)
    if vehicleActor:get():isUseGroundRunGauge() == true and vehicleActor:get():hasRider() == true then
      return
    end
  end
  if targetActor:get():isSubPlayer() == true then
    return
  end
  if ToClient_IsWarringBothGuild(targetActor:get()) then
    return
  end
...
function FGlobal_Panel_Monster_Bar_RePos()
  if Panel_LocalWar ~= nil and Panel_LocalWar:GetShow() then
    ...
    Panel_Monster_Bar:ComputePosAllChild()
  end
end
...
registerEvent("FromClient_StartSequencePlayerControlable", "hideRecentTargetInfo")
registerEvent("FromClient_LeaveSequencePossessMode", "hideRecentTargetInfo")
