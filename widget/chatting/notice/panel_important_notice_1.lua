...
function PaGlobal_ImportantNotice:initialize()
  ...
  PaGlobal_ImportantNotice._isConsole = _ContentsGroup_RenewUI
  ...
  PaGlobal_ImportantNotice._ui._staticText_message:SetTextMode(__eTextMode_AutoWrap)
  PaGlobal_ImportantNotice._ui._staticText_calculate:SetTextMode(__eTextMode_AutoWrap)
...
function PaGlobal_ImportantNotice:registEventHandler()
  ...
  self._ui._static_clipArea:addInputEvent("Mouse_LUp", "HandleEventLUp_ImportantNotice_WebLink()")
...
function PaGlobal_ImportantNotice:showNoticeText(noticeIndex)
  ...
  self._ui._static_clipArea:SetSize(uiTextMessage:GetTextSizeX(), uiTextMessage:GetTextSizeY())
  if true == msgInfo:isLinkedWebsite() then
    self._ui._static_clipArea:SetIgnore(false)
    self._currentWebLink = msgInfo:getLinkedWebsite()
  else
    self._ui._static_clipArea:SetIgnore(true)
    self._currentWebLink = nil
  end
...
function PaGlobal_ImportantNotice:goToWebLink()
  if nil == self._currentWebLink then
    return
  end
  ToClient_OpenChargeWebPage(self._currentWebLink)
end
