...
local chatEmoticon = {
  ...
  _ui_console = {
    _static_BottomPc = UI.getChildControl(_panel, "Static_BottomBg_ConsoleUI"),
    _static_Y_ConsoleUI = nil,
    _static_B_ConsoleUI = nil,
    _static_A_ConsoleUI = nil
  },
...
function chatEmoticon:createControl()
  ...
  for index = 0, self._imoticonCount - 1 do
    if nil == self._iconFocus[index] then
      self._iconFocus[index] = UI.createAndCopyBasePropertyControl(self._ui._static_EmoticonListBG, "Static_Focus", self._ui._static_EmoticonListBG, "FocusBG_" .. index)
      self._iconSlot[index] = UI.createAndCopyBasePropertyControl(self._ui._static_EmoticonListBG, "Static_Emoticon_Small", self._ui._static_EmoticonListBG, "IconBG_" .. index)
...
function chatEmoticon:initControl()
  ...
  self._ui_console._static_Y_ConsoleUI = UI.getChildControl(self._ui_console._static_BottomPc, "StaticText_Y_ConsoleUI")
  self._ui_console._static_B_ConsoleUI = UI.getChildControl(self._ui_console._static_BottomPc, "StaticText_B_ConsoleUI")
  self._ui_console._static_A_ConsoleUI = UI.getChildControl(self._ui_console._static_BottomPc, "StaticText_A_ConsoleUI")
  if true == _ContentsGroup_UsePadSnapping then
    self._ui._button_WinClose:SetShow(false)
    self._ui._button_Cancel:SetShow(false)
    self._ui._button_Apply:SetShow(false)
    self._ui_console._static_BottomPc:SetShow(true)
    Panel_Chat_Emoticon:SetSize(Panel_Chat_Emoticon:GetSizeX(), Panel_Chat_Emoticon:GetSizeY() - self._ui._button_Apply:GetSizeY())
    self._ui_console._static_BottomPc:SetPosY(Panel_Chat_Emoticon:GetPosY() + Panel_Chat_Emoticon:GetSizeY())
    _panel:registerPadEvent(__eConsoleUIPadEvent_Up_B, "PaGlobalFunc_ChatEmoticon_Close()")
    _panel:registerPadEvent(__eConsoleUIPadEvent_Up_Y, "EventHandler_ChatEmoticon_ApplyEmoticon()")
    self._keyGuideAlign = {
      self._ui_console._static_Y_ConsoleUI,
      self._ui_console._static_A_ConsoleUI,
      self._ui_console._static_B_ConsoleUI
    }
    PaGlobalFunc_ConsoleKeyGuide_SetAlign(self._keyGuideAlign, self._ui_console._static_BottomPc, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP)
  else
    self._ui_console._static_BottomPc:SetShow(false)
  ...
  self._initPosX = Panel_Chat_Emoticon:GetPosX()
  self._initPosY = Panel_Chat_Emoticon:GetPosY()
end
function chatEmoticon:resetData()
  ...
  if self._imoticonCount > 0 then
    ...
    self:IconMouseClickEvent(self._currentSetIndex, true)
  end
end
function PaGlobalFunc_ChatEmoticon_Open()
  Panel_Chat_Emoticon:SetPosX(chatEmoticon._initPosX)
  Panel_Chat_Emoticon:SetPosY(chatEmoticon._initPosY)
  chatEmoticon:resetData()
  chatEmoticon:update()
  chatEmoticon:open()
  PaGlobalFunc_ChatEmoticon_SetCheck(true)
  chatEmoticon._messangerId = nil
end
function PaGlobalFunc_ChatEmoticon_OpenByMessanger(posX, posY, messangerId, isPopUp)
  if nil == posX or nil == posY or nil == messangerId or nil == isPopUp then
    return
  end
  Panel_Chat_Emoticon:CloseUISubApp()
  chatEmoticon:resetData()
  chatEmoticon:update()
  chatEmoticon:open()
  if true == isPopUp then
    Panel_Chat_Emoticon:OpenUISubApp()
  else
    Panel_Chat_Emoticon:SetPosX(posX)
    Panel_Chat_Emoticon:SetPosY(posY)
  end
  chatEmoticon._messangerId = messangerId
  if true == _ContentsGroup_UsePadSnapping then
...
function PaGlobalFunc_ChatEmoticon_CloseByMessanger(messangerIdstr)
  if nil == chatEmoticon._messangerId or nil == messangerIdstr then
    return
  end
  if chatEmoticon._messangerId ~= messangerIdstr then
    return
  end
...
function EventHandler_ChatEmoticon_IconClick(index)
  chatEmoticon:IconMouseClickEvent(index, false, false)
end
function EventHandler_ChatEmoticon_IconRClick(index)
  chatEmoticon:IconMouseClickEvent(index, true, false)
  chatEmoticon:applyMouseClickEvent()
end
function chatEmoticon:IconMouseClickEvent(index, rClick, isResetData)
  local EmoticonSs = ToClient_getEmoticonInfoByIndex(index)
...
function chatEmoticon:applyMouseClickEvent()
  ...
  if true == _ContentsGroup_UsePadSnapping then
    if nil ~= chatEmoticon._messangerId and nil ~= Panel_FriendInfoList_All then
      PaGlobalFunc_FriendInfoList_All_SetFocusEdit()
      PaGlobalFunc_FriendInfoList_All_FriendMessangerInputEmoticon(replaceText)
      PaGlobalFunc_FriendInfoList_All_KillFocusEdit()
    end
  elseif true == ChatInput_IsChattingFocus() then
    ChatInput_addEmoticon(replaceText)
  elseif true == PaGlobalFunc_FriendInfoList_All_IsFocusEdit() then
    PaGlobalFunc_FriendInfoList_All_FriendMessangerInputEmoticon(replaceText)
  elseif true == FriendMessanger_IsFocusEdit() then
    friend_inputEmoticon(replaceText)
  end
end
