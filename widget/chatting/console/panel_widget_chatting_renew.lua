...
function ChattingInfo:initialize()
  ...
  self._ui.list2_macro:registEvent(__ePAUIList2EventType_LuaChangeContent, "PaGlobalFunc_ChattingInfo_PushMacro")
  self._ui.list2_macro:createChildContent(__ePAUIList2ElementManagerType_List)
...
function ChattingInfo:registEventHandler()
  if true == _ContentsGroup_UsePadSnapping and false == ToClient_IsDevelopment() then
  ...
  _panel:registerPadEvent(__eConsoleUIPadEvent_LTPress_Y, "Input_ChattingInfo_PressedLTY()")
...
function PaGlobalFunc_ChattingInfo_PushMacro(content, key)
  ...
  txt_macro:SetTextMode(__eTextMode_LimitText)
...
function ChattingInfo:open()
  ...
  if nil ~= HandleEventLUp_ChracterInfo_All_CloseAll then
  ...
  if nil ~= Panel_Window_SetVoiceChat_All and Panel_Window_SetVoiceChat_All:GetShow() then
  ...
  if false == _ContentsGroup_NewUI_Mail_All then
    ...
    if nil ~= Panel_Window_Mail_All and true == Panel_Window_Mail_All:GetShow() then
      PaGlobal_Window_Mail_All_Close()
    end
  end
  if nil ~= Panel_Widget_Menu_Remake and Panel_Widget_Menu_Remake:GetShow() then
  ...
  if nil ~= Panel_Window_Achievement_BookShelf_All and Panel_Window_Achievement_BookShelf_All:GetShow() then
    PaGlobalFunc_Achievement_BookShelf_All_Close()
  end
  _panel:SetShow(true)
  ...
  if false == PaGlobalFunc_ChattingViewer_GetIsShowSmall() then
    self._ui.txt_keyGuideLTY:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHATTING_SET_SISE_SMALL"))
  else
    self._ui.txt_keyGuideLTY:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHATTING_SET_SISE_BIG"))
  end
...
function Input_ChattingInfo_PressedLTY()
  local self = ChattingInfo
  PaGlobalFunc_ChattingViewer_ToggleSize()
  if false == PaGlobalFunc_ChattingViewer_GetIsShowSmall() then
    self._ui.txt_keyGuideLTY:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHATTING_SET_SISE_SMALL"))
  else
    self._ui.txt_keyGuideLTY:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHATTING_SET_SISE_BIG"))
...
function InputPadY_ChattingInfo_MacroShowToggle(isShow)
  local self = ChattingInfo
  if isPadPressed(__eJoyPadInputType_LeftTrigger) and isShow then
    return
  end
...
function InputPadX_ChattingInfo_MacroMoveSelect(isUp)
  ...
  self._ui.stc_selectedMacro:SetPosY(38 + 38 * (self._currentMacro - 1))
end
...
function Input_ChattingInfo_OnPadBNotEditting()
  if 0 < string.len(ChattingInfo._ui.edit_chatContent:GetText()) then
    PaGlobalFunc_ChattingInfo_CancelAction()
    PaGlobalFunc_ChattingInfo_CancelMessage()
    if isPadDown(__eJoyPadInputType_B) then
      return
    end
  end
...
function ChattingInfo:updateChannelIcon()
  for ii = 1, CHAT_TYPE.Count - 1 do
    if nil ~= self._ui.stc_channelList[ii] then
      self._ui.stc_channelList[ii]:SetShow(false)
    end
  end
  local chatType = self._availableChannelList[self._currentChatTypeIndex]
  if nil ~= _chatTypeUV[chatType] and nil ~= self._ui.stc_channelList[chatType] then
...
function PaGlobalFunc_ChattingInfo_UpdatePermission()
  ...
  if nil ~= selfPlayerWrapper then
    ...
    self.permissions[CHAT_TYPE.Channel] = true
    ...
    if true == selfPlayerWrapper:get():isGuildMember() then
      ...
      if __eGuildMemberGradeMaster == myGrade or __eGuildMemberGradeSubMaster == myGrade or __eGuildMemberGradeAdviser == myGrade or __eGuildMemberGradeScribe == myGrade or __eGuildMemberGradeChiefOfStaff == myGrade then
        self.permissions[CHAT_TYPE.GuildManager] = false
      else
        self.permissions[CHAT_TYPE.GuildManager] = false
      end
    else
...
function PaGlobalFunc_ChattingInfo_SetShowFunctionKeyGuide(isSender)
  ...
  local senderPlatformType
  if true == ToClient_isTotalGameClient() then
    senderPlatformType = PaGlobalFunc_ChattingHistory_GetPlatformType()
  end
  local selfPlayerPlatform = ToClient_getGamePlatformType()
  ChattingInfo._ui.txt_keyGuideLTX:SetShow(isSender and ToClient_isPS() and selfPlayerPlatform == senderPlatformType)
