...
local ChattingHistory = {
  ...
  _tabName = {
    ...
    [TAB_TYPE.CHANNEL] = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_MENU_CHAT_GROUP")
  },
  _partyType = 0
}
...
local _tabButtonTextrue = {
  [TAB_TYPE.CUSTOM] = "Renewal/Button/Console_TapBtn_01.dds",
  [TAB_TYPE.GENERAL] = "Renewal/Button/Console_TapBtn_01.dds",
  [TAB_TYPE.WORLD] = "Renewal/Button/Console_TapBtn_01.dds",
  [TAB_TYPE.GUILD] = "Renewal/Button/Console_TapBtn_01.dds",
  [TAB_TYPE.SYSTEM] = "Renewal/Button/Console_TapBtn_01.dds",
  [TAB_TYPE.CHANNEL] = "Renewal/Button/Console_TapBtn_02.dds"
}
...
function ChattingHistory:initialize()
  ...
  for ii = 1, self._tabCount do
    ...
    local rdoButtonPosPadding = 10
    self._ui.rdo_chatTypeList[ii]:SetPosX((ii - 1) * self._rdoButtonsXGap + rdoButtonPosPadding)
    ...
    radioButton:ChangeTextureInfoName(_tabButtonTextrue[ii])
  ...
  self._ui.txt_keyGuideLTX = UI.getChildControl(self._ui.stc_keyGuideGroup, "StaticText_KeyGuideLTX")
  ...
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyguide, self._ui.stc_keyGuideGroup, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_LEFT)
...
function ChattingHistory:registEventHandler()
  ...
  if _ContentsGroup_isPS4UI then
    _panel:registerPadEvent(__eConsoleUIPadEvent_LTPress_X, "PaGlobalFunc_ChattingInfo_ShowConsoleProfile_InParty()")
  end
end
...
function ChattingHistory:createPool(poolIndex)
  local ChatUIPool = {
    ...
    _maxcount_ChattingLinkedParty = ChattingHistory._cloneCount,
  ...
  function ChatUIPool:initialize(poolIndex)
    ...
    for ii = 1, ch._cloneCount do
      ...
      ChatUIPool._list_ChattingLinkedParty[ii] = UI.createAndCopyBasePropertyControl(ch._ui.stc_sampleBG, "StaticText_ChatLinkedParty", ch._listPanel[poolIndex], "StaticText_ChatLinkedParty" .. ii)
...
function PaGlobalFunc_ChattingHistory_Open(chatType, byParty, partyType)
  if nil == byParty or false == byParty then
    ...
    if nil == partyType then
      ChattingHistory._partyType = ToClient_GetPartyType()
    else
      ChattingHistory._partyType = partyType
    end
    _panel:registerPadEvent(__eConsoleUIPadEvent_Up_A, "")
    if CppEnums.PartyType.ePartyType_Normal == ChattingHistory._partyType then
      ChattingHistory._ui.txt_keyGuideA:SetText(PAGetString(Defines.StringSheet_RESOURCE, "INTERACTION_BTN_PARTY"))
      _panel:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobalFunc_ChattingInfo_InviteParty_InParty()")
    elseif CppEnums.PartyType.ePartyType_Large == ChattingHistory._partyType and _ContentsGroup_LargeParty then
      ChattingHistory._ui.txt_keyGuideA:SetText(PAGetString(Defines.StringSheet_RESOURCE, "INTERACTION_BTN_LARGEPARTY"))
      _panel:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobalFunc_ChattingInfo_InviteLargeParty_InParty()")
    end
  end
...
function PaGlobalFunc_ChattingHistory_ChangePartyType(partyType)
  ...
  if CppEnums.PartyType.ePartyType_Normal == partyType then
    ChattingHistory._ui.txt_keyGuideA:SetText(PAGetString(Defines.StringSheet_RESOURCE, "INTERACTION_BTN_PARTY"))
    _panel:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobalFunc_ChattingInfo_InviteParty_InParty()")
  elseif CppEnums.PartyType.ePartyType_Large == partyType and _ContentsGroup_LargeParty then
    ChattingHistory._ui.txt_keyGuideA:SetText(PAGetString(Defines.StringSheet_RESOURCE, "INTERACTION_BTN_LARGEPARTY"))
    _panel:registerPadEvent(__eConsoleUIPadEvent_Up_A, "PaGlobalFunc_ChattingInfo_InviteLargeParty_InParty()")
  end
end
...
function ChattingHistory:update(chatPanel, panelIndex, isShow)
  ...
  if isChangeFontSize() == true then
    ...
    ToClient_getFontWrapper("BaseFont_10_Chat"):changeFontSize(self:convertChatFontTypeToFontSize(fontType))
    ChatPanel_Update()
  end
...
function PaGlobalFunc_ChattingInfo_InviteLargeParty_InParty()
  if false == ToClient_isCommunicationAllowed() then
    local messageBoxMemo = PAGetString(Defines.StringSheet_GAME, "LUA_DONOTHAVE_PRIVILEGE")
    local messageBoxData = {
      title = PAGetString(Defines.StringSheet_GAME, "LUA_WARNING"),
...
function PaGlobalFunc_ChattingInfo_ShowConsoleProfile_InParty()
  local self = ChattingHistory
  if false == PaGlobalFunc_PartySetting_GetShow() then
    return
  end
...
function ChattingHistory:setTabTo(index)
  ...
  for ii = 1, #self._ui.rdo_chatTypeList do
    self._ui.rdo_chatTypeList[ii]:ChangeTextureInfoNameAsync(_tabButtonTextrue[ii])
...
function ChattingHistory:updateSelectionBox()
  ...
  local senderPlatformType = PaGlobalFunc_ChattingHistory_GetPlatformType()
  if true == PaGlobalFunc_PartySetting_GetShow() then
    ...
    self._ui.txt_keyGuideLTX:SetShow(_ContentsGroup_isPS4UI and nil ~= sender and "" ~= sender and nil ~= xuid and "" ~= xuid and selfPlayerPlatformType == senderPlatformType)
    if selfPlayerPlatformType == senderPlatformType then
      _panel:registerPadEvent(__eConsoleUIPadEvent_LTPress_X, "PaGlobalFunc_ChattingInfo_ShowConsoleProfile_InParty()")
    end
  else
    self._ui.txt_keyGuideA:SetShow(false)
...
function Input_ChatHistory_ChangeToNextTab(isNext)
  ...
  if index < 1 then
    index = #self._ui.rdo_chatTypeList
...
function PaGlobalFunc_ChattingHistory_GetSelectedSenderChatContent()
  local self = ChattingHistory
  local chatPanel = ToClient_getChattingPanel(self._currentTab - 1)
  local chattingMessage = chatPanel:getChattingMessageByIndex(self._currentSelectedMessage[self._currentTab] - 1)
  if nil ~= chattingMessage and CHAT_TYPE.System ~= chattingMessage.chatType and CHAT_TYPE.Notice ~= chattingMessage.chatType and false == chattingMessage.isMe then
    return chattingMessage:getContent()
  else
    return nil
  end
end
function PaGlobalFunc_ChattingHistory_GetSelectedSenderChatType()
  local self = ChattingHistory
  local chatPanel = ToClient_getChattingPanel(self._currentTab - 1)
  local chattingMessage = chatPanel:getChattingMessageByIndex(self._currentSelectedMessage[self._currentTab] - 1)
  if nil ~= chattingMessage and CHAT_TYPE.System ~= chattingMessage.chatType and CHAT_TYPE.Notice ~= chattingMessage.chatType and false == chattingMessage.isMe then
    return chattingMessage.chatType
  else
    return nil
  end
end
function PaGlobalFunc_ChattingHistory_GetSelectedSenderUserNo()
  local self = ChattingHistory
  local chatPanel = ToClient_getChattingPanel(self._currentTab - 1)
  local chattingMessage = chatPanel:getChattingMessageByIndex(self._currentSelectedMessage[self._currentTab] - 1)
  if nil ~= chattingMessage and CHAT_TYPE.System ~= chattingMessage.chatType and CHAT_TYPE.Notice ~= chattingMessage.chatType and false == chattingMessage.isMe then
    return chattingMessage:getUserNo()
  else
    return nil
  end
end
function PaGlobalFunc_ChattingHistory_GetSelectedSenderChatSendServerTime()
  local self = ChattingHistory
  local chatPanel = ToClient_getChattingPanel(self._currentTab - 1)
  local chattingMessage = chatPanel:getChattingMessageByIndex(self._currentSelectedMessage[self._currentTab] - 1)
  if nil ~= chattingMessage and CHAT_TYPE.System ~= chattingMessage.chatType and CHAT_TYPE.Notice ~= chattingMessage.chatType and false == chattingMessage.isMe then
    return chattingMessage:getChatSendServerTime()
  else
    return nil
  end
end
function PaGlobalFunc_ChattingHistory_GetSelectedSenderChatTime()
  local self = ChattingHistory
  local chatPanel = ToClient_getChattingPanel(self._currentTab - 1)
  local chattingMessage = chatPanel:getChattingMessageByIndex(self._currentSelectedMessage[self._currentTab] - 1)
  if nil ~= chattingMessage and CHAT_TYPE.System ~= chattingMessage.chatType and CHAT_TYPE.Notice ~= chattingMessage.chatType and false == chattingMessage.isMe then
    return chattingMessage:getTime()
  else
    return nil
  end
end
function PaGlobalFunc_ChattingHistory_GetSelectedSenderIsGameManager()
  local self = ChattingHistory
  local chatPanel = ToClient_getChattingPanel(self._currentTab - 1)
  local chattingMessage = chatPanel:getChattingMessageByIndex(self._currentSelectedMessage[self._currentTab] - 1)
  if nil ~= chattingMessage and CHAT_TYPE.System ~= chattingMessage.chatType and CHAT_TYPE.Notice ~= chattingMessage.chatType and false == chattingMessage.isMe then
    return chattingMessage.isGameManager
  else
    return nil
  end
end
...
function PaGlobalFunc_ChattingHistory_GetPlatformType()
  local self = ChattingHistory
  local chatPanel = ToClient_getChattingPanel(self._currentTab - 1)
  local chattingMessage = chatPanel:getChattingMessageByIndex(self._currentSelectedMessage[self._currentTab] - 1)
  if nil ~= chattingMessage and CHAT_TYPE.System ~= chattingMessage.chatType and CHAT_TYPE.Notice ~= chattingMessage.chatType and false == chattingMessage.isMe and true == ToClient_isTotalGameClient() and nil ~= chattingMessage:getPlatformType() then
    return chattingMessage:getPlatformType()
  else
    return nil
  end
end
function PaGlobalFunc_ChattingHistory_getCurrentSelectMessage()
  local self = ChattingHistory
  local chatPanel = ToClient_getChattingPanel(self._currentTab - 1)
  local chattingMessage = chatPanel:getChattingMessageByIndex(self._currentSelectedMessage[self._currentTab] - 1)
...
function PaGlobalFunc_ChattingHistory_GetSelectedSenderUserId()
  local self = ChattingHistory
  local chatPanel = ToClient_getChattingPanel(self._currentTab - 1)
  local chattingMessage = chatPanel:getChattingMessageByIndex(self._currentSelectedMessage[self._currentTab] - 1)
  if nil ~= chattingMessage and CHAT_TYPE.System ~= chattingMessage.chatType and CHAT_TYPE.Notice ~= chattingMessage.chatType and false == chattingMessage.isMe then
    return chattingMessage:getSenderUserId()
  else
    return nil
  end
end
...
function ChattingHistory:convertChatFontTypeToFontSize(chattingFontType)
  local fontSize = 14
