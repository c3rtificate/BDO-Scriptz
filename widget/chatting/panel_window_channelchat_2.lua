...
function PaGlobalFunc_ChannelChat_OpenBySelect(selectTitleName)
  PaGlobal_ChannelChat:prepareOpen(selectTitleName)
end
...
function FromClient_ChannelChatList(isClear)
  ...
  if channelChatRoomCount > self._serverListCount then
    for index = self._maxListCount, channelChatRoomCount - 1 do
      PaGlobal_ChannelChat:addChannelList(index)
    end
  end
  self._maxListCount = channelChatRoomCount
  ...
  local isFindSelectTargetRoom = false
  for index = 0, self._serverListCount - 1 do
    ...
    if nil ~= ChannelChatRoomInfo then
      ...
      if currentRoomNo == roomNo and isFindSelectTargetRoom == false then
        self._selectIndex = index
      end
      if self._selectTitleString ~= nil and self._selectTitleString == roomName then
        isFindSelectTargetRoom = true
        self._selectIndex = index
      end
  ...
  local myChannelChatRoomInfo = ToClient_getMyChannelChatRoom(__eChannelChatRoomType_System)
  if isFindSelectTargetRoom == true then
    HandleEventLUp_ChannelChat_Confirm()
  end
end
...
function FromClient_LoginVoiceChatServer()
  if true == PaGlobal_ChannelChat._isFirstLogin then
    ...
    if false == isExistCacheData then
      ...
      if nil == currentRoomNo or currentRoomNo <= 0 or currentRoomNo > PaGlobal_ChannelChat._serverListCount then
