...
function HandleEventLUp_ChatSubMenuReportChat()
  if PaGlobal_ChatSubMenu._currentPoolIndex ~= nil and PaGlobal_ChatSubMenu._clickedMessageIndex ~= nil and PaGlobal_ChatSubMenu._clickedMsg ~= nil then
    ...
    elseif PaGlobal_ChatSubMenu._clickedMsg == nil or PaGlobal_ChatSubMenu._clickedMsg == "" then
      FromClient_Chat_Report_Failed("LUA_CHATREPORT_NO_EMOTICON_REPORT_MSG")
    else
      PaGlobal_Window_Chat_Report_All_Open(PaGlobal_ChatSubMenu._clickedName, PaGlobal_ChatSubMenu._clickedUserNickName, PaGlobal_ChatSubMenu._senderUserNo, PaGlobal_ChatSubMenu._clickedMsg, PaGlobal_ChatSubMenu._chatType, PaGlobal_ChatSubMenu._chatSendTime, PaGlobal_ChatSubMenu._chatSendServerTime, PaGlobal_ChatSubMenu._clickedUserId)
    end
  end
  PaGlobal_ChatSubMenu:SetShow(false)
end
...
function HandleEventLUp_InviteMiniGameYacht()
  local self = PaGlobal_ChatSubMenu
  if self == nil then
    return
  end
  if self._clickedUserNickName == nil then
    return
  end
  if PaGlobalFunc_MiniGameYacht_Invite_Open == nil then
    return
  end
  PaGlobalFunc_MiniGameYacht_Invite_Open(self._clickedUserNickName)
  self:SetShow(false)
end
