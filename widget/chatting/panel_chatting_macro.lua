...
local StyleUI = {
  ...
  _c_btn_GuildAlliance = UI.getChildControl(Panel_Chatting_Macro, "Button_GuildAlliance"),
  _c_btn_Team = UI.getChildControl(Panel_Chatting_Macro, "Button_Team"),
...
if isGameTypeKorea() then
  macroChatTypeCount = 7
elseif isGameTypeJapan() then
  macroChatTypeCount = 7
else
  macroChatTypeCount = 6
end
local startPosY = 7
local ChatMacro = {
  _chatType = {},
...
function ChatMacro:initialize()
  ...
  for idx = 0, macroCount - 1 do
    ui._macroBG = UI.createControl(__ePAUIControl_Static, Panel_Chatting_Macro, "Static_MacroBG_" .. idx)
    CopyBaseProperty(StyleUI._c_macroBG, ui._macroBG)
    ui._macroBG:SetShow(true)
    ui._macroBG:SetPosY(startPosY + idx * 32)
    ui._macroKey = UI.createControl(__ePAUIControl_StaticText, ui._macroBG, "StaticText_MacroKey_" .. idx)
    ...
    ChatMacro._buttonChatType[idx] = UI.createControl(__ePAUIControl_Button, ui._macroBG, "Button_Normal_" .. idx)
    ...
    ChatMacro._editChatMessage[idx] = UI.createControl(__ePAUIControl_Edit, ui._macroBG, "Edit_InputMacro_" .. idx)
    ...
    if isGameTypeKorea() then
      ...
      StyleUI._c_btn_GuildAlliance:SetPosX(85)
      StyleUI._c_btn_Team:SetPosX(85)
    ...
    elseif isGameTypeTaiwan() then
      ...
      StyleUI._c_btn_WorldWithItem:SetPosX(85)
      ChatMacro._editChatMessage[idx]:SetPosX(140)
    elseif _ContentsOption_CH_GameType then
      Panel_Chatting_Macro:SetSize(390, 355)
      StyleUI._c_macroBG:SetSize(380, 30)
      ui._macroBG:SetSize(380, 30)
      ui._macroKey:SetSize(100, 20)
      StyleUI._c_macroBG:SetPosX(5)
      ui._macroBG:SetPosX(5)
      StyleUI._c_btn_Normal:SetPosX(105)
      StyleUI._c_btn_Party:SetPosX(105)
      StyleUI._c_btn_Guild:SetPosX(105)
      StyleUI._c_btn_World:SetPosX(105)
      StyleUI._c_btn_GuildAlliance:SetPosX(105)
      StyleUI._c_btn_Team:SetPosX(105)
      StyleUI._c_btn_WorldWithItem:SetPosX(105)
      ChatMacro._editChatMessage[idx]:SetPosX(160)
    else
      Panel_Chatting_Macro:SetSize(370, 355)
      StyleUI._c_macroBG:SetSize(360, 30)
      ui._macroBG:SetSize(360, 30)
  ...
  StyleUI._c_btn_GuildAlliance:SetShow(false)
  StyleUI._c_btn_Team:SetShow(false)
...
function HandleClicked_ChatMacroType(index)
  ...
  if 4 == ChatMacro._chatType[index] and false == _ContentsGroup_guildAlliance then
    ChatMacro._chatType[index] = ChatMacro._chatType[index] + 1
  end
  if 5 == ChatMacro._chatType[index] and false == isLocalWar and false == isArsha and false == isSavageDefence then
    ChatMacro._chatType[index] = ChatMacro._chatType[index] + 1
  end
  ...
  elseif 4 == ChatMacro._chatType[index] then
    copyUI = StyleUI._c_btn_GuildAlliance
  elseif 5 == ChatMacro._chatType[index] then
    copyUI = StyleUI._c_btn_Team
  elseif 6 == ChatMacro._chatType[index] then
...
function ChatMacro:update()
  for index = 0, macroCount - 1 do
    ...
    elseif 5 == ChatMacro._chatType[index] then
      copyUI = StyleUI._c_btn_Team
    elseif 6 == self._chatType[index] then
      copyUI = StyleUI._c_btn_WorldWithItem
    end
