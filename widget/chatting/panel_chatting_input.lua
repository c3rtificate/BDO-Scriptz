...
local isArsha = _ContentsGroup_Arsha
...
local chatInput = {
  ...
  control = {
    ...
    button_Guide = UI.getChildControl(Panel_Chatting_Input, "Button_Guide")
  },
  buttonIds = {
    [0] = nil,
    [1] = "Button_Anounce",
    ...
    [CppEnums.ChatType.Channel] = "Button_Channel",
    [CppEnums.ChatType.GuildManager] = "Button_GuildManager"
  },
...
function chatInput:init()
  ...
  if true == _ContentsOption_CH_CHAT_Translate_OnlyChina then
    self.control.macroButton:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHAT_MACRO_M"))
    self.control.socialButton:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHAT_SOCIALACTION_S"))
    self.control.radioButton_Emoticon:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHAT_EMOTICON_E"))
  end
  if 4 == lastMemoryChatType then
    ...
    if self.control.radioButton_Emoticon:GetShow() then
      self.control.button_Guide:SetPosX(self.control.edit:GetPosX() + self.control.edit:GetSizeX() + 100)
    else
      self.control.button_Guide:SetPosX(self.control.edit:GetPosX() + self.control.edit:GetSizeX() + 70)
    end
  end
  self.control.buttons[lastMemoryChatType]:SetShow(true)
  if true == _ContentsOption_CH_WepHelperControl then
    self.control.button_Guide:SetShow(false)
  end
...
function ChatInput_UpdateChannelChatBtn(isShow)
  local self = chatInput
  if true == _ContentsGroup_ChannelChat then
    if true == isShow then
      local lastChatType = ToClient_getGameUIManagerWrapper():getLuaCacheDataListNumber(__eMemoryRecentChat)
      if 0 == lastChatType then
        self.lastChatType = UI_CT.Public
      elseif 4 ~= lastChatType then
        self.lastChatType = lastChatType
      end
      local myChannelChatRoomInfo = ToClient_getMyChannelChatRoom(__eChannelChatRoomType_System)
      if true == myChannelChatRoomInfo:isDefined() then
        self.isChatTypeChangedMode = not self.isChatTypeChangedMode
        ChatInput_TypeButtonClicked(self.lastChatType)
      end
    elseif false == self.permissions[UI_CT.Channel] then
      self.isChatTypeChangedMode = true
      if self.lastChatType == UI_CT.Channel then
        ChatInput_TypeButtonClicked(UI_CT.Public)
      else
        ChatInput_TypeButtonClicked(self.lastChatType)
      end
    end
  ...
  if self.isChatTypeChangedMode then
    ...
    if permission then
      ...
      if UI_CT.Private == chatType then
        ...
        if self.control.radioButton_Emoticon:GetShow() then
          self.control.button_Guide:SetPosX(self.control.whisperEdit:GetPosX() + self.control.whisperEdit:GetSizeX() + 90)
        else
          self.control.button_Guide:SetPosX(self.control.whisperEdit:GetPosX() + self.control.whisperEdit:GetSizeX() + 60)
        end
      else
        self.control.whisperEdit:SetShow(false)
        self.control.whisperNotice:SetShow(false)
...
function ChatInput_UpdatePermission()
  ...
  if nil ~= selfPlayerWrapper then
    ...
    if true == ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_Solare) then
      self.permissions[UI_CT.Public] = false
      self.permissions[UI_CT.WorldWithItem] = false
      self.permissions[UI_CT.World] = false
      self.permissions[UI_CT.Private] = false
    end
    if isArsha then
    ...
    if _ContentsGroup_ChannelChat then
      local chatRoomInfo = ToClient_getMyChannelChatRoom(__eChannelChatRoomType_System)
      self.permissions[UI_CT.Channel] = chatRoomInfo:isDefined()
    end
    if true == selfPlayerWrapper:get():isGuildMember() then
      local myGrade = selfPlayerWrapper:getGuildMemberGrade()
      if __eGuildMemberGradeMaster == myGrade or __eGuildMemberGradeSubMaster == myGrade or __eGuildMemberGradeAdviser == myGrade or __eGuildMemberGradeScribe == myGrade or __eGuildMemberGradeChiefOfStaff == myGrade then
        self.permissions[UI_CT.GuildManager] = true
      else
        self.permissions[UI_CT.GuildManager] = false
      end
    else
      self.permissions[UI_CT.GuildManager] = false
    end
  ...
  for chatType, btn in pairs(self.control.buttons) do
    ...
    if self.permissions[chatType] then
      self.availableChatType[#self.availableChatType + 1] = chatType
    end
  end
end
...
function ChatInput_SendMessage()
  ...
  elseif nil ~= message and UI_CT.Channel == memoryChatType then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_SymbolNo, "eErrNoChannelChatSendMessageFailed"))
  end
  self.control.edit:SetEditText("", true)
end
...
function ChatInput_TypeButtonOn(chatType)
  ...
  local self = chatInput
  local button = self.control.buttons[chatType]
  if -1 == chatShortCutKey_Value[chatType] then
    return
  end
  ...
  self.control.noticeShortcut:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_CHAT_INPUT_ALT", "number", tostring(chatShortCutKey_Value[chatType])))
...
function ChatInput_CheckReservedKey()
  ...
  elseif isKeyPressed(VCK.KeyCode_MENU) and (isKeyDown_Once(VCK.KeyCode_0) or isKeyDown_Once(VCK.KeyCode_1) or isKeyDown_Once(VCK.KeyCode_2) or isKeyDown_Once(VCK.KeyCode_3) or isKeyDown_Once(VCK.KeyCode_4) or isKeyDown_Once(VCK.KeyCode_5) or isKeyDown_Once(VCK.KeyCode_6) or isKeyDown_Once(VCK.KeyCode_7) or isKeyDown_Once(VCK.KeyCode_8) or isKeyDown_Once(VCK.KeyCode_9) or isKeyDown_Once(VCK.KeyCode_OEM_MINUS) or isKeyDown_Once(VCK.KeyCode_F1)) then
    ChatInput_ChangeChatType_Immediately(toChangeChatType)
  end
end
function ChatInput_ChangeTypeByTab()
  local currChatType = chatInput.lastChatType
  local currChatTypeIndex = -1
  if UI_CT.Private == currChatType then
    return
  end
  for ii = 1, #chatInput.availableChatType do
    if currChatType == chatInput.availableChatType[ii] then
      currChatTypeIndex = ii
    end
  end
  if UI_CT.Private == chatInput.availableChatType[currChatTypeIndex + 1] then
    currChatTypeIndex = currChatTypeIndex + 1
  end
  if currChatTypeIndex >= #chatInput.availableChatType then
    currChatTypeIndex = 0
  end
  chatInput.isChatTypeChangedMode = false
  ChatInput_ChangeChatType_Immediately(chatInput.availableChatType[currChatTypeIndex + 1])
end
...
function ChatInput_CheckInstantCommand()
  ...
  if isProcess then
    ...
    if isReply == true then
      lastWhispersId = ToClient_GetLastWhisperTarget()
    end
...
function ChatInput_Show()
  ...
  ChatInput_UpdateChannelChatBtn(true)
  local self = chatInput
  SetFocusEdit(chatInput.control.edit)
  ...
  if _ContentsGroup_Emoticon and 0 < ToClient_getEmoticonInfoList() then
    ...
    self.control.button_Guide:SetPosX(self.control.radioButton_Emoticon:GetPosX() + 30)
  else
    self.control.radioButton_Emoticon:SetShow(false)
    self.control.button_Guide:SetPosX(self.control.radioButton_Emoticon:GetPosX())
  end
  ...
  ToClient_ClearLinkedPartyRecruitInfo()
  if true == self.control.buttons[UI_CT.Channel]:GetShow() and false == self.permissions[UI_CT.Channel] then
    self.isChatTypeChangedMode = true
    ChatInput_TypeButtonClicked(UI_CT.Public)
  end
  if true == self.isChatTypeChangedMode then
    ChatInput_TypeButtonClicked(self.lastChatType)
    ChatInput_TypeButtonOut()
  end
  self.control.nameTypeButton:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CHAT_NAMETYPE_" .. ToClient_getChatNameType()))
end
...
function ChatInput_addEmoticon(inputStr)
  ...
  local message = self.control.edit:GetEditText()
  local originTextSize = chatInput.control.edit:GetEditTextSize()
  if originTextSize > 0 then
    self.control.edit:AddEditText(inputStr)
  else
...
function ChatInput_IsChattingFocus()
  local self = chatInput
  if true == self.control.whisperEdit:GetFocusEdit() or true == self.control.edit:GetFocusEdit() then
    return true
  end
  return false
end
...
function FGlobal_ChattingInput_LinkedItemByInventory(slotNo, inventoryType)
  if UI_CT.World ~= chatInput.lastChatType and UI_CT.Guild ~= chatInput.lastChatType and UI_CT.Public ~= chatInput.lastChatType and UI_CT.Party ~= chatInput.lastChatType and UI_CT.WorldWithItem ~= chatInput.lastChatType and UI_CT.Private ~= chatInput.lastChatType and UI_CT.RolePlay ~= chatInput.lastChatType and UI_CT.Channel ~= chatInput.lastChatType and UI_CT.GuildManager ~= chatInput.lastChatType then
    return
  end
...
function ChatInput_CheckRemoveLinkedItem()
  if UI_CT.World ~= chatInput.lastChatType and UI_CT.Guild ~= chatInput.lastChatType and UI_CT.Public ~= chatInput.lastChatType and UI_CT.Party ~= chatInput.lastChatType and UI_CT.WorldWithItem ~= chatInput.lastChatType and UI_CT.Private ~= chatInput.lastChatType and UI_CT.RolePlay ~= chatInput.lastChatType and UI_CT.Channel ~= chatInput.lastChatType then
    return
  end
...
function HandleEvnetLUp_ChattingInput_ShowWebHelper()
  ...
  ChatInput_Close()
  TooltipSimple_Hide()
  Panel_WebHelper_ShowToggle("Chatting")
end
...
function FromClient_Init_ChatInput()
  ...
  self.control.macroButton:addInputEvent("Mouse_On", "HandleMouseOn_ChatButtonToolTip(true, 0)")
  self.control.macroButton:addInputEvent("Mouse_Out", "HandleMouseOn_ChatButtonToolTip(false, 0)")
  self.control.socialButton:addInputEvent("Mouse_LUp", "HandleClicked_ToggleChatMacro(1)")
  self.control.socialButton:addInputEvent("Mouse_On", "HandleMouseOn_ChatButtonToolTip(true, 1)")
  self.control.socialButton:addInputEvent("Mouse_Out", "HandleMouseOn_ChatButtonToolTip(false, 1)")
  self.control.radioButton_Emoticon:addInputEvent("Mouse_LUp", "HandleClicked_ToggleChatMacro(2)")
  self.control.radioButton_Emoticon:addInputEvent("Mouse_On", "HandleMouseOn_ChatButtonToolTip(true,2)")
  self.control.radioButton_Emoticon:addInputEvent("Mouse_Out", "HandleMouseOn_ChatButtonToolTip(false,2)")
  ...
  self.control.button_Guide:addInputEvent("Mouse_LUp", "HandleEvnetLUp_ChattingInput_ShowWebHelper()")
  self.control.button_Guide:addInputEvent("Mouse_On", "HandleMouseOn_ChatButtonToolTip(true,3)")
  self.control.button_Guide:addInputEvent("Mouse_Out", "HandleMouseOn_ChatButtonToolTip(false,3)")
end
function FromClient_ChatInput_ExpirienceWikiLink(title, linkString)
  if nil == linkString or "" == linkString then
    return
  end
  if true == Panel_Chatting_Input:GetShow() then
    ChatInput_Close()
  end
  ChatInput_Show()
  local text = tostring(linkString) .. tostring(title)
  chatInput.control.edit:SetEditText(text, true)
end
function HandleMouseOn_ChatButtonToolTip(isShow, number)
  ...
  local desc = ""
  if chatInput.config.toggleType.chattingMacro == number then
    targetControl = chatInput.control.macroButton
    desc = PAGetString(Defines.StringSheet_GAME, "LUA_CHATMACRO_TITLE")
  elseif chatInput.config.toggleType.socialAction == number then
    targetControl = chatInput.control.socialButton
    desc = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_SOCIALMENU_TITLE")
  elseif chatInput.config.toggleType.emoticon == number then
    targetControl = chatInput.control.radioButton_Emoticon
    desc = PAGetString(Defines.StringSheet_RESOURCE, "PANEL_CHATEMOTICON_TITLE")
  elseif number == 3 then
    targetControl = chatInput.control.button_Guide
    desc = PAGetString(Defines.StringSheet_GAME, "LUA_CHATHELP_TITLE")
  end
  TooltipSimple_Show(targetControl, desc)
end
registerEvent("FromClient_ExpirienceWikiLink", "FromClient_ChatInput_ExpirienceWikiLink")
...
registerEvent("FromClient_GroundMouseLDown", "FromClient_GroundMouseClickForChatting")
