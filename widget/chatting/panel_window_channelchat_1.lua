...
function PaGlobal_ChannelChat:setKeyGuideAlign()
  local keyGuideList = {
    self._ui_console.stc_TextA,
    self._ui_console.stc_TextB
  }
  PaGlobalFunc_ConsoleKeyGuide_SetAlign(keyGuideList, self._ui_console.stc_bottomBg, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP)
end
...
function PaGlobal_ChannelChat:registEventHandler()
  ...
  if false == self._isConsole then
    ...
    Panel_Window_ChannelChat:registerPadEvent(__eConsoleUIPadEvent_Up_B, "PaGlobalFunc_ChannelChat_Close()")
    for index = 0, self._serverListCount - 1 do
      self._ui.btn_title[index]:registerPadEvent(__eConsoleUIPadEvent_DpadUp, "PaGlobal_ChannelChat:PadScroll(true," .. index .. ")")
      self._ui.btn_title[index]:registerPadEvent(__eConsoleUIPadEvent_DpadDown, "PaGlobal_ChannelChat:PadScroll(false," .. index .. ")")
    end
  end
...
function PaGlobal_ChannelChat:prepareOpen(selectTitleName)
  if nil == Panel_Window_ChannelChat then
    return
  end
  ...
  if selectTitleName ~= nil then
    self._selectTitleString = selectTitleName
  else
    self._selectTitleString = nil
  end
...
function PaGlobal_ChannelChat:PadScroll(isUp, index)
  local UpScrollIndexRange = self._startItemIndex + self._config._slotCols - 1
  local DownScrollIndexRange = self._startItemIndex + (self._config._slotCount - 1) * self._config._slotCols
  if true == isUp and index <= UpScrollIndexRange then
    self:Scroll(true)
  elseif false == isUp and index >= DownScrollIndexRange then
    self:Scroll(false)
  end
end
...
function PaGlobal_ChannelChat:setTextureControl(control, stringHashKey)
  ...
  if "LUA_CHANNELCHAT_ROOM_DESC_29" == stringHashKey or "LUA_CHANNELCHAT_ROOM_DESC_30" == stringHashKey or "LUA_CHANNELCHAT_ROOM_DESC_31" == stringHashKey or "LUA_CHANNELCHAT_ROOM_DESC_32" == stringHashKey or "LUA_CHANNELCHAT_ROOM_DESC_33" == stringHashKey or "LUA_CHANNELCHAT_ROOM_DESC_34" == stringHashKey or "LUA_CHANNELCHAT_ROOM_DESC_35" == stringHashKey then
    control:ChangeTextureInfoName("Combine/Etc/Combine_Etc_ChannelChat_01.dds")
  end
  _PA_ASSERT_NAME(CppEnums.NewClassType == __eClassType_PRSA, "\236\131\136\235\161\156\236\154\180 ClassType \235\163\168\236\149\132 \236\158\145\236\151\133 \236\139\156 \237\153\149\236\157\184\237\149\180\236\163\188\236\132\184\236\154\148. \236\139\160\234\183\156 \237\129\180\235\158\152\236\138\164 \235\140\128\237\153\148\234\183\184\235\163\185 \237\133\141\236\138\164\236\179\144 \234\178\189\235\161\156 \236\132\164\236\160\149", "\236\161\176\234\180\145\235\175\188")
...
function PaGlobal_ChannelChat:addChannelList(index)
  if self._ui.btn_title[index] ~= nil then
    return
  end
  local btn_title = UI.getChildControl(Panel_Window_ChannelChat, "RadioButton_Channel_Chat_temp")
  local cloneSlot = {}
  cloneSlot.btn_title = UI.cloneControl(btn_title, Panel_Window_ChannelChat, "ChannelChat_Title_" .. index)
  cloneSlot.stc_title = UI.getChildControl(cloneSlot.btn_title, "StaticText_Title")
  cloneSlot.stc_desc = UI.getChildControl(cloneSlot.btn_title, "StaticText_Money")
  cloneSlot.btn_radio = UI.getChildControl(cloneSlot.btn_title, "Button_Radio")
  cloneSlot.btn_bg = UI.getChildControl(cloneSlot.btn_title, "Static_DecoBG")
  if 0 ~= index then
    local slotPosX = cloneSlot.btn_title:GetPosX()
    local slotPosY = cloneSlot.btn_title:GetPosY()
    if 1 == index % 2 then
      slotPosX = slotPosX + 10 + cloneSlot.btn_title:GetSizeX()
      slotPosY = slotPosY + (10 + cloneSlot.btn_title:GetSizeY()) * ((index - 1) / 2)
    else
      slotPosY = slotPosY + (10 + cloneSlot.btn_title:GetSizeY()) * (index / 2)
    end
    cloneSlot.btn_title:SetPosXY(slotPosX, slotPosY)
    cloneSlot.btn_title:SetShow(true)
  end
  local x1, y1, x2, y2 = 0, 0, 0, 0
  local textureKey = "LUA_CHANNELCHAT_ROOM_DESC_01"
  local baseTextureUV = self._bg_UV[textureKey]
  ...
  local baseTexture = cloneSlot.btn_bg:getBaseTexture()
  cloneSlot.btn_bg:ChangeTextureInfoName("Combine/Etc/Combine_Etc_ChannelChat.dds")
  x1, y1, x2, y2 = setTextureUV_Func(cloneSlot.btn_bg, baseTextureUV.x1, baseTextureUV.y1, baseTextureUV.x2, baseTextureUV.y2)
  baseTexture:setUV(x1, y1, x2, y2)
  cloneSlot.btn_bg:setRenderTexture(baseTexture)
  cloneSlot.btn_title:SetShow(false)
  self._ui.btn_title[index] = cloneSlot.btn_title
  self._ui.stc_title[index] = cloneSlot.stc_title
  self._ui.stc_desc[index] = cloneSlot.stc_desc
  self._ui.btn_radio[index] = cloneSlot.btn_radio
  self._ui.btn_bg[index] = cloneSlot.btn_bg
  self._ui.btn_title[index]:addInputEvent("Mouse_LUp", "HandleEventLUp_Select_ChannelChat(" .. index .. ")")
  self._ui.btn_radio[index]:addInputEvent("Mouse_LUp", "HandleEventLUp_Select_ChannelChat(" .. index .. ")")
  UIScroll.InputEventByControl(self._ui.btn_title[index], "PaGlobal_ChannelChat:Scroll")
  UIScroll.InputEventByControl(self._ui.stc_desc[index], "PaGlobal_ChannelChat:Scroll")
  ...
  self._ui.btn_title[index]:isValidate()
  self._ui.stc_title[index]:isValidate()
  self._ui.stc_desc[index]:isValidate()
  self._ui.btn_radio[index]:isValidate()
  self._ui.btn_bg[index]:isValidate()
end
