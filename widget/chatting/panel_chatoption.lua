...
local isArshaOpen = _ContentsGroup_Arsha
...
local channel_ChannelChatting = false
local channel_GuildManager = false
...
local color_ChannelChatting = UI_color.C_FF55a1dd
local color_GuildManager = UI_color.C_FFC694FF
...
local channel_SystemEnchant = false
...
local rdo_FontSizeSmall = UI.getChildControl(fontSizeBG, "RadioButton_FontSizeSmall")
local rdo_FontSizeSmall2 = UI.getChildControl(fontSizeBG, "RadioButton_FontSizeSmall2")
local rdo_FontSizeNormal = UI.getChildControl(fontSizeBG, "RadioButton_FontSizeNormal")
local rdo_FontSizeNormal2 = UI.getChildControl(fontSizeBG, "RadioButton_FontSizeNormal2")
local rdo_FontSizeBig = UI.getChildControl(fontSizeBG, "RadioButton_FontSizeBig")
...
PaGlobal_Util_RegistHelpTooltipEvent(_buttonQuestion, "\"Chatting\"")
...
local divisionPosY = _check_Division:GetPosY()
...
local checkTimePosY = _checkButton_ChatTime:GetPosY()
...
local channelChattingTitleBg = UI.getChildControl(Panel_ChatOption, "Static_NewChannelTitleBg")
local channelChattingOptionBg = UI.getChildControl(Panel_ChatOption, "Static_NewChannelOptionBg")
local comboBox_ChannelChattingFilterBg = UI.getChildControl(channelChattingOptionBg, "Combobox_ChatServer")
local comboBox_List = UI.getChildControl(comboBox_ChannelChattingFilterBg, "Combobox_List")
local comboBox_selectItem = UI.getChildControl(comboBox_List, "Combobox_1_List_BackStatic")
local stc_systemFilterLine = UI.getChildControl(Panel_ChatOption, "Static_YLine")
...
function HandleClicked_ChattingTypeFilter_ChannelChatting(panelIdex)
  local self = chatOptionData
  local check = btnFilter[eChatButtonType.eChatChannelChatting].chatFilter:IsCheck()
  channel_ChannelChatting = check
  TooltipSimple_Hide()
  RegisterUpdate_ChatOption()
end
function HandleClicked_ChattingTypeFilter_GuildManager(panelIdex)
  local self = chatOptionData
  local check = btnFilter[eChatButtonType.eChatGuildManager].chatFilter:IsCheck()
  channel_GuildManager = check
  TooltipSimple_Hide()
  RegisterUpdate_ChatOption()
end
...
function HandleClicked_ChattingColor_ChannelChatting(panelIndex)
  local self = chatOptionData
  local checkColor = btnFilter[eChatButtonType.eChatChannelChatting].chatColor:SetColor(UI_color.C_FF55a1dd)
  color_ChannelChatting = checkColor
  FGlobal_ChattingColor_GetColor(panelIndex, UI_CT.Channel, eChatButtonType.eChatChannelChatting, false)
  TooltipSimple_Hide()
end
function HandleClicked_ChattingColor_GuildManager(panelIndex)
  local self = chatOptionData
  local checkColor = btnFilter[eChatButtonType.eChatGuildManager].chatColor:SetColor(UI_color.C_FFC694FF)
  color_GuildManager = checkColor
  FGlobal_ChattingColor_GetColor(panelIndex, UI_CT.GuildManager, eChatButtonType.eChatGuildManager, false)
  TooltipSimple_Hide()
end
...
function FGlobal_ChatOption_ChatFilterInitialize(chat, panelIndex)
  ...
  if true == _ContentsGroup_ChannelChat then
    btnFilter[eChatButtonType.eChatChannelChatting].chatFilter:SetCheck(chat:isShowChatType(UI_CT.Channel))
  end
  btnFilter[eChatButtonType.eChatGuildManager].chatFilter:SetCheck(chat:isShowChatType(UI_CT.GuildManager))
  btnSystemFilter[eChatSystemButtonType.eChatSystem].chatFilter:SetCheck(chat:isShowChatType(chatSystemType[eChatSystemButtonType.eChatSystem]))
  for index = 1, chatOptionData.useSystemFilterCount - 1 do
    btnSystemFilter[index].chatFilter:SetCheck(chat:isShowChatSystemType(chatSystemType[index]))
  end
  ...
  if true == _ContentsGroup_ChannelChat then
    channel_ChannelChatting = chat:isShowChatType(UI_CT.Channel)
  end
  channel_GuildManager = chat:isShowChatType(UI_CT.GuildManager)
end
...
function FGlobal_ChatOption_ChannelChattingInitialize()
  if false == _ContentsGroup_ChannelChat then
    channelChattingTitleBg:SetShow(false)
    channelChattingOptionBg:SetShow(false)
    return
  end
  channelChattingTitleBg:SetShow(false)
  channelChattingOptionBg:SetShow(false)
  comboBox_ChannelChattingFilterBg:SetShow(false)
end
function HandleEventLUp_ChannelChatting_ToggleFilter()
  comboBox_ChannelChattingFilterBg:ToggleListbox()
end
function HandleEventLUp_ChannelChatting_SelectChannel()
  comboBox_ChannelChattingFilterBg:SetSelectItemIndex(comboBox_ChannelChattingFilterBg:GetSelectIndex())
  comboBox_ChannelChattingFilterBg:ToggleListbox()
end
function setEnableSystemChildButton(enabled)
  ...
  for idx = 1, self.useSystemFilterCount - 1 do
...
function HandleClicked_ChattingSystemTypeFilter_Undefine(useIndex)
  local self = chatOptionData
  local check = btnSystemFilter[useIndex].chatFilter:IsCheck()
  channel_SystemUndefine = check
  RegisterUpdate_ChatOption()
end
function HandleClicked_ChattingSystemTypeFilter_PrivateItem(useIndex)
  local self = chatOptionData
  local check = btnSystemFilter[useIndex].chatFilter:IsCheck()
  channel_SystemPrivateItem = check
  RegisterUpdate_ChatOption()
end
function HandleClicked_ChattingSystemTypeFilter_PartyItem(useIndex)
  local self = chatOptionData
  local check = btnSystemFilter[useIndex].chatFilter:IsCheck()
  channel_SystemPartyItem = check
  RegisterUpdate_ChatOption()
end
function HandleClicked_ChattingSystemTypeFilter_Market(useIndex)
  local self = chatOptionData
  local check = btnSystemFilter[useIndex].chatFilter:IsCheck()
  channel_SystemMarket = check
  RegisterUpdate_ChatOption()
end
function HandleClicked_ChattingSystemTypeFilter_Worker(useIndex)
  local self = chatOptionData
  local check = btnSystemFilter[useIndex].chatFilter:IsCheck()
  channel_SystemWorker = check
  RegisterUpdate_ChatOption()
end
function HandleClicked_ChattingSystemTypeFilter_Harvest(useIndex)
  local self = chatOptionData
  local check = btnSystemFilter[useIndex].chatFilter:IsCheck()
  channel_SystemHarvest = check
  RegisterUpdate_ChatOption()
end
function HandleClicked_ChattingSystemTypeFilter_Enchant(useIndex)
  local self = chatOptionData
  local check = btnSystemFilter[useIndex].chatFilter:IsCheck()
  channel_SystemEnchant = check
  RegisterUpdate_ChatOption()
end
function createCheckBoxButton(btnObject, buttonName, buttonText, isCheck, posX, posY, fontColor)
  btnObject.chatFilter = UI.createControl(__ePAUIControl_CheckButton, _msgFilter_BG, buttonName)
...
function createRadioButton(btnObject, buttonName, posX, posY)
  btnObject.chatColor = UI.createControl(__ePAUIControl_RadioButton, _msgFilter_BG, buttonName)
...
function ChattingOption_Initialize(panelIdex, _transparency, isCombinedMainPanel)
  ...
  if false == _ContentsGroup_ItemMarketKor2 then
    stc_systemFilterLine:ChangeTextureInfoNameAsync("new_ui_common_forlua/window/chatting/chattingline1.dds")
    stc_systemFilterLine:SetSize(11, 138)
  end
  local index = 0
  if not chatPanel[panelIdex] then
    ...
    local posX = 0
    local posY = 0
    self.useSystemFilterCount = 0
    for idx = 0, self.chatSystemFilterCount - 1 do
      local useIndex = self.useSystemFilterCount
      posX = self.slotSystemTypeStartX
      posY = self.slotSystemTypeStartY + useIndex * self.slotGapY
      btnSystemFilter[useIndex] = {}
      if eChatSystemButtonType.eChatSystem == idx then
        createCheckBoxButton(btnSystemFilter[useIndex], "ChatSystemOption_Btn_" .. useIndex .. "_" .. panelIdex, PAGetString(Defines.StringSheet_GAME, "CHATTING_TAB_SYSTEM"), chat:isShowChatType(UI_CT.System), posX, posY, UI_color.C_FFEFEFEF)
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_LUp", "HandleClicked_ChattingTypeFilter_System( " .. useIndex .. " )")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_On", "HandleOn_ChattingOption_Tooltip(true, 11, " .. useIndex .. ")")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_Out", "HandleOn_ChattingOption_Tooltip(false)")
        createRadioButton(btnSystemFilter[useIndex], "ChatSystemOption_Color_" .. useIndex .. "_" .. panelIdex, posX, posY)
        local chatColorIndex = chat:getChatSystemColorIndex(UI_CT.System)
        if -1 == chatColorIndex then
          btnSystemFilter[useIndex].chatFilter:SetFontColor(UI_color.C_FFC4BEBE)
          btnSystemFilter[useIndex].chatColor:SetColor(UI_color.C_FFC4BEBE)
        else
          btnSystemFilter[useIndex].chatFilter:SetFontColor(FGlobal_ColorList(chatColorIndex))
          btnSystemFilter[useIndex].chatColor:SetColor(FGlobal_ColorList(chatColorIndex))
        end
        btnSystemFilter[useIndex].chatColor:addInputEvent("Mouse_LUp", "HandleClicked_ChattingColor_MainSystem( " .. panelIdex .. " )")
        btnSystemFilter[useIndex].chatColor:addInputEvent("Mouse_On", "HandleOn_ChattingOption_Tooltip(true, 98, " .. useIndex .. ")")
        btnSystemFilter[useIndex].chatColor:addInputEvent("Mouse_Out", "HandleOn_ChattingOption_Tooltip(false)")
        btnSystemFilter[useIndex].chatColor:SetPosY(posY)
        btnSystemFilter[useIndex].chatColor:SetPosX(btnSystemFilter[useIndex].chatFilter:GetPosX() + btnSystemFilter[useIndex].chatFilter:GetSizeX() + btnSystemFilter[useIndex].chatFilter:GetTextSizeX() + 10)
        chatSystemType[useIndex] = UI_CT.System
        self.useSystemFilterCount = self.useSystemFilterCount + 1
      elseif eChatSystemButtonType.eChatSystemUndefine == idx then
        createCheckBoxButton(btnSystemFilter[useIndex], "ChatSystemOption_Btn_" .. useIndex .. "_" .. panelIdex, PAGetString(Defines.StringSheet_GAME, "LUA_CHATTING_TAB_SYSTEM_UNDEFINE"), chat:isShowChatSystemType(UI_CST.Undefine), posX + self.slotSystemTypeChildButtonGapX, posY, UI_color.C_FFC4BEBE)
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_LUp", "HandleClicked_ChattingSystemTypeFilter_Undefine( " .. useIndex .. " )")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_On", "HandleOn_ChattingOption_Tooltip(true, 12, " .. useIndex .. ")")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_Out", "HandleOn_ChattingOption_Tooltip(false)")
        chatSystemType[useIndex] = UI_CST.Undefine
        self.useSystemFilterCount = self.useSystemFilterCount + 1
      elseif eChatSystemButtonType.eChatSystemPrivateItem == idx then
        createCheckBoxButton(btnSystemFilter[useIndex], "ChatSystemOption_Btn_" .. useIndex .. "_" .. panelIdex, PAGetString(Defines.StringSheet_GAME, "LUA_CHATTING_TAB_SYSTEM_PRIVATEITEM"), chat:isShowChatSystemType(UI_CST.PrivateItem), posX + self.slotSystemTypeChildButtonGapX, posY, UI_color.C_FFC4BEBE)
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_LUp", "HandleClicked_ChattingSystemTypeFilter_PrivateItem( " .. useIndex .. " )")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_On", "HandleOn_ChattingOption_Tooltip(true, 13, " .. useIndex .. ")")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_Out", "HandleOn_ChattingOption_Tooltip(false)")
        chatSystemType[useIndex] = UI_CST.PrivateItem
        self.useSystemFilterCount = self.useSystemFilterCount + 1
      elseif eChatSystemButtonType.eChatSystemPartyItem == idx then
        createCheckBoxButton(btnSystemFilter[useIndex], "ChatSystemOption_Btn_" .. useIndex .. "_" .. panelIdex, PAGetString(Defines.StringSheet_GAME, "LUA_CHATTING_TAB_SYSTEM_PARTYITEM"), chat:isShowChatSystemType(UI_CST.PartyItem), posX + self.slotSystemTypeChildButtonGapX, posY, UI_color.C_FFC4BEBE)
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_LUp", "HandleClicked_ChattingSystemTypeFilter_PartyItem( " .. useIndex .. " )")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_On", "HandleOn_ChattingOption_Tooltip(true, 14, " .. useIndex .. ")")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_Out", "HandleOn_ChattingOption_Tooltip(false)")
        chatSystemType[useIndex] = UI_CST.PartyItem
        self.useSystemFilterCount = self.useSystemFilterCount + 1
      elseif eChatSystemButtonType.eChatSystemMarket == idx then
        if true == _ContentsGroup_ItemMarketKor2 then
          createCheckBoxButton(btnSystemFilter[useIndex], "ChatSystemOption_Btn_" .. useIndex .. "_" .. panelIdex, PAGetString(Defines.StringSheet_GAME, "LUA_CHATTING_TAB_SYSTEM_MARKET"), chat:isShowChatSystemType(UI_CST.Market), posX + self.slotSystemTypeChildButtonGapX, posY, UI_color.C_FFC4BEBE)
          btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_LUp", "HandleClicked_ChattingSystemTypeFilter_Market( " .. useIndex .. " )")
          btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_On", "HandleOn_ChattingOption_Tooltip(true, 15, " .. useIndex .. ")")
          btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_Out", "HandleOn_ChattingOption_Tooltip(false)")
          chatSystemType[useIndex] = UI_CST.Market
          self.useSystemFilterCount = self.useSystemFilterCount + 1
        end
      elseif eChatSystemButtonType.eChatSystemWorker == idx then
        createCheckBoxButton(btnSystemFilter[useIndex], "ChatSystemOption_Btn_" .. useIndex .. "_" .. panelIdex, PAGetString(Defines.StringSheet_GAME, "LUA_CHATTING_TAB_SYSTEM_WORKER"), chat:isShowChatSystemType(UI_CST.Worker), posX + self.slotSystemTypeChildButtonGapX, posY, UI_color.C_FFC4BEBE)
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_LUp", "HandleClicked_ChattingSystemTypeFilter_Worker( " .. useIndex .. " )")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_On", "HandleOn_ChattingOption_Tooltip(true, 16, " .. useIndex .. ")")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_Out", "HandleOn_ChattingOption_Tooltip(false)")
        chatSystemType[useIndex] = UI_CST.Worker
        self.useSystemFilterCount = self.useSystemFilterCount + 1
      elseif eChatSystemButtonType.eChatSystemHarvest == idx then
        createCheckBoxButton(btnSystemFilter[useIndex], "ChatSystemOption_Btn_" .. useIndex .. "_" .. panelIdex, PAGetString(Defines.StringSheet_GAME, "LUA_CHATTING_TAB_SYSTEM_HARVEST"), chat:isShowChatSystemType(UI_CST.Harvest), posX + self.slotSystemTypeChildButtonGapX, posY, UI_color.C_FFC4BEBE)
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_LUp", "HandleClicked_ChattingSystemTypeFilter_Harvest( " .. useIndex .. " )")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_On", "HandleOn_ChattingOption_Tooltip(true, 17, " .. useIndex .. ")")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_Out", "HandleOn_ChattingOption_Tooltip(false)")
        chatSystemType[useIndex] = UI_CST.Harvest
        self.useSystemFilterCount = self.useSystemFilterCount + 1
      elseif eChatSystemButtonType.eChatSystemEnchant == idx then
        createCheckBoxButton(btnSystemFilter[useIndex], "ChatSystemOption_Btn_" .. useIndex .. "_" .. panelIdex, PAGetString(Defines.StringSheet_GAME, "DIALOG_BUTTON_QUEST_ENCHANT"), chat:isShowChatSystemType(UI_CST.Enchant), posX + self.slotSystemTypeChildButtonGapX, posY, UI_color.C_FFC4BEBE)
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_LUp", "HandleClicked_ChattingSystemTypeFilter_Enchant( " .. useIndex .. " )")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_On", "HandleOn_ChattingOption_Tooltip(true, 35, " .. useIndex .. ")")
        btnSystemFilter[useIndex].chatFilter:addInputEvent("Mouse_Out", "HandleOn_ChattingOption_Tooltip(false)")
        chatSystemType[useIndex] = UI_CST.Enchant
        self.useSystemFilterCount = self.useSystemFilterCount + 1
      else
  ...
  for idx = 0, self.chatFilterCount - 1 do
    ...
    elseif eChatButtonType.eChatNotice == idx then
    ...
    elseif eChatButtonType.eChatChannelChatting == idx then
      if true == _ContentsGroup_ChannelChat then
        local chatColorIndex = chat:getChatColorIndex(UI_CT.Channel)
        if -1 == chatColorIndex then
          btnFilter[idx].chatFilter:SetFontColor(UI_color.C_FF55a1dd)
          btnFilter[idx].chatColor:SetColor(UI_color.C_FF55a1dd)
        else
          btnFilter[idx].chatFilter:SetFontColor(FGlobal_ColorList(chatColorIndex))
          btnFilter[idx].chatColor:SetColor(FGlobal_ColorList(chatColorIndex))
        end
        btnFilter[idx].chatColor:SetPosX(btnFilter[idx].chatFilter:GetPosX() + btnFilter[idx].chatFilter:GetSizeX() + btnFilter[idx].chatFilter:GetTextSizeX() + 10)
        btnFilter[idx].chatColor:addInputEvent("Mouse_LUp", "HandleClicked_ChattingColor_ChannelChatting(" .. panelIdex .. ")")
        btnFilter[idx].chatColor:SetShow(true)
      end
    elseif eChatButtonType.eChatGuildManager == idx then
      local chatColorIndex = chat:getChatColorIndex(UI_CT.GuildManager)
      if -1 == chatColorIndex then
        btnFilter[idx].chatFilter:SetFontColor(UI_color.C_FFC694FF)
        btnFilter[idx].chatColor:SetColor(UI_color.C_FFC694FF)
      else
        btnFilter[idx].chatFilter:SetFontColor(FGlobal_ColorList(chatColorIndex))
        btnFilter[idx].chatColor:SetColor(FGlobal_ColorList(chatColorIndex))
      end
      btnFilter[idx].chatColor:SetPosX(btnFilter[idx].chatFilter:GetPosX() + btnFilter[idx].chatFilter:GetSizeX() + btnFilter[idx].chatFilter:GetTextSizeX() + 10)
      btnFilter[idx].chatColor:addInputEvent("Mouse_LUp", "HandleClicked_ChattingColor_GuildManager(" .. panelIdex .. ")")
      btnFilter[idx].chatColor:SetShow(true)
    end
  end
  local optionLineCount = math.ceil(optionCount / self._slotsCols)
  local lineGapY = 40
  if optionLineCount < self.useSystemFilterCount then
    optionLineCount = self.useSystemFilterCount
  end
  if optionLineCount < 5 then
    optionLineCount = 5
  end
  local addOffset = 0
  local defaultCount = 12
  if defaultCount < optionCount then
    local overCount = optionCount - defaultCount
    local addRow = math.ceil(overCount / 2)
    local gap = 30
    addOffset = addRow * gap
    _check_Division:SetPosY(divisionPosY + addOffset)
    _checkButton_ChatTime:SetPosY(checkTimePosY + addOffset)
  end
  Panel_ChatOption:SetSize(Panel_ChatOption:GetSizeX(), panelSizeY + addOffset + (optionLineCount - 5) * lineGapY)
  _msgFilter_BG:SetSize(_msgFilter_BG:GetSizeX(), msgFilterBg_SizeY + addOffset + (optionLineCount - 5) * lineGapY)
  if true == _ContentsGroup_NameTypeEqually then
    stringHeadTitleBg:SetShow(false)
    stringHeadBg:SetShow(false)
    fontSizeTitleBg:SetSpanSize(fontSizeTitleBg:GetSpanSize().x, stringHeadTitleBg:GetSpanSize().y)
    fontSizeBG:SetSpanSize(fontSizeBG:GetSpanSize().x, stringHeadBg:GetSpanSize().y)
    Panel_ChatOption:SetSize(Panel_ChatOption:GetSizeX(), Panel_ChatOption:GetSizeY() - stringHeadTitleBg:GetSizeY() - stringHeadBg:GetSizeY())
  end
  _button_Confirm:ComputePos()
  _button_Cancle:ComputePos()
  _button_blockList:ComputePos()
  _button_resetColor:ComputePos()
  ...
  FGlobal_ChatOption_ChatFilterInitialize(chat, panelIdex)
  if roleplayTypeOpen then
  ...
  channel_SystemEnchant = chat:isShowChatSystemType(UI_CST.Enchant)
...
function chatFilter_Initialize(panelIdex, self, chat)
  ...
  for idx = 0, self.chatFilterCount - 1 do
    local tempBtn = {}
    tempBtn.chatFilter = UI.createControl(__ePAUIControl_CheckButton, _msgFilter_BG, "ChatOption_Btn_" .. idx .. "_" .. panelIdex)
    CopyBaseProperty(msgFilter_Chkbox, tempBtn.chatFilter)
    tempBtn.chatFilter:SetShow(false)
    tempBtn.chatColor = UI.createControl(__ePAUIControl_RadioButton, _msgFilter_BG, "ChatOption_Color_Btn_" .. idx .. "_" .. panelIdex)
    ...
    elseif eChatButtonType.eChatChannelChatting == idx then
      btnFilter[idx].chatFilter:SetFontColor(UI_color.C_FF55a1dd)
      btnFilter[idx].chatFilter:SetCheck(chat:isShowChatType(UI_CT.Channel))
      btnFilter[idx].chatFilter:SetText(PAGetString(Defines.StringSheet_GAME, "CHATTING_CHANNEL"))
      btnFilter[idx].chatFilter:SetEnableArea(0, 0, btnFilter[idx].chatFilter:GetSizeX() + btnFilter[idx].chatFilter:GetTextSizeX() + 3, btnFilter[idx].chatFilter:GetSizeY())
      btnFilter[idx].chatFilter:addInputEvent("Mouse_LUp", "HandleClicked_ChattingTypeFilter_ChannelChatting( " .. panelIdex .. " )")
      btnFilter[idx].chatColor:addInputEvent("Mouse_LUp", "HandleClicked_ChattingColor_ChannelChatting( " .. panelIdex .. ")")
      btnFilter[idx].chatFilter:addInputEvent("Mouse_On", "HandleOn_ChattingOption_Tooltip(true, 33, " .. idx .. ")")
      btnFilter[idx].chatFilter:addInputEvent("Mouse_Out", "HandleOn_ChattingOption_Tooltip(false)")
      btnFilter[idx].chatFilter:SetShow(_ContentsGroup_ChannelChat)
      btnFilter[idx].chatColor:SetShow(_ContentsGroup_ChannelChat)
    elseif eChatButtonType.eChatGuildManager == idx then
      btnFilter[idx].chatFilter:SetFontColor(UI_color.C_FFC694FF)
      btnFilter[idx].chatFilter:SetCheck(chat:isShowChatType(UI_CT.GuildManager))
      btnFilter[idx].chatFilter:SetText(PAGetString(Defines.StringSheet_GAME, "CHATTING_TAB_GUILDMANAGER"))
      btnFilter[idx].chatFilter:SetShow(true)
      btnFilter[idx].chatFilter:SetEnableArea(0, 0, btnFilter[idx].chatFilter:GetSizeX() + btnFilter[idx].chatFilter:GetTextSizeX() + 3, btnFilter[idx].chatFilter:GetSizeY())
      btnFilter[idx].chatFilter:addInputEvent("Mouse_LUp", "HandleClicked_ChattingTypeFilter_GuildManager( " .. panelIdex .. " )")
      btnFilter[idx].chatColor:addInputEvent("Mouse_LUp", "HandleClicked_ChattingColor_GuildManager( " .. panelIdex .. ")")
      btnFilter[idx].chatFilter:addInputEvent("Mouse_On", "HandleOn_ChattingOption_Tooltip(true, 34, " .. idx .. ")")
      btnFilter[idx].chatFilter:addInputEvent("Mouse_Out", "HandleOn_ChattingOption_Tooltip(false)")
    end
...
function HandledClicked_ChattingColorReset(panelIndex)
  ...
  chat:setChatColor(UI_CT.Channel, -1)
  chat:setChatColor(UI_CT.GuildManager, -1)
  ...
  for idx = 0, self.chatFilterCount - 1 do
    ...
    elseif eChatButtonType.eChatGuildManager == idx and true == _ContentsGroup_ChannelChat then
      local chatColorIndex = chat:getChatColorIndex(UI_CT.GuildManager)
      if -1 == chatColorIndex then
        btnFilter[idx].chatFilter:SetFontColor(UI_color.C_FFC694FF)
        btnFilter[idx].chatColor:SetColor(UI_color.C_FFC694FF)
...
function HandleClicked_ChattingOption_SetFilter(panelIdex)
  ...
  chat:setShowChatType(UI_CT.Channel, channel_ChannelChatting)
  chat:setShowChatType(UI_CT.GuildManager, channel_GuildManager)
  ...
  chat:setShowChatSystemType(UI_CST.Enchant, channel_SystemEnchant)
  ...
  Panel_ChatOption:SetIgnore(true)
  ChattingColor_Hide()
  ToClient_saveChattingPanelOptionLog(panelIdex)
end
...
function ChattingOption_Open(penelIdex, drawPanelIndex, isCombinedMainPanel)
  ...
  FGlobal_ChatOption_ChannelChattingInitialize()
