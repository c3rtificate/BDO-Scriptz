...
function ChattingFilter_InsertFilterString()
  ...
  if _ContentsOption_CH_CheckProhibitedWord == true then
    ToClient_CheckChinaProhibitedWord(filterString, __eSceneIdChattingFilter)
    return
  end
...
function ChattingFilter:registEventHandler()
  ...
  registerEvent("FromClient_CheckChinaProhibitedWord_ChattingFilter", "FromClient_CheckChinaProhibitedWord_ChattingFilter")
end
function ChattingFilter:registMessageHandler()
end
function FromClient_CheckChinaProhibitedWord_ChattingFilter(filteredText)
  ...
  if false == ToClient_InsertBlockStringList(filteredText) then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_CHATTING_FILTER_NOWORD_ACK"))
    self.ui.edit_Filter:SetEditText("", true)
    return
  end
