...
function HandleEventLUp_NPCShop_ALL_BuySome()
  ...
  if PaGlobal_NPCShop_ALL._value._currentTabIndex == PaGlobal_NPCShop_ALL._ENUMTABINDEXSELL then
...
function PaGlobalFunc_NPCShop_ALL_IsExchangeItem(idx, itemWrapper)
  ...
  local isDuelOriogin = itemWrapper:isSetLimitOption(__eItemLimitOptionType_LimitItemUseOriginalDuelItem)
  if true == isDuplicatedForDuel or true == isDuelOriogin then
    return true
  end
...
function PaGlobalFunc_NPCShop_ALL_SellAllEtcItem()
  if nil == Panel_Dialog_NPCShop_All or false == Panel_Dialog_NPCShop_All:GetShow() then
    return
  end
  ...
  playerWrapper:get():requestSellAtOnceItems(toWhereType)
end
function PaGlobalFunc_NPCShop_SilverChange(itemCount)
  if nil == Panel_Dialog_NPCShop_All or false == Panel_Dialog_NPCShop_All:GetShow() then
    return
  end
