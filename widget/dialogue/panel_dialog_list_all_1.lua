...
function PaGlobal_DialogList_All:registEventHandler()
  ...
  if _ContentsGroup_QuestBackEndLoading == true then
    registerEvent("FromClient_QuestBackEndLoadComplete", "FromClient_DialogList_QuestBackEndLoadComplete")
  end
end
function PaGlobal_DialogList_All:resize()
  ...
  local getPanelPosY = getScreenSizeY() - Panel_Npc_Dialog_All:GetSizeY() - Panel_Dialog_List_All:GetSizeY() - 200
  if getPanelPosY <= 0 then
    getPanelPosY = 10
  else
    getPanelPosY = getScreenSizeY() - Panel_Npc_Dialog_All:GetSizeY() - Panel_Dialog_List_All:GetSizeY() - 200
  end
  Panel_Dialog_List_All:SetPosX(getScreenSizeX() / 2 + Panel_Dialog_List_All:GetSizeX() / 2)
  Panel_Dialog_List_All:SetPosY(getPanelPosY)
...
function PaGlobal_DialogList_All:updateDialog(isPropose)
  ...
  if false == self._isConsole and Auto_NotifyChangeDialog ~= nil then
...
function PaGlobal_DialogList_All:dialogTalkControlSet(control, key)
  ...
  local stc_needExplorePoint = UI.getChildControl(control, "Static_NeedExplorePoint")
  ...
  stc_needExplorePoint:SetShow(false)
  ...
  stc_needExplorePoint:SetFontColor(Defines.Color.C_FFDDC39E)
  ...
  if true == dialogButton._enable then
    ...
    stc_needExplorePoint:SetMonoTone(false)
    ...
    stc_needExplorePoint:SetMonoTone(true, true)
  end
  ...
  elseif true == dialogButton:isBuyItemByPoint() then
    setChangeDialogIcon(dialogButton._dialogButtonType)
    self:setNeedExplorePoint(stc_needExplorePoint, stc_spacebarIcon, displayData)
  else
    stc_typeIcon:SetShow(false)
  end
  ...
  if true == displayData:empty() then
    ...
    if true == isNeedThings and true == isNeedItem then
      ...
      txt_needItemText:SetText("x" .. makeDotMoney(needItemCount))
...
function PaGlobal_DialogList_All:dialogQuestControlSet(control, key)
  ...
  local dialogText = dialogButton:getText()
  local displayData = Dialog_getButtonDisplayData(index)
  if _ContentsGroup_QuestBackEndLoading == true and dialogButton ~= nil then
    local isLoadedBackEndStatic = true
    if dialogButton._param == 1 then
      isLoadedBackEndStatic = false
    end
    local buttonQuestSSW = dialogButton:getQuestStaticStatus()
    if buttonQuestSSW ~= nil and (isLoadedBackEndStatic == false or buttonQuestSSW:isHaveBackEndData() == false) then
      local buttonQuestType
      if buttonQuestSSW:isMainQuest() == true then
        buttonQuestType = CppEnums.DialogState.eDialogState_MainQuestList
      else
        buttonQuestType = CppEnums.DialogState.eDialogState_QuestList
      end
      self:pushQuestLoadingData(buttonQuestSSW, buttonQuestType, key)
    end
  end
  local btn_Dialog = UI.getChildControl(control, "RadioButton_Dialog")
  local stc_typeIcon = UI.getChildControl(btn_Dialog, "Static_TypeIcon")
  ...
  if CppEnums.DialogState.eDialogState_QuestList == tostring(dialogButton._linkType) or CppEnums.DialogState.eDialogState_DisplayQuest == tostring(dialogButton._linkType) or CppEnums.DialogState.eDialogState_ProgressQuest == tostring(dialogButton._linkType) then
    if 0 <= dialogButton._dialogButtonType and dialogButton._dialogButtonType < __eQuestType_Count then
      ToClient_ChangeQuestIconByQuestType(stc_typeIcon, dialogButton._dialogButtonType)
      stc_typeIcon:SetShow(true)
    else
      stc_typeIcon:SetShow(false)
    end
...
function PaGlobal_DialogList_All:setNeedEnergy(control, needWp, Wp, isNeedItem, needItem, spacebarIcon)
  ...
  if false == self._isConsole then
    control:SetIgnore(false)
    control:addInputEvent("Mouse_On", "PaGlobalFunc_DialogMain_All_WpContirubuteTooltip(control,true,true)")
    control:addInputEvent("Mouse_Out", "PaGlobalFunc_DialogMain_All_WpContirubuteTooltip(control,true,false)")
  end
end
function PaGlobal_DialogList_All:setNeedExplorePoint(control, spacebarIcon, displayData)
  ...
  if true == displayData:empty() then
    ...
    needPoint = displayData._needPoint
  end
  control:SetShow(true)
  control:SetText(tostring(needPoint) .. "/" .. tostring(explorePoint:getRemainedPoint()))
  local spanSizeX = control:GetSizeX() + control:GetTextSizeX() + spacebarIcon:GetTextSizeX() + 10
  control:SetSpanSize(spanSizeX, 3)
  if false == self._isConsole then
    control:SetIgnore(false)
    control:addInputEvent("Mouse_On", "PaGlobalFunc_DialogMain_All_WpContirubuteTooltip(control,false,true)")
    control:addInputEvent("Mouse_Out", "PaGlobalFunc_DialogMain_All_WpContirubuteTooltip(control,false,false)")
  end
end
...
function PaGlobal_DialogList_All:clickList(index)
  ...
  local function clickDialogButtonReq()
    ...
    if displayData:empty() then
      ...
      if Panel_Dialogue_Itemtake_All ~= nil then
  ...
  if false == dialogButton:isExchange() then
    ...
    elseif CppEnums.DialogButtonType.eDialogButton_Sequence == dialogButton._dialogButtonType and CppEnums.DialogState.eDialogState_Talk == tostring(dialogButton._linkType) then
  ...
  PaGlobal_DialogMain_All:hideGroupEnchantButton()
end
...
function PaGlobal_DialogList_All:effectItemCheck(itemKey)
  if nil == getSelfPlayer() then
    return
  end
  local selfProxy = getSelfPlayer():get()
  ...
  for i = 1, invenSize - 1 do
    if not inventory:empty(i) then
      ...
      if nil ~= itemWrapper then
        ...
        local _itemKey = itemSSW:get()._key:get()
        if itemKey == _itemKey then
          if true == itemWrapper:hasPurchasedItemEffect() then
            return true
          else
            return false
          end
...
function PaGlobal_DialogList_All:clearQuestLoadingTable()
  if Panel_Dialog_List_All == nil or _ContentsGroup_QuestBackEndLoading == false then
    return
  end
  self._questLoadingDataList = Array.new()
end
function PaGlobal_DialogList_All:pushQuestLoadingData(questWrapper, updateListType, s64_listContentKey)
  if Panel_Dialog_List_All == nil or _ContentsGroup_QuestBackEndLoading == false or questWrapper == nil or s64_listContentKey == nil then
    return
  end
  ...
  local newLoadingData = {
    ...
    _updateListType = updateListType,
    _s64_listContentKey = s64_listContentKey
  }
  if self:isExistLoadingDataAnyOne() == true then
    for index = #self._questLoadingDataList, 1, -1 do
      ...
      if value ~= nil then
        ...
        elseif value._updateListType ~= newLoadingData._updateListType then
          table.remove(self._questLoadingDataList, index)
        elseif value._s64_listContentKey == newLoadingData._s64_listContentKey then
          table.remove(self._questLoadingDataList, index)
        end
      end
    end
...
function PaGlobal_DialogList_All:isExistLoadingDataAnyOne()
  if Panel_Dialog_List_All == nil or _ContentsGroup_QuestBackEndLoading == false then
    return false
  end
  if self._questLoadingDataList == nil then
