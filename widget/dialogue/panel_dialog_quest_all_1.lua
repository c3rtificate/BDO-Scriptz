...
function PaGlobal_DialogQuest_All:controlInit_All()
  ...
  self._ui.stc_removeRewardBg = UI.getChildControl(self._ui.stc_rewardMain, "Static_RemoveRewardBG")
  self._ui.txt_removeReward = UI.getChildControl(self._ui.stc_removeRewardBg, "StaticText_Quest_Reward_Remove")
  self._ui.stc_moneyRewardBG = UI.getChildControl(self._ui.stc_rewardMain, "Static_MoneyRewardBG")
  ...
  for index = 0, self._maxColCount - 1 do
    ...
    self._ui.stc_removeRewardList[index] = UI.getChildControl(self._ui.stc_removeRewardBg, "Static_Quest_Reward_Remove" .. tostring(index + 1))
  end
  for index = self._maxColCount, self._maxBaseSlotCount - 1 do
    ...
    self._ui.stc_removeRewardList[index] = UI.cloneControl(self._ui.stc_removeRewardList[cloneControlIndex], self._ui.stc_removeRewardBg, "Static_Quest_Reward_Remove" .. tostring(index + 1))
    ...
    self._ui.stc_removeRewardList[index]:SetSpanSize(self._ui.stc_removeRewardList[index]:GetSpanSize().x, self._ui.stc_removeRewardList[index]:GetSpanSize().y + addSpanY)
    self._ui.stc_basicRewardList[index]:ComputePos()
    self._ui.stc_selectRewardList[index]:ComputePos()
    self._ui.stc_removeRewardList[index]:ComputePos()
    self._ui.stc_basicRewardList[index]:SetShow(false)
    self._ui.stc_selectRewardList[index]:SetShow(false)
    self._ui.stc_removeRewardList[index]:SetShow(false)
  end
  ...
  for index = 0, self._maxRemoveSlotCount - 1 do
    local backRemoveBg = UI.createControl(__ePAUIControl_Static, self._ui.stc_removeRewardList[index], "RemoveRewardBg_" .. index)
    backRemoveBg:SetIgnore(true)
    self._uiBackRemoveRewardBg[index] = backRemoveBg
    local removeSlot = {}
    SlotItem.new(removeSlot, "RemoveReward_" .. index, index, backRemoveBg, self._itemSlotConfig)
    removeSlot:createChild()
    removeSlot.icon:SetPosX(1)
    removeSlot.icon:SetPosY(1)
    removeSlot.icon:SetIgnore(false)
    self._listRemoveRewardSlots[index] = removeSlot
    Panel_Tooltip_Item_SetPosition(index, removeSlot, "Dialog_QuestReward_Remove")
  end
  ...
  self._ui.txt_removeReward:SetTextMode(__eTextMode_AutoWrap)
  ...
  self._ui.txt_removeReward:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_NPC_COMPLETEREMOVEITEM"))
  ...
  self._removeRewardBgSizeY = self._ui.stc_removeRewardBg:GetSizeY()
end
...
function PaGlobal_DialogQuest_All:prepareOpen()
  if Panel_Dialog_Quest_All == nil then
    return
  end
  if Panel_Dialog_Quest_All:GetShow() == true then
...
function PaGlobal_DialogQuest_All:open()
  ...
  Panel_Dialog_Quest_All:SetShow(true, true)
end
...
function PaGlobal_DialogQuest_All:registEventHandler()
  ...
  Panel_Dialog_Quest_All:RegisterShowEventFunc(true, "PaGlobalFunc_DialogQuest_All_ShowAni()")
end
function PaGlobal_DialogQuest_All:resize()
  ...
  local removeRewardBgSizeY = self._removeRewardBgSizeY
  ...
  self._ui.stc_removeRewardBg:SetSize(self._ui.stc_removeRewardBg:GetSizeX(), removeRewardBgSizeY)
  ...
  local function setDialogSizeSpan()
    ...
    PaGlobalFunc_OutsidePanel(Panel_Dialog_Quest_All)
  end
  ...
  local function setSelectRewardSize()
    ...
    local selectRewardBgSizeY = self._ui.stc_selectRewardBg:GetSizeY() + 20
    bottomSizeY = bottomSizeY - selectRewardBgSizeY
    panelSizeY = panelSizeY - selectRewardBgSizeY
    self._ui.stc_rewardMain:SetSize(self._ui.stc_rewardMain:GetSizeX(), bottomSizeY)
    self._ui.stc_selectRewardBg:SetShow(false)
  end
  local function setRemoveRewardSize()
    self._ui.stc_removeRewardBg:SetShow(true)
    self._ui.stc_removeRewardBg:ComputePos()
    if 0 < self._removeRewardCount then
      return
    end
    local removeRewardBgSizeY = self._ui.stc_removeRewardBg:GetSizeY() + 20
    bottomSizeY = bottomSizeY - removeRewardBgSizeY
    panelSizeY = panelSizeY - removeRewardBgSizeY
    self._ui.stc_rewardMain:SetSize(self._ui.stc_rewardMain:GetSizeX(), bottomSizeY)
    self._ui.stc_removeRewardBg:SetShow(false)
  end
  local function setAddRewardSize()
    if self._maxColCount < self._baseRewardCount then
      ...
      self._ui.stc_removeRewardBg:SetPosY(self._ui.stc_selectRewardBg:GetPosY() + self._ui.stc_selectRewardBg:GetSizeY() + 10)
    end
    ...
    if self._maxColCount < self._removeRewardCount then
      local addRowCount = math.ceil(self._removeRewardCount / self._maxColCount) - 1
      local addRowSizeY = addRowCount * self._rewardIconSpanYDiff
      bottomSizeY = bottomSizeY + addRowSizeY
      panelSizeY = panelSizeY + addRowSizeY
      self._ui.stc_rewardMain:SetSize(self._ui.stc_rewardMain:GetSizeX(), bottomSizeY)
      self._ui.stc_removeRewardBg:SetSize(self._ui.stc_removeRewardBg:GetSizeX(), removeRewardBgSizeY + addRowSizeY)
    end
  end
  local function setMoneyRewardSize()
    bottomSizeY = bottomSizeY + 35
    panelSizeY = panelSizeY + 35
    self._ui.stc_rewardMain:SetSize(self._ui.stc_rewardMain:GetSizeX(), bottomSizeY)
    self._ui.stc_moneyRewardBG:SetPosY(10)
    self._ui.stc_basicRewardBg:SetPosY(50)
    if self._maxColCount < self._baseRewardCount then
      local addRowCount = math.ceil(self._baseRewardCount / self._maxColCount) - 1
      local addRowSizeY = addRowCount * self._rewardIconSpanYDiff + 40
      bottomSizeY = bottomSizeY + addRowSizeY
      panelSizeY = panelSizeY + addRowSizeY
      self._ui.stc_rewardMain:SetSize(self._ui.stc_rewardMain:GetSizeX(), bottomSizeY)
      self._ui.stc_basicRewardBg:SetSize(self._ui.stc_basicRewardBg:GetSizeX(), baseRewardBgSizeY + addRowSizeY)
  ...
  if true == self._ui.stc_moneyRewardBG:GetShow() then
    ...
    self._ui.stc_basicRewardBg:SetPosY(10)
  end
  Panel_Dialog_Quest_All:SetSize(Panel_Dialog_Quest_All:GetSizeX(), panelSizeY)
  if true == self._isComplete then
...
function PaGlobal_DialogQuest_All:validate()
  ...
  self._ui.stc_removeRewardBg:isValidate()
  self._ui.txt_removeReward:isValidate()
...
function PaGlobal_DialogQuest_All:rewardDataSet(dialogData)
  ...
  for index = 1, baseCount do
    ...
    _baseRewardList[index]._isMoney = baseReward:isMoney()
    if _baseRewardList[index]._isMoney == true then
      self._ui.stc_moneyRewardBG:SetShow(true)
      self._ui.stc_moneyTextValue:SetText(makeDotMoney(baseReward._itemCount))
    end
  ...
  local removeDataCount = dialogData:getCompleteRemoveDataCount()
  local _completeRemoveItemList = Array.new()
  for index = 1, removeDataCount do
    local completeRemoveItemWrapper = dialogData:getCompleteRemoveItemWrapperAt(index - 1)
    if completeRemoveItemWrapper ~= nil then
      _completeRemoveItemList[index] = Array.new()
      _completeRemoveItemList[index]._itemKey = completeRemoveItemWrapper:getItemEnchantKey()
      _completeRemoveItemList[index]._s64_count = completeRemoveItemWrapper:getItemCount()
    end
  end
  self:setRewardList(_baseRewardList, _selectRewardList, _completeRemoveItemList, questData, dialogData)
end
function PaGlobal_DialogQuest_All:setRewardList(baseReward, selectReward, completeRemoveItemList, questData, dialogData)
  local selfPlayer = getSelfPlayer()
  if nil == selfPlayer then
    return
  end
  ...
  self._removeRewardCount = #completeRemoveItemList
  for index = 0, self._maxSelectSlotCount - 1 do
  ...
  for index = 0, self._maxBaseSlotCount - 1 do
    ...
    if index < self._baseRewardCount then
      if baseReward[index + 1]._isMoney == false then
        local isGrowReward = self:setRewardIcon(self._listBaseRewardSlots[baseRewardSlotIndex], baseReward[index + 1], baseRewardSlotIndex, "main")
        if true == isGrowReward then
          self._uiBackBaseRewardBg[baseRewardSlotIndex]:SetShow(false)
        else
          self._uiBackBaseRewardBg[baseRewardSlotIndex]:SetShow(true)
        end
  ...
  for index = 0, self._maxSelectSlotCount - 1 do
    ...
    if index < self._selectRewardCount then
      ...
      if true == self._isComplete then
        if self._isConsole == true then
          self._listSelectRewardSlots[index].icon:addInputEvent("Mouse_On", "HandleEventLUp_DialogQuest_All_SelectReward(" .. index .. ")")
          self._listSelectRewardSlots[index].icon:addInputEvent("Mouse_Out", "HandleEventMouseOut_DialogQuest_All_SelectReward()")
        else
    ...
    if index < self._selectRewardCount then
      local isExploreExp = false
      if nil ~= dialogData then
        local rewardWrapper = dialogData:getSelectRewardAt(index)
        if nil ~= rewardWrapper and __eRewardExploreExp == rewardWrapper._type then
          local nameString = PAGetString(Defines.StringSheet_GAME, "MAINSTATUS_CONTRIBUTE")
          self._selectRewardItemNameList[index] = nameString .. " " .. tostring(selectReward[index + 1]._exploreExp)
          isExploreExp = true
        end
      end
      if false == isExploreExp then
  ...
  if self._maxColCount < self._removeRewardCount then
    local rowCount = math.ceil(self._removeRewardCount / self._maxColCount)
    local maxShowIndex = self._maxColCount * rowCount
    for index = self._maxColCount, maxShowIndex - 1 do
      if self._ui.stc_removeRewardList[index] ~= nil then
        self._ui.stc_removeRewardList[index]:SetShow(true)
      end
    end
  end
  for index = 0, self._maxRemoveSlotCount - 1 do
    self._uiBackRemoveRewardBg[index]:EraseAllEffect()
    if index < self._removeRewardCount then
      local isHaveEnoughItem = self:setRemoveRewardIcon(self._listRemoveRewardSlots[index], completeRemoveItemList[index + 1], index)
      self._uiBackRemoveRewardBg[index]:SetShow(true)
      self._uiBackRemoveRewardBg[index]:SetMonoTone(not isHaveEnoughItem, not isHaveEnoughItem)
    else
      self._uiBackRemoveRewardBg[index]:SetShow(false)
      self._uiBackRemoveRewardBg[index]:SetMonoTone(false, false)
    end
  end
...
function PaGlobal_DialogQuest_All:setRemoveRewardIcon(uiSlot, completeRemoveItemData, index)
  uiSlot:clearItem()
  uiSlot.icon:SetAlpha(1)
  local itemStatic = getItemEnchantStaticStatus(ItemEnchantKey(completeRemoveItemData._itemKey))
  uiSlot:setItemByStaticStatus(itemStatic, completeRemoveItemData._s64_count, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, true)
  uiSlot._item = completeRemoveItemData._itemKey
  if true == self._isConsole then
    if _ContentsGroup_RenewUI_Tooltip == false then
      uiSlot.icon:addInputEvent("Mouse_On", "")
      uiSlot.icon:addInputEvent("Mouse_Out", "Panel_Tooltip_Item_Show_GeneralStatic(" .. index .. ", \"Dialog_QuestReward_Remove\", false)")
      self._ui.stc_basicRewardList[index]:registerPadEvent(__eConsoleUIPadEvent_Up_X, "Panel_Tooltip_Item_Show_GeneralStatic(" .. index .. ",\"Dialog_QuestReward_Remove\", true)")
    else
      uiSlot.icon:addInputEvent("Mouse_On", "")
      uiSlot.icon:addInputEvent("Mouse_Out", "")
      self._ui.stc_basicRewardList[index]:registerPadEvent(__eConsoleUIPadEvent_Up_X, "HandlePadEventX_DialogQuest_All_ItemTooltip(" .. completeRemoveItemData._itemKey .. ")")
    end
  else
    Panel_Tooltip_Item_SetPosition(index, uiSlot, "Dialog_QuestReward_Remove")
    uiSlot.icon:addInputEvent("Mouse_On", "Panel_Tooltip_Item_Show_GeneralStatic(" .. index .. ", \"Dialog_QuestReward_Remove\", true)")
    uiSlot.icon:addInputEvent("Mouse_Out", "Panel_Tooltip_Item_Show_GeneralStatic(" .. index .. ", \"Dialog_QuestReward_Remove\", false)")
  end
  return ToClient_isHaveEnoughItem(completeRemoveItemData._itemKey, completeRemoveItemData._s64_count)
end
