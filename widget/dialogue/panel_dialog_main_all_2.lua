...
function PaGlobalFunc_DialogMain_All_Close(isSetWait, isTest, isAttacked, calledByQuestBranch)
  PaGlobal_DialogMain_All:prepareClose(isSetWait, isTest, isAttacked, calledByQuestBranch)
end
...
function PaGlobalFunc_DialogMain_All_SubPanelSetShow(isShow)
  ...
  if true == isShow then
    ...
    if PaGlobal_DialogIntimacy_All ~= nil then
      PaGlobalFunc_DialogIntimacy_All_Open()
    end
    if PaGlobal_InterestKnowledge_All ~= nil then
    ...
    if PaGlobal_HorseWithoutOwner ~= nil then
      PaGlobalFunc_Panel_Dialog_HorseWithoutOwner_All_Close()
    end
  end
end
...
function PaGlobalFunc_DialogMain_All_InnerPanelShow(count, targetWindowList)
...
function HandleEventLUp_DialogMain_All_BackClick()
  ...
  if true == _ContentsGroup_NewUI_DialogSearch_All then
    if Panel_Widget_Dialog_Search_All ~= nil and Panel_Widget_Dialog_Search_All:IsShow() then
      PaGlobalFunc_DialogSearch_All_Close()
    end
  elseif false == _ContentsGroup_RenewUI_SearchMode then
    if Panel_Dialog_Search ~= nil and Panel_Dialog_Search:IsShow() then
      searchView_Close()
    end
  elseif PaGlobalFunc_SearchMode_IsSearchMode ~= nil and true == PaGlobalFunc_SearchMode_IsSearchMode() then
    searchView_Close()
  end
  ToClient_SetFilterType(0, false)
  if false == PaGlobal_DialogMain_All._isConsole and FGlobal_BlackSpiritAdventure_Close_CheckUiSub ~= nil then
  ...
  PaGlobal_DialogMain_All._isDialogVoiceTalking = false
...
function HandleEventLUp_DialogMain_All_FuncButton(index, isType)
  ...
  if PaGlobalFunc_InterestKnowledge_All_Close ~= nil then
    PaGlobalFunc_InterestKnowledge_All_Close()
  end
  if PaGlobalFunc_Panel_Dialog_HorseWithoutOwner_All_Close ~= nil then
    PaGlobalFunc_Panel_Dialog_HorseWithoutOwner_All_Close()
  end
  if _ContentsGroup_SpiritGroupEnchant == true then
    PaGlobal_DialogMain_All._ui.button_banner:SetShow(false)
  end
end
...
function HandleEventLUp_DialogMain_All_CheckVoiceAutoContinue(tabKeyCheck)
  if nil == Panel_Npc_Dialog_All then
    return
  end
  if false == _ContentsGroup_DialogVoiceAutoContinue then
    return
  end
  if PaGlobal_DialogMain_All._curLine >= PaGlobal_DialogMain_All._maxLine then
    return
  end
  if false == PaGlobal_DialogMain_All._ui.stc_caption:GetShow() then
    return
  end
  local isCheck = false
  if true == tabKeyCheck then
    isCheck = not PaGlobal_DialogMain_All._ui.chk_caption:IsCheck()
    PaGlobal_DialogMain_All._ui.chk_caption:SetCheck(isCheck)
  else
    isCheck = PaGlobal_DialogMain_All._ui.chk_caption:IsCheck()
  end
  PaGlobal_DialogMain_All._isCacheAutoNextContinue = isCheck
  ToClient_getGameUIManagerWrapper():setLuaCacheDataListBool(__eDialogVoiceAutoContinueOn, isCheck, CppEnums.VariableStorageType.eVariableStorageType_User)
  local tabBtnSpanSizeX = 0
  if true == isCheck then
    PaGlobal_DialogMain_All._ui.stc_captionSequencing:SetShow(true)
    PaGlobal_DialogMain_All._ui.chk_caption:SetCheck(true)
    PaGlobal_DialogMain_All._ui.txt_captionStr:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CAPTION_MENUALSKIP"))
    tabBtnSpanSizeX = PaGlobal_DialogMain_All._ui.txt_captionStr:GetTextSizeX() + PaGlobal_DialogMain_All._ui.txt_captionStr:GetSpanSize().x + PaGlobal_DialogMain_All._ui.stc_caption:GetSpanSize().x
    PaGlobal_DialogMain_All._ui.txt_captionTab:SetSpanSize(tabBtnSpanSizeX, PaGlobal_DialogMain_All._ui.txt_captionTab:GetSpanSize().y)
    PaGlobal_DialogMain_All._ui.stc_captionTabByConsole:SetSpanSize(tabBtnSpanSizeX, PaGlobal_DialogMain_All._ui.stc_captionTabByConsole:GetSpanSize().y)
  else
    PaGlobal_DialogMain_All._ui.stc_captionSequencing:SetShow(false)
    PaGlobal_DialogMain_All._ui.chk_caption:SetCheck(false)
    PaGlobal_DialogMain_All._ui.txt_captionStr:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_CAPTION_AUTOSKIP"))
    tabBtnSpanSizeX = PaGlobal_DialogMain_All._ui.txt_captionStr:GetTextSizeX() + PaGlobal_DialogMain_All._ui.txt_captionStr:GetSpanSize().x + PaGlobal_DialogMain_All._ui.stc_caption:GetSpanSize().x
    PaGlobal_DialogMain_All._ui.txt_captionTab:SetSpanSize(tabBtnSpanSizeX, PaGlobal_DialogMain_All._ui.txt_captionTab:GetSpanSize().y)
    PaGlobal_DialogMain_All._ui.stc_captionTabByConsole:SetSpanSize(tabBtnSpanSizeX, PaGlobal_DialogMain_All._ui.stc_captionTabByConsole:GetSpanSize().y)
  end
  if false == PaGlobal_DialogMain_All._isDialogVoiceTalking then
...
function HandleEventOnOut_DialogMain_All_BtnTextLimitTooltip(isShow, index)
  ...
  if nil == PaGlobal_DialogMain_All._funcBtnList[index]._btn then
    return
  end
  TooltipSimple_Show(PaGlobal_DialogMain_All._funcBtnList[index]._btn, PaGlobal_DialogMain_All._funcBtnList[index]._btn:GetText())
end
...
function FromClient_DialogMain_All_onScreenResize()
  ...
  PaGlobal_DialogMain_All:resize(true)
end
...
function PaGlobalFunc_DialogMain_All_GetFuncPositionNewQuestButton()
  ...
  Position._PosX = PaGlobal_DialogMain_All._funcBtnList[Index]._btn:GetPosX()
  Position._PosY = PaGlobal_DialogMain_All._funcBtnList[Index]._btn:GetPosY()
  return Position
end
...
function PaGlobalFunc_DialogMain_All_MentalGameBtnWpUpdate()
  ...
  for index = 0, funcBtnCount - 1 do
    ...
    if tonumber(functionBtn._param) == CppEnums.ContentsType.Contents_IntimacyGame then
      PaGlobal_DialogMain_All._funcBtnList[index]._btn:SetText(functionBtn:getText() .. " (" .. functionBtn:getNeedWp() .. "/" .. playerWp .. ")")
    end
  end
end
...
function PaGlobalFunc_DialogMain_All_ConsoleKeyGuideSet(isShow, isEvent)
  ...
  local talker = dialog_getTalker()
  if 0 == isAvailableSelecting or nil ~= talker then
    PaGlobal_DialogMain_All._ui_console.stc_guideIconY:SetShow(false)
  else
    PaGlobal_DialogMain_All._ui_console.stc_guideIconY:SetShow(isShow)
  end
...
function PaGlobalFunc_DialogList_All_IsOpenForCaphras()
  return PaGlobal_DialogMain_All._isOpenForCaphras
end
function PaGlobalFunc_DialogList_All_ClearIsOpenForCaphras()
  PaGlobal_DialogMain_All._isOpenForCaphras = false
end
function PaGlobalFunc_DialogMain_All_UpdateDialogText()
  if nil == PaGlobal_DialogMain_All._mainDialog then
    return
  end
  if nil == PaGlobal_DialogMain_All._mainDialog[PaGlobal_DialogMain_All._curLine] then
    return
  end
  ...
  if ToClient_IsDevelopment() == true then
    local hasMentalCardKey = dialogData:isHaveMentalKeyInDesc()
    PaGlobal_DialogMain_All._ui.btn_Mental:SetShow(hasMentalCardKey)
  end
  ...
  if false == PaGlobal_DialogMain_All._isConsole and Auto_NotifyChangeDialog ~= nil then
  ...
  if false == PaGlobal_DialogMain_All._isDialogVoiceTalking then
    PaGlobal_DialogMain_All._ui.stc_caption:SetShow(false)
  end
  return realDialog
end
...
function PaGlobalFunc_DialogMain_All_SetExchangeNpcDialogBeforClose()
  local dialogData = ToClient_GetCurrentDialogData()
  if nil == dialogData then
    ToClient_PopDialogueFlush()
    return
  ...
  PaGlobal_DialogMain_All._prevExchangeDialog = mainDialog
end
function PaGlobalFunc_DialogMain_All_SetNpcDialogReset()
  ...
  if PaGlobal_DialogMain_All._prevExchangeDialog ~= nil then
    mainDialog = PaGlobal_DialogMain_All._prevExchangeDialog
    PaGlobal_DialogMain_All._prevExchangeDialog = nil
  end
  PaGlobal_DialogMain_All._mainDialog = {}
  PaGlobal_DialogMain_All._maxLine = 0
  PaGlobal_DialogMain_All._curLine = 0
...
function PaGlobalFunc_DialogMain_All_RestoreUI(isSetWait)
  ...
  if checkHpAlertPostEvent ~= nil then
...
local questMainAlret = false
local OnAlretTime = 1
local currentTime = 0
function PaGlobalFunc_QuestMainAlretXXX(deltaTime)
  if true == questMainAlret then
    currentTime = currentTime + deltaTime
    if currentTime > OnAlretTime then
      if nil ~= PaGlobal_MainQuestWidget_CheckQuestEffect then
        PaGlobal_MainQuestWidget_CheckQuestEffect()
      end
      currentTime = 0
      questMainAlret = false
    end
  end
end
function PaGlobalFunc_DialogMain_All_ResetRenderMode()
  SetUIMode(Defines.UIMode.eUIMode_Default)
  if nil ~= PaGlobal_MainQuestWidget_CheckQuestNakMessage and true == PaGlobal_MainQuestWidget_CheckQuestNakMessage() then
    questMainAlret = true
  end
  if nil ~= Panel_Npc_Dialog_All then
    PaGlobal_DialogMain_All._renderMode:reset()
...
function FromClient_DialogMain_All_StartDialogVoice()
  if nil == Panel_Npc_Dialog_All then
    return
  end
  if false == _ContentsGroup_DialogVoiceAutoContinue then
  ...
  PaGlobal_DialogMain_All._isDialogVoiceTalking = true
  PaGlobal_DialogMain_All._ui.stc_caption:SetShow(true)
  HandleEventLUp_DialogMain_All_CheckVoiceAutoContinue()
end
function FromClient_DialogMain_All_EndDialogVoiceAndAutoContinue()
  if nil == Panel_Npc_Dialog_All then
    return
  end
  if false == Panel_Npc_Dialog_All:GetShow() then
  ...
  if false == PaGlobal_DialogMain_All._isCacheAutoNextContinue then
    return
  end
  if false == PaGlobal_DialogMain_All._ui.stc_caption:GetShow() then
    return
...
function FromClient_DialogMain_QuestBackEndLoadComplete(s32_questNoRaw)
  ...
  if dialogData:isLoadingMainDialogQuestBackEnd() == false then
    return
  end
  if dialogData:getLoadingMainDialogQuestNoRaw() ~= s32_questNoRaw then
    return
  end
  ToClient_LoadingMainDialogInfoUpdate()
  PaGlobalFunc_DialogMain_All_Open()
end
