function PaGlobal_DialogExchange_All:initialize()
  ...
  if slotNeed.enchantText ~= nil then
  ...
  if slotResult.enchantText ~= nil then
...
function PaGlobal_DialogExchange_All:update()
  ...
  if 0 >= self._dialogListCount then
    PaGlobalFunc_DialogMain_All_SetExchangeNpcDialogBeforClose()
...
function PaGlobal_DialogExchange_All:resize()
  ...
  Panel_Dialog_Exchange_All:SetPosX(getScreenSizeX() / 2 + Panel_Dialog_List_All:GetSizeX() / 2)
  Panel_Dialog_Exchange_All:SetPosY(getPanelPosY)
  if true == self._isConsole then
...
function PaGlobal_DialogExchange_All:dialogNormalNpcControlSet(control, key)
  ...
  if false == isWp then
    local caphrasStr = ""
    if needItemCaphrasLevel > 0 then
    ...
    if needItemEnchantLevel > 15 then
    ...
    if true == self._isConsole and 2 == needItemColorGrade and (false == dialogButton._enable or false == dialogButton._invenPushable or false == dialogButton:checkConditions()) then
      txt_beforeName:SetText(self:getGradeToColorString(0) .. enchantLevelString .. needItemName .. caphrasStr .. "<PAOldColor> x" .. tostring(needItemCount))
    end
    slotNeedItem.icon:SetShow(true)
    stc_wpItem:SetShow(false)
    if needItemCaphrasLevel > 0 then
      ...
      local haveCount = tostring(ToClient_InventoryGetItemCountAll(ItemEnchantKey(needItemStaticKey, needItemEnchantLevel)))
    ...
    if true == self._isConsole and 2 == needItemColorGrade and (false == dialogButton._enable or false == dialogButton._invenPushable or false == dialogButton:checkConditions()) then
      txt_beforeName:SetText(self:getGradeToColorString(0) .. needItemName)
    end
    txt_haveCount:SetText("<PAColor0xFFFFC832>" .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_EXCHANGE_ENERGY", "count", Wp) .. "<PAOldColor>")
