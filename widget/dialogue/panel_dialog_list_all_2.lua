...
function PaGlobalFunc_DialogList_All_ShowAni()
  ...
  moveAni:SetStartPosition(getScreenSizeX() / 2 + Panel_Dialog_List_All:GetSizeX(), getPanelPosY)
  moveAni:SetEndPosition(getScreenSizeX() / 2 + Panel_Dialog_List_All:GetSizeX() / 2, getPanelPosY)
...
function PaGlobalFunc_DialogList_All_HideAni()
  local moveAni = Panel_Dialog_List_All:addMoveAnimation(0, 0.15, CppEnums.PAUI_ANIM_ADVANCE_TYPE.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
  moveAni:SetStartPosition(Panel_Dialog_List_All:GetPosX(), Panel_Dialog_List_All:GetPosY())
  moveAni:SetEndPosition(getScreenSizeX() / 2 + Panel_Dialog_List_All:GetSizeX(), Panel_Dialog_List_All:GetPosY())
  moveAni.IsChangeChild = true
  local aniInfo4 = Panel_Dialog_List_All:addColorAnimation(0, 0.15, CppEnums.PAUI_ANIM_ADVANCE_TYPE.PAUI_ANIM_ADVANCE_COS_HALF_PI)
  aniInfo4:SetStartColor(Defines.Color.C_FFFFFFFF)
  aniInfo4:SetEndColor(Defines.Color.C_00FFFFFF)
  aniInfo4.IsChangeChild = true
  aniInfo4:SetHideAtEnd(true)
end
...
function PaGlobalFunc_DialogMain_All_WpContirubuteTooltip(control, isWp, isShow)
  if nil == control or nil == Panel_Dialog_List_All or false == isShow then
    TooltipSimple_Hide()
    return
  end
  local name = PAGetString(Defines.StringSheet_GAME, "MAINSTATUS_DESC_WP")
  local desc = ""
  if false == isWp then
    name = PAGetString(Defines.StringSheet_GAME, "LUA_CONTRIBUTE_POINT_DESC")
  end
  TooltipSimple_Show(control, name, desc)
end
function FromClient_DialogList_QuestBackEndLoadComplete(s32_questNoRaw)
  if Panel_Dialog_List_All == nil or _ContentsGroup_QuestBackEndLoading == false then
    return
  end
  if Panel_Dialog_List_All:GetShow() == false then
    return
  end
  if PaGlobal_DialogList_All:isExistLoadingDataAnyOne() == false then
    return
  end
  local s64_updateTargetKey, updateTargetListType
  for index = 1, #PaGlobal_DialogList_All._questLoadingDataList do
    local value = PaGlobal_DialogList_All._questLoadingDataList[index]
    if value ~= nil and value._questNoRaw == s32_questNoRaw then
      s64_updateTargetKey = value._s64_listContentKey
      updateTargetListType = value._updateListType
      table.remove(PaGlobal_DialogList_All._questLoadingDataList, index)
      break
    end
  end
  if s64_updateTargetKey == nil or updateTargetListType == nil then
    return
  end
  local updateTargetListContent = PaGlobal_DialogList_All._ui.list2_button:GetContentByKey(s64_updateTargetKey)
  if updateTargetListContent == nil then
    return
  end
  ToClient_QuestDialogButtonTextUpdate(Int64toInt32(s64_updateTargetKey) - 1)
  if updateTargetListType == CppEnums.DialogState.eDialogState_QuestList or updateTargetListType == CppEnums.DialogState.eDialogState_MainQuestList then
    PaGlobalFunc_DialogList_All_UdpateQuestButtonlist(updateTargetListContent, s64_updateTargetKey)
  elseif updateTargetListType == CppEnums.DialogState.eDialogState_Help then
    PaGlobalFunc_DialogList_All_UdpateHelpButtonlist(updateTargetListContent, s64_updateTargetKey)
  elseif updateTargetListType == CppEnums.DialogState.eDialogState_Talk then
    PaGlobalFunc_DialogList_All_UdpateTalkButtonlist(updateTargetListContent, s64_updateTargetKey)
  else
    return
  end
  return
