...
function PaGlobalFunc_DialogQuest_All_ShowAni()
  local moveAni = Panel_Dialog_Quest_All:addMoveAnimation(0, 0.3, CppEnums.PAUI_ANIM_ADVANCE_TYPE.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
  local uiOffsetX = 0
  if true == PaGlobal_DialogQuest_All._isConsole then
    uiOffsetX = (getOriginScreenSizeX() - getScreenSizeX()) / 2
  end
  local getPanelPosY = getScreenSizeY() - Panel_Npc_Dialog_All:GetSizeY() - Panel_Dialog_Quest_All:GetSizeY() - 200
  if getPanelPosY <= 0 then
    getPanelPosY = 10
  else
    getPanelPosY = getScreenSizeY() - Panel_Npc_Dialog_All:GetSizeY() - Panel_Dialog_Quest_All:GetSizeY() - 200
  end
  ...
  if true == PaGlobal_DialogList_All._isConsole then
    Panel_Dialog_Quest_All:SetPosX(getScreenSizeX())
  end
end
...
function HandleEventMouseOut_DialogQuest_All_SelectReward()
...
function HandleEventEnter_DialogQuest_All_SelectConfirmReward()
  local _doConfirmYes = function()
    if true == PaGlobal_DialogQuest_All._isComplete then
      PaGlobalFunc_QuestRewardItemLog(PaGlobal_DialogQuest_All._selectIndex)
    end
...
function PaGlobalFunc_DialogQuest_All_MentalTemp()
  ...
  if hasMentalCardKey == false then
    return
  end
  local size = dialogData:getHaveMentalKeyInDescCount()
  local text = ""
  for i = 0, size - 1 do
    local key = dialogData:getHaveMentalKeyInDescAt(i)
    if key ~= 0 then
      text = text .. "" .. tostring(key) .. " ,"
    end
  end
  if text ~= "" then
    Proc_ShowMessage_Ack("\237\152\132\236\158\172 \235\139\164\236\157\180\236\150\188\235\161\156\234\183\184 desc \236\151\144 \237\149\160\235\139\185\235\144\156  key: " .. tostring(text))
  end
end
