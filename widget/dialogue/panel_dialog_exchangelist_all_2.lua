...
function PaGlobalFunc_DialogExchange_All_ShowAni()
  ...
  local getPanelPosY = getScreenSizeY() - Panel_Npc_Dialog_All:GetSizeY() - Panel_Dialog_Exchange_All:GetSizeY() - 200
  if getPanelPosY <= 0 then
    getPanelPosY = 10
  else
    getPanelPosY = getScreenSizeY() - Panel_Npc_Dialog_All:GetSizeY() - Panel_Dialog_Exchange_All:GetSizeY() - 200
  end
  moveAni:SetStartPosition(getScreenSizeX() / 2 + Panel_Dialog_Exchange_All:GetSizeX(), getPanelPosY)
  moveAni:SetEndPosition(getScreenSizeX() / 2 + Panel_Dialog_Exchange_All:GetSizeX() / 2, getPanelPosY)
...
function PaGlobalFunc_DialogExchange_All_HideAni()
  local moveAni = Panel_Dialog_Exchange_All:addMoveAnimation(0, 0.15, CppEnums.PAUI_ANIM_ADVANCE_TYPE.PAUI_ANIM_ADVANCE_SIN_HALF_PI)
  moveAni:SetStartPosition(Panel_Dialog_Exchange_All:GetPosX(), Panel_Dialog_Exchange_All:GetPosY())
  moveAni:SetEndPosition(getScreenSizeX() / 2 + Panel_Dialog_Exchange_All:GetSizeX(), Panel_Dialog_Exchange_All:GetPosY())
  moveAni.IsChangeChild = true
  local aniInfo4 = Panel_Dialog_Exchange_All:addColorAnimation(0, 0.15, CppEnums.PAUI_ANIM_ADVANCE_TYPE.PAUI_ANIM_ADVANCE_COS_HALF_PI)
...
function HandleEventLUp_ExchangeList_All_ClickButton(index)
  ...
  if PaGlobal_DialogExchange_All._isConsole == false then
  ...
  elseif true == PaGlobal_DialogExchange_All:sealedItemCheck(dialogButton:getNeedItemKey()) then
    ...
    do
      ...
      if (CppEnums.DialogButtonType.eDialogButton_Exchange == dialogButton._dialogButtonType or CppEnums.DialogButtonType.eDialogButton_ExceptExchange == dialogButton._dialogButtonType) and needItemCount > 0 then
        do
          local resultItemWrapper = getItemEnchantStaticStatus(ItemEnchantKey(dialogButton:getResultItemKeyRaw()))
          local checkInventoryMaxCount = false
          if nil == resultItemWrapper then
            ...
            if resultItemWrapper:isStackable() == false and resultItemWrapper:isApplyDirectly() == false then
              checkInventoryMaxCount = true
            end
          end
          local itemEnchantKey = ItemEnchantKey(dialogButton:getNeedItemKey())
          local itemStaticWrapper = getItemEnchantStaticStatus(itemEnchantKey)
          if nil ~= itemStaticWrapper then
            ...
            local itemCount = 0
            if true == dialogButton:isEnduranceExchangeItemForStack() then
              itemCount = Int64toInt32(ToClient_InventoryGetItemCountAllCheckEndurance(itemEnchantKey))
            else
              itemCount = Int64toInt32(ToClient_InventoryGetItemCountAll(itemEnchantKey))
            end
            local max_exchangeCount = math.floor(itemCount / needItemCount)
            if checkInventoryMaxCount == true then
              max_exchangeCount = math.min(max_exchangeCount, getInventoryFreeCount())
            end
            ...
            local function ExchangeMessageBoxExchangeItemForStack(inputNum)
              local _exchangeCount = Int64toInt32(inputNum)
              local function doExchange()
                local maxCount = _exchangeCount - 1
                for ii = 0, maxCount do
                  dialogData:setExchangeCount(1)
                  clickDialogButtonReq()
                  if ii < maxCount then
                    PaGlobal_DialogExchange_All._isExchange = false
                  elseif nil == resultItemWrapper then
                    PaGlobalFunc_DialogExchange_All_SetResultItemMessage(_exchangeCount, nil)
                  else
                    PaGlobalFunc_DialogExchange_All_SetResultItemMessage(_exchangeCount, resultItemName)
                  end
            ...
            if true == dialogButton:isExchangeItemForStack() or true == dialogButton:isEnduranceExchangeItemForStack() then
              Panel_NumberPad_Show(true, toInt64(0, max_exchangeCount), 0, ExchangeMessageBoxExchangeItemForStack)
              return
            end
