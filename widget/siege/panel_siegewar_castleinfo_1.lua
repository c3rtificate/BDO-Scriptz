...
function PaGlobal_SiegeWar_CastleInfo:prepareOpen()
  ...
  if ToClient_IsVillageSiegeBeing() == true and ToClient_GetMinorSiegeMode() == __eMinorSiegeModeNoReady then
    return
  end
...
function PaGlobal_SiegeWar_CastleInfo:addCastleInfo()
  ...
  local addSizeY = PaGlobal_SiegeWar_AngerGage_GetMainBgSizeY()
    ...
    if 0 ~= householdNo then
      ...
      if nil == self._ui._stc_castleInfoList[ii] and nil ~= buildingInfo then
        ...
        if nil ~= tempControl then
          ...
          UI.setLimitTextAndAddTooltip(castleInfo._stc_castleRegionName)
        end
