...
if true == _ContentsOption_CH_HPMP_Bar_Color then
  RED_A0 = Defines.Color.C_00FFCC1B
  RED_A1 = Defines.Color.C_FFFFCC1B
end
...
local characterStatValue = {
  prevOffence = ToClient_getOffence(),
  prevAwakenOffence = ToClient_getDefence(),
  prevDefence = ToClient_getAwakenOffence()
}
local DamagePanel = {}
...
local function createDamageControl(target)
  ...
  local damageData = UI.createControl(__ePAUIControl_NumStatic, target._panel, "StaticText_Damage")
  ...
  for idx, value in pairs(effectControlSetting) do
    effectControl = UI.createControl(__ePAUIControl_Static, target._panel, value._name)
...
local function createDamagePanel()
  for index = 1, DamagePanel_Count do
    local panel = UI.createPanelAndSetPanelRenderMode("damagePanel_" .. tostring(index), Defines.UIGroup.PAGameUIGroup_ScreenEffect, PAUIRenderModeBitSet({
...
local function SetAnimationControl(targetStatic, posX, posY, timeRate, isShow)
...
local function SetAnimation_CounterAttack(targetStatic, posX, posY, timeRate, isShow)
  targetStatic:SetPosX(posX)
  targetStatic:SetPosY(posY + 500)
  targetStatic:SetShow(isShow)
  targetStatic:SetAlpha(0)
...
local function SetAnimation_KindDamage(targetStatic, posX, posY, timeRate, isShow)
...
local function initialize()
  UpdatePanel = UI.createPanelAndSetPanelRenderMode("Panel_DamageDisplay", uiGruopValue, PAUIRenderModeBitSet({
    Defines.RenderMode.eRenderMode_Default,
    Defines.RenderMode.eRenderMode_SequencePlayerControl
  }))
  UpdatePanel:SetShow(true, false)
...
function DamageOutputFunction_OnDamage(attakeeKeyRaw, effectNumber, effectType, additionalDamageType, posFloat3, isTribeAttack, attackerActorKeyRaw, isNotRandom)
  ...
  local isOnRenderMissEffect = ToClient_getGameOptionControllerWrapper():getRenderMissEffect()
  for _, control in pairs(target.effectList) do
    control:SetShow(false)
    control:ResetVertexAni(true)
  end
  if (101 == effectType or 100 == effectType or 96 == effectType or 97 == effectType or 93 == effectType or 92 == effectType or 91 == effectType) and 0 == effectNumber then
    return
  end
  ...
  elseif 91 == effectType then
    startendColor = WHITE_A0
    middleColor = WHITE_A1
    timeRate = 4
    showSymbol = 1
    isDamageShow = false
  elseif 92 == effectType then
    startendColor = WHITE_A0
    middleColor = WHITE_A1
    timeRate = 4
    showSymbol = 1
  ...
  elseif effectType == 99 then
    ...
    showSymbol = 1
    isDamageShow = false
  elseif effectType == 100 then
    startendColor = WHITE_A0
    middleColor = WHITE_A1
    timeRate = 4
    showSymbol = 1
    isDamageShow = false
  elseif effectType == 101 then
    startendColor = WHITE_A0
    middleColor = WHITE_A1
    timeRate = 4
    showSymbol = 1
  ...
  local isShowTargetPanel = isShowAttackEffect
  if isShowTargetPanel == false and isOnRenderMissEffect == true and (effectType == 5 or effectType == 14) then
    isShowTargetPanel = true
    isDamageShow = false
  end
  targetPanel:SetShow(isShowTargetPanel, false)
  ...
  local dmgMeterOption = ToClient_getGameOptionControllerWrapper():getOnDamageMeter()
  local attackerActor = getCharacterActor(attackerActorKeyRaw)
  local onDmgCondition = false
  if attackerActorKeyRaw == selfPlayer:getActorKey() or nil ~= attackerActor and true == attackerActor:isOwnerSelfPlayer() then
    local attakeeActor = getCharacterActor(attakeeKeyRaw)
    if nil ~= attakeeActor then
      onDmgCondition = checkActiveCondition(attakeeActor:getCharacterKey())
    end
  end
  local onDmgMeter = dmgMeterOption and onDmgCondition and _ContentsGroup_DamageMeter or ToClient_getDmgMeterStateForTest()
  if true == onDmgMeter then
    local nameStatic = target.damage
    local numberWidth = 0
    local numberHeight = 50
    if effectType <= 6 or effectType == 14 then
      setDamageNameStatic(nameStatic, effectNumber, showSymbol, startendColor)
      numberWidth = nameStatic:getNumberWidth()
      SetAnimationControl(nameStatic, -numberWidth / 2, baseY, timeRate, isShowTargetPanel)
  ...
  elseif 1 == effectType or 2 == effectType or 3 == effectType or 4 == effectType then
    ...
    SetAnimationControl(target.effectList[effectType], baseX, baseY, timeRate, isShowTargetPanel)
  elseif effectType == 5 or effectType == 14 then
    local missEffectOffsetY = 50
    baseX = baseX - effectControlSetting[5]._sizeX / 2
    if isOnRenderMissEffect == true then
      SetAnimationControl(target.effectList[5], baseX, baseY - missEffectOffsetY, timeRate, isShowTargetPanel)
    end
    baseY = baseY - 30
  elseif 8 == effectType then
    baseX = baseX - effectControlSetting[6]._sizeX
    SetAnimationControl(target.effectList[6], baseX, baseY - 60, timeRate, isShowTargetPanel)
  elseif 91 == effectType then
  ...
  elseif 96 == effectType then
    ...
    baseY = baseY + 5
    SetAnimationControl(target.effectList[96], baseX, baseY, timeRate, isShowTargetPanel)
  elseif 97 == effectType then
    baseX = baseX - effectControlSetting[97]._sizeX
    baseY = baseY + 5
    SetAnimationControl(target.effectList[97], baseX, baseY, timeRate, isShowTargetPanel)
  elseif 99 == effectType then
    baseX = baseX - effectControlSetting[99]._sizeX
    baseY = baseY + 50
    SetAnimationControl(target.effectList[99], baseX, baseY - 50, timeRate, isShowTargetPanel)
  elseif 100 == effectType then
    baseX = baseX - effectControlSetting[100]._sizeX
    baseY = baseY + 5
    SetAnimationControl(target.effectList[100], baseX, baseY - 50, timeRate, isShowTargetPanel)
  elseif 101 == effectType then
    baseX = baseX - effectControlSetting[101]._sizeX
    baseY = baseY + 5
    SetAnimationControl(target.effectList[101], baseX, baseY - 50, timeRate, isShowTargetPanel)
  else
    UI.ASSERT(false, "\236\157\180\236\131\129\237\149\156 \237\154\168\234\179\188\235\178\136\237\152\184\234\176\128 \235\147\164\236\150\180\236\152\180.")
  end
  if isDamageShow then
    if additionalDamageType == 0 then
      if effectType == 1 or effectType == 14 then
        if attackeeIsSelfPlayer then
      ...
      SetAnimationControl(target.effectList[7], baseX, baseY, timeRate, isShowTargetPanel)
    elseif 1 == additionalDamageType then
      ...
      SetAnimation_CounterAttack(target.effectList[8], baseX, baseY, timeRate, isShowTargetPanel)
    elseif 2 == additionalDamageType then
      ...
      SetAnimation_CounterAttack(target.effectList[9], baseX, baseY, timeRate, isShowTargetPanel)
    elseif 3 == additionalDamageType then
      ...
      SetAnimation_CounterAttack(target.effectList[10], baseX, baseY, timeRate, isShowTargetPanel)
    elseif 4 == additionalDamageType then
      ...
      SetAnimation_CounterAttack(target.effectList[12], baseX, baseY, timeRate, isShowTargetPanel)
    elseif effectType == 1 then
      if attackeeIsSelfPlayer then
      ...
      SetAnimation_CounterAttack(target.effectList[1], baseX, baseY, timeRate, isShowTargetPanel)
    elseif effectType == 14 then
      if attackeeIsSelfPlayer then
        target.effectList[1]:AddEffect("Ui_Damage_Critical", false, 0, 0)
        render_setPointBlur(0.045, 0.05)
      ...
      SetAnimationControl(target.effectList[1], baseX, baseY, timeRate, isShowTargetPanel)
    elseif 99 == additionalDamageType then
    end
  end
  if 101 == effectType or 100 == effectType or 96 == effectType or 97 == effectType or 93 == effectType or 92 == effectType or 91 == effectType then
    ...
    local posOffsetY = 10
    if 101 == effectType or 100 == effectType then
      posOffsetY = -50
    end
    baseX = baseX + effectControlSetting[effectType]._sizeX
    SetAnimationControl(arrowControl, baseX, baseY + posOffsetY, timeRate, isShowTargetPanel)
  end
end
...
function FromClient_AddDamageProcess(attakeeKeyRaw, effectNumber, effectType, additionalDamageType, posFloat3, isTribeAttack, attackerActorKeyRaw)
  if false == ToClient_getGameOptionControllerWrapper():getRenderHitEffectParty() then
    ...
    elseif actorKeyRaw ~= attakeeKeyRaw and actorKeyRaw ~= attackerActorKeyRaw then
      return
    end
  end
...
function FromClient_Damage_ChangeEquipment()
  ToClient_updateAttackStat()
  local nowOffence = ToClient_getOffence()
  local nowDefence = ToClient_getDefence()
  local nowAwakenOffence = ToClient_getAwakenOffence()
  local offenceChanged = characterStatValue.prevOffence ~= nowOffence
  local defenceChanged = characterStatValue.prevDefence ~= nowDefence
  local awakenOffenceChanged = characterStatValue.prevAwakenOffence ~= nowAwakenOffence
  if not offenceChanged and not defenceChanged and not awakenOffenceChanged then
    return
  end
  local selfProxy = getSelfPlayer():get()
  local selfActorKey = getSelfPlayer():getActorKey()
  local selfPosition = float3(selfProxy:getPositionX(), selfProxy:getPositionY(), selfProxy:getPositionZ())
  local attackChanged = offenceChanged or awakenOffenceChanged
  local attackDifference
  local defenceDifference = nowDefence - characterStatValue.prevDefence
  if offenceChanged and not awakenOffenceChanged then
    attackDifference = nowOffence - characterStatValue.prevOffence
  elseif not offenceChanged and awakenOffenceChanged then
    attackDifference = nowAwakenOffence - characterStatValue.prevAwakenOffence
  else
    attackDifference = nowOffence - characterStatValue.prevOffence
  end
  if attackChanged and not defenceChanged then
    DamageOutputFunction_OnDamage(selfActorKey, attackDifference, 91, 10, selfPosition, false, selfActorKey, false)
  elseif defenceChanged and not attackChanged then
    DamageOutputFunction_OnDamage(selfActorKey, defenceDifference, 92, 10, selfPosition, false, selfActorKey, false)
  else
    DamageOutputFunction_OnDamage(selfActorKey, attackDifference, 91, 10, selfPosition, false, selfActorKey, false)
    selfPosition = float3(selfPosition.x, selfPosition.y - 55, selfPosition.z)
    DamageOutputFunction_OnDamage(selfActorKey, defenceDifference, 92, 10, selfPosition, false, selfActorKey, false)
  end
  characterStatValue.prevOffence = nowOffence
  characterStatValue.prevDefence = nowDefence
  characterStatValue.prevAwakenOffence = nowAwakenOffence
end
function FromClient_OceanTendencyChangedValue(actorKeyRaw, oceanTendencyValue)
  local selfWrapper = getSelfPlayer()
  local selfProxy = selfWrapper:get()
  local selfActorKeyRaw = selfWrapper:getActorKey()
  if selfActorKeyRaw == actorKeyRaw and oceanTendencyValue < 0 then
    Proc_ShowMessage_Ack(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_DOWN_INFO", "value", oceanTendencyValue))
  end
end
...
registerEvent("FromClient_OceanTendencyChangedValue", "FromClient_OceanTendencyChangedValue")
...
registerEvent("EventEquipmentUpdate", "FromClient_Damage_ChangeEquipment")
initialize()
