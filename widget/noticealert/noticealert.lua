...
local function createNoticeMsg()
  ...
  for index = 1, _maxNoticeCount do
    local noticePanel = UI.createPanelAndSetPanelRenderMode("Panel_NoticeAlert" .. index, Defines.UIGroup.PAGameUIGroup_GameSystemMenu, PAUIRenderModeBitSet({
    ...
    local bgMsg = UI.createControl(__ePAUIControl_Static, noticePanel, "NoticeAlertBG_" .. index)
    CopyBaseProperty(_bg_Msg, bgMsg)
    bgMsg:SetShow(true)
    local textMsg = UI.createControl(__ePAUIControl_StaticText, noticePanel, "NoticeAlert_Text_" .. index)
...
local function updatePosition()
  ...
  for index = 1, _maxNoticeCount do
    local realIndex = (index + 5) % _maxNoticeCount + 5.5
...
function Proc_NoticeAlert_Ack(message, noticeType, noticeValue)
  if CppEnums.EChatNoticeType.HuntingStart == noticeType or CppEnums.EChatNoticeType.HuntingEnd == noticeType or CppEnums.EChatNoticeType.HuntingAlarm == noticeType or CppEnums.EChatNoticeType.HuntingTimeOut == noticeType then
    if nil ~= message then
  ...
  if CppEnums.EChatNoticeType.LauLauShow == noticeType or CppEnums.EChatNoticeType.LauLauEnd == noticeType then
    if nil ~= message then
      FGlobal_WorldBossShow(message, noticeType, noticeValue)
    end
    return
  ...
  if CppEnums.EChatNoticeType.SavageDefence == noticeType and nil ~= noticeValue and 0 ~= noticeValue then
    PaGlobal_ActionMessage_New_Open(noticeValue, message)
    return
  end
  ...
  if noticeType == CppEnums.EChatNoticeType.Defence and noticeValue ~= 0 and message ~= nil and noticeValue ~= nil then
    if noticeValue == 1 then
      audioPostEvent_SystemUi(30, 8)
      PaGlobal_ActionMessage_New_Open(noticeValue, message)
      return
    elseif noticeValue == 2 then
      audioPostEvent_SystemUi(30, 9)
      PaGlobal_ActionMessage_New_Open(noticeValue, message)
      return
    elseif noticeValue == 3 or noticeValue == 4 then
      PaGlobal_ActionMessage_New_Open(noticeValue, message)
      return
    elseif noticeValue == 5 then
      audioPostEvent_SystemUi(19, 16)
      PaGlobal_ActionMessage_New_Open(noticeValue, message)
      return
    elseif noticeValue == 6 then
      audioPostEvent_SystemUi(25, 4)
      PaGlobal_ActionMessage_New_Open(noticeValue, message)
      return
    elseif noticeValue == 7 or noticeValue == 8 or noticeValue == 9 or noticeValue == 10 then
      PaGlobal_Widget_RabbitLiverScrambleTimer_Close()
      audioPostEvent_SystemUi(25, 4)
      PaGlobal_ActionMessage_New_Open(noticeValue, message)
      return
    end
  end
  if ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_Solare) == true and CppEnums.EChatNoticeType.ActionNakAll == noticeType and nil ~= message and nil ~= noticeValue then
    if 1 == noticeValue then
      audioPostEvent_SystemUi(30, 8)
      PaGlobal_ActionMessage_New_Open(noticeValue, message)
      return
  ...
  if true == PaGlobal_GlobalNak_Open(noticeType, noticeValue, message) then
    return
  end
  if CppEnums.EChatNoticeType.Ocean == noticeType or CppEnums.EChatNoticeType.Invasion == noticeType or CppEnums.EChatNoticeType.Noti == noticeType then
    if nil ~= message then
      local messageType = PaGlobal_RewardSelect_GetNoiceMessageType(noticeType)
      Proc_ShowMessage_Ack_For_RewardSelect(message, 5, messageType)
    end
    return
  end
...
function Proc_InstanceField_NoticeAlert_Ack(message, noticeType, noticeValue)
  ...
  if CppEnums.EChatNoticeType.AtoraxionClear == noticeType then
    if nil == PaGlobalFunc_Atoraxion_NakClearOpen then
      return
    end
    PaGlobalFunc_Atoraxion_NakClearOpen()
    return
  end
  posIndex = posIndex % _maxNoticeCount + 1
  noticeAlertList[posIndex].textMsg:SetFontColor(Chatting_MessageColor(CppEnums.ChatType.Notice, noticeType))
