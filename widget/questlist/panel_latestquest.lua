...
function PaGlobal_LatestQuest:updateVisibleCount()
  self._currentVisibleCount = ToClient_GetLatestQuestShowCount()
end
function PaGlobal_LatestQuest:isShownQuest(questNo)
  for ii = 0, self._currentVisibleCount - 1 do
...
function PaGlobal_LatestQuest:setIcon(index, questType)
  if index < self._maxVisibleCnt then
    ...
    ToClient_ChangeQuestIconByQuestType(questEntry._uiQuestIcon, questType)
  end
end
function PaGlobal_LatestQuest:setTitle(index, groupId, questId)
  if index < self._maxVisibleCnt then
    ...
    questEntry._uiQuestTitle:SetTextMode(__eTextMode_LimitText)
  ...
  if index < self._maxVisibleCnt then
    ...
    if nil ~= uiQuestGroupInfo then
      if true == uiQuestGroupInfo:isGroupQuest() then
        ...
        questEntry._uiGroupTitle:SetTextMode(__eTextMode_LimitText)
...
function PaGlobal_LatestQuest:setConditions(index, PosY, uiQuestInfo)
  if index < self._maxVisibleCnt then
    ...
    if false == uiQuestInfo:isSatisfied() then
      ...
      questEntry._uiCompleteNpc:SetTextMode(__eTextMode_Limit_AutoWrap)
      ...
      FGlobal_ChangeOnTextureForDialogQuestCompleteIcon(questEntry._uiQuestIcon)
...
function PaGlobal_LatestQuest:findShownIndexInLatestQuest(questGroupNo, questId)
  local uiIndex = -1
  local latestQuestShowCount = 0
  if true == _ContentsGroup_NewUI_QuestOption_All then
    latestQuestShowCount = PaGlobal_Quest_Option_All_GetLatestQuestShowCount()
  else
    latestQuestShowCount = FGlobal_CheckedQuestOptionPanel_GetLatestQuestShowCount()
  end
...
function FGlobal_LatestQuest_UpdateVisibleCount()
  PaGlobal_LatestQuest:updateVisibleCount()
end
