...
function LatestQuest:registEventHandler()
  ...
  registerEvent("FromClient_ResetTimeAttackQuest", "PaGlobal_LatestQuest_Update")
  if _ContentsGroup_QuestBackEndLoading == true then
    registerEvent("FromClient_QuestBackEndLoadComplete", "FromClient_LatestQuest_QuestBackEndLoadComplete")
  end
end
function LatestQuest:update()
  ...
  for questIdx = 0, questCount - 1 do
    ...
    if nil == mainQuestNo or questNo._quest ~= mainQuestNo._quest or questNo._group ~= mainQuestNo._group then
      ...
      local currentPosY = 0
      if uiQuestInfo ~= nil and uiQuestInfo:isHaveBackEndData() == false then
        self:pushQuestLoadingData(uiQuestInfo)
      end
  ...
  if uiIdx < currentCount then
    ...
    self._ui.txt_RemainQuest:SetPosY(totalPosY + self._config.questInterval)
  else
...
function LatestQuest:setIconInfo(uiQuestInfo, uiIdx)
  ...
  ToClient_ChangeQuestIconByQuestType(questIcon, uiQuestInfo:getQuestType())
end
function LatestQuest:setConditionInfo(uiQuestInfo, uiIdx)
  ...
  if false == uiQuestInfo:isSatisfied() then
    ...
    if true == isGroupQuest then
      ...
      PosY = PosY + txt_condition[0]:GetTextSizeY() + subGap
    else
      txt_condition[0]:SetPosY(txt_title:GetPosY() + txt_title:GetTextSizeY())
      PosY = PosY + txt_condition[0]:GetTextSizeY() + txt_title:GetTextSizeY()
    end
    ...
    FGlobal_ChangeOnTextureForDialogQuestCompleteIcon(questIcon)
  ...
  if 0 ~= Int64toInt32(time) then
    ...
    PosY = PosY + txt_condition[uiQuestInfo:getDemandCount()]:GetTextSizeY()
  end
...
function LatestQuest:clearQuestLoadingTable()
  if _panel == nil or _ContentsGroup_QuestBackEndLoading == false then
    return
  end
  self._questLoadingDataList = Array.new()
end
function LatestQuest:pushQuestLoadingData(uiQuestInfo)
  if _panel == nil or _ContentsGroup_QuestBackEndLoading == false or uiQuestInfo == nil then
    return
  end
...
function LatestQuest:isExistLoadingDataAnyOne()
  if _panel == nil or _ContentsGroup_QuestBackEndLoading == false then
    return false
  end
  if self._questLoadingDataList == nil then
...
function FromClient_LatestQuest_QuestBackEndLoadComplete(s32_questNoRaw)
  if _panel == nil or _ContentsGroup_QuestBackEndLoading == false then
    return
  end
  ...
  if LatestQuest:isExistLoadingDataAnyOne() == false then
    return
  end
  local isExistUpdateTarget = false
  for index = 1, #LatestQuest._questLoadingDataList do
    local value = LatestQuest._questLoadingDataList[index]
    if value ~= nil and value._questNoRaw == s32_questNoRaw then
      isExistUpdateTarget = true
      table.remove(LatestQuest._questLoadingDataList, index)
      break
    end
  end
  if isExistUpdateTarget == true then
    LatestQuest:update()
  end
end
...
function PaGlobal_LatestQuest_OnResize()
  if PaGlobal_MainQuest_GetPosX == nil or PaGlobal_MainQuest_GetPosY == nil or PaGlobal_MainQuest_GetSizeY == nil then
    return
  end
  local posX = PaGlobal_MainQuest_GetPosX()
  local posY = PaGlobal_MainQuest_GetPosY() + PaGlobal_MainQuest_GetSizeY()
  if PaGlobalFunc_GuildQuestWidget_GetShow ~= nil and PaGlobalFunc_GuildQuestWidget_GetShow() == true then
