...
PaGlobal_MainQuest = {
  ...
  _maxConditionCnt = 5,
  _closeableLevel = 50,
  _isPS4 = ToClient_isPS(),
  _mainQuestGroupNo = 0,
  _mainQuestQuestNo = 0,
  _questLoadingDataList = nil,
  eUPDATE_FUNCTYPE = {UPDATE = 1},
  _isBlabla = false,
  _blabla = {
    [1] = {
      _position = CppEnums.QuickSlotPosition.eDpadDown,
      _questGroup = 21117,
      _questId = 3,
      _message = {
        PAGetString(Defines.StringSheet_GAME, "LUA_MAINQUEST_BlACKSPIRIT_DIALOG_1")
      }
    },
    [2] = {
      _position = CppEnums.QuickSlotPosition.eLTDpadDown,
      _questGroup = 21117,
      _questId = 4,
      _message = {
        PAGetString(Defines.StringSheet_GAME, "LUA_MAINQUEST_BlACKSPIRIT_DIALOG_2")
      }
    }
  }
}
function PaGlobal_MainQuest:initialize()
  ...
  self._uiTooltip:SetTextMode(__eTextMode_AutoWrap)
  self._uiTooltip:SetShow(false)
  self._uiTooltip:SetNotAbleMasking(true)
  self._uiQuestTitle:SetTextMode(__eTextMode_LimitText)
  self._uiQuestCompleteNpc:SetTextMode(__eTextMode_LimitText)
  self._uiQuestGroupTitle:SetTextMode(__eTextMode_LimitText)
...
function PaGlobal_MainQuest:update()
  ...
  if nil ~= uiQuestInfo then
    ...
    if 0 ~= ToClient_GetUiInfo(CppEnums.PAGameUIType.PAGameUIPanel_MainQuest, 0, CppEnums.PanelSaveType.PanelSaveType_IsShow) then
      PaGlobal_MainQuest:OpenMainQuest()
    end
  end
end
...
function PaGlobal_MainQuest:clearQuestLoadingTable()
  if Panel_MainQuest == nil or _ContentsGroup_QuestBackEndLoading == false then
    return
  end
  self._questLoadingDataList = Array.new()
end
function PaGlobal_MainQuest:pushQuestLoadingData(uiQuestInfo, updateFuncType)
...
function PaGlobal_MainQuest:isExistLoadingDataAnyOne()
  if Panel_MainQuest == nil or _ContentsGroup_QuestBackEndLoading == false then
    return false
  end
  if self._questLoadingDataList == nil then
...
function PaGlobal_MainQuest:setIconInfo(uiQuestInfo)
  ...
  ToClient_ChangeQuestIconByQuestType(self._uiQuestTypeIcon, uiQuestInfo:getQuestType())
end
function PaGlobal_MainQuest:setConditionInfo(uiQuestInfo, startPosY)
  ...
  if not uiQuestInfo._isCleared and not uiQuestInfo._isProgressing then
    ...
    self._uiQuestCompleteNpc:SetLineRender(false)
    startPosY = startPosY + self._uiQuestCompleteNpc:GetSizeY()
  elseif 1 == checkCondition then
    for conditionIndex = 0, uiQuestInfo:getDemandCount() - 1 do
      ...
      startPosY = startPosY + uiQuestCondition:GetSizeY()
    end
  elseif 0 == checkCondition then
    ...
    self._uiQuestCompleteNpc:SetLineRender(true)
    self._uiQuestCompleteNpc:SetFontColor(Defines.Color.C_FFF26A6A)
    self._uiQuestCompleteNpc:SetShow(true)
    FGlobal_ChangeOnTextureForDialogQuestCompleteIcon(self._uiQuestTypeIcon)
    startPosY = startPosY + self._uiQuestCompleteNpc:GetSizeY()
    FGlobal_QuestWidget_AutoReleaseNavi(uiQuestInfo)
    self:SetBlabla(uiQuestInfo)
  end
  if nil ~= PaGlobal_QuickMenu_SetShowBlackSpirit and false == self._isBlabla then
    PaGlobal_QuickMenu_SetShowBlackSpirit(false, nil, nil)
  end
...
function HandleClicked_ShowMainQuestDetail(groupId, questId, checkCondition, groupTitle, questGroupCount)
  if false == _ContentsGroup_RenewUI then
    ...
    PaGlobalFunc_QuestInfo_All_Detail(groupId, questId, checkCondition, groupTitle, questGroupCount, true)
...
function PaGlobal_MainQuest:SetBlabla(uiQuestInfo)
  if false == self._isPS4 then
    return
  end
  local questNo = uiQuestInfo:getQuestNo()
  ...
  for index = 1, #self._blabla do
    if self._blabla[index]._questGroup == groupId and self._blabla[index]._questId == questId then
      local descIndex = 1
      if nil ~= PaGlobal_QuickMenu_SetShowBlackSpirit then
        PaGlobal_QuickMenu_SetShowBlackSpirit(true, self._blabla[index]._message[descIndex], self._blabla[index]._position)
      end
      self._isBlabla = true
      break
    end
...
function PaGlobal_MainQuest:setMainQuestNo()
  local questList = ToClient_GetQuestList()
  local uiQuestInfo = questList:getMainQuestInfo()
  if nil ~= uiQuestInfo then
    self._mainQuestGroupNo = uiQuestInfo:getQuestNo()._group
    self._mainQuestQuestNo = uiQuestInfo:getQuestNo()._quest
  end
end
function PaGlobal_MainQuest_SetMainQuestNo()
  PaGlobal_MainQuest:setMainQuestNo()
end
function PaGlobal_MainQuest:isShownQuest(questNo)
  if self._mainQuestGroupNo == questNo._group and self._mainQuestQuestNo == questNo._quest then
    return true
  else
    return false
  end
...
function FromClient_MainQuestWidget_ResetPosition()
  ...
  local relativePosX = ToClient_GetUiInfo(CppEnums.PAGameUIType.PAGameUIPanel_MainQuest, 0, CppEnums.PanelSaveType.PanelSaveType_RelativePositionX)
  local relativePosY = ToClient_GetUiInfo(CppEnums.PAGameUIType.PAGameUIPanel_MainQuest, 0, CppEnums.PanelSaveType.PanelSaveType_RelativePositionY)
  if 0 == ToClient_GetUiInfo(CppEnums.PAGameUIType.PAGameUIPanel_MainQuest, 0, CppEnums.PanelSaveType.PanelSaveType_IsShow) then
    Panel_MainQuest:SetShow(false)
  else
  ...
  if relativePosX == -1 and relativePosY == -1 then
    local initPosX = getOriginScreenSizeX() - Panel_MainQuest:GetSizeX() - 20
  ...
  elseif relativePosX == 0 and relativePosY == 0 or false == _ContentsGroup_RenewUI then
    ...
    Panel_MainQuest:SetRelativePosX(relativePosX)
    Panel_MainQuest:SetRelativePosY(relativePosY)
    Panel_MainQuest:SetPosX(sizeX * relativePosX - Panel_MainQuest:GetSizeX() / 2)
    Panel_MainQuest:SetPosY(sizeY * relativePosY - Panel_MainQuest:GetSizeY() / 2)
  end
...
function FromClient_MainQuestWidget_QuestBackEndLoadComplete(s32_questNoRaw)
  ...
  if PaGlobal_MainQuest:isExistLoadingDataAnyOne() == false then
    return
  end
  local updateFuncType
  for index = 1, #PaGlobal_MainQuest._questLoadingDataList do
    local value = PaGlobal_MainQuest._questLoadingDataList[index]
    if value ~= nil and value._questNoRaw == s32_questNoRaw then
      updateFuncType = value._updateFuncType
      table.remove(PaGlobal_MainQuest._questLoadingDataList, index)
      break
    end
  end
  if PaGlobal_MainQuest:isExistLoadingDataAnyOne() == false then
    ...
    if updateFuncType == PaGlobal_MainQuest.eUPDATE_FUNCTYPE.UPDATE then
      PaGlobal_MainQuest:update()
    else
      return
    end
