...
local checkedQuestWidget = {
  _ui = {
    ...
    _static_BackSpace = UI.getChildControl(_static_active, "Static_BackSpace"),
  ...
  _questIndex = 0,
  _prevPos = 0,
  _recentGroupQuest = false,
  _recentGroupQuestNo = nil,
...
function checkedQuestWidget:hidePanelConditionCheck()
  if nil == Panel_CheckedQuest then
    _PA_ASSERT_NAME(false, "Panel_CheckedQuest\236\157\180 nil \236\158\133\235\139\136\235\139\164.", "\236\160\149\236\167\128\237\152\156")
    return
  end
  ...
  local hideCondition = {
    ...
    ToClient_IsInstanceFieldPlayerbyContentsType(__eInstanceContentsType_MiniGame),
  ...
  if true == bHide then
    Panel_CheckedQuest:SetRenderAndEventHide(true)
  else
    Panel_CheckedQuest:SetRenderAndEventHide(false)
  end
end
function checkedQuestWidget:initControl()
  ...
  for index = 0, self._config._maxQuestListCnt - 1 do
    ...
    elements._uiBackSpace = UI.createAndCopyBasePropertyControl(_static_active, "Static_BackSpace", elements._uiGroupBG, "uiBackSpace_" .. index)
    elements._uiBackSpace:SetShow(false)
    ...
    for jj = 0, self._config._maxConditionCnt - 1 do
      ...
      elements._uiConditions[jj]:SetTextMode(__eTextMode_AutoWrap)
    end
  ...
  self._ui._static_BackSpace:SetShow(false)
end
...
function PaGlobalFunc_CheckedQuestWidget_ShowTitleBG(isShow, index)
  ...
  if true == _ContentsGroup_BackSpaceClearQuestGuide then
    if true == isShow then
      local groupId = checkedQuestWidget._backSpaceQuestData._groupId
      local questId = checkedQuestWidget._backSpaceQuestData._questId
      local uiIndex = checkedQuestWidget:findShownIndexInCheckedQuest(groupId, questId)
      if -1 ~= uiIndex and nil ~= checkedQuestWidget._uiQuestList[uiIndex]._uiBackSpace then
        checkedQuestWidget._uiQuestList[uiIndex]._uiBackSpace:SetShow(false)
      end
    else
      local QusetUi = checkedQuestWidget._uiQuestList[index]
      if nil == QusetUi then
        return
      end
      if nil ~= QusetUi._uiNaviBtn and true == QusetUi._uiNaviBtn:GetShow() or nil ~= QusetUi._uiAutoNaviBtn and true == QusetUi._uiAutoNaviBtn:GetShow() or nil ~= QusetUi._uiHideBtn and true == QusetUi._uiHideBtn:GetShow() or nil ~= QusetUi._uiGiveupBtn and true == QusetUi._uiGiveupBtn:GetShow() then
        return
      end
      PaGlobal_CheckedQuest_UpdateBackSpaceQuest()
    end
...
function FromClient_QuestWidget_UpdateQuestList(groupNo, questNo, isNotRefresh)
  if isNotRefresh == nil then
    checkedQuestWidget:update(groupNo, questNo)
  elseif isNotRefresh == true then
    checkedQuestWidget:updateRenew(groupNo, questNo)
  end
end
function FromClient_QuestWidget_UpdateGuildQuestList()
  checkedQuestWidget:update(nil, nil, true)
end
function checkedQuestWidget:update(groupNo, questNo, isGuildQuest)
  if false == self._isInitialized then
    return
  end
  local tracer1 = Profile_BeginUIFunction("checkedQuestWidget:update-1")
  FGlobal_MainQuest_Update(groupNo, questNo)
  ...
  local tracer3 = Profile_BeginUIFunction("checkedQuestWidget:update-3")
  self:updateQuestList(self._startPosition, groupNo, questNo, isGuildQuest)
  Profile_EndUIFunction(tracer3)
  local tracer4 = Profile_BeginUIFunction("checkedQuestWidget:update-4")
  PaGlobal_SummonBossTutorial_Manager:checkQuestCondition()
  if Panel_LifeTutorial:GetShow() then
    FGlobal_LifeTutorial_Check()
  end
  Profile_EndUIFunction(tracer4)
  local tracer5 = Profile_BeginUIFunction("checkedQuestWidget:update-5")
  PaGlobal_CheckedQuest_UpdateBackSpaceQuest()
  Profile_EndUIFunction(tracer5)
  FromClient_questWidget_ResetPosition()
end
function checkedQuestWidget:updateRenew(groupNo, questNo)
  if false == self._isInitialized then
    return
  end
  FGlobal_MainQuest_UpdateRenew(groupNo, questNo)
  self:updateQuestListRenew(groupNo, questNo)
  PaGlobal_SummonBossTutorial_Manager:checkQuestCondition()
  if Panel_LifeTutorial:GetShow() == true then
    FGlobal_LifeTutorial_Check()
  end
  if PaGlobal_CheckedQuest_UpdateBackSpaceQuest ~= nil then
    PaGlobal_CheckedQuest_UpdateBackSpaceQuest()
  end
end
...
function checkedQuestWidget:checkPosition()
  if nil == Panel_MainQuest or false == Panel_MainQuest:GetShow() then
    return
  end
  local Rect1 = {}
  local Rect2 = {}
...
function checkedQuestWidget:clearQuestLoadingTable()
  if Panel_CheckedQuest == nil or _ContentsGroup_QuestBackEndLoading == false then
    return
  end
  self._questLoadingDataList = Array.new()
end
function checkedQuestWidget:pushQuestLoadingData(uiQuestInfo, updateFuncType)
  if Panel_CheckedQuest == nil or _ContentsGroup_QuestBackEndLoading == false or uiQuestInfo == nil or updateFuncType == nil then
    return
  end
  ...
  local newLoadingData = {
    ...
    _updateFuncType = updateFuncType
  }
  if self:isExistLoadingDataAnyOne() == true then
    for index = #self._questLoadingDataList, 1, -1 do
      ...
      if value ~= nil then
        ...
        elseif value._updateFuncType == newLoadingData._updateFuncType then
          table.remove(self._questLoadingDataList, index)
        end
      end
    end
...
function checkedQuestWidget:isExistLoadingDataAnyOne()
  if Panel_CheckedQuest == nil or _ContentsGroup_QuestBackEndLoading == false then
    return false
  end
  if self._questLoadingDataList == nil then
...
function HandleClicked_QuestWidget_OptionButton()
  if true == _ContentsGroup_NewUI_QuestOption_All then
    PaGlobal_Quest_Option_All_Open()
  else
...
function HandleClieked_GuildQuestWidget_Giveup()
  local isRightGuildQuestComplete = false
  if _ContentsGroup_GuildAuthoritySeparation == true then
    isRightGuildQuestComplete = PaGlobalFunc_Guild_Authorization_RightCheck(__eGuildRightNew_Quest)
  else
    isRightGuildQuestComplete = PaGlobalFunc_CheckedQuest_All_RightCheck(__eGuildRightType_QuestComplete)
  end
  if not isRightGuildQuestComplete then
    if true == _ContentsGroup_GuildRightInfo then
      if _ContentsGroup_GuildAuthoritySeparation == true then
        PaGlobalFunc_Guild_Authorization_ShowErrMessage(__eGuildRightNew_Quest)
      else
        Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_CHECKEDQUEST_GUILDRIGHT_QUEST_GIVEUP"))
      end
    else
...
function QuestWidget_QuestGiveUp_Confirm()
  ...
  PaGlobalFunc_CheckedQuest_CloseQuestInfo()
end
...
function PaGlobalFunc_CheckedQuest_CloseQuestInfo()
...
function checkedQuestWidget:updateQuestList(startPosition, groupNo, questNo, isGuildQuest)
  if false == self._isInitialized then
    return
  end
  ...
  local tracer1 = Profile_BeginUIFunction("checkedQuestWidget:updateQuestList-1")
  Profile_EndUIFunction(tracer1)
  self:clear()
  local tracer2 = Profile_BeginUIFunction("checkedQuestWidget:updateQuestList-2")
  self:doReleaseCheckForTutorial()
  Profile_EndUIFunction(tracer2)
  self._shownListCount = 0
  local tracer3 = Profile_BeginUIFunction("checkedQuestWidget:updateQuestList-3")
  self:setQuestDataList(groupNo, questNo, isGuildQuest)
  Profile_EndUIFunction(tracer3)
  local tracer4 = Profile_BeginUIFunction("checkedQuestWidget:updateQuestList-4")
  self:setQuestUIlist()
  Profile_EndUIFunction(tracer4)
end
function checkedQuestWidget:updateQuestListRenew(groupNo, questNo)
  ...
  self._needLatestQuestUpdate = false
  local latestShowCount = ToClient_GetLatestQuestShowCount()
  if latestShowCount > 1 then
    for idx, quest in pairs(self._latestDataList) do
      if quest.group == groupNo and quest.quest == questNo then
        self._needLatestQuestUpdate = true
      end
    end
  end
  ...
  self:setLatestQuest()
  self:setNormalQuest()
  if self._needLatestQuestUpdate == true then
    self:setLatestQuestUi(self._latestUIIndex)
  else
    local uiQuestInfo = ToClient_GetQuestInfo(groupNo, questNo)
    if uiQuestInfo == nil then
      return
    end
    if _ContentsGroup_QuestBackEndLoading == true and uiQuestInfo:isHaveBackEndData() == false then
      self:pushQuestLoadingData(uiQuestInfo, self.eUPDATE_FUNCTYPE.UPDATE_RENEW)
    end
    for idx, questUI in pairs(self._uiQuestList) do
      if questUI._questNo._groupId == groupNo and questUI._questNo._questId == questNo then
        local questState
        local endTime_s64 = uiQuestInfo:getEndTime()
        if endTime_s64 ~= toInt64(0, 0) then
          if uiQuestInfo:isTimeOver() == true then
            questState = self._config._questState._fail
          elseif uiQuestInfo:isSatisfied() == false or uiQuestInfo:isSatisfied() == true and endTime_s64 >= toInt64(0, 0) then
            questState = self._config._questState._progressing
          else
            questState = self._config._questState._complete
          end
        elseif uiQuestInfo:isSatisfied() == false then
          questState = self._config._questState._progressing
        else
          questState = self._config._questState._complete
        end
        if questState == self._config._questState._complete then
          questUI._uiQuestTitle:EraseAllEffect()
          questUI._uiQuestTitle:AddEffect("UI_Quest_Complete_GoldAura", true, -10, 0)
        elseif questState == self._config._questState._fail then
          questUI._uiQuestTitle:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_WIDGET_CHECKEDQUEST_FAIL_TITLE", "string", string))
        end
        local index = 0
        local demandList = self:setDemandConditions(uiQuestInfo)
        local demandStateList = self:setDemandStateConditions(uiQuestInfo)
        local isDone = questState == self._config._questState._complete
        for conditionIndex, conition in pairs(questUI._uiConditions) do
          conition:SetShow(false)
        end
        for _, demand in pairs(demandList) do
          questUI._uiConditions[index]:SetText(demand)
          questUI._uiConditions[index]:SetShow(true)
          if isDone == true then
            questUI._uiConditions[index]:SetTextMode(__eTextMode_LimitText)
            questUI._uiConditions[index]:SetFontColor(4291617922)
            questUI._uiConditions[index]:SetLineCount(1)
            questUI._uiConditions[index]:SetLineRender(false)
            self:setNaviButtonInfo(questUI, uiQuestInfo:isSatisfied(), uiQuestInfo:isCompleteBlackSpirit())
          elseif demandStateList ~= nil and demandStateList[index + 1] == self._config._questState._complete then
            questUI._uiConditions[index]:SetTextMode(__eTextMode_AutoWrap)
...
function checkedQuestWidget:refreshControlSize()
  local latestQuestStartPosY = 0
  local normalQuestStartPosY = 0
  local count = #self._uiQuestList
  local normalQuestStartIndex = 4
  for i = 0, normalQuestStartIndex - 1 do
    local quest = self._uiQuestList[i]
    local questSizeY = 0
    if quest ~= nil and quest._uiGroupBG:GetShow() == true then
      quest._uiGroupBG:SetPosY(latestQuestStartPosY)
      questSizeY = questSizeY + quest._uiTitleBG:GetSizeY() + 5
      if quest._uiGroupTitle:GetShow() == true then
        questSizeY = questSizeY + quest._uiGroupTitle:GetSizeY() + 5
      end
      for idx, condition in pairs(quest._uiConditions) do
        if condition:GetShow() == true then
          questSizeY = questSizeY + condition:GetTextSizeY() + 2
        end
      end
      if quest._uiEndTime ~= nil and quest._uiEndTime:GetShow() then
        questSizeY = questSizeY + quest._uiEndTime:GetTextSizeY() + 3
      end
      questSizeY = questSizeY + 10
      latestQuestStartPosY = latestQuestStartPosY + questSizeY
      quest._uiGroupBG:SetSize(quest._uiGroupBG:GetSizeX(), questSizeY)
    end
  end
  for i = normalQuestStartIndex, count do
    local quest = self._uiQuestList[i]
    if quest ~= nil and quest._uiGroupBG:GetShow() == true then
      quest._uiGroupBG:SetPosY(normalQuestStartPosY)
      normalQuestStartPosY = normalQuestStartPosY + quest._uiTitleBG:GetSizeY() + 5
      if quest._uiGroupTitle:GetShow() == true then
        normalQuestStartPosY = normalQuestStartPosY + quest._uiGroupTitle:GetSizeY() + 5
      end
      for idx, condition in pairs(quest._uiConditions) do
        if condition:GetShow() == true then
          normalQuestStartPosY = normalQuestStartPosY + condition:GetTextSizeY() + 2
          self._ui._frame_Content:SetSize(self._ui._frame_Content:GetSizeX(), normalQuestStartPosY)
          self._ui._frame_VerticalScroll:SetSize(self._ui._frame_VerticalScroll:GetSizeX(), normalQuestStartPosY)
          self._ui._frame_VerticalScroll:SetPosY(normalQuestStartPosY)
          self._ui._frame_VerticalScroll:SetEnableArea(0, 0, self._ui._frame_VerticalScroll:GetSizeX(), normalQuestStartPosY)
        end
      end
      normalQuestStartPosY = normalQuestStartPosY + 10
    end
  end
end
function checkedQuestWidget:setQuestUIlist()
  ...
  local tracer1 = Profile_BeginUIFunction("checkedQuestWidget:setQuestUIlist-Guild")
  if true == self._isGuildQuestProgressing then
    uiIndex = self:setGuildQuestUi(uiIndex)
  end
  Profile_EndUIFunction(tracer1)
  local tracer2 = Profile_BeginUIFunction("checkedQuestWidget:setQuestUIlist-Normal")
  if true == _ContentsGroup_NormalGroupQuest and true == checkedQuestWidget._recentGroupQuest then
    uiIndex = self:setGroupQuestUi(uiIndex)
  end
  Profile_EndUIFunction(tracer2)
  local tracer3 = Profile_BeginUIFunction("checkedQuestWidget:setQuestUIlist-latest")
  local viewLimitCount = ToClient_GetLatestQuestShowCount()
  if viewLimitCount > 0 then
    self._latestUIIndex = uiIndex
    uiIndex = self:setLatestQuestUi(uiIndex)
  end
  Profile_EndUIFunction(tracer3)
  local tracer4 = Profile_BeginUIFunction("checkedQuestWidget:setQuestUIlist-ETC")
  local posY = 0
  for ii = 0, 3 do
  ...
  Profile_EndUIFunction(tracer4)
  local tracer5 = Profile_BeginUIFunction("checkedQuestWidget:setQuestUIlist-ETC1")
  ...
  Profile_EndUIFunction(tracer5)
  local tracer6 = Profile_BeginUIFunction("checkedQuestWidget:setQuestUIlist-ETC2")
  posY = 0
  ...
  Profile_EndUIFunction(tracer6)
  local tracer7 = Profile_BeginUIFunction("checkedQuestWidget:setQuestUIlist-ETC3")
  self:setShowConditionText()
  Profile_EndUIFunction(tracer7)
  local tracer8 = Profile_BeginUIFunction("checkedQuestWidget:setQuestUIlist-ETC4")
  local widgetType = 0
  if true == _ContentsGroup_NewUI_QuestOption_All then
    widgetType = PaGlobal_Quest_Option_All_GetSelectedWidgetType()
  else
    widgetType = FGlobal_GetSelectedWidgetType()
  end
...
function checkedQuestWidget:setGroupQuestUi(uiIndex)
  local questUi = self._uiQuestList[uiIndex]
  if nil == questUi then
    return uiIndex
  end
  local isDone = true
  if false == self._recentGroupQuest then
    return uiIndex
  end
  if nil == self._recentGroupQuestNo then
    return uiIndex
  end
  questUi._uiGroupBG:SetShow(true)
  local titleString = ToClient_GetGroupQuestTitle(self._recentGroupQuestNo._group, self._recentGroupQuestNo._quest)
  self:setQuestBG(questUi._uiTitleBG, self._config._questType._normal)
  self:setQuestTitle(questUi._uiQuestTitle, titleString, self._config._questState._acceptable)
  self:setQuestTypeIcon(questUi._uiQuestIcon, 0, isDone)
  questUi._uiTitleBG:SetShow(false)
  local startPosition = questUi._uiTitleBG:GetSizeY()
  local textSize = startPosition + 5
  local groupName = ToClient_GetNormalGroupName(self._recentGroupQuestNo._group, self._recentGroupQuestNo._quest)
  local groupTotalCount = ToClient_GetNormalGroupQuestCount(self._recentGroupQuestNo._group, self._recentGroupQuestNo._quest)
  local groupCurrentIndex = ToClient_GetNormalGroupQuestIndex(self._recentGroupQuestNo._group, self._recentGroupQuestNo._quest)
  local uiString = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_GROUPQUEST_WORDS_TITLE", "group_quest_name", groupName, "group_quest_now", tostring(groupCurrentIndex + 1), "group_quest_total", tostring(groupTotalCount))
  questUi._uiConditions[0]:SetText(uiString)
  questUi._uiConditions[0]:SetShow(true)
  questUi._uiConditions[0]:SetPosY(textSize)
  questUi._questNo._groupId = self._recentGroupQuestNo._group
  questUi._questNo._questId = self._recentGroupQuestNo._quest
  textSize = textSize + questUi._uiConditions[0]:GetTextSizeY() + 2
  local index = 0
...
function checkedQuestWidget:setNormalQuestUi(uiIndex)
  for _, data in pairs(self._normalDataList) do
    ...
    local isShow = ToClient_IsGroupQuestShow(questUi._questNo._groupId, questUi._questNo._questId)
    questUi._uiGroupBG:SetShow(isShow)
    if isShow == true then
      questUi._uiTitleBG:SetShow(false)
      local startPosition = questUi._uiTitleBG:GetSizeY()
...
function checkedQuestWidget:setLatestQuestUi(uiIndex)
  if true == _ContentsGroup_NormalGroupQuest then
    for _, data in pairs(self._latestDataList) do
      if nil == self._recentGroupQuestNo then
        break
      end
      if data.group == self._recentGroupQuestNo._group and data.quest == self._recentGroupQuestNo._quest then
        local questUi = self._uiQuestList[uiIndex]
        if nil == questUi then
          return uiIndex
        end
  ...
  for _, data in pairs(self._latestDataList) do
    ...
    if nil == self._recentGroupQuestNo or data.group ~= self._recentGroupQuestNo._group and data.quest ~= self._recentGroupQuestNo._quest then
      local isDone = data.state == self._config._questState._complete
      questUi._uiGroupBG:SetShow(true)
      self:setQuestBG(questUi._uiTitleBG, self._config._questType._latest)
      self:setQuestTitle(questUi._uiQuestTitle, data.QuestName, data.state)
      ...
      questUi._questNo._questId = data.quest
      local textSize = startPosition + 5
      for questIndex, questCondition in pairs(questUi._uiConditions) do
        questCondition:SetShow(false)
      end
      local index = 0
      for _, demandString in pairs(data.demand) do
        questUi._uiConditions[index]:SetText(demandString)
...
function checkedQuestWidget:setGuildQuestUi(uiIndex)
  ...
  self:setQuestTypeIcon(questUi._uiQuestIcon, __eQuestType_Guild, isDone)
  questUi._uiCompleteNpc:SetText("")
...
function checkedQuestWidget:setQuestDataList(groupNo, questNo, isGuildQuest)
  self._guildData = nil
  self:setGuildQuest()
  if true == isGuildQuest then
    return
  end
  self._latestDataList = {}
  self._normalDataList = {}
...
function checkedQuestWidget:setGuildQuest()
  local tracer1 = Profile_BeginUIFunction("checkedQuestWidget:setGuildQuest")
  ...
  if true == questData.isProgressing then
    ...
    if questData.remainTime <= 0 then
...
function checkedQuestWidget:setLatestQuest()
  local tracer1 = Profile_BeginUIFunction("checkedQuestWidget:setLatestQuest")
  local questListInfo = ToClient_GetQuestList()
  ...
  if 0 == viewLimitCount or 0 == questCount then
    ...
    local lastedDataListCount = table.getn(self._latestDataList)
    local viewCount = 0
    for ii = 0, questCount - 1 do
      if viewLimitCount <= lastedDataListCount then
        return
      end
      local questNo = questListInfo:getLatestQuestAt(ii)
      if true == _ContentsGroup_NormalGroupQuest and nil == self._recentGroupQuestNo then
        local currentIndex = ToClient_GetNormalGroupQuestCount(questNo._group, questNo._quest)
        if 0 ~= currentIndex then
          self._recentGroupQuestNo = questNo
        end
      end
      local uiQuestInfo = ToClient_GetQuestInfo(questNo._group, questNo._quest)
      if _ContentsGroup_QuestBackEndLoading == true and uiQuestInfo:isHaveBackEndData() == false then
        self:pushQuestLoadingData(uiQuestInfo, self.eUPDATE_FUNCTYPE.UPDATE_LATEST)
      end
...
function checkedQuestWidget:setNormalQuest()
  local tracer1 = Profile_BeginUIFunction("checkedQuestWidget:setNormalQuest()")
  local questListInfo = ToClient_GetQuestList()
  ...
  for questIndex = 0, self._questGroupCount - 1 do
    ...
    if true == questGroupInfo:isGroupQuest() then
      ...
      local uiQuestInfo = questGroupInfo:getQuestAt(0)
      if _ContentsGroup_QuestBackEndLoading == true and uiQuestInfo:isHaveBackEndData() == false then
        self:pushQuestLoadingData(uiQuestInfo, self.eUPDATE_FUNCTYPE.UPDATE_NORMAL)
      end
...
function checkedQuestWidget:setTargetQuest(uiQuestInfo)
  if nil == uiQuestInfo then
    return
  end
  if _ContentsGroup_QuestBackEndLoading == true and uiQuestInfo:isHaveBackEndData() == false then
    self:pushQuestLoadingData(uiQuestInfo, self.eUPDATE_FUNCTYPE.UPDATE_TARGET)
  end
  local tracer1 = Profile_BeginUIFunction("checkedQuestWidget:setTargetQuest()")
  self:questInfo(uiQuestInfo, true, questIndex, 0, nil, 0, self._config._questType._normal, true)
  Profile_EndUIFunction(tracer1)
end
  ...
  if true == self._welcomeToTheWorld then
      ...
      if nil ~= temp_questGroupInfo then
        if true == temp_questGroupInfo:isGroupQuest() then
          ...
          for questGroupIdx = 0, tempGroupCount - 1 do
            ...
            if tempQuestInfo._isProgressing and not tempQuestInfo._isCleared and self._welcomeToTheWorld and not TutorialQuestCompleteCheck() and (3 == tempQuestInfo:getQuestType() or 4 == tempQuestInfo:getQuestType() or 5 == tempQuestInfo:getQuestType() or 6 == tempQuestInfo:getQuestType()) then
    ...
    if self._welcomeToTheWorld and not TutorialQuestCompleteCheck() and temp_progressCount > 0 then
      ...
      for index = 0, temp_progressCount - 1 do
        if nil ~= self.questNoSaveForTutorial[index]._questGroupIndex and nil ~= self.questNoSaveForTutorial[index]._questGroupIdx then
...
function checkedQuestWidget:questInfo(uiQuestInfo, isSingle, questGroupIndex, questIndex, groupTitle, questGroupCount, questType, isUpdate)
  if false == uiQuestInfo._isCleared and true == uiQuestInfo._isProgressing then
    self:ProgressQuest(uiQuestInfo, isSingle, groupTitle, questGroupCount, questGroupIndex, questType, isUpdate)
  end
end
...
function checkedQuestWidget:isAcceptedQuest(uiQuestInfo, isSingle, groupTitle, questGroupCount, questGroupIndex, questType, isUpdate)
  local questData = {}
  ...
  if true ~= isUpdate and true == isAlreadyShow then
    return
  end
...
function checkedQuestWidget:ProgressQuest(uiQuestInfo, isSingle, groupTitle, questGroupCount, questGroupIndex, questType, isUpdate)
  local questData = {}
  local questNo = uiQuestInfo:getQuestNo()
  local questGroupId = questNo._group
  local questId = questNo._quest
  ...
  if self._config._questType._latest == questType and table.getn(self._latestDataList) < ToClient_GetLatestQuestShowCount() then
    table.insert(self._latestDataList, questData)
  elseif true == isUpdate then
    local index = self:findNormalDataIndex(questData)
    if nil ~= index then
      self._normalDataList[index] = questData
    else
      local isInLatestQuest = false
      for idx, data in pairs(self._latestDataList) do
        if data.quest == questId and data.group == questGroupId then
          isInLatestQuest = true
          break
        end
      end
      if isInLatestQuest == false then
...
function checkedQuestWidget:findNormalDataIndex(questData)
  for index = 1, #self._normalDataList do
    local data = self._normalDataList[index]
    if nil ~= data and data.group == questData.group and data.quest == questData.quest then
      return index
    end
  end
  return nil
...
function checkedQuestWidget:setDemandConditions(uiQuestInfo)
  ...
  elseif true == uiQuestInfo:isCompleteBlackSpirit() then
    demandList[1] = PAGetString(Defines.StringSheet_GAME, "LUA_QUESTCOMPLETE_DESC")
  else
    demandList[1] = PAGetStringParam1(Defines.StringSheet_GAME, "QUESTLIST_COMPLETETARGET", "getCompleteDisplay", uiQuestInfo:getCompleteDisplay())
  end
  return demandList
end
...
function checkedQuestWidget:setQuestTypeIcon(control, questType, isDone)
  ...
  if not isDone then
    ToClient_ChangeQuestIconByQuestType(control, questType)
  else
    FGlobal_ChangeOnTextureForDialogQuestCompleteIcon(control)
  end
end
function checkedQuestWidget:setQuestTitle(control, string, state)
  ...
  control:useGlowFont(true, "RealFont_14_Glow", 4287655978)
  control:EraseAllEffect()
...
function checkedQuestWidget:setVisibleConvenienceButtonByIndex(index, show)
  ...
  local uiElem = self._uiQuestList[index]
  if true == self._recentGroupQuest and uiElem._questNo._groupId == self._recentGroupQuestNo._group and uiElem._questNo._questId == self._recentGroupQuestNo._quest then
    uiElem._uiHideBtn:SetShow(false)
    uiElem._uiAutoNaviBtn:SetShow(false)
    uiElem._uiNaviBtn:SetShow(false)
    uiElem._uiGiveupBtn:SetShow(false)
  ...
  if true == show then
    ...
    if self._config._questType._guild == uiElem._questType then
      ...
      if true == isRightGuildQuestComplete then
...
function checkedQuestWidget:handleMouseOver_HelpPop(show, target, index)
  ...
  if true == show then
    ...
    TooltipSimple_Show(control, "", msg, false)
  else
    TooltipSimple_Hide()
  end
  if true == _ContentsGroup_BackSpaceClearQuestGuide then
...
function FGlobal_QuestWidget_Open()
  FGlobal_QuestWidget_OpenFunc()
  if ToClient_WorldMapIsShow() then
    WorldMapPopupManager:increaseLayer(true)
    WorldMapPopupManager:push(Panel_CheckedQuest, true, FGlobal_QuestWidget_OpenFunc, FGlobal_QuestWidget_CloseFunc)
  end
end
function FGlobal_QuestWidget_OpenFunc()
  Panel_CheckedQuest:SetShow(true, true)
  Panel_MainQuest:SetShow(true, false)
end
function FGlobal_QuestWidget_CloseFunc()
  Panel_CheckedQuest:SetShow(false)
  Panel_MainQuest:SetShow(false)
end
function FGlobal_QuestWidget_OpenSeparate()
  local mainQuestShow = true
  local checkedQuestShow = true
  if PaGlobal_UIModify ~= nil then
    mainQuestShow = PaGlobal_UIModify._panelControl[PaGlobal_UIModify._panelID.MainQuest].isShow
    checkedQuestShow = PaGlobal_UIModify._panelControl[PaGlobal_UIModify._panelID.Quest].isShow
    if mainQuestShow == nil then
      mainQuestShow = false
    end
    if checkedQuestShow == nil then
      checkedQuestShow = false
    end
  end
  Panel_CheckedQuest:SetShow(checkedQuestShow, true)
  Panel_MainQuest:SetShow(mainQuestShow, false)
  if ToClient_WorldMapIsShow() then
    WorldMapPopupManager:increaseLayer(true)
...
function HandleClicked_ShowQuestInfo(questGroupId, questId, questCondition_Chk)
  ...
  PaGlobalFunc_QuestInfo_All_Detail(questGroupId, questId, questCondition_Chk, questGroupTitle, questGroupCount, true)
  HandleEventLUp_Quest_All_SetTabMenu(PaGlobal_Quest_All._TABTYPE.PROGRESS)
...
function checkedQuestWidget:findTarget(questGroupId, questId, condition, isAuto)
  ...
  if questInfo ~= nil and isShowMessage == true then
    ...
    elseif 0 ~= condition and 99 ~= condition then
      local isUIMode = GetUIMode()
      if isUIMode == Defines.UIMode.eUIMode_Default then
        Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_PROCMESSAGE_QUEST_ALEADY_FINDWAY"))
      end
    end
  end
  self._isAutoRun = isAuto
  PaGlobalFunc_Quest_All_UpdateFindWay(questGroupId, questId, isAuto)
end
...
function _QuestWidget_FindTarget_DrawMapPath(questGroupId, questId, condition, isAuto)
  ...
  local questRaw = questInfo:getKey()
  ...
  elseif 99 == condition then
    ...
    local questCompleteConditionCount = questInfo:getDemadCount()
    local questPosCount = questInfo:getQuestPositionCount()
    local questPositionType = questInfo:getQuestPositionType()
    if questPosCount == 0 then
    else
      if nil ~= PaGlobalFunc_Window_MessageBoxElvia_All_Open then
        PaGlobalFunc_Window_MessageBoxElvia_All_Open()
      end
      self._positionList = {}
      self._positionList._questGroupId = questGroupId
      self._positionList._questId = questId
      local progressQuestWrapper = ToClient_getProgressingQuestAtKey(questRaw)
      local totalLength = 0
      for questPositionIndex = 0, questPosCount - 1 do
        ...
        self._positionList[questPositionIndex]._key = nil
        ...
        if questCompleteConditionCount <= questPosCount then
          if progressQuestWrapper ~= nil then
            local questDemandWrapper = progressQuestWrapper:getDemandAt(questPositionIndex)
            if questDemandWrapper ~= nil then
              local isEmptyDemandWrapper = questDemandWrapper._currentCount == 0 and questDemandWrapper._destCount == 0
              if isEmptyDemandWrapper == true or questDemandWrapper._currentCount < questDemandWrapper._destCount then
      ...
      math.random(1, 10)
      if questPosCount == questCompleteConditionCount then
        if questPosCount > 1 then
          local notClearedIndexList = Array.new()
          if progressQuestWrapper ~= nil then
            for idx = 0, questCompleteConditionCount - 1 do
              local questDemandWrapper = progressQuestWrapper:getDemandAt(idx)
              if questDemandWrapper ~= nil and questDemandWrapper._currentCount < questDemandWrapper._destCount then
                notClearedIndexList:push_back(idx)
              end
            end
          end
          local notClearedIndexCount = #notClearedIndexList
          if questPositionType == __eQuestPositionType_Random then
            self._lastGuideIndex = nil
            if notClearedIndexCount > 0 then
              local randomIndex = math.random(1, notClearedIndexCount)
              guideIndex = notClearedIndexList[randomIndex]
            end
          elseif questPositionType == __eQuestPositionType_InOrder then
            if notClearedIndexCount > 0 then
              if self._lastGuideIndex == nil then
                guideIndex = notClearedIndexList[1]
                self._lastGuideIndex = guideIndex
              else
                self._lastGuideIndex = (self._lastGuideIndex + 1) % notClearedIndexCount
                guideIndex = self._lastGuideIndex
              end
            end
          elseif questPositionType == __eQuestPositionType_Unlock then
            self._lastGuideIndex = nil
            if notClearedIndexCount > 0 then
              guideIndex = notClearedIndexList[1]
            end
          else
            UI.ASSERT_NAME(false, "questPositionType\236\157\180 \236\132\164\236\160\149\235\144\152\236\167\128 \236\149\138\236\157\128 \236\157\152\235\162\176\234\176\128 \236\158\136\235\139\164?", "\236\157\180\236\163\188")
          end
        end
      elseif questPosCount > 1 then
        self._lastGuideIndex = nil
      ...
      if guideIndex ~= nil and self._positionList[guideIndex]._key ~= nil and selfPlayer:getNavigationMovePathIndex() ~= self._positionList[guideIndex]._key then
        if ToClient_GetNaviGuideGroupNo(self._positionList[guideIndex]._key) ~= 0 then
          ToClient_DeleteNaviGuideByGroup(0)
        end
        selfPlayer:setNavigationMovePath(self._positionList[guideIndex]._key)
        selfPlayer:checkNaviPathUI(self._positionList[guideIndex]._key)
      end
      audioPostEvent_SystemUi(0, 14)
    end
  end
...
function _QuestWidget_ReturnQuestState()
  return checkedQuestWidget._questGroupId, checkedQuestWidget._questId, checkedQuestWidget._naviInfoAgain, checkedQuestWidget._isAutoRun
end
...
function FromClient_UpdateQuestSetPos(questNoRaw, updateFlag, isSatisfied)
  if false == checkedQuestWidget._isInitialized then
    return
  end
  checkedQuestWidget:updateQuestSetPos()
  if questNoRaw ~= nil and updateFlag ~= nil and updateFlag == __eProgressQuestUpdateFlag_SatisfiedCondition then
    local questInfoWrapper = questList_getQuestInfo(questNoRaw)
    if questInfoWrapper ~= nil and questInfoWrapper:isMainQuest() == true and 0 < questInfoWrapper:getQuestPositionCount() then
      local groupId = questInfoWrapper:getQuestNo()._group
      local questId = questInfoWrapper:getQuestNo()._quest
      local conditionCheck = QuestConditionCheckType.eQuestConditionCheckType_Progress
      if checkedQuestWidget._naviInfoAgain == false then
        HandleClicked_QuestWidget_FindTarget(groupId, questId, conditionCheck, false)
      end
      HandleClicked_QuestWidget_FindTarget(groupId, questId, conditionCheck, false)
    end
  end
end
function FromClient_AcceptNormalGroupQuestInformation(questNo)
  if false == _ContentsGroup_NormalGroupQuest then
    return
  end
  if nil == checkedQuestWidget._recentGroupQuestNo then
    return
  end
  if checkedQuestWidget._recentGroupQuestNo._group == questNo._group and checkedQuestWidget._recentGroupQuestNo._quest == questNo._quest then
    checkedQuestWidget._recentGroupQuest = false
  end
end
function FromClient_ClearNormalGroupQuestInformation(questNo)
  if false == _ContentsGroup_NormalGroupQuest then
    return
  end
  local nextQuestNo = ToClient_GetGroupQuestInfo(questNo._group, questNo._quest)
  if 0 == nextQuestNo._group or 0 == nextQuestNo._quest then
    checkedQuestWidget._recentGroupQuest = false
    return
  end
  checkedQuestWidget._recentGroupQuest = true
  checkedQuestWidget._recentGroupQuestNo = nextQuestNo
end
function checkedQuestWidget:updateQuestSetPos()
  ...
  self._updateTime = 1
  local tracer2 = Profile_BeginUIFunction("updateQuestSetPos-2")
  local newEquipGap = 0
  if true == Panel_NewEquip:GetShow() then
  ...
  Profile_EndUIFunction(tracer2)
  local tracer3 = Profile_BeginUIFunction("updateQuestSetPos-3")
...
function FGlobal_WindowQuestToggle()
  PaGlobalFunc_Quest_All_Toggle()
end
function FromClient_CheckedQuestWidget_QuestBackEndLoadComplete(s32_questNoRaw)
  if Panel_CheckedQuest == nil or _ContentsGroup_QuestBackEndLoading == false then
    return
  end
  ...
  if checkedQuestWidget:isExistLoadingDataAnyOne() == false then
    return
  end
  local updateFuncType
  for index = 1, #checkedQuestWidget._questLoadingDataList do
    local value = checkedQuestWidget._questLoadingDataList[index]
    if value ~= nil and value._questNoRaw == s32_questNoRaw then
      updateFuncType = value._updateFuncType
      table.remove(checkedQuestWidget._questLoadingDataList, index)
      break
    end
  end
  if checkedQuestWidget:isExistLoadingDataAnyOne() == false then
    if updateFuncType == nil then
      return
    end
    local questSSW = questList_getQuestInfo(s32_questNoRaw)
    if questSSW == nil then
      return
    end
    local questGroup = questSSW:getQuestNo()._group
    local questId = questSSW:getQuestNo()._quest
    if updateFuncType == checkedQuestWidget.eUPDATE_FUNCTYPE.UPDATE_TARGET then
      local uiQuestInfo = ToClient_GetQuestInfo(questGroup, questId)
      if uiQuestInfo ~= nil and uiQuestInfo:isSatisfied() == false then
        checkedQuestWidget:setTargetQuest(uiQuestInfo)
      end
    elseif updateFuncType == checkedQuestWidget.eUPDATE_FUNCTYPE.UPDATE_LATEST then
      checkedQuestWidget:setLatestQuest()
    elseif updateFuncType == checkedQuestWidget.eUPDATE_FUNCTYPE.UPDATE_NORMAL then
      checkedQuestWidget:setNormalQuest()
    elseif updateFuncType == checkedQuestWidget.eUPDATE_FUNCTYPE.UPDATE_RENEW then
      checkedQuestWidget:updateRenew(questGroup, questId)
    else
      return
    end
  end
...
function checkedQuestWidget:registEventHandler()
  ...
  registerEvent("FromClient_ClearNormalGroupQuestInformation", "FromClient_ClearNormalGroupQuestInformation")
  registerEvent("FromClient_AcceptNormalGroupQuestInformation", "FromClient_AcceptNormalGroupQuestInformation")
  registerEvent("FromClient_UpdateQuestList", "FromClient_QuestWidget_UpdateQuestList")
  registerEvent("FromClient_UpdateProgressGuildQuestList", "FromClient_QuestWidget_UpdateGuildQuestList")
  ...
  if _ContentsGroup_QuestBackEndLoading == true then
    registerEvent("FromClient_QuestBackEndLoadComplete", "FromClient_CheckedQuestWidget_QuestBackEndLoadComplete")
  end
end
...
function PaGlobal_CheckedQuest_UpdateBackSpaceQuest()
  if false == _ContentsGroup_BackSpaceClearQuestGuide then
    return
  end
  local backSpcaeQuestData = checkedQuestWidget._backSpaceQuestData
  if nil ~= PaGlobalFunc_MainQuestWidget_ShowBackSpaceKeyGuid then
    PaGlobalFunc_MainQuestWidget_ShowBackSpaceKeyGuid(false)
  end
  if 0 < backSpcaeQuestData._groupId then
    local uiIndex = checkedQuestWidget:findShownIndexInCheckedQuest(backSpcaeQuestData._groupId, backSpcaeQuestData._questId)
    if -1 ~= uiIndex and nil ~= checkedQuestWidget._uiQuestList[uiIndex]._uiBackSpace then
      checkedQuestWidget._uiQuestList[uiIndex]._uiBackSpace:SetShow(false)
    end
  end
  local curIndex = 1
  local backSpaceIndex = -1
  checkedQuestWidget._clearQuestList = {}
  if nil ~= Panel_MainQuest and true == Panel_MainQuest:GetShow() and nil ~= PaGlobalFunc_MainQuestWidget_GetMainQuestNo then
    local mainQuestNo = PaGlobalFunc_MainQuestWidget_GetMainQuestNo()
    if nil ~= mainQuestNo then
      local groupId = mainQuestNo._group
      local questId = mainQuestNo._quest
      local uiQuestInfo = ToClient_GetQuestInfo(groupId, questId)
      if nil ~= uiQuestInfo and true == uiQuestInfo:isSatisfied() then
        if nil ~= backSpcaeQuestData and backSpcaeQuestData._groupId == mainQuestNo._group and backSpcaeQuestData._questId == mainQuestNo._quest then
          backSpaceIndex = curIndex
        end
        local elements = {}
        elements._index = curIndex
        elements._groupId = mainQuestNo._group
        elements._questId = mainQuestNo._quest
        elements._isCompleteBlackSpirit = uiQuestInfo:isCompleteBlackSpirit()
        elements._isMainQuest = true
        checkedQuestWidget._clearQuestList[curIndex] = elements
        curIndex = curIndex + 1
      end
    end
  end
  if nil ~= Panel_CheckedQuest and true == Panel_CheckedQuest:GetShow() then
    for _, data in ipairs(checkedQuestWidget._latestDataList) do
      if nil ~= data and data.state == checkedQuestWidget._config._questState._complete then
        if nil ~= backSpcaeQuestData and backSpcaeQuestData._groupId == data.group and backSpcaeQuestData._questId == data.quest then
          backSpaceIndex = curIndex
        end
        local elements = {}
        elements._index = curIndex
        elements._groupId = data.group
        elements._questId = data.quest
        elements._isCompleteBlackSpirit = data.isCompleteBlackSpirit
        elements._isMainQuest = false
        checkedQuestWidget._clearQuestList[curIndex] = elements
        curIndex = curIndex + 1
      end
    end
    for _, data in ipairs(checkedQuestWidget._normalDataList) do
      if nil ~= data and data.state == checkedQuestWidget._config._questState._complete then
        if nil ~= backSpcaeQuestData and backSpcaeQuestData._groupId == data.group and backSpcaeQuestData._questId == data.quest then
          backSpaceIndex = curIndex
        end
  ...
  if -1 == backSpaceIndex then
    backSpaceIndex = 1
  end
  if 1 ~= curIndex and nil ~= checkedQuestWidget._clearQuestList[backSpaceIndex] then
    checkedQuestWidget._backSpaceQuestData._index = checkedQuestWidget._clearQuestList[backSpaceIndex]._index
    checkedQuestWidget._backSpaceQuestData._groupId = checkedQuestWidget._clearQuestList[backSpaceIndex]._groupId
    checkedQuestWidget._backSpaceQuestData._questId = checkedQuestWidget._clearQuestList[backSpaceIndex]._questId
    checkedQuestWidget._backSpaceQuestData._isCompleteBlackSpirit = checkedQuestWidget._clearQuestList[backSpaceIndex]._isCompleteBlackSpirit
    checkedQuestWidget._backSpaceQuestData._isMainQuest = checkedQuestWidget._clearQuestList[backSpaceIndex]._isMainQuest
    if true == checkedQuestWidget._backSpaceQuestData._isMainQuest then
      if nil ~= PaGlobalFunc_MainQuestWidget_ShowBackSpaceKeyGuid then
        PaGlobalFunc_MainQuestWidget_ShowBackSpaceKeyGuid(true)
      end
    else
      local uiIndex = checkedQuestWidget:findShownIndexInCheckedQuest(checkedQuestWidget._backSpaceQuestData._groupId, checkedQuestWidget._backSpaceQuestData._questId)
      if -1 ~= uiIndex and nil ~= checkedQuestWidget._uiQuestList[uiIndex]._uiBackSpace then
        checkedQuestWidget._uiQuestList[uiIndex]._uiBackSpace:SetShow(true)
      end
    end
  else
    checkedQuestWidget._backSpaceQuestData._index = -1
  end
end
function HandleEventBackSpaceUp_CheckedQuest_ShowClearQuestNavi()
  if false == _ContentsGroup_BackSpaceClearQuestGuide then
    return
  end
  PaGlobal_CheckedQuest_UpdateBackSpaceQuest()
  if -1 == checkedQuestWidget._backSpaceQuestData._index or #checkedQuestWidget._clearQuestList < 1 then
    local messageText = PAGetString(Defines.StringSheet_GAME, "LUA_CHECKQUEST_CLEARQUEST_NOTEXIST")
    if nil ~= messageText and "" ~= messageText then
      Proc_ShowMessage_Ack(messageText)
    end
    return
  end
  local index = checkedQuestWidget._backSpaceQuestData._index
  local groupId = checkedQuestWidget._backSpaceQuestData._groupId
  local questId = checkedQuestWidget._backSpaceQuestData._questId
  PaGlobal_CheckedQuest_FindTargetByClearQuest(groupId, questId, checkedQuestWidget._backSpaceQuestData._isCompleteBlackSpirit)
  local nextIndex = -1
  if nil ~= checkedQuestWidget._clearQuestList[index + 1] then
    nextIndex = index + 1
  elseif nil ~= checkedQuestWidget._clearQuestList[1] then
    nextIndex = 1
  else
    return
  end
  if -1 ~= nextIndex then
    ...
    checkedQuestWidget._backSpaceQuestData._index = checkedQuestWidget._clearQuestList[nextIndex]._index
    checkedQuestWidget._backSpaceQuestData._groupId = checkedQuestWidget._clearQuestList[nextIndex]._groupId
    checkedQuestWidget._backSpaceQuestData._questId = checkedQuestWidget._clearQuestList[nextIndex]._questId
    checkedQuestWidget._backSpaceQuestData._isCompleteBlackSpirit = checkedQuestWidget._clearQuestList[nextIndex]._isCompleteBlackSpirit
    checkedQuestWidget._backSpaceQuestData._isMainQuest = checkedQuestWidget._clearQuestList[nextIndex]._isMainQuest
    if true == checkedQuestWidget._backSpaceQuestData._isMainQuest then
      if nil ~= PaGlobalFunc_MainQuestWidget_ShowBackSpaceKeyGuid then
        PaGlobalFunc_MainQuestWidget_ShowBackSpaceKeyGuid(true)
      end
...
function PaGlobal_CheckedQuest_FindTargetByClearQuest(groupId, questId, isCompleteBlackSpirit, questTitle)
  if nil == groupId or nil == questId or nil == isCompleteBlackSpirit then
    return
  end
  local isAutoNavi = true
  local conditionCheck = QuestConditionCheckType.eQuestConditionCheckType_Complete
  if true == isCompleteBlackSpirit then
    HandleClicked_CallBlackSpirit()
  else
    self._naviInfoAgain = true
    HandleClicked_QuestWidget_FindTarget(groupId, questId, conditionCheck, isAutoNavi)
    local uiQuestInfo = ToClient_GetQuestInfo(groupId, questId)
    if nil ~= uiQuestInfo then
      local title = uiQuestInfo:getTitle()
      if nil ~= title and "" ~= title then
        local messageText = PAGetStringParam1(Defines.StringSheet_GAME, "LUA_CHECKQUEST_CLEARQUEST_MOVE", "name", title)
        Proc_ShowMessage_Ack(messageText)
      end
    end
  end
end
function PaGlobalFunc_CheckedQuest_All_RightCheck(eRightType)
  local selfPlayer = getSelfPlayer()
  if nil == selfPlayer then
    return false
  end
  local function OriginalCheck()
    ...
    if __eGuildRightType_QuestComplete == eRightType then
      return isMasters
    end
    return false
  end
