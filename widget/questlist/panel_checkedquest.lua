...
PaGlobal_CheckedQuest = {
  ...
  eUPDATE_FUNCTYPE = {UPDATE_PROGRESS = 1}
}
...
function PaGlobal_CheckedQuest:initDefaults()
  ...
  self._uiHelpWidget = UI.createControl(__ePAUIControl_StaticText, Panel_CheckedQuest, "HelpWindow_For_QuestWidget")
  ...
  self._uiHelpWidget:SetTextMode(__eTextMode_AutoWrap)
...
function PaGlobal_CheckedQuest:createGuildQuest()
  ...
  for idx = 1, 5 do
    ...
    self._guildQuest._Condition[idx]:SetTextMode(__eTextMode_AutoWrap)
...
function PaGlobal_CheckedQuest:getTotalListCount()
  ...
  if nil ~= FGlobal_LatestQuest_UpdateVisibleCount then
    FGlobal_LatestQuest_UpdateVisibleCount()
  end
  if nil ~= PaGlobal_MainQuest_SetMainQuestNo then
    PaGlobal_MainQuest_SetMainQuestNo()
  end
...
function PaGlobal_CheckedQuest:updateQuestList(startPosition)
  ...
  if 0 == isEmptyNormalQuestGroup() and true == ToClient_isProgressingGuildQuest() then
    ...
    self._uiScrollBar:SetInterval(0)
    ...
    local widgetType
    if true == _ContentsGroup_NewUI_QuestOption_All then
      if nil ~= PaGlobal_Quest_Option_All_GetSelectedWidgetType then
        widgetType = PaGlobal_Quest_Option_All_GetSelectedWidgetType()
      end
    elseif nil ~= FGlobal_GetSelectedWidgetType then
      widgetType = FGlobal_GetSelectedWidgetType()
    end
    if nil ~= widgetType then
  ...
  if true == welcomeToTheWorld then
      ...
      if nil ~= temp_questGroupInfo then
        if true == temp_questGroupInfo:isGroupQuest() then
          ...
          for questGroupIdx = 0, tempGroupCount - 1 do
            ...
            if tempQuestInfo._isProgressing and not tempQuestInfo._isCleared and welcomeToTheWorld and not TutorialQuestCompleteCheck() and (3 == tempQuestInfo:getQuestType() or 4 == tempQuestInfo:getQuestType() or 5 == tempQuestInfo:getQuestType() or 6 == tempQuestInfo:getQuestType()) then
    ...
    if welcomeToTheWorld and not TutorialQuestCompleteCheck() and temp_progressCount > 0 then
      ...
      for index = 0, temp_progressCount - 1 do
        if nil ~= questNoSaveForTutorial[index]._questGroupIndex and nil ~= questNoSaveForTutorial[index]._questGroupIndex then
...
function PaGlobal_CheckedQuest:ProgressQuest(questGroupInfo, uiQuestInfo, tmp_nextPosY, isSingle, groupTitle, questGroupCount, questGroupIndex)
  ...
  if _ContentsGroup_QuestBackEndLoading == true and uiQuestInfo ~= nil and uiQuestInfo:isHaveBackEndData() == false then
    self:pushQuestLoadingData(uiQuestInfo, eUPDATE_FUNCTYPE.UPDATE_PROGRESS, questGroupInfo, tmp_nextPosY, isSingle, groupTitle, questGroupCount, questGroupIndex)
  end
...
function PaGlobal_CheckedQuest:setQuestTypeIcon(idx, uiQuestInfo)
  ...
  ToClient_ChangeQuestIconByQuestType(uiQuestIcon, uiQuestInfo:getQuestType())
end
function PaGlobal_CheckedQuest:setQuestTitle(idx, uiQuestInfo)
  ...
  uiQuestTitle:SetTextMode(__eTextMode_LimitText)
...
function PaGlobal_CheckedQuest:setQuestGroupTitle(idx, isSingle, groupTitle, questId, questGroupCount)
  ...
  if false == isSingle then
    ...
    uiGroupTitle:SetTextMode(__eTextMode_LimitText)
...
function PaGlobal_CheckedQuest:setQuestCondition(idx, uiQuestInfo, questNo, posY)
  ...
  if false == uiQuestInfo:isSatisfied() then
    for index = 0, uiQuestInfo:getDemandCount() - 1 do
      ...
      uiCondition:SetTextMode(__eTextMode_AutoWrap)
    ...
    FGlobal_ChangeOnTextureForDialogQuestCompleteIcon(uiQuestIcon)
    if false == isSingle then
...
function PaGlobal_CheckedQuest:clearQuestLoadingTable()
  if Panel_CheckedQuest == nil or _ContentsGroup_QuestBackEndLoading == false then
    return
  end
  self._questLoadingDataList = Array.new()
end
function PaGlobal_CheckedQuest:pushQuestLoadingData(uiQuestInfo, updateFuncType, param1, param2, param3, param4, param5, param6)
...
function PaGlobal_CheckedQuest:isExistLoadingDataAnyOne()
  if Panel_CheckedQuest == nil or _ContentsGroup_QuestBackEndLoading == false then
    return false
  end
  if self._questLoadingDataList == nil then
...
function HandleClicked_ShowQuestInfo(questGroupId, questId, questCondition_Chk, groupTitle, questGroupCount)
  ...
  if false == _ContentsGroup_RenewUI then
    ...
    PaGlobalFunc_Quest_All_SetProgress()
    PaGlobalFunc_QuestInfo_All_Detail(questGroupId, questId, questCondition_Chk, groupTitle, questGroupCount, fromQuestWidget)
  ...
  elseif 99 == condition then
    ...
    if questPosCount == 0 then
      ...
      for questPositionIndex = 0, questPosCount - 1 do
        ...
        _positionList[questPositionIndex]._key = nil
      ...
      if questPosCount == questCompleteConditionCount then
        if questPosCount > 1 then
          ...
          elseif questPositionType == __eQuestPositionType_InOrder then
            if notClearedIndexCount > 0 then
              if _lastGuideIndex == nil then
                guideIndex = notClearedIndexList[1]
                _lastGuideIndex = guideIndex
              else
                _lastGuideIndex = (_lastGuideIndex + 1) % notClearedIndexCount
                guideIndex = _lastGuideIndex
              end
            end
      ...
      if guideIndex ~= nil and _positionList[guideIndex]._key ~= nil and selfPlayer:getNavigationMovePathIndex() ~= _positionList[guideIndex]._key then
        if ToClient_GetNaviGuideGroupNo(_positionList[guideIndex]._key) ~= 0 then
          ToClient_DeleteNaviGuideByGroup(0)
        end
        selfPlayer:setNavigationMovePath(_positionList[guideIndex]._key)
        selfPlayer:checkNaviPathUI(_positionList[guideIndex]._key)
      end
...
function _QuestWidget_ReturnQuestState()
  return _questGroupId, _questId, _naviInfoAgain, _isAutoRun
end
...
function FromClient_Panel_updateBlackSpirit()
  ...
  if darkSpiritFirstTime == true and isClearedQuest == true then
    ...
    PaGlobal_CheckedQuest._uiDarkSpirit_Notice:SetTextMode(__eTextMode_AutoWrap)
...
function QuestAutoNpcNavi_Over(isNpcNaviShow)
  ...
  if playerLevel >= 4 then
    if isNpcNaviShow == true then
      ...
      PaGlobal_CheckedQuest._uiNotice_NpcNavi:SetTextMode(__eTextMode_AutoWrap)
...
function questGiveUp_Over(isGiveShow)
  ...
  if playerLevel >= 4 then
    if isGiveShow == true then
      ...
      PaGlobal_CheckedQuest._uiNotice_GiveUp:SetTextMode(__eTextMode_AutoWrap)
...
function QuestReward_Over(isRewardShow)
  ...
  if playerLevel >= 4 then
    if isRewardShow == true then
      ...
      PaGlobal_CheckedQuest._uiNotice_Reward:SetTextMode(__eTextMode_AutoWrap)
...
function FromClient_CheckedQuest_QuestBackEndLoadComplete(s32_questNoRaw)
  if Panel_CheckedQuest == nil or _ContentsGroup_QuestBackEndLoading == false then
    return
  end
  ...
  if PaGlobal_CheckedQuest:isExistLoadingDataAnyOne() == false then
    return
  end
  local updateFuncType, param1, param2, param3, param4, param5, param6
  for index = 1, #PaGlobal_CheckedQuest._questLoadingDataList do
    local value = PaGlobal_CheckedQuest._questLoadingDataList[index]
    if value ~= nil and value._questNoRaw == s32_questNoRaw then
      updateFuncType = value._updateFuncType
      param1 = value._param1
      param2 = value._param2
      param3 = value._param3
      param4 = value._param4
      param5 = value._param5
      param6 = value._param6
      table.remove(PaGlobal_CheckedQuest._questLoadingDataList, index)
      break
    end
  end
  if PaGlobal_CheckedQuest:isExistLoadingDataAnyOne() == false then
    ...
    if updateFuncType == PaGlobal_CheckedQuest.eUPDATE_FUNCTYPE.UPDATE_PROGRESS then
      PaGlobal_CheckedQuest:ProgressQuest(param1, uiQuestInfo, param2, param3, param4, param5, param6)
    else
      return
    end
  end
...
registerEvent("FromClient_ApplyUISettingPanelInfo", "FromClient_questWidget_ResetPosition")
...
if _ContentsGroup_QuestBackEndLoading == true then
  registerEvent("FromClient_QuestBackEndLoadComplete", "FromClient_CheckedQuest_QuestBackEndLoadComplete")
end
