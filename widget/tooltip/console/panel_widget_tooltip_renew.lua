...
local _targetData = {
  [Defines.TooltipTargetType.Item] = {
    ...
    COMPONENT_TYPE.LIGHTSTONE,
    COMPONENT_TYPE.GUARANTEE_ENCHANT,
...
local backendNeedUpdateItemKeyList = Array.new()
...
function TooltipInfo:initialize()
  _componentOption = {
    ...
    [COMPONENT_TYPE.ARTIFACTS] = {fillDataFunc = TooltipInfo_updateARTIFACTS},
    ...
    [COMPONENT_TYPE.LIGHTSTONE] = {fillDataFunc = TooltipInfo_updateLIGHTSTONE, isInsideClippingArea = true},
    [COMPONENT_TYPE.GUARANTEE_ENCHANT] = {fillDataFunc = TooltipInfo_updateGUARANTEE_ENCHANT, isInsideClippingArea = true},
  ...
  self._ui.stc_components = {
    [1] = {
      ...
      [COMPONENT_TYPE.ARTIFACTS] = UI.getChildControl(self._ui.stc_BG[1], "ARTIFACTS"),
      ...
      [COMPONENT_TYPE.LIGHTSTONE] = UI.getChildControl(self._ui.stc_scrollContent[1], "LIGHTSTONE"),
      [COMPONENT_TYPE.GUARANTEE_ENCHANT] = UI.getChildControl(self._ui.stc_scrollContent[1], "GUARANTEE_ENCHANT_COUNT"),
  ...
  for ii = 2, self._poolCount do
    ...
    self._ui.stc_components[ii][COMPONENT_TYPE.ARTIFACTS] = UI.getChildControl(self._ui.stc_BG[ii], "ARTIFACTS")
    ...
    self._ui.stc_components[ii][COMPONENT_TYPE.LIGHTSTONE] = UI.getChildControl(self._ui.stc_scrollContent[ii], "LIGHTSTONE")
    self._ui.stc_components[ii][COMPONENT_TYPE.GUARANTEE_ENCHANT] = UI.getChildControl(self._ui.stc_scrollContent[ii], "GUARANTEE_ENCHANT_COUNT")
...
function TooltipInfo:initPool(poolIndex)
  ...
  self._ui.stc_artifactBG[poolIndex] = {}
  self._ui.stc_artifactIcon[poolIndex] = {}
  self._ui.stcTxt_artifactName[poolIndex] = {}
  for jj = 1, 2 do
    self._ui.stc_artifactBG[poolIndex][jj] = UI.getChildControl(self._ui.stc_components[poolIndex][COMPONENT_TYPE.ARTIFACTS], "Static_ArtifactsSlotBg_" .. jj)
    if self._ui.stc_artifactBG[poolIndex][jj] ~= nil then
      self._ui.stc_artifactIcon[poolIndex][jj] = UI.getChildControl(self._ui.stc_artifactBG[poolIndex][jj], "Static_SocketIcon")
    end
    self._ui.stcTxt_artifactName[poolIndex][jj] = UI.getChildControl(self._ui.stc_components[poolIndex][COMPONENT_TYPE.ARTIFACTS], "StaticText_" .. jj)
  end
...
function TooltipInfo:registEventHandler()
  ...
  if _ContentsGroup_ItemBackEndLoading == true then
    registerEvent("FromClient_ItemBackEndLoadComplete", "FromClient_TooltipInfo_BackEndLoadComplete")
  end
end
function FromClient_TooltipInfo_BackEndLoadComplete(loadedItemKey, loadedEnchantLevel)
  ...
  if backendNeedUpdateItemKeyList == nil then
    return
  end
  local loadedEnchantKey = ItemEnchantKey(loadedItemKey, loadedEnchantLevel)
  for idx = 1, #backendNeedUpdateItemKeyList do
    local savedInfo = backendNeedUpdateItemKeyList[idx]
    if savedInfo ~= nil then
      local savedItemkey = savedInfo._itemEnchantKey
      if loadedEnchantKey:get() == savedItemkey:get() then
        local itemESSW = getItemEnchantStaticStatus(loadedEnchantKey)
        if itemESSW ~= nil then
          local savedUIIdex = savedInfo._currentPoolNo
          if TooltipInfo._ui.stc_components[savedUIIdex][COMPONENT_TYPE.DESCRIPTION] ~= nil then
            local content = UI.getChildControl(TooltipInfo._ui.stc_components[savedUIIdex][COMPONENT_TYPE.DESCRIPTION], "StaticText_Content")
            local content1 = UI.getChildControl(TooltipInfo._ui.stc_components[savedUIIdex][COMPONENT_TYPE.DESCRIPTION], "StaticText_Content1")
            content1:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_TOOLTIP_DESC_TITLE") .. " " .. itemESSW:getDescription())
            local content2 = UI.getChildControl(TooltipInfo._ui.stc_components[savedUIIdex][COMPONENT_TYPE.DESCRIPTION], "StaticText_Content2")
            content2:SetPosY(content1:GetTextSizeY())
            local tooltipComponentSize = TooltipInfo._ui.stc_components[savedUIIdex][COMPONENT_TYPE.DESCRIPTION]
            tooltipComponentSize:SetSize(_defaultSizeX, content:GetTextSizeY() + content1:GetTextSizeY() + content2:GetTextSizeY())
            TooltipInfo:resizeComponentAfterCallback(savedUIIdex, savedInfo._tooltipTargetType)
            table.remove(backendNeedUpdateItemKeyList, idx)
          end
        end
      end
    end
...
function TooltipInfo:resizeComponentAfterCallback(currentPool, tooltipTargetType)
  if _ContentsGroup_ItemBackEndLoading == false then
    return
  end
  if currentPool == nil or tooltipTargetType == nil then
    return
  end
  if currentPool <= 0 or currentPool > 2 then
    return
  end
  ...
  for ii = 1, #_targetData[tooltipTargetType] do
    if nil ~= _targetData[tooltipTargetType][ii] then
      ...
      if SPLIT == componentIndex then
        ...
        local component = TooltipInfo._ui.stc_components[currentPool][componentIndex]
        if component ~= nil and true == component:GetShow() then
          ...
          if componentIndex == COMPONENT_TYPE.HEADER then
            local txt_name = UI.getChildControl(TooltipInfo._ui.stc_components[currentPool][COMPONENT_TYPE.HEADER], "StaticText_ItemName")
            local tempSizeY = (txt_name:GetTextSizeY() - txt_name:GetSizeY()) / 2
            if tempSizeY > 0 then
              TooltipInfo._ui.stc_components[currentPool][COMPONENT_TYPE.TYPE]:SetPosY(TooltipInfo._ui.stc_components[currentPool][COMPONENT_TYPE.TYPE]:GetPosY() - tempSizeY)
  ...
  if yPosInClippingArea > _clipAreaYSize then
    TooltipInfo._ui.stc_scrollContent[currentPool]:SetSize(_defaultSizeX, yPosInClippingArea)
    TooltipInfo._ui.stc_scrollContent[currentPool]:SetPosY(0)
    TooltipInfo._ui.stc_scrollBtn[currentPool]:SetPosY(-(TooltipInfo._ui.stc_scrollBtn[currentPool]:GetSizeY() * 0.5))
    TooltipInfo._ui.stc_scrollBG[currentPool]:SetShow(true)
  else
    TooltipInfo._ui.stc_scrollBG[currentPool]:SetShow(false)
  end
end
function PaGlobalFunc_TooltipInfo_GetShow()
  return _panel:GetShow()
end
function PaGlobalFunc_TooltipInfo_Open(tooltipDataType, data, tooltipTargetType, targetX, compareX, slotType, isEquippedItem)
  TooltipInfo:open(tooltipDataType, data, tooltipTargetType, targetX, compareX, nil, nil, slotType, isEquippedItem)
end
...
local tempDataList
function TooltipInfo:open(tooltipDataType, data, tooltipTargetType, targetX, compareX, isMyWallet, enchantKey, slotType, isEquippedItem)
  ...
  if itemWrapper ~= nil and itemWrapper:empty() == true then
  ...
  self:compose(itemWrapper, itemSSW, tooltipTargetType, targetX, isMyWallet, enchantKey, slotType, isEquippedItem)
  tempDataList = {}
  tempDataList.tooltipDataType = tooltipDataType
  tempDataList.data = data
  if Defines.TooltipDataType.ItemWrapper == tooltipDataType then
    tempDataList.data = data:returnCopy()
  end
  tempDataList.tooltipTargetType = tooltipTargetType
  tempDataList.targetX = targetX
  tempDataList.compareX = compareX
  tempDataList.isMyWallet = isMyWallet
  tempDataList.enchantKey = enchantKey
  tempDataList.slotType = slotType
  if Defines.TooltipDataType.ItemSSWrapper == tooltipDataType then
    tempDataList.itemKey = data:get()._key
  else
    tempDataList.itemKey = nil
  end
  if CppEnums.ItemType.Equip ~= itemType or Defines.TooltipTargetType.ItemWithoutCompare == tooltipTargetType or true == itemSSW:isVehicleItem() then
    ...
    if nil ~= equipItemWrapper then
      ...
      self:compose(equipItemWrapper, equipItemSSW, tooltipTargetType, xPos, isMyWallet, enchantKey, slotType, true)
...
function TooltipInfo:compose(itemWrapper, itemSSW, tooltipTargetType, targetX, isMyWallet, enchantKey, slotType, isEquippedItem)
  _currentYPos = _defaultYPos
    ...
    if nil ~= _targetData[tooltipTargetType][ii] then
      ...
      if SPLIT == componentIndex then
        ...
        if nil ~= updateFunc and "function" == type(updateFunc) then
          local componentHaveValidData = updateFunc(itemWrapper, itemSSW, tooltipTargetType, isMyWallet, enchantKey, slotType, isEquippedItem)
...
function TooltipInfo_updateARTIFACTS(itemWrapper, itemSSW, tooltipTargetType, isMyWallet, enchantKey, slotType)
  local soketCount = 0
  ...
  for socketSlotIdx = 1, lightStoneMaxCount do
    self._ui.stc_artifactIcon[_currentPool][socketSlotIdx]:SetShow(false)
    self._ui.stcTxt_artifactName[_currentPool][socketSlotIdx]:SetShow(false)
    self._ui.stc_artifactBG[_currentPool][socketSlotIdx]:SetShow(false)
  end
  for jewelIdx = 0, soketCount - 1 do
    local socketName = self._ui.stcTxt_artifactName[_currentPool][jewelIdx + 1]
    local socketSlot = self._ui.stc_artifactIcon[_currentPool][jewelIdx + 1]
    if item ~= nil then
      itemEnchantSSW = itemWrapper:getPushedItem(jewelIdx)
    else
      itemEnchantSSW = nil
      if item ~= nil then
        local pushedKey = item:getPushedKey(jewelIdx)
        if pushedKey ~= nil and 0 < pushedKey:get() then
          itemEnchantSSW = getItemEnchantStaticStatus(pushedKey)
        end
      elseif slotType ~= nil and slotType == "ArtifactBag" and isArtifact == true and _ContentsGroup_LightStoneBag == true and PaGlobalFunc_ArtifactBag_CurrentMouseOverBagIndex ~= nil then
      ...
      elseif slotType ~= nil and slotType == "ArtifactPreset" and isArtifact == true and _ContentsGroup_LightStoneBag == true and PaGlobalFunc_ArtifactPreset_CurrentMouseOverPresetNo ~= nil then
    ...
    self._ui.stc_artifactBG[_currentPool][jewelIdx + 1]:SetShow(true)
    if nil ~= itemEnchantSSW then
      ...
      PAGlobalFunc_SetItemTextColor(socketName, itemEnchantSSW)
      ...
      if nil == skillSSW then
        ...
        if true ~= isArtifact or buffList == "" then
        end
      end
    else
...
function TooltipInfo_updateLIGHTSTONE(itemWrapper, itemSSW, tooltipTargetType, isMyWallet, enchantKey, slotType, isEquippedItem)
  if _ContentsGroup_ArtifactItem == false then
    return false
  end
  local componentIsShow = false
  local isArtifact = itemSSW:get():isArtifact()
  ...
  local desc = ToClient_getLightStoneSet()
  if desc == nil then
    return false
  end
  if isEquippedItem ~= true then
    return false
  end
  local content = UI.getChildControl(self._ui.stc_components[_currentPool][COMPONENT_TYPE.LIGHTSTONE], "StaticText_Content")
  local content1 = UI.getChildControl(self._ui.stc_components[_currentPool][COMPONENT_TYPE.LIGHTSTONE], "StaticText_Content1")
  content1:SetText(desc)
  componentIsShow = true
  self._ui.stc_components[_currentPool][COMPONENT_TYPE.LIGHTSTONE]:SetSize(content:GetSizeX(), content:GetTextSizeY() + content1:GetTextSizeY() + _defaultGapBetweenComponent)
  content1:SetPosY(content:GetTextSizeY())
  return componentIsShow
end
...
function TooltipInfo_updateDESCRIPTION(itemWrapper, itemSSW, tooltipTargetType, isMyWallet, enchantKey, slotType)
  ...
  if _ContentsGroup_JewelPreset == true and itemSSW ~= nil then
      ...
      if nil ~= groupMaxCount then
        ...
        if nil ~= groupName then
          if 1000 == groupMaxCount then
            desc = desc .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_TOOLTIP_CRYSTALGROUP_NOLIMIT", "groupName", groupName) .. [[


]]
          else
            desc = desc .. PAGetStringParam2(Defines.StringSheet_GAME, "LUA_TOOLTIP_CRYSTALGROUP_MAXCOUNT", "groupName", groupName, "groupMaxCount", groupMaxCount) .. [[
    ...
    local socketCount = 0
    if itemSSW:get():getEnchant():empty() == false then
      socketCount = itemSSW:get():getEnchant()._socketCount
    end
    if socketCount > 0 then
      if itemSSW:getEquipSlotNo() == __eEquipSlotNoAwakenWeapon then
        desc = desc .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_TOOLTIP_CRYSTALSLOTDESC1", "soketCount", socketCount) .. "\n"
      elseif itemSSW:getEquipSlotNo() == __eEquipSlotNoAvatarChest then
        desc = desc .. PAGetString(Defines.StringSheet_GAME, "LUA_TOOLTIP_CRYSTALSLOTDESC2") .. "\n"
      end
    end
  end
  ...
  if true == isInventoryBagItem and __eTInventorySlotNoUndefined ~= bagItemSlotNo then
    ...
    desc = desc .. PAGetString(Defines.StringSheet_GAME, "LUA_TOOLTIP_ITEM_BAGSIZE")
    desc = desc .. "\n"
    local itemNameinBag = ""
    ...
    for index = 0, bagSize - 1 do
      ...
      if bagItemWrapper ~= nil then
        if isFishBag == true then
          ...
          local addString = tostring(bagItemWrapper:getStaticStatus():getName())
  ...
  local itemdesc = ""
  if "" ~= itemSSW:getDescription() then
    itemdesc = itemSSW:getDescription()
  else
    itemdesc = PAGetString(Defines.StringSheet_GAME, "GAME_MESSAGE_WAIT_PROCESS")
    if _ContentsGroup_ItemBackEndLoading == true then
      local infoList = {
        _currentPoolNo = _currentPool,
        _itemEnchantKey = bagItemKey,
        _tooltipTargetType = tooltipTargetType
      }
      backendNeedUpdateItemKeyList:push_back(infoList)
    end
  end
  desc1 = PAGetString(Defines.StringSheet_GAME, "LUA_TOOLTIP_DESC_TITLE") .. " " .. itemdesc
  local item_type = itemSSW:getItemType()
  local isNodeFreeTrade = itemSSW:get():isNodeFreeTrade()
  if isNodeFreeTrade then
    desc2 = desc2 .. " " .. PAGetString(Defines.StringSheet_RESOURCE, "PANEL_TOOLTIP_ITEM_NODEFREETRADE_DESC")
  end
...
- ]] .. itemSSW:getEnchantDescription()
  ...
  local content1 = UI.getChildControl(self._ui.stc_components[_currentPool][COMPONENT_TYPE.DESCRIPTION], "StaticText_Content1")
  content1:SetPosY(content:GetTextSizeY())
  content1:SetTextMode(__eTextMode_AutoWrap)
  content1:SetText(desc1)
  local content2 = UI.getChildControl(self._ui.stc_components[_currentPool][COMPONENT_TYPE.DESCRIPTION], "StaticText_Content2")
  content2:SetPosY(content1:GetTextSizeY())
  content2:SetTextMode(__eTextMode_AutoWrap)
  content2:SetText(desc2)
  self._ui.stc_components[_currentPool][COMPONENT_TYPE.DESCRIPTION]:SetSize(_defaultSizeX, content:GetTextSizeY() + content1:GetTextSizeY() + content2:GetTextSizeY())
  return "" ~= desc or "" ~= desc1 or "" ~= desc2
end
...
function TooltipInfo_updateGUARANTEE_ENCHANT(itemWrapper, itemSSW)
  local component = self._ui.stc_components[_currentPool][COMPONENT_TYPE.GUARANTEE_ENCHANT]
  local content = UI.getChildControl(component, "StaticText_GuaranteeEnchant_Count")
  local ssEnchantCount = itemSSW:getSSEnchantCount()
  local currentEnchantCount = itemSSW:getCurrentEnchantCount()
  local isGuaranteeEnchantShow = false
  if _ContentsGroup_AdventureMode == true and ssEnchantCount ~= 0 then
    isGuaranteeEnchantShow = true
    content:SetText(PAGetStringParam2(Defines.StringSheet_RESOURCE, "UI_TOOLTIP_ITEM_GUARANTEE_ENCHANT", "currentCount", tostring(currentEnchantCount), "maxCount", tostring(ssEnchantCount)))
    component:SetSize(_defaultSizeX, content:GetTextSizeY())
    self._ui.stc_components[_currentPool][COMPONENT_TYPE.ITEM_MARKET_INFO]:SetSize(_defaultSizeX, content:GetPosY() + content:GetSizeY())
  end
  return isGuaranteeEnchantShow
end
...
function TooltipInfo_updateMAXENCHANTER(itemWrapper, itemSSW, tooltipDataType)
  ...
  if false == isNamingAbleItem then
    return false
  end
  local isCashItem = itemSSW:get():isCash()
  if true == isCashItem then
    return false
  end
  ...
  if true == isTradeItem and nil ~= itemWrapper then
    ...
    if nil ~= item then
      if item:getBuyingPrice_s64() > Defines.s64_const.s64_0 then
        text = text .. " : <PAColor0xFFEEEEEE>" .. PAGetStringParam1(Defines.StringSheet_GAME, "LUA_COMMON_MONEY", "value", tostring(makeDotMoney(item:getBuyingPrice_s64()))) .. "<PAOldColor>"
      else
