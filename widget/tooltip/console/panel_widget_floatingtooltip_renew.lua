...
local FloatingTooltip = {
  _ui = {
    stc_components = {
      ...
      [COMPONENT_TYPE.ITEM_PULLOUT] = UI.getChildControl(_panel, "StaticText_KeyGuidePullOut"),
      [COMPONENT_TYPE.ITEM_MOVE] = UI.getChildControl(_panel, "StaticText_KeyGuideMove")
    }
  },
  _defaultXGap = 8
}
...
local _targetData = {
  ...
  [Defines.TooltipTargetType.CombineBag] = {
    COMPONENT_TYPE.HEADER,
    COMPONENT_TYPE.WEIGHT_PRICE,
    COMPONENT_TYPE.ENDURANCE,
    COMPONENT_TYPE.KEY_GUIDE,
    COMPONENT_TYPE.ITEM_PULLOUT,
    COMPONENT_TYPE.ITEM_MOVE
  }
}
...
function FloatingTooltip:initialize()
  _componentOption = {
    ...
    [COMPONENT_TYPE.ITEM_PULLOUT] = {fillDataFunc = FloatingTooltip_updateKEY_GUIDE_FULLOUT},
    [COMPONENT_TYPE.ITEM_MOVE] = {fillDataFunc = FloatingTooltip_updateKEY_GUIDE_MOVE}
  }
...
function FloatingTooltip_updateWEIGHT_PRICE(itemWrapper, itemSSW, tooltipTargetType, slotType)
  ...
  if isTradeItem then
    if s64_originalPrice > Defines.s64_const.s64_0 and 0 == enchantLevel then
      if __ePriceType_Shell == itemSSW:getTradePriceType() then
        txt_price:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_COMMON_RAVENMONEY", "value", tostring(makeDotMoney(s64_originalPrice))))
      else
  ...
  elseif s64_sellPrice > Defines.s64_const.s64_0 and 0 == enchantLevel then
    if __ePriceType_Shell == itemSSW:getTradePriceType() then
      txt_price:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_COMMON_RAVENMONEY", "value", tostring(makeDotMoney(s64_sellPrice))))
    else
...
function FloatingTooltip_updateKEY_GUIDE_FULLOUT()
  return true
end
function FloatingTooltip_updateKEY_GUIDE_MOVE()
  return true
end
...
function PaGlobalFunc_FloatingTooltip_GetEnchantLevelStr(itemSSW)
  ...
  if CppEnums.ItemClassifyType.eItemClassify_Accessory == itemSSW:getItemClassify() and (itemSSW:isSpecialEnchantItem() == false or itemSSW:isKingAccessory() == true) then
    ...
    elseif 6 == enchantLevel then
      enchantStr = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_6")
      if true == _ContentsGroup_KoreanEnchantGrade then
        enchantStr = "\236\154\180"
      end
    elseif 7 == enchantLevel then
      enchantStr = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_7")
      if true == _ContentsGroup_KoreanEnchantGrade then
        enchantStr = "\236\154\176"
      end
    elseif 8 == enchantLevel then
      enchantStr = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_8")
      if true == _ContentsGroup_KoreanEnchantGrade then
        enchantStr = "\237\146\141"
      end
    elseif 9 == enchantLevel then
      enchantStr = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_9")
      if true == _ContentsGroup_KoreanEnchantGrade then
        enchantStr = "\235\139\168"
      end
    elseif 10 == enchantLevel then
      enchantStr = PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_10")
      if true == _ContentsGroup_KoreanEnchantGrade then
        enchantStr = "\237\153\152"
      end
    end
  end
  if itemSSW:get():isCash() then
