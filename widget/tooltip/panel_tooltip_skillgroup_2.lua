function Panel_SkillTooltip_Show(slotNo, isShowNextLevel, SlotType, isReserveSkill, isResetable)
  ...
  if nil ~= skillStatic then
    ...
    if nil ~= blackSkillTypeSS and blackSkillTypeSS:isValidLocalizing() then
      ...
      PaGlobal_TooltipSkill_All.TooltipSkillGroup_BlackSpirit.main:SetShow(false, false)
    end
  end
  skillStatic = getSkillStaticStatus(skillNo, 1)
  if nil ~= skillStatic then
  ...
  if PaGlobalFunc_SkillGroup_GetMouseOnSkillGroupNo ~= nil then
  ...
  for ii = 0, panelCount - 1 do
    ...
    if minPosX > posX then
      minPosX = posX
    end
    if maxPosX < posX + panel:GetSizeX() then
      maxPosX = posX + panel:GetSizeX()
    end
    panel:SetPosX(posX)
    panel:SetPosY(posY)
    prevPanel = panel
  ...
  if minPosX < 0 then
    local flowValueX = minPosX * -1
    for ii = 0, panelCount - 1 do
      local panel = panelList[ii]
      panel:SetPosX(panel:GetPosX() + flowValueX + 15)
    end
  end
  if screenSizeX < maxPosX then
    local flowValueX = maxPosX - screenSizeX
    for ii = 0, panelCount - 1 do
      local panel = panelList[ii]
      panel:SetPosX(panel:GetPosX() - flowValueX - 15)
    end
...
function HandleMOver_SkillWindow_ToolTipShow(skillNo, isShowNextLevel, SlotType, isFusion)
  if (true == _ContentsGroup_RenewUI or true == _ContentsGroup_UsePadSnapping) and nil ~= PaGlobal_SkillGroup._ui._stc_playShow and true == PaGlobal_SkillGroup._ui._stc_playShow:IsCheck() then
    return
  end
...
function SkillEvent_SkillGroupWindow_ClearSkill(skillPointType)
  if nil ~= PaGlobalFunc_SkillGroup_IsSearchState and true == PaGlobalFunc_SkillGroup_IsSearchState() then
    return
  end
...
function Panel_SkillTooltip_SetPosition_ForConsole(control, slotType)
  ...
  if "SelectTypeBox" ~= slotType then
    return
  end
  local panelCount = 0
  local panelList = {}
  ...
  local toolTipSize = {width = 0, hieght = 0}
  local posX = control:GetParentPosX()
