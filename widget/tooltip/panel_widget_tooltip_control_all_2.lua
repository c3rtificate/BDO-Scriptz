function PaGlobal_Tooltip_Control_All:setTooltipInfo(target, inputValue, isSSW, isItemWrapper, chattingLinkedItem, index, isNextEnchantInfo, invenSlotNo, itemNamingStr)
  ...
  if nil ~= target._ui.txt_FamilyBag then
    if false == _ContentsGroup_FamilyInventory or false == itemSSW:checkPushFamilyInventory() then
      target._ui.txt_FamilyBag:SetShow(false)
    else
      target._ui.txt_FamilyBag:SetShow(true)
    end
  end
  ...
  PAGlobalFunc_SetItemTextColor(target._ui.txt_itemName, itemSSW)
  ...
  if itemSSW:get():isItemInstallation() then
    ...
    if interiorPoint > 0 then
      ...
      if nil ~= isMansionItemKey and true == isMansionItemKey then
        target._ui.txt_isEnchantable:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_MANSION_INSTALLITEM_TOOLTIP", "point", interiorPoint))
      else
  ...
  if (_ContentsGroup_AlchemyStone or _ContentsGroup_AlchemyFigureHead or _ContentsGroup_QuestBook) and nil ~= itemWrapper and (32 == itemSSW:get():getContentsEventType() or 37 == itemSSW:get():getContentsEventType()) then
...
- ]] .. itemSSW:getEnchantDescription()
  ...
  if isTradeItem == true and not isSSW then
    if item:getBuyingPrice_s64() > _const.s64_0 then
      target._ui.txt_itemPrice_transportBuy_value:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_COMMON_MONEY", "value", tostring(makeDotMoney(item:getBuyingPrice_s64()))))
    else
  ...
  if isTradeItem then
    ...
    if s64_originalPrice > _const.s64_0 and 0 == enchantLevel then
      if __ePriceType_Shell == itemSSW:getTradePriceType() then
        target._ui.txt_itemPrice_storeSell_value:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_COMMON_RAVENMONEY", "value", tostring(makeDotMoney(s64_originalPrice))))
      else
        target._ui.txt_itemPrice_storeSell_value:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_COMMON_MONEY", "value", tostring(makeDotMoney(s64_originalPrice))))
      end
    ...
    if s64_sellPrice > _const.s64_0 and 0 == enchantLevel then
      if true == itemSSW:isTradePrice() then
        target._ui.txt_itemPrice_storeSell_value:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_COMMON_RAVENMONEY", "value", tostring(makeDotMoney(s64_sellPrice))))
      else
        target._ui.txt_itemPrice_storeSell_value:SetText(PAGetStringParam1(Defines.StringSheet_GAME, "LUA_COMMON_MONEY", "value", tostring(makeDotMoney(s64_sellPrice))))
      end
      target._ui.txt_itemPrice_storeSell_value:SetFontColor(4292726146)
    else
      target._ui.txt_itemPrice_storeSell_value:SetText(PAGetString(Defines.StringSheet_GAME, "PANEL_TOOLTIP_NOT_SELLING_ITEM"))
...
function PaGlobal_Tooltip_Control_All:setEnchantLevel(target, itemSSW)
  ...
  if CppEnums.ItemClassifyType.eItemClassify_Accessory == itemSSW:getItemClassify() and (itemSSW:isSpecialEnchantItem() == false or itemSSW:isKingAccessory() == true) then
    ...
    elseif 6 == enchantLevel then
      target._ui.txt_enchantLevel:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_6"))
      if true == _ContentsGroup_KoreanEnchantGrade then
        target._ui.txt_enchantLevel:SetText("\236\154\180")
      end
      target._ui.txt_enchantLevel:SetShow(true)
    elseif 7 == enchantLevel then
      target._ui.txt_enchantLevel:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_7"))
      if true == _ContentsGroup_KoreanEnchantGrade then
        target._ui.txt_enchantLevel:SetText("\236\154\176")
      end
      target._ui.txt_enchantLevel:SetShow(true)
    elseif 8 == enchantLevel then
      target._ui.txt_enchantLevel:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_8"))
      if true == _ContentsGroup_KoreanEnchantGrade then
        target._ui.txt_enchantLevel:SetText("\237\146\141")
      end
      target._ui.txt_enchantLevel:SetShow(true)
    elseif 9 == enchantLevel then
      target._ui.txt_enchantLevel:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_9"))
      if true == _ContentsGroup_KoreanEnchantGrade then
        target._ui.txt_enchantLevel:SetText("\235\139\168")
      end
      target._ui.txt_enchantLevel:SetShow(true)
    elseif 10 == enchantLevel then
      target._ui.txt_enchantLevel:SetText(PAGetString(Defines.StringSheet_GAME, "LUA_COMMON_ENCHANTLEVEL_10"))
      if true == _ContentsGroup_KoreanEnchantGrade then
        target._ui.txt_enchantLevel:SetText("\237\153\152")
      end
      target._ui.txt_enchantLevel:SetShow(true)
    end
  end
...
function PaGlobal_Tooltip_Control_All:setTooltipPos(target, item, itemSSW, itemWrapper, bottomTarget, changeItemNamePos, iconMovePos, elementgap, dynamicMaxEndurance, chattingLinkedItem, useLimitShow, soketCount)
  ...
  if nil ~= target._ui.txt_FamilyBag and true == target._ui.txt_FamilyBag:GetShow() then
    target._ui.txt_FamilyBag:SetPosY(TooltipYPos)
    TooltipYPos = self:getBottomPos(target._ui.txt_FamilyBag) + elementgap
  end
