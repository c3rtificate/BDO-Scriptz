function PaGlobal_Window_Stat_All:initialize()
  ...
  PaGlobal_Window_Stat_All._ui._txt_consoleKeyGuildB = UI.getChildControl(PaGlobal_Window_Stat_All._ui._txt_consoleKeyGuide, "StaticText_B_ConsoleUI")
  ...
  PaGlobal_Window_Stat_All._majorLimitPanel = UI.getChildControl(Panel_Worldmap_Territory, "Static_StatLimit")
  PaGlobal_Window_Stat_All._minorLimitPanel = UI.getChildControl(Panel_Worldmap_NodeTooltip_VillageWar_All, "Static_StatLimit")
  ...
  PaGlobal_Window_Stat_All._defaultDescSizeY = PaGlobal_Window_Stat_All._ui._txt_desc:GetSizeY()
  ...
  if true == _ContentsGroup_UsePadSnapping then
    ...
    showKeyGuideList:push_back(PaGlobal_Window_Stat_All._ui._txt_consoleKeyGuildB)
    PaGlobalFunc_ConsoleKeyGuide_SetAlign(showKeyGuideList, PaGlobal_Window_Stat_All._ui._txt_consoleKeyGuide, CONSOLEKEYGUID_ALIGN_TYPE.eALIGN_TYPE_RIGHTAUTOWRAP, nil, nil)
  else
...
function PaGlobal_Window_Stat_All:update(isCheck)
  if nil == Panel_Window_Stat_All then
    return
  end
  ...
  if PaGlobal_Window_Stat_All._majorLimitPanel ~= nil and PaGlobal_Window_Stat_All._majorLimitPanel:GetShow() == true then
    isUpdate = true
  end
  if PaGlobal_Window_Stat_All._minorLimitPanel ~= nil and PaGlobal_Window_Stat_All._minorLimitPanel:GetShow() == true then
    isUpdate = true
  end
  if isCheck == true and isUpdate == false then
    return
  end
  ...
  local awakenWeaponContentsOpen = false
  if nil ~= PaGlobalFunc_Util_GetIsAwakenWeaponContentsOpen then
    awakenWeaponContentsOpen = PaGlobalFunc_Util_GetIsAwakenWeaponContentsOpen(classtype)
  elseif nil ~= PaGlobal_Equipment_All then
    awakenWeaponContentsOpen = PaGlobal_Equipment_All._awakenWeaponContentsOpen
  end
  local isSetAwakenWeapon = ToClient_getEquipmentItem(__eEquipSlotNoAwakenWeapon)
  if true == awakenWeaponContentsOpen and nil ~= isSetAwakenWeapon then
  ...
  for ii = 1, PaGlobal_Window_Stat_All._INDEX_COUNT - 1 do
    ...
    elseif ii == self._INDEX_DDV then
    elseif ii == self._INDEX_DPV then
      if _ContentsGroup_MissRenew then
        tempValue = ToClient_GetAddedDefenceFromStat(ToClient_getDefence())
      end
    elseif ii == self._INDEX_RDV then
    elseif ii == self._INDEX_RPV then
      if _ContentsGroup_MissRenew then
