...
local isInstrument = _ContentsGroup_InstrumentShop
...
local npcTypeText = {
  ...
  [UI_ST.eSpawnType_SupplyShop] = PAGetString(Defines.StringSheet_GAME, "LUA_WIDGET_TOWNNPCNAVI_NPCTYPETEXT_39"),
  [UI_ST.eSpawnType_RandomShopDay] = PAGetString(Defines.StringSheet_GAME, "LUA_WIDGET_TOWNNPCNAVI_NPCTYPETEXT_34"),
  [UI_ST.eSpawnType_Instrument] = PAGetString(Defines.StringSheet_GAME, "LUA_WIDGET_TOWNNPCNAVI_NPCTYPETEXT_35")
}
local iconTexture = {
...
if false == _ContentsGroup_RemasterUI_Main_RightTop then
  if nil ~= Panel_PersonalIcon then
    ...
    toggleBtn:addInputEvent("Mouse_LUp", "PaGlobal_NpcNavi_All_ShowToggle()")
  end
else
