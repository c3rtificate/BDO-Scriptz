...
_text_Msg:SetTextMode(__eTextMode_AutoWrap)
Panel_NakMessage:setFlushAble(false)
function Proc_ShowMessage_Ack(message, showRate, posX, posY, isSoundOff, isBgShow)
  Proc_ShowMessage_Ack_WithOut_ChattingMessage(message, showRate, isSoundOff, isBgShow)
...
function Proc_ShowMessage_Ack_WithOut_ChattingMessage(message, showRate, isSoundOff, isBgShow)
  if PaGlobal_ReplayController_GetIsShowNak ~= nil and PaGlobal_ReplayController_GetIsShowNak() == false then
    return
  end
  ...
  local _darkBg = UI.getChildControl(Panel_NakMessage, "Static_DarkBg")
  if nil == isBgShow or false == isBgShow then
    _darkBg:SetShow(false)
  else
    _darkBg:SetShow(true)
  end
  curIndex = curIndex + 1
