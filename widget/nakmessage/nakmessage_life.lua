...
function PaGlobal_NakMessage_Life:showLifeNak_OverArtisan(lifeType, lifeGrade)
  ...
  local userName = selfPlayer:getUserNickname()
  ...
  local descStr = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_NAK_LIFELVUP_BASIC", "Name", userName, "lifetype", lifeTypeString, "grade", gradeStirng)
...
function PaGlobal_NakMessage_Life:showLifeNak_Basic(lifeType, lifeGrade)
  ...
  local userName = selfPlayer:getUserNickname()
  local lifeTypeString = PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_LIFE" .. tostring(lifeType))
  if lifeTypeString == nil or lifeTypeString == "" then
    return
  end
  local gradeStirng
  if lifeGrade == 4 and _ContentsOption_CH_GameType == true then
    gradeStirng = PAGetString(Defines.StringSheet_GAME, "LUA_LIFELEVEL_GROUP_" .. tostring(lifeGrade) .. "_CH")
  else
    gradeStirng = PAGetString(Defines.StringSheet_GAME, "LUA_LIFELEVEL_GROUP_" .. tostring(lifeGrade))
  end
  if gradeStirng == nil or gradeStirng == "" then
    return
  end
  local descStr = PAGetStringParam3(Defines.StringSheet_GAME, "LUA_NAK_LIFELVUP_BASIC", "Name", userName, "lifetype", lifeTypeString, "grade", gradeStirng)
