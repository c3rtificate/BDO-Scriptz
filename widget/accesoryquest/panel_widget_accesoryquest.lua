...
function AccesoryQuest:initialize()
  ...
  PaGlobalFunc_AccesoryQuest_UnRenderPanel()
end
...
function AccesoryQuest:update()
  if _ContentsGroup_InstanceSiege == true then
    local enterInstanceFieldKey = ToClient_GetEnterToInstanceFieldKey()
    if enterInstanceFieldKey:isDefined() == true and enterInstanceFieldKey:getType() == __eInstanceContentsType_Siege then
      self:close()
      return
    end
  end
...
function AccesoryQuest:resetPosition()
  ...
  if true == _panel:GetShow() then
    ...
    elseif nil ~= Panel_Radar and false == Panel_Radar:GetShow() then
      _panel:SetPosX(Panel_Radar:GetPosX() - Panel_Widget_AccesoryQuest:GetSizeX())
    end
    if true == self._isConsole then
...
function PaGlobalFunc_AccesoryQuest_UnRenderPanel()
  if nil == Panel_Widget_AccesoryQuest then
    return
  end
  local selfPlayer = getSelfPlayer()
  ...
  if true == bHide then
    Panel_Widget_AccesoryQuest:SetRenderAndEventHide(true)
  else
    Panel_Widget_AccesoryQuest:SetRenderAndEventHide(false)
  end
end
