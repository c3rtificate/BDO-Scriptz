...
function remoteInven:Init()
  for index = 0, self.maxSlotCount - 1 do
    local temp = {}
    temp.slotBg = UI.createControl(__ePAUIControl_Static, Panel_RemoteInventory, "Static_SlotBg_" .. index)
    ...
    temp.itemSlotBg = UI.createControl(__ePAUIControl_Static, temp.slotBg, "Static_ItemSlotBg_" .. index)
    CopyBaseProperty(tempSlot.itemSlotBg, temp.itemSlotBg)
    temp.itemName = UI.createControl(__ePAUIControl_StaticText, temp.slotBg, "StaticText_ItemName_" .. index)
    CopyBaseProperty(tempSlot.itemName, temp.itemName)
    temp.itemName:SetShow(false)
    temp.itemName:SetTextMode(__eTextMode_AutoWrap)
    temp.itemCount = UI.createControl(__ePAUIControl_StaticText, temp.slotBg, "StaticText_ItemCount_" .. index)
    CopyBaseProperty(tempSlot.itemCount, temp.itemCount)
    temp.itemCount:SetShow(false)
    temp.leftTime = UI.createControl(__ePAUIControl_StaticText, temp.slotBg, "StaticText_LeftTime_" .. index)
  ...
  for index = 0, self.maxSlotCount - 1 do
    ...
    if nil == itemWrapper then
      ...
      if itemSSW and not itemWrapper:getExpirationDate():isIndefinite() then
        ...
        local isFishItemInFishBag = itemWrapper:isFishItemInFishBag()
        local leftPeriod = FromClient_getTradeItemExpirationDate(itemExpiration, itemWrapper:getStaticStatus():get()._expirationPeriod, isNewTradeItem, isFishItemInFishBag)
...
function HandleClicked_InventorySlot(slotNo)
  ...
  if Panel_RemoteWarehouse:GetShow() then
    PaGlobal_Warehouse_All_PushFromInventoryItem(itemCount_s64, inventoryType, slotNo, actorKeyRaw)
    return
  end
  ...
  elseif not itemStatic:isUseToVehicle() then
    local function useTradeItem()
      if true == _ContentsGroup_NewUI_Inventory_All then
        PaGlobalFunc_Inventory_All_UseItemTargetSelf(inventoryType, slotNo)
      else
