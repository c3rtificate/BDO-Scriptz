loadUI("UI_Data/Widget/Buff/UI_BuffPanel.xml", "Panel_AppliedBuffList", Defines.UIGroup.PAGameUIGroup_MainUI, PAUIRenderModeBitSet({
...
function PaGlobalFunc_AppliedBuffList_ResetPosition()
  ...
  elseif relativePosX == 0 and relativePosY == 0 then
    ...
    local bias = 0.9
    local bposY = math.floor(getOriginScreenSizeY() * relativePosY - Panel_AppliedBuffList:GetSizeY() / 2 + bias)
    ...
    Panel_AppliedBuffList:SetPosY(bposY)
  end
  if true == _ContentsGroup_RenewUI and relativePosX <= 0 and relativePosY <= 0 then
...
function FromClient_StartSequencePlayerControlable_BuffList()
  Panel_AppliedBuffList:SetAlpha(0)
  local aniInfo = UIAni.AlphaAnimation(1, Panel_AppliedBuffList, 0, 0.8)
...
registerEvent("FromClient_StartSequencePlayerControlable", "FromClient_StartSequencePlayerControlable_BuffList")
