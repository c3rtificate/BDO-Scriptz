function PaGlobal_NpcNavi_All:initialize()
  ...
  local staticForPadSnapping = UI.getChildControl(Panel_NpcNavi_All, "Static_Bg5")
  self._ui._btn_FindItem = UI.getChildControl(staticForPadSnapping, "Button_FindItem")
  self._ui._btn_HuntingInfo = UI.getChildControl(staticForPadSnapping, "Button_HuntingInfo")
  ...
  if false == self._isConsole then
    ...
    if false == _ContentsGroup_Othilita then
...
function PaGlobal_NpcNavi_All:registEventHandler()
  ...
  self._ui._btn_FindItem:addInputEvent("Mouse_LUp", "HandleEventLUp_NpcNaviAll_OpenFindItem()")
  self._ui._btn_HuntingInfo:addInputEvent("Mouse_LUp", "HandleEventLUp_NpcNaviAll_OpenHuntingInfo()")
  self._ui._btn_FindItem:setButtonShortcuts("PANEL_WINDOW_TOTALSEARCH_TITLE")
  self._ui._btn_HuntingInfo:setButtonShortcuts("PANEL_DROPITEM_TITLE")
...
function PaGlobal_NpcNavi_All:getOverItemSpawnRegionData()
  ...
  local regionInfoList = PaGlobal_NpcNavi_All:findRegionKey(regionKey)
  local data
  for key, value in pairs(regionInfoList) do
    if data == nil then
      data = ToClient_getNpcInfoByRegionAndKey(value, npcKey)
    end
  end
  return data
end
...
function PaGlobal_NpcNavi_All:findRegionKey(regionKeyRaw)
  ...
  for index = 0, regionInfoCount - 1 do
    ...
    if regionWrapper:getAreaName() == regionInfo:getAreaName() then
      regionInfoList[ii] = regionInfo:getRegionKey()
      ii = ii + 1
    end
  end
...
function PaGlobal_NpcNavi_All:getCharacterString(npcData)
  ...
  if false == IsUseDynamicBSS() then
    if npcData:getTitle() == "" or npcData:getTitle() == nil then
      ...
      title = npcData:getTitle()
      inputString = "<PAColor0xffefefef>" .. title .. " " .. npcData:getName() .. " "
    end
  else
    title = getNpcTitle(npcData:getKeyRaw())
    if nil == title then
      title = ""
    end
  ...
  if nil ~= executeDialogData then
    local explorePointInfo = ToClient_getExplorePoint()
  ...
  if false == NpcConditionCheck(npcData:getKeyRaw()) then
    if title ~= "" then
      inputString = "<PAColor0xffbfbfbf>" .. title .. " " .. PAGetString(Defines.StringSheet_GAME, "LUA_NPCNAVI_UNKNOWN_NPC")
    else
...
local sortComparer = function(lhs, rhs)
  if lhs.territoryName == rhs.territoryName then
    ...
    return lhs.territoryKey < rhs.territoryKey
  end
end
function PaGlobal_NpcNavi_All:npcListUpdate()
  ...
  for index = 0, regionInfoCount - 1 do
    ...
    _npcNavi_TargetInfo.exploreKey = regionInfo:getExplorationKey()
  end
...
function PaGlobal_NpcNavi_All:newNpcListUpdate()
  ...
  for index = 0, regionInfoCount - 1 do
    ...
    if true == regionInfo:get():isAccessibleArea() then
      ...
      regionInfoList[addCount + 1].exploreKey = regionInfo:getExplorationKey()
      addCount = addCount + 1
    end
  end
  table.sort(regionInfoList, sortComparer)
...
function PaGlobal_NpcNavi_All:setNpcNaviBtn()
  ...
  for index = 1, #self._iconTextureData do
    local value = self._iconTextureData[index]
    if value ~= nil then
      local tempTable = {_btn = nil, _spawnType = nil}
      local showIcon = false
      if value.type == UI_ST.eSpawnType_TerritorySupply then
        showIcon = ToClient_IsContentsGroupOpen("22")
      elseif value.type == UI_ST.eSpawnType_TerritoryTrade then
        showIcon = ToClient_IsContentsGroupOpen("26")
      elseif value.type == UI_ST.eSpawnType_Instrument then
        showIcon = _ContentsGroup_InstrumentShop
      elseif value.type == UI_ST.eSpawnType_AbyssOneEnterPosGuide then
        showIcon = _ContentsGroup_ProjectAbyssOne == true and ToClient_IsLearnedKnowledge(11354) == true
      elseif value.type == UI_ST.eSpawnType_TraningVehicleShop then
        showIcon = ToClient_IsContentsGroupOpen("57")
      else
        showIcon = true
      end
      if showIcon == true then
        tempTable._spawnType = value.type
...
function PaGlobal_NpcNavi_All:getIconData(spawnType)
  if spawnType == nil then
    return nil
  end
  for key, value in pairs(self._iconTextureData) do
    if value ~= nil and value.type == spawnType then
      return value
    end
  end
  return nil
...
function PaGlobal_NpcNavi_All:setNpcBtnTexture(icon, spawnType)
  if icon == nil or spawnType == nil then
    return
  end
  local iconData = self:getIconData(spawnType)
  if iconData == nil then
    return
  end
  local textureDir = iconData.dds
  local iconSize = iconData.size
  local color = iconData.color
  icon:SetSize(iconSize, iconSize)
  icon:ComputePos()
  icon:SetColor(color)
  icon:ChangeTextureInfoNameAsync(textureDir)
  local x1, y1, x2, y2 = setTextureUV_Func(icon, iconData.x1, iconData.y1, iconData.x2, iconData.y2)
