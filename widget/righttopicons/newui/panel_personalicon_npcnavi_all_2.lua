...
function InputMLUp_NpcNavi_DrawLine(key)
  ...
  local title = spawnRegionData:getTitle()
  local hasTitle = title ~= nil and title ~= ""
  if hasTitle == true or NpcConditionCheck(spawnRegionData:getKeyRaw()) then
...
function HandleEventLUp_NpcNaviAll_NaviStart(spawnType, isAuto)
  ...
  if spawnType == CppEnums.SpawnType.eSpawnType_AbyssOneEnterPosGuide and ToClient_IsLearnedKnowledge(11354) == false then
    NotifyDisplay(PAGetString(Defines.StringSheet_GAME, "LUA_NAVI_MSG_SEARCHPORTAL"))
    return
  end
  local curChannelData = getCurrentChannelServerData()
  if curChannelData._isMain == false and spawnType == CppEnums.SpawnType.eSpawnType_TerritoryTrade then
...
function HandleEventLUp_NpcNaviAll_OpenFindItem()
  PaGlobalFunc_TotalSearch_Open()
end
function HandleEventLUp_NpcNaviAll_OpenHuntingInfo()
...
function HandleEventLUp_NpcNaviAll_TapSelectWithPCPad(isUp)
  ...
  elseif PaGlobal_NpcNavi_All._eTAB.BOOKMARK < PaGlobal_NpcNavi_All._currentTab then
    PaGlobal_NpcNavi_All._currentTab = PaGlobal_NpcNavi_All._eTAB.NPC
  end
...
function PaGlobalFunc_NpcNaviAll_ShowScrollButton()
  ...
  elseif true == _ContentsGroup_Othilita then
