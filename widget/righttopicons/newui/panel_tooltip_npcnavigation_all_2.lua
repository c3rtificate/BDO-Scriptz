...
function PaGlobal_NpcNaviTooltip_OverBarUpdate(isShow)
  ...
  local isUiMode = getInputMode() == CppEnums.EProcessorInputMode.eProcessorInputMode_UiMode
  ...
  if spawnRegionData == nil then
    self:prepareClose()
    return
  end
  if NpcConditionCheck(spawnRegionData:getKeyRaw()) == false then
    ...
    self._ui.txt_NpcName:SetSize(self._ui.txt_NpcName:GetTextSizeX(), self._ui.txt_NpcName:GetSizeY())
    if _npcNavi_Target ~= nil then
      for _, control in pairs(giftIcon) do
        control:SetShow(false)
      end
    end
    SizeX = self._ui.txt_NpcName:GetSizeX() + self._ui.txt_NpcName:GetSpanSize().x * 2
    ...
    if executeDialogData ~= nil then
      explorePointInfo = ToClient_getExplorePoint()
    end
