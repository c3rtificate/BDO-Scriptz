...
function PaGlobal_Widget_TownSimpleNavi_All:registEventHandler()
  ...
  registerEvent("FromClient_NewKnowledge", "FromClient_Widget_TownSimpleNavi_All_Refresh")
...
function PaGlobal_Widget_TownSimpleNavi_All:updateNpcNaviButton()
  ...
  for index = 1, CppEnums.SpawnType.eSpawnType_Count - 1 do
    if nil ~= self._iconTextureData[index] then
      ...
      elseif index == CppEnums.SpawnType.eSpawnType_AbyssOneEnterPosGuide then
        if _ContentsGroup_ProjectAbyssOne == true and ToClient_IsLearnedKnowledge(11354) == true then
          spawnType = index
        end
      else
        spawnType = index
