...
function ItemLog.new(index, bgControl, padding)
  local log = {
    ...
    itemEnchantKey = 0,
    slotNo = __eTInventorySlotNoUndefined,
    itemCount = 0,
    rowIndex = -1,
    remainShowingTime = -1,
    remainHidingTime = -1
  }
  log.titleControl:SetTextMode(__eTextMode_AutoWrap)
...
function ItemLogManager:initialize()
  ...
  local isShow = ToClient_GetUiInfo(CppEnums.PAGameUIType.PAGameUIPanel_ItemLogRenew, 0, CppEnums.PanelSaveType.PanelSaveType_IsShow)
  if isShow ~= -1 then
    if isShow == 0 then
      ItemLogManager._isShow = false
      ItemLogManager._panel:SetShow(false)
    else
      ItemLogManager._isShow = true
      ItemLogManager._panel:SetShow(true)
    end
  else
...
registerEvent("onScreenResize", "PaGlobalFunc_ItemLog_ResetPosition")
registerEvent("FromClient_ApplyUISettingPanelInfo", "PaGlobalFunc_ItemLog_ResetPosition")
...
function PaGlobalFunc_ItemLogHandleItemAdd(itemEnchantKey, slotNo, itemCount, whereType)
  if _ContentsGroup_RenewUI == false and ItemLogManager._isShow == false then
    return
  end
    ...
    if showingLogCount > 0 then
      ...
      for i = table.getn(self._showingLogs), 1, -1 do
        if UIindex > self._config._maxShowingLogSize then
          _PA_LOG("ItemLogRenew", "ItemLogRenew Showing Index Error!")
        else
...
function PaGlobalFunc_ItemLog_ResetPosition()
  local relativePosX = ToClient_GetUiInfo(CppEnums.PAGameUIType.PAGameUIPanel_ItemLogRenew, 0, CppEnums.PanelSaveType.PanelSaveType_RelativePositionX)
  local relativePosY = ToClient_GetUiInfo(CppEnums.PAGameUIType.PAGameUIPanel_ItemLogRenew, 0, CppEnums.PanelSaveType.PanelSaveType_RelativePositionY)
  local isShow = ToClient_GetUiInfo(CppEnums.PAGameUIType.PAGameUIPanel_ItemLogRenew, 0, CppEnums.PanelSaveType.PanelSaveType_IsShow)
  if isShow ~= -1 then
    if isShow == 0 then
      ItemLogManager._isShow = false
  ...
  if relativePosX == -1 and relativePosY == -1 then
    Panel_Widget_ItemLog_Renew:SetPosX(scrX * 0.68 - Panel_Widget_ItemLog_Renew:GetSizeX() / 2)
    local ratioY = 0.75
    if true == ToClient_isTotalGameClient() then
      ratioY = 0.68
    end
    Panel_Widget_ItemLog_Renew:SetPosY(scrY * ratioY - Panel_Widget_ItemLog_Renew:GetSizeY() / 2)
    Panel_Widget_ItemLog_Renew:ComputePos()
  elseif relativePosX == 0 and relativePosY == 0 then
    ...
    Panel_Widget_ItemLog_Renew:SetRelativePosX(relativePosX)
    Panel_Widget_ItemLog_Renew:SetRelativePosY(relativePosY)
    Panel_Widget_ItemLog_Renew:SetPosX(scrX * relativePosX - Panel_Widget_ItemLog_Renew:GetSizeX() / 2)
    Panel_Widget_ItemLog_Renew:SetPosY(scrY * relativePosY - Panel_Widget_ItemLog_Renew:GetSizeY() / 2)
    Panel_Widget_ItemLog_Renew:ComputePos()
  end
end
