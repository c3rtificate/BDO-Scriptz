...
local checkedQuestWidget = {
  _ui = {
    ...
    _static_Daily = UI.getChildControl(_static_active, "Static_Abyssone_Daily")
  },
  _config = {
    _giveupLimitLv = 10,
    _maxQuestListCnt = 30,
...
function checkedQuestWidget:initControl()
  ...
  self._ui._static_Daily:SetShow(false)
end
...
function checkedQuestWidget:checkPosition()
  if nil == Panel_Widget_MainQuest_AbyssOne or false == Panel_Widget_MainQuest_AbyssOne:GetShow() then
    return
  end
  local Rect1 = {}
  local Rect2 = {}
  Rect1.left = Panel_Widget_MainQuest_AbyssOne:GetPosX()
  Rect1.top = Panel_Widget_MainQuest_AbyssOne:GetPosY()
  Rect1.right = Rect1.left + Panel_Widget_MainQuest_AbyssOne:GetSizeX()
  Rect1.bottom = Rect1.top + Panel_Widget_MainQuest_AbyssOne:GetSizeY()
...
function checkedQuestWidget:groupQuestInfo(questGroupInfo, questGroupIndex, questType)
  ...
  for questIndex = 0, questGroupInfo:getQuestCount() - 1 do
    local uiQuestInfo = questGroupInfo:getQuestAt(questIndex)
    if uiQuestInfo:getQuestRegion() == CppEnums.QuestRegionType.eQuestRegionType_AbyssOne then
...
function checkedQuestWidget:questInfo(uiQuestInfo, isSingle, questGroupIndex, questIndex, groupTitle, questGroupCount, questType, isUpdate)
  if false == uiQuestInfo._isCleared and true == uiQuestInfo._isProgressing and uiQuestInfo:getQuestRegion() == CppEnums.QuestRegionType.eQuestRegionType_AbyssOne then
...
function checkedQuestWidget:setVisibleConvenienceButtonByIndex(index, show)
  ...
  if true == show then
    if self._config._questType._guild == uiElem._questType then
      ...
      if not (selfLevel > self._config._giveupLimitLv) or self._config._questType._normal == uiElem._questType then
      end
...
function checkedQuestWidget:setNaviButtonInfo(questUi, isSatisfied, isCompleteBlackSpirit)
  ...
  if self._questGroupId ~= questGroupId or self._questId ~= questId or true == self._naviInfoAgain then
...
function FGlobal_QuestWidget_CloseFunc()
  if ToClient_WorldMapIsShow() == true then
    Panel_CheckedQuest:SetShow(false, true)
...
function FGlobal_QuestWidget_OpenSeparate()
  ...
  Panel_Widget_MainQuest_AbyssOne:SetShow(mainQuestShow, false)
  if ToClient_WorldMapIsShow() then
    WorldMapPopupManager:increaseLayer(true)
...
function FGlobal_QuestWidget_Close()
  Panel_CheckedQuest:SetShow(false, false)
  Panel_Widget_MainQuest_AbyssOne:SetShow(false, false)
...
function checkedQuestWidget:findTarget(questGroupId, questId, condition, isAuto)
  ...
  if uiQuestInfo == nil then
    ...
    return
  elseif uiQuestInfo:getQuestRegion() ~= CppEnums.QuestRegionType.eQuestRegionType_AbyssOne then
    Proc_ShowMessage_Ack(PAGetString(Defines.StringSheet_GAME, "LUA_PROCMESSAGE_QUEST_NO_FINDWAY"))
    return
  end
...
function FromClient_questWidget_ResetPosition()
  ...
  elseif Panel_CheckedQuest:GetRelativePosX() == 0 and Panel_CheckedQuest:GetRelativePosY() == 0 then
    Panel_CheckedQuest:SetPosX(getScreenSizeX() - Panel_CheckedQuest:GetSizeX() - 16)
    Panel_CheckedQuest:SetPosY(PaGlobalFunc_AbyssOneInfo_GetPosY() + PaGlobalFunc_AbyssOneInfo_GetSizeY() + Panel_Widget_MainQuest_AbyssOne:GetSizeY() + 20 + newEquipGap)
  else
    checkedQuestWidget._updateTime = -1
    checkedQuestWidget:updateQuestSetPos()
  end
  Panel_CheckedQuest:SetShow(true)
  PaGlobalFunc_Quest_UpdatePosition()
...
function PaGlobal_CheckedQuest_UpdateBackSpaceQuest()
  ...
  if nil ~= Panel_Widget_MainQuest_AbyssOne and true == Panel_Widget_MainQuest_AbyssOne:GetShow() and nil ~= PaGlobalFunc_MainQuestWidget_GetMainQuestNo then
