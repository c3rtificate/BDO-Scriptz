...
function checkedQuestWidget:update(groupNo, questNo, isGuildQuest)
  ...
  if PaGlobal_SummonBossTutorial_Manager ~= nil then
    PaGlobal_SummonBossTutorial_Manager:checkQuestCondition()
  end
  if Panel_LifeTutorial ~= nil and Panel_LifeTutorial:GetShow() == true then
...
function checkedQuestPanel_Init()
  ...
  local newEquipGap = 0
  local posY = 0
  if Panel_NewEquip ~= nil and Panel_NewEquip:GetShow() == true then
    newEquipGap = Panel_NewEquip:GetSizeY() + 15
  end
  Panel_CheckedQuest:SetPosX(getScreenSizeX() - Panel_CheckedQuest:GetSizeX() - 20)
  Panel_CheckedQuest:SetPosY(PaGlobalFunc_AbyssOneInfo_GetPosY() + PaGlobalFunc_AbyssOneInfo_GetSizeY() + 10 + newEquipGap)
...
function checkedQuestWidget:setQuestDataList(groupNo, questNo, isGuildQuest)
  ...
  if nil == groupNo or nil == questNo then
    ...
    self:setLatestQuest()
    local uiQuestInfo = ToClient_GetQuestInfo(groupNo, questNo)
    if nil ~= uiQuestInfo and false == uiQuestInfo:isSatisfied() then
      self:setTargetQuest(uiQuestInfo)
    else
      self._normalDataList = {}
      self:setNormalQuest()
    end
...
function checkedQuestWidget:setGuildQuest()
  ...
  if PaGlobal_TutorialManager ~= nil and PaGlobal_TutorialManager:isDoingTutorial() then
    return
  end
  local questData = {}
...
function checkedQuestWidget:questWidget_MouseOver(show)
  local widgetType = CppEnums.QuestWidgetType.eQuestWidgetType_Simple
  if true == show then
...
function FGlobal_QuestWidget_AutoReleaseNavi(uiQuestInfo)
  local questNo = uiQuestInfo:getQuestNo()
  if nil == checkedQuestWidget._positionList then
    return
  end
  if questNo._group == checkedQuestWidget._positionList._questGroupId and questNo._quest == checkedQuestWidget._positionList._questId then
    checkedQuestWidget._positionList = {}
    ...
    for xx = x1, x2, 10 do
      for yy = y1, y2, 10 do
        if xx >= _x1 and xx <= _x2 and yy >= _y1 and yy <= _y2 then
          Panel_CheckedQuest:SetPosY(PaGlobalFunc_AbyssOneInfo_GetPosY() + PaGlobalFunc_AbyssOneInfo_GetSizeY() + 65 + newEquipGap)
        end
      end
    end
  end
...
function FromClient_questWidget_ResetPosition()
  local newEquipGap = 0
  if Panel_NewEquip ~= nil and true == Panel_NewEquip:GetShow() then
    newEquipGap = Panel_NewEquip:GetSizeY()
    local _x1 = Panel_NewEquip:GetPosX()
    local _x2 = Panel_NewEquip:GetPosX() + Panel_NewEquip:GetSizeX()
    local _y1 = Panel_NewEquip:GetPosY()
  ...
  if Panel_CheckedQuest:GetRelativePosX() == -1 and Panel_CheckedQuest:GetRelativePosY() == -1 then
    ...
    local initPosY = PaGlobalFunc_AbyssOneInfo_GetPosY() + PaGlobalFunc_AbyssOneInfo_GetSizeY() + 130 + newEquipGap
  ...
  elseif Panel_CheckedQuest:GetRelativePosX() == 0 and Panel_CheckedQuest:GetRelativePosY() == 0 then
    Panel_CheckedQuest:SetPosX(getScreenSizeX() - Panel_CheckedQuest:GetSizeX() - 16)
    Panel_CheckedQuest:SetPosY(PaGlobalFunc_AbyssOneInfo_GetPosY() + PaGlobalFunc_AbyssOneInfo_GetSizeY() + Panel_MainQuest:GetSizeY() + 20 + newEquipGap)
  else
