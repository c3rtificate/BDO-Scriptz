...
Panel_QuickSlot:SetPosY(getScreenSizeY() - Panel_QuickSlot:GetSizeY() - 10)
...
function QuickSlot_DropHandler(slotIndex)
  ...
  elseif DragManager.dragStartPanel == Panel_Window_SkillGroup then
    quickSlot_RegistSkill(slotIndex, DragManager.dragSlotInfo)
    if PaGlobal_SkillGroup_QuickSlot ~= nil then
...
function QuickSlot_Click(slotIndex)
  ...
  if itemSlot ~= nil then
    if itemSlot.item ~= nil and itemSlot.item.cooltime:GetShow() == false then
    ...
    if itemSlot.skill ~= nil and itemSlot.skill.cooltime:GetShow() == false then
      audioPostEvent_SystemUi(8, 2)
      _AudioPostEvent_SystemUiForXBOX(8, 2)
      ...
      if skillStaticWrapper:getUiDisplayType() == true then
        SpiritGuide_Show()
      end
    end
  end
  if QuickSlot_DropHandler(slotIndex) == false then
    local whereType
    if itemSlot.item ~= nil then
      whereType = QuickSlot_GetInventoryTypeFrom(quickSlotInfo._type)
    ...
    if _ContentsGroup_UISkillGroupTreeLayOut == true and Panel_Window_SkillGroup_Controller ~= nil and Panel_Window_SkillGroup_Controller:GetShow() == true then
      if itemSlot.skill ~= nil then
        PaGlobalFunc_SkillGroup_SkillAction(itemSlot.skill.skillNo)
      end
    elseif itemSlot.item ~= nil then
      if isNearFusionCore() == true and isFusionItem(whereType, itemSlot.keyValue) == true then
...
function QuickSlot_DragStart(slotIndex)
  ...
  local quickSlotInfo = getQuickSlotItem(slotIndex)
  if PaGlobal_TutorialManager ~= nil and PaGlobal_TutorialManager:isDoingTutorial() == true and 502 == quickSlotInfo._itemKey:get() and 0 == slotIndex then
...
function QuickSlot_UpdatePerFrame(fDeltaTime)
  ...
  for idx, slot in ipairs(self.slots) do
    ...
    if PaGlobal_TutorialManager ~= nil and PaGlobal_TutorialManager:isDoingTutorial() == true and PaGlobal_TutorialManager:isCheckHpPotionSetting() == false and 1 == idx and 517 ~= quickSlotInfo._itemKey:getItemKey() then
  ...
  if nil ~= itemWrapper and (itemWrapper:getStaticStatus():get():isItemSkill() or itemWrapper:getStaticStatus():get():isUseToVehicle()) then
    if itemType == 0 then
      if isUseNewQuickSlot() then
        ...
        if PaGlobal_TutorialManager ~= nil and PaGlobal_TutorialManager:isDoingTutorial() == true then
          HP_Push_NextSlot(0)
...
function FGlobal_QuickSlot_CheckDefaultQuickSlotShowAble()
  if true == isUseNewQuickSlot() then
    return false
  elseif false == _ContentsGroup_Tutorial_Renewal and PaGlobal_TutorialManager ~= nil and PaGlobal_TutorialManager:isAllowShowQuickSlot() == false then
    return false
  end
  return true
end
function QuickSlot_UpdateData()
  ...
  if _ContentsGroup_UISkillGroupTreeLayOut == true and PaGlobal_SkillGroup_QuickSlot ~= nil then
    PaGlobal_SkillGroup_QuickSlot:update()
  end
end
