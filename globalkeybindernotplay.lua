...
function GlobalKeyBinder_UpdateNotPlay(deltaTime)
  ...
  if nil ~= Panel_Login and Panel_Login:GetShow() or nil ~= Panel_Login_Renew and Panel_Login_Renew:GetShow() or nil ~= Panel_Login_Remaster and Panel_Login_Remaster:GetShow() or nil ~= Panel_Lobby_Login_All and Panel_Lobby_Login_All:GetShow() then
    ...
    elseif Panel_Window_Common_Description ~= nil and Panel_Window_Common_Description:GetShow() == true then
      if GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_ESCAPE) then
        PaGlobalFunc_Common_Description_Close()
      end
    ...
    elseif GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_RETURN) then
      if false == _ContentsGroup_UsePadSnapping and nil ~= PaGlobal_Login_All._ui._btn_login and true == PaGlobal_Login_All._ui._btn_login:IsEnable() then
        if Panel_Addiction_Prevention_All ~= nil then
          if Panel_Addiction_Prevention_All:GetShow() == false then
  ...
  if (true == _ContentsGroup_SeasonContents or true == _ContentsGroup_PreSeason) and nil ~= Panel_Window_SeasonBanner and true == Panel_Window_SeasonBanner:GetShow() then
    if false == _ContentsGroup_UsePadSnapping then
      if GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_ESCAPE) or GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_RETURN) or GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_SPACE) then
  ...
  if nil ~= Panel_CustomizationMain then
    if GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_ESCAPE) and true == characterCreateIsOnLookAtCameraTimerRunning() then
      characterCreateCancelIntroAction()
      return
    end
    if true == Panel_CustomizationMain:GetShow() then
      if GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_RETURN) then
        PaGlobal_EnterInputKey()
        return
      end
      if nil ~= uiEdit and GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_ESCAPE) then
        ClearFocusEdit()
        return
      end
    end
  end
  if true == _ContentsGroup_SelectStartPosition and nil ~= Panel_Window_StartBranch and true == Panel_Window_StartBranch:GetShow() and GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_ESCAPE) then
    HandleEventLUp_StartBranch_All_Cancel()
    return
  end
  if nil ~= Panel_CharacterSelect_All and true == Panel_CharacterSelect_All:GetShow() and GlobalKeyBinder_CheckKeyPressed(VCK.KeyCode_ESCAPE) then
    if nil ~= Panel_Window_Season_Character_Create and true == Panel_Window_Season_Character_Create:GetShow() then
