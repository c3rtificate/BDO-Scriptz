...
function workerWrapper:getWorkerUpgradeNowRate()
  if nil == self._workerWrapper then
    return 0
  end
  return self._workerWrapper:getWorkerUpgradeNowRate()
end
...
function workerWrapper:getOriginName()
  self:reflesh()
  return self._workerWrapper:getName()
end
function workerWrapper:getName()
  self:reflesh()
  local nickName = self._workerWrapper:getNickName()
  if nil ~= nickName then
    return nickName
  end
  return self._workerWrapper:getName()
end
function workerWrapper:getNickName()
  self:reflesh()
  return self._workerWrapper:getNickName()
end
function workerWrapper:setNickName(nickName)
  self:reflesh()
  return self._workerWrapper:setNickName(nickName)
end
function workerWrapper:clearNickName()
  self:reflesh()
  return self._workerWrapper:clearNickName()
end
...
function workerWrapper:getWorkerType()
  self:reflesh()
  return self._workerWrapper:get():getWorkerStaticStatus()._workerType
end
function workerWrapper:getStorageRegionName()
  self:reflesh()
  if self:isWorking() == false then
    return ""
  end
  if self._workingWrapper:isType(CppEnums.NpcWorkingType.eNpcWorkingType_PlantZone) == false then
    return ""
  end
  local esSSW = ToClient_getItemExchangeSourceStaticStatusWrapperByWorker(self._workerNoRaw)
  if esSSW == nil then
  ...
  return self._workingWrapper:getWorkingStorageTownName()
end
