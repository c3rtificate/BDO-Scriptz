...
function PaGlobalFunc_GetServantBriefSpec(testCase, param1)
  ...
  elseif testCase >= TestCaseEnum.eMatingCancle then
    ...
    if nil ~= myAuctionInfo then
      local index = param1 + myAuctionInfo:getCurrentPage() * PaGlobal_ServantMarket_All._MAXSLOTCOUNT
      servantInfo = myAuctionInfo:getServantAuctionListAt(index)
    end
  else
...
function servantSupport:prepareTest(TestListFunc)
  ...
  PaGlobal_ServantList_All:loadServantList(true)
...
function servantSupport:servantExchangeTest(stableSlotNo1, stableSlotNo2, fromWhereType, name)
  ...
  return stable_mix(servantNo1, servantNo2, CppEnums.ItemWhereType.eMoneyInventory, name)
end
...
function PaGlobalFunc_useExpansionItemForTest(servantItemKey, name)
  ...
  inventoryUseItem(CppEnums.ItemWhereType.eCashInventory, slotNo, __eEquipSlotNoCount, true)
  return 0
end
