...
function AutoState_DialogInteraction:checkChangeState()
  ...
  if false == _isNextBtnShow and true == newMainQuest then
    for i = 0, nDlgButtoncount - 1 do
      ...
      if nil ~= dialogButtonLink and nil ~= questStaticStatus and questStaticStatus:getQuestType() == __eQuestType_BlackSpirit and false == ToClient_isProgressingQuest(questStaticStatus:getQuestNo()._group, questStaticStatus:getQuestNo()._quest) then
